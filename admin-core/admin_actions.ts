...
// module's exports. That is the arrangement `tests/admin_actions_layer.js`
// asserts, and the tables the views share stay module-level data beside them.
// ---------------------------------------------------------------------------
...
import refreshTokenCrypto = require('../oauth-oidc/refresh_token_crypto');
...
class AdminActions {
  ...
  private refused(code, result) {
    ...
    log.debug("Entering AdminActions.refused().");
    log.debug("Leaving AdminActions.refused().");
    return errorCodes.mark(result, code);
  }

  // The code a domain module already put on its own result, in either of the
  // two ways one does: the non-enumerable mark, or an `errorCode` member on a
  // result that is internal to this layer and never sent (the truststore's is).
  private innerCode(result) {
    const { log, errorCodes } = this.deps;
    log.debug("Entering AdminActions.innerCode().");
    if (!result || typeof result !== 'object') {
      log.debug("Leaving AdminActions.innerCode().");
      return '';
    }
    log.debug("Leaving AdminActions.innerCode().");
  ...
  // A result handed up from the domain module that owns the change. Its own
  // code wins, because the owner knows the specific condition; this layer's
  // names only the door it came through, and is used only when the owner gave
  // none.
  private refusedBy(code, result) {
    const { log, errorCodes } = this.deps;
    log.debug("Entering AdminActions.refusedBy().");
    if (result && result.ok === false) {
      errorCodes.mark(result, this.innerCode(result) || code);
    }
    log.debug("Leaving AdminActions.refusedBy().");
    return result;
  }
  ...
  setTruststore(value) {
    ...
    truststore = value;
    log.debug("Leaving AdminActions.setTruststore().");
  }
  ...
  // sloppy: the only thing read out of it is the jti, which is then looked up
  // in this service's own registry. A forged token yields a jti this service
  // never issued, and revoking a jti that was never issued invalidates nothing.
  ...
  jtiFrom(target) {
    ...
    if (!text) {
      log.debug("Leaving AdminActions.jtiFrom(). Nothing was given.");
    ...
    if (parts.length === 5) {
      ...
      if (opened && opened.jti) {
        log.debug("Leaving AdminActions.jtiFrom(). Read the jti out of an " +
                  "encrypted refresh token.");
        return { jti: String(opened.jti), claims: opened,
                 how:
                   'read out of an encrypted refresh token after decrypting ' +
                   'it' };
      }
    }
    if (parts.length !== 3) {
      log.debug("Leaving AdminActions.jtiFrom(). Treating it as a jti.");
      return { jti: text, how: 'read as a jti' };
    }
    try {
      const claims = JSON.parse(b64uDecode(parts[1]).toString('utf8'));
      log.debug("Leaving AdminActions.jtiFrom(). Read the jti out of a JWT.");
      ...
      log.debug("Leaving AdminActions.jtiFrom(). It has three parts but is " +
                "not a JWT: " +
                e.message);
  ...
  tokenAction(body) {
    ...
    if (action === 'revoke' || action === 'restore') {
      const found = this.jtiFrom(body.target || body.jti || body.token);
      if (!found.jti) {
        log.debug("Leaving AdminActions.tokenAction(). No jti was given.");
        return this.refused('STS-ADMIN-0502', { ok: false, errors: ['Give a ' +
                                     'jti, or paste the whole token and the ' +
                                          'jti will be read out of it.'] });
      }
      if (action === 'restore') {
        const was = stats.restore(found.jti);
        log.debug("Leaving AdminActions.tokenAction(). Restored.");
        return { ok: true, jti: found.jti,
                 message: was ? 'The token with jti ' + found.jti + ' is no ' +
                                'longer revoked (NON-SPEC — a real ' +
                                'authorization server cannot undo a ' +
                                'revocation).'
    ...
    // ONE ASSERTION, TICKET OR SVID — this service's own position on a
    // credential it cannot recall.
    //
    // **IT CHANGES NOTHING OUT THERE AND THE MESSAGE SAYS SO, EVERY TIME.**
    // That sentence is the whole reason this action can exist without being a
    // lie: a relying party validates a SAML assertion's signature and its
    // Conditions and asks nobody; a Kerberos service decrypts a ticket with a
    // key it already has; an X509-SVID chains to a bundle. None of them will
    // ever ask this service, so the credential goes on working until it
    // expires.
    //
    ...
    //
    // Addressed by `artifact` (the row handle) rather than by the protocol's
    // own identifier, because a Kerberos ticket has none at all.
    if (action === 'revoke-artifact' || action === 'restore-artifact') {
      const handle = String(body.artifact || body.key || '').trim();
      if (!handle) {
        log.debug("Leaving AdminActions.tokenAction(). No artifact was named.");
        return this.refused('STS-ADMIN-0503', { ok: false, errors: ['Name ' +
                                     'the credential in `artifact`, as the ' +
                                     '`key` on every row of GET ' +
                                     '/admin-api/tokens gives it. It is this ' +
                                     'service\'s own handle rather than the ' +
                                     'protocol\'s, because a Kerberos ticket ' +
                                          'carries no identifier anybody can ' +
                                          'quote.'] });
      }
      const record = stats.artifactByKey(handle);
      if (!record) {
        log.debug("Leaving AdminActions.tokenAction(). No such artifact.");
        return this.refused('STS-ADMIN-0504', { ok: false, errors: ['Nothing ' +
            'here is called "' + handle + '" ' +
                                          'any more. Either it was never a ' +
                                     'credential in this register, or it has ' +
                                     'been forgotten to the cap since the ' +
                                     'page naming it was drawn — at which ' +
                                     'point this service has no position on ' +
                                          'it left to state.'] });
      }
      const notReached = ' THE HOLDER HAS NOT BEEN TOLD and cannot be by ' +
        'this action: nothing consults this service when ' +
        ...
        'subscribed, and that SAML Single Logout can carry it for an ' +
        'assertion issued through a browser profile.';
      if (action === 'restore-artifact') {
        const was = stats.restoreArtifact(record);
        log.debug("Leaving AdminActions.tokenAction(). Restored an artifact.");
      ...
      log.debug("Leaving AdminActions.tokenAction(). Revoked an artifact.");
      return { ok: true, artifact: handle, kind: record.kind,
    ...
    // would send it, into the same set of revoked jtis RFC 7009's
    // /oauth2/revoke writes to. What it saves is three clicks and, more to the
    // point, the mistake of revoking two of the three and believing the grant
    // is dead — a refresh token left behind mints a new access token, which is
    // the whole reason a set is worth being a row.
    //
    // THE MEMBERS ARE RE-READ HERE and never taken from the form, which is the
    // same rule terminate() follows and for the same reason: a page can be
    // posted an hour after it was drawn, and acting on the list it drew would
    // revoke a jti that has since been forgotten to the cap while missing one
    // issued since. The form carries the set KEY and nothing else.
    if (action === 'revoke-set' || action === 'restore-set') {
      const setKey = String(body.set || body.setKey || '').trim();
      if (!setKey) {
        log.debug("Leaving AdminActions.tokenAction(). No set was given.");
        return this.refused('STS-ADMIN-0505', { ok: false, errors: ['Give a ' +
                                          'set, as the `setKey` on every row ' +
                                          'of GET /admin-api/tokens names ' +
                                          'it.'] });
      }
      const set = stats.issuedSetByKey(setKey);
      if (!set) {
        log.debug("Leaving AdminActions.tokenAction(). No such set.");
        return this.refused('STS-ADMIN-0506', { ok: false, errors: ['Nothing ' +
            'here is called "' + setKey + '" ' +
                                     'any more. Either it was never a set, ' +
                                     'or it has been forgotten to the cap ' +
                                     'since the page this button is on was ' +
                                          'drawn — the registry holds at ' +
                                          'most ' +
      ...
      if (!revocable.length) {
        // A REFUSAL RATHER THAN A SUCCESS THAT DID NOTHING. It is a narrower
        // case than it was before 2026-09-05, when it covered every assertion,
        // ticket and SVID: those are revocable now — in this service's own
        // record, which is a different claim from being honoured and is why
        // `revocationReach` exists. What is left here is a set holding only
        // credentials with NO HANDLE TO ACT ON, which is the signed UserInfo
        // response and the OID4VP Request Object: no jti, nothing to name.
        log.debug("Leaving AdminActions.tokenAction(). Nothing in the set " +
                  "can be revoked.");
        return this.refused('STS-ADMIN-0507', { ok: false, setKey: setKey,
                                                errors: [
          'Nothing in this set can be revoked. It holds ' +
          set.kinds.join(', ') +
          ', and none of those carries an identifier to act on — a signed ' +
          'UserInfo response has no jti, and the WS-Trust JWT is signed ' +
          'directly rather than through signJwt(). There is nothing to name ' +
          'in a revocation.'] });
      }
        ...
        if (member.revocationReach === 'record-only') {
          ...
          moved = action === 'restore-set'
            ...
            : stats.revokeArtifact(record,
                                   'the admin console (the whole set ' +
                                   setKey +
                                   ')');
        } else {
          reached += 1;
      ...
      log.debug("Leaving AdminActions.tokenAction(). " + verb + " " + changed +
                " of " +
      ...
      return { ok: true, setKey: setKey, set: setKey,
               ...
               // two numbers rather than folded into one: the caller is
               // entitled to know that three of the four credentials it just
               // revoked go on working, and a single count would have hidden
               // it.
               reachedProtocol: reached, recordOnly: recordOnly,
               kinds: revocable.map(function (member) { return member.kind; }),
               message: (changed
                 ? (action === 'restore-set' ? 'Restored ' : 'Revoked ') +
                 changed
                 : 'Nothing changed: all ' + revocable.length +
                 ' were already ' +
                   verb) +
                 ' of the ' + revocable.length + ' revocable credential(s) ' +
                                                 'in this set (' +
                 ...
                 (recordOnly
                   ? ' ' + recordOnly + ' of them was marked IN THIS ' +
                     ...
                     'when an assertion, a ticket or an SVID is presented, ' +
                     'so those go on working out there until they expire.'
                   : '') };
    }
    ...
    if (action === 'revoke-kind') {
      ...
      if (stats.REVOCABLE_KINDS.indexOf(kind) < 0) {
        log.debug("Leaving AdminActions.tokenAction(). Not a revocable kind.");
        return this.refused('STS-ADMIN-0508', { ok: false, errors: ['"' + kind +
                                                                    '" ' +
    ...
    if (action === 'revoke-subject') {
      ...
      if (!subject) {
        log.debug("Leaving AdminActions.tokenAction(). No subject was given.");
        return this.refused('STS-ADMIN-0509', { ok: false, errors: ['Give a ' +
            'subject or a username.'] });
      }
    ...
    // The users page's button. It is not the same thing as revoke-subject
    // above, and the difference is the reason it exists: that one matches a
    // `sub` or a `username` EXACTLY, which is what somebody typing into the box
    // on the tokens page means, while a user on the users page is an identity
    // that has been seen under several spellings — `alice`,
    // `urn:uuid:<entryUUID>`, `alice@STS.MOCK`. Revoking "for alice" from that
    // page has to mean all of them, or the page would offer a button that
    // visibly missed half of its own table.
    if (action === 'revoke-user') {
      const key = String(body.user || '').trim();
      if (!key) {
        log.debug("Leaving AdminActions.tokenAction(). No user was given.");
        return this.refused('STS-ADMIN-0509', { ok: false, errors: ['Give a ' +
            'user, as the users page names them.'] });
      }
      const count = stats.revokeWhere(function (record) {
      ...
      return { ok: true, revoked: count, user: key,
               message: 'Revoked ' + count + ' token(s) for ' + key + ' — ' +
                        'every spelling of that identity, not just the one ' +
                        'the row showed.' };
    }
    ...
    if (action === 'revoke-all') {
      ...
      log.debug("Leaving AdminActions.tokenAction(). Revoked everything: " +
                count + ".");
      return { ok: true, revoked: count,
               message: 'Revoked ' + count + ' token(s) — every access ' +
                        'token, ID Token and refresh token this service has ' +
                        'issued and still remembers.' };
    }

    log.debug("Leaving AdminActions.tokenAction(). Unknown action.");
  ...
  // re-collects from every family, ends what was named, writes the one audit
  // row for the act and reports what it could not end and why — all of which a
  ...
  sessionsAction(body, opts) {
    ...
    if (!logoutReader) {
      log.debug("Leaving AdminActions.sessionsAction(). No logout reader.");
      return this.refused('STS-ADMIN-0501', { ok: false, errors: [
    ...
    if (action !== 'revoke') {
      log.debug("Leaving AdminActions.sessionsAction(). Unknown action.");
      return this.refused('STS-ADMIN-0500',
                          { ok: false, errors: ['Unknown action "' + action +
        '". There is one: revoke.'] });
    }
    ...
    if (!key || !selected) {
      log.debug("Leaving AdminActions.sessionsAction(). Nothing named.");
      return this.refused('STS-ADMIN-0510', { ok: false, errors: [
        'Both "key" (whose session it is) and "select" (the session id, in ' +
        'the form the sessions list gives it — "session:…", "ldap:…" or ' +
    ...
    const said = []
      ...
      .concat((result.skipped || [])
        .map(function (one) { return one.message; }))
      .join(' ');
    if (done) {
      log.debug("Leaving AdminActions.sessionsAction(). Ended.");
      return { ok: true, message: said || 'the session was ended',
               result: result };
    }
    log.debug("Leaving AdminActions.sessionsAction(). Nothing was ended.");
    return this.refused(this.innerCode(result) || 'STS-ADMIN-0511',
                        { ok: false, result: result, errors: [
      said || (unknown
        ? 'Nothing here is called "' + selected + '" any more. It had ' +
          'already ended, or the list this button came from was drawn before ' +
          'it did.'
        : 'Nothing was ended.') ] });
  }
  ...
  logoutAction(body) {
    ...
    if (!logoutReader) {
      log.debug("Leaving AdminActions.logoutAction(). No logout reader is " +
                "installed.");
      return this.refused('STS-ADMIN-0501', { ok: false, errors: ['The ' +
                                   'logout module is not loaded in this ' +
                                        'process, so there is nothing to ' +
                                        'end. ' +
                                        'See /admin/logout, which says ' +
                                        'why.'] });
    }
    if (!user) {
      log.debug("Leaving AdminActions.logoutAction(). No user was named.");
      return this.refused('STS-ADMIN-0512', { ok: false, errors: ['Name the ' +
    ...
    if (action === 'end') {
      ...
      if (!selection.length) {
        ...
        // empty selection is a reader who ticked nothing and pressed a button;
        // a POST to /logout with an empty body is a caller who asked for
        // everything. Same absence, opposite intent, and the difference is
        // which door it arrived at.
        log.debug("Leaving AdminActions.logoutAction(). Nothing was selected.");
        return this.refused('STS-ADMIN-0513', { ok: false, errors: ['Nothing ' +
                                     'was selected. Use the global logout ' +
                                     'button to end everything — this action ' +
                                     'ends only what it is given, so that an ' +
                                          'empty form cannot sign somebody ' +
    ...
    if (action === 'restore-token') {
      // NON-SPEC, and it is /admin/tokens' restore reached from here rather
      // than a second one: stats.restore() is the same function against the
      // same set. `jtiFrom()` answers an OBJECT — `{ jti, how }` — because it
      // also accepts a whole JWT and has to say where the jti came from. This
      // read the object itself as the jti: `stats.restore()` was handed one and
      // matched nothing, so the action ALWAYS reported "was not revoked, so
      // nothing changed" and put `[object Object]` in the sentence where the
      // jti belongs. It is the same call tokenAction() makes further up, where
      // it was always spelt `found.jti` — which is why /admin/tokens' restore
      // worked and this one did not.
      const found = this.jtiFrom(String(body.jti || body.target || ''));
      if (!found.jti) {
        log.debug("Leaving AdminActions.logoutAction().");
        return this.refused('STS-ADMIN-0502', { ok: false, errors: ['Name ' +
            'the token to restore in `jti`.'] });
      }
      const was = stats.restore(found.jti);
      log.debug("Leaving AdminActions.logoutAction().");
      return { ok: true, jti: found.jti,
               message: 'NON-SPEC: the token with jti ' + found.jti +
               (was ? ' is no longer revoked.' : ' was not revoked, so ' +
                                                 'nothing changed.') +
  ...
  permissionsAction(body) {
    ...
    // WHICH APPLICATION, ASKED ONCE FOR EACH SIDE. The two names are
    // deliberately different — `resource` for the application that EXPOSES a
    // permission and `client` for the one that HOLDS it — because this is the
    // one feature here where a body naming the wrong one still succeeds and
    // writes a grant onto the API instead of onto its caller. `application` is
    // accepted for either as a convenience for a caller editing one entry, and
    // the refusals below name the field that was missing rather than "an
    // application".
    ...
    if (needsResource.indexOf(action) >= 0 && !resource) {
      log.debug("Leaving AdminActions.permissionsAction(). No resource named.");
      return this.refused('STS-ADMIN-0514', { ok: false, errors: ['Which ' +
                                   ...
                                   'belongs to, not the one that will ask ' +
                                        'for it.'] });
    }
    if (needsClient.indexOf(action) >= 0 && !client) {
      log.debug("Leaving AdminActions.permissionsAction(). No client named.");
      return this.refused('STS-ADMIN-0515', { ok: false, errors: ['Which ' +
                                   'application holds it? Send `client` with ' +
                                   'the identifier exactly as the registry ' +
                                   'holds it — this is the application that ' +
                                   'will name the permission in a `scope`, ' +
                                        'not the one that exposes it.'] });
    }

    if (action === 'set-permission-base') {
      // The empty value is legal and CLEARS the base — see setBaseUri()'s
      // message, which says what that does to the permissions still on the
      // entry. It is the same convention every `set` on /admin/applications
      // follows.
      ...
      log.debug("Leaving AdminActions.permissionsAction(). " +
                "set-permission-base " +
                (result.ok ? 'ok' : 'refused') + ".");
      return this.refusedBy('STS-ADMIN-0516', result);
    }
    ...
    if (action === 'define-permission') {
      ...
      log.debug("Leaving AdminActions.permissionsAction(). define-permission " +
                (result.ok ? 'ok' : 'refused') + ".");
      return this.refusedBy('STS-ADMIN-0516', result);
    }
    ...
    if (action === 'remove-permission') {
      ...
      const result = appPermissions.removePermission(resource,
                                                     String(body.name || ''),
                                                     actor);
      log.debug("Leaving AdminActions.permissionsAction(). remove-permission " +
                (result.ok ? 'ok' : 'refused') + ".");
      return this.refusedBy('STS-ADMIN-0516', result);
    }
    ...
    if (action === 'grant-permission') {
      const result = appPermissions.grant(client, String(body.permission || ''),
                                          actor);
      log.debug("Leaving AdminActions.permissionsAction(). grant-permission " +
                (result.ok ? 'ok' : 'refused') + ".");
      return this.refusedBy('STS-ADMIN-0516', result);
    }
    ...
    if (action === 'revoke-permission') {
      const result = appPermissions.revoke(client, String(body.permission ||
                                                          ''),
                                           actor);
      log.debug("Leaving AdminActions.permissionsAction(). revoke-permission " +
                (result.ok ? 'ok' : 'refused') + ".");
      return this.refusedBy('STS-ADMIN-0516', result);
    }

    log.debug("Leaving AdminActions.permissionsAction(). Unknown action.");
    return this.refused('STS-ADMIN-0500',
                   { ok: false, errors: ['Unknown action "' + action + '". ' +
        'The ' +
  ...
  noXacml() {
    ...
    log.debug("Leaving AdminActions.noXacml().");
    return { xacml: false,
  ...
  //
  // `xacml_admin.js`'s action functions answer `{ ok, why }` because that is
  // what `respondToAction()` draws on a console page. **EVERY OTHER ACTION
  // RESOURCE ON `/admin-api` ANSWERS `{ ok, errors: [...] }`** — ssf,
  // permissions, realms, spiffe — and
  // `tests/vendored/sts_admin_api_operations.js` reads that array on every one
  // of them: it walks every documented POST resource, sends an unknown action,
  // and requires the refusal SENTENCE to name the actions, because
  ...
  //
  // So the conversion happens HERE, at the one function the management API
  // calls and the console does not — the console's own POST handlers call
  // `combinedAction()` directly. `why` is kept BESIDE `errors` rather than
  // being replaced, because a caller reading it is reading something true and a
  // rename would break it for nothing.
  //
  ...
  xacmlAction(body) {
    const { log } = this.deps;
    const self = this;
    log.debug('Entering AdminActions.xacmlAction(). action=' + (body ||
                                                                {}).action);
    if (!xacmlPages) {
      log.debug('Leaving AdminActions.xacmlAction(). No XACML module.');
      return this.refused('STS-ADMIN-0501',
                          { ok: false, errors: [this.noXacml().message] });
    }
    const answered = xacmlPages.action(body);
    // `issue-pep-certificate` (2026-09-13) answers a PROMISE — a key pair and a
    // certificate are both asynchronous — and every other XACML action answers
    // a result. Settled here rather than by making every action asynchronous,
    // so that the in-process tests calling `combinedAction()` without awaiting
    // are unaffected; the caller of THIS function settles either shape with
    // `Promise.resolve()`.
    if (answered && typeof answered.then === 'function') {
      log.debug('Leaving AdminActions.xacmlAction(). Asynchronous.');
      return answered.then(converted);
    }
    log.debug('Leaving AdminActions.xacmlAction().');
    ...
    function converted(result) {
      ...
      if (!result.ok && !result.errors && result.why) {
        ...
        return self.refused(self.innerCode(result) || 'STS-ADMIN-0517',
                       Object.assign({}, result, { errors: [result.why] }));
      }
      log.debug('Leaving converted(). ok=' + result.ok);
      return self.refusedBy('STS-ADMIN-0517', result);
    }
  }
  ...
  userFieldsFrom(body) {
    ...
    // A JSON body's own `attributes` object first, so a caller sending both
    // gets the flat fields merged OVER it rather than one of the two silently
    // winning.
    ...
    log.debug("Leaving AdminActions.userFieldsFrom(). " +
  ...
  truthy(value) {
    ...
    if (value === true) {
      log.debug("Leaving AdminActions.truthy().");
      return true;
    }
  ...
  private signOutEverywhere(who, ctx, why) {
    ...
    if (!logoutReader) {
      log.debug("Leaving AdminActions.signOutEverywhere(). No logout module.");
    ...
    log.debug("Leaving AdminActions.signOutEverywhere(). Ended " +
              (result.terminated || []).length + ".");
    return { ended: true, terminated: (result.terminated || []).length,
             message: result.message };
  }

  private credentialAdminAction(action, body, ctx) {
    const { log, credentials, auditLog, accountSignals } = this.deps;
    log.debug("Entering AdminActions.credentialAdminAction(). action=" +
              action);
    if (CREDENTIAL_ADMIN_ACTIONS.indexOf(action) < 0) {
      log.debug("Leaving AdminActions.credentialAdminAction(). Not one of " +
                "them.");
      return null;
    }
    const who = String(body.user || body.username || '').trim();
    if (!who) {
      log.debug("Leaving AdminActions.credentialAdminAction(). No person " +
                "named.");
      return this.refused('STS-ADMIN-0518', { ok: false, errors: ['Name the ' +
          'person in `user`.'] });
    }
    ...
    if (action === 'reset-password') {
      ...
      if (!set.ok) {
        ...
        return this.refusedBy('STS-ADMIN-0780', set);
      }
      ...
      const signedOut = this.signOutEverywhere(who, ctx, 'a password reset');
      ...
      return { ok: true, username: who, password: password,
               ...
               message: 'The password of ' + who + ' was reset. IT IS SHOWN ' +
                        ...
                        'trust. ' + (forced
                          ...
                          : 'The forced change could NOT be recorded, so ' +
                            'they will not be asked to choose their own. ') +
    ...
    if (action === 'issue-password-reset') {
      ...
      if (!issued.ok) {
        log.debug("Leaving AdminActions.credentialAdminAction(). The link " +
                  "was refused.");
        return this.refusedBy('STS-ADMIN-0781', issued);
      }
      ...
      if (!removed.ok) {
        ...
        log.debug("Leaving AdminActions.credentialAdminAction(). The " +
                  "password stayed.");
        return this.refusedBy('STS-ADMIN-0782', removed);
      }
      ...
      if (removed.removed) {
        accountSignals.credentialChanged({ username: who,
          ...
          reasonUser: 'Your password was revoked. Use the reset link you ' +
                      'were sent to choose a new one.' });
      }
      accountSignals.credentialChangeRequired({ username: who,
      ...
      return { ok: true, username: who,
               ...
               message: (mailed ? 'THE LINK WAS NOT MAILED: ' +
                        ...
                        'stores only a hash. Send it to them by a channel ' +
                        'you trust: anybody holding it can set their ' +
                        'password. ' +
                        (removed.removed ? 'Their old password was removed, ' +
                          'so until the link is used they cannot sign in ' +
                          'with a password. ' : '') +
                        'They were signed out of everything: ' +
                        signedOut.message };
    }
    ...
    if (action === 'disable-primary-keys') {
      ...
      if (!result.ok) {
        log.debug("Leaving AdminActions.credentialAdminAction(). Keys " +
                  "refused.");
        return this.refusedBy('STS-ADMIN-0783', result);
      }
      ...
      return { ok: true, username: who, removed: result.removed,
               message: result.removed.length + ' primary security key(s) of ' +
                        who + ' removed. They sign in with their password ' +
                        'now; a key they hold as a second factor is ' +
                        'untouched.' };
    }
    ...
    if (action === 'disable-mfa') {
      ...
      audited('admin.mfa.disabled',
              (result.ok ? 'disabled' : 'could not disable') + ' every ' +
      ...
      // credentials are gone, and a receiver not told would go on trusting
      // them.
      if (removed.totp) {
        accountSignals.credentialChanged({ username: who,
          ...
          reasonAdmin: 'An administrator disabled every second factor of ' +
                       who + '.',
          reasonUser: 'Your authenticator app was removed from your ' +
                      'account.' });
      }
      ...
      if (!result.ok) {
        log.debug("Leaving AdminActions.credentialAdminAction(). Second " +
                  "factors refused.");
        return this.refusedBy('STS-ADMIN-0784', result);
      }
      const requirement = credentials.mfaRequirementFor(who);
      log.debug("Leaving AdminActions.credentialAdminAction(). disable-mfa.");
    ...
    if (!result.ok) {
      log.debug("Leaving AdminActions.credentialAdminAction(). The " +
                "requirement was refused.");
      return this.refusedBy('STS-ADMIN-0785', result);
    }
    ...
    return { ok: true, username: who, required: wanted,
             ...
             message: wanted
               ? 'A second factor is now required of ' + who + '. ' +
                 (mech.mfaRequired
                   ? 'They already hold one, so nothing changes at their ' +
                     'next sign-in.'
  ...
  usersAction(body, context?) {
    ...
    log.debug("Entering AdminActions.usersAction(). action=" + (body.action ||
                                                                '(none)'));
    const action = String(body.action || '');
    // WHO IS DOING IT, for the audit rows the two clears write. Read from the
    // caller rather than from a cookie in here, because the management API
    // calls this same function with an actor of its own — a function that
    // reached for a session would only work from one of the two doors. `via` is
    // the door. `base` (2026-09-13) is the address the request arrived on,
    // realm prefix included, so a password reset link can be handed back WHOLE
    // — the one thing an administrator does with it is send it to somebody.
    ...
    if (action === 'set-attribute' || action === 'add-attribute' ||
      ...
      log.debug("Leaving AdminActions.usersAction(). " + action + " " +
                (result.ok ? "ok." : "refused."));
      return this.refusedBy('STS-ADMIN-0817', result);
    }
    ...
    if (credentialAnswer) {
      log.debug("Leaving AdminActions.usersAction(). A credential action " +
                "answered.");
    ...
    //
    // **THE LINK IS RETURNED ONCE AND NEVER AGAIN**, like the bootstrap
    // password and a client secret, because what is stored is a hash — this
    // service cannot produce it a second time, only replace it. Issuing again
    ...
    if (action === 'issue-activation') {
      ...
      return { ok: true, username: who, expiresAt: issued.expiresAt,
               ...
               message: 'An activation link for ' + who + ' is valid until ' +
                        issued.expiresAt +
                        '. IT IS SHOWN ONCE — this service ' +
                        'stores only a hash and cannot produce it again. ' +
                        'Send it to them by whatever channel you already ' +
                        'use; anybody who has it can complete the account ' +
                        'setup, so treat it as the credential it is. Issuing ' +
                        'another invalidates this one.' };
    }
    ...
    // never a primary credential here, so it drops the account to one factor
    // and never to none. Removing a KEY can — it may be the only credential on
    // the account — which is why it goes through `credentials.removeKey()`
    // rather than a write of its own: that function is where the last-way-in
    // refusal lives, and an operator must not be able to do what the person
    // themselves is stopped from doing.
    //
    ...
    if (action === 'clear-totp') {
      ...
      if (!who) {
        log.debug("Leaving AdminActions.usersAction(). No person named.");
        return this.refused('STS-ADMIN-0518', { ok: false,
                                                errors: ['Name the ' +
                                     'person whose authenticator app is ' +
                                          'being cleared.'] });
      }
      const result = credentials.removeTotp(who);
      auditLog.record({
        category: 'authentication', action: 'admin.mfa.totp.cleared',
        actor: ctx.actor, target: who, outcome: result.ok ? 'success' :
                                                'failure',
      ...
      if (result.ok) {
        ...
        log.info('admin: the authenticator app for "' + who +
                 '" was cleared by ' +
                 (ctx.actor || 'an unnamed caller') + ' (' + ctx.via + '). ' +
                 'That account is down to one factor and they can enrol ' +
                 'again from /portal/mfa.');
      }
      log.debug("Leaving AdminActions.usersAction(). clear-totp " +
                (result.ok ? "ok." : "refused."));
      return this.refusedBy('STS-ADMIN-0520', result);
    }

    // ---------------------------------------------------------------------
    // THE THIRD REMOVAL (2026-09-10), AND IT STOPPED BEING AN ISSUING CONTROL
    // ON 2026-09-11.
    //
    // Clearing an authenticator app or a key takes a factor AWAY and that is
    // all it does. Clearing the recovery codes used to do something else as
    // well: it **re-armed the automatic issue**, because `ensureBackupCodes()`
    // did nothing while a set existed, so the next second factor the person
    // enrolled issued a new one. That was the whole path to a second set and it
    // was deliberately an operator's act.
    //
    ...
    // re-arms.
    //
    // **IT IS KEPT BECAUSE THE OTHER REASON FOR IT NEVER WENT AWAY**: a set
    // this process cannot read, or one an operator has reason to believe has
    // been copied, is an operator's to remove — and unlike the person, an
    // operator can do it without being shown ten new strings.
    //
    ...
    if (action === 'clear-backup-codes') {
      ...
      if (!who) {
        log.debug("Leaving AdminActions.usersAction(). No person named.");
        return this.refused('STS-ADMIN-0518', { ok: false, errors: ['Name ' +
                                     'the person whose recovery codes are ' +
                                          'being cleared.'] });
      }
      ...
      if (result.ok) {
        accountSignals.recoveryInformationChanged({ username: who,
          reasonAdmin: 'An administrator cleared the recovery codes of ' + who +
                       '.' });
        log.info('admin: the recovery codes for "' + who +
                 '" were cleared by ' +
                 (ctx.actor || 'an unnamed caller') + ' (' + ctx.via + '). ' +
    ...
    if (action === 'clear-key') {
      ...
      if (!who) {
        log.debug("Leaving AdminActions.usersAction(). No person named.");
        return this.refused('STS-ADMIN-0518', { ok: false, errors: ['Name ' +
                                     'the person whose security key is being ' +
                                          'removed.'] });
      }
    ...
    // operator to change the generated password — and no such operation was
    // ever written. Somebody following either instruction got a 404 naming an
    // endpoint this service documents.
    //
    // It is `credentials.setPassword()` and nothing else: hashed there, never
    // here, and never written as an attribute — which is also why
    // `userPassword` is refused by the attribute door on `create`.
    if (action === 'set-password') {
      const who = String(body.user || body.username || '').trim();
      if (!who) {
        log.debug("Leaving AdminActions.usersAction(). No user was named.");
        return this.refused('STS-ADMIN-0518', { ok: false, errors: ['Name ' +
            'the person whose password to set.'] });
      }
      const wanted = String(body.password || '');
      const generate = this.truthy(body.generate);
      if (!wanted && !generate) {
        log.debug("Leaving AdminActions.usersAction(). No password.");
        return this.refused('STS-ADMIN-0523', { ok: false, errors: ['Send ' +
                                     '`password` with the password to set, ' +
      ...
      if (!generate && body.passwordConfirm !== undefined &&
        ...
        log.debug("Leaving AdminActions.usersAction(). The two passwords " +
                  "differ.");
        return this.refused('STS-ADMIN-0524', { ok: false, errors: ['The two ' +
      ...
      const set = credentials.setPassword(who, password,
                                          { generated: generate });
      if (!set.ok) {
        log.debug("Leaving AdminActions.usersAction(). The password was " +
                  "refused.");
        return this.refusedBy('STS-ADMIN-0525', set);
      }
      accountSignals.credentialChanged({ username: who,
        credentialType: 'password', changeType: 'update', via: ctx.via,
      ...
      return { ok: true, username: who, generated: generate,
               ...
               // already holds back to them buys nothing and puts it in a
               // second place — a proxy log, a browser history, a test fixture.
               password: generate ? password : undefined,
               message: generate
                 ? 'A password for ' + who + ' has been generated and set. ' +
                   'IT IS SHOWN ONCE — this service stores a scrypt hash and ' +
    ...
    if (action === 'create') {
      if (!directoryWriter) {
        log.debug("Leaving AdminActions.usersAction(). No directory is " +
                  "loaded.");
        return this.refused('STS-ADMIN-0501', { ok: false, errors: ['No LDAP ' +
                                     'directory is loaded in this process, ' +
                                     'so there is nowhere to put a person. ' +
                                     'The rest of this page is unaffected — ' +
                                     'it reports what this service has seen, ' +
      ...
      if (!result.ok) {
        log.debug("Leaving AdminActions.usersAction(). create refused.");
        return this.refusedBy('STS-ADMIN-0526', result);
      }
      // ------------------------------------------------------------------
      // AND THE CREDENTIAL, WHICH IS A SECOND ACT ON A PERSON WHO NOW EXISTS.
      //
      // **THE ORDER IS LOAD-BEARING AND SO IS THE FAILURE MODE.** A password
      // can only be written onto an entry, so the entry has to be there first —
      // and that means a create whose credential step fails has ALREADY put
      // somebody in the directory. It is reported as a SUCCESS carrying a loud
      // failure rather than as a failure, because answering `ok: false` would
      // send an operator to press Create again and meet "that username is
      // taken", which is the least informative true sentence available.
      //
      ...
      const credential = String(body.credential || DEFAULT_CREDENTIAL).trim() ||
                         DEFAULT_CREDENTIAL;
      const answer: Record<string, any> = { ok: true, dn: result.dn,
      ...
      if (credential === 'password' || credential === 'generate') {
        ...
        if (!generated && String(body.password || '') === '') {
          ...
          if (!set.ok) {
            ...
            if (generated) {
              ...
              credentialSaid = ' A password has been GENERATED for them and ' +
                'is shown once: this service stores a scrypt hash and cannot ' +
        ...
        answer.credentialError = 'Unknown credential option "' + credential +
          '". The four are: none, password, generate, activation. Nothing ' +
          'was set.';
      }
      ...
      log.debug("Leaving AdminActions.usersAction(). Created " + result.dn +
                ".");
      answer.message = result.dn + ' now exists in the directory' +
        (result.invented
          ? ', with the invented person behind that name written onto it — ' +
            'so a credential issued ' +
    ...
    log.debug("Leaving AdminActions.usersAction(). Unknown action.");
    return this.refused('STS-ADMIN-0500',
                   { ok: false, errors: ['Unknown action "' + action + '". ' +
        'The ' +
  ...
  // slot above. This function reads the form and phrases the answer, which is
  // the same split usersAction() keeps with createUser() and the reason POST
  // /admin-api/groups/{action} can call straight into it without a second
  ...
  // client sees and what a token SAYS, and changes nothing about what that
  ...
  groupsAction(body) {
    ...
    if (!groupWriter) {
      ...
      // reader whose table is plainly full of groups needs telling that it is
      // the WRITES that are absent rather than the directory.
      log.debug("Leaving AdminActions.groupsAction(). No directory writer is " +
                "installed.");
      return this.refused('STS-ADMIN-0501', { ok: false, errors: ['No LDAP ' +
                                   'directory is loaded in this process, so ' +
                                   'there is nowhere to put a group. The ' +
                                   'rest of this page is unaffected — if it ' +
                                   'is showing you groups, they are being ' +
                                   'read through a different slot and only ' +
                                        'the writes are missing.'] });
    }

    if (action === 'create') {
      const name = String(body.group || body.displayName || body.cn || '')
        .trim();
      ...
      const result = groupWriter.createGroup(name, {
        origin: 'console',
        note: String(body.note || '').trim() ||
              'created by hand on the admin console rather than by a ' +
              'directory client',
        channel: 'console',
      ...
      if (!result.ok) {
        log.debug("Leaving AdminActions.groupsAction(). create refused.");
        return this.refusedBy('STS-ADMIN-0527', result);
      }
      log.debug("Leaving AdminActions.groupsAction(). Created " + result.dn +
                ".");
        ...
        message: result.dn + ' now exists' +
          (result.members.length
            ...
            ' with no members at all — an empty groupOfNames is something a ' +
              'real directory refuses (RFC 4519 makes `member` MUST) and ' +
              'this ' +
          ...
          (result.dangling.length
            ? ' ' + result.dangling.length + ' of those member value(s) name ' +
              ...
              'here does referential integrity, and the Dangling column ' +
              'above is where they show up.'
            : '') +
          ' IT GRANTS NOTHING. No endpoint in this service checks a group, ' +
          'and the only two that decide anything are the console roles on ' +
    ...
    if (action === 'add-member') {
      ...
      if (!result.ok) {
        log.debug("Leaving AdminActions.groupsAction(). add-member refused.");
        return this.refusedBy('STS-ADMIN-0528', result);
      }
      log.debug("Leaving AdminActions.groupsAction(). " +
                (result.changed ? "Added." : "Already a member."));
      return result;
    }

    log.debug("Leaving AdminActions.groupsAction(). Unknown action.");
    return this.refused('STS-ADMIN-0500',
                   { ok: false, errors: ['Unknown action "' + action + '". ' +
                                 'The two are: create, add-member. Removing ' +
                                 'a member is /admin/rbac for the two ' +
                                 'console roles, and an ldapmodify or a SCIM ' +
                                 'PATCH ' +
                                      'for every other group; deleting one ' +
  ...
  applicationFieldsFrom(body) {
    ...
    // the flat fields merged over it rather than one of the two silently
    // winning.
    ...
    log.debug("Leaving AdminActions.applicationFieldsFrom(). " +
              Object.keys(fields).length +
        " " +
        "field(s).");
    return fields;
  ...
  // `load-resource-metadata` compares a document against and which are
  ...
  applicationsAction(body, protocols, context) {
    ...
    const self = this;
    log.debug("Entering AdminActions.applicationsAction(). action=" +
              (body.action || '(none)'));
    const action = String(body.action || '');
    ...
    if (needsOne.indexOf(action) >= 0 && !identifier) {
      log.debug("Leaving AdminActions.applicationsAction(). No application " +
                "named.");
      return this.refused('STS-ADMIN-0529', { ok: false, errors: ['Which ' +
                                   'application? Send `application` with the ' +
                                   'identifier exactly as this registry ' +
                                   'holds it — the client_id, wtrealm, ' +
                                   'AppliesTo, entityID or service principal ' +
                                   'name.'] });
    }
    ...
    if (action === 'create') {
      ...
      // is ONE FIELD REPEATED, and helpers.parseBody() builds a plain object,
      // so `body.protocol` is whichever box happened to be ticked last and
      // every other one is silently gone. listField() re-reads the raw body
      // with getAll(); the two routes that reach this function both call it, so
      // the console's form and a JSON body land here as the same list.
      //
      // The fallback to `body.protocols` is for a caller that did not go
      // through either route — a test requiring this module — and it is what
      // keeps the one-argument signature this function used to have working.
      ...
      const result = applications.createApplication({
        ...
        // JSON caller may. What went was the select that asked a person to
        // guess.
        kind: String(body.kind || ''),
        protocols: asked,
        fields: this.applicationFieldsFrom(body)
      });
      log.debug("Leaving AdminActions.applicationsAction(). create " +
                (result.ok ? 'ok' : 'refused') + ".");
      if (!result.ok) {
               ...
               message: '"' + result.application.identifier + '" is in the ' +
                        ...
                        (Object.keys(this.applicationFieldsFrom(body)).length
                          ? 'The attributes given with it are on the entry ' +
                            ...
                            'against, by exact string comparison, and the ' +
                            'rest are recorded rather than checked.'
                          :
                          'It carries no identifier and no redirect URI yet. ' +
                            'Give it the ones it is allowed and RFC 9700 ' +
    ...
    //
    // THE CHANGE IS BUILT FROM THREE NAMED FIELDS, as `set`/`add`/`remove`'s
    // is, so nothing else in the body — an `observed` flag above all — can
    // reach the registry.
    ...
    if (action === 'issue-software-statement') {
      ...
      log.debug("Leaving AdminActions.applicationsAction(). " +
                "issue-software-statement " +
                (result.ok ? 'ok' : 'refused') + ".");
      return this.refusedBy('STS-ADMIN-0649', result);
    }

    // ---------------------------------------------------------------------
    // THE ONE ACTION HERE THAT IS ASYNCHRONOUS, and the only one in this
    // console that makes an outbound request.
    //
    // It returns a PROMISE where every other action returns an object, which
    // both dispatchers handle by checking for a `then`. That is a small
    // ugliness and it is deliberately smaller than the alternatives: making
    // every action async would put an `await` in front of forty call sites to
    // serve one, and giving this one its own route would mean a second door
    // onto the applications resource that the parity rule would then have to
    // describe twice.
    //
    // WHY IT IS AN ACTION AND NOT PART OF ISSUING: sp_metadata.ts's header
    // argues it at length — an assertion that had to wait on somebody else's
    // web server makes every sign-in as reliable as that server. This writes
    // the certificate onto the entry and issuing reads the entry.
    if (action === 'refresh-metadata') {
      log.debug("Leaving AdminActions.applicationsAction(). Fetching " +
                "metadata for " +
                identifier + ".");
    ...
    // answer with the application it describes. IT CREATES NOTHING; the create
    // is `create`, with the fields this answer proposes. It is an action and
    // not a view because the fetch is an outbound request an administrator
    // causes, and both admin surfaces gate a POST behind Admin Write. A
    // promise, like `refresh-metadata`, for that action's reason.
    if (action === 'load-resource-metadata') {
      ...
      log.debug("Leaving AdminActions.applicationsAction(). Loading RFC 9728 " +
                "metadata.");
      return resourceMetadata.load({ document: body.document, url: body.url,
        ...
        .then(function (result) {
          return self.refusedBy('STS-ADMIN-0644', result);
        });
    }

    // ---------------------------------------------------------------------
    // A TLS CLIENT CERTIFICATE FOR THE APPLICATION (RFC 8705, 2026-09-13), from
    // the Credentials section of its own page.
    // `tls_client_certificates.issue()` decides everything about the
    // certificate — the realm's `tls-client` Issuing CA, clientAuth, the
    // urn:sts:application: name the token endpoint's implicit mapping reads,
    // the cap — and packages it; this action decides that the application
    // exists and that the file password is one. A PROMISE, for
    // `refresh-metadata`'s reason: generating the key and exporting the PKCS#12
    // are asynchronous.
    //
    ...
    // as a page of downloads rather than redirecting, and `/admin-api` hands
    // the files back in the JSON. The password protecting them is neither
    // stored nor audited.
    if (action === 'issue-tls-client-certificate') {
      const target = applications.get(identifier);
      if (!target) {
        log.debug("Leaving AdminActions.applicationsAction(). No such " +
                  "application.");
        return this.refused('STS-ADMIN-0722', { ok: false, errors: ['There ' +
            'is no application called "' + identifier + '" here.'] });
      }
      ...
      if (passwordSaid) {
        log.debug("Leaving AdminActions.applicationsAction(). The file " +
                  "password.");
        return this.refused('STS-ADMIN-0721', { ok: false,
                                                errors: [passwordSaid] });
      }
      ...
      return tlsClientCertificates.issue(undefined, {
        ...
        if (!made.ok) {
          log.debug("Leaving AdminActions.applicationsAction(). The issue " +
                    "was refused.");
          return self.refusedBy('STS-ADMIN-0720', made);
        }
        const issued = made.issued;
        return tlsClientCertificates.bundle(issued, String(body.password))
          .then(function (files) {
            log.debug("Leaving AdminActions.applicationsAction(). A TLS " +
                      "client certificate was issued to " + holder + ".");
            return {
              ok: true,
              message: 'A TLS client certificate was issued to "' + holder +
                       '" (serial ' + issued.serialHex + '). This is the ' +
                       'only time its private key can be downloaded.',
    ...
    if (action === 'revoke-tls-client-certificate') {
      ...
      log.debug("Leaving AdminActions.applicationsAction(). " +
                "revoke-tls-client-certificate " +
                (done.ok ? 'ok' : 'refused') + ".");
      return this.refusedBy('STS-ADMIN-0723', done);
    }

    if (action === 'revoke-registration') {
      // Not a delete: the entry stays and its history with it. This is RFC
      // 7592's delete reached from the console instead of from the client that
      // holds the registration access token — the same function, so the outcome
      // is the same one and not a second reading of what "unregistered" means.
      const before = applications.get(identifier);
      if (!before) {
        log.debug("Leaving AdminActions.applicationsAction().");
        return this.refused('STS-ADMIN-0533', { ok: false, errors: ['There ' +
            'is no application called "' + identifier + '" ' +
            'here.'] });
      }
      if (!before.registered) {
        log.debug("Leaving AdminActions.applicationsAction().");
        return this.refused('STS-ADMIN-0534',
      ...
      log.debug("Leaving AdminActions.applicationsAction(). The registration " +
                "was revoked.");
      return { ok: true, application: applications.get(identifier),
               message: 'The RFC 7591 registration for "' + identifier + '" ' +
                        'is gone, along with its client_secret and its ' +
    ...
    if (action === 'forget') {
      ...
      log.debug("Leaving AdminActions.applicationsAction(). forget " +
                (result.ok ? 'ok' : 'refused') + ".");
      return this.refusedBy('STS-ADMIN-0535', result);
    }

    log.debug("Leaving AdminActions.applicationsAction(). Unknown action.");
    ...
    // is not a cosmetic drift — this repository's own
    // tests/vendored/admin_api.js READS this sentence to check that every
    // console action has an /admin-api operation, so an action missing from it
    // is an action the parity check cannot see, and the API could lose the
    // operation entirely with nothing failing. A generated list cannot be short
    // by one.
    return this.refused('STS-ADMIN-0500',
                   { ok: false, errors: ['Unknown action "' + action + '". ' +
        'The ' +
  ...
  asAction(body) {
    ...
    if (action === 'create') {
      ...
      log.debug("Leaving AdminActions.asAction(). create " + (result.ok ? 'ok' :
                                                              'refused') +
                ".");
      if (!result.ok) {
        log.debug("Leaving AdminActions.asAction().");
        return this.refusedBy('STS-ADMIN-0536', result);
      }
      log.debug("Leaving AdminActions.asAction().");
      return { ok: true, profile: result.profile,
               message: 'The "' + result.profile.id + '" authorization ' +
                                                      'server is published ' +
                                                          'at ' +
    ...
    if (action === 'set') {
      const result = authorizationServers.setMember(id, body.member,
                                                    body.value);
      log.debug("Leaving AdminActions.asAction(). set " + (result.ok ? 'ok' :
                                                           'refused') +
                ".");
      return this.refusedBy('STS-ADMIN-0536', result);
    }
    if (action === 'remove') {
      ...
      log.debug("Leaving AdminActions.asAction(). remove " + (result.ok ? 'ok' :
                                                              'refused') +
                ".");
      return this.refusedBy('STS-ADMIN-0536', result);
    }
    if (action === 'reset') {
      ...
      log.debug("Leaving AdminActions.asAction(). reset " + (result.ok ? 'ok' :
                                                             'refused') +
                ".");
      return this.refusedBy('STS-ADMIN-0536', result);
    }
    if (action === 'delete') {
      ...
      log.debug("Leaving AdminActions.asAction(). delete " + (result.ok ? 'ok' :
                                                              'refused') +
                ".");
      return this.refusedBy('STS-ADMIN-0536', result);
    }
    log.debug("Leaving AdminActions.asAction(). Unknown action.");
    return this.refused('STS-ADMIN-0500',
                   { ok: false, errors: ['Unknown action "' + action + '". ' +
  ...
  saml2Action(body): any {
    ...
    if (!identifier) {
      log.debug("Leaving AdminActions.saml2Action(). No service provider " +
                "named.");
      return this.refused('STS-ADMIN-0537', { ok: false, errors: ['Name the ' +
          'service provider by its entityID, in `sp`.'] });
    }

    if (action === 'register') {
      const result = applications.createApplication({
        identifier: identifier, kind: SAML2_SP_KIND, protocol: 'SAML 2.0',
        note: 'registered as a SAML 2.0 service provider from the admin ' +
              'console',
        fields: { samlEntityId: identifier }
      });
      log.debug("Leaving AdminActions.saml2Action(). register " +
                (result.ok ? 'ok' : 'refused') + ".");
      if (!result.ok) {
      ...
      return { ok: true, application: result.application,
               message: 'Registered. Its identity provider metadata is at ' +
                        '/saml2/metadata/' +
                        encodeURIComponent(saml2.slugOf(identifier)) +
                        ', and ' +
                        'it would have been created by the first ' +
                        'AuthnRequest or metadata fetch anyway — registering ' +
                        'it early is what gives you a document to hand ' +
                        'somebody now.' };
    }
  ...
  saml11Action(body) {
    ...
    if (!identifier) {
      log.debug("Leaving AdminActions.saml11Action(). No relying party named.");
      return this.refused('STS-ADMIN-0538', { ok: false, errors: ['Name the ' +
          'relying party by its identifier, in `rp`.'] });
    }
    ...
    if (action === 'register') {
      ...
      log.debug("Leaving AdminActions.saml11Action(). register " +
                (result.ok ? 'ok' : 'refused') + ".");
      if (!result.ok) {
        log.debug("Leaving AdminActions.saml11Action().");
        return this.refusedBy('STS-ADMIN-0530', result);
      }
      log.debug("Leaving AdminActions.saml11Action().");
      return { ok: true, application: result.application,
               message: 'Registered. Its identity provider metadata is at ' +
                        '/saml11/metadata/' +
                        encodeURIComponent(saml11.slugOf(identifier)) + ', ' +
                        'and it would have been created by the first flow or ' +
                        'metadata fetch anyway — registering it early is ' +
                        'what ' +
                        'gives you a document to hand somebody now, and a ' +
                        'name to put in providerId so that nothing has to be ' +
                        'guessed.' };
    }
    log.debug("Leaving AdminActions.saml11Action(). Unknown action.");
    return this.refused('STS-ADMIN-0500',
                   { ok: false, errors: ['Unknown action "' + action + '". ' +
  ...
  mfaAction(body, context) {
    ...
    if (MFA_ACTIONS.indexOf(action) < 0) {
      log.debug("Leaving AdminActions.mfaAction(). Not an action of this " +
                "resource.");
      return this.refused('STS-ADMIN-0500',
                     { ok: false, errors: ['Unknown action "' + action + '". ' +
          'There are ' +
    ...
    log.debug("Leaving AdminActions.mfaAction(). Handing " + action +
              " to usersAction().");
    return this.usersAction(body, context);
  }

  // Both writes, and neither decides anything: admin_rbac.js holds the rules
  // and this reads two fields off a body. `actor` is threaded through so the
  ...
  rbacAction(body, context) {
    ...
    if (action === 'grant') {
      ...
      log.debug("Leaving AdminActions.rbacAction(). grant " +
                (result.ok ? "ok." : "refused."));
      return this.refusedBy('STS-ADMIN-0539', result);
    }
    if (action === 'revoke') {
      const result = rbac.revoke(username, role, ctx);
      log.debug("Leaving AdminActions.rbacAction(). revoke " +
                (result.ok ? "ok." : "refused."));
      return this.refusedBy('STS-ADMIN-0539', result);
    }

    log.debug("Leaving AdminActions.rbacAction(). Unknown action.");
    return this.refused('STS-ADMIN-0500',
                   { ok: false, errors: ['Unknown action "' + action + '". ' +
  ...
  consentAction(body) {
    ...
    const action = String(body.action || '');
    const actor = String(body.actor || '');
    // `client` for the application half and `username` for the person half,
    ...
    // right-looking thing onto the wrong entry. `application` is accepted for
    // `client` as a convenience for a caller editing one entry.
    const client = String(body.client || body.application || '').trim();
    const username = String(body.username || body.user || '').trim();
    ...
    if (action === 'grant-global-consent') {
      ...
      log.debug("Leaving AdminActions.consentAction(). grant-global-consent " +
                (result.ok ? 'ok' : 'refused') + ".");
    ...
    if (action === 'revoke-consent') {
      ...
      if (result.ok) {
        ...
        // same division `rbacAction()` has: the actor is the person whose
        // session got them through the gate, and the module underneath has no
        // request to read one from. `consent_screen.js` writes its own row for
        // the same reason from the other side — there the actor is the person
        // consenting.
        auditLog.audit({ action: 'consent.revoke', actor: actor, target: client,
                      protocol: 'OAuth 2.0 / OIDC', channel: 'http',
    ...
    if (action === 'forget-user-consent') {
      ...
      if (result.ok) {
        auditLog.audit({ action: 'consent.revoke', actor: actor,
                         target: username,
                      protocol: 'OAuth 2.0 / OIDC', channel: 'http',
    ...
    log.debug("Leaving AdminActions.consentAction(). Unknown action.");
    return this.refused('STS-ADMIN-0500',
                   { ok: false, errors: ['Unknown action "' + action + '". ' +
        'The ' +
  ...
  roleMemberKindOf(kind) {
    ...
    log.debug("Leaving AdminActions.roleMemberKindOf().");
  ...
  rolesAction(body, context) {
    ...
    if (action === 'create-role') {
      ...
      return result.ok ? result
        : this.refused(this.innerCode(result) || 'STS-ADMIN-0542',
    ...
    if (action === 'delete-role') {
      ...
      if (!result.ok) {
        log.debug("Leaving AdminActions.rolesAction(). delete-role refused.");
      ...
      auditLog.audit({ action: 'roles.delete', actor: actor, target: name,
                    ...
                    detail: 'deleted the role "' + name + '"' +
                            (stillRequired.length
                               ...
                               stillRequired.join(', ')
                               : '') });
      log.debug("Leaving AdminActions.rolesAction(). delete-role ok.");
      return { ok: true, name: name, stillRequired: stillRequired,
               message: 'The role "' + name + '" is gone.' +
                        (stillRequired.length
                           ? ' ' + numberWord(stillRequired.length) +
                             ' application(s) still require it and can now ' +
                             'be issued nothing: ' + stillRequired.join(', ') +
    ...
    if (action === 'describe-role') {
      ...
      if (!row) {
        log.debug("Leaving AdminActions.rolesAction(). describe-role refused.");
        return this.refused('STS-ADMIN-0543', { ok: false, errors: ['There ' +
            'is no role called "' + name + '".'] });
      }
      ...
      // description would empty the membership, which is the one mistake in
    ...
    if (action === 'add-member' || action === 'remove-member') {
      const kindRow = this.roleMemberKindOf(kind);
      if (!kindRow) {
        log.debug("Leaving AdminActions.rolesAction(). Unknown member kind.");
        return this.refused('STS-ADMIN-0544', { ok: false, errors: ['"' + kind +
                                                                    '" ' +
      ...
      if (!member) {
        log.debug("Leaving AdminActions.rolesAction(). No member named.");
        return this.refused('STS-ADMIN-0545', { ok: false,
                                                errors: ['`member` ' +
            'names the ' + kindRow.label +
                                          ' being ' + (action === 'add-member'
                                            ? 'added to' : 'removed from') +
                                     ' the role, and it is required. `role` ' +
                                          'is the role\'s own name.'] });
      }
      const row = roles.read(name);
      if (!row) {
        // A BUILT-IN ROLE IS NAMED IN THE REFUSAL RATHER THAN FALLING THROUGH
        // TO "no such role", because that is the mistake somebody will actually
        // make: the six built-in roles are in every menu on this page, they
        // have no members and cannot be given any, and "there is no role called
        // ALL_AUTHENTICATED_USERS" would be a flatly false sentence about a
        // role the page had just drawn.
        if (roles.isBuiltIn(name)) {
          log.debug("Leaving AdminActions.rolesAction(). Built-in role.");
          return this.refused('STS-ADMIN-0546',
                              { ok: false, errors: ['"' + name + '" ' +
                                       ...
                                       'matches it holds it, always. Create ' +
                                       'a role of your own to give somebody ' +
                                       'something they do not already ' +
                                       'have.'] });
        }
      ...
      // from a login form, a SAML subject, a Kerberos principal and a
      ...
      if (action === 'add-member') {
        if (at >= 0) {
          log.debug("Leaving AdminActions.rolesAction(). Already a member.");
          return this.refused('STS-ADMIN-0547',
        ...
        if (at < 0) {
          log.debug("Leaving AdminActions.rolesAction(). Not a member.");
          return this.refused('STS-ADMIN-0548',
                              { ok: false, errors: ['"' + member + '" ' +
              'does not hold "' +
                                       name + '" as ' + kindRow.label + '.'] });
        }
  ...
  //
  // `names` is the second argument for the same reason vcAction() has one: a
  // list that may appear more than once in a form body is not something
  // helpers.parseBody() can answer, so the caller reads it with listField() and
  // hands it in. It is only read by the `attributes` action; the other six
  // ignore it.
  //
  ...
  // /admin/claims naming `saml2` is refused by name instead of quietly changing
  ...
  claimsAction(body, names, allowed) {
    ...
    if (sets.indexOf(setId) < 0) {
      log.debug("Leaving AdminActions.claimsAction(). No such set here.");
      return this.refused('STS-ADMIN-0551', { ok: false, errors: ['There is ' +
          'no claim set called "' + setId + '" ' +
    ...
    if (action === 'add') {
      ...
      log.debug("Leaving AdminActions.claimsAction(). add -> ok=" + result.ok);
      return result.ok
    ...
    if (action === 'remove') {
      ...
      if (remaining.length === stats.claimSet(setId).length) {
        log.debug("Leaving AdminActions.claimsAction(). Nothing named that.");
        return this.refused('STS-ADMIN-0553',
                            { ok: false, errors: ['The ' + label + ' ' +
            'claim set has no claim called "' + name + '".'] });
      }
      const result = stats.setClaimSet(setId, remaining);
      log.debug("Leaving AdminActions.claimsAction(). remove -> ok=" +
                result.ok);
      return result.ok
        ? { ok: true, set: setId, claims: result.claims,
            message: 'Removed "' + name + '" from the ' + label +
                     ' claim set.' }
        : this.refusedBy('STS-ADMIN-0552', result);
    }
    ...
    if (action === 'clear') {
      ...
      log.debug("Leaving AdminActions.claimsAction(). Cleared.");
    ...
    if (action === 'replace') {
      ...
      if (typeof entries === 'string') {
        try {
          ...
          log.debug("Leaving AdminActions.claimsAction(). The JSON did not " +
                    "parse.");
          return this.refused('STS-ADMIN-0554', { ok: false, errors: ['That ' +
              'is not valid JSON: ' + e.message] });
        }
      }
      if (!Array.isArray(entries)) {
        log.debug("Leaving AdminActions.claimsAction(). Not an array.");
        return this.refused('STS-ADMIN-0555', { ok: false, errors: ['Give a ' +
                                     'JSON ARRAY of {"name": ..., "value": ' +
                                     '...} objects. An empty array clears ' +
                                          'the set.'] });
      }
      const result = stats.setClaimSet(setId, entries);
      log.debug("Leaving AdminActions.claimsAction(). replace -> ok=" +
                result.ok);
      return result.ok
        ? { ok: true, set: setId, claims: result.claims,
    ...
    // ------------------------------------------------------------------------
    // The three that act on the DIRECTORY ATTRIBUTE half of a set rather than
    // on the typed claims above.
    //
    // They are three actions and not one with a mode, because each is a
    // different thing to authorise and a different row in the audit log:
    // `attributes` carries a list somebody chose, and the other two carry
    // nothing and mean the extremes. A single action taking a list would make
    // "select all" a caller's job to construct — the whole catalogue in a POST
    // body to say "all of them" — which is a list that has to be updated every
    // time the catalogue is.
    //
    // What the split does NOT do is make an empty `attributes` unambiguous, and
    // it is worth being plain about that rather than implying otherwise. An
    // empty list and an absent one both mean "the selection is nothing", so a
    // caller that misspells the field clears the set. Three things make that
    // recoverable rather than silent, and they are the reason it is not refused
    // instead: the reply names every attribute it `removed`, the audit log
    // keeps a row saying the same, and unticking every box and pressing Update
    // is a legitimate way to clear a set that a refusal would have to break.
    // `attributes-clear` exists so that a caller which MEANS it can say so, and
    // so that the console's button does not depend on submitting an empty form.
    //
    // The console's buttons are form posts for the same reason every other
    // control here is: app.js sets `script-src 'none'` for the whole service,
    // so a browser-side "tick every box" is not available and would not be
    // taken if it were — a server-side select-all leaves an audit row, and a
    // scripted one would leave the boxes ticked and the set unchanged until
    // somebody pressed Update.
    // ------------------------------------------------------------------------
    if (action === 'attributes') {
      const result = claimAttributes.setSelection(setId, names || [], 'select');
      log.debug("Leaving AdminActions.claimsAction(). attributes -> ok=" +
                result.ok);
      return result.ok
        ? { ok: true, set: setId, attributes: result.attributes,
            ...
            message: 'The ' + label + ' set now carries ' +
                     ...
                     'with the value on that person\'s entry; nothing ' +
    ...
    if (action === 'attributes-all') {
      ...
      log.debug("Leaving AdminActions.claimsAction(). attributes-all -> ok=" +
                result.ok);
      return result.ok
        ? { ok: true, set: setId, attributes: result.attributes,
            added: result.added, removed: result.removed,
            message: 'The ' + label + ' set now carries every attribute in ' +
                                      'the catalogue — ' +
                     result.attributes.length + ' of them. That is a ' +
                     'legitimate thing to test and it makes a large token; ' +
                     'it is not a mistake this page will correct.' }
        : this.refusedBy('STS-ADMIN-0552', result);
    }

    if (action === 'attributes-clear') {
      const result = claimAttributes.clearSelection(setId);
      log.debug("Leaving AdminActions.claimsAction(). attributes-clear -> ok=" +
                result.ok);
      return result.ok
  ...
  vcAction(body, names) {
    ...
    if (action === 'select' || action === 'replace') {
      // `replace` is the same operation under the name a test would look for,
      // and the same name /admin/claims uses for "here is the whole set at
      // once".
      const result = vcClaims.setSelection(names);
      if (!result.ok) {
        log.debug("Leaving AdminActions.vcAction(). The selection was " +
                  "refused.");
        return this.refusedBy('STS-ADMIN-0556', result);
      }
      const sweep = vcClaims.populateDirectory();
      log.debug("Leaving AdminActions.vcAction(). Selected " +
      ...
      return { ok: true, selected: result.selected, added: result.added,
               ...
               message: 'A credential issued from now on carries ' +
                        ...
                        '. Removed: ' + (result.removed.join(', ') ||
                                         'nothing') + '.' +
                        this.sweepText(sweep) };
    }

    if (action === 'add' || action === 'remove') {
      const name = String(body.attribute || body.name || '').trim();
      if (!name) {
        log.debug("Leaving AdminActions.vcAction(). No attribute was named.");
        return this.refused('STS-ADMIN-0557', { ok: false, errors: ['Name ' +
            'the attribute to ' + action + '.'] });
      }
      const lower = name.toLowerCase();
      const already =
          current.some(function (n) { return n.toLowerCase() === lower; });
      if (action === 'add' && already) {
        log.debug("Leaving AdminActions.vcAction(). Already selected.");
        return this.refused('STS-ADMIN-0558', { ok: false, errors: ['"' + name +
                                                                    '" ' +
            'is already in the claim set.'] });
      }
      if (action === 'remove' && !already) {
        log.debug("Leaving AdminActions.vcAction(). Not selected.");
        return this.refused('STS-ADMIN-0559', { ok: false, errors: ['"' + name +
                                                                    '" ' +
      ...
      if (!result.ok) {
        log.debug("Leaving AdminActions.vcAction(). The attribute was " +
                  "refused.");
        return this.refusedBy('STS-ADMIN-0556', result);
      }
      const sweep = vcClaims.populateDirectory();
      log.debug("Leaving AdminActions.vcAction(). " + action + " -> " +
          result.selected.length + " " +
          "selected.");
      return { ok: true, selected: result.selected, sweep: sweep,
               message: (action === 'add' ? 'Added ' : 'Removed ') + name +
                        '. A credential issued from now on carries ' +
                        result.selected.length +
                        ' claim(s).' + this.sweepText(sweep) };
    }

    if (action === 'defaults') {
      const result = vcClaims.resetSelection();
      const sweep = vcClaims.populateDirectory();
      log.debug("Leaving AdminActions.vcAction(). Restored the defaults.");
      return { ok: true, selected: result.selected, sweep: sweep,
               message: 'The claim set is the default ' +
                        result.selected.length +
    ...
    if (action === 'populate') {
      const sweep = vcClaims.populateDirectory();
      log.debug("Leaving AdminActions.vcAction(). Populated only.");
      return this.refusedBy('STS-ADMIN-0560', {
        ok: sweep.ok, errors: sweep.errors, selected: current, sweep: sweep,
        message: 'Populated the directory for the current claim set.' +
                 this.sweepText(sweep) });
    }

    log.debug("Leaving AdminActions.vcAction(). Unknown action.");
    return this.refused('STS-ADMIN-0500',
                   { ok: false, errors: ['Unknown action "' + action + '". ' +
                                 'The five are: select, add, remove, ' +
                                      'defaults, populate.'] });
  }
  ...
  // decides what an issued credential CARRIES; this decides what the Verifier
  // at /oid4vp/verifier ASKS FOR, and the two are deliberately separate
  // settings because the interesting states are the ones where they disagree. A
  // Verifier asking for a claim the issuer is not minting is the negative that
  // exercises a wallet's "I cannot satisfy this request" path, and there is no
  // way to reach it if one page sets both.
  //
  ...
  //
  // It is the third page here that changes what a protocol endpoint does, and
  // the only one whose effect is visible in a document this service SENDS
  ...
  vpConfigAction(body, names) {
    ...
    if (action === 'select' || action === 'replace') {
      ...
      // `claims` member asks for the WHOLE credential, so unticking everything
      // is how somebody tests that. It is stated in the message rather than
      // left to be discovered from a presentation that disclosed everything.
      const result = vpConfig.setRequested(names);
      if (!result.ok) {
        log.debug("Leaving AdminActions.vpConfigAction(). The selection was " +
                  "refused.");
        return this.refusedBy('STS-ADMIN-0561', result);
      }
      log.debug("Leaving AdminActions.vpConfigAction(). " +
    ...
    if (action === 'add') {
      ...
      if (!result.ok) {
        log.debug("Leaving AdminActions.vpConfigAction(). The claim was " +
                  "refused.");
        return this.refusedBy('STS-ADMIN-0561', result);
      }
      const known = vpConfig.rowFor(name);
      log.debug("Leaving AdminActions.vpConfigAction(). Added " + name + ".");
    ...
    if (action === 'remove') {
      ...
      if (!result.ok) {
        log.debug("Leaving AdminActions.vpConfigAction(). Nothing named that.");
        return this.refusedBy('STS-ADMIN-0561', result);
      }
      log.debug("Leaving AdminActions.vpConfigAction(). Removed " + name + ".");
      return { ok: true, requested: result.requested,
    ...
    if (action === 'defaults') {
      ...
      log.debug("Leaving AdminActions.vpConfigAction(). Restored the startup " +
                "request.");
      return { ok: true, requested: result.requested,
    ...
    if (action === 'format') {
      ...
      if (!result.ok) {
        log.debug("Leaving AdminActions.vpConfigAction(). No such format.");
        return this.refusedBy('STS-ADMIN-0561', result);
      }
      log.debug("Leaving AdminActions.vpConfigAction(). Default format is " +
                result.format +
                ".");
      return { ok: true, format: result.format,
               message: 'A request that does not name a format now asks for ' +
                        'a ' +
                        result.format +
    ...
    log.debug("Leaving AdminActions.vpConfigAction(). Unknown action.");
    return this.refused('STS-ADMIN-0500',
                   { ok: false, errors: ['Unknown action "' + action + '". ' +
                                 'The five are: select, add, remove, ' +
                                      'defaults, format.'] });
  ...
  realmsAction(body) {
    ...
    if (action === 'create') {
      ...
      // and merges overrides properly, the console's own form has no such
      // field, and a silently ignored parameter has no failure to point at.
      //
      ...
      return { ok: true, realm: result.realm.id,
               ...
               message: 'The realm "' + result.realm.id + '" is defined. ' +
                        'Every HTTP endpoint this service has now answers ' +
                        'under ' +
    ...
    if (action === 'update') {
      ...
      log.debug("Leaving AdminActions.realmsAction(). update ok.");
    ...
    if (action === 'set') {
      ...
      if (!result.ok) {
        log.debug("Leaving AdminActions.realmsAction(). set refused.");
        return this.refusedBy('STS-ADMIN-0562', result);
      }
      log.debug("Leaving AdminActions.realmsAction(). set ok.");
      return { ok: true, realm: id, key: key,
               message: key + ' is set on the "' + id + '" realm. It applies ' +
                        'to the next request that arrives under that ' +
                        'realm\'s prefix, and to nothing else.' };
    }
    ...
    if (action === 'unset') {
      ...
      if (!result.ok) {
        log.debug("Leaving AdminActions.realmsAction(). unset refused.");
        return this.refusedBy('STS-ADMIN-0562', result);
      }
      log.debug("Leaving AdminActions.realmsAction(). unset ok.");
      return { ok: true, realm: id, key: key,
               message: key + ' is no longer set on the "' + id + '" realm; ' +
                        'it comes from whatever this service as a whole is ' +
                        'configured with.' };
    }
    ...
    if (action === 'remove') {
      ...
      // app.js is about to rewrite into the realm that is being deleted,
      // because that is the realm the request arrived in. The reader would be
      // redirected into a prefix that had stopped existing one instruction
      // earlier and would meet `Cannot GET`. Everything else about the removal
      // would have worked, which is what makes it worth refusing rather than
      // fixing: the fix is to do it from another realm, and that is a sentence
      // rather than a special case in the redirect.
      // ------------------------------------------------------------------
      if (id && id === realms.currentId()) {
        log.debug("Leaving AdminActions.realmsAction(). A realm may not " +
                  "remove itself.");
        return this.refused('STS-ADMIN-0563', { ok: false, errors: ['This ' +
            'request arrived inside the "' + id +
          '" realm, so removing it would leave the caller on a path that no ' +
          'longer exists — the console would be redirected into a prefix ' +
          'that had stopped existing one instruction earlier. Do it from ' +
  ...
  configAction(body) {
    ...
    if (action === 'set') {
      ...
      if (!result.ok) {
        log.debug("Leaving AdminActions.configAction(). set refused.");
        return this.refusedBy('STS-ADMIN-0564', result);
      }
      log.debug("Leaving AdminActions.configAction(). set ok.");
    ...
    if (action === 'set-many') {
      ...
      // rows submits the editable ones and nothing else — which is why an
      // absent key is silently skipped rather than treated as an attempt to
      // clear it.
      const wanted = Object.keys(body).filter(function (name) {
        return name !== 'action' && self.configKnows(name);
      });
      if (!wanted.length) {
        log.debug("Leaving AdminActions.configAction(). set-many named " +
                  "nothing.");
        return this.refused('STS-ADMIN-0565', { ok: false, errors: ['No ' +
          'settings were posted. Every name must be one of the keys GET ' +
      ...
      log.debug("Leaving AdminActions.configAction(). set-many ok, " +
                changed.length + " " +
          "changed.");
      return { ok: true, applied: wanted, changed: changed,
               settings: wanted.map(function (key) {
                 return config.describe(self.configSettingFor(key));
               }),
    ...
    if (action === 'reset') {
      ...
      if (!result.ok) {
        log.debug("Leaving AdminActions.configAction(). reset refused.");
        return this.refusedBy('STS-ADMIN-0564', result);
      }
      log.debug("Leaving AdminActions.configAction(). reset ok.");
      return { ok: true, key: key,
               setting: config.describe(this.configSettingFor(key)),
    ...
    log.debug("Leaving AdminActions.configAction(). Unknown action.");
    return this.refused('STS-ADMIN-0500',
                   { ok: false, errors: ['Unknown action "' + action + '". ' +
  ...
  configKnows(key) {
    ...
    log.debug("Leaving AdminActions.configKnows().");
  ...
  configSettingFor(key) {
    ...
    log.debug("Leaving AdminActions.configSettingFor().");
  ...
  // form posts every one of its fields at once, and applying some of them
  // before refusing another would leave the service issuing tokens with a
  // lifetime combination nobody asked for and the page showing it as though it
  // had been chosen.
  //
  ...
  tokenLifetimesAction(body) {
    const { log, config } = this.deps;
    const self = this;
    log.debug("Entering AdminActions.tokenLifetimesAction(). action=" + (body &&
        body.action));
    const action = String((body && body.action) || '').trim();
    ...
    if (action === 'set') {
      ...
      if (unknown.length) {
        log.debug("Leaving AdminActions.tokenLifetimesAction(). Unknown " +
                  "field(s): " +
                  unknown.join(', '));
        return this.refused('STS-ADMIN-0566', { ok: false, errors: ['This ' +
            'action sets only ' + TOKEN_LIFETIME_KEYS.join(', ') +
      ...
      if (!wanted.length) {
        log.debug("Leaving AdminActions.tokenLifetimesAction(). Nothing was " +
                  "posted.");
        return this.refused('STS-ADMIN-0565', { ok: false, errors: ['No ' +
            'lifetime was posted. Name at least one of ' +
      ...
      if (errors.length) {
        log.debug("Leaving AdminActions.tokenLifetimesAction(). Refused: " +
                  errors.length +
            " " +
            "problem(s).");
        return this.refused('STS-ADMIN-0564', { ok: false, errors: errors });
      ...
      log.debug("Leaving AdminActions.tokenLifetimesAction(). " +
                changed.length +
                " changed.");
      return { ok: true, applied: wanted, changed: changed,
               settings: wanted.map(function (key) {
    ...
    if (action === 'defaults') {
      ...
      log.debug("Leaving AdminActions.tokenLifetimesAction(). Cleared " +
                cleared.length +
                ".");
      return { ok: true, cleared: cleared,
               ...
               message: cleared.length
                 ...
                 : 'None of the four was overridden here, so nothing changed ' +
                   '— they are already coming from the environment or from ' +
                   'one of the two appconfig files.' };
    }

    log.debug("Leaving AdminActions.tokenLifetimesAction(). Unknown action.");
    return this.refused('STS-ADMIN-0500',
                   { ok: false, errors: ['Unknown action "' + action + '". ' +
  ...
  samlAssertionRowFor(key) {
    ...
    log.debug("Leaving AdminActions.samlAssertionRowFor().");
  ...
  samlAssertionsAction(body) {
    const { log, config } = this.deps;
    const self = this;
    log.debug("Entering AdminActions.samlAssertionsAction(). action=" + (body &&
        body.action));
    const action = String((body && body.action) || '').trim();

    if (action === 'set') {
      // Only the keys this page owns, and only the ones actually posted. A
      // field named here that is not one of them is refused BY NAME rather than
      // ignored: this action's whole surface is SAML_ASSERTION_KEYS, and a
      // caller that misspelt one deserves to be told rather than to watch
      // nothing happen.
      const posted = Object.keys(body || {}).filter(function (name) {
        return !FORM_FURNITURE[name];
      });
      ...
      if (unknown.length) {
        log.debug("Leaving AdminActions.samlAssertionsAction(). Unknown " +
                  "field(s): " +
                  unknown.join(', '));
        return this.refused('STS-ADMIN-0566', { ok: false, errors: ['This ' +
            'action sets only ' + SAML_ASSERTION_KEYS.join(', ') +
          '. It was also given: ' + unknown.join(', ') + '. Every other ' +
          'setting is on /admin/config and POST /admin-api/config/set.'] });
      }
      const wanted = posted.filter(function (name) {
      ...
      if (!wanted.length) {
        log.debug("Leaving AdminActions.samlAssertionsAction(). Nothing was " +
                  "posted.");
        return this.refused('STS-ADMIN-0565', { ok: false, errors: ['No ' +
            'setting was posted. Name at least one of ' +
      ...
      if (errors.length) {
        log.debug("Leaving AdminActions.samlAssertionsAction(). Refused: " +
                  errors.length +
            " " +
            "problem(s).");
        return this.refused('STS-ADMIN-0564', { ok: false, errors: errors });
      ...
      log.debug("Leaving AdminActions.samlAssertionsAction(). " +
                changed.length +
                " changed.");
      return { ok: true, applied: wanted, changed: changed,
               settings: wanted.map(function (key) {
               ...
               message: (changed.length
                 ? changed.length + ' setting(s) changed: ' +
                   changed.map(function (key) {
                     const row = self.samlAssertionRowFor(key);
    ...
    if (action === 'defaults') {
      ...
      log.debug("Leaving AdminActions.samlAssertionsAction(). Cleared " +
                cleared.length +
                ".");
      return { ok: true, cleared: cleared,
               ...
               message: cleared.length
                 ...
                 : 'None of the sixteen was overridden here, so nothing ' +
                   'changed — they are already coming from the environment ' +
                   'or from one of the two appconfig files.' };
    }

    log.debug("Leaving AdminActions.samlAssertionsAction(). Unknown action.");
    return this.refused('STS-ADMIN-0500',
                   { ok: false, errors: ['Unknown action "' + action + '". ' +
        'The two are: set, defaults.'] });
  }
  ...
  signalsAction(body) {
    ...
    if (SIGNALS_CONSOLE_ACTIONS.indexOf(action) < 0) {
      ...
      // `tests/vendored/sts_admin_api_operations.js` matches exactly that out
      // of `errors` on every action resource this API declares, and
      ...
      // `pki_admin.js`'s reason: a second action added tomorrow cannot leave
      // the sentence short by one.
      log.debug("Leaving AdminActions.signalsAction(). Unknown action.");
      return this.refused('STS-ADMIN-0500',
                     { ok: false, errors: ['Unknown action "' + action + '". ' +
    ...
    log.debug("Leaving AdminActions.signalsAction(). " + gone + " dropped.");
  ...
  ssfAction(body) {
    const { log } = this.deps;
    const self = this;
    log.debug("Entering AdminActions.ssfAction().");
    const asked = body || {};
    if (!signalsReporter) {
      log.debug("Leaving AdminActions.ssfAction(). Not installed.");
      return Promise.resolve(this.refused('STS-ADMIN-0501', { ok: false,
                                                              errors: [
        'ssf/ssf.ts is not loaded in this process, so there is nothing to ' +
        'act on.'] }));
    }
    const name = String(asked.action || '');
    log.debug("Leaving AdminActions.ssfAction(). " + name);
    return Promise.resolve(signalsReporter.action(name, asked, null))
                  .then(function (result) {
      return self.refusedBy('STS-ADMIN-0567', result);
    });
  }
  ...
  caepAction(body) {
    const { log } = this.deps;
    const self = this;
    log.debug("Entering AdminActions.caepAction().");
    const asked = body || {};
    if (!caepReporter) {
      log.debug("Leaving AdminActions.caepAction(). Not installed.");
      return Promise.resolve(this.refused('STS-ADMIN-0501', { ok: false,
                                                              errors: [
        'ssf/ssf.ts is not loaded in this process, so there is nothing to ' +
        'act on.'] }));
    ...
    const name = String(asked.action || '');
    log.debug("Leaving AdminActions.caepAction(). " + name);
  ...
  riscAction(body) {
    const { log } = this.deps;
    const self = this;
    log.debug("Entering AdminActions.riscAction().");
    const asked = body || {};
    if (!riscReporter) {
      log.debug("Leaving AdminActions.riscAction(). Not installed.");
      return Promise.resolve(this.refused('STS-ADMIN-0501', { ok: false,
                                                              errors: [
        'ssf/ssf.ts is not loaded in this process, so there is nothing to ' +
        'act on.'] }));
    ...
    const name = String(asked.action || '');
    log.debug("Leaving AdminActions.riscAction(). " + name);
  ...
  truststoreAction(body, context) {
    ...
    if (TRUSTSTORE_ACTIONS.indexOf(action) < 0) {
      ...
      log.debug("Leaving AdminActions.truststoreAction(). Unknown action.");
      return this.refused('STS-ADMIN-0500',
                     { ok: false, errors: ['Unknown action "' + action + '". ' +
          'There are ' +
    ...
    if (!truststore) {
      log.debug("Leaving AdminActions.truststoreAction(). Not installed.");
      return this.refused('STS-ADMIN-0501', { ok: false, errors: ['The ' +
        'client-certificate truststore is not installed in this process — ' +
        'tls/tls_server.js was not handed to the console — so there is ' +
        'nothing to change.'] });
    }
    ...
    if (action === 'add') {
      ...
      if (result.error && !result.added) {
        log.debug("Leaving AdminActions.truststoreAction(). add refused.");
        return this.refused(this.innerCode(result) || 'STS-ADMIN-0568',
      ...
      log.debug("Leaving AdminActions.truststoreAction(). added=" +
                added.length);
      // PERSISTED ONLY IF EVERY ANCHOR THIS CALL ADDED WAS WRITTEN DOWN: a
      // store that took some and refused the rest (a full directory) must not
      // be reported as having kept them all.
      const addedPrints =
      ...
      log.debug("Leaving AdminActions.truststoreAction().");
      return { ok: true, added: added.length,
               duplicates: result.duplicates || 0,
               anchors: after.anchors.length,
               ...
               message: added.length + ' anchor(s) added' +
                 ...
                 'anchor(s); connections already open keep the truststore ' +
                 'they were made under. ' + (persisted
    ...
    if (result.error) {
      log.debug("Leaving AdminActions.truststoreAction(). remove refused.");
      return this.refused(this.innerCode(result) || 'STS-ADMIN-0569',
                          { ok: false, errors: [result.error],
    ...
    auditLog.audit({ action: 'admin.truststore.change', actor: actor,
                     ...
                     detail: { action: 'remove', via: via,
                               ...
                               source: gone.source } });
    log.debug("Leaving AdminActions.truststoreAction(). removed.");
             ...
             message: 'Removed ' + gone.subject + '. The next handshake no ' +
               ...
               'under. ' + (gone.source === 'file'
                 ? 'IT CAME FROM tls.trustAnchorsFile AND COMES BACK AT THE ' +
                   'NEXT START — take it out of that file to remove it for ' +
                   'good.'
  ...
  // THE ACTIONS.
  //
  // Three handlers, one per page, and each is what BOTH the console form and
  // the management API call — with `action` taken from the URL there instead of
  // from a hidden input here. That is rule 7's arrangement, and it is what
  // makes an API operation most of the cost of a console control rather than a
  ...
  spiffeCommaList(value) {
    ...
    log.debug("Leaving AdminActions.spiffeCommaList().");
  ...
  spiffeUnknownAction(action, known) {
    ...
    log.debug("Leaving AdminActions.spiffeUnknownAction().");
    return this.refused('STS-ADMIN-0500',
                        { ok: false,
  ...
  spiffeEntriesAction(body) {
    ...
    if (action === 'create') {
      const result = spiffeRegistry.createEntry({
        ...
        selectors: this.spiffeCommaList(body.selectors)
          .map(spiffeRegistry.parseSelector).filter(Boolean),
        dnsNames: this.spiffeCommaList(body.dnsNames),
        federatesWith: this.spiffeCommaList(body.federatesWith),
      ...
      log.debug("Leaving AdminActions.spiffeEntriesAction(). create " +
                (result.ok ? 'ok' : 'refused') + ".");
      if (!result.ok) {
        log.debug("Leaving AdminActions.spiffeEntriesAction().");
        return this.refusedBy('STS-ADMIN-0570', result);
      }
      log.debug("Leaving AdminActions.spiffeEntriesAction().");
    ...
    if (action === 'update') {
      ...
      if (!id) {
        log.debug("Leaving AdminActions.spiffeEntriesAction(). No entry " +
                  "named.");
        return this.refused('STS-ADMIN-0571', { ok: false, errors: ['Which ' +
            'entry? Send `entry` with its id.'] });
      }
      const field = String(body.field || '').trim();
      if (spiffeRegistry.EDITABLE.indexOf(this.fieldToAttribute(field)) < 0) {
        log.debug("Leaving AdminActions.spiffeEntriesAction(). Not editable.");
        return this.refused('STS-ADMIN-0572',
                            { ok: false, errors: ['"' + field + '" ' +
          'is not a field this page may change. The editable ones are what ' +
          'the entry may DO: spiffeId, parentId, selectors, dnsNames, ' +
      ...
      if (field === 'selectors') {
        changes.selectors = this.spiffeCommaList(raw)
          .map(spiffeRegistry.parseSelector).filter(Boolean);
      } else if (field === 'dnsNames' || field === 'federatesWith') {
        changes[field] = this.spiffeCommaList(raw);
      ...
      log.debug("Leaving AdminActions.spiffeEntriesAction(). update " +
                (result.ok ? 'ok' : 'refused') + ".");
      if (!result.ok) {
        log.debug("Leaving AdminActions.spiffeEntriesAction().");
        return this.refusedBy('STS-ADMIN-0570', result);
    ...
    if (action === 'delete') {
      ...
      log.debug("Leaving AdminActions.spiffeEntriesAction(). delete " +
                (result.ok ? 'ok' : 'refused') + ".");
      if (!result.ok) {
        log.debug("Leaving AdminActions.spiffeEntriesAction().");
        return this.refusedBy('STS-ADMIN-0570', result);
      ...
      return { ok: true, id: id,
               message: 'The entry is gone. Anything holding an SVID minted ' +
                        'from it keeps that SVID until it expires — SPIFFE ' +
                        'has no revocation.' };
    }

    log.debug("Leaving AdminActions.spiffeEntriesAction(). Unknown action.");
  ...
  fieldToAttribute(field) {
    ...
    log.debug("Leaving AdminActions.fieldToAttribute().");
  ...
  spiffeAgentsAction(body) {
    ...
    if (SPIFFE_AGENT_ACTIONS.indexOf(action) >= 0 && !id) {
      log.debug("Leaving AdminActions.spiffeAgentsAction(). No agent named.");
      return this.refused('STS-ADMIN-0573', { ok: false,
                                              errors: ['Which agent? ' +
                                   'Send `agent` with its SPIFFE ID, which ' +
                                        'is under /spire/agent/.'] });
    }
    if (action === 'ban' || action === 'unban') {
      const result = spiffeRegistry.setAgentBanned(id, action === 'ban', '');
      log.debug("Leaving AdminActions.spiffeAgentsAction(). " + action + " " +
                (result.ok ? 'ok' : 'refused') + ".");
      if (!result.ok) {
        log.debug("Leaving AdminActions.spiffeAgentsAction().");
        return this.refusedBy('STS-ADMIN-0574', result);
      }
      log.debug("Leaving AdminActions.spiffeAgentsAction().");
    ...
    if (action === 'delete') {
      ...
      log.debug("Leaving AdminActions.spiffeAgentsAction(). delete " +
                (result.ok ? 'ok' : 'refused') + ".");
      if (!result.ok) {
        log.debug("Leaving AdminActions.spiffeAgentsAction().");
        return this.refusedBy('STS-ADMIN-0574', result);
      ...
      return { ok: true, id: id,
               message: 'That agent is forgotten. It reappears the moment it ' +
                        ...
                        'deleting is forgetting, not revoking. Ban it ' +
                        'instead if that is what you meant.' };
    }
    log.debug("Leaving AdminActions.spiffeAgentsAction(). Unknown action.");
    return this.spiffeUnknownAction(action, SPIFFE_AGENT_ACTIONS);
  }
  ...
  //
  // It decides NOTHING itself. Every branch calls `federation.js`, which owns
  // the schema, the modes and the refusals — the same division
  // `applicationsAction()` keeps with `applications.js`. A validation written
  ...
  async federationAction(body) {
    ...
    if (action === 'create') {
      ...
      log.debug("Leaving AdminActions.federationAction(). create " +
                (result.ok ? 'ok' : 'refused') + ".");
      if (!result.ok) {
    ...
    if (!id) {
      log.debug("Leaving AdminActions.federationAction(). No relationship " +
                "named.");
      return this.refused('STS-ADMIN-0576', { ok: false, errors: ['Name the ' +
          'relationship by its id, in `id`.'] });
    }

    if (action === 'set' || action === 'add-value' || action ===
        'remove-value') {
  ...
  async spiffeAction(body) {
    ...
    if (action === 'rotate') {
      ...
      try {
        ...
        log.debug("Leaving AdminActions.spiffeAction(). Rotation failed.");
        return this.refused('STS-ADMIN-0577', { ok: false, errors: ['The ' +
            'authority could not be rotated: ' +
                                          e.message] });
      }
      if (!done.length) {
        log.debug("Leaving AdminActions.spiffeAction(). Nothing named.");
        return this.refused('STS-ADMIN-0578', { ok: false, errors: ['Rotate ' +
            'what? `which` is x509, jwt or both.'] });
      }
      ...
      log.debug("Leaving AdminActions.spiffeAction(). Rotated.");
      return { ok: true, message: 'Rotated: ' + done.join(' and ') + '. The ' +
        'previous authority is still published in the bundle, so SVIDs ' +
        'already issued go on verifying; the bundle sequence is now ' +
        spiffeCa.sequence() + '.' };
    }
    ...
    if (action === 'federation-set') {
      ...
      if (!name) {
        log.debug("Leaving AdminActions.spiffeAction(). No trust domain.");
        return this.refused('STS-ADMIN-0579', { ok: false,
                                                errors: ['Which trust ' +
                                     'domain? Send `trustDomain` with its ' +
                                          'name — other.example, not ' +
      ...
      const result = spiffeCa.setFederatedBundle(name, body.document, {
        ...
        bundleEndpointProfile: String(body.bundleEndpointProfile ||
                                      'https_web'),
        endpointSpiffeId: String(body.endpointSpiffeId || '')
      });
      if (!result.ok) {
        log.debug("Leaving AdminActions.spiffeAction(). Refused.");
        return this.refused(this.innerCode(result) || 'STS-ADMIN-0580',
                            { ok: false, errors: [result.reason] });
      }
      auditLog.audit({ action: 'spiffe.bundle.change', actor: '',
                    protocol: 'SPIFFE',
                    channel: 'internal', target: name,
                    summary: 'A federated bundle for ' + name + ' was set ' +
                             'from the console',
                    detail: { created: result.created } });
      log.debug("Leaving AdminActions.spiffeAction(). Federated bundle set.");
    ...
    if (action === 'federation-remove') {
      ...
      if (!removed) {
        log.debug("Leaving AdminActions.spiffeAction(). Not held.");
        return this.refused('STS-ADMIN-0581', { ok: false, errors: ['This ' +
                                          'service holds no bundle for the ' +
                                          'trust domain ' + name + '.'] });
      }
      auditLog.audit({ action: 'spiffe.bundle.change', actor: '',
                    protocol: 'SPIFFE',
                    channel: 'internal', target: name,
                    summary: 'A federated bundle for ' + name + ' was ' +
                             'removed from the console', detail: {} });
      log.debug("Leaving AdminActions.spiffeAction(). Removed.");
      return { ok: true, message: 'The bundle for ' + name + ' is gone. Any ' +
        'entry that federates with it keeps the name and simply contributes ' +
        'no bundle, which is the same state as a relationship configured ' +
        'before its bundle arrives.' };
    }

    log.debug("Leaving AdminActions.spiffeAction(). Unknown action.");
  ...
  kerberosPrincipalsAction(body, context) {
    ...
    if (action === 'create-service') {
      ...
      log.debug("Leaving AdminActions.kerberosPrincipalsAction(). Unknown " +
                "action.");
      return this.refused('STS-ADMIN-0602', { ok: false,
                                              errors: ['Unknown action "' +
        action + '". There are ' +
        numberWord(KERBEROS_PRINCIPAL_ACTIONS.length) +
        ': ' + KERBEROS_PRINCIPAL_ACTIONS.join(', ') + '.'] });
    }
    if (result && result.ok) {
      result.trustRealm = realms.currentId();
    }
    log.debug("Leaving AdminActions.kerberosPrincipalsAction(). ok=" +
              !!(result && result.ok));
    return this.refusedBy('STS-ADMIN-0602', result);
  }
  ...
  tokenLifetimeKeys() {
    ...
    log.debug("Leaving AdminActions.tokenLifetimeKeys().");
  ...
  samlAssertionKeys() {
    ...
    log.debug("Leaving AdminActions.samlAssertionKeys().");
