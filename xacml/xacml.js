...
// argues that at the role. `XACML_API` rather than `XACML_PEP_API`, because an
// operator narrowing one surface and not the other needs two names to target,
// and a policy layer that cannot express the distinction its own register
// makes is one somebody will work around.
//
...
// no verified DN, holds only `EVERYBODY`, and is refused ON THE POLICY — which
// is the correct and visible outcome rather than a second refusal spelt
...
// ===========================================================================
function xacmlAccess(req, res, action, what) {
  log.debug('Entering xacmlAccess(). action=' + action);
  const identity = callerIdentity(req);
  // THE NAME IS THE VERIFIED DN OR NOTHING AT ALL, for `pepAccess()`'s reason:
  // an unverified certificate's DN is a name the caller chose for itself, and
  // handing one to `rolesOf()` would be an unauthenticated caller naming
  // itself into a group.
  ...
  if (policy.allowed) {
    log.debug('Leaving xacmlAccess(). Allowed.');
  ...
  fail(res, 403, 'access_denied',
    ...
    String(config.value('roles.xacmlUserGroup') || '') + '"), which is what ' +
  ...
  log.debug('Leaving xacmlAccess(). Refused.');
...
app.post('/xacml/pdp', function (req, res) {
  ...
  // WRITE AND NOT READ, which is the one action choice in this family that is
  // arguable. Nothing in the repository changes here — but a decision request
  // is a document this service PARSES and EVALUATES on its own thread against
  // every policy in the repository, which is work a caller directs rather than
  // a page a caller reads. Filing it as `read` would put it in the same bucket
  // as fetching the policy list, and an operator writing "readers may look,
  // nobody may drive the engine" would have no way to say it.
  if (!xacmlAccess(req, res, accessGate.ACTION.WRITE, 'a decision request')
        .allowed) {
  ...
  // `pdp` row on that page carries the four decision counts and an EMPTY
  // allowed/refused cell rather than a zero. A zero there would read as "it
  // refused nothing", which is a claim about an enforcement this service was
  // not present for.
...
app.get('/xacml/policies', function (req, res) {
  ...
  if (!xacmlAccess(req, res, accessGate.ACTION.READ, 'a policy listing')
        .allowed) {
...
app.get('/xacml/protected', function (req, res) {
  ...
  // enforcement below asks whether the SUBJECT NAMED IN `?subject=` may reach
  // the resource — which is the thing this endpoint exists to demonstrate and
  // is not an authentication of anybody.
  //
  ...
  // admission was about the connection. Conflating them would turn this into
  // an endpoint that reports what the caller already knows.
  // ---------------------------------------------------------------------
  if (!xacmlAccess(req, res, accessGate.ACTION.READ,
...
// `pepAccess()` went in front of that endpoint, in this same file, on the same
// day. Both go through the four-step chain below now, and the four endpoints
// at the top of this file go through `xacmlAccess()`, which is the same chain
// with `XACML_USER` on the end.
//
// **WHAT DID NOT CHANGE IS THE ARGUMENT.** A PDP is still not an authorization
// boundary and the identity that matters is still IN the request — the gate
// decides who may ASK and nothing it learns reaches `decide()`. A policy is
// still a rule, and a rule nobody can read is a rule nobody can check; the
// documents are still served in full to anybody the policy admits. What
// changed is that *who may read it* turned out to be a different question from
// *is it redacted*, once these became the documents this service decides its
// own admissions with.
//
...
function callerIdentity(req) {
  ...
  // PRESENTED AND VERIFIED, KEPT APART (2026-09-06). `authenticated` has always
  // meant "a certificate arrived" on this door, and it still does — RFC 8705
  // binding rests on the certificate rather than on anybody's opinion of it,
  // and `xacml.pepRequireCertificate` was written against that meaning.
  // `verified` is the new, stronger question: did the chain build to an anchor
  // in this service's truststore. **THE DIFFERENCE IS WHAT MAKES A DN WORTH
  // READING**: a name off an unverified certificate is a name the caller chose
  // for itself, and the identity resolution below must not rest on one.
...
// AN IDENTITY.** Four steps, and each is a different question that a different
// part of this service answers:
//
...
//      quietly answer no to every group-derived role — REMOTE_PEPS among them.
...
//      the request rather than being tested here, so an operator who edits that
//      policy changes this answer too.
//
// **WHY THE GATE RATHER THAN AN `if` ON THE ROLE.** Because the whole design of
// this family is that access decisions are POLICY decisions, and a hard-coded
// role test on three endpoints would be the one surface in this service whose
// answer no document explains and no `/admin/xacml/decide` reproduces.
//
// WHAT IT DOES NOT DO: it does not require a certificate. That is
// `xacml.pepRequireCertificate`'s decision, it is made at the registration, and
// it can be turned off — a deployment that has turned it off has said it wants
// unauthenticated PEPs, and this gate then sees an anonymous subject holding
// only EVERYBODY and refuses it on the policy, which is the correct and
// visible outcome rather than a second refusal spelt differently.
// ===========================================================================
function pepAccess(req, res, action, what, known) {
  log.debug('Entering pepAccess(). action=' + action);
  // `known` IS AN OPTIONAL PRE-RESOLVED IDENTITY, and it exists for exactly one
  // caller: `POST /xacml/pip` rate-limits on the DN, so it has to know who is
  // asking BEFORE it asks whether they may. Reading the certificate twice would
  // be a second `getPeerCertificate()` and a second DN parse per request — and,
  // worse, two places that could come to disagree about who the caller is.
  // Omitting it is what the other three doors do and is unchanged.
  const identity = known || callerIdentity(req);
  // THE NAME IS THE VERIFIED DN OR NOTHING AT ALL. An unverified certificate's
  // DN must never reach `rolesOf()`: it would be an unauthenticated caller
  // naming itself into a group, which is the one thing this whole chain exists
  // to prevent.
  const name = identity.verified ? identity.dn : '';
  const held = name
    ? roles.rolesOf({ kind: 'user', name: name, authenticated: true })
  ...
  if (policy.allowed) {
    log.debug('Leaving pepAccess(). Allowed.');
    return { allowed: true, identity: identity, roles: held };
  }
  log.info('xacml: the access policy refused ' + what + ' for ' +
           (name || 'an unidentified caller') + '. ' + policy.why);
  ...
  fail(res, 403, 'access_denied',
    ...
    'presenting a client certificate this service VERIFIES — the issuing CA ' +
    'goes in with POST /tls/trust — whose subject resolves to a directory ' +
    'entry that is a member of the group named by roles.remotePepGroup ("' +
    String(config.value('roles.remotePepGroup') || '') + '"), which is what ' +
    ...
    (identity.verified
      ...
      : (identity.authenticated
          ...
          : 'presented no client certificate at all.')) +
    ' xacml.enforceAccess turns this layer off.');
  log.debug('Leaving pepAccess(). Refused.');
  return { allowed: false };
}
...
app.post('/xacml/pep/register', function (req, res) {
  ...
  // caller refused by both should hear about the policy, because that is the
  // one an operator changes.
  const access = pepAccess(req, res, accessGate.ACTION.WRITE,
                           'a remote PEP registration');
  if (!access.allowed) {
    log.debug('Leaving POST /xacml/pep/register. The access policy refused.');
    return;
  }
  ...
  if (required && !identity.authenticated) {
    ...
    fail(res, 401, 'invalid_client', plain
      ? 'This registration arrived on a PLAIN HTTP connection, which cannot ' +
        ...
        'unregistered PEP that can pull still decides — but the PULL itself ' +
        'needs a verified certificate holding REMOTE_PEPS, so a plain-HTTP ' +
        'deployment needs xacml.enforceAccess off as well.'
  ...
  res.status(result.created ? 201 : 200).type('application/json')
     ...
     .send(JSON.stringify({
       ...
       authenticated: identity.authenticated,
       // PRESENTED AND VERIFIED, ANSWERED SEPARATELY (2026-09-06). A PEP that
       // reads only `authenticated` learns that its certificate arrived; this
       // is the one that says the chain built to an anchor this service holds,
       // which is what everything downstream of the DN rests on. Said back in
       // the reply rather than left to be discovered, for the same reason the
       // notify verdict is: a PEP whose certificate this service will never
       // recognise should find out while somebody is still looking at the
       // deployment.
...
// the argument: this header said "no credential is required" for one day after
// that stopped being true, which is exactly the drift the endpoint list on
// `GET /xacml` now makes machine-readable.
// ---------------------------------------------------------------------------
app.get('/xacml/pep/policies', function (req, res) {
  ...
  // **THIS ENDPOINT USED TO NEED NO CREDENTIAL AT ALL AND THE OLD ARGUMENT FOR
  // THAT IS STILL SOUND** — a policy is a rule, and a rule nobody can read is
  // a rule nobody can check. What changed is not the value of publishing
  ...
  if (!access.allowed) {
    log.debug('Leaving GET /xacml/pep/policies. The access policy refused.');
    return;
  }
  ...
  const root = store.root();
  // ===========================================================================
  // THIS SERVICE'S OWN POLICIES ARE NOT PUSHED (2026-09-06), AND THEY ARE THE
  // ONLY THINGS EVER WITHHELD FROM A PEP.
  //
  // Two documents in this repository are not about somebody else's boundary at
  // all — they are how THIS SERVICE decides its own questions:
  //
  ...
  // remote decision a Deny, which is the exact shape of the defect
  // `xacml-pep/CLAUDE.md` records having cost a run already.
  //
  // The second reason is the ordinary one and it is worth saying too: these
  // documents state the conditions under which this service lets people in, and
  // handing them to every registered enforcement point publishes that. They
  // remain readable at `GET /xacml/policies`, which is where a PERSON reads the
  // repository and where "a rule nobody can check is a rule nobody can trust"
  // still applies.
  //
  // FILTERED BY NAME AND NOT BY A FLAG ON THE ENTRY, because the two names are
  // SETTINGS: an operator who points `xacml.accessPolicy` at a document of
  // their own has made THAT one internal, and a stored flag would still be on
  // the old one. Reading the settings is reading the same answer
  // `xacml_access_pep.js` and `xacml_role_pep.js` read.
  // ===========================================================================
  ...
  res.status(200).type('application/json').set('Cache-Control', 'no-store')
     ...
     .send(JSON.stringify({
       ...
       policies: rows,
       // NAMED RATHER THAN SILENTLY ABSENT. A PEP whose policy count disagrees
       // with the console's is exactly what that column exists to show, and a
       // reader comparing the two needs to know the difference is deliberate.
       // The names are all that is published — never the documents.
       withheld: withheld,
       note: 'Every ENABLED policy EXCEPT this service\'s own. A disabled ' +
             ...
             'readable at GET /xacml/policies.' +
             (withheld.length
...
app.post('/xacml/pep/heartbeat', function (req, res) {
  ...
  if (!access.allowed) {
    log.debug('Leaving POST /xacml/pep/heartbeat. The access policy refused.');
    return;
  }
  const body = parseBody(req);
  const identity = access.identity;
...
// service's PIP *is* the embedded directory (`xacml_pip.js`'s first sentence),
// and a process in another container has no access to it. So a policy with an
// attribute designator the request did not carry resolves to an EMPTY BAG out
// there and to a real value in here, and **the same policy decides two
// different ways in two enforcement points** — which is the drift a shared
// repository exists to prevent, reappearing one layer down.
//
...
//
// ---------------------------------------------------------------------------
// THAT RESPONSE SHAPE IS THE WHOLE DESIGN, AND IT IS WHAT MAKES THE REMOTE PEP
// BEHAVE LIKE THE EMBEDDED ONE.
//
...
// whether an empty bag ends a decision is settled by the designator and by the
// function the bag is handed to, and both of those are in the CALLER's engine.
// Applying it here would move a decision across a network boundary and answer
// a question nobody asked — and it would make an absent attribute an ERROR on
// the wire, which is the classic PIP defect in its most damaging form.
//
// ---------------------------------------------------------------------------
// `<Unresolved>` IS THE ONE THING HERE THAT IS NOT XACML, AND IT IS OUT OF THE
// WAY ON PURPOSE.
//
// A bag can be empty for five reasons that need five different fixes, and to a
// PDP they are one empty bag and must be. That is correct for deciding and
// useless for debugging — `xacml_pip.js` logs the difference at debug level,
// in a log that is in another container as far as the caller is concerned.
//
// So the reasons come back in a `<Unresolved>` element in **this service's own
// namespace**, a sibling of the `<Attributes>` rather than inside them. A PEP
// that reads only the XACML core namespace — which is every PEP — never sees
// it, so the payload stays a clean request fragment; a person or a PEP that
// wants to know why finds it named. Adding a diagnostic INSIDE the core
...
// entry, named by whoever asked. That is a strictly stronger thing to hand out
// than a policy document: a policy is a rule anybody may check, and `mail`,
// `employeeType` and `departmentNumber` are facts about a person. The narrower
// role is the right one, and the caller this was built for — a remote
// enforcement point resolving a designator mid-evaluation — holds it already.
//
...
// decision read from the other side: the switch follows the CALLER rather than
// the name, so a deployment that has said it wants no enforcement point
...
// This service's own namespace, for the two elements XACML does not define.
// It is NOT the core namespace, and that separation is the `<Unresolved>`
...
function pipWhy(designator, subject, stored, mapped) {
  ...
  return 'The entry for "' + subject + '" holds ' + count + ' value(s) for "' +
         ...
         'Indeterminate: one bad value among five must not take the other ' +
         'four with it.';
}
...
// were asked in, because that is what a `<Request>` looks like and a caller is
// invited to splice this straight into one. Several `<Attributes>` of the same
// category IS legal — it is how the Multiple Decision Profile's scheme 2.3
// works — but a fragment that used it here would be a request fragment shaped
...
function writePipResponse(answers, unresolved) {
  ...
  byCategory.forEach(function (group) {
    ...
    group.attributes.forEach(function (answer) {
      ...
      // in process, and a PEP splicing this into a request it then echoes must
      // not start reporting this service's directory contents back to its own
      // callers.
  ...
  if (unresolved.length) {
    ...
    unresolved.forEach(function (one) {
      parts.push('    <Designator Category="' + xmlEscape(one.category) +
                 ...
                 '" MustBePresent="' + (one.mustBePresent ? 'true' : 'false') +
                 '">');
...
//
// `xacml_xml.js` reads a `<Request>` and an `<AttributeDesignator>` and is held
// to 454 of 455 OASIS conformance cases, so what a DESIGNATOR is is not this
// file's to re-check — a schema over it would be a second, worse reading of a
// specification this directory implements. What those readers do NOT do is
// bound a string: `requiredAttribute()` returns whatever the attribute holds,
// and a conformance suite has no opinion about an AttributeId a megabyte long.
//
// **THREE OF THESE COME BACK OUT AGAIN**, which is what makes the bound matter
// rather than being tidiness. An unresolved designator is echoed into
...
// caller choosing the length chooses how much of somebody else's audit log and
// this process's memory it spends.
//
// The caps are `validation.js`'s own: an AttributeId and a category are
// IDENTIFIERs (they are URIs in practice, but a URI cap of 2048 on a name this
// service only ever compares and echoes buys nothing), and a subject is a NAME
...
app.post('/xacml/pip', async function (req, res) {
  ...
  // `/xacml/pdp` is named for the PDP — and the first draft therefore put it
  // behind `xacml.enabled` alone, on the reasoning that a PIP is not part of
  ...
  // needs to know whether they may — two questions about one certificate, and
  // reading it twice would be two places that could come to disagree about
  // the answer. `pepAccess()` takes it as an argument for exactly this door.
  // ---------------------------------------------------------------------
  const identity = callerIdentity(req);
  ...
  // certificate chain, resolve a DN and evaluate an access policy as fast as
  // it could send — refused every time and never counted. Put here, the
  // ADDRESS bucket bounds that, and the IDENTITY bucket bounds the case that
  // actually costs something: a caller this service ADMITTED reading
  ...
  // several a second and every one is legitimate — sharing the sign-in number
  // would have switched this endpoint off for its only caller, and it would
  // have done it SILENTLY, because `xacml-pep/pip.js` treats a refused query
  // as an empty bag and goes on deciding. `xacml.pipMaxPerWindow` is that
  // number and `websecurity.js` argues the argument that carries it.
  //
  ...
  if (!within.ok) {
    ...
    pipFail(res, 429, 'too_many_requests',
      ...
      'security.rateLimitWindowS). A remote PEP that meets this is querying ' +
      'per decision with no caching — and note that it treats a refusal as ' +
      'an empty bag and goes on deciding on less information, so raising the ' +
      'limit is a real change rather than a cosmetic one.');
    return;
  }
  ...
  // `app.js` parses every body as TEXT, which is exactly what an XML endpoint
  // wants — the same reason `/xacml/pdp` reads the raw source.
  const raw = typeof req.body === 'string' ? req.body
    : (Buffer.isBuffer(req.body) ? req.body.toString('utf8') : '');
  let request;
  let designators;
  try {
    ...
    // process's memory one request costs; and it THROWS, which is fine inside
    // a policy load and is one more thing to catch on a request path.
    // `validation.js`'s wrapper caps at `CAP.LARGE` and answers a refusal,
    // and it is the same function every other XML door in this service reads
    // a body with — so the ceiling is one number in one place.
    //
    // **ENTITY EXPANSION IS NOT A HAZARD HERE AND THAT IS MEASURED RATHER
    // THAN ASSUMED.** `@xmldom/xmldom` resolves no entity declared in a DTD,
    // internal or external: a billion-laughs document and an
    // `<!ENTITY xxe SYSTEM "file:///etc/passwd">` both come back as *entity
    // not found*, refused as not well-formed. So there is no expansion limit
    // to set and no external resolver to disable — and this paragraph is here
    ...
    if (xml.localName(root) !== 'PIPRequest') {
      throw model.syntaxError(
        'A PIP query\'s root element must be <PIPRequest> in the namespace ' +
        PIP_NS + '; this one is <' + xml.localName(root) + '>. It carries ' +
    ...
    // means the same thing on this wire as it does in the document the PEP is
    // evaluating — including the reading of MustBePresent, where absent and
    // false are recorded as different things.
    designators = nodes.map(function (node) {
      ...
      //
      // `readExpression()` is held to 454 of 455 OASIS conformance cases, so
      // what a designator IS is not this file's to re-check — a schema over it
      // would be a second, worse reading of a specification this directory
      // implements. What a conformance suite has no opinion about is an
      // AttributeId a megabyte long.
      //
      // **THESE THREE COME BACK OUT AGAIN**, which is what makes the bound
      // matter rather than being tidiness: an unresolved designator is echoed
      // into `<Unresolved>`, every one of them is named in the
      // `xacml.pip.query` audit row, and both are places a caller does not get
    ...
    // matters more here: an unresolved designator is a legitimate ANSWER, so a
  ...
  // THE SUBJECT IS BOUNDED TOO, and it is the one scalar that goes somewhere
  // other than a log: `locateSubject()` hands it to the directory's
  // `locateEntry()`, which walks the tree comparing it against every DN, every
  // uid and every certificate subject. It is a NAME by this service's own
  // reckoning — the same cap `userFor()` puts on one — and refusing a longer
  // one here is refusing it at the only door that takes it from a stranger.
  const subjectProblem = pipScalarProblem('subject-id',
  ...
  // RESOLVED ONCE, HERE, for the reason `resolverFor()` resolves its own copy
  // once per decision rather than once per designator: a caller asking about
  // six attributes of one person must not be able to see six different people
  // because somebody wrote to the directory in between. This is the same
  // lookup through the same function, and it exists only so `<Unresolved>` can
  // say which of the five reasons applied.
  ...
  designators.forEach(function (designator) {
    ...
    if (values.length) {
      answers.push({
        ...
        // THE LEXICAL FORM. `writeValue()` is `parseValue()`'s exact inverse
        // and is the one function that knows how each datatype is written;
        // anything else here would be a second, worse spelling of a dateTime.
    ...
    unresolved.push({
      ...
      why: pipWhy(designator, subject, stored, mapped)
    });
  });
  audit.audit({
    action: 'xacml.pip.query',
    // THE PEP THAT ASKED, not the subject it asked about. Two identities are
    // in this request and the audit row is about who reached the endpoint —
    // the subject is in the detail, which is where a reader looking for "who
    // has been asking about alice" will search.
  ...
  res.status(200).type('application/xml').set('Cache-Control', 'no-store')
     .send(writePipResponse(answers, unresolved));
    ...
    endpoints: [
      ...
      { method: 'POST', path: '/xacml/pip',
        what: 'the Policy Information Point, in XACML\'s own XML: POST a ' +
              '<PIPRequest> carrying the <Request> you are deciding and one ' +
              '<AttributeDesignator> per attribute, get <Attributes> back in ' +
              'the shape a <Request> carries them. For a remote PEP, whose ' +
              'engine has no directory of its own',
        requires: REMOTE_PEP_ROLE }
    ],
...
app.get('/xacml', function (req, res) {
  ...
  res.status(200).type('text/html').set('Cache-Control', 'no-store').send(
    ...
    '<p>' + (info.access.enforced
      ? 'Every endpoint above requires a <strong>client certificate</strong> ' +
        'this service verifies against an anchor in its own truststore ' +
        '(<code>POST /tls/trust</code>). The subject DN is resolved to a ' +
        'directory entry, and that entry&rsquo;s group membership is what ' +
        'grants the role in the last column: <code>' +
        xmlEscape(info.access.xacmlUserGroup || '(none — nobody holds it)') +
        '</code> grants <code>XACML_USER</code> and <code>' +
        xmlEscape(info.access.remotePepGroup || '(none — nobody holds it)') +
        '</code> grants <code>REMOTE_PEPS</code>. <strong>The certificate ' +
        'says who and the group says whether</strong> &mdash; a perfectly ' +
        'valid certificate for a common name in neither group is fully ' +
        'authenticated and still refused.'
      : 'Access enforcement is <strong>off</strong> ' +
        '(<code>xacml.enforceAccess</code>), so every endpoint above answers ' +
        'without a credential. The roles in the last column are what would ' +
        'be required with it on.') + '</p>' +
