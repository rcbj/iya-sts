...
// ENTRY is already an attribute source, because `xacml_pip.js` IS the embedded
...
// place `xacml_role_pep.js` looks: the five surfaces above are ordinary entries
...
// the reason `xacml_role_pep.js` records at length: `ou=policies` is per realm,
// and a policy seeded once in the default realm leaves every realm created
...
function builtInPolicy() {
  ...
  return { policy: built.policy, name: accessPolicyName(), builtIn: true };
}

// ---------------------------------------------------------------------------
// **THE OVERRIDE HAD NEVER ONCE BEEN PICKED UP, AND THAT IS WHAT THIS FUNCTION
// LOOKED LIKE (2026-09-06):**
//
...
// the PDP resolves `PolicyIdReference` through — so it has no `get` method at
// all and `typeof repository.get === 'function'` is false on every call. The
// guard was written as a defensive one and was in fact the whole condition:
// `found` was always null, the built-in document always answered, and
// `xacml.accessPolicy` was a documented setting, offered on the console, that
// did nothing. Two further mistakes were hiding under it — the name is an ENTRY
// name and that map is keyed by PolicyId, and a value in it is a parsed policy
// rather than a row with an `enabled` flag — so even a `get` would have missed.
//
// It reads the entry the way `xacml_role_pep.js`'s `issuancePolicy()` does,
// which is the function this one's header has always claimed to follow.
//
// **AND THE DISABLED CASE NOW BEHAVES AS THAT FUNCTION DOES, which is a change
// rather than a fix.** It used to fall back to the built-in document. That
// makes the console's Disable button mean "evaluate something else instead",
// which is the exact thing `issuancePolicy()` refuses to do and calls a lie —
// and the two behaving oppositely was invisible, because the branch was
// unreachable. Falling back is also not what keeps an operator safe here:
// `decide()` below ALLOWS when no policy is loaded, precisely so that a broken
// or absent policy cannot close the door on the console that is the only place
// to fix it. So disabling the access policy now means what the button says —
// this layer stops deciding, every gated surface behaves as it did before the
// policy existed, and it is logged at error level rather than passing quietly.
// ---------------------------------------------------------------------------
function accessPolicy() {
  log.debug('Entering accessPolicy().');
  const name = accessPolicyName();
  const row = store.read(name);
  if (!row) {
    log.debug('Leaving accessPolicy(). Using the built-in one.');
  ...
  try {
    ...
    log.debug('Leaving accessPolicy(). A repository entry answers.');
...
// **`{ type, lexical }` AND NOT `{ dataType, value }`.** The engine's attribute
// value carries the datatype as `type` and the unparsed text as `lexical`, and
// the plausible-looking names are the trap: a value built the other way is not
// rejected, it simply never matches, and the decision comes back as a clean
// Deny with no error in it at all. `issuer` and `includeInResult` are part of
// the shape too. This is `xacml_role_pep.js`'s helper, and the duplication is
...
//   RESOURCE         what is being reached, whose it is, and what it requires.
...
function buildRequest(asked, held, required) {
  ...
  return {
    ...
    // CATEGORY IS PRESENT EVEN THOUGH IT IS EMPTY.** Both are required by the
    // engine and neither is obvious: a request built without them evaluates to
    // Indeterminate with `Cannot read properties of undefined (reading
    // 'forEach')`, which under deny-unless-permit is a Deny — so every surface
    // refused everybody and the reason named the policy rather than the
    // request. `xacml_role_pep.js` builds the same four; this is the second
    // caller and the shape is the contract.
    ...
    // the two names one level apart are exactly the trap: a request built with
    // `attributes` at the top is not malformed enough to be rejected — it
    // evaluates to Indeterminate with `Cannot read properties of undefined
    // (reading 'forEach')`, which under deny-unless-permit is a Deny. So every
    // surface refused everybody, and the reason named the POLICY rather than
    // the request that never reached it.
    categories: [
      { category: model.CATEGORY.ACCESS_SUBJECT, id: null, content: null,
        attributes: [
          attribute(model.ATTRIBUTE.SUBJECT_ID, [subject.name || '']),
          attribute(ATTRIBUTE.ROLE, held),
          // A BOOLEAN and not a string, so a policy compares it with
          // `boolean-equal` rather than against the text "true" — which is the
          // kind of thing that works until somebody writes "True".
          attribute(ATTRIBUTE.AUTHENTICATED,
                    [subject.authenticated === false ? 'false' : 'true'],
                    model.TYPE.BOOLEAN)
        ] },
      { category: model.CATEGORY.RESOURCE, id: null, content: null,
        attributes: [
          attribute(model.ATTRIBUTE.RESOURCE_ID, [asked.resource]),
          ...
          // ownerless surfaces and the portal at once. `attribute()` drops an
          // empty string, so `asked.owner || ''` produces that empty bag
          // rather than a bag holding one value nothing can ever equal.
          attribute(ATTRIBUTE.OWNER, [asked.owner || ''])
        ] },
      { category: model.CATEGORY.ACTION, id: null, content: null,
        attributes: [
          attribute(model.ATTRIBUTE.ACTION_ID, [asked.action])
        ] },
...
function decide(asked) {
  log.debug('Entering decide(). resource=' + asked.resource +
            ' action=' + asked.action);
  ...
  // `admin_rbac.js` decides and this does not take over. Unioned in rather than
  // replacing, so a policy can be written against either.
  ...
  if (!loaded.policy) {
    // **ALLOWED, AND THIS IS THE OPPOSITE OF WHAT THE ISSUANCE PEP DOES WITH A
    // MISSING POLICY** — worth understanding rather than reading as an
    // inconsistency. There, a narrowed application is REFUSED because somebody
    // deliberately restricted it and the restriction cannot be evaluated.
  ...
  const request = buildRequest(asked, held, required);
  const answer = pdp.evaluate(loaded.policy, request, {
    repository: store.repository(),
    // THE SAME PIP THE PUBLIC ENDPOINT USES — see the same note in
    // xacml_role_pep.js. A decision made here against a different attribute
    // source from the one `/xacml/pdp` and `/admin/xacml/decide` use would be
    // a decision nobody could reproduce on the page built to reproduce it.
    resolver: pip.resolverFor(request)
  });

  if (answer.decision === model.DECISION.PERMIT) {
    log.debug('Leaving decide(). Permit.');
    return allowed('The access policy permitted it.', answer);
  }

  // The three refusals mean different things to whoever has to fix one, so the
  // sentence says which — the same division the issuance PEP makes.
...
// ARMING THE GATE, at require time, which is what `xacml.js` requiring this
// module at 23c does. Before that line every access decision is allowed.
gate.setDecider(decide);
log.info('xacml: the embedded access PEP is armed. Every access decision in ' +
         'this service — the admin console, the management API, the User ' +
         'Portal, SCIM and the SPIRE Server API — is now a XACML decision ' +
         'against the "' + accessPolicyName() + '" policy, with the SUBJECT ' +
         'taken from the session\'s security context and the subject\'s own ' +
         'directory entry available to the policy through the PIP.');
...
// override ENTRY exists is a different question from whether one is DECIDING,
// and `builtIn` alone cannot tell "nobody has written one" from "somebody
// wrote one and disabled it".
//
// **THE `disabled` CASE IS NOW THE SAME AS THE ISSUANCE PEP'S**, and it was
// not until 2026-09-06 — see `accessPolicy()` above, where the divergence and
// its removal are argued. What still differs is the CONSEQUENCE and that is
// deliberate: a disabled issuance policy REFUSES a narrowed application, and a
// disabled access policy ALLOWS every gated surface, because refusing there
// would close the console that is the only place to fix it. Two different
// answers to "what does not deciding mean here", from one answer to "is it
// deciding".
// ---------------------------------------------------------------------------
function accessPolicyState() {
  log.debug('Entering accessPolicyState().');
  const name = accessPolicyName();
  const loaded = accessPolicy();
  ...
  // different places: `entry`/`enabled` are facts about the DIRECTORY and come
  // from `store.read()`; `builtIn` is a fact about the DECISION and comes from
  // `accessPolicy()` above, which is what the PEP actually calls.
  const row = store.read(name);
  ...
  if (!out.ok) {
    ...
    out.effect = 'The repository entry "' + name + '" decides. It overrides ' +
  ...
  log.debug('Leaving accessPolicyState(). ' +
