...
// shape: `XacmlMonitor` takes the logger, the settings, the error-code
// registry, the realm registry, the replication reader, the PEP register, the
// catalogue, the counter store and the instant counting started through its
// constructor (`XacmlMonitorDeps`). The store is still declared at module
// scope, as `realms.map()`, because a store becomes per realm at its
...
// One asker of the PDP, as the catalogue describes it.
interface PepDefinition {
  id: string;
  label: string;
  kind: string;
  where: string;
...
// One asker's counters in this process.
...
// What an asker says about one decision.
interface Outcome {
  decision?: string;
  allowed?: boolean;
  undischargeable?: boolean;
}

// What the policy repository holds, as the caller hands it over.
...
interface XacmlMonitorDeps {
  ...
  peps: { all(): Json[] };
  catalogue: PepDefinition[];
  counters: CounterStore;
  // When the counting started: stamped at require time.
  startedAt: string;
}
...
class XacmlMonitor {
  ...
  constructor(private readonly deps: XacmlMonitorDeps) {
    ...
    deps.catalogue.forEach(function (row) {
      byId[row.id] = row;
    });
  ...
  // -------------------------------------------------------------------------
  // THE CATALOGUE. Every asker of the PDP in this process, in the order the
  // page draws them.
  //
  // `enforces: false` is the one row that is not a PEP and it is the reason
  // this is a table with a flag rather than two lists: everything else about
  // the `/xacml/pdp` row — its decision counts, its last decision, its place on
  // the page — is identical to a PEP's, and two lists would have meant two
  // renderers that could disagree about how a decision is displayed.
  //
  ...
  static catalogue(log: { debug(message: string): void },
    ...
    const rows: PepDefinition[] = [
      { id: 'protected',
        ...
        enforces: true,
        // THE ONLY ONE WHOSE BIAS IS SETTABLE, and the page says so beside
        // every other row: `xacml.pepBias` governs THIS PEP. The issuance and
        // access PEPs are deny-biased by construction — an issuance or an
        // access that is not permitted does not happen — and a setting that
        // appeared to change that would silently do nothing.
        bias: function () {
          log.debug("Entering bias().");
          log.debug("Leaving bias().");
          return settings.value('xacml.pepBias') === 'permit-biased'
      ...
      { id: 'issuance',
        ...
        guards: 'Everything this service MINTS: an access token, an ID ' +
                ...
                'verifiable credential, a sign-on session.',
        enforces: true,
        bias: function () {
          log.debug("Entering bias().");
          log.debug("Leaving bias().");
    ...
    log.debug("Leaving XacmlMonitor.catalogue().");
    return rows;
  }

  private emptyRow(): CounterRow {
    const { log } = this.deps;
    log.debug("Entering XacmlMonitor.emptyRow().");
    log.debug("Leaving XacmlMonitor.emptyRow().");
    return { decisions: 0, allowed: 0, refused: 0,
             permit: 0, deny: 0, notApplicable: 0, indeterminate: 0,
             other: 0,
             undischargeable: 0,
             lastAt: null, lastDecision: null, lastAllowed: null };
  }

  private rowFor(id: string): CounterRow {
    const { log, counters } = this.deps;
    log.debug("Entering XacmlMonitor.rowFor().");
    if (!counters.has(id)) {
      counters.set(id, this.emptyRow());
    }
    log.debug("Leaving XacmlMonitor.rowFor().");
    return counters.get(id);
  }

  // -------------------------------------------------------------------------
  // THE ONE CALL EVERY ASKER MAKES, AND IT IS PUT IN THE `allowed()`/
  // `refused()` FUNNEL RATHER THAN AT THE RETURN SITES.
  //
  // Each embedded PEP already has exactly two functions every one of its
  // answers passes through, which is what makes this two lines per module
  // instead of eleven — and, much more importantly, what makes a return path
  // added later counted BY CONSTRUCTION rather than by whoever adds it
  // remembering. That is the same argument `common/delegation.js`'s header
  // makes for its funnel, and the same one `crypto.js` makes about being the
  // one place this service signs.
  //
  // **IT MUST NEVER THROW INTO ITS CALLER.** Every call site is on the path of
  // an authorization decision — an issuance, a sign-in, a request to a gated
  ...
  record(id: string, outcome?: Outcome | null): void {
    ...
    try {
      if (!this.byId[id]) {
        ...
        // would otherwise appear as a row on a page whose whole claim is that
        // it lists every asker of the PDP in this process, and a row nothing
        // describes is worse than a missing one.
        log.warn(errorCodes.tag('STS-XACML-0063') +
                 'xacml: a decision was recorded against "' + id + '", ' +
                 'which is not one of ' +
                 'the ' + catalogue.length + ' askers xacml_monitor.js ' +
                 'knows about. It is NOT counted — /admin/xacml/monitor ' +
                 'claims to list every one of them, and a row with no ' +
                 'description would break that claim rather than extend ' +
                 'it. Add it to PEPS.');
        log.debug("Leaving XacmlMonitor.record().");
        return;
      }
      // -----------------------------------------------------------------
      // EVERYTHING IS READ BEFORE ANYTHING IS WRITTEN, AND THAT ORDER IS A
      // FIX RATHER THAN A STYLE.
      //
      // The first version incremented `decisions` and THEN read the outcome.
      // A caller whose object threw on a property access — which is exactly
      // what `tests/xacml_monitor.js` provokes, and what a getter, a Proxy or
      // a half-built object does in real life — left the row with the
      // decision counted and no bucket, no allowed and no refused. The row's
      // own arithmetic then no longer reconciled: `allowed + refused +
      // unenforced` was one short of `decisions`, permanently, with nothing
      // to say why.
      //
      ...
      const enforces = this.byId[id].enforces;
      ...
      row.decisions += 1;
      row[bucket] += 1;
      // `allowed` is only counted where the asker ENFORCES. For the PDP
      // endpoint it is absent rather than false, and the difference is the
      // point: an undefined enforcement is "this service never saw one", and
      // `false` would have been "it refused".
      if (enforces) {
        if (wasAllowed) {
          row.allowed += 1;
        } else {
      ...
      //
      // `rowFor()` hands back the live object and the lines above change it
      // in place, which a `realms.map()` cannot see: only `set()` and
      // `delete()` tell the persistence journal a key moved. So the row in
      // `sts_minted` held whatever the FIRST decision of the process wrote
      // through `rowFor()`'s `set()`, and never moved again. On one node
      // nothing reads that row. With two, each node's `merge()` adds its own
      // live tally to the OTHER node's frozen one, and the two pages disagree
      // about one service: the suite's `cluster` mode read the issuance PEP
      // at 126 on node A and 142 on node B across a page load that decided
      // nothing, and 21 then 17 in a run of that job alone — the number went
      // DOWN, which no late-arriving decision can explain.
      // `oauth2_monitor.js` always set its row back; this file did not.
      //
      // The store is declared `observation: true`, so this does not make the
      // access PEP's every console read a request the barrier holds; a
      // request that wrote anything else is held and the row commits with
      // it.
      // -----------------------------------------------------------------
      counters.set(id, row);
    } catch (error) {
      // SWALLOWED, and the comment is the reason rather than an apology: this
      // is on the path of every issuance and every gated request in the
      // service.
      log.error(errorCodes.tag('STS-XACML-0062') +
                'xacml: a decision counter threw and was ignored; the ' +
                'decision itself is unaffected: ' + error.message);
    }
    log.debug("Leaving XacmlMonitor.record().");
  }
  ...
  // /admin-api/xacml/monitor` answers, out of ONE call so the page and the
  // JSON cannot disagree — the rule every view in this console follows.
  //
  // `policies` comes through an argument rather than by requiring
  // `xacml_store.ts` here. Not to avoid a cycle (there is none) but because
  // this file is required by the access PEP, which is reached from `common/`:
  // keeping its require list to modules that register no route is what makes
  // it obviously safe to require from anywhere, and the caller already holds
  // the store.
  // -------------------------------------------------------------------------
  // -------------------------------------------------------------------------
  ...
  // PROCESS'S.
  //
  // `counters` is declared `merge: 'own'`: a decision count is an INCREMENT,
  // so two processes writing one row would make the page alternate between
  // their tallies while looking entirely plausible. Each writes its own and
  // the fan-in is here, in the one function that reports any of it.
  //
  ...
  private merge(id: string): CounterRow {
    ...
    log.debug("Entering XacmlMonitor.merge().");
    const mine = counters.has(id) ? counters.get(id) : this.emptyRow();
    const theirs = replication.remoteRows('xacml_monitor.counters',
                                          undefined, id);
    if (!theirs.length) {
      log.debug("Leaving XacmlMonitor.merge().");
      return mine;
    }
    const out: CounterRow = Object.assign(this.emptyRow(), mine);
  ...
  snapshot(policies?: PolicyCounts | null): Json {
    ...
    const { log, config, errorCodes, realms, peps, catalogue,
            startedAt } = this.deps;
    log.debug('Entering XacmlMonitor.snapshot().');
    const held = policies || { total: 0, enabled: 0, root: null };

    const rows = catalogue.map(function (definition) {
      const counted = self.merge(definition.id);
      return {
        id: definition.id,
    ...
    // The remote half, out of the register. `all()` already computes
    // `current` and `stale` per row against `syncToken()` and
    // `xacml.pepStaleAfterS`, so nothing about staleness is worked out twice.
    let remoteRows: Json[] = [];
    try {
      remoteRows = peps.all();
    } catch (error) {
      // A REGISTER THAT CANNOT BE READ MUST NOT TAKE THE PAGE DOWN. The
      // embedded half of this snapshot is this process's own memory and is
      // always answerable; the remote half needs the directory, and a build
      // with no `ldap_server.js` has none.
    ...
    const remote = remoteRows.map(function (row) {
      return {
        ...
        enforces: true,
        bias: row.bias || null,
        // ITS OWN COUNTS, in its own process, reported on a heartbeat. Named
        // the same as the embedded rows' so the page has one renderer, and the
        // page says on every remote row where the numbers came from.
    ...
    const here = this.totalOf(rows);
    const there = this.totalOf(remote);
    const json = {
      // THE GLOBAL SECTION. What the page's tiles are drawn from.
      policies: { total: held.total, enabled: held.enabled, root: held.root },
      peps: {
        embedded: rows.filter(function (row) {
          return row.kind === 'embedded';
        }).length,
        remote: remote.length,
        // The PDP endpoint is deliberately NOT in this figure. It is not a
        // PEP, and counting it would make "how many enforcement points are
        // there" answer one too many on a service with none.
      ...
      // WHEN THESE NUMBERS START FROM. A count with no epoch is a count
      // somebody will read as all-time.
      since: startedAt,
    ...
    log.debug('Leaving XacmlMonitor.snapshot(). ' + rows.length +
              ' local asker(s), ' + remote.length + ' remote PEP(s).');
    return json;
  }

  // -------------------------------------------------------------------------
  // THE TOTALS OVER A SET OF ROWS, AND THE FOURTH FIGURE THAT MAKES THEM ADD
  // UP.
  //
  // `allowed` and `refused` skip a row that does not enforce, which is the
  // whole reason those members are null rather than 0 there: `Number(null)` is
  // 0 and would have quietly added the PDP endpoint's non-existent refusals to
  // the refusal total.
  //
  // **BUT THEN `allowed + refused` IS LESS THAN `decisions`, AND A READER WHO
  // NOTICES THAT IS OWED AN ANSWER RATHER THAN A FOOTNOTE.** The gap is real
  // and it is exactly the decisions `POST /xacml/pdp` produced for somebody
  // else's PEP: this service evaluated them and never saw what was done with
  // them. So it is COUNTED, as `unenforced`, and the page draws it — which
  // turns "4 decisions, 1 allowed, 2 refused" from an arithmetic error a reader
  // has to resolve into three numbers and the one that explains them.
  //
  // The first draft left it out and the discrepancy showed up the first time
  // the page was driven with real traffic on it. A total that does not add up
  // is the kind of thing that makes somebody distrust every other number
  // beside it.
  // -------------------------------------------------------------------------
  private totalOf(rows: Json[]): Totals {
    const { log } = this.deps;
    log.debug("Entering XacmlMonitor.totalOf().");
    const out: Totals = { decisions: 0, allowed: 0, refused: 0,
                          unenforced: 0, undischargeable: 0 };
    ...
    log.debug("Leaving XacmlMonitor.totalOf().");
    return out;
  }
  ...
  // It clears THIS REALM'S counters and not every realm's, because that is
  // what `realms.map()`'s `clear()` means (`realmMap()` is the whole-map door)
  ...
  resetForTests(): void {
    ...
    counters.clear();
    log.debug('Leaving XacmlMonitor.resetForTests().');
  }
}
