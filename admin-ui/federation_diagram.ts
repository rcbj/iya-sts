'use strict';
//
// File: federation_diagram.ts
//
// ===========================================================================
// THE FEDERATION PICTURE. `federation/federation_graph.ts`'s graph, drawn.
//
// It is a LIBRARY, like `delegation_map.ts` beside it: it registers no route,
...
// one console.
//
// It is NAMED `federation_diagram.ts` AND NOT `federation_map.ts` for one
...
// shape: `FederationDiagram` takes the logger, the XML escaper, the error
// codes, `@dagrejs/dagre` and the three functions it borrows from
// `./delegation_map` (`textWidth`, `wrapLabel`, `hexPath`) through its
...
interface FederationDiagramDeps {
  log: typeof helpers.log;
  xmlEscape: typeof helpers.xmlEscape;
  errorCodes: typeof errorCodes;
  dagre: typeof dagre;
  textWidth: typeof delegationMap.textWidth;
  wrapLabel: typeof delegationMap.wrapLabel;
  hexPath: typeof delegationMap.hexPath;
}
...
// marker named after the old idea — `delegation_map.ts`'s rule, and its prefix
...
// `delegation_map.ts`'s where the two pictures draw the same thing, because two
...
class FederationDiagram {
  ...
  private markerId(colour) {
    const { log } = this.deps;
    log.debug("Entering FederationDiagram.markerId().");
    log.debug("Leaving FederationDiagram.markerId().");
    return 'fd-arrow-' + colour.replace('#', '');
  }

  private esc(v) {
    const { log, xmlEscape } = this.deps;
    log.debug("Entering FederationDiagram.esc().");
    log.debug("Leaving FederationDiagram.esc().");
    return xmlEscape(v == null ? '' : String(v));
  }

  private round(n) {
    const { log } = this.deps;
    log.debug("Entering FederationDiagram.round().");
    log.debug("Leaving FederationDiagram.round().");
    return Math.round(n * 10) / 10;
  }

  private trim(text, max) {
    const { log } = this.deps;
    log.debug("Entering FederationDiagram.trim().");
    const value = String(text == null ? '' : text);
    log.debug("Leaving FederationDiagram.trim().");
  ...
  //
  // `delegation_map.ts`'s `measure()` makes the argument and it holds here
  // unchanged: they are the same question asked twice, and two functions would
  // be two chances for the drawing to be a shape the layout did not reserve
  // room for.
  //
  ...
  //     identity provider. It is the same shape `/admin/delegation/map` uses
  //     for this service, deliberately — a reader who has learnt it there reads
  //     it here without being told.
  //   * A RECTANGLE IS A PARTY THAT CONSUMES WHAT AN IDENTITY SERVICE ISSUES:
  //     an application at this end, or a foreign service provider at the far
  //     end. Again the delegation picture's own vocabulary.
  ...
  //     local shape is. That is the same three-state rule the delegation
  //     picture follows for a party the directory has never heard of, narrowed
  //     to the one distinction that matters here.
  //
  // So a foreign identity provider is a DASHED HEXAGON and a foreign service
  // provider is a DASHED RECTANGLE, and the two facts a reader needs — what
  // kind of party is this, and is it ours — are carried by two independent
  // properties of the shape rather than by four shapes that have to be
  // memorised.
  // ---------------------------------------------------------------------------
  private measure(look) {
    const { log, textWidth, wrapLabel } = this.deps;
    log.debug("Entering FederationDiagram.measure().");
    ...
    const textH = lines.length * LINE_HEIGHT +
                  (subLines.length ? subLines.length * (LINE_HEIGHT - 2) + 2
                                   : 0);
    if (look.shape === 'hexagon') {
      log.debug("Leaving FederationDiagram.measure().");
      return {
        shape: 'hexagon',
  ...
  // ---------------------------------------------------------------------------
  // WHAT A BOX IS CALLED AND HOW IT IS DRAWN. One place, so that the picture
  // and its legend cannot come to disagree.
  //
  // `options.resolve` may override it — the console passes one so that a box
  // can link to that party's own page — but the SHAPE is decided here from the
  // node's kind and is not the caller's to change. A caller that could would
  // eventually draw a partner as a local box, which is the one distinction this
  // picture is built to carry.
  // ---------------------------------------------------------------------------
  private lookOf(node, options) {
    const { log } = this.deps;
    log.debug("Entering FederationDiagram.lookOf(). kind=" + node.kind);
    ...
    log.debug("Leaving FederationDiagram.lookOf().");
    return merged;
  }

  // ---------------------------------------------------------------------------
  // WHAT A LINE LOOKS LIKE, AND IT IS THE RELATIONSHIP'S STATE AND NOTHING
  // ELSE.
  //
  ...
  //                                      rather than a fault.
  //   * ENABLED AND NOT CONFIGURED       RED, dashed. THE LOUD ONE, and it
  //                                      earns being the only red on the page:
  //                                      it is a relationship that will REFUSE
  //                                      at the moment somebody tries to use
  //                                      it, and it is the state that looks
  //                                      finished from every angle except this
  //                                      one.
  //   * A BROKER THAT CANNOT BROKER      AMBER, dashed. The relationship
  //                                      itself is fine and its onward partner
  //                                      is not, so the person meets the
  //                                      password box instead of the partner —
  //                                      the silent fallback this whole
  //                                      feature is careful about, and the
  //                                      only failure here that produces a
  //                                      working sign-in.
  //
  ...
  // consequence is invisible everywhere else, and that is exactly what a
  // silently un-brokered relationship is.
  // ---------------------------------------------------------------------------
  private edgeLook(edge) {
    const { log } = this.deps;
    log.debug("Entering FederationDiagram.edgeLook().");
    const row = edge.row || {};
    if (row.enabled && !row.ready) {
      log.debug("Leaving FederationDiagram.edgeLook().");
  ...
  private useText(use) {
    const { log } = this.deps;
    log.debug("Entering FederationDiagram.useText().");
    if (!use) {
      log.debug("Leaving FederationDiagram.useText().");
      return '';
    }
  ...
  //
  // That cap is `delegation_map.ts`'s and the reason is its reason: a fourth
  // line was tried there and it is what turns a diagram into a page of text
  // laid out badly. What each line says differs by which of the three kinds of
  // line it is, and the three are the three claims the picture makes.
  // ---------------------------------------------------------------------------
  private edgeLabelLines(edge) {
    const { log } = this.deps;
    const self = this;
    log.debug("Entering FederationDiagram.edgeLabelLines().");
    const row = edge.row || {};
    const lines = [];
    if (edge.relation === 'signs-in') {
      // AN APPLICATION'S PEOPLE, THROUGH A NAMED RELATIONSHIP. The
      // relationship's id leads, because an application with two partners draws
      // two of these and the whole question is which is which.
      lines.push('via ' + self.trim(row.id, 24));
      const counts = self.useText(edge.use);
      if (counts) {
        lines.push(counts);
      }
      ...
      lines.push('asks · ' + self.trim(row.protocolLabel, 20));
      if (row.brokersTo) {
        lines.push('brokered to ' + self.trim(row.brokersTo, 20));
      } else {
        lines.push(self.trim(row.mechanismLabel, 32));
      }
      // -------------------------------------------------------------------
      ...
      //
      // What HAS happened for this partner is on `edge.use`: the brokered
      // pair's own row, carried onto this arrow by `federation_graph.ts`
      // precisely because the arrow it belongs to is drawn here rather than on
      // the onward relationship. A partner brokered through a
      // service-provider-side relationship really has had people signed in for
      // it, and that is the number.
      // -------------------------------------------------------------------
      const counts = self.useText(edge.use);
      if (counts) {
        lines.push(counts);
      ...
      // THIS SERVICE CONSUMING FROM A PARTNER. The relationship, what it
      // speaks, and HOW MANY APPLICATIONS ARE BEHIND IT — which is the number a
    ...
    log.debug("Leaving FederationDiagram.edgeLabelLines().");
    return lines.slice(0, 3);
  }

  // THE WHOLE STORY, on hover. Everything the three label lines had no room
  // for, and it is where the state sentence lives — the picture says the state
  // in COLOUR, which is fast to read and impossible to quote.
  private edgeTitle(edge) {
    const { log } = this.deps;
    log.debug("Entering FederationDiagram.edgeTitle().");
    const row = edge.row || {};
    const parts = [];
    if (edge.relation === 'signs-in') {
      ...
      if (edge.use && edge.use.configured === false) {
        parts.push('IT IS NO LONGER CONFIGURED TO: nothing names this pair ' +
                   'any more, and the counts below are what happened before ' +
                   'that changed. They are shown rather than dropped so that ' +
                   'the relationship\'s own totals still add up.');
      }
      ...
      parts.push('This foreign service provider asks this service to ' +
                 ...
                 row.id + '" (' + row.protocolLabel + '). This service ' +
                 'ASSERTS to it: the arrow is the request, not the ' +
                 'assertion.');
      ...
      if (edge.use && edge.use.authentications) {
        parts.push(edge.use.users + ' person(s) and ' +
                   edge.use.authentications +
                   ' sign-in(s) have been brokered for it through "' +
                   edge.brokeredTo + '". The relationship\'s OWN counters ' +
                   'read zero and always will: nothing increments them, ' +
                   'because what they count is assertions CONSUMED and this ' +
                   'side issues them.');
      }
      ...
      parts.push(row.applicationCount
        ...
        : 'NO application here is configured to authenticate through it. ' +
          'It is still reachable at /federation/login/' + row.id +
          ', which needs no configuration to reach.');
      if (row.unattributed) {
  ...
  // picture SAYING it could not be drawn — `delegation_map.ts`'s rule and its
  // reason: a drawing on a console page must not be able to take the page down.
  // Everything else on /admin/federation/map is built from the same graph
  // WITHOUT going through the layout, so the tables are worth reading with no
  ...
  render(graph: any, options?: any): Drawing {
    ...
    const self = this;
    log.debug("Entering FederationDiagram.render().");
    try {
      const svg = self.renderUnguarded(graph || { nodes: [], edges: [] },
                                       options || {});
      log.debug("Leaving FederationDiagram.render(). " + svg.svg.length +
      ...
      log.debug("Leaving FederationDiagram.render().");
      return {
  ...
  private renderUnguarded(graph: any, options: any): Drawing {
    const { log, dagre, textWidth } = this.deps;
    const self = this;
    log.debug("Entering FederationDiagram.renderUnguarded().");
    ...
    nodes.forEach(function (node) {
      const look = self.lookOf(node, options);
      const size = self.measure(look);
    ...
    //
    // `delegation_map.ts` has about a hundred lines that hand out rows and
    // lanes so that two labels do not land on top of each other. It needs them
    // because it took the coordinate pass away from dagre — every party is on
    // one centreline there, so dagre no longer knows where anything is and
    // cannot reserve room for anything.
    //
    // This picture keeps dagre's coordinates, so it can simply say how big each
    // label is: dagre gives an edge label a rank of its own and routes around
    // it. The result is the same guarantee for none of the code, and it is the
    // reason `ranksep` is generous — that gap is where the labels live.
    // ---------------------------------------------------------------------
    const labels = {};
    edges.forEach(function (edge) {
      ...
      const lines = self.edgeLabelLines(edge);
      let w = 0;
      lines.forEach(function (one) {
    ...
    const edgeMarkup = edges.map(function (edge) {
      ...
      return self.edgeMarkupFor(edge, laid, labels[edge.id], prefix);
    }).join('');
    ...
    const svg = '<svg xmlns="http://www.w3.org/2000/svg" ' +
      ...
      'sans-serif"><title>' +
      self.esc(options.label || 'Federation relationships') +
      '</title>' +
      defs + edgeMarkup + nodeMarkup + '</svg>';

    log.debug("Leaving FederationDiagram.renderUnguarded(). " + nodes.length +
              " box(es), " + Object.keys(labels).length + " line(s).");
    return { svg: svg, width: width, height: height,
             nodes: nodes.length, edges: Object.keys(labels).length,
             failed: '' };
  }

  // A line, its arrowhead and its words. dagre hands back the points; the path
  // is a simple polyline through them rather than a spline, because a spline
  // through four points that dagre already routed around the boxes buys nothing
  // and can bow back INTO one.
  private edgeMarkupFor(edge, laid, label, prefix) {
    const { log } = this.deps;
    const self = this;
    log.debug("Entering FederationDiagram.edgeMarkupFor().");
    const look = self.edgeLook(edge);
    const d = laid.points.map(function (point, i) {
      return (i === 0 ? 'M' : 'L') + self.round(point.x) + ' ' +
        self.round(point.y);
    }).join('');
    ...
    const x = laid.x == null
      ? laid.points[Math.floor(laid.points.length / 2)].x : laid.x;
    const y = laid.y == null
      ? laid.points[Math.floor(laid.points.length / 2)].y : laid.y;
    const top = y - label.height / 2;
    const panel = '<rect x="' + self.round(x - label.width / 2) + '" y="' +
      self.round(top) +
      '" width="' + self.round(label.width) + '" height="' +
      self.round(label.height) +
      '" rx="3" fill="' + PAPER + '" fill-opacity="0.92" stroke="' + LINE +
      '" stroke-width="0.6"/>';
    const words = label.lines.map(function (one, i) {
      return '<text x="' + self.round(x) + '" y="' +
        self.round(top + LABEL_PAD_Y + (i + 1) * (LINE_HEIGHT - 1) - 3) +
        '" text-anchor="middle" font-size="' + EDGE_SIZE + '" fill="' +
        (i === 0 ? INK : QUIET) + '">' + self.esc(one) + '</text>';
    }).join('');

    log.debug("Leaving FederationDiagram.edgeMarkupFor().");
    return '<g><title>' + self.esc(self.edgeTitle(edge)) + '</title>' + path +
           panel + words + '</g>';
  }

  // One box. `at` is dagre's, and dagre gives a CENTRE where every shape below
  // is drawn from its top left — so the conversion happens once, here, rather
  // than in each shape.
  private nodeMarkupFor(entry, at, options) {
    const { log, hexPath } = this.deps;
    const self = this;
    log.debug("Entering FederationDiagram.nodeMarkupFor().");
    ...
    const shape = look.shape === 'hexagon'
      ...
      : '<rect x="' + self.round(x) + '" y="' + self.round(y) + '" width="' +
        self.round(size.width) + '" height="' + self.round(size.height) +
    ...
    const texts = size.lines.map(function (one, i) {
      return '<text x="' + self.round(cx) + '" y="' +
        self.round(textTop + i * LINE_HEIGHT) +
        '" text-anchor="middle" font-size="' + LABEL_SIZE +
        '" font-weight="600" fill="' + INK + '">' + self.esc(one) + '</text>';
    }).join('') + size.subLines.map(function (one, i) {
      return '<text x="' + self.round(cx) + '" y="' +
        self.round(textTop + size.lines.length * LINE_HEIGHT +
                   i * (LINE_HEIGHT - 2)) +
        '" text-anchor="middle" font-size="' + SUB_SIZE + '" fill="' + QUIET +
        '">' + self.esc(one) + '</text>';
    }).join('');

    const body = '<title>' + self.esc(look.title || entry.node.id) +
                 '</title>' + shape + texts;

    // A LINK ONLY WHEN THE CALLER ASKED FOR ONE, and it is
    // `delegation_map.ts`'s reason unchanged: the picture is served two ways —
    ...
    // root-relative links into the current realm on the way out of a
    // `text/html` response ONLY, so a link in a standalone SVG would also be a
    // link that quietly left the realm.
    if (options.links && look.href) {
      log.debug("Leaving FederationDiagram.nodeMarkupFor().");
...
export = {
  ...
  COLOURS: FederationDiagram.COLOURS,
  hexPath: delegationMap.hexPath
};
