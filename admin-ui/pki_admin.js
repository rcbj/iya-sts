...
// is the same arrangement `admin-ui/crypto_metadata.js` could NOT have (it sits
...
//
// **IT IS A FIELD ON THE SAME ACTION RATHER THAN A SECOND ACTION.** Issuing to
// a person and issuing to an application differ in two things — which
// subjectAltName the certificate carries and which entry the result is written
// onto — and everything else about the act is identical: the same Issuing CA,
// the same profile, the same certificate. A second action would be a second
// answer to *how does this service issue a signing key pair*, and it is the
// second one that stops getting the next fix. `common/pki.js`'s SUBJECT_KINDS
...
// because the list of realms is the console's question — that module is handed
// scope ids and has no opinion about which exist.
async function rebuildEveryScope() {
  log.debug('Entering rebuildEveryScope().');
  // EVERY realm, not the ones this page draws — see `everyRealmId()`.
  const scopes = [pki.PROCESS_SCOPE].concat(everyRealmId());
  let done = 0;
  ...
  log.debug('Leaving rebuildEveryScope(). ' + done + ' branch(es).');
  return done;
}
...
async function recertifyScope(scopeId) {
  log.debug('Entering recertifyScope(). scope=' + scopeId);
  let done = 0;
  ...
  log.debug('Leaving recertifyScope(). ' + done + ' certificate(s).');
  return done;
}
...
//
// **NO PRIVATE KEY IS IN IT**, and that is `common/pki.js`'s doing rather than
// this file's: `describe()` drops every one of them on the way out, so a
// caller here could not leak the Root's key by forgetting.
// ---------------------------------------------------------------------------
function pkiJson(req, draft) {
  log.debug('Entering pkiJson().');
  ...
  const json = {
    ...
    // documented: `POST /admin-api/pki/build` learns what it may send from the
    // service instead of from a copy of the list in a document.
    ...
    // Every application this realm has, with whatever the issue control put on
    // its entry. It is READ OFF THE ENTRIES rather than out of a register of
    // this module's own, because there is no such register — `common/pki.js`
    // hands a key pair over once and keeps no copy, so `ou=applications` is
    ...
    issued: pki.PURPOSE_IDS.reduce(function (rows, purpose) {
      ...
      return rows.concat(applications.list().map(function (one) {
        ...
        return {
          ...
          // not accept what it signs as an authorization grant. It is PER
          // PROFILE too: being trusted to assert as a JWT is not being trusted
          // to assert as SAML.
          assertionIssuers: valuesOf(fields[table.issuerAttribute]),
          // What the party registered ITSELF, which for RFC 7523 is a JWKS and
          // for RFC 7522 is a certificate — the one place the two profiles'
    ...
    purposes: pki.PURPOSES.map(function (one) {
      return { id: one.id, label: one.label, what: one.what,
               attributes: PURPOSE_WRITES[one.id].attributes.slice(),
               issuerAttribute: PURPOSE_WRITES[one.id].issuerAttribute };
    }),
    // ---------------------------------------------------------------------
    // THE TREE THIS REALM IS UNDER (2026-09-11): the service Root, the process
    // branch, and THIS REALM'S Intermediate with an Issuing CA per use case
    // and what each one has certified beneath it.
    //
    // **IT WAS EVERY REALM'S FOR A DAY**, and `realmIdsForTree()` above argues
    // the narrowing at length. The short of it: the console shows one realm at
    // a time everywhere else, and this page was the one that did not.
    //
    ...
    // the same question the page does and the switcher is the answer for both.
    // A view that answered more than the page drew would be two answers to
    // "what is this realm's certificate authority", which is the rule this
    ...
    // always read and what RFC 7523 needs. This is the rest of it, and the two
    // are one store read twice rather than two stores.
    // ---------------------------------------------------------------------
    tree: pki.describeTree(realmIdsForTree()),
...
// `realms.DEFAULT_ID` is the store's own spelling and resolves to the same row
// from every realm, which is the property this list needs and `''` never had.
//
// **THIS IS THE REBUILD'S LIST AND IT IS NOT THE PAGE'S** (2026-09-11). It was
// both until this date, under a name — `realmIdsForTree()` — that said so, and
// splitting them is the whole of that change: replacing the Root MUST reach
// every branch in the process, because a branch left hanging from a Root that
// no longer exists chains to nothing and every path through it is refused. A
// page that draws one realm and a rebuild that walks one realm are not the
// same requirement, and the one name covering both is how a narrowing of the
...
// The realm this request arrived in, as the PKI store spells it: `default` for
// the default realm, for the reason above — and `realms.current().id` is
// already that spelling, which is why this is a read and not a conversion. One
// function, because `scopeFrom()`, the tree, the revocation pane and the
...
// every settings form on it reads and writes the realm it was reached in, and
// this page was the one that read the whole process. Drawing every realm's
// branch made the page grow with the realm count, put a Rebuild button for
// somebody else's authority on a page that is about yours, and — the part that
// actually matters — offered a Revoke on a certificate issued in a realm this
// operator had not switched to.
//
...
// reason `rebuildEveryScope()` is here — so the question *which branches does a
...
// ---------------------------------------------------------------------------
// ===========================================================================
// ISSUING TO A PERSON (2026-09-11), and the three ways it differs from issuing
// to an application. Everything else about the act — the hierarchy, the
...
// ===========================================================================
async function issueToPerson(identifier, body) {
  ...
  if (!held) {
    ...
    log.debug('Leaving issueToPerson(). Nobody by that name.');
  ...
  return {
    ...
    why: 'A ' + record.keyAlg + ' signing key pair was issued to "' +
         ...
         'the entry and there is no read door for it, which is deliberate: ' +
         'the alternative was a console page that prints somebody\'s private ' +
         'key on every visit.'
  };
}
...
async function pkiAction(body) {
  log.debug('Entering pkiAction(). action=' + (body && body.action));
  const action = String((body && body.action) || '');

  // =====================================================================
  // A SCOPED ACTION MAY ONLY NAME A BRANCH THIS REALM CAN SEE (2026-09-11).
  //
  // The seven actions below carry a `scope`, and until this change any of them
  // would act on any realm's branch — which was harmless while the page drew
  // every branch and is not now. **A page that hides a branch and an action
  // that still edits it is the worse of the two halves**: the operator cannot
  // see what they changed, and the realm switcher — which is this console's
  // one answer to "which realm am I acting on" — stops being the answer for
  // this page alone.
  //
  ...
  if (SCOPED_ACTIONS.indexOf(action) >= 0 && !scopeVisible(scopeFrom(body))) {
    log.debug('Leaving pkiAction(). A scope this realm does not draw.');
    return refuse('That action names the "' + scopeFrom(body) + '" branch, ' +
                  'and this page is the "' + realmLabel() + '" realm\'s. A ' +
  ...
  if (action === 'build') {
    ...
    const built = await pki.buildChain(undefined, {
      // The FORM wins over the setting and the setting is the default, which is
      // the shape every settings-backed control in this console has. An empty
      // `pki.signatureAlgorithm` means "the right one for the key algorithm" —
      // see that row's description for why a fixed value there is wrong for EC.
  ...
  if (action === 'clear') {
    ...
    return { ok: true,
             why: 'The "' + realmLabel() + '" realm\'s certificate authority ' +
  ...
  if (action === 'issue') {
    ...
    return { ok: true,
             why: 'A ' + record.keyAlg + ' signing key pair was issued to "' +
                  ...
                  'this realm\'s Issuing CA, and written onto its entry. ' +
                  table.handleLabel + '=' +
                  (purpose === 'saml' ? record.certificateThumbprint
                                      : record.kid) + ', ' +
  ...
  if (action === 'revoke') {
    ...
    let removed = 0;
    PURPOSE_WRITES[purpose].attributes.forEach(function (name) {
      const done = applications.updateApplication(identifier, {
    ...
    return { ok: true,
             ...
             why: 'The ' + pki.purposeFor(purpose).label + ' key pair was ' +
                  'taken off "' + identifier + '". The other profile\'s key ' +
                  'pair, if it holds one, is untouched. ' +
                  ...
                  'valid, still chains to this realm\'s Root, and is NOT on ' +
                  'any revocation list — what changed is that this service ' +
                  'will no longer accept an assertion signed with that key, ' +
                  'because the key is no longer registered against this ' +
                  'application. To make this service\'s CRL and OCSP ' +
                  'responder say the certificate is revoked as well, use the ' +
                  'Revoke control in the revocation pane, against the ' +
                  'authority that issued it. Somebody dealing with a ' +
                  'compromised key pair wants both.' };
  }
  ...
  if (action === 'build-root') {
    ...
    // the button says.
    const rebuilt = await rebuildEveryScope();
    log.debug('Leaving pkiAction(). A new Root.');
    return { ok: true,
             why: 'A new Root CA was built for this service, and ' + rebuilt +
                  ...
                  'TRUSTS THIS SERVICE — the new Root is on this page and at ' +
                  'GET /admin-api/pki. The signing keys themselves are ' +
                  'unchanged, so nothing that verifies against the published ' +
                  'JWKS is affected.',
             root: built.root };
  }
  ...
  if (action === 'build-scope') {
    ...
    return { ok: true,
             why: 'That branch was rebuilt — a new Intermediate CA and a new ' +
                  'Issuing CA for every use case under it — and ' + again +
                  ' certificate(s) were re-minted from the new authorities. ' +
                  'The Root is untouched, because every other scope hangs ' +
                  'from it.' };
  }
  ...
  if (action === 'recertify') {
    ...
    return { ok: true,
             why: done.recertified + ' certificate(s) were renewed under the ' +
                  'same ' + useCaseId + ' Issuing CA, with fresh serials and ' +
                  'a fresh validity window. THE KEYS ARE UNTOUCHED — this is ' +
                  'a renewal rather than a regeneration, so nothing that ' +
                  'verifies against the published keys stops verifying.' };
  }
  ...
  if (action === 'pin-key') {
    const scope = scopeFrom(body);
    const done = await pki.pinKeyPair(scope, String(body.useCase || '').trim(),
  ...
  // `common/pki_revocation.js`, because they are statements about the register
  // rather than about a console — and a copy of any of them here would be the
  // half that eventually disagreed with the CRL this service actually signs.
  // =====================================================================
  ...
  if (action === 'release-hold') {
    ...
    return { ok: true,
             why: 'The hold on ' + String(body.serialHex || '').trim() +
                  ' is lifted and the serial is off that authority\'s list. ' +
                  'A validator that cached the previous CRL will go on ' +
                  'calling it revoked until that copy expires, which is ' +
  ...
  if (action === 'apply-profile') {
    ...
    log.debug('Leaving pkiAction(). The profile was applied.');
    return { ok: true, draft: applied,
             why: 'The extension boxes, the default validity and the Common ' +
                  'Name now say what a ' +
  ...
  if (action === 'issue-certificate') {
    const issued = await authoring.issue(undefined, authoring.draftFrom(body));
    if (!issued.ok) {
      log.debug('Leaving pkiAction(). The issue failed.');
  ...
  if (action === 'remove-object') {
    ...
    return { ok: true, draft: authoring.draftFrom(body),
             why: 'That object and its key pair are gone. Anything it ISSUED ' +
                  'is kept — those certificates are still valid documents — ' +
                  'and will now say that their issuer is missing.' };
  }
  ...
  if (action === 'clear-store') {
    ...
    return { ok: true, draft: authoring.draftFrom(body),
             why: cleared.removed + ' key pair(s) and their certificates ' +
                  ...
                  'anything those keys signed is still a valid document that ' +
                  'still chains to whatever signed IT.' };
  }
...
function chainTable(chain) {
  ...
  if (!chain) {
    log.debug('Leaving chainTable(). No hierarchy.');
    return '';
  }
  const rows = chain.tiers.map(function (tier) {
    return '<tr>' +
      ...
      '<td><code>' + esc(tier.serialHex.slice(0, 16)) + '&hellip;</code></td>' +
...
function pemBlocks(chain) {
  log.debug('Entering pemBlocks().');
  if (!chain) {
    log.debug('Leaving pemBlocks(). No hierarchy.');
    return '';
  }
  // The certificates, in full, because the ONE thing a relying party has to be
  // given out of band is the Root — and a page that showed a thumbprint and
  // made somebody find the bytes elsewhere would be a page that stops at the
  // interesting part. They are public: a certificate is the half of a key pair
  // that is meant to be handed around.
...
function signatureOptions(json, keyAlg) {
  log.debug('Entering signatureOptions(). keyAlg=' + keyAlg);
  ...
  log.debug('Leaving signatureOptions().');
...
//
// **THE BUTTONS DO NOT SHARE A NAME.** `/admin/users/new` records what happens
// when two submit buttons are both called `action`: the console suite finds a
// form by the action it posts, `form.elements.action` becomes a RadioNodeList
// whose value is empty, and the page's main button reads as a control that
// reaches nothing. So the Issue button is unnamed behind a hidden
// `action=issue-certificate`, and every other button has a name of its own
// that `paneActionFrom()` reads FIRST.
// ===========================================================================
...
function extensionCards(json, draft) {
  log.debug('Entering extensionCards().');
  const cards = [];

  cards.push(extCard(
    checkField(draft, 'pki_ext_bc', 'basicConstraints',
               'Whether this certificate may act as a CA, and how many CAs ' +
               'may follow it. RFC 5280 requires it to be critical in a CA ' +
               'certificate.') +
    checkField(draft, 'pki_bc_critical', 'critical',
               'RFC 5280: MUST be critical in a CA certificate.'),
    checkField(draft, 'pki_bc_ca', 'cA',
               'This certificate may sign other certificates.') +
    textField(draft, 'pki_bc_pathlen', 'pathLenConstraint',
              'How many further CA certificates may appear below this one. 0 ' +
              'means it may only sign leaves. Empty means unlimited.',
              ' size="6" placeholder="(unlimited)"')));

  cards.push(extCard(
    checkField(draft, 'pki_ext_ku', 'keyUsage',
               'What the certified key may be used for. RFC 5280 says this ' +
               'SHOULD be critical.') +
    checkField(draft, 'pki_ku_critical', 'critical',
               'RFC 5280: SHOULD be critical.'),
    '<div class="pki-flags">' + json.workbench.keyUsageBits.map(function (bit) {
      return checkField(draft, 'pki_ku_' + bit.name, bit.name, bit.what);
    }).join('') + '</div>'));

  cards.push(extCard(
    checkField(draft, 'pki_ext_eku', 'extendedKeyUsage',
               'The purposes this certificate is for. A TLS client refuses a ' +
               'server certificate without serverAuth; a server refuses a ' +
               'client certificate without clientAuth.') +
  ...
  cards.push(extCard(
    checkField(draft, 'pki_ext_skid', 'subjectKeyIdentifier',
               'The SHA-1 of this certificate’s public key, RFC 5280 ' +
               'section 4.2.1.2 method (1) — the same value every other ' +
               'implementation computes, so key identifiers match across ' +
               'tools.'), ''));

  cards.push(extCard(
    checkField(draft, 'pki_ext_akid', 'authorityKeyIdentifier',
               'Identifies the ISSUER’s key, so a validator can find the ' +
               'right CA certificate when several share a subject.'),
    checkField(draft, 'pki_akid_issuer_serial',
  ...
  cards.push(extCard(
    checkField(draft, 'pki_ext_san', 'subjectAltName',
               'The names this certificate is for. For TLS this — not the ' +
               'Common Name — is what a client checks.') +
    checkField(draft, 'pki_san_critical', 'critical',
               'Should be critical when the subject DN is empty, and only ' +
               'then.'),
    areaField(draft, 'pki_san', 'Names, one per line',
              'dns:example.com — ip:10.0.0.1 or ip:2001:db8::1 — ' +
              'email:user@example.com — uri:https://example.com/x — ' +
              'upn:user@EXAMPLE.COM — krb5:host/x@EXAMPLE.COM — rid:1.2.3.4 ' +
              '— dirname:CN=alt,O=Example — othername:<oid>:<base64 DER>.',
              2, 'dns:localhost\nip:127.0.0.1')));

  cards.push(extCard(
    checkField(draft, 'pki_ext_ian', 'issuerAltName',
               'Alternative names for the issuer. Same syntax as ' +
               'subjectAltName.') +
    checkField(draft, 'pki_ian_critical', 'critical',
               'Should not normally be critical.'),
    areaField(draft, 'pki_ian', 'Names, one per line',
              'The same syntax as subjectAltName above.', 1,
              'uri:https://ca.example.com/')));

  cards.push(extCard(
    checkField(draft, 'pki_ext_cdp', 'cRLDistributionPoints',
               'Where to fetch the CRL that would revoke this certificate. ' +
               'NOTE that this service publishes none: the extension is a ' +
               'URL you are asserting, not a promise this mock keeps.') +
    checkField(draft, 'pki_cdp_critical', 'critical', 'Rarely critical.'),
    areaField(draft, 'pki_cdp', 'URLs, one per line',
              'Where the CRL that would revoke this certificate is ' +
              'published.', 1, 'http://crl.example.com/issuing.crl')));

  cards.push(extCard(
    checkField(draft, 'pki_ext_freshest', 'freshestCRL',
               'Where to fetch the DELTA CRL. The same shape as ' +
               'cRLDistributionPoints and a different extension.'),
    areaField(draft, 'pki_freshest', 'URLs, one per line',
              'Where the delta CRL is published.', 1,
              'http://crl.example.com/delta.crl')));

  cards.push(extCard(
    checkField(draft, 'pki_ext_aia', 'authorityInfoAccess',
               'Where to reach the issuer: its OCSP responder, and a copy of ' +
               'its own certificate for a client that was sent an incomplete ' +
               'chain.'),
  ...
  cards.push(extCard(
    checkField(draft, 'pki_ext_sia', 'subjectInfoAccess',
               'Services offered by the SUBJECT of this certificate, rather ' +
               'than by its issuer.'),
    areaField(draft, 'pki_sia', 'Access descriptions, one per line',
              'The same syntax as authorityInfoAccess above.', 1,
              'carepository:http://example.com/certs/')));

  cards.push(extCard(
    checkField(draft, 'pki_ext_policies', 'certificatePolicies',
               'The policies this certificate was issued under, with the two ' +
               'qualifiers RFC 5280 defines.') +
    checkField(draft, 'pki_policies_critical', 'critical',
               'Critical means a validator that cannot process the policy ' +
  ...
  cards.push(extCard(
    checkField(draft, 'pki_ext_policy_mappings', 'policyMappings',
               'Declares that one policy OID in the issuer’s domain is ' +
               'equivalent to another in the subject’s. RFC 5280 says ' +
               'this SHOULD be critical, and it is.'),
    areaField(draft, 'pki_policy_mappings', 'Mappings, one per line',
              '<issuer policy oid>=<subject policy oid>.', 1,
              '1.3.6.1.4.1.99999.1.1=1.3.6.1.4.1.88888.1.1')));

  cards.push(extCard(
    checkField(draft, 'pki_ext_policy_constraints', 'policyConstraints',
               'Requires an explicit policy, or inhibits policy mapping, ' +
               'after a number of further certificates. RFC 5280: MUST be ' +
               'critical, and it is.'),
    textField(draft, 'pki_require_explicit_policy', 'requireExplicitPolicy',
              'How many further certificates may appear before an acceptable ' +
              'policy is required. Empty means it is not set.',
              ' size="6" placeholder="(not set)"') +
    textField(draft, 'pki_inhibit_policy_mapping', 'inhibitPolicyMapping',
              'How many further certificates may still map policies. Empty ' +
              'means it is not set.', ' size="6" placeholder="(not set)"')));

  cards.push(extCard(
    checkField(draft, 'pki_ext_name_constraints', 'nameConstraints',
  ...
  cards.push(extCard(
    checkField(draft, 'pki_ext_inhibit_any', 'inhibitAnyPolicy',
               'How many further certificates may still use the anyPolicy ' +
               'OID. RFC 5280: MUST be critical, and it is.'),
    textField(draft, 'pki_inhibit_any_skip', 'skipCerts',
              '0 means anyPolicy is not accepted below this certificate at ' +
              'all.', ' size="6" placeholder="0"')));

  cards.push(extCard(
    checkField(draft, 'pki_ext_pkup', 'privateKeyUsagePeriod',
               'A validity period for the PRIVATE key that is shorter than ' +
               'the certificate’s — signatures made after it are not to ' +
               'be trusted, while the certificate goes on validating them.'),
    textField(draft, 'pki_pkup_not_before', 'notBefore',
    ...
    textField(draft, 'pki_pkup_not_after', 'notAfter',
              'When the private key must stop signing — earlier than the ' +
              'certificate’s own notAfter, which goes on validating what ' +
              'was signed before it. Empty leaves it out.',
              ' size="18" placeholder="2027-01-01T00:00"')));

  cards.push(extCard(
    checkField(draft, 'pki_ext_tls_feature', 'TLS Feature (RFC 7633)',
               'The TLS extensions a server promises to support. 5 is ' +
               'status_request — "must-staple"; 17 is status_request_v2.'),
    areaField(draft, 'pki_tls_feature', 'Extension numbers, one per line',
              'One TLS extension number per line.', 1, '5')));

  cards.push(extCard(
    checkField(draft, 'pki_ext_ocsp_nocheck', 'id-pkix-ocsp-nocheck',
               'Tells a validator not to check the revocation status of this ' +
               'certificate — correct on an OCSP responder’s own ' +
               'certificate and almost nowhere else.'), ''));
  ...
  cards.push(extCard(
    checkField(draft, 'pki_ext_ns_comment', 'Netscape comment',
               'A free-text comment some tools display when showing the ' +
               'certificate.'),
    textField(draft, 'pki_ns_comment', 'Comment', 'Free text.',
              ' size="40" placeholder="Issued by the mock STS"')));

  cards.push(extCard(
    '<strong' + admin.tip('Any extension at all, by OID and base64 DER — ' +
    'including one this page has never heard of. Without this the extension ' +
  ...
  log.debug('Leaving extensionCards(). ' + cards.length + ' card(s).');
...
function certificateColumn(json, draft) {
  log.debug('Entering certificateColumn().');
  ...
  const html =
    ...
    selectField(draft, 'pki_profile', 'Profile',
                'What kind of certificate this is. It sets the default ' +
                'extensions, the default validity, and whether it is ' +
                'self-signed.',
                // **THE LABEL IS THE PROFILE'S OWN AND NOTHING IS APPENDED
                // TO IT.** `root-ca` is already called "Root CA
                // (self-signed)" in the encoder's table, so a page adding its
                // own "(self-signed)" printed it twice — which is what
                // happens every time a renderer restates a fact the table it
                // is reading already carries.
    ...
    '<button type="submit" name="defaults" value="1"' +
      admin.tip('Rewrite the extension boxes, the default validity and the ' +
                'profile\'s Common Name from the profile chosen beside this ' +
                'button. Nothing is issued and a name you typed is kept.') +
    ...
    selectField(draft, 'pki_issuer', 'Signed by',
                'The certificate authority that signs this certificate. Only ' +
                'authorities whose private key is still in this service are ' +
                'listed — one whose key was never kept cannot sign. The ' +
                'three tiers come from the hierarchy above; anything else is ' +
                'a CA issued from this pane.', issuerOptions) +
    '</div>' +
    '<div class="pki-row">' +
    selectField(draft, 'pki_pq_mode', 'Cryptographic Approach',
                'Which of the three ways this certificate carries ' +
                'post-quantum cryptography. It filters the Key Algorithm ' +
                'list beside this column and narrows the Signature Algorithm ' +
                'below; Hybrid additionally uses the alternative key pair, ' +
                'which goes into the X.509 (2019) alternative-signature ' +
                'extensions.',
    ...
    '<button type="submit" name="defaults" value="1"' +
      admin.tip('Redraw the two algorithm menus for the approach chosen ' +
                'beside this button. It is the same button as the one above ' +
                'it: with no script on this console, narrowing a menu is a ' +
                'round trip.') + '>Apply</button>' +
    '</div>' +
    admin.note(esc(mode.note), mode.label) +
    '<div class="pki-row">' +
    selectField(draft, 'pki_sig_alg', 'Signature Algorithm',
                'The algorithm the ISSUER signs with. The list is what the ' +
                'signing key can actually produce' +
                (wb.signerLabel ? ' — it is ' + wb.signerLabel + ' — ' : ' ') +
                'because an RSA key cannot make an ECDSA signature and ' +
                'offering it produces a Web Crypto error that names neither.',
                [{ value: '', label: '(the right one for the signing key)' }]
    ...
    '</div>' +
    '<div class="pki-row">' +
    textField(draft, 'pki_serial', 'Serial Number (hex)',
    ...
    textField(draft, 'pki_validity_years', 'Validity (years)',
              ...
              'intermediate, 5 for an issuing CA, 1 for a leaf.', ' size="5"') +
    '</div>' +
    '<div class="pki-row">' +
    textField(draft, 'pki_not_before', 'Not Before',
              'Optional — empty means the moment the button is pressed. ' +
              'Anything Date can read; what is encoded is the instant, in ' +
              'UTC. A date at or after 2050 is encoded as a GeneralizedTime, ' +
              'as RFC 5280 requires — a UTCTime there is read as 1950, i.e. ' +
              'a certificate that expired seventy years ago.',
...
function keyPairColumn(json, draft) {
  log.debug('Entering keyPairColumn().');
  const wb = json.workbench;
  ...
  const html =
    ...
    admin.note(
      ...
      'page differs in kind from the debugger\'s: that page generates in the ' +
      'browser because its whole claim is that the key never leaves it, and ' +
      'this one holds the certificate authority, so the key belongs in the ' +
      'process that signs. It is the same module either way ' +
      '(<code>common/vendored/key_material.js</code>), so the algorithms, ' +
      'the PEM/JWK conversion and the keystore formats are identical rather ' +
      'than similar.',
    ...
    (slow.length
      ? admin.warn(
        ...
        'the next HTTP caller, not the KDC on port 88, not the LDAP socket. ' +
        'It is deliberately not moved to <code>common/worker_pool.js</code>: ' +
        'that pool runs this service\'s own reading of the post-quantum ' +
      ...
      : '') +
    '<div class="pki-row">' +
    selectField(draft, 'pki_key_alg', 'Key Algorithm',
                'The algorithm and parameters of the key pair. RSA sizes are ' +
                'the modulus; the EC entries are the NIST curves; Ed25519 is ' +
                'the only Edwards curve here. The list is narrowed by the ' +
                'Cryptographic Approach in the column beside this one.',
                algOptions) +
    checkField(draft, 'pki_key_jwk', 'show as JWK',
    ...
    '</div>' +
    '<div class="pki-row">' +
    checkField(draft, 'pki_reuse_key', 'reuse the key pair below',
               'Certify the key pair already in the boxes below instead of ' +
               'generating a new one — a CA re-issuing its own certificate, ' +
               'or the pair the store\'s "Use this key pair" button loaded ' +
               'here. Cleared, every issue starts from a fresh pair.') +
    checkField(draft, 'pki_save_keys', 'keep the private key in this service',
               'THIS IS THE ONE CONTROL ON THIS PANE THAT MEANS SOMETHING ' +
               'DIFFERENT FROM THE DEBUGGER\'S. There it decides whether the ' +
               'private key is written to the browser\'s localStorage; here ' +
               'there is no browser store, so it decides whether the issued ' +
               'object keeps its private key in this realm\'s keystore row. ' +
               'Cleared, the certificate and the public key are stored and ' +
               'the private half is discarded the moment the reply is ' +
               'written — so the object can be inspected and used as a trust ' +
               'anchor, and can never sign again or be exported.') +
    '</div>' +
    areaField(draft, 'pki_private_key', 'Private Key',
              'PKCS#8 PEM (or JWK). It signs, and for a CA it goes on ' +
              'signing long after this certificate was issued. There is no ' +
              'Copy button because this console has no script; the box ' +
              'selects.', 4) +
    areaField(draft, 'pki_public_key', 'Public Key',
    ...
    areaField(draft, 'pki_csr', 'CSR (PKCS#10)',
              ...
              'built from the SAME inputs the certificate was — a request ' +
              'assembled from a second reading of the form would differ in ' +
              'ways nobody could see. THREE EXTENSIONS TRAVEL AND THE REST ' +
              'DO NOT: key usage, extended key usage, basic constraints and ' +
              'subjectAltName. subjectKeyIdentifier and ' +
              'authorityKeyIdentifier are the ISSUER\'s to compute, and a ' +
              'requester asserting them is asking a CA to certify its own ' +
              'arithmetic. Empty until the box above is ticked and something ' +
              'is issued.', 3) +
    ...
    admin.note(
      ...
      'when the Cryptographic Approach is <em>Hybrid</em>; for a self-signed ' +
      'certificate this pair makes the second signature, and for an issued ' +
      'one the ISSUER\'s alternative key does and this one is only certified.',
    ...
    checkField(draft, 'pki_alt_reuse_key', 'reuse the pair below',
               'Certify the alternative pair already in the boxes instead of ' +
    ...
    '</div>' +
    areaField(draft, 'pki_alt_private_key', 'Alternative Private Key',
              'PKCS#8 PEM. For a self-signed certificate this is what signs ' +
              'the preTBSCertificate; for an issued one the issuer\'s own ' +
              'alternative private key is used instead and this one is only ' +
              'certified.', 3) +
    areaField(draft, 'pki_alt_public_key', 'Alternative Public Key',
              'SubjectPublicKeyInfo PEM. This is what goes into the ' +
              'subjectAltPublicKeyInfo extension.', 2) +
    '<div class="pki-group">Export</div>' +
    admin.note(
      'Writes out the key pair of the object SELECTED in the store below, or ' +
      'the pair in the boxes above when nothing is selected. It is the same ' +
      'export <code>/admin/keys</code> uses &mdash; ' +
    ...
    '<label' + admin.tip('Required for PKCS#12. For PEM and DER it encrypts ' +
      'the private key as a PBES2 EncryptedPrivateKeyInfo; for JWK it wraps ' +
      'the set in a PBES2 JWE. Left empty, the private key is written out in ' +
      'the clear.') + '>Password <input type="password" ' +
...
function subjectColumn(json, draft) {
  log.debug('Entering subjectColumn().');
  const titles = {
    pki_dn_cn: 'commonName (2.5.4.3) — what this certificate is called. The ' +
               'profile fills it in and replaces its own default when the ' +
               'profile changes, but never a name you typed. For a TLS ' +
    ...
    pki_dn_ou: 'organizationalUnitName (2.5.4.11) — the division within the ' +
               ...
               'attributes below; a DN may carry several.',
    ...
    pki_dn_email: 'emailAddress (1.2.840.113549.1.9.1) — a legacy PKCS#9 ' +
                  ...
                  'decorative.',
    pki_dn_dc: 'domainComponent (0.9.2342.19200300.100.1.25) — one label of ' +
               'a DNS name, as Active Directory writes a DN (DC=example, ' +
               'DC=com). Add further DC=… lines to Further attributes below, ' +
               'in order.',
    pki_dn_uid: 'userId (0.9.2342.19200300.100.1.1) — the account name, as a ' +
                'directory holds it.',
  ...
  const boxes = json.workbench.dnFields.map(function (one) {
    return textField(draft, one.field, labels[one.field] || one.attr,
                     titles[one.field] || '',
  ...
  const html =
    ...
    admin.note(
      'Written in the order shown &mdash; a Name is an ordered RDNSequence, ' +
      'and a reordered DN is a different name that chains to nothing. For a ' +
      'TLS server, note that the name a client checks is in ' +
    ...
    areaField(draft, 'pki_dn_extra',
              'Further attributes (one NAME=value or OID=value per line)',
              'Appended in order. Recognised names include SN, GN, title, ' +
...
// which is also why they have names of their own rather than sharing `action`.
// ---------------------------------------------------------------------------
function storeTable(json, draft) {
  log.debug('Entering storeTable().');
  const wb = json.workbench;
  if (!wb.objects.length) {
    log.debug('Leaving storeTable(). Empty.');
    ...
    return '<input type="hidden" name="pki_selected" value="' +
      ...
      'with the realm, and in development mode gone with the process, which ' +
      'is the rule the signing key already follows.',
      'The store is empty');
  }
  const rows = wb.objects.slice().reverse().map(function (one) {
    return '<tr>' +
      ...
      '<td><code>' + esc(one.serialHex.slice(0, 16)) + '&hellip;</code></td>' +
  ...
  log.debug('Leaving storeTable(). ' + wb.objects.length + ' row(s).');
...
// store, in one form.
function certificatePane(json, draft) {
  log.debug('Entering certificatePane().');
  const wb = json.workbench;
  const html =
    ...
    admin.note(
      ...
      'responder, time stamping, smartcard logon and Kerberos PKINIT. Every ' +
      'X.509v3 extension RFC 5280 defines is below, plus the ones in common ' +
      ...
      'true of every address <em>you type</em> &mdash; what changed is that ' +
      'the certificates this service issues <em>itself</em> now name ' +
      'addresses it really answers: each authority signs a CRL and runs an ' +
      'OCSP responder, and anything issued from one of them can be revoked ' +
      'in the pane below. An object minted here from an authority whose key ' +
      'is in this process is in that register too. One typed at a URL of ' +
      'your own is not, and nothing here will pretend otherwise.</p>',
      'What this pane is, and how it differs from the page it is modelled on') +
    '<form method="post" action="/admin/pki/certificate">' +
    '<input type="hidden" name="action" value="issue-certificate">' +
    '<div class="pki-cols">' +
    certificateColumn(json, draft) +
    keyPairColumn(json, draft) +
    subjectColumn(json, draft) +
    '</div>' +
    '<div class="pki-group">X.509v3 Extensions</div>' +
    admin.note(
      'Every extension RFC 5280 defines, plus the ones in common use that it ' +
      'does not, plus anything at all by OID. <strong>The <em>critical</em> ' +
      'flag is separately settable on each</strong>: a validator must reject ' +
      'a certificate carrying a critical extension it does not understand, ' +
      'so making the wrong one critical is a good way to find out what your ' +
      'stack actually implements. Four of them are fixed critical because ' +
      'RFC 5280 says MUST or SHOULD and a box that could clear it would ' +
      'produce a certificate nothing profiles — policyMappings, ' +
      'policyConstraints and inhibitAnyPolicy.',
      'Which extensions are here, and what the critical flag costs') +
    extensionCards(json, draft) +
    '<div class="pki-row">' +
    '<button type="submit"' +
    admin.tip('Generate a key pair unless "reuse the key pair below" is ' +
              'ticked, build the certificate, sign it, and keep both in the ' +
              'store below.') +
    ...
    admin.note(
      ...
      'At most ' + esc(String(wb.maxObjects)) + ' are kept: past that the ' +
      'oldest goes and the reply says so, because every one of these is ' +
      'sealed, written to the store and pushed to every request worker when ' +
      'it changes.',
      'What this store is') +
    storeTable(json, draft) +
    '</form>';
  log.debug('Leaving certificatePane().');
  return html;
}
...
// every other button carries a name of its own (see the pane's header for what
// two buttons sharing `action` cost `/admin/users/new`).
//
...
function paneActionFrom(body) {
  ...
  log.debug('Leaving paneActionFrom(). ' + out.action);
  return out;
}
...
// use case under each — with what that authority has actually certified listed
// beneath it, because a tree of authorities with nothing under them cannot be
// told from a tree that is wired to nothing.
//
...
function treeSection(json) {
  log.debug('Entering treeSection().');
  const tree = json.tree;
  if (!tree || !tree.rootBuilt) {
    log.debug('Leaving treeSection(). No Root.');
    return admin.warn(
      '<strong>This service has no Root CA.</strong> It is built at startup ' +
      'unless <code>pki.autoBuild</code> is off, so seeing this means either ' +
      'that setting is off or the build failed — the startup log says which. ' +
      'Every key this service holds is self-signed until there is one, which ' +
      'is what this service did before 2026-09-11.',
      'No certificate authority');
  }
  let rows = tierRow(tree.root, 0);
  tree.scopes.forEach(function (scope) {
    ...
    rows += tierRow(scope.intermediate, 1,
    ...
    scope.issuing.forEach(function (one) {
      ...
      rows += tierRow(one.ca, 2,
                      ' <span class="pki-scope">' + esc(one.id) + '</span>');
...
// ---------------------------------------------------------------------------
function scopeControls(json, scope) {
  log.debug('Entering scopeControls(). scope=' + scope.scope);
  ...
  scope.issuing.forEach(function (one) {
    html += '<tr>' +
      ...
      '<td>' +
        '<form method="post" action="/admin/pki">' +
        '<input type="hidden" name="action" value="reissue-use-case">' +
        '<input type="hidden" name="scope" value="' + esc(scope.scope) + '">' +
        '<input type="hidden" name="useCase" value="' + esc(one.id) + '">' +
        '<button type="submit"' +
        admin.tip('Generate a new key pair for this Issuing CA and re-issue ' +
                  'it from this scope’s Intermediate, then re-certify ' +
                  'everything that hung under it. The other use cases are ' +
                  'untouched, which is the whole reason each has an ' +
        ...
        (one.certified.length
          ? '<form method="post" action="/admin/pki">' +
            ...
            admin.tip('Re-issue the certificates under this CA from the same ' +
                      ...
                      'not a regeneration, so nothing that verifies against ' +
                      'the published keys stops verifying.') +
  ...
  html += '</tbody></table>';
  log.debug('Leaving scopeControls().');
  return html;
}
...
function rootControls(json) {
  ...
  return '<h4>The Root CA</h4>' +
    ...
    algorithmOptions(json, (tree.root && tree.root.keyAlg) ||
                     config.value('pki.keyAlgorithm')) + '</select></label> ' +
    ...
    admin.note(
      'Paste a CA certificate and its private key and this service will hang ' +
...
//
// **THERE ARE NOW TWO CONTROLS ON THIS PAGE WITH THE WORD *REVOKE* ON THEM AND
// THEY DO COMPLETELY DIFFERENT THINGS.** That is the thing to get right here,
// because getting it wrong is not a bug a reader would ever see:
//
//   * the `revoke` action in the Applications table — which predates this pane
//     — TAKES THE KEY PAIR OFF an application's directory entry. It stops this
//     service ACCEPTING what that key signs, and the certificate goes on
//     chaining to this realm's Root for anybody who only checks the chain. It
//     is a change to `ou=applications`.
...
//
// A serial number is unique only WITHIN one issuer, so *revoke serial 4f2a* is
// not a question this service can answer — it needs to know which CA is being
// asked to say it. That is also why there is one CRL and one OCSP responder
// per authority rather than one per realm: a list per realm would be a
// document with no valid issuer, and nothing could sign it.
//
...
function revocationModel() {
  log.debug('Entering revocationModel().');
  ...
  // more than untidy — a revocation is permanent, it is made BY AN ISSUER, and
  // the issuer's name on a row is the only thing that says which realm you are
  // about to change.
  //
  // **IT READ `pki.knownScopes()` UNTIL THIS CHANGE**, which is every scope in
  // the keystore. That function had already been got wrong once here — it
  // carries the process branch itself, so concatenating `PROCESS_SCOPE` listed
  // every process authority twice with two sets of Revoke buttons that both
  // worked — and naming the two scopes outright is what stops a third reading
  // of what it happens to filter. An unbuilt scope needs no filtering either:
  // `authorities()` asks `describeScope()` and skips a branch that is not
  // there.
  const scopes = [pki.PROCESS_SCOPE, currentRealmScope()]
  ...
  const authorities = pkiRevocation.authorities(scopes).map(function (one) {
    ...
    return {
      ...
      issued: pkiRevocation.issuedList(one.scope, one.ca).map(function (cert) {
      ...
      revoked: revoked,
      // A SERIAL ON THE LIST THAT THIS AUTHORITY DID NOT ISSUE IS LEGAL AND IS
      // REPORTED SEPARATELY. RFC 5280 does not require a CA to still hold a
      // record of what it signed in order to revoke it, and this service
      // genuinely reaches that state: a leaf superseded by a rotation is
      // revoked and then REPLACED in the register, so the old serial is on the
      // list with nothing left to point at. Drawing it in the issued table
      // would be inventing a certificate; dropping it would hide most of what
  ...
  log.debug('Leaving revocationModel(). ' + authorities.length +
...
function reasonSelect(name) {
  ...
  return '<select name="' + esc(name) + '">' +
    pkiRevocation.REASONS.map(function (one) {
      ...
      // default of `unspecified` would be the one value RFC 5280 section 5.3.1
      // says to OMIT the extension for, so the commonest act on this pane
      // would produce the least informative entry available.
      return '<option value="' + esc(one.id) + '"' +
    ...
    const control = cert.revoked
      ? (cert.held
          ? '<form method="post" action="/admin/pki">' +
            ...
            admin.tip('Take this serial off the list. Only a ' +
                      ...
                      'validator is entitled to cache a permanent revocation ' +
                      'for as long as the CRL it read says it is fresh.') +
      ...
      : '<form method="post" action="/admin/pki">' +
        ...
        admin.tip('Put this serial on ' + authority.label + '’s revocation ' +
                  ...
                  'it changes is what this service’s CRL and OCSP responder ' +
                  'say about this serial from now on.') +
        '>Revoke</button></form>';
...
function revocationPane(json) {
  log.debug('Entering revocationPane().');
  const model = json.revocation;
  if (!model.authorities.length) {
    log.debug('Leaving revocationPane(). Nothing built.');
    return '<h3>Revoking a certificate</h3>' +
      admin.note('There is no certificate authority in the ' +
                 esc(realmLabel()) + ' realm or on the process branch yet, ' +
                 'so there is nothing here to revoke and no list to publish. ' +
                 'Build one above. Another realm&rsquo;s authorities are not ' +
                 'counted &mdash; they are revoked in that realm.');
  }
  ...
  log.debug('Leaving revocationPane(). ' + model.authorities.length +
            ' authority(ies).');
  return '<h3>Revoking a certificate</h3>' + what + rotation + reasons +
    model.authorities.map(authorityBlock).join('');
}
...
function renderPki(req, res, draft, banner, extra, certificate) {
  ...
  const what = admin.note(
    ...
    'realm</strong> &mdash; a Root CA, an Intermediate CA and an Issuing CA ' +
    '&mdash; and issues signing key pairs from the bottom of it to ' +
  ...
  const buildForm =
    ...
    (chain ? admin.warn(
      'A hierarchy already exists. Building again <strong>replaces</strong> ' +
      'it, and every certificate issued from the old one chains to nothing ' +
      'the moment it does &mdash; this service keeps no copy of what it ' +
      'issued, so none of them can be listed here.',
      'This replaces what is there') : '') +
    '<form method="post" action="/admin/pki">' +
    '<input type="hidden" name="action" value="build">' +
    '<label>Key algorithm <select name="keyAlg">' +
      algorithmOptions(json, keyAlg) + '</select></label> ' +
  ...
  const issueForm = chain
    ? '<h3>Issue a signing key pair to an application</h3>' +
      admin.tip(
        ...
        'profile, and taking one off leaves the other working. This service ' +
        'keeps ' +
        'NO second copy of the private key — the entry is where it lives, ' +
        'and it is SEALED there: AES-256-GCM under the same key-encryption ' +
        'key as the certificate authority above, wherever that key outlives ' +
      ...
      '<label>Key algorithm <select name="leafKeyAlg">' +
        ...
        ')</option>' + algorithmOptions(json, '') + '</select></label> ' +
  ...
  // answers with a PAGE carrying the private key, where every other control on
  // this page 303s with its message on the query string. A private key on a
  // query string is a private key in the browser history, the access log and
  // the next request's Referer header.
  //
  ...
  const personForm = chain
    ? '<h3>Issue a signing key pair to a person</h3>' +
      admin.tip(
        'RFC 7523 section 2.1 does not say the issuer of an assertion has to ' +
        'be an application. Claim 1 asks only that <code>iss</code> be “a ' +
        'unique identifier for the JWT issuer”, and claim 2 says the ' +
        '<code>sub</code> of an authorization grant “typically identifies an ' +
        'authorized accessor or resource owner”. So a person holding a key ' +
...
//
// Every other control in this console goes through `respondToAction()`, which
// 303s back with a message on the query string. This one cannot: what it has
// to hand back is the FORM — the profile applied, the key pair generated, the
// refusal with every field still in it — and a query string is not where a
// hundred and fifteen fields go. `/admin/users/new` made exactly this
// argument first and for the same reason.
//
// **A JSON CALLER STILL GETS JSON**, so `/admin-api`'s behaviour is unchanged
// and a test may drive either door.
// ---------------------------------------------------------------------------
app.post('/admin/pki/certificate', function (req, res) {
  ...
  const pressed = paneActionFrom(body);
  ...
  pkiAction(asked).then(function (result) {
    ...
    // The draft a refusal carries where it has one, and what was posted where
    // it does not — a refusal that redrew the form empty would be worse than
    // the refusal.
...
// arrangement `/admin/keys/export` has and for its reasons, including the two
// that are easy to skip:
//
//   * **ADMIN WRITE, asked for explicitly.** Reading this console needs Admin
//     Read; taking a private key out of it needs the other role. It is a
//     GET-shaped act done as a POST for exactly that reason.
//   * **ONE FILE IS THE BODY; SEVERAL ARE A ZIP THIS SERVICE WILL NOT BUILD.**
//     The DER export is two files (private and public) and the private one is
//     sent, with the reply saying the public half comes out of it with one
//     openssl command.
//
...
app.post('/admin/pki', function (req, res) {
  ...
  // **AWAITED, AND THE HANDLER IS WRAPPED.** Express 4 does not look at what a
  // handler returns, so a promise that rejects here would be an unhandled
...
//
// The pane's POST renders a page because what it hands back is a FORM with a
// hundred and fifteen fields in it. This one renders a page because what it
// hands back is **a private key**, and `admin.respondToAction()` 303s with its
// message on the query string — which would put that key in the browser
// history, in this service's own access log, and in the `Referer` header of
// the next request the browser makes.
//
// It is the SAME `issue` action with `target=person`, so `/admin-api/pki/issue`
// drives it too and rule 7 is satisfied without a second operation. **A JSON
// caller still gets JSON**, exactly as the pane's route does.
// ---------------------------------------------------------------------------
app.post('/admin/pki/person', function (req, res) {
  ...
  // The TARGET is forced rather than read. This route exists for one control
  // and a body that arrived here naming `application` would otherwise be
  // answered by the application branch — which writes onto an entry this form
  // never mentioned, and would then render the page with an application's
  // private key on it, which is the one thing the route was written to keep
