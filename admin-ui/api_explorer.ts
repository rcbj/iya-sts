...
// (#32) the page is a service page — `admin_scope.ts` hides and refuses it to a
...
import adminApi = require('../mgmt-api/admin_api');
import spec = require('../mgmt-api/admin_api_spec');
...
// What the console's gate says about the reader, as far as this page reads it.
interface GateState {
  read?: boolean;
  write?: boolean;
  username?: string;
...
interface ApiExplorerDeps {
  log: typeof helpers.log;
  baseUrlOf: typeof helpers.baseUrlOf;
  errorCodes: typeof errorCodes;
  realms: typeof realms;
  admin: typeof admin;
  adminViews: { gateStateFor(req: Req): GateState };
...
class ApiExplorer {
  ...
  // ---------------------------------------------------------------------------
  private scopesFor(gate: GateState): string {
    const { log } = this.deps;
    log.debug("Entering ApiExplorer.scopesFor().");
    const scopes = [];
    if (gate.read) {
    ...
    log.debug("Leaving ApiExplorer.scopesFor().");
    return scopes.join(' ');
  }

  // The audience `/admin-api` answers to, computed the way that file computes
  // it: outside any realm, because the credential is service-wide.
  private audienceFor(req: Req): string {
    const { log, realms, baseUrlOf, adminApi } = this.deps;
    log.debug("Entering ApiExplorer.audienceFor().");
    log.debug("Leaving ApiExplorer.audienceFor().");
  ...
  private tokenFor(req: Req, gate: GateState): string {
    ...
    const scope = self.scopesFor(gate);
    if (!scope) {
      ...
      // empty roster with `admin.openWhenEmpty` off is drawn a refusal page,
      // and a build with no directory can hold no role at all — and there is
      // then no honest scope to put in a token. The page draws and says Try
      // it will be refused, which is true. (This read `admin.authRequired`
      // until that setting was removed on 2026-09-06; the gate is
      // unconditional now.)
      log.debug("Leaving ApiExplorer.tokenFor(). No role, so no scope.");
      return '';
    }
    try {
      ...
      log.debug("Leaving ApiExplorer.tokenFor(). Minted a token with scope " +
                scope + ".");
      return token;
    } catch (e) {
      // SWALLOWED WITH A REASON: the explorer is useful without a token —
      // every operation is still described and the curl line is still shown
      // — and a console page that 500s because a credential could not be
      // minted would be a worse answer than one that renders and says so.
      log.debug("Caught in ApiExplorer.tokenFor(): " +
                ((e && e.message) || e));
      ...
      log.debug("Leaving ApiExplorer.tokenFor(). It failed.");
      return '';
    }
  }

  // What `?format=json` and the mirroring operation answer. NOT the whole
  ...
  explorerJson(req: Req): Json {
    ...
    log.debug("Leaving ApiExplorer.explorerJson().");
    return {
      page: PATH,
      api: adminApi.BASE,
      ...
      // WHAT A READER OF THIS PAGE MAY ACTUALLY DO, which is the one thing
      // here that is about the caller rather than about the document — and it
      // is EMPTY when this is reached through `/admin-api/api-explorer`,
      // because there is no console session on such a request. That is honest
      // rather than a gap: reporting a token's own scopes back to the caller
      // that just sent them would be telling somebody what they had said.
      scope: self.scopesFor(gate),
      audience: self.audienceFor(req),
      // The token is NOT in this reply. It is a credential, and `?format=json`
  ...
  registerRoutes(app: Json): void {
    ...
    // that `'self'` suffices — `'unsafe-inline'` would be the clause that
    // mattered, and this page never needs it.
    // -------------------------------------------------------------------------
    ...
    log.debug("Leaving ApiExplorer.registerRoutes().");
  }
}
...
helpers.log.info('The API explorer is at ' + PATH + ': every operation of ' +
                 ...
                 'that API on 2026-09-09, when that API began requiring an ' +
                 'access token a browser has no way to carry — so it is ' +
                 'behind the console\'s own session and roles now, and the ' +
