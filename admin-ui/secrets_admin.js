...
// about that the way `encryption_admin.js` is about its own. `secrets.js`
...
// an arbitrary one is drawn, exactly as `database_admin.js`'s `cell()` is for
...
function cell(value) {
  ...
  // A certificate fingerprint, a mounted path or an ARN runs past the width of
  // the page; `clipped()` is the console's own control for that and opens out
  // on a click, so nothing is lost.
  //
  // **A VALUE WITH SPACES IN IT IS PROSE AND IS GIVEN TWICE THE ROOM.** Some
  // of what a probe answers is a SENTENCE — `capabilities` ends with a verdict
  // this service composed — and clipping a sentence at the width that suits a
  // fingerprint hides the half that says what to do about it, behind a control
  // whose label is "click the value to select it all, then copy". An
  // identifier is the thing worth folding; a sentence is the thing worth
  // reading.
...
//
// `database_admin.js` tells 42P01 from 42501 because "an older server" and
...
// `respond()`'s contract and why `/admin-api/secrets` cannot disagree with the
// page (rule 7).
//
...
function secretsJson() {
  ...
  return secrets.storeReport().then(function (report) {
    ...
    log.debug('Leaving secretsJson(). ' + out.stores.length + ' store(s), ' +
              out.failed.length + ' probe(s) unavailable.');
    return out;
  });
}
...
function renderSecrets(req, res) {
  log.debug('Entering renderSecrets().');
  secretsJson().then(function (json) {
...
function body(json) {
  ...
  const tiles = '<div class="tiles">' +
    ...
    admin.tile(json.persistingKeys ? 'durable' : 'ephemeral', 'signing keys') +
  ...
  const what = admin.note(
    ...
    'secrets come from, whether it actually got them, and what the store at ' +
    'the other end is doing</strong>. It reads them and never writes one: ' +
    'the key-encryption key is generated by nobody here, and neither is the ' +
    'database password.</p>' +
    '<p><strong>Three pages touch this subject and they answer three ' +
    'questions.</strong> <a href="/admin/config">Configuration</a>, under ' +
    'Server configuration, holds the <code>keys.*</code> settings and says ' +
    'what this service is <em>set up</em> to do. ' +
    '<a href="/admin/encryption">Encryption</a> says what is <em>sealed</em> ' +
    'and with what. This one says what is at the other end of that: a ' +
    'file’s mode and mtime, or a store’s seal state, its version, ' +
    'its leader, the certificate this service proves itself with and when ' +
    'that expires, what its policy actually grants, and every version of the ' +
    'secret the store has kept. <strong>All of that can be broken while ' +
    'every settings row is right.</strong></p>' +
    '<p><strong>No secret value appears here and there is no control on this ' +
    'page.</strong> No reveal, no rotate, no test-read. A reveal is the end ' +
    'of the key. A rotate would destroy everything sealed under it, because ' +
    'this service has no re-sealing pass — which is exactly why the ' +
    'key is written once. And a test-read would be this console causing the ' +
    'one thing the whole design avoids: the key in this process’s ' +
    'memory because somebody opened a page. Every probe below reads ' +
    'METADATA — a stat, a <code>sys</code> endpoint, a version history, ' +
    'a <code>DescribeSecret</code> — and not one of them fetches a ' +
    'stored value.</p>',
  ...
  const refusals = admin.warn(
    ...
    '403 is usually the good news.</strong> The identity this service holds ' +
    'in a secret store is bound to two read paths and nothing else — it ' +
    'cannot write its own key, cannot rotate it out from under the data ' +
    ...
    'refusals would be hiding the evidence for the claim.</p>' +
    '<p>Each probe is run, timed and caught <strong>separately</strong>, ' +
    'bounded by <code>keys.storeProbeTimeoutMs</code> at ' +
    ...
    'Why a failed probe here is not the same as a failed probe anywhere else');
  ...
  const unused = (!json.persistingKeys && !json.productMode)
    ? admin.warn(
        '<p>This service is in <strong>development</strong> mode and is not ' +
        'persisting signing keys, so <strong>it has never asked for the ' +
        'key-encryption key</strong>. Everything below about that secret ' +
        'describes what <em>would</em> happen, and the ordinary ' +
        '<em>unavailable</em> rows on this page are this service correctly ' +
        'not needing something.</p>' +
        '<p>Signing keys are generated on every start and held in memory ' +
        'here, which is what makes this service disposable. ' +
        '<code>global.mode=product</code>, or ' +
        '<code>keys.source=persisted</code>, is what makes the key real ' +
        '— and in product mode a key that cannot be read is a service ' +
...
// ---------------------------------------------------------------------------
function secretBlock(row, json) {
  log.debug('Entering secretBlock(). secret=' + row.secret);
  const notes = json.notes[row.secret] || { heading: row.secret, what: '',
                                            without: '', rotating: '' };
  if (!row.configured) {
    log.debug('Leaving secretBlock(). Not configured.');
    return '<h3>' + admin.esc(notes.heading) + '</h3>' +
      admin.note(
        '<p>' + notes.what + '</p>' +
        '<p><strong>No provider is configured</strong>, so this service does ' +
        'not read it from a store. <code>' +
  ...
  const read = last
    ...
    : admin.note('<p><strong>This process has not read it.</strong> That is ' +
  ...
  const where = '<table class="grid"><tbody>' +
    ...
    Object.keys(row.where || {}).filter(function (key) {
      ...
      return '<tr><th>' + label(key) + '</th><td>' + cell(row.where[key]) +
             '</td><td class="why"></td></tr>';
    }).join('') +
    (row.field
      ? '<tr><th>Field</th><td><code>' + admin.esc(row.field) + '</code></td>' +
        '<td class="why">Which member is taken when what is stored is a JSON ' +
        'object. A stored value that is not JSON is taken whole. Set by ' +
        '<code>' + admin.esc(row.settings.field) + '</code>.</td></tr>'
      : '') +
  ...
  return '<h3>The stores</h3>' +
    ...
    json.stores.map(function (store) {
      return '<h4>' + admin.esc(store.label) + ' <span class="muted">' +
        ...
        (store.probes.length
          ? probeRows(store.probes)
          : admin.note('This provider publishes nothing about the store ' +
                       'itself beyond what is already on the secret above ' +
                       '&mdash; a cloud secret manager is an endpoint and an ' +
                       'access policy, and everything it will say is said ' +
                       'about the secret.'));
    }).join('');
}
...
app.get('/admin/secrets', function (req, res) {
  ...
  renderSecrets(req, res);
  log.debug('Leaving GET /admin/secrets.');
});

log.info('The secret store report is at /admin/secrets: where the ' +
         'key-encryption key and the database password come from, whether ' +
         'this process read them, and what the store at the other end is ' +
         'doing. No secret value appears on it and it has no control.');
