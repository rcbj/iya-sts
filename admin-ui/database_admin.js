...
//
// Every cell here came out of a `SELECT *`, so this function is the only thing
// that decides how an arbitrary postgres value is drawn — and the cases below
// are each a real shape that arrives, rather than defensive programming:
//
...
//   * **A `bigint` arrives as a STRING** from `pg`, deliberately — it does not
//     fit in a double — so it must not be run through a numeric formatter that
//     would quietly round it.
...
//     this role may not read. It is drawn as the refusal it is rather than as
//     a value, because it is the one string on this page that would otherwise
...
function cell(value) {
  ...
  // A definition or a version banner runs to hundreds of characters and would
...
// A column name as a person reads it. `n_tup_hot_upd` is PostgreSQL's name and
// is what somebody searching its documentation will type, so it is KEPT and
// shown as itself — this only replaces the underscores for the eye. The raw
...
function probeFailure(id, probe) {
  ...
  const why = probe.code === '42P01'
    ? 'This server version does not have that view.' +
      (probe.expected
        ? ' It arrived in PostgreSQL ' + probe.expected + ', so this is the ' +
          'ordinary answer on anything older and not a fault.'
        : '')
    : (probe.code === '42501'
...
// A single-row probe, drawn as label/value pairs. A wide row — thirty columns
  ...
  return '<table class="grid"><tbody>' +
    keys.map(function (key) {
      return '<tr><th>' + columnLabel(key) + '</th><td>' +
             cell(probe.row[key]) + '</td></tr>';
    }).join('') +
    '</tbody></table>';
}

// A many-row probe, drawn as a table whose HEADINGS ARE THE KEYS OF THE FIRST
// ROW. That is the whole reason this page survives a major version change, and
// it is why an empty result has to be handled here rather than falling out of
...
// the renderer, and not in SQL — a division in the probe would make the probe
// version-dependent again for no gain, and these are the four a reader would
// otherwise do in their head and get wrong.
//
...
function derived(probes) {
  log.debug('Entering derived().');
  ...
  const out = {
    cacheHitPercent: ratio(db.blks_hit, db.blks_read),
    ...
    // DEAD TUPLES AS A SHARE OF LIVE ONES, which is the bloat signal. Summed
    // across the schema rather than per table, because the per-table figure is
    // in the table below and a reader wants one number first.
    ...
    // AN INDEX NOTHING HAS EVER SCANNED. The most actionable number here on a
    // database that has been running, and MEANINGLESS on one that has just
    // started — so the page reports the count and says which of the two
    // situations the reader is in rather than implying the first.
...
// compared against a RUNNING SERVER. A database built by an older copy of that
// file, or by hand, or half-migrated, satisfies both and is missing a table.
//
// The driver would create what is missing on the next open, which is exactly
// why this matters: the role it dials with holds no CREATE, so on a
...
function schemaDrift(report) {
  ...
  const indexes = ((report.probes.indexes && report.probes.indexes.rows) || [])
  ...
  // reverse — a table in the schema the driver never heard of — is NOT drift
  // and is deliberately not reported: an operator may keep whatever they like
  // in that schema, and a page calling their table an error would be this
  // service claiming a namespace it does not own.
  ...
  return persistence.databaseMetrics().then(function (report) {
    const out = {
      what: 'Everything PostgreSQL will tell this service about itself, and ' +
            'the state of the schema this service owns in it.',
    ...
    if (!report.available) {
      out.why = report.why;
      log.debug('Leaving databaseJson(). No database.');
      return out;
    }
    ...
    if (!report.ok) {
      ...
      log.debug('Leaving databaseJson(). The collection failed.');
      return out;
    }
    ...
    out.schema = ((report.probes.server && report.probes.server.row) || {})
      .search_schema || null;
    out.derived = derived(report.probes);
    out.schemaExpected = { version: report.schemaVersion };
    out.schemaDrift = schemaDrift(report);
...
function renderDatabase(req, res) {
  log.debug('Entering renderDatabase().');
  databaseJson().then(function (json) {
...
function body(json) {
  ...
  if (!json.available) {
    log.debug('Leaving body(). Not available.');
    return admin.note(
      ...
      'and lock, the checkpointer and the write-ahead log — beside per-table ' +
      'and per-index statistics for the schema this service owns, its sizes, ' +
      'its columns and its constraints. <a href="/admin/persistence">The ' +
      'persistence page</a> is where the mode is configured and is what this ' +
  ...
  const what = admin.note(
    ...
    (json.statementTimeoutSet ? '' :
      ' &mdash; <strong>which this server would not accept, so that bound is ' +
      'NOT in force</strong>') +
  ...
  const connection = admin.note(
    ...
    '<p><strong>The pool figures are this PROCESS\'s and the backend counts ' +
    'are the SERVER\'s</strong>, and they answer different questions: ' +
    'PostgreSQL can say how many connections exist, and only the client can ' +
    ...
    'connection, so the numbers are what it was doing when you asked rather ' +
    'than what it is doing because you asked &mdash; on a pool whose maximum ' +
    'is four, counting our own would invent a quarter of it.</p>',
    'Which end each number comes from');

  // THE NARROWED ANSWER, said once and prominently rather than as a footnote
  // under a table somebody has already misread.
  ...
  if (!json.ok) {
    log.debug('Leaving body(). The collection failed.');
    return tiles + what + admin.warn(
      '<p>No statistics could be collected at all: <code>' +
      admin.esc(String(json.error)) + '</code></p>' +
      '<p>That is ONE fact rather than twenty &mdash; it means no connection ' +
      ...
      'The service itself may still be answering: it holds the directory in ' +
  ...
  return tiles + what + connection + narrowed +
         ...
         SECTIONS.map(function (section) {
           return sectionBlock(section, json);
         }).join('') +
         failureBlock(json);
}
...
function derivedBlock(json) {
  ...
  return '<h3>The four ratios</h3>' + admin.note(
    ...
    '<tr><th>Dead tuples</th><td>' +
      (d.deadTuplePercent === null ? '<span class="muted">no rows yet</span>'
    ...
    '</tbody></table>' + unused +
    '<p class="muted"><strong>Every figure on this page is cumulative since ' +
    (d.statsReset
      ? admin.esc(String(new Date(d.statsReset).toISOString()
          .replace('T', ' ').replace(/\..*$/, 'Z')))
      : 'the statistics were last reset') +
    '</strong>, which is what PostgreSQL counts from. A cache hit ratio over ' +
    'the life of a server says nothing about the last hour, and reading it ' +
    'as a current figure is the one misunderstanding this page can actually ' +
    'cause.</p>',
...
function driftBlock(json) {
  ...
  const body = drift.missing.length
    ? admin.warn(
        '<p><strong>' + drift.missing.length + ' object(s) this service\'s ' +
    ...
    : admin.note(
        ...
        'neither of those is ever compared against a <em>running server</em> ' +
        '&mdash; so a database built by an older copy of that file, or by ' +
        'hand, or half-migrated, satisfies both and is missing a table.</p>' +
        '<p>The reverse is deliberately <em>not</em> reported: a table in ' +
        'this schema the driver never heard of is an operator\'s business, ' +
  ...
  return '<h3>' + admin.esc(section.heading) + '</h3>' +
    ...
    ids.map(function (id) {
      ...
      return '<h4>' + admin.esc(id) +
        ...
        (probe.shape === 'rows' ? rowsTable(probe) : rowTable(probe));
    }).join('');
}
...
function failureBlock(json) {
  ...
  return '<h3>What could not be collected</h3>' + admin.note(
    ...
    'operator\'s grants, and one rejection must not take the other nineteen ' +
    'with it.</p>' +
    '<table class="grid"><thead><tr><th>Probe</th><th>What it would show</th>' +
    '<th>SQLSTATE</th><th>Why not</th></tr></thead><tbody>' +
    json.failed.map(function (id) {
      return probeFailure(id, json.probes[id]);
    }).join('') +
    '</tbody></table>',
...
app.get('/admin/database', function (req, res) {
  ...
  renderDatabase(req, res);
  log.debug('Leaving GET /admin/database.');
});

log.info('The database report is at /admin/database: everything PostgreSQL ' +
         'will tell this service about itself, and the state of the schema ' +
         'this service owns in it. It is empty unless persistence.mode is ' +
