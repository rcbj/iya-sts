'use strict';
//
// File: delegation_map.ts
//
// ---------------------------------------------------------------------------
// THE DELEGATION PICTURE. `common/delegation.js`'s graph, drawn.
//
// It is a LIBRARY, like `admin_rbac.ts` next door and like `dpop.js`,
...
// shape: `DelegationMap` takes the logger, the XML escaper, the error codes
// and `@dagrejs/dagre` through its constructor, and every function this file
...
// layout threw — the reason, which the page shows instead of a picture.
interface Drawing {
  svg: string;
  width: number;
  height: number;
...
class DelegationMap {
  ...
  private markerId(colour) {
    const { log } = this.deps;
    log.debug("Entering DelegationMap.markerId().");
    log.debug("Leaving DelegationMap.markerId().");
    return 'dm-arrow-' + colour.replace('#', '');
  }

  private tailId(colour) {
    const { log } = this.deps;
    log.debug("Entering DelegationMap.tailId().");
    log.debug("Leaving DelegationMap.tailId().");
  ...
  textWidth(text, size) {
    ...
    log.debug("Leaving DelegationMap.textWidth().");
    return units * size;
  }
  ...
  wrapLabel(text, maxChars, maxLines) {
    ...
    if (!lines.length) {
      log.debug("Leaving DelegationMap.wrapLabel().");
      return [''];
    }
  ...
  private esc(v) {
    ...
    log.debug("Entering DelegationMap.esc().");
    log.debug("Leaving DelegationMap.esc().");
  ...
  //
  // One function answers both, because they are the same question asked twice
  // and two functions would be two chances for the drawing to be a shape the
  // layout did not reserve room for. `measure()` returns the size AND the
  // wrapped lines, and the drawing below takes both rather than re-wrapping.
  // ---------------------------------------------------------------------------
  private measure(node, look) {
    const { log } = this.deps;
    const self = this;
    log.debug("Entering DelegationMap.measure().");
    const lines = self.wrapLabel(look.label, MAX_LABEL_CHARS, 2);
    // TWO SUB-LINES AT MOST, AND THEY ARE DIFFERENT SENTENCES. The first is
    // what kind of thing this is — `application`, `person + application` — and
    // the second, added 2026-08-27, is the IDENTIFIER a protocol would have to
    ...
    // entityID is a URL and would otherwise push `application` off its own
    // line.
    //
    ...
    const subLines = (look.sublabel ?
                      self.wrapLabel(look.sublabel, MAX_LABEL_CHARS + 6, 1)
                      : [])
      .concat(look.identifier ?
              self.wrapLabel(look.identifier, MAX_LABEL_CHARS + 6, 2) : []);
    let textW = 0;
    lines.forEach(function (one) {
      textW = Math.max(textW, self.textWidth(one, LABEL_SIZE));
    });
    subLines.forEach(function (one) {
      textW = Math.max(textW, self.textWidth(one, SUB_SIZE));
    });
    const textH = lines.length * LINE_HEIGHT +
                  (subLines.length ? subLines.length * LINE_HEIGHT - 2 : 0);

    if (look.shape === 'person') {
      // The figure sits above the name rather than beside it. A person's label
      // is a username and is short; a figure to the left of it would make every
      // person-box a wide rectangle in a picture where a rectangle already
      // means something else.
      log.debug("Leaving DelegationMap.measure().");
      return {
        shape: 'person',
    ...
    // `application` and `both`. The second is the first with a figure inside
    // it, so it is the same rectangle with room made on the left.
  ...
  personGlyph(x, y, colour, dashed, scale) {
    const { log } = this.deps;
    const self = this;
    log.debug("Entering DelegationMap.personGlyph().");
    const s = scale || 1;
    const w = FIGURE_W * s;
    const h = FIGURE_H * s;
    const cx = x + w / 2;
    ...
    log.debug("Leaving DelegationMap.personGlyph().");
    return '<g fill="none" stroke="' + colour + '" stroke-width="' + (1.6 * s) +
      '" stroke-linecap="round"' + dash + '>' +
      '<circle cx="' + self.round(cx) + '" cy="' + self.round(headY) + '" r="' +
      self.round(headR) + '"/><path ' +
      'd="M' + self.round(cx) + ' ' + self.round(neck) + 'V' + self.round(hip) +
        'M' + self.round(cx - arm) + ' ' + self.round(neck + 4 * s) +
        'H' + self.round(cx + arm) +
        'M' + self.round(cx) + ' ' + self.round(hip) + 'L' +
        self.round(cx - arm) + ' ' +
      self.round(foot) +
        'M' + self.round(cx) + ' ' + self.round(hip) + 'L' +
        self.round(cx + arm) + ' ' +
  ...
  hexPath(x, y, w, h) {
    const { log } = this.deps;
    const self = this;
    log.debug("Entering DelegationMap.hexPath().");
    const cut = Math.min(22, w / 4);
    log.debug("Leaving DelegationMap.hexPath().");
    return 'M' + self.round(x + cut) + ' ' + self.round(y) +
      'H' + self.round(x + w - cut) +
      'L' + self.round(x + w) + ' ' + self.round(y + h / 2) +
      'L' + self.round(x + w - cut) + ' ' + self.round(y + h) +
      'H' + self.round(x + cut) +
      'L' + self.round(x) + ' ' + self.round(y + h / 2) + 'Z';
  }

  private round(n) {
    const { log } = this.deps;
    log.debug("Entering DelegationMap.round().");
    log.debug("Leaving DelegationMap.round().");
    return Math.round(n * 10) / 10;
  }

  // WHERE A LINE LEAVES A BOX. The point at which the segment from that box's
  // centre towards `towards` crosses the box's own edge, so an arrowhead sits
  // on the boundary rather than under the label in the middle. dagre does this
  // for the lines it routes; these are the ones it never saw.
  private boundaryPoint(at, size, towards) {
    const { log } = this.deps;
    log.debug("Entering DelegationMap.boundaryPoint().");
    ...
    if (!dx && !dy) {
      ...
      log.debug("Leaving DelegationMap.boundaryPoint(). The two boxes share " +
                "a centre.");
    ...
    log.debug("Leaving DelegationMap.boundaryPoint().");
  ...
  //
  // `boundaryPoint()` above scales a ray that STARTS at a box's own centre,
  // which is every line in the picture but the fanned ones: those are aimed at
  // a point `STS_FAN_SEP` to one side of the hexagon's centre, so the ray they
  // travel on does not pass through it and scaling it would land beside the
  // shape. This clips the segment against the box's two slabs instead and
  // answers with the point where it ENTERS — the far side is behind the shape
  // and is never the one wanted.
  //
  // `aim` is INSIDE the box (the fan is capped so that it is), so there is
  // always a crossing; a caller that passed one outside would get the entry of
  // the extended line, which is still on the boundary.
  // ---------------------------------------------------------------------------
  private crossingPoint(from, aim, centre, size) {
    const { log } = this.deps;
    const self = this;
    log.debug("Entering DelegationMap.crossingPoint().");
    const dx = aim.x - from.x;
    const dy = aim.y - from.y;
    if (!dx && !dy) {
      // The same divide-by-zero guard boundaryPoint() carries, for the
      // same reason: NaN in a path draws nothing at all.
      log.debug("Leaving DelegationMap.crossingPoint(). The line has no " +
                "length.");
    ...
    log.debug("Leaving DelegationMap.crossingPoint().");
    return { x: from.x + dx * t, y: from.y + dy * t };
  }

  // ---------------------------------------------------------------------------
  // AN EDGE'S ROUTE. dagre hands back the points a line should pass through —
  // one per rank it crosses, including the invisible ranks it made for the
  // labels — and a polyline through them has a corner at every one.
  //
  // So they are smoothed: a curve through the MIDPOINTS of consecutive
  // segments, with each original point as the control. That is the standard
  // trick and it has one property worth the two lines it costs — the curve
  // touches the first and last points exactly, so a line still starts on the
  // edge of the box it leaves and ends on the edge of the box it enters, which
  // is the one thing a reader would notice if it were approximate.
  // ---------------------------------------------------------------------------
  private edgePath(points) {
    const { log } = this.deps;
    const self = this;
    log.debug("Entering DelegationMap.edgePath().");
    if (!points || points.length < 2) {
      log.debug("Leaving DelegationMap.edgePath().");
      return '';
    }
    if (points.length === 2) {
      log.debug("Leaving DelegationMap.edgePath().");
      return 'M' + self.round(points[0].x) + ' ' + self.round(points[0].y) +
             'L' + self.round(points[1].x) + ' ' + self.round(points[1].y);
    }
    let d = 'M' + self.round(points[0].x) + ' ' + self.round(points[0].y);
    for (let i = 1; i < points.length - 1; i++) {
      const midX = (points[i].x + points[i + 1].x) / 2;
      const midY = (points[i].y + points[i + 1].y) / 2;
      d += 'Q' + self.round(points[i].x) + ' ' + self.round(points[i].y) + ' ' +
           self.round(midX) + ' ' + self.round(midY);
    }
    const last = points[points.length - 1];
    d += 'L' + self.round(last.x) + ' ' + self.round(last.y);
    log.debug("Leaving DelegationMap.edgePath().");
    return d;
  }
  ...
  //      refused is the row a person came to this page to find, and it is the
  //      one state that must survive being glanced at.
  ...
  //   3. IT IS A DELEGATION RELATIONSHIP — the MODE's colour, amber or green,
  //      the pairing modeCell() uses in the table.
  //   4. IT IS A TRUST RELATIONSHIP — indigo, the console's own colour. What
  //      this line says is what the credential was FOR, which is a fact about
  //      this service rather than about either party.
  // ---------------------------------------------------------------------------
  private edgeLook(edge) {
    const { log } = this.deps;
    log.debug("Entering DelegationMap.edgeLook().");
    ...
    //
    //   * `signed-in` — the person authenticated TO this service. DOTTED,
    //     because it is the one line that is not a credential going anywhere;
    //     it is how everything else on the picture came to be allowed.
    //   * `issued-for` — a credential naming this person went to that
    //     application. Solid indigo, the same weight as `reaches`, because it
    //     IS the trust relationship: what this token is FOR, said about a grant
    ...
    // happened. Every other look above describes an act: a credential was
    // issued, refused, or carried a chain. A `may-reach` line says a client
    // application has been GRANTED a permission on a resource application and
    // nothing more — nobody has asked for it, no token exists, and there is no
    ...
    // exercised has performed neither. Colouring it green would tell a reader
    // who has learnt the pairing something false.
    //
    // **DASHED UNTIL IT HAS BEEN USED, SOLID AFTERWARDS**, and that one bit is
    // the most useful thing the configured picture says. `asked` is set when
    // the client's own entry records having requested that permission in a
    // `scope`, so a dashed line is a grant nobody has needed — which is the
    // reading a configuration register exists for and the one an acts diagram
    // can never give, because a grant nobody used draws no act at all.
    ...
    if (edge.relation === 'acts-for') {
      ...
      // thing being said is that part of this line is not known: an
      // unconstrained delegation reaches its target through a service this KDC
      // was never told the name of, and a solid line would assert a hop that
      // nobody can name.
      log.debug("Leaving DelegationMap.edgeLook().");
  ...
  // The words on a line. Up to three short ones — what kind of relationship,
  // the mechanism, and how it came out — because everything else is in the
  // <title> and in the tables under the picture. A fourth line was tried and it
  // is what turns a diagram into a page of text laid out badly.
  //
  // THE ONE EXCEPTION IS THE SIGN-IN LINE, which is allowed a fourth and a
  // fifth because it is the one line that ABSORBED others: there was a line per
  // protocol family until 2026-08-26, each with three lines of label, and they
  // joined the same two boxes and were therefore drawn on top of each other.
  // The list it carries instead is shorter than what it replaced.
  private edgeLabelLines(edge, labelOf) {
    const { log } = this.deps;
    const self = this;
    log.debug("Entering DelegationMap.edgeLabelLines().");
    const lines = [];
    if (edge.relation === 'issued') {
      ...
      // ONE STATEMENT AND THEN A LIST — see `signedInLines()`. There is
      // exactly one of these lines per person, and what varies is on it: `1 ×
      // OAuth 2.0 / OIDC (sign-in screen)`, `2 × OAuth 2.0 (token exchange)`.
      lines.push('signed in');
      self.signedInLines(edge).forEach(function (one) {
        lines.push(one);
      });
    } else if (edge.relation === 'issued-for') {
      // AND THE GRANT, WHICH IS THE WHOLE LABEL. `typeLabel` carries the flow's
      // own name — `Authorization Code grant`, `Refresh Token grant` — so this
      // is the same expression the mechanism gets on a delegation line, which
      ...
      if (edge.permissionName) {
        lines.push(self.trim(edge.permissionName, 22));
      }
      ...
      // be one where the single most useful thing on it was invisible to
      // anybody who had not read the key.
      lines.push(edge.asked ? 'asked for' : 'never asked for');
      if (edge.typeLabel) {
        lines.push(self.shortType(edge.typeLabel));
      }
    } else {
      lines.push(edge.subject
        ? 'reaches as ' + self.trim(labelOf(edge.subject), 20) : 'reaches');
      ...
      // none, which is what this test is: an ACT has no scope claim behind it,
      // so a line about one has nothing to say here and says nothing rather
      // than saying `default permissions` about a Kerberos ticket.
      //
      // **AN EMPTY ARRAY IS AN ANSWER AND IS DRAWN AS ONE.** The token named
      // this resource — by the client_id spelling, which takes the value off
      // the scope claim — and asked for none of its permissions, and `default
      // permissions` is this service's word for that. Leaving the line blank
      // there would make the commonest case indistinguishable from a line the
      // renderer has not been taught about, and it is exactly the distinction
      // the configured picture makes with `never asked for`.
      if (edge.permissions) {
        lines.push(edge.permissions.length
          ? self.trim(edge.permissions.join(', '), 26)
          : 'default permissions');
      }
      if (edge.typeLabel) {
        lines.push(self.shortType(edge.typeLabel));
    ...
    const counts = [];
    // A CREDENTIAL COUNT WHERE THE LINE IS ABOUT CREDENTIALS, an act count
    // where it is about acts. The two are different units and one column for
    // both would report `3 issued` on a line that carries three tokens and on a
    // line that carries three delegations, which are not comparable numbers.
    if (edge.relation === 'issued-for') {
      if (edge.credentials) {
      ...
      if (edge.issued) {
        counts.push(edge.issued + ' issued');
      }
      if (edge.refused) {
        counts.push(edge.refused + ' refused');
      }
      // An `issued` line in a person's picture carries credentials that came
      // out of no delegation act at all, so its act count can be zero while it
      // is the busiest line on the page.
    ...
    log.debug("Leaving DelegationMap.edgeLabelLines().");
    ...
    // there are who it is as, what mechanism issued it, and how many
    // credentials — none of which the permission can be inferred from. Dropping
    // one of those to make room was the alternative and each of them is
    // somebody's reason for reading the picture. Four lines is 44px, which is
    // what the sign-in line already occupies, so the bands below measure it and
  ...
  // said before the fold. That is not defensive dressing: `delegation_map.ts`
  ...
  // ---------------------------------------------------------------------------
  private signedInLines(edge) {
    const { log } = this.deps;
    const self = this;
    log.debug("Entering DelegationMap.signedInLines().");
    const entries = edge.authentications || [];
    const out = [];
    if (!entries.length) {
      if (edge.protocol) {
        out.push(self.trim(edge.protocol, 26));
      }
      if (edge.acts) {
        out.push(edge.acts + ' time' + (edge.acts === 1 ? '' : 's'));
      }
      log.debug("Leaving DelegationMap.signedInLines(). No list on the " +
                "edge; " + out.length +
    ...
    shown.forEach(function (one) {
      // `shortType()` on the method for the reason it is used on a
      // mechanism everywhere else here: `sign-in screen (password and a
      // security key)` is a sentence and `sign-in screen` is the noun. The
      // parenthetical is in the tooltip.
      out.push(self.trim(one.count + ' × ' + one.protocol +
                         (one.method ? ' (' + self.shortType(one.method) + ')'
                                     : ''),
                         SIGNED_IN_CHARS));
    });
    ...
    log.debug("Leaving DelegationMap.signedInLines(). " + out.length +
              " line(s) for " + entries.length + " entry/entries.");
    return out;
  }

  // The mechanism's label, minus the parenthetical the table has room for.
  // Every one of the eight is a phrase; the picture wants the noun.
  private shortType(label) {
    const { log } = this.deps;
    const self = this;
    log.debug("Entering DelegationMap.shortType().");
    log.debug("Leaving DelegationMap.shortType().");
    return self.trim(String(label).replace(/\s*\(.*\)\s*$/, '')
                       .replace(/\s+—.*$/, ''),
                     26);
  }

  private trim(text, max) {
    const { log } = this.deps;
    log.debug("Entering DelegationMap.trim().");
    const value = String(text == null ? '' : text);
    log.debug("Leaving DelegationMap.trim().");
  ...
  //
  // and the default below is what a caller that has no directory gets — the
  // shape the ROLE implies, dashed, because "we do not know" is a state and not
  // a failure. `spiffe_server.js` and the two SAML modules all pass a resolver
  // of their own to somebody; this is the same arrangement and the reason is
  // the one in the header: what a party IS belongs to the console, where the
  // registry and the directory reader are, and where a box GOES belongs here.
  // ---------------------------------------------------------------------------
  private defaultResolve(node) {
    const { log } = this.deps;
  ...
  //   options   resolve(node) as above; `links`, which wraps every box in an
  //             <a> to its page in this console; `id`, a prefix for every
  //             generated id in the document, because two pictures in one
  //             HTML page would otherwise share <defs> ids and the second
  //             one's arrowheads would be the first one's.
  //
  ...
  // gives about the table it feeds: a drawing on a console page must not be
  // able to take the page down. Everything else on /admin/delegation/map — the
  // tables, the counts, the legend — is worth reading with no picture above it,
  ...
  render(graph: any, options?: any): Drawing {
    ...
    const self = this;
    log.debug("Entering DelegationMap.render().");
    try {
      const svg = self.renderUnguarded(graph || { nodes: [], edges: [] },
                                       options || {});
      log.debug("Leaving DelegationMap.render(). " + svg.svg.length +
                " bytes of SVG, " + svg.width + "x" + svg.height + ".");
      return svg;
    } catch (e) {
      ...
      log.debug("Leaving DelegationMap.render().");
      return {
        svg: '<p class="err">The picture could not be drawn: ' +
             self.esc(e.message) +
             '. Everything below is unaffected — the tables are built from ' +
             'the same graph and do not go through the layout.</p>',
        width: 0, height: 0, nodes: 0, edges: 0, failed: e.message
      };
    }
  }

  private renderUnguarded(graph: any, options: any): Drawing {
    const { log, dagre } = this.deps;
    const self = this;
    log.debug("Entering DelegationMap.renderUnguarded().");
    const resolve = typeof options.resolve === 'function' ? options.resolve :
                    function (node) {
                      return self.defaultResolve(node);
                    };
    ...
    // carries. The caller supplies it because it is the same question
    // `resolve()` answers and the answer belongs to whoever knows the
    // directory; with no resolver the identity is its own name, which is what
    // `defaultResolve()` already does for a box.
    ...
    // THE ISSUER IS DRAWN, NOT LAID OUT — see the note on STS_BAND_SEP. It
    // comes out of the graph handed to dagre along with every line that touches
    // it, and is put back afterwards in a band of its own above everything
    // else.
    const stsNode =
    ...
    const stsEdges = edges.filter(function (one) {
      // A line with the hexagon at BOTH ends cannot exist — `delegation.js`
      // never makes one — and would be drawn as a point if it did, so it is
      // dropped here rather than divided by zero below.
      return (isSts(one.from) || isSts(one.to)) && one.from !== one.to;
    });

    // NOT a multigraph, and it stopped being one on 2026-09-01 for a reason
    // worth reading before it is put back. It was one because two chains
    // between the same pair of boxes are two lines — `alice -> frontend` by
    // classic constrained delegation and the same pair by RBCD are two
    // arrangements — and that is still true of the PICTURE. It was never true
    // of what dagre is asked for: everything it computes about an edge is
    // thrown away below (its routes described the staircase this file flattens,
    // and its label positions described a gap this file measures for itself),
    // and the only thing kept out of this whole layout is the ORDER of the
    // boxes. Two lines between one pair cannot change an order.
    //
    ...
    // applications each granting the other four two permissions is five boxes
    // and FORTY lines — has both a pair in each direction AND parallel lines
    // within a pair, and dagre 1.x positions a dummy node at NaN when it meets
    // the two together. `assignNodeIntersects()` then subtracts that NaN, finds
    // neither a dx nor a dy, and throws `Not possible to find intersection
    // inside of the rectangle` out of the whole render — so the page said the
    // picture could not be drawn, of a graph that draws perfectly. Neither
    // ingredient does it alone: parallel lines in one direction are fine, and a
    // pair in both directions is fine. Collapsing is what removes the one this
    // file does not need.
    //
    ...
    // ranking, for the same reason — and `partyEdges` is untouched, so every
    // one of the forty is still drawn. `compound` is off — there are no nested
    // boxes here.
    ...
    nodes.forEach(function (node) {
      const look = resolve(node) || self.defaultResolve(node);
      const size = self.measure(node, look);
    ...
    edges.forEach(function (edge) {
      ...
      edgeLabels[edge.id] = self.edgeLabelLines(edge, nameOf);
    });
    ...
    partyEdges.forEach(function (edge) {
      ...
      let width = 0;
      lines.forEach(function (one) {
        width = Math.max(width, self.textWidth(one, EDGE_SIZE));
      });
      // The key is the ORDERED pair: `a -> b` and `b -> a` are two lines to
      // dagre and always were, and collapsing those two would be collapsing a
      // cycle into an edge — which is the one thing that WOULD change the order
      // it answers.
      ...
      // The widest and the tallest of the labels collapsed here. dagre reserves
      // a rank for a label and this is what it reserves; the gap the picture
      // actually leaves is measured again by `labelWidthOf()` below, off the
      // individual lines, so this figure has only to be honest rather than
      // exact.
      ...
      // pulled together four times as hard as a pair joined by one, which is
      // the whole of what the parallel lines were saying to the ordering pass.
      pair.weight += 3;
    });
    ...
    pairs.forEach(function (pair) {
      g.setEdge(pair.from, pair.to, {
        ...
        // LAYERS of the model, and a longer minlen on any one of them would
        // open a gap that says something the model does not.
        minlen: 1
      });
    });
    ...
    // which is done by writing back into dagre's OWN nodes rather than into a
    // map beside them so that everything downstream — the label fitting, the
    ...
    // anything else. In a `rankdir: 'LR'` layout the RANK is the x, so every
    // node dagre puts on one rank has the SAME x and is told apart only by the
    // y — which is the coordinate that was just thrown away. A chain has one
    // node per rank and came out perfect; a FAN — one person, four
    // applications, which is the ordinary shape of a busy person's picture —
    // came out as four boxes drawn exactly on top of each other.
    // `tests/delegation_map_bands.js` had that fixture already and caught it.
    //
    // So the row owns both coordinates and dagre is left with the ONE thing it
    // is being kept for: the ORDER. Its rank assignment is the depth of the
    // chain and its ordering pass is the arrangement within a rank that crosses
    // fewest lines, so sorting by (x, then y) is that whole result read off as
    // a sequence — rank by rank, and inside a rank in the order it chose.
    //
    ...
    // what dagre was doing: `ranksep` reserved a rank for an edge label and
    // hoped it fitted, and this measures the label and leaves exactly that
    // much. It is also what makes the straight-line rule below decidable at all
    // — "are these two neighbours" is a question about a sequence, not about
    // geometry.
    // -------------------------------------------------------------------------
    const order = partyNodes.filter(function (node) {
      return !!g.node(node.id);
    })
    ...
    function labelWidthOf(edge) {
      ...
      (edgeLabels[edge.id] || []).forEach(function (one) {
        widest = Math.max(widest, self.textWidth(one, EDGE_SIZE));
      });
    ...
    // between the same two boxes is a second mechanism — `multigraph` is why
    // they both exist — and drawing both along the row would draw them on top
    // of each other. Everything else arcs under it.
    const straightAt = {};
    const route = {};
    partyEdges.forEach(function (edge) {
    ...
    // PACKED. The gap holds the straight line's label with air on both sides,
    // or is `GAP_MIN` where no line lies in it — two boxes that nothing joins
    // still have to look like two boxes.
    let cursor = MARGIN;
    order.forEach(function (node, index) {
    ...
    // apart have their labels a fixed distance apart, and three lines of text
    // at `LINE_HEIGHT` do not fit in `ARC_LANE_H`. The first version of this
    // used the constant and the picture came back with `acts for / Token
    // exchange / 1 issued` written across the arc under it — which is the
    // failure the issuer's label rows were invented to fix, one band lower.
    //
    // In the party band's OWN coordinates: `bandH` is not known yet, because
    // how deep the issuer's band is depends on how ITS labels fit and that is
    // settled below. The routing adds it, which is what the placement loop does
    // to every node position anyway.
    // -------------------------------------------------------------------------
    const laneApex = [];
    ...
    // dagre's own graph, because half of what is in them did not come from
    // dagre: the hexagon's position and its lines are computed below, and the
    // renderer must not be able to tell which half it is drawing.
    const placed = {};
    const routed = {};
    ...
    // NOT dagre's height, which describes the staircase that was just
    // flattened. The row is as tall as its tallest box, and under it whatever
    // the arcs needed — nothing at all where every line lies along the row.
    ...
    if (!partyNodes.length) {
      ...
      // asked for the size of a graph with no nodes and answers with the
      // margins only, which would leave the hexagon with nothing to be centred
      // over.
      partyW = MARGIN * 2;
      partyH = 0;
    }
    ...
    // parties centred under IT when they are not — which is what a picture of
    // one application looks like.
    ...
    // its own line, which is the sort of wrong that looks like a rounding
    // error.
    const stsAt = stsNode
      ? { x: width / 2, y: MARGIN + stsSize.height / 2 }
      : null;

    // HOW FAR TO ONE SIDE OF THE ISSUER'S CENTRE EACH LINE IS AIMED. See the
    // note on STS_FAN_SEP: the lines of ONE party are the same segment computed
    // once per line, so a party with two of them gets one path drawn over the
    // other and two labels seated over one line. They are grouped by the party
    // at the far end — every line touching the hexagon touches exactly one —
    // and spread about the centre, so a party with a single line is aimed at
    // the centre and is drawn exactly as it was.
    const stsFan = {};
    const stsLinesOf = {};
    ...
    Object.keys(stsLinesOf).forEach(function (partyId) {
      ...
      // Capped by the shape: the aim has to stay inside the hexagon, or the
      // line it describes leaves it somewhere the reader can see it did not.
    ...
    // the note on LABEL_ROW_H: the answer is decided by the labels rather than
    // by a constant, so it is settled before anything is positioned.
    const stsLabelled = stsEdges.filter(function (edge) {
      ...
      let labelW = 0;
      lines.forEach(function (one) {
        labelW = Math.max(labelW, self.textWidth(one, EDGE_SIZE));
      });
      return {
        edge: edge, partyId: partyId,
        width: Math.ceil(labelW) + 10,
        height: lines.length * (LINE_HEIGHT - 2) + 6,
        // WHERE THIS LINE IS AIMED, so that its label is seated on the line
        // that is actually drawn rather than on the one that would have been
        // drawn if this were the party's only line. Zero for every party with
        // one.
        fan: stsFan[edge.id] || 0,
        // Sorted on where the line ENDS, so the rows are filled left to right
        // and a reader following the fan outwards meets them in order.
    ...
    // assignment, or null when two of them would still overlap — the caller
    // tries again with one more row. Greedy and first-fit, which is enough: the
    // labels are sorted by where their lines end, so a row is filled left to
    // right and a clash is always with the one immediately before it.
    function fitLabels(rows) {
      log.debug("Entering fitLabels(). rows=" + rows);
      const gap = Math.max(STS_BAND_SEP,
                           rows * LABEL_ROW_H + LABEL_ROW_PAD * 2);
      ...
      for (let i = 0; i < stsLabelled.length; i++) {
        ...
        for (let r = 0; r < rows && !put; r++) {
          ...
          const x = originX +
            (one.partyX - originX) * ((y - stsCentre.y) / span);
    ...
    if (!fitted) {
      // Past the giving-up point. Everything goes in the last row it was
      // offered and some of them will overlap, which is the honest outcome
      // rather than a band taller than the diagram — see the note on
      // MAX_LABEL_ROWS.
      log.warn('delegation_map: ' + stsLabelled.length + ' lines out of the ' +
               ...
               'the relationship table under the picture carry the same ' +
               'words.');
      fitted = fitLabels(MAX_LABEL_ROWS) || { gap: STS_BAND_SEP, at: {} };
    }

    // The gap exists to hold the issuer's labels and to separate two bands, so
    // a picture with only the hexagon in it has neither and gets neither: an
    // empty register draws one shape and 78 pixels of nothing under it
    // otherwise.
    ...
    Object.keys(berths).forEach(function (id) {
      ...
      //
      // Which gives the whole rule in two lines. Arcs going LEFT take the left
      // of the edge and arcs going RIGHT take the right, because two arcs
      // leaving in opposite directions from berths on the wrong sides of each
      // other cross immediately. And within each side the SHALLOWEST is the
      // outermost — so going left, lanes ascend from the left; going right,
      // they descend into it. The far box's x is not consulted at all: the lane
      // is what a crossing is actually about, and it usually agrees with
      // distance and does not have to.
      //
      // No two arcs at one box can share a lane — a lane holds arcs that do not
      // overlap horizontally, and two arcs off one box always overlap there —
      // so this is a total order and the id is only a tie-break that should
      // never be reached. It is here so that a picture drawn twice from one
      // graph is the same picture, which is what lets a reader compare it with
      // the one they were looking at a moment ago.
      // ---------------------------------------------------------------------
      ...
      held.forEach(function (one, index) {
        berthAt[one.id + '|' + one.end] =
          placed[id].x - span / 2 + step * index;
      });
    });
    // A berth if one was assigned, and the old lean if not — which is every arc
    // this loop is asked for that the block above did not see, and is not a
    // state that should arise. It is a FALLBACK rather than an assertion for
    // the reason `render()` is wrapped: a drawing on a console page must
    // not be able to take the page down, and a line a third of the way off
    // centre is a worse picture and still a picture.
    ...
    //
    // A straight line is clipped to both boxes so the arrowhead lands on an
    // edge, which is `boundaryPoint()`'s whole job. An arc is a cubic that
    // leaves the BOTTOM of one box and enters the bottom of the other, with
    // both controls on its lane — which is the one shape that comes back to the
    // row at both ends without a corner, and whose deepest point is three
    // quarters of the way down to the controls. That fraction is why the apex
    // is computed and not assumed: the label goes there, and a label at the
    // control depth would sit clear of the line it belongs to.
    // -------------------------------------------------------------------------
    partyEdges.forEach(function (edge) {
      const how = route[edge.id];
      ...
      if (how.straight) {
        const from = self.boundaryPoint(placed[edge.from],
                                        drawn[edge.from].size,
                                        placed[edge.to]);
        const to = self.boundaryPoint(placed[edge.to], drawn[edge.to].size,
                                      placed[edge.from]);
        routed[edge.id] = {
          points: [from, to],
      ...
      // ITS BERTH ON EACH BOX'S BOTTOM EDGE, assigned above. The y is the
      // bottom edge itself, so the start is hidden under the box the way
      ...
      // WHERE THE CURVE ACTUALLY DIPS TO, solved for rather than assumed. A
      // cubic whose two controls share a y is at ((y0 + y1) / 8) + (3 / 4)c
      // when it is halfway along, which is where its lowest point is when the
      // ends are level — and they are NOT always level here, because two boxes
      // on one centreline have different heights when one of their labels
      // wrapped. So the control is computed from the lane's apex instead of the
      // apex from the control: the label goes at the apex, and a label a
      // box-height away from its own line is a label belonging to nothing.
      ...
      routed[edge.id] = {
        ...
        // `edgePath()`'s smoothing — which is built for a polyline with
        // corners in it and would round this one away from its own lane.
        path: 'M' + self.round(from.x) + ' ' + self.round(from.y) +
              'C' + self.round(from.x) + ' ' + self.round(control) + ' ' +
              self.round(to.x) + ' ' + self.round(control) + ' ' +
              self.round(to.x) + ' ' + self.round(to.y),
    ...
    if (stsNode) {
      ...
      stsEdges.forEach(function (edge) {
        ...
        // — where all but the outermost of these lines leave it — and a few
        // pixels generous at the two slanted ends. An arrow starting a little
        // inside the shape it is leaving is invisible; the alternative is
        // intersecting a six-sided path, for that. THE AIM, which is the
        // hexagon's centre for all but a fanned line — see the note on
        // STS_FAN_SEP. The party end is scaled along the ray it travels on,
        // exactly as an unfanned line is; the hexagon end cannot be, because
        // that ray no longer passes through the hexagon's centre, so it is
        // clipped against the shape instead.
        const partyId = isSts(edge.from) ? edge.to : edge.from;
        const fan = stsFan[edge.id] || 0;
        const aim = { x: stsAt.x + fan, y: stsAt.y };
        const partyEnd = self.boundaryPoint(placed[partyId],
                                            drawn[partyId].size, aim);
        const stsEnd = fan
          ? self.crossingPoint(placed[partyId], aim, stsAt, stsSize)
          : self.boundaryPoint(stsAt, stsSize, placed[partyId]);
        ...
        routed[edge.id] = {
          points: [from, to],
          // Halfway along when there is no label to seat, which is where
    ...
    const defs = '<defs>' + ARROW_COLOURS.map(function (colour) {
      return '<marker id="' + prefix + '-' + self.markerId(colour) +
        '" viewBox="0 ' +
        '0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" ' +
        'orient="auto-start-reverse"><path d="M0 0L10 5L0 10z" ' +
        'fill="' + colour + '"/></marker>' +
        // The tail. See tailId() for why there is one and why it is the
        // shape and the size it is. `orient="auto"` looks pointless on a disc
        // and is not: it is what makes `refX="0"` mean "forward ALONG THE LINE"
        // rather than "to the right of the page", which is the default and
        // which set the disc beside the start of a line that leaves downwards
        // instead of below it — straddling the box's own bottom edge, where the
        // box (painted after the edges) covered its upper half and drew it as a
        // half moon.
        '<marker id="' + prefix + '-' + self.tailId(colour) +
        '" viewBox="0 0 10 ' +
        '10" refX="0" refY="5" ' +
    ...
    // THE EDGES AND WAS NOT UNTIL 2026-09-01. Each edge used to emit its own
    // line and then its own label in one group, so document order interleaved
    // them: the label of the FIRST line was painted before the fortieth line
    // was drawn, and the fortieth line then went straight across the words. On
    // the delegated permission example — five applications each granting the
    // other four two permissions — 27 of the 40 labels were crossed by another
    // line and 25 of them were crossed by a line painted ON TOP, which is what
    // the picture looked unreadable BECAUSE OF. The lane assignment was not the
    // problem and is not touched: no two label panels overlapped there, then or
    // now.
    //
    // The panel is semi-transparent on purpose — a label sits on its own line
    // and that line should read through it — so a crossing line is not hidden,
    // it is knocked back behind the words rather than drawn over them. Which is
    // the honest answer for a diagram this dense: the crossing is real and the
    // picture should show it; it must not be shown by writing it through a
    // word.
    //
    ...
    const edgeParts = edges.map(function (edge) {
      ...
      const d = laid.path || self.edgePath(laid.points);
      if (!d) {
        return '';
      }
      const title = self.edgeTitle(edge);
      const lines = edgeLabels[edge.id] || [];
      // dagre gives the label's CENTRE and the box it reserved for it. The
      // first baseline is half the text's height above that centre plus most of
      // a cap height back down — an SVG <text> hangs from its baseline, not
      // from its top, and getting this wrong puts every edge label one line
      // above its own background panel.
      const labelStep = LINE_HEIGHT - 2;
      const labelTop =
        (laid.y || 0) - ((lines.length - 1) * labelStep) / 2 + 3.5;
      const label = lines.length
        ? '<g><rect x="' + self.round((laid.x || 0) - (laid.width || 0) / 2) +
          '" y="' + self.round((laid.y || 0) - (laid.height || 0) / 2) +
          '" width="' + self.round(laid.width || 0) + '" height="' +
          self.round(laid.height || 0) +
          '" rx="3" fill="' + PAPER + '" fill-opacity=".88"/>' +
          lines.map(function (one, i) {
            return '<text x="' + self.round(laid.x || 0) + '" y="' +
              self.round(labelTop + i * labelStep) + '" text-anchor="middle" ' +
              'font-size="' + EDGE_SIZE + '" fill="' +
              (i === 0 ? look.colour : QUIET) + '"' +
              (i === 0 ? ' font-weight="600"' : '') + '>' + self.esc(one) +
              '</text>';
          }).join('') + '</g>'
        : '';
      return {
        line: '<g><title>' + self.esc(title) + '</title>' +
          '<path d="' + d + '" fill="none" stroke="' + look.colour + '" ' +
              'stroke-width="' +
          look.weight + '"' +
          (look.dash ? ' stroke-dasharray="' + look.dash + '"' : '') +
          ' marker-start="url(#' + prefix + '-' + self.tailId(look.colour) +
          ')"' +
          ' marker-end="url(#' + prefix + '-' + self.markerId(look.colour) +
          ')"/></g>',
        label: label ? '<g><title>' + self.esc(title) + '</title>' + label +
                       '</g>' :
               ''
      };
    ...
    const nodeMarkup = nodes.map(function (node) {
      ...
      return self.nodeMarkupFor(drawn[node.id], at, options);
    }).join('');

    const svg = '<svg xmlns="http://www.w3.org/2000/svg" ' +
      'viewBox="0 0 ' + width + ' ' + height + '" width="' + width +
      '" height="' + height +
      '" role="img" aria-label="' + self.esc(options.label || 'Delegation ' +
                                                         'relationships') +
      '"><title>' + self.esc(options.label || 'Delegation ' +
                                                          'relationships') +
      '</title>' +
      defs +
    ...
    log.debug("Leaving DelegationMap.renderUnguarded().");
    return { svg: svg, width: width, height: height,
             nodes: nodes.length, edges: edges.length };
  }

  // The sentence a line says on hover. It is the one place in the picture with
  // room for the whole of it, which is why the drawn label is allowed to be
  // three short lines.
  private edgeTitle(edge) {
    const { log } = this.deps;
    const self = this;
    log.debug("Entering DelegationMap.edgeTitle().");
    const parts = [];
    if (edge.relation === 'issued') {
      ...
      parts.push('This party AUTHENTICATED to this service — the sign-in ' +
      ...
      // picture a reader can copy into a client's `scope` and have work, so it
      // is quoted exactly rather than described.
      parts.push('MAY reach — a configured delegated permission, not ' +
                 'something that has happened.');
      if (edge.permissionId) {
        parts.push('The permission is ' + edge.permissionId +
                   '. A client sends that string as an OAuth scope; the ' +
                   'access token comes back audienced to ' +
                   (edge.baseUri || 'the ' +
                       'base URI') +
      ...
      // WHICH AUDIENCE, WHEN THE BOX IS NOT NAMED AFTER IT. A token addressed
      // to `https://apigw1.example.com` is drawn as the application that
      // registered that URI, and without this line there is nothing anywhere on
      // the page connecting the two — the label says who the credential is for
      // and the box says a name the token never mentions. Said only where they
      // DIFFER: on a line whose box IS the audience it would be the same string
      // twice.
      if (edge.audience && edge.audience !== edge.to) {
        parts.push('Addressed to ' + edge.audience +
                   (edge.audienceRegistered
                     ...
                     : ', which no application here has registered — so it ' +
                       'is drawn as itself.'));
      }
      ...
      // permissions` is asking what the token DID carry, and `openid profile`
      // is the whole answer in the ordinary case.
      if (edge.permissions && edge.permissions.length) {
        parts.push('Carries the delegated permission' +
                   (edge.permissions.length === 1 ? ' ' : 's ') +
                   edge.permissions.join(', ') + ' — the names on the ' +
                   'token\'s scope claim that this resource DEFINES. A ' +
                   'client asks for one by sending the whole identifier (the ' +
                   'base URI followed by the name) as a scope; the token ' +
                   'comes back audienced to the base with the bare name on ' +
    ...
    if (edge.typeLabel) {
      // The protocol only where the line HAS one. A sign-in line carries
      // several and leaves the field empty (see user_graph.js), and ` — ` with
      // nothing in front of it reads as a missing word rather than as an absent
      // one.
    ...
    if (edge.mode) {
      parts.push(edge.mode === 'impersonation'
        ? 'IMPERSONATION: what came out names the initial identity and ' +
          'nothing else, so the far end cannot tell an intermediary was ' +
          'involved.'
        : 'DELEGATION: what came out carries the chain, so the far end can ' +
          'see who is really asking.');
    }
    ...
    if (edge.relation === 'may-reach') {
      ...
      // exchanged" is an observation about acts on a line that describes no
      // act. What such a line has to say is whether the client has ever ASKED,
      // and the `may-reach` branch above has already said it.
      log.debug("Leaving DelegationMap.edgeTitle(). may-reach; no act or " +
                "credential count belongs on it.");
      return parts.join('\n');
    }
    if (edge.relation === 'issued-for') {
      parts.push(edge.credentials + ' credential(s).');
    } else if (edge.relation === 'signed-in') {
      parts.push(edge.acts + ' authentication(s), all of them accepted — ' +
                 'this service checks no password in any family.');
    } else if (edge.acts) {
      parts.push(edge.acts + ' act(s): ' + edge.issued + ' issued, ' +
                 edge.refused + ' refused.');
      if (edge.credentials) {
        parts.push(edge.credentials +
                   ' credential(s) from the issued register.');
      }
    } else {
      // NO ACTS AT ALL, which is what every line the person's picture adds
      // looks like: a credential was issued and nothing was delegated. `0
      // act(s): 0 issued, 0 refused` was printed here until 2026-08-26 and
      // reads as a delegation that was tried and came to nothing, which is the
      // opposite of what happened — see common/user_graph.ts. The count of
      // credentials is the whole of what such a line has to report.
      parts.push(edge.credentials + ' credential(s) from the issued ' +
                 'register, and no delegation act: nothing was exchanged to ' +
                 'get them.');
    }
  ...
  private nodeMarkupFor(entry, at, options) {
    const { log } = this.deps;
    const self = this;
    log.debug("Entering DelegationMap.nodeMarkupFor().");
    ...
    if (size.shape === 'person') {
      shape = self.personGlyph(at.x - FIGURE_W / 2, y, stroke, dashed, 1);
      textTop = y + FIGURE_H + 6 + LABEL_SIZE - 2;
    } else if (size.shape === 'sts') {
      shape = '<path d="' + self.hexPath(x, y, size.width, size.height) +
        '" fill="' +
        WASH +
      ...
      shape = '<rect x="' + self.round(x) + '" y="' + self.round(y) +
        '" width="' +
        self.round(size.width) +
        '" height="' + self.round(size.height) + '" rx="5" fill="' + fill +
        '" ' +
            'stroke="' +
        stroke + '" stroke-width="1.5"' + dash + '/>';
      if (size.shape === 'both') {
        // The person INSIDE the application's rectangle — see the header.
        // Scaled to fit the box's height rather than the figure's own, so a
        // one-line label and a two-line label do not get different-sized
        // people.
        const scale = Math.min(1, (size.height - 8) / FIGURE_H);
        shape += self.personGlyph(x + 6, at.y - (FIGURE_H * scale) / 2,
                                  stroke, dashed, scale);
    ...
    const texts = size.lines.map(function (one, i) {
      return '<text x="' + self.round(textX) + '" y="' +
        self.round(textTop + i * LINE_HEIGHT) +
        '" text-anchor="' + anchor + '" font-size="' + LABEL_SIZE + '" ' +
        'font-weight="600" ' +
        'fill="' + (look.ink || INK) + '">' + self.esc(one) + '</text>';
    }).join('') + size.subLines.map(function (one, i) {
      return '<text x="' + self.round(textX) + '" y="' +
        self.round(textTop + (size.lines.length + i) * LINE_HEIGHT - 1) +
        '" text-anchor="' + anchor + '" font-size="' + SUB_SIZE + '" fill="' +
        QUIET +
        '">' + self.esc(one) + '</text>';
    }).join('');

    const body = '<title>' + self.esc(look.title || node.id) + '</title>' +
                 shape + texts;

    // A LINK ONLY WHEN THE CALLER ASKED FOR ONE. The picture is served two ways
    // — inline in the console page, where a box should go to that party's page,
    // and as a standalone document at ?format=svg, where the href would be a
    ...
    if (options.links && look.href) {
      log.debug("Leaving DelegationMap.nodeMarkupFor().");
      return '<a href="' + self.esc(look.href) + '">' + body + '</a>';
    }
    log.debug("Leaving DelegationMap.nodeMarkupFor().");
    return '<g>' + body + '</g>';
  }
  ...
  // which was already the "second set of shapes" that `delegationMapKey()`'s
  // own header warns about — and the tail disc would have made it a second set
  // of two. So the sample is drawn HERE, out of the same numbers the markers
  // are.
  //
  ...
  // the arithmetic — `TAIL_SIZE`, `TAIL_R` and the arrowhead's own geometry —
  ...
  edgeSample(colour: string, dash?: string, weight?: number): string {
    const { log } = this.deps;
    const self = this;
    log.debug("Entering DelegationMap.edgeSample().");
    const stroke = weight || 1.8;
    // What the markers come out as at this stroke width. `markerUnits` is left
    // at its default of `strokeWidth` on both, so both scale with the line.
    const tail = (TAIL_R / 10) * TAIL_SIZE * stroke;
    const head = 7 * stroke;
    const x1 = 4 + tail * 2;
    const x2 = 58 - head;
    log.debug("Leaving DelegationMap.edgeSample().");
    return '<circle cx="' + self.round(4 + tail) + '" cy="20" r="' +
      self.round(tail) +
      '" fill="' + colour + '"/>' +
      '<path d="M' + self.round(x1) + ' 20H' + self.round(x2) + '" stroke="' +
      colour +
      '" stroke-width="' + stroke + '" fill="none"' +
      (dash ? ' stroke-dasharray="' + dash + '"' : '') + '/>' +
      '<path d="M' + self.round(x2) + ' ' + self.round(20 - head / 2) +
      'L58 20L' +
      self.round(x2) + ' ' + self.round(20 + head / 2) + 'z" fill="' +
      colour + '"/>';
  }
}
