...
const WORDS = { pq: 'PQC', composite: 'PQC+', kem: 'PQC KEM',
                hybrid: 'PQC alt' };
...
// The icon for a classification, or '' where there is none — so a caller can
// concatenate it after every row's algorithm without asking first.
function badge(info) {
  log.debug("Entering badge().");
  if (!info || !WORDS[info.kind]) {
    log.debug("Leaving badge(). Classical.");
    return '';
  }
  const said = pqcSupport.sentence(info);
  log.debug("Leaving badge(). " + info.kind);
  return '<span class="pqc-badge pqc-' + esc(info.kind) + '" role="img" ' +
    'aria-label="' + esc(said) + '" title="' + esc(said) + '" style="' +
    BASE_STYLE + KIND_STYLE[info.kind] + '">' + LATTICE +
    '<span aria-hidden="true">' + esc(WORDS[info.kind]) + '</span></span>';
}
...
function badgeFor(options) {
  log.debug("Entering badgeFor().");
  log.debug("Leaving badgeFor().");
  return badge(pqcSupport.of(options));
}

// The key a page draws once, above its tables: what each of the four marks
// means, drawn with the marks themselves so the legend cannot drift from them.
function legend() {
  log.debug("Entering legend().");
  const sample = function (kind, label, standard) {
    log.debug("Entering sample().");
    log.debug("Leaving sample().");
    return badge({ kind: kind, label: label, standard: standard });
  };
  log.debug("Leaving legend().");
  return admin.note(
    '<strong>' + sample('pq', 'ML-DSA-65', 'FIPS 204') + ' marks a key pair ' +
    'that uses a post-quantum algorithm</strong> — ML-DSA or SLH-DSA. ' +
    sample('composite', 'ML-DSA-44 + Ed25519',
           'draft-ietf-lamps-pq-composite-sigs') + ' is a COMPOSITE: one key ' +
    ...
    'itself is not post-quantum. Hover an icon for the algorithm. A key with ' +
