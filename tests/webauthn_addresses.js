...
function run(t) {
  ...
  withSettings({ 'webauthn.allowedOrigins':
    ...
    function () {
      t.equal(authn.expectedOriginFor(base, credentialFrom('https://sso.example.com')),
              'https://sso.example.com',
              'set, an origin ON THE LIST is the one the verifier expects — and ' +
              'a trailing slash in the setting is not part of an origin');
      t.equal(authn.expectedOriginFor(base,
                                      credentialFrom('https://sts.example.com:8443')),
              'https://login.example.com',
              'while the origin the REQUEST arrived at is not accepted just for ' +
              'being the request\'s — the list is the whole answer, and a value ' +
              'off it is answered with the list\'s first so the verifier refuses ' +
              'it in its own words');
      t.equal(authn.expectedOriginFor(base, credentialFrom('https://evil.example')),
              'https://login.example.com',
              'and neither is an origin nobody listed');
      t.equal(authn.expectedOriginFor(base, { response: { clientDataJSON: '%%%' } }),
              'https://login.example.com',
              'and client data that will not decode is left to the verifier to ' +
              'refuse, rather than throwing here');
    });
  ...
  withSettings({ 'webauthn.rpId': 'attacker.example' }, function () {
    t.equal(authn.rpIdProblem(base), '',
            'DEVELOPMENT mode keeps the fallback — no refusal, the host is used ' +
    ...
    withSettings({ 'global.mode': 'product' }, function () {
      ...
      t.check(/webauthn\.rpId/.test(problem) && /global\.publicBaseUrl/.test(problem),
              'PRODUCT mode REFUSES the ceremony, naming the setting and the fix',
              problem);
    });
  });
  withSettings({ 'webauthn.rpId': 'example.com', 'global.mode': 'product' },
    function () {
      ...
      t.check(authn.rpIdProblem('https://notexample.com') !== '',
              'and a host that ends in the RP ID as a STRING but not at a label ' +
              'boundary is refused — `endsWith()` alone is the mistake');
    });
  withSettings({ 'global.mode': 'product' }, function () {
    t.equal(authn.rpIdProblem(base), '',
            'and with no RP ID configured there is nothing to refuse — the host ' +
            'is the RP ID, which global.publicBaseUrl pins');
  });

  // -----------------------------------------------------------------------
  t.log.info('=== 3. both ceremonies ask the same functions ===');
  const root = path.join(__dirname, '..');
  const authnSource = fs.readFileSync(path.join(root, 'authn', 'authn.js'), 'utf8');
  const portalSource = fs.readFileSync(path.join(root, 'portal', 'portal.js'), 'utf8');
  t.check(/const expectedOrigin = expectedOriginFor\(base, credential\);/
          ...
          'rpIdProblem() — read as source, because the only other evidence is a ' +
          'ceremony a browser performs');
  t.check(/origin: authn\.expectedOriginFor\(base, credential\)/.test(portalSource) &&
...
module.exports = {
  ...
  describe: 'webauthn.allowedOrigins as the whole list where it is set, and an ' +
            'RP ID that does not fit refused in product mode rather than ' +
