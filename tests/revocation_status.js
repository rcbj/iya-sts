...
//   * **A WORKER'S VIEW OF A CLIENT CERTIFICATE** — `request_pool.peerOf()` on a
//     real TLS socket, decoded the way the worker decodes it. No endpoint
...
//     REMOVES, AND AN INDIRECT CRL** — sections 8 to 11 — are documents a
//     third party signs, and the only honest fixture is one OPENSSL made: a real
...
function asIssuer(one) {
  return { certificatePem: one.pem, privateKeyPem: one.key, keyAlg: 'rsa-2048' };
}
...
async function makeCrl(issuer, serials, options) {
  ...
  if (serials.length) {
    crl.revokedCertificates = serials.map(function (serialHex) {
      ...
      entry.revocationDate = new pkijs.Time({ type: 0, value: new Date(now - 30000) });
...
function crlServer() {
  ...
  const server = http.createServer(function (req, res) {
    ...
    req.on('end', function () {
      ...
      if (!route) {
        res.writeHead(404); // error-code: none — a test fixture, not the service
        res.end();
        return;
      }
      const send = function () {
        ...
        if (route.ocsp && (req.method !== 'POST' ||
          ...
          res.writeHead(405); // error-code: none — a test fixture, not the service
          res.end();
          return;
        }
        if (route.ocsp) {
        ...
        res.writeHead(route.status || 200,
                      { 'Content-Type': route.contentType ||
                        (route.ocsp ? 'application/ocsp-response' : 'application/pkix-crl') });
        res.end(body);
      };
      if (route.delayMs) {
  ...
  try {
    keys.forEach(function (key) {
      ...
      if (refused && refused.ok === false) {
        throw new Error('could not set ' + key + ': ' + JSON.stringify(refused));
      }
    });
    return await fn();
  } finally {
...
async function thePolicy(t) {
  t.log.info('=== 1. the policy: auto chooses by mode, and a named value wins ===');
  const status = require('../common/revocation_status');
  const mode = require('../common/mode');
  t.equal(mode.isProduct(), false, 'this file starts in development mode');
  t.equal(status.policy().configured, 'auto', 'pki.revocationCheck defaults to auto');
  ...
  await withSettings({ 'global.mode': 'product' }, async function () {
    t.equal(status.policy().effective, 'hard-fail',
            'and HARD-FAIL in product, through mode.refusesUnknownRevocationStatus()');
  });
  await withSettings({ 'pki.revocationCheck': 'off' }, async function () {
    t.equal(status.policy().effective, 'off', 'a named value wins over the mode');
  });
}
...
async function theRegister(t, realm, crlBase) {
  t.log.info('=== 2. the register: verifyLeaf() refuses what /admin/pki revoked ===');
  ...
  const one = await pki.issueSigningKeyPair(REALM, { identifier: 'rev-app-one' });
  const two = await pki.issueSigningKeyPair(REALM, { identifier: 'rev-app-two' });
  const three = await pki.issueSigningKeyPair(REALM, { identifier: 'rev-app-three' });
  ...
  const good = await pki.verifyLeaf(REALM, leafOne.certificatePem, leafOne.chainPem);
  t.check(good.ok && good.revocation && good.revocation.status === 'good',
          'a leaf this realm issued verifies, and the verdict says it was LOOKED ' +
  ...
  t.check(good.revocation && good.revocation.links.some(function (link) {
          ...
          'REGISTER — nothing is fetched for a certificate this service signed');

  revocation.revoke(REALM, 'assertions', { serialHex: serialOf(leafOne.certificatePem),
                                            reason: 'keyCompromise' });
  const refused = await pki.verifyLeaf(REALM, leafOne.certificatePem, leafOne.chainPem);
  const errorCodes = require('../common/error_codes');
  t.check(!refused.ok && refused.revocation && refused.revocation.status === 'revoked',
  ...
  t.check(viaGrant && viaGrant.errorCode === 'STS-PKI-0118' && !viaGrant.key,
          ...
          'than "does not chain here" — the certificate is ours and was withdrawn',
          JSON.stringify(viaGrant && { code: viaGrant.errorCode, error: viaGrant.error }));

  await withSettings({ 'pki.revocationCheck': 'off' }, async function () {
    const off = await pki.verifyLeaf(REALM, leafOne.certificatePem, leafOne.chainPem);
    t.check(off.ok && off.revocation.checked === false,
            'with pki.revocationCheck=off the same certificate is accepted and ' +
            'the verdict says nothing was consulted');
  });
  ...
  revocation.revoke(REALM, 'assertions', { serialHex: serialOf(leafTwo.certificatePem),
  ...
  const released = status.localVerdictFor({ leaf: leafTwo.certificatePem, chain: [],
                                            verified: true });
  t.check(!released.refused && released.status === 'good',
          'and released, it is good again on the very next check — the register ' +
          'is read per certificate and never cached', released.why);
  ...
  revocation.revoke(REALM, 'intermediate', { serialHex: serialOf(issuingPem),
                                              reason: 'cACompromise' });
  const orphan = status.localVerdictFor({ leaf: three.issued.certificatePem, chain: [],
                                          verified: true });
  t.check(orphan.refused && orphan.revoked && orphan.revoked.depth === 1,
          'A REVOKED ISSUING CA REVOKES EVERY LEAF UNDER IT, found by walking UP ' +
          'through the tiers this service holds — the client did not send the ' +
          'Issuing CA, and not sending it must not be how that is got around',
  ...
  const svid = await realms.run(realm, function () {
    return ca.mintX509Svid('spiffe://' + ca.trustDomain() + '/ns/t/sa/revocation-probe');
  });
  const svidCert = new nodeCrypto.X509Certificate(svid.chainPem[0]);
  const id = 'spiffe://' + realms.run(realm, function () { return ca.trustDomain(); }) +
  ...
  t.check(before.ok && before.revocation && before.revocation.status === 'good',
          'a fresh SVID verifies and was looked up', JSON.stringify(before).slice(0, 300));
  ...
  t.check(!after.ok && after.errorCode === 'STS-PKI-0118' &&
          ...
          'and on hold it is refused, with the revocation code the caller\'s audit ' +
          'row carries', JSON.stringify(after).slice(0, 300));
  ...
  const foreignRoot = await issue({ subject: 'CN=rev foreign root', ca: true, pathLen: 1 });
  ...
  await withSettings({ 'pki.revocationCheck': 'hard-fail' }, async function () {
    const local = status.localVerdictFor({ leaf: foreignLeaf.pem, chain: [foreignRoot.pem],
                                           verified: true });
    t.check(!local.refused && local.status === 'unknown' &&
            local.unknown.every(function (one) { return one.kind === 'not-consulted'; }),
            'the register-only door answers a foreign certificate NOT-CONSULTED and ' +
            'does not refuse it even under hard-fail', local.why);
  });
}
...
async function theForeignCrl(t, fixture, realmId) {
  ...
  const root = await issue({ subject: 'CN=rev external root', ca: true, pathLen: 1 });
  const issuing = await issue({ subject: 'CN=rev external issuing', ca: true,
                                crls: [base + '/root.crl'], issuer: asIssuer(root) });
  const leafFor = function (name, urls) {
    return issue({ subject: 'CN=' + name, crls: urls, issuer: asIssuer(issuing) });
  };
  ...
  t.check(hits['/issuing.crl'] === 1 && hits['/root.crl'] === 1,
          'and BOTH lists were fetched — the leaf\'s at the Issuing CA and the ' +
          'Issuing CA\'s at the Root', JSON.stringify(hits));
  ...
  routes['/revoking.crl'] = { body: await makeCrl(issuing, [revokedLeaf.serialHex]) };
  ...
  t.check(revoked.status === 'revoked' && revoked.refused &&
          ...
          'a leaf its issuer\'s CRL lists is REVOKED, with the reason off the ' +
          'entry, and refused under soft-fail', revoked.why);
  ...
  t.check(soft.status === 'unknown' && !soft.refused &&
          soft.unknown.some(function (one) { return one.kind === 'unreachable'; }),
          'a list that answers 404 is UNKNOWN, and soft-fail accepts it', soft.why);
  await withSettings({ 'pki.revocationCheck': 'hard-fail' }, async function () {
    const hard = await status.verdictFor(input(lost));
    t.check(hard.refused && errorCodes.codeOf(hard) === 'STS-PKI-0119',
            'and HARD-FAIL refuses the same certificate with STS-PKI-0119 — the ' +
            'attacker who can block a fetch does not get in', hard.why);
  });
  const lostHits = hits['/missing.crl'];
  await status.verdictFor(input(lost));
  t.equal(hits['/missing.crl'], lostHits,
          'and the FAILURE is remembered for pki.revocationFailureRetryS: a dead ' +
          'server costs one request per window, not one per certificate');
  status.resetCache();
  ...
  t.equal(hits['/missing.crl'], lostHits + 2,
          'while a retry window of ZERO — a legal value — dials on every check');
  ...
  routes['/forged.crl'] = { body: await makeCrl(issuing, [], { signWith: stranger }) };
  ...
  routes['/stale.crl'] = { body: await makeCrl(issuing, [],
                                               { nextUpdateMs: Date.now() - 600000 }) };
  const nearly = await leafFor('rev nearly stale', [base + '/nearly.crl']);
  routes['/nearly.crl'] = { body: await makeCrl(issuing, [],
                                                { nextUpdateMs: Date.now() - 60000 }) };
  const delta = await leafFor('rev delta', [base + '/delta.crl']);
  routes['/delta.crl'] = { body: await makeCrl(issuing, [], { criticalExtension: true }) };
  await withSettings({ 'pki.revocationCheck': 'hard-fail' }, async function () {
    const f = await status.verdictFor(input(forged));
    t.check(f.refused && /signature does not verify/.test(f.why),
            'a CRL whose SIGNATURE does not verify against the issuer is not a ' +
            'list about that issuer — refused under hard-fail, naming the signature',
            f.why);
    ...
    t.check(!n.refused && n.status === 'good',
            'while one a minute past its nextUpdate is inside pki.revocationClockSkewS ' +
            'and still used', n.why);
    const d = await status.verdictFor(input(delta));
    t.check(d.refused && /critical extension/.test(d.why),
            'a CRL carrying a critical extension this service does not implement ' +
            'is unusable (RFC 5280 section 6.3.3)', d.why);
  });
  ...
  await withSettings({ 'pki.revocationCheck': 'hard-fail',
                       'pki.revocationFetchTimeoutMs': 200 }, async function () {
    ...
    t.check(v.refused && /did not answer within 200ms/.test(v.why) &&
            ...
            'a CRL server slower than pki.revocationFetchTimeoutMs is abandoned at ' +
            'the timeout, not waited for', v.why);
    const r = await status.verdictFor(input(moved));
    t.check(r.refused && /redirect is not followed/.test(r.why),
            'a redirect is NOT FOLLOWED — the only address the issuer signed is ' +
            'the one in the certificate', r.why);
  });
  t.equal(hits['/issuing.crl'], 1,
          'and nothing reached the address the redirect pointed at');

  // AN UNVERIFIED CHAIN NAMES URLS ANYBODY COULD HAVE WRITTEN: nothing is dialled.
  ...
  await withSettings({ 'pki.revocationCheck': 'hard-fail' }, async function () {
    ...
    t.check(!v.refused && v.unknown.some(function (one) {
            ...
            'a chain that did NOT VERIFY is not refused on revocation (it is refused, ' +
            'where it matters, for not verifying)', v.why);
  });
  ...
  const ldapOnly = await leafFor('rev ldap', ['ldap://127.0.0.1:1/cn=x?certificateRevocationList;binary']);
  await withSettings({ 'pki.revocationCheck': 'hard-fail' }, async function () {
    const v = await status.verdictFor(input(bare));
    t.check(!v.refused && v.unknown.some(function (one) {
              return one.kind === 'no-distribution-point';
            }),
            'under hard-fail a certificate whose issuer names NO list is accepted — ' +
            'there is no fetch an attacker could block', v.why);
    const l = await status.verdictFor(input(ldapOnly));
    t.check(!l.refused && /does not dial/.test(l.why),
            'and one naming only an ldap: point is the same case: this service ' +
            'dials http and https and nothing else', l.why);
    await withSettings({ 'pki.revocationRequireDistributionPoint': true }, async function () {
      const strict = await status.verdictFor(input(bare));
      t.check(strict.refused,
              'unless pki.revocationRequireDistributionPoint is on', strict.why);
    });
  });
  ...
  routes['/impostor.crl'] = { body: await makeCrl(impostor, [impostorLeaf.serialHex]) };
  t.equal(new nodeCrypto.X509Certificate(impostor.pem).subject, joseCert.subject,
  ...
  t.check(fooled.status === 'revoked' && fooled.revoked.source === 'crl',
          'a leaf under a foreign CA NAMED like one of this service\'s authorities ' +
          'is answered from ITS OWN CRL, not from this service\'s register — the ' +
          'issuer is matched by signature, not by name', fooled.why);
  ...
  if (haveOpenSsl()) {
    ...
    try {
      ...
      fs.writeFileSync(path.join(scratch, 'ca.cnf'),
        ...
        '\ndefault_md=sha256\ndefault_crl_days=1\n');
      childProcess.execFileSync('openssl', ['ca', '-config', path.join(scratch, 'ca.cnf'),
      ...
      routes['/openssl.crl'] = { body: fs.readFileSync(path.join(scratch, 'crl.pem')) };
      const v = await status.verdictFor(input(target));
      t.check(v.status === 'revoked' && v.revoked.reason === 'keyCompromise',
              'a CRL OPENSSL wrote, served as PEM, is read, verified and found to ' +
              'revoke the leaf — the reading that is not pkijs agreeing with itself',
              v.why);
    } finally {
      fs.rmSync(scratch, { recursive: true, force: true });
    }
  } else {
    t.log.warn('openssl is not installed; the independent CRL reading was not run');
  }
...
// response by `openssl ocsp` answering the request THIS SERVICE built, and every
// CRL but the indirect ones by `openssl ca -gencrl`. EC P-256 keys throughout,
// because an RSA key per certificate is most of this file's running time and the
// verifier is not what the key type is testing.
// ===========================================================================
function openssl(args, cwd) {
  return childProcess.execFileSync('openssl', args, { cwd: cwd, stdio: 'pipe' });
}
...
function answerOcsp(spec, requestDer) {
  ...
  try {
    openssl(['ocsp', '-index', spec.index, '-CA', spec.ca, '-rsigner', spec.signer,
...
// stays well-formed DER, so what fails is the verification and nothing before it.
...
async function opensslFixture(dir, base) {
  const read = function (name) { return fs.readFileSync(path.join(dir, name), 'utf8'); };
  const url = function (route) { return base + route; };
  const sections = [
    ...
    'keyUsage=critical,keyCertSign,cRLSign', 'subjectKeyIdentifier=hash',
    'authorityKeyIdentifier=keyid', 'crlDistributionPoints=URI:' + url('/o-root.crl'),
    '[responder_ext]', 'basicConstraints=CA:FALSE', 'keyUsage=critical,digitalSignature',
    'extendedKeyUsage=OCSPSigning',
    '[noeku_ext]', 'basicConstraints=CA:FALSE', 'keyUsage=critical,digitalSignature',
    ...
    '[nosign_issuer_ext]', 'basicConstraints=CA:FALSE', 'keyUsage=critical,digitalSignature',
    '[nosign_ca_ext]', 'basicConstraints=critical,CA:TRUE', 'keyUsage=critical,keyCertSign',
    'crlDistributionPoints=URI:' + url('/o-root.crl'),
    '[subca_ext]', 'basicConstraints=critical,CA:TRUE',
    'keyUsage=critical,keyCertSign,cRLSign', 'crlDistributionPoints=URI:' + url('/s-onlyuser.crl')
  ];
  ...
  const LEAVES = [
    // 8. OCSP
    ['ocspGood', '1001', ['authorityInfoAccess=OCSP;URI:' + url('/ocsp/issuer')]],
    ['ocspRevoked', '1002', ['authorityInfoAccess=OCSP;URI:' + url('/ocsp/delegated'),
                             'crlDistributionPoints=URI:' + url('/o-empty.crl')]],
    ['ocspUnknown', '1003', ['authorityInfoAccess=OCSP;URI:' + url('/ocsp/issuer'),
                             'crlDistributionPoints=URI:' + url('/o-empty.crl')]],
    ['ocspUnknownCrlRevokes', '1004', ['authorityInfoAccess=OCSP;URI:' + url('/ocsp/issuer'),
                                       'crlDistributionPoints=URI:' + url('/o-revokes.crl')]],
    ['ocspWrongKey', '1005', ['authorityInfoAccess=OCSP;URI:' + url('/ocsp/impostor')]],
    ['ocspWrongKeyFallback', '100a', ['authorityInfoAccess=OCSP;URI:' + url('/ocsp/impostor'),
                                      'crlDistributionPoints=URI:' + url('/o-empty.crl')]],
    ['ocspNoEku', '1006', ['authorityInfoAccess=OCSP;URI:' + url('/ocsp/noeku')]],
    ['ocspStale', '1007', ['authorityInfoAccess=OCSP;URI:' + url('/ocsp/stale')]],
    ['ocspReplay', '1008', ['authorityInfoAccess=OCSP;URI:' + url('/ocsp/replay')]],
    ['ocspDedupe', '1009', ['authorityInfoAccess=OCSP;URI:' + url('/ocsp/dedupe')]],
    ['ocspTampered', '100b', ['authorityInfoAccess=OCSP;URI:' + url('/ocsp/tampered')]],
    ['ocspExpiredResponder', '100c', ['authorityInfoAccess=OCSP;URI:' + url('/ocsp/expired')]],
    ['ocspOtherCertificate', '100d', ['authorityInfoAccess=OCSP;URI:' + url('/ocsp/other')]],
    ['ocspTryLater', '100e', ['authorityInfoAccess=OCSP;URI:' + url('/ocsp/trylater')]],
    // 9. DELTA
    ['deltaRevokes', '2001', ['crlDistributionPoints=URI:' + url('/d-base.crl')]],
    ['deltaRemoves', '2002', ['crlDistributionPoints=URI:' + url('/d-base.crl')]],
    ['heldNoDelta', '2003', ['crlDistributionPoints=URI:' + url('/d4-base.crl')]],
    ['revokedNoDelta', '2004', ['crlDistributionPoints=URI:' + url('/d4-base.crl')]],
    ['deltaMismatch', '2005', ['crlDistributionPoints=URI:' + url('/d3-base.crl'),
                               'freshestCRL=URI:' + url('/d3-delta.crl')]],
    ['deltaAtPoint', '2006', ['crlDistributionPoints=URI:' + url('/d-delta.crl')]],
    ['deltaNotNewer', '2007', ['crlDistributionPoints=URI:' + url('/d5-base.crl')]],
    ['deltaComplete', '2008', ['crlDistributionPoints=URI:' + url('/d6-base.crl')]],
    ['deltaOtherScope', '2009', ['crlDistributionPoints=URI:' + url('/d7-base.crl')]],
    // 10. INDIRECT
    ...
    ['indirectUnnamed', '3005', ['crlDistributionPoints=URI:' + url('/i-unnamed.crl')]],
    ['indirectFlat', '3006', ['crlDistributionPoints=cdp_flat']],
    ['directNamedEntry', '3007', ['crlDistributionPoints=URI:' + url('/i-direct-named.crl')]],
    ['indirectNoCrlSign', '3008', ['crlDistributionPoints=cdp_nosign']],
    // 11. SCOPE
    ['scopeOnlyCa', '4001', ['crlDistributionPoints=URI:' + url('/s-onlyca.crl')]],
    ['scopeOtherName', '4003', ['crlDistributionPoints=URI:' + url('/s-other.crl')]],
    ['scopePartial', '4004', ['crlDistributionPoints=URI:' + url('/s-key.crl')]],
  ...
  ['indirect', 'forged', 'flat'].forEach(function (name) {
    sections.push('[cdp_' + name + ']', 'fullname=URI:' + url('/i-' + name + '.crl'),
  ...
  const selfSigned = function (name, subject, extensions) {
    key(name);
    openssl(['req', '-x509', '-new', '-config', 'ext.cnf', '-extensions', extensions,
             '-key', name + '.key', '-subj', subject, '-days', '2', '-out', name + '.pem'], dir);
    return { name: name, pem: read(name + '.pem'), key: read(name + '.key'),
             path: path.join(dir, name + '.pem'), keyPath: path.join(dir, name + '.key') };
  };
  const issued = function (name, subject, issuer, serial, extensions) {
    key(name);
    openssl(['req', '-new', '-config', 'ext.cnf', '-key', name + '.key', '-subj', subject,
             '-out', name + '.csr'], dir);
    openssl(['x509', '-req', '-in', name + '.csr', '-CA', issuer + '.pem', '-CAkey',
             issuer + '.key', '-set_serial', '0x' + serial, '-days', '2', '-extfile',
             'ext.cnf', '-extensions', extensions, '-out', name + '.pem'], dir);
    return { name: name, pem: read(name + '.pem'), key: read(name + '.key'), serial: serial,
             path: path.join(dir, name + '.pem'), keyPath: path.join(dir, name + '.key') };
  };

  const fx = { dir: dir, base: base };
  fx.root = selfSigned('root', '/CN=rev ossl root', 'root_ext');
  fx.issuing = issued('issuing', '/CN=rev ossl issuing', 'root', '0100', 'issuing_ext');
  fx.responder = issued('responder', '/CN=rev ossl responder', 'issuing', '0200', 'responder_ext');
  fx.noEku = issued('noeku', '/CN=rev ossl responder without eku', 'issuing', '0201', 'noeku_ext');
  // A self-signed certificate carrying the ISSUING CA'S OWN NAME and another key.
  fx.impostor = selfSigned('impostor', '/CN=rev ossl issuing', 'impostor_ext');
  fx.crlIssuer = issued('crlissuer', '/CN=rev ossl crl issuer', 'root', '0300', 'crl_issuer_ext');
  // The same name again, self-signed: carries the right name and chains nowhere.
  fx.crlIssuerImpostor = selfSigned('crlissuer-impostor', '/CN=rev ossl crl issuer',
                                    'impostor_ext');
  fx.subCa = issued('subca', '/CN=rev ossl sub ca', 'issuing', '4002', 'subca_ext');
  fx.crlIssuerNoSign = issued('crlissuer-nosign', '/CN=rev ossl crl issuer without crlsign',
                              'root', '0301', 'nosign_issuer_ext');
  fx.noSignCa = issued('nosignca', '/CN=rev ossl ca without crlsign', 'root', '0400',
                       'nosign_ca_ext');
  ...
  fs.writeFileSync(path.join(dir, 'ext.cnf'), sections.join('\n') + '\n');
  fx.noSignLeaf = issued('nosign-leaf', '/CN=rev ossl under a ca without crlsign',
  ...
  openssl(['req', '-new', '-config', 'ext.cnf', '-key', 'expired-responder.key', '-subj',
           '/CN=rev ossl expired responder', '-out', 'expired-responder.csr'], dir);
  ...
  fs.writeFileSync(path.join(dir, 'issue.cnf'),
    ...
    '\ndefault_md=sha256\npolicy=any\nunique_subject=no\n[any]\ncommonName=supplied\n');
  openssl(['ca', '-batch', '-notext', '-config', 'issue.cnf', '-in', 'expired-responder.csr',
           '-cert', 'issuing.pem', '-keyfile', 'issuing.key', '-startdate', '20200101000000Z',
           '-enddate', '20200102000000Z', '-extfile', 'ext.cnf', '-extensions',
           'responder_ext', '-out', 'expired-responder.pem'], dir);
  fx.expiredResponder = { name: 'expired-responder', pem: read('expired-responder.pem'),
                          path: path.join(dir, 'expired-responder.pem'),
                          keyPath: path.join(dir, 'expired-responder.key') };
  fx.leaf = {};
  LEAVES.forEach(function (one) {
    fx.leaf[one[0]] = issued('leaf-' + one[0], '/CN=rev ossl ' + one[0], 'issuing', one[1],
                             one[0] + '_ext');
  });
  return fx;
}
...
function opensslCrl(fx, signer, label, crlNumber, lines, extensionLines) {
  ...
  const args = ['ca', '-config', path.join(d, 'ca.cnf'), '-gencrl', '-keyfile',
                signer.keyPath, '-cert', signer.path, '-out', path.join(d, 'crl.pem')];
...
// distribution point naming the URL and indirectCRL, and entries each optionally
// preceded by a certificateIssuer — and the caller then has `openssl crl` verify
// it against its signer, so the document is read by something other than pkijs.
async function indirectCrl(signer, spec) {
  ...
  const tagged = function (tag, value, constructed) {
    return constructed
      ? new asn1js.Constructed({ idBlock: { tagClass: 3, tagNumber: tag }, value: value })
      : new asn1js.Primitive({ idBlock: { tagClass: 3, tagNumber: tag }, valueHex: value });
  };
  const signerCert = pkijs.Certificate.fromBER(buffer(signer.pem));
  const idpFields = [tagged(0, [tagged(0, [tagged(6, plain(Buffer.from(spec.url)))], true)], true)];
  ...
  if (spec.caIssuers && spec.caIssuers.length) {
    crlExtensions.push(new pkijs.Extension({ extnID: '1.3.6.1.5.5.7.1.1', critical: false,
      extnValue: new pkijs.InfoAccess({ accessDescriptions: spec.caIssuers.map(function (url) {
  ...
  crl.revokedCertificates = spec.entries.map(function (one) {
    const entry = new pkijs.RevokedCertificate();
    let bytes = Buffer.from(one.serial.length % 2 ? '0' + one.serial : one.serial, 'hex');
    if (bytes[0] & 0x80) {
      bytes = Buffer.concat([Buffer.from([0]), bytes]);
    }
    entry.userCertificate = new asn1js.Integer({ valueHex: plain(bytes) });
    entry.revocationDate = new pkijs.Time({ type: 0, value: new Date(now - 30000) });
    const extensions = [new pkijs.Extension({ extnID: '2.5.29.21', critical: false,
      extnValue: new asn1js.Enumerated({ value: 1 }).toBER(false) })];
...
async function theOcsp(t, fx) {
  t.log.info('=== 8. OCSP: good, revoked, unknown, the wrong key, no EKU, stale, ' +
             'replayed, and one request for two callers ===');
  const status = require('../common/revocation_status');
  const errorCodes = require('../common/error_codes');
  const routes = fx.routes;
  const hits = fx.hits;
  const index = path.join(fx.dir, 'ocsp-index.txt');
  fs.writeFileSync(index, [indexLine('1001'), indexLine('1002', 'keyCompromise'),
                           indexLine('1005'), indexLine('100A'), indexLine('1006'),
                           indexLine('1007'), indexLine('1008'), indexLine('1009'),
                           indexLine('100B'), indexLine('100C'), indexLine('100D'),
                           indexLine('100E')].join(''));
  ...
  t.check(good.status === 'good' && goodLink.answeredBy === 'ocsp' &&
          goodLink.ocspResponder === 'issuer' && goodLink.ocspNonce === 'matched',
          'OCSP GOOD: a response openssl signed with the ISSUER\'s key, echoing the ' +
          'nonce this service sent, answers the leaf good',
  ...
  t.check(revoked.status === 'revoked' && revoked.refused &&
          revokedLink.answeredBy === 'ocsp' && revokedLink.ocspResponder === 'delegated' &&
          revoked.revoked.reason === 'keyCompromise' && /OCSP responder/.test(revoked.why),
          'OCSP REVOKED, from a DELEGATED responder the issuer certified with ' +
          'id-kp-OCSPSigning: refused with the reason openssl put in the response',
          revoked.why);
  t.equal(errorCodes.codeOf(revoked), 'STS-PKI-0118', 'carrying STS-PKI-0118');
  t.equal(hits['/o-empty.crl'], undefined,
          'and the CRL the same certificate names was NOT fetched — OCSP answered first');

  const unknown = await status.verdictFor(input(fx.leaf.ocspUnknown));
  t.check(unknown.status === 'unknown' && !unknown.refused &&
          unknown.unknown.some(function (one) { return one.kind === 'responder-unknown'; }),
          'OCSP UNKNOWN: a signed `unknown` is unknown, and soft-fail accepts it',
          unknown.why);
  t.check(hits['/o-empty.crl'] >= 1 && /does not list it/.test(unknown.why),
          'and it is NOT UPGRADED by the CRL that does not list the certificate — ' +
          'the CRL was read and the answer stayed unknown', unknown.why);
  await withSettings({ 'pki.revocationCheck': 'hard-fail' }, async function () {
    const hard = await status.verdictFor(input(fx.leaf.ocspUnknown));
    t.check(hard.refused && errorCodes.codeOf(hard) === 'STS-PKI-0119',
            'while HARD-FAIL refuses a certificate the issuer\'s own responder does not ' +
            'know, with STS-PKI-0119', hard.why);
  });
  const outvoted = await status.verdictFor(input(fx.leaf.ocspUnknownCrlRevokes));
  t.check(outvoted.status === 'revoked' && outvoted.links[0].answeredBy === 'crl',
          'and a CRL that LISTS the certificate still wins over a responder\'s unknown',
          outvoted.why);
  ...
  await withSettings({ 'pki.revocationCheck': 'hard-fail' }, async function () {
    ...
    t.check(wrong.refused && wrong.status === 'unknown' &&
            ...
            'A RESPONDER SIGNING WITH THE WRONG KEY — a certificate carrying the ' +
            'issuer\'s own NAME over another key — is not believed, and hard-fail ' +
            'refuses', wrong.why);
    ...
    t.check(noEku.refused && /carries no id-kp-OCSPSigning/.test(noEku.why),
            'A DELEGATED RESPONDER WITHOUT THE EKU — issued by the right CA, signing ' +
            'correctly — is not a responder, and hard-fail refuses', noEku.why);
    const tampered = await status.verdictFor(input(fx.leaf.ocspTampered));
    t.check(tampered.refused && /key does not verify its signature/.test(tampered.why),
            'a delegated responder with the EKU whose response has one bit of its ' +
            'signature flipped is not believed', tampered.why);
    const expired = await status.verdictFor(input(fx.leaf.ocspExpiredResponder));
    t.check(expired.refused && /outside its validity period/.test(expired.why),
            'nor is a delegated responder whose certificate has EXPIRED', expired.why);
    const tryLater = await status.verdictFor(input(fx.leaf.ocspTryLater));
    t.check(tryLater.refused && /tryLater/.test(tryLater.why),
            'a responder answering tryLater gave no answer, and the policy decides',
            tryLater.why);
  });
  const fallback = await status.verdictFor(input(fx.leaf.ocspWrongKeyFallback));
  t.check(fallback.status === 'good' && fallback.links[0].answeredBy === 'crl' &&
          /not signed by anybody the issuer authorised/.test(fallback.links[0].why),
          'and where the certificate also names a CRL, an untrustworthy responder is ' +
          'replaced by the list — with the responder\'s failure kept on the link',
  ...
  openssl(['ocsp', '-issuer', fx.issuing.path, '-cert', fx.leaf.ocspStale.path, '-no_nonce',
           '-reqout', 'stale-req.der'], fx.dir);
  openssl(['ocsp', '-index', index, '-CA', fx.issuing.path, '-rsigner', fx.issuing.path,
           '-rkey', fx.issuing.keyPath, '-reqin', 'stale-req.der', '-respout',
           'stale-resp.der'], fx.dir);
  routes['/ocsp/stale'] = { body: fs.readFileSync(path.join(fx.dir, 'stale-resp.der')),
                            contentType: 'application/ocsp-response' };
  // The same pre-produced response, served for a DIFFERENT certificate.
  routes['/ocsp/other'] = { body: fs.readFileSync(path.join(fx.dir, 'stale-resp.der')),
                            contentType: 'application/ocsp-response' };
  await withSettings({ 'pki.revocationCheck': 'hard-fail' }, async function () {
    const other = await status.verdictFor(input(fx.leaf.ocspOtherCertificate));
    t.check(other.refused && /no single response names its issuer and serial/.test(other.why),
            'a valid, signed, nonce-less response ABOUT ANOTHER CERTIFICATE answers ' +
            'nothing about this one — the CertID must match', other.why);
  });
  ...
  t.check(noNonce.status === 'good' && noNonce.links[0].ocspNonce === 'absent',
          'a response that echoes NO nonce is believed by default — the pre-produced ' +
          'responses of RFC 5019 cannot carry one', noNonce.why);
  status.resetCache();
  await withSettings({ 'pki.revocationCheck': 'hard-fail',
                       'pki.revocationOcspRequireNonce': true }, async function () {
    const strict = await status.verdictFor(input(fx.leaf.ocspStale));
    t.check(strict.refused && /echoes no nonce/.test(strict.why),
            'unless pki.revocationOcspRequireNonce, which refuses it', strict.why);
  });
  status.resetCache();
  await new Promise(function (resolve) {
    setTimeout(resolve, Math.max(0, 1300 - (Date.now() - producedAt)));
  });
  await withSettings({ 'pki.revocationCheck': 'hard-fail', 'pki.revocationOcspMaxAgeS': 1,
                       'pki.revocationClockSkewS': 0 }, async function () {
    const stale = await status.verdictFor(input(fx.leaf.ocspStale));
    t.check(stale.refused && /STALE/.test(stale.why) && /no nextUpdate/.test(stale.why),
            'A STALE RESPONSE — no nextUpdate, and a thisUpdate older than ' +
            'pki.revocationOcspMaxAgeS — is refused under hard-fail', stale.why);
  });
  ...
  routes['/ocsp/replay'] = { body: fs.readFileSync(path.join(fx.dir, 'replay-resp.der')),
                             contentType: 'application/ocsp-response' };
  await withSettings({ 'pki.revocationCheck': 'hard-fail' }, async function () {
    const replay = await status.verdictFor(input(fx.leaf.ocspReplay));
    t.check(replay.refused && /replay/.test(replay.why),
            'a response echoing SOMEBODY ELSE\'S NONCE is a replay and is refused, ' +
            'however valid its signature', replay.why);
  });

  // THE CLOCK, moved rather than waited for: a day and a half on, a response
  // whose nextUpdate was a day away is past it; an hour back, one produced now is
  // in the future.
  const realNow = Date.now;
  status.resetCache();
  try {
    Date.now = function () { return realNow() + 36 * 3600 * 1000; };
    ...
    status.resetCache();
    Date.now = function () { return realNow() - 3600 * 1000; };
    const early = await status.verdictFor(input(fx.leaf.ocspGood));
    t.check(early.links[0].status === 'unknown' && /in the future/.test(early.links[0].why),
            'and one whose thisUpdate is further in the future than ' +
            'pki.revocationClockSkewS is not believed either', early.links[0].why);
  } finally {
    Date.now = realNow;
  }
  status.resetCache();
  ...
  const both = await Promise.all([status.verdictFor(input(fx.leaf.ocspDedupe)),
                                  status.verdictFor(input(fx.leaf.ocspDedupe))]);
  t.check(both[0].status === 'good' && both[1].status === 'good' &&
          hits['/ocsp/dedupe'] === 1,
          'TWO CALLERS AT ONCE make ONE request to the responder', JSON.stringify(hits));
  await status.verdictFor(input(fx.leaf.ocspDedupe));
  t.equal(hits['/ocsp/dedupe'], 1,
          'and a third is answered from the cache, until the response\'s nextUpdate');
  ...
  await withSettings({ 'pki.revocationOcsp': 'after-crl' }, async function () {
    ...
    t.check(later.status === 'good' && later.links[0].answeredBy === 'crl' &&
            ...
            'pki.revocationOcsp=after-crl asks the CRL FIRST, and a CRL that answers ' +
            'means the responder is never dialled', later.why);
    const onlyOcsp = await status.verdictFor(input(fx.leaf.ocspGood));
    t.check(onlyOcsp.status === 'good' && onlyOcsp.links[0].answeredBy === 'ocsp',
            'while a certificate naming no CRL is still answered by its responder',
            onlyOcsp.why);
  });
  await withSettings({ 'pki.revocationOcsp': 'off' }, async function () {
    const off = await status.verdictFor(input(fx.leaf.ocspGood));
    t.check(off.status === 'unknown' && hits['/ocsp/issuer'] === issuerHits + 1 &&
            off.unknown.some(function (one) { return one.kind === 'no-distribution-point'; }),
            'and pki.revocationOcsp=off asks no responder at all', off.why);
  });
}

async function theDeltas(t, fx) {
  t.log.info('=== 9. delta CRLs: one that revokes, one that removes, one built on ' +
             'another base, and one that cannot be reached ===');
  const status = require('../common/revocation_status');
  const routes = fx.routes;
  const input = function (leaf) {
    return { leaf: leaf.pem, chain: chainOf(fx), verified: true };
  ...
  routes['/d-base.crl'] = { body: opensslCrl(fx, fx.issuing, 'd-base', '0A',
    [indexLine('2002', 'certificateHold')], ['freshestCRL=URI:' + fx.base + '/d-delta.crl']) };
  ...
  // freshestCRL ON THE CERTIFICATE for this one, and a delta built on number 99.
  routes['/d3-base.crl'] = { body: opensslCrl(fx, fx.issuing, 'd3-base', '14', []) };
  routes['/d3-delta.crl'] = { body: opensslCrl(fx, fx.issuing, 'd3-delta', '15',
    [indexLine('2005', 'keyCompromise')], ['2.5.29.27=critical,ASN1:INTEGER:99']) };
  // A delta NOT NEWER than its base; a freshestCRL naming a COMPLETE list; and a
  // delta whose issuing distribution point is not its base's.
  ...
  routes['/d5-delta.crl'] = { body: opensslCrl(fx, fx.issuing, 'd5-delta', '27',
    [indexLine('2007', 'keyCompromise')], ['2.5.29.27=critical,ASN1:INTEGER:40']) };
  ...
  routes['/d7-delta.crl'] = { body: opensslCrl(fx, fx.issuing, 'd7-delta', '3D',
    [indexLine('2009', 'keyCompromise')], ['2.5.29.27=critical,ASN1:INTEGER:60']) };
  ...
  t.check(revokes.status === 'revoked' && /with its delta at/.test(revokes.links[0].why) &&
          /d-delta\.crl/.test(revokes.links[0].deltaUrl || ''),
          'A DELTA THAT REVOKES: the base CRL does not list the leaf, the delta its ' +
          'freshestCRL names does, and the merged answer is revoked', revokes.links[0].why);
  const removes = await status.verdictFor(input(fx.leaf.deltaRemoves));
  t.check(removes.status === 'good' && /with its delta at/.test(removes.links[0].why),
          'A DELTA THAT REMOVES: the base lists the leaf on hold, the delta says ' +
          'removeFromCRL, and the merged answer is good', removes.links[0].why);
  await withSettings({ 'pki.revocationCheck': 'hard-fail' }, async function () {
    ...
    t.check(mismatch.refused && mismatch.status === 'unknown' &&
            ...
            'A DELTA WITH A MISMATCHED BASE NUMBER — built on list 99 when the base ' +
            'here is 20 — is not merged; its revocation is not believed, and hard-fail ' +
            'refuses the certificate it would have been about', mismatch.why);
    const held = await status.verdictFor(input(fx.leaf.heldNoDelta));
    t.check(held.refused && held.status === 'unknown',
            'a base listing a certificate ON HOLD whose delta cannot be fetched is ' +
            'unknown — the hold is exactly what a delta may have released', held.why);
    const permanent = await status.verdictFor(input(fx.leaf.revokedNoDelta));
    t.check(permanent.status === 'revoked' && permanent.revoked.reason === 'keyCompromise',
            'while a base\'s PERMANENT revocation stands without its delta — no newer ' +
            'list can un-revoke keyCompromise', permanent.why);
    ...
    t.check(notNewer.refused && notNewer.status === 'unknown' &&
            ...
            'a delta whose own cRLNumber is not greater than its base\'s is not newer ' +
            'than the list it would update', notNewer.why);
    ...
    t.check(complete.refused && /not a delta CRL/.test(complete.why),
            'a freshestCRL naming a COMPLETE list is not a delta to merge', complete.why);
    ...
    t.check(atPoint.refused && /DELTA CRL/.test(atPoint.why),
            'and a delta served at a cRLDistributionPoint is not taken for the complete ' +
            'list', atPoint.why);
  });
}
...
async function theIndirect(t, fx) {
  ...
  const input = function (leaf, extra) {
    return { leaf: leaf.pem, chain: chainOf(fx).concat(extra || []), verified: true };
  };
  ...
  // follows it with none, so RFC 5280 section 5.3.3 carries that issuer forward.
  const listed = await indirectCrl(fx.crlIssuer, { url: fx.base + '/i-indirect.crl', entries: [
    { serial: '3002' },
    { serial: '3001', certificateIssuerPem: fx.issuing.pem },
    { serial: '3003' }
  ] });
  fs.writeFileSync(path.join(fx.dir, 'indirect.crl'), listed);
  const openSslReads = String(childProcess.spawnSync('openssl', ['crl', '-inform', 'DER',
    '-in', path.join(fx.dir, 'indirect.crl'), '-CAfile', fx.crlIssuer.path, '-noout', '-text'],
    { encoding: 'utf8' }).stderr) + String(childProcess.spawnSync('openssl', ['crl', '-inform',
    'DER', '-in', path.join(fx.dir, 'indirect.crl'), '-noout', '-text'],
    { encoding: 'utf8' }).stdout);
  t.check(/verify OK/.test(openSslReads) && /Certificate Issuer/.test(openSslReads) &&
          /Indirect CRL/.test(openSslReads),
          'the indirect CRL pkijs built is one OPENSSL reads, verifies against the CRL ' +
          'issuer and shows as indirect with a certificateIssuer entry',
  ...
  routes['/n-nosign.crl'] = { body: opensslCrl(fx, fx.noSignCa, 'n-nosign', '01',
                                               [indexLine('5001', 'keyCompromise')]) };
  await withSettings({ 'pki.revocationCheck': 'hard-fail' }, async function () {
    ...
    t.check(named.refused && named.status === 'unknown' &&
            ...
            'an entry carrying a certificateIssuer in a list that is NOT declared ' +
            'indirect makes the list unusable rather than being read', named.why);
    const noSign = await status.verdictFor(input(fx.leaf.indirectNoCrlSign,
                                                 [fx.crlIssuerNoSign.pem]));
    t.check(noSign.refused && noSign.status === 'unknown' && /may not sign CRLs/.test(noSign.why),
            'a CRL issuer whose certificate lacks cRLSign is not authorised, whatever ' +
            'the certificate names', noSign.why);
    const directNoSign = await status.verdictFor({ leaf: fx.noSignLeaf.pem,
      chain: [fx.noSignCa.pem, fx.root.pem], verified: true });
    t.check(directNoSign.refused && /does not include cRLSign/.test(directNoSign.why),
            'nor is a DIRECT list signed by an issuer whose keyUsage leaves out cRLSign ' +
            '(RFC 5280 section 4.2.1.3)', directNoSign.why);
  });
  ...
  await withSettings({ 'pki.revocationCheck': 'hard-fail' }, async function () {
    ...
    t.check(nobody.refused && nobody.status === 'unknown' &&
            ...
            'with the CRL issuer\'s certificate nowhere to be found, an indirect list ' +
            'cannot be trusted and hard-fail refuses', nobody.why);
    const inChain = await status.verdictFor(input(fx.leaf.indirectRevoked, [fx.crlIssuer.pem]));
    t.check(inChain.status === 'revoked' && inChain.links[0].indirect === true,
            'AN INDIRECT CRL REVOKING A CERTIFICATE FROM A SECOND CA: the leaf\'s ' +
            'cRLIssuer names it, its certificate came with the chain and chains to the ' +
            'same Root, and the entry that names the leaf\'s issuer revokes it', inChain.why);
  });
  await withSettings({ 'pki.revocationCrlIssuersFile': issuersFile }, async function () {
    const fromFile = await status.verdictFor(input(fx.leaf.indirectRevoked));
    t.check(fromFile.status === 'revoked',
            'and the same answer with the signer found in pki.revocationCrlIssuersFile',
            fromFile.why);
    const other = await status.verdictFor(input(fx.leaf.indirectOtherIssuer));
    t.check(other.status === 'good',
            'an entry with NO certificateIssuer before it belongs to the CRL issuer — ' +
            'the same serial under the leaf\'s own issuer is good', other.why);
    const carried = await status.verdictFor(input(fx.leaf.indirectCarried));
    t.check(carried.status === 'revoked',
            'while an entry AFTER a certificateIssuer belongs to that issuer (RFC 5280 ' +
            'section 5.3.3)', carried.why);
    await withSettings({ 'pki.revocationCheck': 'hard-fail' }, async function () {
      const forged = await status.verdictFor(input(fx.leaf.indirectForged));
      t.check(forged.refused && forged.status === 'unknown' && forged.status !== 'revoked' &&
              /signature does not verify/.test(forged.why),
              'a list signed by a certificate that carries the named CRL issuer\'s NAME ' +
              'and chains nowhere is not believed — the real CRL issuer\'s key does not ' +
              'verify it, and the impostor is not a signer it may be checked against',
              forged.why);
      ...
      t.check(unnamed.refused && unnamed.status === 'unknown' &&
              ...
              'AN INDIRECT CRL FROM A SIGNER THE CERTIFICATE DOES NOT NAME — a real, ' +
              'properly signed list revoking this very leaf, from a CRL issuer its ' +
              'distribution point never mentioned — revokes nothing, and hard-fail ' +
              'refuses', unnamed.why);
      ...
      t.check(flat.refused && /does not declare itself indirect/.test(flat.why),
              'and a list at a cRLIssuer point that does not declare indirectCRL is ' +
              'not the list the certificate named', flat.why);
    });
  });
  ...
  status.resetCache();
  await withSettings({ 'pki.revocationCheck': 'hard-fail',
                       'pki.revocationCrlIssuersFile': impostorOnly }, async function () {
    const onlyImpostor = await status.verdictFor(input(fx.leaf.indirectForged));
    t.check(onlyImpostor.refused && /do not chain to an authority/.test(onlyImpostor.why),
            'and with ONLY the impostor to go on — the right name, cRLSign, listed in ' +
            'the issuers file — no signer is authorised at all, because it does not ' +
            'chain to an authority the leaf\'s own path passes through', onlyImpostor.why);
  });
}
...
async function theScope(t, fx) {
  ...
  const idp = function (label, lines) {
    return ['issuingDistributionPoint=critical,@idp_' + label, '[idp_' + label + ']']
      .concat(lines);
  };
  const OTHERS = 'CACompromise,affiliationChanged,superseded,cessationOfOperation,' +
  ...
  routes['/s-onlyuser.crl'] = { body: opensslCrl(fx, fx.issuing, 's-onlyuser', '01', [],
    idp('b', ['fullname=URI:' + fx.base + '/s-onlyuser.crl', 'onlyuser=TRUE'])) };
  ...
  routes['/s-key.crl'] = { body: opensslCrl(fx, fx.issuing, 's-key', '01', [],
    idp('d', ['fullname=URI:' + fx.base + '/s-key.crl', 'onlysomereasons=keyCompromise'])) };
  routes['/s-rest.crl'] = { body: opensslCrl(fx, fx.issuing, 's-rest', '01', [],
    idp('e', ['fullname=URI:' + fx.base + '/s-rest.crl', 'onlysomereasons=' + OTHERS])) };
  ...
  await withSettings({ 'pki.revocationCheck': 'hard-fail' }, async function () {
    ...
    t.check(onlyCa.refused && /covers only CA certificates/.test(onlyCa.why),
            'a list with onlyContainsCACerts is not about an end-entity certificate',
            onlyCa.why);
    const onlyUser = await status.verdictFor(input(fx.subCa.pem));
    t.check(onlyUser.refused && /covers only end-entity certificates/.test(onlyUser.why),
            'and one with onlyContainsUserCerts is not about a CA certificate',
            onlyUser.why);
    const otherName = await status.verdictFor(input(fx.leaf.scopeOtherName.pem));
    t.check(otherName.refused && /names a different list/.test(otherName.why),
            'a list whose issuing distribution point names ANOTHER list is not the one ' +
            'the certificate pointed at, whoever signed it', otherName.why);
    const partial = await status.verdictFor(input(fx.leaf.scopePartial.pem));
    t.check(partial.refused &&
            partial.unknown.some(function (one) { return one.kind === 'incomplete-reasons'; }),
            'a list covering only keyCompromise (onlySomeReasons) does not make a ' +
            'certificate good — the other reasons are unanswered', partial.why);
    const pointReasons = await status.verdictFor(input(fx.leaf.scopePointReasons.pem));
    t.check(pointReasons.refused &&
            pointReasons.unknown.some(function (one) { return one.kind === 'incomplete-reasons'; }),
            'and the same when the DISTRIBUTION POINT names only keyCompromise, over a ' +
            'list that covers everything', pointReasons.why);
    const complete = await status.verdictFor(input(fx.leaf.scopeComplete.pem));
    t.check(complete.status === 'good' && !complete.refused,
            'while two points whose lists cover keyCompromise and every other reason ' +
            'between them answer good', complete.why);
  });
}
...
async function extendedChildBody() {
  ...
  if (!haveOpenSsl()) {
    t.bad('openssl is required for sections 8 to 11 and is not installed',
          'install openssl; these documents must come from an implementation that ' +
...
// ldapjs server on ephemeral ports, TLS on one of them), the two delta checks no
// fixture above could reach, and a REGISTERED certificate asked about with no
// chain presented. A fixture of its own rather than more rows in
...
async function closingFixture(dir, base, ldapsUrl, ldapUrl) {
  ...
  const sections = [
    ...
    '[issuing_ext]', 'basicConstraints=critical,CA:TRUE',
    'keyUsage=critical,keyCertSign,cRLSign', 'subjectKeyIdentifier=hash',
    'authorityKeyIdentifier=keyid', 'crlDistributionPoints=URI:' + url('/c-root.crl'),
    'authorityInfoAccess=caIssuers;URI:' + url('/ca/root.cer'),
    '[crl_issuer_ext]', 'basicConstraints=CA:FALSE',
    'keyUsage=critical,cRLSign,digitalSignature', 'subjectKeyIdentifier=hash',
    '[impostor_ext]', 'basicConstraints=critical,CA:TRUE',
    'keyUsage=critical,keyCertSign,cRLSign,digitalSignature',
    '[server_ext]', 'basicConstraints=CA:FALSE', 'keyUsage=critical,digitalSignature',
    'extendedKeyUsage=serverAuth', 'subjectAltName=IP:127.0.0.1',
    '[resp_nocheck_ext]', 'basicConstraints=CA:FALSE', 'keyUsage=critical,digitalSignature',
    'extendedKeyUsage=OCSPSigning', '1.3.6.1.5.5.7.48.1.5=ASN1:NULL',
    'crlDistributionPoints=URI:' + url('/r-responders.crl'),
    '[resp_revoked_ext]', 'basicConstraints=CA:FALSE', 'keyUsage=critical,digitalSignature',
    'extendedKeyUsage=OCSPSigning', 'crlDistributionPoints=URI:' + url('/r-responders.crl'),
    '[resp_unknown_ext]', 'basicConstraints=CA:FALSE', 'keyUsage=critical,digitalSignature',
    'extendedKeyUsage=OCSPSigning', 'crlDistributionPoints=URI:' + url('/r-missing.crl'),
    'authorityInfoAccess=OCSP;URI:' + url('/ocsp/self'),
    '[resp_good_ext]', 'basicConstraints=CA:FALSE', 'keyUsage=critical,digitalSignature',
    'extendedKeyUsage=OCSPSigning', 'crlDistributionPoints=URI:' + url('/r-empty.crl'),
    '[cdp_cai]', 'fullname=URI:' + url('/c-indirect.crl'), 'CRLissuer=dirName:crl_issuer_dn',
    '[cdp_p7]', 'fullname=URI:' + url('/c-pkcs7.crl'), 'CRLissuer=dirName:crl_issuer_dn',
    '[cdp_imp]', 'fullname=URI:' + url('/c-impostor.crl'), 'CRLissuer=dirName:crl_issuer_dn',
    '[cdp_ldapcai]', 'fullname=URI:' + url('/c-ldapcai.crl'), 'CRLissuer=dirName:crl_issuer_dn',
    '[crl_issuer_dn]', 'O=revc', 'CN=revc crl issuer',
    '[cdp_rel]', 'relativename=rel_dn', '[rel_dn]', 'CN=ldaprel',
    '[cdp_relmulti]', 'relativename=relm_dn', '[relm_dn]', 'CN=ldaprel', '+OU=second value'
  ];
  ...
  const LEAVES = [
    ...
    ['rNocheck', '6001', ['authorityInfoAccess=OCSP;URI:' + url('/ocsp/nocheck')]],
    ['rRevoked', '6002', ['authorityInfoAccess=OCSP;URI:' + url('/ocsp/revoked')]],
    ['rUnknown', '6003', ['authorityInfoAccess=OCSP;URI:' + url('/ocsp/unknown')]],
    ...
    ['caiRollover', '6104', ['crlDistributionPoints=URI:' + url('/c-rollover.crl')]],
    ['caiRolloverNoAia', '6105', ['crlDistributionPoints=URI:' + url('/c-rollover-noaia.crl')]],
    // 14. ldap and ldaps
    ['lRevoked', '6201', ['crlDistributionPoints=URI:' + ldapsUrl + '/cn=ldapcrl%2Co=revc']],
    ['lPlain', '6202', ['crlDistributionPoints=URI:' + ldapUrl + '/cn=plaincrl%2Co=revc']],
    ['lReferral', '6203', ['crlDistributionPoints=URI:' + ldapsUrl + '/cn=referral%2Co=revc']],
    ['lHuge', '6204', ['crlDistributionPoints=URI:' + ldapsUrl + '/cn=huge%2Co=revc']],
    ['lSilent', '6205', ['crlDistributionPoints=URI:' + ldapsUrl + '/cn=silent%2Co=revc']],
    ...
    ['lTwins', '6208', ['crlDistributionPoints=URI:' + ldapsUrl + '/cn=twins%2Co=revc']],
    ['lCaIssuers', '6209', ['crlDistributionPoints=cdp_ldapcai']],
    ['lStray', '6210', ['crlDistributionPoints=URI:' + ldapsUrl + '/cn=ldapcrl%2Co=revc?userPassword']],
    ['lRelativeMulti', '6211', ['crlDistributionPoints=cdp_relmulti']],
    // 15. the two delta checks
    ['dOtherSigner', '6301', ['crlDistributionPoints=URI:' + url('/x-base.crl')]],
    ...
    ['gRevokedChain', '6401', ['crlDistributionPoints=URI:' + url('/g-revokes.crl')]],
    ['gRevokedAia', '6402', ['crlDistributionPoints=URI:' + url('/g-revokes.crl'),
                             'authorityInfoAccess=caIssuers;URI:' + url('/ca/issuing.cer')]],
    ...
    ['gDeadAia', '6405', ['crlDistributionPoints=URI:' + url('/g-revokes.crl'),
                          'authorityInfoAccess=caIssuers;URI:' + url('/ca/missing.cer')]],
    ['gWrongAia', '6406', ['crlDistributionPoints=URI:' + url('/g-revokes.crl'),
                           'authorityInfoAccess=caIssuers;URI:' + url('/ca/crlissuer.cer')]]
  ];
  LEAVES.forEach(function (one) {
  ...
  fx.issuing = issued('issuing', '/O=revc/CN=revc issuing', 'root', '0100', 'issuing_ext');
  ...
  fx.issuingNew = issued('issuing-new', '/O=revc/CN=revc issuing', 'root', '0101', 'issuing_ext');
  fx.crlIssuer = issued('crlissuer', '/O=revc/CN=revc crl issuer', 'root', '0300',
                        'crl_issuer_ext');
  fx.crlIssuerTwin = issued('crlissuer-twin', '/O=revc/CN=revc crl issuer', 'root', '0301',
                            'crl_issuer_ext');
  fx.crlIssuerImpostor = selfSigned('crlissuer-impostor', '/O=revc/CN=revc crl issuer',
                                    'impostor_ext');
  fx.server = issued('server', '/O=revc/CN=127.0.0.1', 'root', '0500', 'server_ext');
  fx.respNocheck = issued('resp-nocheck', '/O=revc/CN=revc responder nocheck', 'issuing',
                          '0200', 'resp_nocheck_ext');
  fx.respRevoked = issued('resp-revoked', '/O=revc/CN=revc responder revoked', 'issuing',
                          '0201', 'resp_revoked_ext');
  fx.respUnknown = issued('resp-unknown', '/O=revc/CN=revc responder unknown', 'issuing',
                          '0202', 'resp_unknown_ext');
  fx.respGood = issued('resp-good', '/O=revc/CN=revc responder good', 'issuing', '0203',
                       'resp_good_ext');
  fx.leaf = {};
  LEAVES.forEach(function (one) {
    fx.leaf[one[0]] = issued('leaf-' + one[0], '/O=revc/CN=revc ' + one[0], 'issuing',
                             one[1], one[0] + '_ext');
  });
  ...
  fx.der = { root: derOf(fx.root), issuing: derOf(fx.issuing),
             ...
             crlIssuerTwin: derOf(fx.crlIssuerTwin), impostor: derOf(fx.crlIssuerImpostor) };
  openssl(['crl2pkcs7', '-nocrl', '-certfile', fx.crlIssuer.path, '-outform', 'DER',
...
function ldapDirectory(tlsMaterial, port) {
  ...
  const answer = function (req, res, next) {
    ...
    if (entry.referral) {
      res.send(res.createSearchReference(['ldaps://elsewhere.example/cn=x,o=revc']));
      res.end();
      return next();
    }
    // Every attribute is sent whatever was asked for: ldapjs's own server-side
    // filtering drops a requested `;binary` type it should have matched, and what
    // is under test is the client's reading, not that filter.
    res.attributes = [];
  ...
  return new Promise(function (resolve) {
    server.listen(port || 0, '127.0.0.1', function () {
      resolve({ server: server, port: server.address().port, entries: entries, hits: hits,
                keyOf: keyOf });
    });
  });
}
...
async function theResponderStatus(t, fx) {
  t.log.info('=== 12. a delegated OCSP responder\'s OWN status: nocheck, revoked, ' +
             'unknown and good, and never asked through OCSP ===');
  const status = require('../common/revocation_status');
  const routes = fx.routes;
  const hits = fx.hits;
  const index = path.join(fx.dir, 'ocsp-index-12.txt');
  fs.writeFileSync(index, [indexLine('6001'), indexLine('6002'), indexLine('6003'),
                           indexLine('6004')].join(''));
  const responder = function (signer) {
    return { ocsp: { dir: fx.dir, index: index, ca: fx.issuing.path,
                     signer: signer.path, key: signer.keyPath } };
  ...
  routes['/r-responders.crl'] = { body: opensslCrl(fx, fx.issuing, 'r-responders', '01',
    [indexLine('0200', 'keyCompromise'), indexLine('0201', 'keyCompromise')]) };
  routes['/r-empty.crl'] = { body: opensslCrl(fx, fx.issuing, 'r-empty', '01', []) };
  const input = function (leaf) {
    return { leaf: leaf.pem, chain: [fx.issuing.pem, fx.root.pem], verified: true };
  };
  await withSettings({ 'pki.revocationCheck': 'hard-fail' }, async function () {
    const nocheck = await status.verdictFor(input(fx.leaf.rNocheck));
    t.check(nocheck.status === 'good' && nocheck.links[0].ocspResponderStatus === 'not-checked' &&
            /id-pkix-ocsp-nocheck/.test(nocheck.links[0].why),
            'A RESPONDER CARRYING id-pkix-ocsp-nocheck IS NOT CHECKED, AND SAYS SO — even ' +
            'though the CRL its certificate names lists it: the issuer that put the ' +
            'extension there decided its revocation is not a question',
            JSON.stringify(nocheck.links[0]));
    t.check(!hits['/r-responders.crl'],
            'and that CRL was not fetched at all', JSON.stringify(hits['/r-responders.crl']));
    const good = await status.verdictFor(input(fx.leaf.rGood));
    t.check(good.status === 'good' && good.links[0].ocspResponderStatus === 'good',
            'a responder whose own CRL does not list it is GOOD, and its answer is used',
            JSON.stringify(good.links[0]));
    const revoked = await status.verdictFor(input(fx.leaf.rRevoked));
    t.check(revoked.status !== 'good' && revoked.refused && /itself REVOKED/.test(revoked.why),
            'A REVOKED RESPONDER\'S ANSWERS ARE UNUSABLE: its signed "good" is not ' +
            'believed, and with nothing else to go on hard-fail refuses', revoked.why);
    ...
    t.check(unknown.refused && /could not be established/.test(unknown.why),
            'A RESPONDER WHOSE OWN STATUS IS UNKNOWN — its CRL is a 404 — IS NOT ' +
            'BELIEVED UNDER HARD-FAIL: the fetch an attacker blocks must not become the ' +
            'way a compromised responder is trusted', unknown.why);
    t.check(!hits['/ocsp/self'],
            'AND ITS STATUS IS NEVER ASKED THROUGH OCSP — its own certificate names a ' +
            'responder, and that responder is not dialled',
  ...
  await withSettings({ 'pki.revocationCheck': 'soft-fail' }, async function () {
    ...
    t.check(revoked.status !== 'good' && /itself REVOKED/.test(revoked.why),
            'a revoked responder is unusable under SOFT-FAIL too — a signed fact is not ' +
            'a status that could not be established', revoked.why);
    const unknown = await status.verdictFor(input(fx.leaf.rUnknown));
    t.check(unknown.status === 'good' && !unknown.refused &&
            unknown.links[0].ocspResponderStatus === 'unknown',
            'while soft-fail believes a responder whose status is unknown, and records ' +
            'that it did', JSON.stringify(unknown.links[0]));
  });
}

async function theCaIssuers(t, fx) {
  t.log.info('=== 13. a list\'s signer fetched from its own caIssuers address: DER, ' +
             'PKCS#7, an impostor, and an issuer\'s rollover key ===');
  const status = require('../common/revocation_status');
  const routes = fx.routes;
  const hits = fx.hits;
  routes['/ca/crlissuer.cer'] = { body: fx.der.crlIssuer, contentType: 'application/pkix-cert' };
  routes['/ca/crlissuer.p7c'] = { body: fx.p7c, contentType: 'application/pkcs7-mime' };
  routes['/ca/impostor.cer'] = { body: fx.der.impostor, contentType: 'application/pkix-cert' };
  routes['/ca/issuing-new.cer'] = { body: fx.der.issuingNew,
                                    contentType: 'application/pkix-cert' };
  routes['/c-indirect.crl'] = { body: await indirectCrl(fx.crlIssuer, {
    url: fx.base + '/c-indirect.crl', caIssuers: [fx.base + '/ca/crlissuer.cer'],
  ...
  routes['/c-rollover.crl'] = { body: opensslCrl(fx, fx.issuingNew, 'c-rollover', '01',
    [indexLine('6104', 'keyCompromise')],
    ['authorityInfoAccess=caIssuers;URI:' + fx.base + '/ca/issuing-new.cer']) };
  routes['/c-rollover-noaia.crl'] = { body: opensslCrl(fx, fx.issuingNew, 'c-rollover-noaia',
  ...
  await withSettings({ 'pki.revocationCheck': 'hard-fail' }, async function () {
    ...
    t.check(der.status === 'revoked' && der.links[0].indirect === true &&
            ...
            'AN INDIRECT CRL WHOSE SIGNER IS NOWHERE HERE IS TRUSTED THROUGH THE ' +
            'caIssuers ADDRESS THE LIST ITSELF NAMES: the DER certificate there carries ' +
            'the name, cRLSign, chains to the leaf\'s Root, and revokes the leaf',
            der.why);
    const again = await status.verdictFor(input(fx.leaf.caiIndirect));
    t.check(again.status === 'revoked' && hits['/ca/crlissuer.cer'] === 1,
            'and the fetched certificate is cached — asked again, it is not fetched again',
            JSON.stringify(hits['/ca/crlissuer.cer']));
    const pkcs7 = await status.verdictFor(input(fx.leaf.caiPkcs7));
    t.check(pkcs7.status === 'revoked' && hits['/ca/crlissuer.p7c'] > 0,
            'and the same from a PKCS#7 certs-only bundle, the other form RFC 5280 ' +
            'section 4.2.2.1 names', pkcs7.why);
    const impostor = await status.verdictFor(input(fx.leaf.caiImpostor));
    t.check(impostor.refused && impostor.status === 'unknown' && /caIssuers/.test(impostor.why),
            'A caIssuers ADDRESS SERVING AN IMPOSTOR — the right name, self-signed, ' +
            'signing the very list that points at it — is not believed: it chains to ' +
            'nothing the leaf\'s path passes through', impostor.why);
    ...
    t.check(rollover.status === 'revoked' && hits['/ca/issuing-new.cer'] > 0,
            'A DIRECT LIST SIGNED WITH THE ISSUER\'S ROLLOVER KEY — the same name, a ' +
            'different key, the same Root — is verified through the certificate its ' +
            'caIssuers address serves, and revokes the leaf', rollover.why);
    ...
    t.check(noAia.refused && noAia.status === 'unknown' &&
            ...
            'while the same list naming no caIssuers address cannot be verified, and ' +
            'says why', noAia.why);
  });
}

async function theLdap(t, fx, ldaps, plain) {
  t.log.info('=== 14. ldap and ldaps distribution points, against a real directory ===');
  const status = require('../common/revocation_status');
  const lcrl = opensslCrl(fx, fx.issuing, 'l-crl', '01',
    ['6201', '6202', '6203', '6204', '6205', '6206', '6207', '6208'].map(function (one) {
      return indexLine(one, 'keyCompromise');
    }));
  const lcrlDer = Buffer.from(String(lcrl).replace(/-----[^-]+-----/g, '').replace(/\s+/g, ''),
                              'base64');
  const crlEntry = { attributes: { objectClass: 'cRLDistributionPoint',
                                   'certificateRevocationList;binary': lcrlDer } };
  ...
  fx.routes['/c-ldapcai.crl'] = { body: await indirectCrl(fx.crlIssuerTwin, {
    ...
    caIssuers: ['ldaps://127.0.0.1:' + ldaps.port + '/cn=crlissuerldap,o=revc?cACertificate;binary'],
  ...
  await withSettings({ 'pki.revocationCheck': 'hard-fail' }, async function () {
    ...
    t.check(noCa.refused && noCa.status === 'unknown' && /connection failed/.test(noCa.why),
            'LDAPS VERIFIES THE DIRECTORY: with the directory\'s CA in neither node\'s ' +
            'store nor pki.revocationLdapCaFile, the TLS handshake fails and hard-fail ' +
            'refuses', noCa.why);
    await withSettings({ 'pki.revocationLdapCaFile': caFile }, async function () {
      status.resetCache();
      const revoked = await status.verdictFor(input(fx.leaf.lRevoked));
      t.check(revoked.status === 'revoked' && hitsOf(ldaps, 'cn=ldapcrl,o=revc') > 0,
              'AN ldaps: DISTRIBUTION POINT IS FETCHED — certificateRevocationList;binary ' +
              'out of a base-object search on a directory whose certificate chains to ' +
              'pki.revocationLdapCaFile — and the list revokes the leaf', revoked.why);
      const plainDefault = await status.verdictFor(input(fx.leaf.lPlain));
      t.check(!plainDefault.refused && plainDefault.status === 'unknown' &&
              /plain ldap/.test(plainDefault.why) && hitsOf(plain, 'cn=plaincrl,o=revc') === 0,
              'A PLAIN ldap: ADDRESS IS NOT DIALLED BY DEFAULT, and the certificate is ' +
              'answered as naming nothing this service dials — no fetch was blocked, so ' +
              'hard-fail does not refuse it', plainDefault.why);
      await withSettings({ 'pki.revocationLdap': 'ldaps-and-ldap' }, async function () {
        const plainOn = await status.verdictFor(input(fx.leaf.lPlain));
        t.check(plainOn.status === 'revoked' && hitsOf(plain, 'cn=plaincrl,o=revc') > 0,
                'while pki.revocationLdap=ldaps-and-ldap dials it and the list revokes',
                plainOn.why);
      });
      ...
      t.check(referral.refused && /referral is not followed/.test(referral.why),
              'A REFERRAL IS NOT FOLLOWED — the address the issuer signed is the only one',
              referral.why);
      ...
      t.check(twins.refused && /2 entries where one was named/.test(twins.why),
              'and a directory answering a base-object search with two entries is not ' +
              'believed about either', twins.why);
      ...
      t.check(!critical.refused && /critical extension/.test(critical.why),
              'AN LDAP URL CARRYING A CRITICAL EXTENSION IS NOT DIALLED (RFC 4516 section ' +
              '2.1), and is reported as such', critical.why);
      await withSettings({ 'pki.revocationMaxCrlBytes': 4096 }, async function () {
        const huge = await status.verdictFor(input(fx.leaf.lHuge));
        t.check(huge.refused && /more than 4096 bytes/.test(huge.why),
                'THE SIZE CAP HOLDS OVER LDAP: the connection\'s bytes are counted as they ' +
                'arrive and the socket destroyed at pki.revocationMaxCrlBytes', huge.why);
      });
      await withSettings({ 'pki.revocationFetchTimeoutMs': 400 }, async function () {
        const started = Date.now();
        const silent = await status.verdictFor(input(fx.leaf.lSilent));
        t.check(silent.refused && /did not answer within 400ms/.test(silent.why) &&
                Date.now() - started < 3000,
                'AND SO DOES THE DEADLINE: a directory that never answers the search is ' +
                'given up on at pki.revocationFetchTimeoutMs', silent.why);
      });
      const relativeNone = await status.verdictFor(input(fx.leaf.lRelative));
      t.check(!relativeNone.refused && /pki\.revocationLdapDirectory/.test(relativeNone.why),
              'A NAME RELATIVE TO ITS CRL ISSUER, WITH NO DIRECTORY CONFIGURED, IS ' +
              'REFUSED BY NAME — the certificate does not say which directory, and a guess ' +
              'is a request to a host nobody signed', relativeNone.why);
      await withSettings({ 'pki.revocationLdapDirectory': 'ldaps://127.0.0.1:' + ldaps.port },
        async function () {
          status.resetCache();
          ...
          t.check(relative.status === 'revoked' &&
                  ...
                  'while with pki.revocationLdapDirectory set, the relative name is joined ' +
                  'to the issuer\'s — "CN=ldaprel,CN=revc issuing,O=revc" — looked up ' +
                  'there, and the list revokes', relative.why);
        });
      ...
      t.check(stray.refused && /userPassword/.test(stray.why) &&
              ...
              'AN LDAP URL NAMING SOME OTHER ATTRIBUTE IS NOT READ — a distribution point ' +
              'is not a way to have this service fetch userPassword', stray.why);
      const multi = await status.verdictFor(input(fx.leaf.lRelativeMulti));
      t.check(!multi.refused && /multi-valued RDN/.test(multi.why),
              'A RELATIVE NAME THAT IS A MULTI-VALUED RDN IS REFUSED BY NAME — it has no one ' +
              'string a directory is sure to index it under', multi.why);
      await withSettings({ 'pki.revocationLdapDirectory': 'ldaps://127.0.0.1:1/extra' },
        async function () {
          const malformed = await status.verdictFor(input(fx.leaf.lRelative));
          t.check(!malformed.refused && /not an ldap:\/\/ or ldaps:\/\/ address/.test(malformed.why),
                  'and a directory setting with anything after the host is refused rather ' +
                  'than having a DN appended to it', malformed.why);
        });
      const parsed = [status.parseLdapUrl('ldap:///cn=x'),
                      ...
                      status.parseLdapUrl('ldaps://h:1636/cn=a%2Co=b?cACertificate;binary')];
      t.check(!parsed[0].ok && /names no host/.test(parsed[0].why) &&
              ...
              parsed[3].ok && parsed[3].port === 1636 && parsed[3].dn === 'cn=a,o=b' &&
              parsed[3].attributes[0] === 'cACertificate;binary',
              'RFC 4516, READ STRICTLY: no host is refused, a scope other than base is ' +
              'refused, a critical extension (!bindname) is refused, and a percent-encoded ' +
              'DN, a port and an attribute are read', JSON.stringify(parsed));
      const ldapCai = await status.verdictFor(input(fx.leaf.lCaIssuers));
      t.check(ldapCai.status === 'revoked' && hitsOf(ldaps, 'cn=crlissuerldap,o=revc') > 0,
              'AND A caIssuers ADDRESS IN ldaps: — cACertificate;binary — supplies an ' +
              'indirect list\'s signer the same way http does', ldapCai.why);
    });
  });
}

async function theDeltaSigners(t, fx) {
  t.log.info('=== 15. the two delta checks no earlier fixture reached: a delta signed ' +
             'by another authorised key, and a base with no cRLNumber ===');
  const status = require('../common/revocation_status');
  const routes = fx.routes;
  routes['/x-base.crl'] = { body: opensslCrl(fx, fx.issuing, 'x-base', '10', [],
    ['freshestCRL=URI:' + fx.base + '/x-delta.crl']) };
  routes['/x-delta.crl'] = { body: opensslCrl(fx, fx.issuingNew, 'x-delta', '11',
  ...
  routes['/x-nonum-delta.crl'] = { body: opensslCrl(fx, fx.issuing, 'x-nonum-delta', '02',
    [indexLine('6302', 'keyCompromise')], ['2.5.29.27=critical,ASN1:INTEGER:1']) };
  routes['/x-ok.crl'] = { body: opensslCrl(fx, fx.issuing, 'x-ok', '20', [],
    ['freshestCRL=URI:' + fx.base + '/x-ok-delta.crl']) };
  routes['/x-ok-delta.crl'] = { body: opensslCrl(fx, fx.issuing, 'x-ok-delta', '21',
    [indexLine('6303', 'keyCompromise')], ['2.5.29.27=critical,ASN1:INTEGER:32']) };
  const openSslText = String(childProcess.spawnSync('openssl', ['crl', '-in',
    path.join(fx.dir, 'crl-x-nonum', 'crl.pem'), '-noout', '-text'], { encoding: 'utf8' }).stdout);
  t.check(/Freshest CRL/.test(openSslText) && !/CRL Number/.test(openSslText),
          'the base list OpenSSL wrote carries a freshestCRL and NO cRLNumber, which is ' +
          'the fixture the base-number check needed', openSslText.slice(0, 300));
  const input = function (leaf) {
    return { leaf: leaf.pem, chain: [fx.issuing.pem, fx.root.pem], verified: true };
  };
  await withSettings({ 'pki.revocationCheck': 'hard-fail' }, async function () {
    const same = await status.verdictFor(input(fx.leaf.dSameSigner));
    t.check(same.status === 'revoked' && /with its delta at/.test(same.links[0].why),
            'the control: a base and a delta from the same issuer and key merge, and the ' +
            'delta revokes', same.links[0].why);
    ...
    t.check(other.refused && other.status === 'unknown' &&
            ...
            'A DELTA SIGNED BY ANOTHER AUTHORISED KEY THAN ITS BASE — the issuer\'s own ' +
            'rollover key, verified through its caIssuers address — is not merged: a ' +
            'delta updates the list its own signer wrote', other.why);
    ...
    t.check(noNumber.refused && noNumber.status === 'unknown' &&
            ...
            'AND A BASE CRL WITH NO cRLNumber CANNOT HAVE A DELTA APPLIED — there is ' +
            'nothing to check the delta\'s BaseCRLNumber against', noNumber.why);
  });
}

async function theRegisteredApi(t, fx) {
  t.log.info('=== 17. a REGISTERED certificate: nothing presented, its issuer fetched ' +
             'from its caIssuers address, and a bare key reported as one ===');
  const status = require('../common/revocation_status');
  const errorCodes = require('../common/error_codes');
  const routes = fx.routes;
  const hits = fx.hits;
  routes['/ca/root.cer'] = { body: fx.der.root, contentType: 'application/pkix-cert' };
  routes['/ca/issuing.cer'] = { body: fx.der.issuing, contentType: 'application/pkix-cert' };
  routes['/g-revokes.crl'] = { body: opensslCrl(fx, fx.issuing, 'g-revokes', '01',
    ['6401', '6402', '6404', '6405', '6406'].map(function (one) {
      return indexLine(one, 'keyCompromise');
    })) };
  await withSettings({ 'pki.revocationCheck': 'hard-fail' }, async function () {
    const withChain = await status.registeredVerdictFor({
      certificate: fx.leaf.gRevokedChain.pem, chain: [fx.issuing.pem, fx.root.pem],
      source: 'a test registration' });
    t.check(withChain.refused && withChain.status === 'revoked' && withChain.registered === true,
            'a registered certificate registered WITH its chain is looked up on the CRL ' +
            'it names, and a revoked one is refused', withChain.why);
    t.equal(errorCodes.codeOf(withChain), 'STS-PKI-0129',
            'carrying STS-PKI-0129 — the REGISTERED code, not the presented one');
    const alone = await status.registeredVerdictFor({
      certificate: String(fx.leaf.gRevokedAia.pem).replace(/-----[^-]+-----/g, '')
        .replace(/\s+/g, ''), source: 'base64 DER, as fedSigningCertificate holds it' });
    t.check(alone.status === 'revoked' && hits['/ca/issuing.cer'] > 0 &&
            (alone.issuersFetched || []).length === 2,
            'A REGISTERED LEAF WITH NOTHING BESIDE IT: its issuer is fetched from its ' +
            'own caIssuers address, and that issuer\'s from its — up to a self-signed ' +
            'Root — and the CRL revokes it', JSON.stringify(alone.issuersFetched) + ' ' + alone.why);
    const good = await status.registeredVerdictFor({ certificate: fx.leaf.gGoodAia.pem,
                                                      source: 'a test registration' });
    t.check(good.status === 'good' && !good.refused,
            'while a good one built the same way is good at every link', good.why);
    const noAia = await status.registeredVerdictFor({ certificate: fx.leaf.gNoAia.pem,
                                                       source: 'a test registration' });
    t.check(!noAia.refused && noAia.status === 'unknown' &&
            noAia.unknown.some(function (one) { return one.kind === 'no-distribution-point'; }),
            'A REGISTERED LEAF WHOSE ISSUER IS NOWHERE AND WHICH NAMES NO caIssuers ' +
            'ADDRESS gives an attacker nothing to block, so hard-fail does not refuse it',
            noAia.why);
    await withSettings({ 'pki.revocationRequireDistributionPoint': true }, async function () {
      const required = await status.registeredVerdictFor({ certificate: fx.leaf.gNoAia.pem,
                                                            source: 'a test registration' });
      t.check(required.refused,
              'unless pki.revocationRequireDistributionPoint says it must', required.why);
    });
    const dead = await status.registeredVerdictFor({ certificate: fx.leaf.gDeadAia.pem,
                                                      source: 'a test registration' });
    t.check(dead.refused && dead.unknown.some(function (one) {
              return one.kind === 'issuer-not-fetched';
            }),
            'WHILE ONE NAMING A caIssuers ADDRESS THAT DID NOT ANSWER IS REFUSED — that ' +
            'is the fetch an attacker blocks', dead.why);
    const wrong = await status.registeredVerdictFor({ certificate: fx.leaf.gWrongAia.pem,
                                                       source: 'a test registration' });
    t.check(wrong.refused && wrong.unknown.some(function (one) {
              return one.kind === 'issuer-not-fetched';
            }) && /none of the 1 certificate/.test(wrong.why),
            'and so is one whose caIssuers address serves a certificate that did not sign ' +
            'it — what is fetched is believed only for its key verifying the one below',
            wrong.why);
    const bare = await status.registeredKeyVerdictFor({ kty: 'EC', crv: 'P-256', kid: 'k1',
      x: 'AAAA', y: 'AAAA' }, 'a bare JWK');
    t.check(bare.bare === true && bare.status === 'unchecked' && !bare.refused &&
            /bare key/.test(bare.why),
            'A BARE JWK — no x5c — IS REPORTED AS NOTHING TO CHECK, never as good', bare.why);
    const x5c = await status.registeredKeyVerdictFor({ kty: 'EC', kid: 'k2',
      x5c: [String(fx.leaf.gRevokedChain.pem).replace(/-----[^-]+-----/g, '').replace(/\s+/g, ''),
            String(fx.issuing.pem).replace(/-----[^-]+-----/g, '').replace(/\s+/g, '')] },
      'a JWK with an x5c');
    t.check(x5c.refused && x5c.status === 'revoked',
            'while a JWK carrying an x5c is answered through that certificate', x5c.why);
  });
  await withSettings({ 'pki.revocationCheck': 'off' }, async function () {
    const off = await status.registeredVerdictFor({ certificate: fx.leaf.gRevokedChain.pem,
                                                     source: 'a test registration' });
    t.check(!off.refused && off.status === 'unchecked',
            'and pki.revocationCheck=off consults nothing for a registered one either',
            off.why);
  });
}
...
async function closingChildBody() {
  ...
  try {
    ...
    const fx = await closingFixture(dir, fixture.base, 'ldaps://127.0.0.1:' + ldapsPort,
                                    'ldap://127.0.0.1:' + plain.port);
    fx.routes = fixture.routes;
    fx.hits = fixture.hits;
    fx.routes['/c-root.crl'] = { body: opensslCrl(fx, fx.root, 'c-root', '01', []) };
    ldaps = await ldapDirectory({ pem: fx.server.pem, key: fx.server.key }, ldapsPort);
    await withSettings({ 'pki.revocationFailureRetryS': 0 }, async function () {
...
function signCompact(header, claims, privateKeyPem) {
  ...
  const signature = nodeCrypto.sign('sha' + String(header.alg).slice(2), Buffer.from(signing),
...
async function secondRealm(label) {
  ...
  const made = realms.create({ id: id, name: id, description: 'Created by ' + __filename });
  if (!made.ok) {
    throw new Error('the realm could not be created: ' + (made.errors || []).join(' '));
  }
  const branch = await pki.ensureScope(id);
  if (!branch.ok) {
    throw new Error('the realm branch could not be built: ' + (branch.errors || []).join(' '));
  }
  return made.realm;
}
...
async function theOwnIndirectSigner(t, fixture) {
  t.log.info('=== 16. THIS SERVICE\'S OWN AUTHORITY as the signer of an indirect CRL ' +
             'about a certificate one of its authorities did not sign ===');
  if (!haveOpenSsl()) {
    t.bad('openssl is required for section 16 and is not installed', 'install openssl');
    return;
  }
  const pki = require('../common/pki');
  const status = require('../common/revocation_status');
  ...
  try {
    fs.writeFileSync(path.join(dir, 'intermediate.pem'), row.intermediate.certificatePem);
    fs.writeFileSync(path.join(dir, 'intermediate.key'), row.intermediate.privateKeyPem);
    const joseSubject = new nodeCrypto.X509Certificate(jose.certificatePem).subject.split('\n');
    const url = fixture.base + '/own-indirect.crl';
    fs.writeFileSync(path.join(dir, 'ext.cnf'), [
      '[req]', 'distinguished_name=dn', 'prompt=no', '[dn]', 'CN=unused',
      '[subca_ext]', 'basicConstraints=critical,CA:TRUE', 'keyUsage=critical,keyCertSign,cRLSign',
      '[leaf_ext]', 'basicConstraints=CA:FALSE', 'keyUsage=critical,digitalSignature',
    ...
    const subCa = issue('subca', '/CN=rev own sub ca', 'intermediate', '7001', 'subca_ext');
    ...
    fixture.routes['/own-indirect.crl'] = { body: await indirectCrl(
      ...
      { url: url, entries: [{ serial: '7002', certificateIssuerPem: subCa }] }) };
    await withSettings({ 'pki.revocationCheck': 'hard-fail', 'pki.revocationFailureRetryS': 0 },
      async function () {
        status.resetCache();
        const verdict = await status.verdictFor({ leaf: leaf,
          chain: [subCa, row.intermediate.certificatePem], verified: true });
        t.check(verdict.status === 'revoked' && verdict.links[0].indirect === true &&
                verdict.links[1] && verdict.links[1].source === 'register',
                'A CERTIFICATE UNDER A SUB-CA NOBODY HERE HOLDS, WHOSE cRLIssuer IS THIS ' +
                'REALM\'S OWN JOSE ISSUING CA, is revoked by the indirect list that ' +
                'authority signed — its certificate found among this service\'s own ' +
                'authorities, authorised because it chains to the realm Intermediate the ' +
                'sub-CA was issued by — while the sub-CA itself is answered by the register',
                verdict.why + ' ' + JSON.stringify(verdict.links.map(function (one) {
                  return one.source;
                })));
      });
  } finally {
...
async function theRegisteredDoors(t) {
  t.log.info('=== 18. the doors that USE a registered certificate: RFC 7523 grant and ' +
             'client authentication, RFC 7522, a federation partner, and OID4VP ===');
  const pki = require('../common/pki');
  const realms = require('../common/realms');
  const revocation = require('../common/pki_revocation');
  ...
  const revoke = function (pem) {
    return revocation.revoke(REALM, useCaseOf(REALM, pem),
      { serialHex: new nodeCrypto.X509Certificate(pem).serialNumber, reason: 'keyCompromise' });
  };
  ...
  const claims = function (iss, sub) {
    ...
    return { iss: iss, sub: sub, aud: AUD, iat: now, exp: now + 120, jti: 'rev-door-' + jti };
  };
  await realms.run(realm, async function () {
    // --- RFC 7523 section 2.1 ---------------------------------------------
    const ISS = 'https://issuer.example.test/rev-grant';
    applications.createApplication({ identifier: 'rev-grant-app', protocols: ['oauth2'] });
    applications.updateApplication('rev-grant-app',
      { attribute: 'oauthAssertionIssuer', mode: 'add', value: ISS });
    const pair = (await pki.issueSigningKeyPair(REALM, { identifier: 'rev-grant-app',
                                                         purpose: 'jwt' })).issued;
    t.check(pair && pair.jwks && pair.jwks.keys[0] && Array.isArray(pair.jwks.keys[0].x5c),
            'a key pair issued from /admin/pki carries its certificate in the JWKS it ' +
            'writes (x5c), which is what the check reads', JSON.stringify(pair.jwks).slice(0, 80));
    applications.updateApplication('rev-grant-app',
      { attribute: 'oauthAssertionJwks', mode: 'set', value: JSON.stringify(pair.jwks) });
    const sign = function (c) {
      return signCompact({ alg: pair.jwsAlg, typ: 'JWT', kid: pair.kid }, c, pair.privateKeyPem);
    };
    const good = await assertionGrant.verify({ assertion: sign(claims(ISS, 'alice')),
                                               audiences: [AUD] });
    t.check(good.ok && good.keyRevocation && good.keyRevocation.status === 'good',
            'RFC 7523 GRANT: an assertion verified by a REGISTERED key reports the key\'s ' +
            'certificate as looked up and good', JSON.stringify(good.keyRevocation || good.description));
    revoke(pair.certificatePem);
    const refused = await assertionGrant.verify({ assertion: sign(claims(ISS, 'alice')),
                                                  audiences: [AUD] });
    t.check(!refused.ok && refused.errorCode === 'STS-PKI-0129' &&
            refused.error === 'invalid_grant' && /REVOKED/.test(refused.description),
            'AND ONCE /admin/pki REVOKES THAT CERTIFICATE, THE SAME REGISTERED KEY NO ' +
            'LONGER VERIFIES A GRANT — invalid_grant, STS-PKI-0129', refused.description);
    // A bare key: the same kind of key with its x5c taken off.
    const barePair = (await pki.issueSigningKeyPair(REALM, { identifier: 'rev-bare-app',
                                                             purpose: 'jwt' })).issued;
    ...
    applications.createApplication({ identifier: 'rev-bare-app', protocols: ['oauth2'] });
    ...
    const bare = await assertionGrant.verify({
      assertion: signCompact({ alg: barePair.jwsAlg, typ: 'JWT', kid: barePair.kid },
    ...
    t.check(bare.ok && bare.keyRevocation && bare.keyRevocation.bare === true &&
            ...
            'A BARE REGISTERED JWK HAS NOTHING TO CHECK, AND THE RESULT SAYS SO — even ' +
            'with its certificate revoked elsewhere, a key registered without it names ' +
            'no issuer', JSON.stringify(bare.keyRevocation || bare.description));

    // --- RFC 7523 section 2.2, client authentication ------------------------
    const clientPair = (await pki.issueSigningKeyPair(REALM, { identifier: 'rev-client',
                                                               purpose: 'jwt' })).issued;
    const clientAssertion = function () {
      return signCompact({ alg: clientPair.jwsAlg, typ: 'JWT', kid: clientPair.kid },
                         claims('rev-client', 'rev-client'), clientPair.privateKeyPem);
    };
    const authOk = await clientAuth.verify({ method: 'private_key_jwt',
      ...
      clientId: 'rev-client', jwks: JSON.stringify(clientPair.jwks), audiences: [AUD] });
    t.check(authOk.ok, 'RFC 7523 CLIENT AUTHENTICATION with a registered key succeeds',
            authOk.description);
    ...
    t.check(!authRefused.ok && authRefused.errorCode === 'STS-PKI-0129',
            'and is refused once the registered key\'s certificate is revoked — the ' +
            'client is answered invalid_client', JSON.stringify(authRefused));

    // --- RFC 7522 ---------------------------------------------------------
    const samlPair = (await pki.issueSigningKeyPair(REALM, { identifier: 'rev-saml',
                                                             purpose: 'saml' })).issued;
    ...
    const samlOk = await samlGrant.verify({ assertion: samlAssertion(), clientId: 'rev-saml',
    ...
    t.check(samlOk.ok && samlOk.certificateRevocation &&
            ...
            'RFC 7522: the registered certificate that verified the assertion is looked ' +
            'up and good', JSON.stringify(samlOk.certificateRevocation || samlOk.description));
    revoke(samlPair.certificatePem);
    const samlRefused = await samlGrant.verify({ assertion: samlAssertion(),
      clientId: 'rev-saml', registeredCertificate: samlPair.certificatePem, audiences: [AUD] });
    ...
    const fedPair = (await pki.issueSigningKeyPair(REALM, { identifier: 'rev-fed',
                                                            purpose: 'saml' })).issued;
    const fakeRes = function () {
      return { statusCode: 200, headersSent: false, body: '',
               status: function (code) { this.statusCode = code; return this; },
               type: function () { return this; }, set: function () { return this; },
               send: function (body) { this.body = body; this.headersSent = true; return this; } };
    };
    const record = { fedId: 'rev-fed-unregistered', fedSigningCertificate: b64Of(fedPair.certificatePem) };
    let proceeded = 0;
    const proceed = function () { proceeded += 1; return 'signed in'; };
    const fedGood = await federationSp.signerStillAccepted({}, fakeRes(), record, proceed);
    t.check(fedGood === 'signed in' && proceeded === 1,
            'FEDERATION: a response verified by a good fedSigningCertificate goes on to ' +
            'the sign-in', JSON.stringify(fedGood));
    revoke(fedPair.certificatePem);
    const res = fakeRes();
    await federationSp.signerStillAccepted({}, res, record, proceed);
    t.check(res.statusCode === 401 && proceeded === 1 && errorCodes.codeOf(res) === 'STS-PKI-0129' &&
            /REVOKED/.test(res.body),
            'and once that certificate is revoked the sign-in is REFUSED with the reason ' +
            'on the page, STS-PKI-0129, and never started', res.statusCode + ' ' +
    ...
    const bareFed = await federationSp.signerStillAccepted({}, fakeRes(), record, proceed,
      { kty: 'RSA', kid: 'partner-bare', n: 'AQAB', e: 'AQAB' });
    t.check(bareFed === 'signed in' && proceeded === 2,
            'while a partner key with no certificate has nothing to check and the ' +
            'sign-in goes on', JSON.stringify(bareFed));

    // --- OID4VP -----------------------------------------------------------
    const vcPair = (await pki.issueSigningKeyPair(REALM, { identifier: 'rev-vc',
                                                           purpose: 'jwt' })).issued;
    const vcGood = await vcVerifier.issuerCertificateRevocation({ ok: true, checks: [],
      issuerCertificatePem: vcPair.certificatePem });
    t.check(vcGood.ok && vcGood.checks.length === 1 && vcGood.checks[0].ok,
            'OID4VP: a trusted issuer certificate that verified a credential is looked up ' +
            'and good, as a check row', JSON.stringify(vcGood.checks));
    revoke(vcPair.certificatePem);
    const vcRefused = await vcVerifier.issuerCertificateRevocation({ ok: true, checks: [],
      issuerCertificatePem: vcPair.certificatePem });
    t.check(!vcRefused.ok && vcRefused.revocationRefused === true && !vcRefused.checks[0].ok,
  ...
  // endpoint CALL them. A grep is the blunt instrument `tests/saml_assertion_grant.js`
  // uses for the same kind of claim: the failure guarded against is a sign-in path
  // added or edited to reach completeSignIn() without the check.
  const fedSource = fs.readFileSync(path.join(ROOT, 'federation', 'federation_sp.js'), 'utf8')
    .split('\n');
  const sites = [];
  ...
  t.check(sites.length === 5 && sites.filter(function (one) { return one.wrapped; }).length === 4 &&
          sites.filter(function (one) { return !one.wrapped; }).every(function (one) {
            return one.opaque;
          }),
          'EVERY FEDERATED SIGN-IN THAT RESTS ON A SIGNATURE REACHES completeSignIn() ' +
          'THROUGH signerStillAccepted() — SAML 2.0 and 1.1, WS-Federation, OIDC and a ' +
          'JWT access token — and the one that does not is the opaque OAuth 2.0 token, ' +
          'where no signature was verified at all', JSON.stringify(sites));
  t.check(/verified: .*jwk: candidates\[i\]|jwk: candidates\[i\]/.test(fedSource.join('\n')) &&
          /\}, verified\.jwk\);/.test(fedSource.join('\n')),
          'and a partner JWT\'s check is handed the key that verified it',
          'verifyForeignJwt() returns jwk');
  const vpSource = fs.readFileSync(path.join(ROOT, 'oid4vc', 'vc_verifier.js'), 'utf8');
  t.check(/verifyPresentation\(presentations\[0\], record\);\s*await issuerCertificateRevocation\(verified\);\s*record\.verdict = \{/
          ...
          'and the OID4VP response endpoint asks before it records a verdict, with the ' +
          'certificate both verifiers report having used', 'vc_verifier.js');
}
...
async function theMainPort(t, minted) {
  ...
  const server = https.createServer({
    cert: tls.serverCertificate().certPem, key: tls.serverCertificate().privateKeyPem,
    ...
    status.annotateRequest(req).then(function () {
      ...
      const shim = { authorized: forwarded ? forwarded.authorized : false,
                     getPeerCertificate: function () { return flat; } };
      ...
      Promise.all([
        ...
        clientAuth.verify({ method: 'self_signed_tls_client_auth', request: req,
                            clientId: 'rev-client',
                            certificateThumbprint: mtls.presentedThumbprint(req) })
      ]).then(function (both) {
  ...
  try {
    const good = await handshake(port, { cert: minted.good.pem + minted.issuing.pem,
                                         key: minted.good.key });
    t.check(good.body.verified === true && good.body.status === 'good',
            'a good foreign client certificate VERIFIES through mtls.peerVerified(), ' +
            'with the annotation app.js makes', JSON.stringify(good.body));
    const bad = await handshake(port, { cert: minted.revoked.pem + minted.issuing.pem,
                                        key: minted.revoked.key });
    t.check(bad.body.verified === false && bad.body.error === 'CERT_REVOKED',
            'a REVOKED one is NOT VERIFIED — which is how the remote XACML PEP and ' +
            'the XACML user chains refuse it without a new question',
            JSON.stringify(bad.body));
    t.check(good.body.forwardedChain >= 1 && good.body.workerStatus === 'good' &&
            bad.body.workerStatus === 'revoked',
            'AND A REQUEST WORKER HANDED THE CERTIFICATE reaches the same verdict: ' +
            'peerOf() now carries the chain above the leaf, without which a worker ' +
            'has no issuer to verify a foreign CRL with',
            JSON.stringify({ good: good.body, bad: bad.body }));
    t.check(good.body.scimScheme === 'clientcert' && bad.body.scimScheme !== 'clientcert',
            'SCIM\'s client-certificate scheme accepts the good certificate and does NOT ' +
            'accept the revoked one as a credential',
            JSON.stringify({ good: good.body.scimScheme, bad: bad.body.scimScheme }));
    t.check(good.body.clientAuthOk === true && bad.body.clientAuthOk === false &&
            bad.body.clientAuthCode === 'STS-PKI-0118',
            'and RFC 8705 client authentication refuses the revoked certificate with the ' +
            'revocation code even though its registered thumbprint MATCHES — a revoked ' +
            'certificate is a client secret its issuer withdrew',
            JSON.stringify({ good: good.body.clientAuthOk, bad: bad.body.clientAuthCode }));
  } finally {
...
async function childBody() {
  const out = {};
  const fixture = await crlServer();
  const root = await issue({ subject: 'CN=rev listener root', ca: true, pathLen: 1 });
  ...
  const good = await issue({ subject: 'CN=rev-listener-good', issuer: asIssuer(issuing),
                             crls: [fixture.base + '/l.crl'] });
  const bad = await issue({ subject: 'CN=rev-listener-bad', issuer: asIssuer(issuing),
                            crls: [fixture.base + '/l.crl'] });
  ...
  const ask = function (port, leaf) {
    return new Promise(function (resolve) {
      const req = https.request({ host: '127.0.0.1', port: port, path: '/tls/whoami',
  ...
  const summary = function (answer) {
    ...
    return { status: answer.status, error: answer.body.error || answer.body.raw,
             ...
             authenticated: auth.authenticated, refused: auth.refusedOnRevocation,
...
function spawnChild(which) {
  ...
  if (!line) {
    return { error: 'the ' + which + ' child produced no result (exit ' +
                    result.status + '): ' + String(result.stderr || '').slice(-1200) };
  }
...
function theListeners(t, got) {
  ...
  t.check(got.strictBad.status === 403 && got.strictBad.authorized === true &&
          ...
          'THE REQUIRED LISTENER ANSWERS 403 for a certificate whose chain VERIFIED ' +
          'and whose issuer\'s CRL revokes it — at the request, because node\'s own ' +
          '`crl` option would refuse every issuer that publishes no list',
          JSON.stringify(got.strictBad));
  t.check(got.strictBad.started === false && got.strictBad.authenticated === false,
          'and starts no session, and reports it as not authenticated',
          JSON.stringify(got.strictBad));
  t.check(got.strictGood.status === 200 && got.strictGood.revocation === 'good' &&
          got.strictGood.authenticated === true,
          'while a good certificate from the same authority is answered 200 and ' +
          'authenticated', JSON.stringify(got.strictGood));
  t.check(got.optionalBad.status === 200 && got.optionalBad.refused === true &&
          got.optionalBad.started === false,
          'THE PERMISSIVE LISTENER still answers 200 — reporting is what it is for — ' +
          'and says REFUSED ON REVOCATION with no session started',
...
function recordingHarness(results) {
  const t = {
    ...
    log: {
      info: function (line) { results.push({ info: line }); },
      warn: function (line) { results.push({ info: 'warning: ' + line }); }
    }
  };
  return t;
}
...
module.exports = {
  ...
  describe: 'revocation CONSULTED: the policy by mode, the register at verifyLeaf ' +
            'and the SPIRE Server API (a hold, a release, a revoked Issuing CA ' +
            'refusing a leaf sent alone), a foreign CRL (good, cached, revoked, ' +
            '404 soft and hard, failure memory, forged, stale, critical extension, ' +
            'timeout, redirect, unverified chain never dialled, no distribution ' +
            'point, an OpenSSL CRL), the main port\'s doors and a worker\'s view ' +
            'over a real handshake, 9443\'s 403, and against OpenSSL-signed documents: ' +
            'OCSP (good, revoked by a delegated responder, unknown, the wrong key, no ' +
            'EKU, stale, replayed, one request for two callers, the order setting), ' +
            'delta CRLs (revokes, removes, a mismatched base, an unreachable delta), ' +
            'indirect CRLs (a second CA revoking, entry attribution, an impostor ' +
            'signer, a signer the certificate does not name) and the issuing ' +
            'distribution point\'s scope and reasons; and a delegated responder\'s own ' +
            'status, a list\'s signer from its caIssuers address, ldap and ldaps against ' +
            'a real directory, the two unreached delta checks, this service\'s own ' +
            'authority as an indirect-CRL signer, and a REGISTERED certificate at every ' +
            'door that uses one',
  run: run
};
