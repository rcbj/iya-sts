...
function search(boundDn, base, filter, attributes, scope) {
  return ldap.performOperation('search', {
    ...
    filter: filter || '(objectclass=*)', attributes: attributes || [], sizeLimit: 0
  });
}

function compare(boundDn, dn, attribute, value) {
  return ldap.performOperation('compare', {
    dn: dn, boundDn: boundDn, channel: 'ldaps', attribute: attribute, value: value
  });
}
...
function run(t) {
  ...
  try {
    ...
    t.check(set && set.ok !== false, 'precondition: the probe person has a password',
            JSON.stringify(set));
    ...
    t.check(devRead.ok === true && devRead.entries.length >= 1 &&
            ...
            'in development an unbound search may filter on and read userPassword — ' +
            'which is what makes the product-mode zero below a refusal',
            JSON.stringify({ ok: devRead.ok, entries: devRead.entries.length }));

    product(function () {
      // ---------------------------------------------------------------------
      t.log.info('=== 1. the bind ===');
      refused(t, bind('', ''), 'InappropriateAuthenticationError', 'STS-LDAP-0070',
              '(anonymous)', 'an anonymous bind');
      refused(t, bind(personDn, PASSWORD, 'ldap'), 'ConfidentialityRequiredError',
              'STS-LDAP-0071', personDn,
              'a correct password on the plain listener, refused before it is read');
      refused(t, bind(personDn, ''), 'UnwillingToPerformError', 'STS-LDAP-0072',
              personDn, 'a DN with an empty password (an unauthenticated bind)');
      ...
      withSettings({ 'security.rateLimitPerIdentity': 3,
        ...
        for (let i = 0; i < 3; i++) {
          ...
          t.check(wrong.ok === false && wrong.errorName === 'InvalidCredentialsError',
                  'wrong password ' + (i + 1) + ' of 3 is invalidCredentials',
                  JSON.stringify(wrong));
        }
        refused(t, bind(personDn, PASSWORD), 'UnwillingToPerformError', 'STS-LDAP-0073',
                personDn, 'the CORRECT password once the DN is over its limit');
        t.check(bind(personDn, PASSWORD, 'ldaps', '198.51.100.9').ok === false,
                'and from another address too, since the DN bucket is what is full');
      });
      websecurity.reset();
      withSettings({ 'security.rateLimitPerIdentity': 50,
        ...
        refused(t, bind(personDn, PASSWORD), 'UnwillingToPerformError', 'STS-LDAP-0073',
                personDn, 'the address is now blocked — the success did not reset it, ' +
                'or one working password would be a way to keep guessing everybody else\'s');
      ...
      refused(t, search('', usersDn), 'InsufficientAccessRightsError', 'STS-LDAP-0074',
              usersDn, 'an unbound search');
      refused(t, compare('', personDn, 'mail', 'x'), 'InsufficientAccessRightsError',
              'STS-LDAP-0074', personDn, 'an unbound compare');
      const nowhere = 'uid=nobody-' + STAMP + ',' + usersDn;
      refused(t, search('', nowhere), 'InsufficientAccessRightsError', 'STS-LDAP-0074',
              nowhere, 'an unbound search of an entry that does not exist is refused ' +
              'the same way, so the refusal says nothing about the tree');
      ...
      t.check(rootDse.ok === true && rootDse.entries.length === 1,
              ...
              JSON.stringify({ ok: rootDse.ok, entries: (rootDse.entries || []).length }));
      ...
      // ---------------------------------------------------------------------
      t.log.info('=== 4. credentials are withheld, from the result, the filter and compare ===');
      const asked = search(personDn, usersDn, '(uid=' + PERSON + ')',
                           ['userPassword', 'pwdHistory', '*']);
      const names = namesReturned(asked);
      t.check(asked.ok === true && asked.entries.length === 1 && !names.userpassword &&
              !names.pwdhistory,
              'userPassword and pwdHistory are not returned even when asked for by name',
      ...
      t.check(presence.ok === true && presence.entries.length === 0,
              'a filter cannot see userPassword: (userPassword=*) matches nobody',
              JSON.stringify({ entries: (presence.entries || []).length }));
      const substring = search(personDn, usersDn,
                               '(&(uid=' + PERSON + ')(userPassword=$scrypt*))');
      t.check(substring.ok === true && substring.entries.length === 0,
              'nor can a substring filter, which is the one-character-at-a-time oracle',
      ...
      t.check(granted, 'precondition: the probe administrator holds Admin Write',
              JSON.stringify(grant));
      const adminRead = search(adminDn, usersDn, '(uid=' + PERSON + ')', ['*', 'userPassword']);
      t.check(adminRead.ok === true && adminRead.entries.length === 1 &&
              !namesReturned(adminRead).userpassword,
              'an administrator is not excepted: no userPassword for them either',
              JSON.stringify(Object.keys(namesReturned(adminRead))));

      // ---------------------------------------------------------------------
      t.log.info('=== 5. operational attributes are the directory\'s to write ===');
      const stamp = ldap.performOperation('modify', {
        dn: personDn, boundDn: adminDn, channel: 'ldaps',
        changes: [{ operation: 'replace',
                    modification: { type: 'createTimestamp', values: ['19700101000000Z'] } }]
      });
    ...
    const shape = ldap.operationRequest('bind', {
      ...
      connection: { encrypted: true, remoteAddress: ADDRESS, ldap: { bindDN: 'cn=anonymous' } }
    });
    t.equal(shape.remoteAddress, ADDRESS,
            'operationRequest() carries remoteAddress, or every dispatched bind shares one bucket');
  } finally {
    websecurity.reset();
    if (granted) {
    ...
    made.forEach(function (name) {
      ...
      if (view && view.entry) {
        ldap.performOperation('del', { dn: view.entry.dn, boundDn: '', channel: 'ldaps' });
      }
    });
  }
}
...
module.exports = {
  ...
  describe: 'product mode: LDAP binds refused anonymous, in the clear and past a rate limit; ' +
            'reads need a bind; credentials never leave; operational attributes read-only',
  run: run
};
