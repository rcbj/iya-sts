...
function fakeReq(method, path, query, body, cookie) {
  ...
  return {
    ...
    get: function (name) { return headers[String(name).toLowerCase()]; }
  };
}
...
function fakeRes() {
  ...
  res.type = function (t) { res.headers['content-type'] = t; return res; };
  res.set = function (k, v) { res.headers[String(k).toLowerCase()] = v; return res; };
  res.setHeader = res.set;
  res.getHeader = function (k) { return res.headers[String(k).toLowerCase()]; };
  res.send = function (b) { res.body = String(b); return res; };
  res.redirect = function (code, url) { res.statusCode = code; res.location = url; return res; };
  return res;
}
...
function soapRequest(inner) {
  return '<soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/"><soap:Body>' +
    ...
    'xmlns:saml="urn:oasis:names:tc:SAML:1.0:assertion" MajorVersion="1" MinorVersion="1" ' +
    'RequestID="_q1" IssueInstant="' + new Date().toISOString() + '">' + inner +
...
function rst(inner, security) {
  return '<s:Envelope xmlns:s="http://www.w3.org/2003/05/soap-envelope" ' +
    ...
    '<s:Header>' + (security ? '<wsse:Security>' + security + '</wsse:Security>' : '') + '</s:Header>' +
...
function run(t) {
  ...
  t.equal(read({ amr: ['pwd'], acr: '1' }).saml2, PPT,
          'an ordinary password sign-in is PasswordProtectedTransport, exactly as before');
  t.equal(read({ amr: ['pwd'], acr: '1' }).saml11, 'urn:oasis:names:tc:SAML:1.0:am:password',
  ...
  t.equal(read({ amr: ['swk'], acr: '1' }).saml2,
          ...
          'A TLS CLIENT CERTIFICATE IS TLSClient AND NOT A PASSWORD — the defect, in both modes');
  ...
  t.equal(read({ amr: ['pwd'], acr: '1', via: 'Kerberos v5 (SPNEGO)' }).saml2,
          ...
          'A KERBEROS TICKET IS Kerberos even when its KDC pre-authentication put pwd in the amr');
  t.equal(read({ amr: [], acr: '0', authenticated: false }).saml2,
          'urn:oasis:names:tc:SAML:2.0:ac:classes:unspecified',
          'THE UNAUTHENTICATED SESSION IS unspecified — it used to be signed as a password');
  t.equal(read({ amr: [], acr: '' }).saml11, 'urn:oasis:names:tc:SAML:1.0:am:unspecified',
          'an amr naming nothing is unspecified');
  t.equal(read({ amr: ['federated'], acr: PPT }).saml2, PPT,
          'a federated session carries the SAML 2.0 class the PARTNER asserted');
  t.equal(read({ amr: ['federated'], acr: PPT }).saml11, 'urn:oasis:names:tc:SAML:1.0:am:password',
          'translated into SAML 1.1 where the two mean the same');
  t.equal(read({ amr: ['federated'], acr: 'urn:oasis:names:tc:SAML:1.0:am:password' }).saml2, PPT,
          'and a SAML 1.1 partner\'s method is translated into SAML 2.0');
  t.equal(read({ amr: ['federated', 'pwd', 'hwk'], acr: '' }).multiFactor, true,
          'an OpenID partner\'s amr behind `federated` is read as its statement');
  ...
  const spec = function (requested, registered) {
    return { requested: requested, registered: registered, fallback: 'https://idp.test/saml2/sp',
             attribute: 'samlAssertionConsumerService', parameter: 'AssertionConsumerServiceURL',
             application: 'sp-1' };
  };
  withMode(config, 'development', function () {
    t.equal(returnAddress.resolve(spec('https://anywhere.test/acs', [])).url,
            'https://anywhere.test/acs', 'development: an unregistered address is used');
    t.equal(returnAddress.resolve(spec('', ['https://a/1', 'https://a/2'])).url, 'https://a/2',
            'development: with none named, the LAST registered value, as both SAML modules took');
    t.equal(returnAddress.resolve(spec('', [])).url, 'https://idp.test/saml2/sp',
            'development: with nothing at all, the built-in mock');
  });
  withMode(config, 'product', function () {
    const refused = returnAddress.resolve(spec('https://evil.test/acs', ['https://sp.test/acs']));
    t.equal(refused.ok, false, 'PRODUCT: an address not registered is refused');
    t.check(/samlAssertionConsumerService/.test(refused.why) && /PRODUCT/.test(refused.why),
            'and the refusal names the attribute to register it on', refused.why);
    t.equal(returnAddress.resolve(spec('https://sp.test/acs/../x', ['https://sp.test/acs'])).ok,
            false, 'the comparison is exact, not a prefix');
    t.equal(returnAddress.resolve(spec('https://sp.test/acs', ['https://sp.test/acs'])).ok, true,
            'a registered address is delivered to');
    t.equal(returnAddress.resolve(spec('', ['https://sp.test/acs'])).url, 'https://sp.test/acs',
            'with none named, the registered one');
    t.equal(returnAddress.resolve(spec('', [])).ok, false,
            'PRODUCT: with nothing registered there is NO fallback to the built-in mock');
  });

  // -------------------------------------------------------------------------
  t.log.info('C. the configured signature algorithm reaches a verifying signature');
  // -------------------------------------------------------------------------
  try {
    ...
    t.equal(verdict.ok, true, 'an assertion signed with rsa-sha384 verifies', verdict.why);
    t.check(/rsa-sha384/.test(verdict.signatureMethod),
            'and its SignatureMethod is the configured one', verdict.signatureMethod);
    const s11 = saml11.buildSaml11Assertion({ subject: 'alice', audience: 'rp' });
    const v11 = stsCrypto.verifyXmlSignature(s11, { element: 'Assertion',
                                                    certPem: helpers.STS.certPem });
    t.check(v11.ok && /rsa-sha384/.test(v11.signatureMethod),
            'the SAML 1.1 builder reads the same setting', v11.signatureMethod + ' ' + v11.why);
  } finally {
    config.clearOverride('saml.signatureAlgorithm');
  }
  t.equal(documentSettings.signatureOptions().sigAlg, stsCrypto.SIG_RSA_SHA256,
          'the default is RSA-SHA256, which is what every signer used before the setting');
  try {
  ...
  t.check(/<md:OrganizationName xml:lang="en">sts</.test(
          ...
          'and the default is the product name, sts (mock-sts until 2026-09-12)');
  ...
  t.check(/AuthnContextClassRef>urn:oasis:names:tc:SAML:2\.0:ac:classes:unspecified</.test(
          ...
          'a SAML 2.0 caller that names no class gets unspecified, not PasswordProtectedTransport');
  const noStatement = saml11.buildSaml11Assertion({
    subject: 'alice', audience: 'rp', authenticationStatement: false, sign: false,
    attributes: [{ namespace: 'urn:x', name: 'uid', value: 'alice' }] });
  t.check(!/AuthenticationStatement/.test(noStatement) && /AttributeStatement/.test(noStatement),
          'authenticationStatement: false leaves the AuthenticationStatement out', noStatement);
  withMode(config, 'product', function () {
    const person = personAttributes.personFor(helpers.userFor('nobody-here'));
    t.check(!('email' in person) || !person.email,
            'product: a person with no directory entry gets no invented mail address');
    const rows = personAttributes.withoutAbsent([{ name: 'mail', value: undefined },
                                                 { name: 'uid', value: 'x' }]);
    t.equal(rows.length, 1, 'and an attribute with no value is omitted rather than signed');
  });
  ...
  t.equal(devAnon.status, 200, 'development: a request with no credential still issues');
  t.check(/ac:classes:unspecified/.test(devAnon.body),
          'and the assertion no longer claims a password sign-in for it', devAnon.body.slice(0, 200));
  const prodAnon = withMode(config, 'product', function () {
    return wstrust.handleRst(rst(''), 'application/soap+xml');
  });
  t.check(prodAnon.status >= 400 && /No credential was presented/.test(prodAnon.body),
          'PRODUCT: a request with no credential is a Fault naming what to present',
  ...
  t.equal(prodAssertion.status, 200,
          'PRODUCT: an assertion THIS STS signed is accepted as a credential', prodAssertion.body.slice(0, 300));
  t.check(/PreviousSession/.test(prodAssertion.body),
          'and the token it issues says PreviousSession, not a password');
  const forged = saml2.buildSamlAssertion('carol', 'https://rp.test', 5, { sign: false });
  ...
  t.check(prodForged.status >= 400 && /does not verify/.test(prodForged.body),
          'PRODUCT: an unsigned assertion is refused rather than believed', prodForged.body.slice(0, 300));
  const devForged = withMode(config, 'development', function () {
    return wstrust.handleRst(rst('', forged), 'application/soap+xml');
  });
  t.equal(devForged.status, 200, 'development: the same assertion is still believed');
  ...
  t.check(prodObo.status >= 400 && /presents no credential of its own/.test(prodObo.body),
          'PRODUCT: a delegation with no requester credential is refused', prodObo.body.slice(0, 300));
  const devObo = withMode(config, 'development', function () {
    return wstrust.handleRst(rst(obo), 'application/soap+xml');
  });
  t.equal(devObo.status, 200, 'development: an anonymous delegation still issues');
  t.equal(devObo.signIn, null,
          'BUT NO SESSION IS STARTED FOR THE DELEGATED SUBJECT — in either mode');

  const yearLong = '<wst:Lifetime xmlns:wsu="http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-utility-1.0.xsd">' +
    '<wsu:Created>2026-01-01T00:00:00Z</wsu:Created><wsu:Expires>2027-01-01T00:00:00Z</wsu:Expires></wst:Lifetime>';
  ...
  const minutes = expires ? (Date.parse(expires[1]) - Date.now()) / 60000 : Infinity;
  t.check(minutes <= Number(config.value('wstrust.maxTokenLifetimeMin')) + 1,
          'a year-long wst:Lifetime is CLAMPED to wstrust.maxTokenLifetimeMin, in development too',
  ...
  try {
    ...
    const jwtRst = withMode(config, 'development', function () {
      return wstrust.handleRst(rst('<wst:TokenType>urn:ietf:params:oauth:token-type:jwt</wst:TokenType>'),
    ...
    t.equal(header.alg, 'ES256', 'the WS-Trust JWT is signed with wstrust.jwtAlgorithm');
  ...
  const devNoCert = withMode(config, 'development', function () {
    return wstrust.handleRst(rst(''), 'application/soap+xml', { encrypt: true });
  });
  t.equal(devNoCert.status, 200,
          'development: ?encrypt=1 with no recipient certificate still returns the plaintext');
  const prodNoCert = withMode(config, 'product', function () {
    return wstrust.handleRst(rst('', saml2.buildSamlAssertion('dan', 'x', 5)).replace(
      /<ds:X509Certificate>[^<]*<\/ds:X509Certificate>/g, ''), 'application/soap+xml', { encrypt: true });
  });
  t.check(prodNoCert.status >= 400,
          'PRODUCT: ?encrypt=1 that cannot encrypt is a Fault, not a plaintext assertion',
  ...
  const assertionWith = function (audiences) {
    const restriction = audiences === null ? '' : audiences.map(function (group) {
    ...
    return new DOMParser().parseFromString(
      '<saml:Assertion xmlns:saml="urn:oasis:names:tc:SAML:2.0:assertion"><saml:Conditions>' +
      restriction + '</saml:Conditions></saml:Assertion>', 'text/xml').documentElement;
  };
  t.equal(fedSp.audienceCheck(assertionWith([[ours]]), 'https://sp.test', record, true).ok, true,
          'an assertion addressed to this service is accepted');
  const other = fedSp.audienceCheck(assertionWith([['https://another-sp.test']]),
                                    'https://sp.test', record, true);
  t.equal(other.ok, false,
          'AN ASSERTION ADDRESSED TO ANOTHER SERVICE PROVIDER IS REFUSED — it was a warning');
  t.check(/fedLocalEntityId/.test(other.why), 'and the refusal names the way out', other.why);
  t.equal(fedSp.audienceCheck(assertionWith([['https://x.test', ours]]), 'https://sp.test',
                              record, true).ok, true,
          'several audiences in one restriction are an OR');
  t.equal(fedSp.audienceCheck(assertionWith([[ours], ['https://x.test']]), 'https://sp.test',
                              record, true).ok, false,
          'several restrictions are an AND');
  t.equal(fedSp.audienceCheck(assertionWith(null), 'https://sp.test', record, true).ok, false,
          'SAML 2.0 with no audience restriction is refused (the profile requires one)');
  t.equal(fedSp.audienceCheck(assertionWith(null), 'https://sp.test', record, false).ok, true,
          'SAML 1.1 / WS-Federation with none is accepted with a warning');
  t.equal(fedSp.ourEntityId('https://sp.test', { fedId: 'p1', fedLocalEntityId: 'urn:us' }),
          'urn:us', 'fedLocalEntityId pins what this service is called to a partner');
  t.equal(fedSp.federatedAmr(['pwd', 'hwk']).join(','), 'federated,pwd,hwk',
          'a partner\'s amr is carried BEHIND federated instead of thrown away');
  ...
  try {
    ...
    t.equal(fedSp.familyAlgorithms('RSA').join(','), 'RS256',
            'federation.jwtAlgorithms narrows the key\'s family and cannot add HS256 to it');
  } finally {
    config.clearOverride('federation.jwtAlgorithms');
  }
  const noPeer = federation.readinessOf({ fedRole: 'service-provider', fedProtocol: 'saml2',
                                          fedSsoUrl: 'https://p/sso',
                                          fedSigningCertificate: 'MII' });
  t.check(noPeer.missing.indexOf('fedPeer') >= 0,
          'A SAML 2.0 RELATIONSHIP WITH NO fedPeer IS NOT FULLY CONFIGURED, in every mode',
          JSON.stringify(noPeer.missing));
  ['saml11', 'wsfed', 'oidc', 'oauth2'].forEach(function (protocol) {
    const r = federation.readinessOf({ fedRole: 'service-provider', fedProtocol: protocol });
  ...
  const attributeQuery = '<samlp:AttributeQuery Resource="urn:rp:test"><saml:Subject>' +
    '<saml:NameIdentifier>erin</saml:NameIdentifier></saml:Subject></samlp:AttributeQuery>';
  const devAttr = withMode(config, 'development', function () { return ask(attributeQuery); });
  t.check(/samlp:Success/.test(devAttr.body) && /AttributeStatement/.test(devAttr.body),
          'development: an AttributeQuery is still answered with its attributes');
  t.check(!/AuthenticationStatement/.test(devAttr.body),
          'BUT CLAIMS NO AUTHENTICATION — it used to carry am:password at the instant of the query');
  const authQuery = function (name) {
    return '<samlp:AuthenticationQuery><saml:Subject><saml:NameIdentifier>' + name +
  ...
  t.check(/samlp:Success/.test(devNobody.body) && !/<saml:Assertion/.test(devNobody.body) &&
          /no authentication is recorded/.test(devNobody.body),
          'AN AUTHENTICATION QUERY ABOUT SOMEBODY WHO NEVER SIGNED IN RETURNS NO ASSERTION, in ' +
          'either mode', devNobody.body.slice(0, 400));
  const who = 'aq-' + Date.now();
  authn.startSession(fakeRes(), who, ['swk'], '1', 'a client certificate', {});
  const devReal = withMode(config, 'development', function () { return ask(authQuery(who)); });
  ...
  const prodAttr = withMode(config, 'product', function () { return ask(attributeQuery); });
  t.check(/samlp:Requester/.test(prodAttr.body) && /PRODUCT/.test(prodAttr.body),
          'PRODUCT: an unauthenticated AttributeQuery is refused', prodAttr.body.slice(0, 400));

  // -------------------------------------------------------------------------
  t.log.info('H. the browser profiles refuse an unregistered address in product');
  // -------------------------------------------------------------------------
  ...
  // whenever a directory is loaded in this process (it is, in a full `npm test`).
  const callSso = function (issuer) {
    ...
    const samlRequest = zlib.deflateRawSync(Buffer.from(authnRequest)).toString('base64');
    const res = fakeRes();
  ...
  t.equal(devSso.statusCode, 303,
          'development: an unregistered AssertionConsumerServiceURL goes on to the sign-in screen');
  ...
  t.check(prodSso.statusCode === 400 && /not registered/.test(prodSso.body),
          'PRODUCT: the same AuthnRequest is refused on a page, and no assertion goes anywhere',
  ...
  const callWsfed = function (realm) {
    ...
    passive(fakeReq('GET', '/wsfed', { wa: 'wsignin1.0', wtrealm: realm,
                                       wreply: 'https://attacker.test/reply' }), res);
    return res;
  };
  t.equal(withMode(config, 'development', function () {
    return callWsfed('urn:dev:rp:' + stamp);
  }).statusCode, 303, 'development: an unregistered wreply goes on to the sign-in screen');
  ...
  try {
    ...
    t.equal(withMode(config, 'development', function () { return saml2sso.idpEntityIdFor(''); }),
            'urn:sts:idp', 'development: an empty saml2.entityId still falls back');
    ...
    t.check(prodMeta.statusCode === 503 && /saml2\.entityId/.test(prodMeta.body),
            'PRODUCT: an empty saml2.entityId publishes no metadata under an invented name',
  ...
  t.check(!/Always &quot;Mock&quot;|Always "Mock"|@sts\.example(?!\.com)/.test(productClaims),
          'PRODUCT: the signed WS-Federation metadata does not describe invented values');
  t.check(/Always "Mock"|Always &quot;Mock&quot;/.test(wsfed.federationMetadata('https://idp.test')),
  ...
  return outboundDone.then(function (answer) {
    t.check(answer.ok === false && /federation\.outbound/.test(answer.why),
            'SP metadata is not fetched when federation.outbound is off', answer.why);
  });
}

module.exports = {
  name: 'saml_family_hardcoded',
  describe: 'the SAML, WS-Trust, WS-Federation and federation half of the 2026-09-12 ' +
            'hard-coded-values sweep: product-mode refusals, and the defects fixed in both modes',
  run: run
};
