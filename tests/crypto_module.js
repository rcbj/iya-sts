...
const log = require('bunyan').createLogger({ name: 'crypto_module',
...
const DOCUMENTS = [
  { name: 'SAML 2.0 Assertion (was saml2.js)',
    ...
    xml: '<saml:Assertion xmlns:saml="' + NS_SAML2 + '" ID="_a2" ' +
         'Version="2.0"><saml:Issuer>sts</saml:Issuer><saml:Subject/>' +
         '</saml:Assertion>' },
  { name: 'SAML 2.0 Response (was saml2_sso.js)',
    root: 'Response', idAttr: 'ID', id: '_r2',
    placement: crypto.PLACEMENT.AFTER_ISSUER,
    xml: '<samlp:Response xmlns:samlp="' + NS_SAMLP2 + '" xmlns:saml="' +
         NS_SAML2 + '" ID="_r2"><saml:Issuer>sts</saml:Issuer><samlp:Status/>' +
         '</samlp:Response>' },
  { name: 'SAML 2.0 metadata (was saml2_sso.js)',
    root: 'EntityDescriptor', idAttr: 'ID', id: '_m2',
    placement: crypto.PLACEMENT.FIRST,
    xml: '<md:EntityDescriptor xmlns:md="' + NS_MD + '" ID="_m2" ' +
         'entityID="urn:sts"><md:IDPSSODescriptor/></md:EntityDescriptor>' },
  { name: 'SAML 1.1 Assertion (was saml11.js)',
    root: 'Assertion', idAttr: 'AssertionID', id: '_a1',
    placement: crypto.PLACEMENT.LAST,
    xml: '<Assertion xmlns="urn:oasis:names:tc:SAML:1.0:assertion" ' +
         'AssertionID="_a1" Issuer="sts" MajorVersion="1" ' +
         'MinorVersion="1"><Conditions/></Assertion>' },
  { name: 'SAML 1.1 Response (was saml11_sso.js)',
    root: 'Response', idAttr: 'ResponseID', id: '_r1',
    placement: crypto.PLACEMENT.FIRST,
    xml: '<samlp:Response xmlns:samlp="urn:oasis:names:tc:SAML:1.0:protocol" ' +
         'ResponseID="_r1"><samlp:Status/></samlp:Response>' },
  { name: 'WS-Federation metadata (was wsfed.js)',
    root: 'EntityDescriptor', idAttr: 'ID', id: '_wf',
    placement: crypto.PLACEMENT.FIRST,
    xml: '<EntityDescriptor xmlns="' + NS_MD + '" ID="_wf" ' +
         'entityID="urn:sts"><RoleDescriptor/></EntityDescriptor>' },
  { name: 'SAML 2.0 AuthnRequest (was federation_sp.js)',
    root: 'AuthnRequest', idAttr: 'ID', id: '_q1',
    placement: crypto.PLACEMENT.AFTER_ISSUER,
    xml: '<samlp:AuthnRequest xmlns:samlp="' + NS_SAMLP2 + '" xmlns:saml="' +
         NS_SAML2 + '" ' +
         'ID="_q1"><saml:Issuer>sts</saml:Issuer><samlp:NameIDPolicy ' +
...
function signTheOldWay(doc, keys) {
  log.debug("Entering signTheOldWay().");
  ...
  const where = doc.placement === crypto.PLACEMENT.FIRST
    ...
    : (doc.placement === crypto.PLACEMENT.LAST
      ...
      : { reference: "/*[local-name(.)='" + doc.root +
                     "']/*[local-name(.)='Issuer']",
          action: 'after' });
  sig.computeSignature(doc.xml, { location: where });
  log.debug("Leaving signTheOldWay().");
...
function verifiesUnderXmlCrypto(signed, doc, keys) {
  log.debug("Entering verifiesUnderXmlCrypto().");
  ...
  if (!sigEl) {
    log.debug("Leaving verifiesUnderXmlCrypto().");
  ...
  try {
    log.debug("Leaving verifiesUnderXmlCrypto().");
    return { ok: !!check.checkSignature(signed), why: 'the signature did not ' +
                                                      'verify' };
  } catch (e) {
    log.debug("Leaving verifiesUnderXmlCrypto().");
...
module.exports = {
  name: 'crypto_module',
  describe: 'common/crypto.js: one signer, one verifier, one cipher — ' +
            'checked against xml-crypto in both directions',
  ...
  run: function (t) {
    ...
    const keys = crypto.selfSignedRsaCertificate(
        { commonName: 'sts-test', serialNumber: '02' });
    const other = crypto.selfSignedRsaCertificate(
        { commonName: 'somebody-else' });

    // -----------------------------------------------------------------------
    t.log.info('A. every document this service signs is accepted by ' +
               'xml-crypto');
    // -----------------------------------------------------------------------
    DOCUMENTS.forEach(function (doc) {
      ...
      t.check(verdict.ok, doc.name + ' verifies under an independent ' +
                                     'implementation',
      ...
      t.check(!/Id="_0"/.test(signed),
              '  and carries no invented Id="_0" — the attribute the schema ' +
              'does not have');
    ...
    DOCUMENTS.forEach(function (doc) {
      ...
      const result = crypto.verifyXmlSignature(signed,
                                               { element: doc.root,
                                                 certPem: keys.certPem });
      t.check(result.ok, doc.name + ' signed by xml-crypto is accepted',
              result.why);
    });

    // -----------------------------------------------------------------------
    t.log.info('C. the refusals — a verifier that only ever says yes guards ' +
               'nothing');
    // -----------------------------------------------------------------------
    const doc2 = DOCUMENTS[0];
    const signed2 = crypto.signXml(doc2.xml, {
      privateKeyPem: keys.privateKeyPem, certPem: keys.certPem,
      placement: doc2.placement });

    const wrongKey = crypto.verifyXmlSignature(signed2,
                                               { element: 'Assertion',
                                                 certPem: other.certPem });
    t.check(!wrongKey.ok, 'a signature made by a different key is refused',
            wrongKey.why);
    t.check(wrongKey.present, '  and is reported as PRESENT rather than ' +
                              'missing — a different fact');

    const tampered = crypto.verifyXmlSignature(
      signed2.replace('<saml:Subject/>',
                      '<saml:Subject><saml:NameID>mallory</saml:NameID>' +
                      '</saml:Subject>'),
    ...
    t.check(tampered.signatureValid && !tampered.referencesValid,
            '  and the reason distinguishes a good signature over changed ' +
            'content from a bad signature');

    const unsigned = crypto.verifyXmlSignature(doc2.xml,
                                               { element: 'Assertion',
                                                 certPem: keys.certPem });
    t.check(!unsigned.ok && !unsigned.present, 'an unsigned element is ' +
                                               'refused as NOT PRESENT');

    const absent = crypto.verifyXmlSignature(signed2,
                                             { element: 'LogoutRequest',
                                               certPem: keys.certPem });
    t.check(!absent.ok && !absent.present, 'an element that is not in the ' +
                                           'document is refused');
    t.check(/no <LogoutRequest> at all/.test(absent.why || ''),
            '  and says the element is absent rather than that it is unsigned',
            absent.why);

    // -----------------------------------------------------------------------
    t.log.info('D. TWO SIGNATURES IN ONE DOCUMENT — the case that was wrong ' +
               'before');
    // -----------------------------------------------------------------------
    ...
    const innerAssertion = crypto.signXml(
      ...
      'AssertionID="_inner" Issuer="sts"><Conditions/></Assertion>',
      { privateKeyPem: keys.privateKeyPem, certPem: keys.certPem,
        placement: crypto.PLACEMENT.LAST })
      .replace(/^<\?xml[^>]*\?>/, '');
    const bothSigned = crypto.signXml(
      '<samlp:Response xmlns:samlp="urn:oasis:names:tc:SAML:1.0:protocol" ' +
      'ResponseID="_outer"><samlp:Status/>' + innerAssertion +
      '</samlp:Response>',
      { privateKeyPem: keys.privateKeyPem, certPem: keys.certPem,
    ...
    const outer = crypto.verifyXmlSignature(bothSigned,
                                            { element: 'Response',
                                              certPem: keys.certPem });
    const inner = crypto.verifyXmlSignature(bothSigned,
                                            { element: 'Assertion',
                                              certPem: keys.certPem });
    t.check(outer.ok, 'the RESPONSE signature verifies', outer.why);
    t.equal(outer.referenceUri, '#_outer', '  and it is the one referencing ' +
                                           'the ResponseID');
    t.check(inner.ok, 'the ASSERTION signature verifies', inner.why);
    t.equal(inner.referenceUri, '#_inner', '  and it is the one referencing ' +
                                           'the AssertionID');
    t.check(outer.referenceUri !== inner.referenceUri,
            'THE TWO ANSWERS ARE ABOUT DIFFERENT ELEMENTS — the whole point ' +
            'of naming one');

    // What a first-signature verifier concludes about this same document: ONE
    // answer, whichever element was asked about. Asserted rather than
    // described, so that a future "simplification" back to that shape goes red
    // here.
    const naive = crypto.xmldsig.verifyXml(bothSigned,
                                           { certPem: keys.certPem });
    ...
    t.check(naive.valid, 'a first-signature verifier reports valid=true on ' +
                         'this document');
    t.equal(naive.references[0].uri, inner.referenceUri,
            '  and its one answer is always about the assertion, whatever ' +
            'was asked');
    t.check(naive.references[0].uri !== outer.referenceUri,
            'SO ASKING IT ABOUT THE RESPONSE GETS AN ANSWER ABOUT THE ' +
            'ASSERTION — the defect naming the element removes',
            'the Response references ' + outer.referenceUri + '; it answered ' +
                'about ' +
    ...
    const wrapVerdict = crypto.verifyXmlSignature(wrapped,
                                                  { element: 'Response',
                                                    certPem: keys.certPem });
    t.check(!wrapVerdict.ok,
            'A SIGNATURE REFERENCING A DIFFERENT ELEMENT IS REFUSED ' +
            '(signature wrapping)',
            wrapVerdict.why);
    t.check(/references "#_inner"/.test(wrapVerdict.why || ''),
            '  and the refusal names what it referenced instead',
            wrapVerdict.why);

    // -----------------------------------------------------------------------
    t.log.info('E. XML encryption round-trips over every algorithm offered');
    // -----------------------------------------------------------------------
    const plain = '<saml:Assertion xmlns:saml="' + NS_SAML2 + '" ' +
        'ID="_e1"><saml:Subject/></saml:Assertion>';
    ...
    t.check(!wrongCert.ok, 'an element encrypted to a DIFFERENT certificate ' +
                           'is refused',
    ...
    ['A128GCM', 'A256GCM'].forEach(function (enc) {
      const compact = crypto.encryptJweCompact('{"probe":true}',
                                               { jwk: jwk, enc: enc });
      t.equal(compact.split('.').length, 5, enc + ' produces a five-part ' +
                                                  'compact JWE');
      const back = crypto.decryptJweCompact(compact,
        { privateKey: pair.privateKey, allowedEnc: ['A128GCM', 'A256GCM'],
          expectedKid: 'k1' });
    ...
    const refusals = [
      ...
      ['an enc this endpoint never offered',
       { privateKey: pair.privateKey, allowedEnc: ['A128GCM'] }],
    ...
    refusals.forEach(function (row, i) {
        ...
        const text = i === 2
          ? (function () {
              ...
              const header = JSON.parse(Buffer.from(parts[0], 'base64url')
                                              .toString('utf8'));
              header.zip = 'DEF';
              parts[0] = Buffer.from(JSON.stringify(header), 'utf8')
                               .toString('base64url');
              return parts.join('.');
            })()
          : compact;
      ...
      t.check(!!threw, 'a JWE with ' + row[0] + ' is refused',
              threw.slice(0, 80));
    });
    ...
    try {
      ...
      crypto.decryptJweCompact(parts.join('.'),
    ...
    // -----------------------------------------------------------------------
    t.log.info('G. the thumbprints agree with the three implementations they ' +
               'replaced');
    // -----------------------------------------------------------------------
    ...
    // shared one builds the canonical JSON from an ORDERED LIST, so the
    // ordering is a property of the code rather than of how somebody typed an
    // object.
    ...
    t.equal(crypto.jwkThumbprint(jwk), legacyRsa,
            'the RFC 7638 thumbprint matches what the two JSON.stringify ' +
            'copies produced');
    ...
    t.equal(crypto.jwkThumbprint(Object.assign({}, jwk,
            ...
            'a key carrying kid/alg/use/key_ops hashes the same — the DPoP ' +
    ...
    t.equal(crypto.jwkThumbprint(ec), nodeCrypto.createHash('sha256')
              .update(JSON.stringify({ crv: ec.crv, kty: ec.kty, x: ec.x,
                                       y: ec.y }))
              .digest('base64url'),
    ...
    t.equal(crypto.certificateThumbprint(keys.certPem,
            ...
            hex.slice(0, 16), "SPIRE's authority id is the same digest, hex, " +
                              "truncated");
    ...
    t.equal(crypto.certificateThumbprint(der),
    ...
    // -----------------------------------------------------------------------
    t.log.info('H. the clock allowance is applied by DEFAULT — the drift ' +
               'this closed');
    // -----------------------------------------------------------------------
    // `oauth2.js` has always said every read-back of one of our own tokens
    // takes `oauth2.clockSkewS`, and scoped it to that file. Four sites in
    // oid4vc/ did not, so a token could be refused there seconds before it
    // should be. The fix is that the shared verifier applies it unless a caller
    // opts out — so the assertion is about the DEFAULT, not about any one call
    // site.
    const skew = crypto.tokenClockSkew();
    t.check(skew > 0, 'there is a configured clock allowance to apply',
            'oauth2.clockSkewS=' + skew);

    const justExpired = Math.floor(Date.now() / 1000) - Math.floor(skew / 2);
    const token = crypto.signJws({ sub: 'probe', exp: justExpired },
                                 keys.privateKeyPem);
    ...
    t.check(acceptedByDefault,
            'A TOKEN INSIDE THE ALLOWANCE IS ACCEPTED WITHOUT THE CALLER ' +
            'ASKING — this is what the four oid4vc/ sites got wrong',
            'exp was ' + Math.floor(skew / 2) + 's ago, allowance is ' + skew +
            's');
    ...
    t.check(algNoneRefused, 'an `alg: none` token is refused by the default ' +
                            'algorithm list');

    // -----------------------------------------------------------------------
    t.log.info('I. constant-time comparison, including the length case that ' +
               'throws');
    // -----------------------------------------------------------------------
    t.check(crypto.constantTimeEquals('s3cret', 's3cret'),
            'equal secrets match');
    t.check(!crypto.constantTimeEquals('s3cret', 's3cres'), 'unequal secrets ' +
        'of one length do not');
    ...
    t.check(!crypto.constantTimeEquals(undefined, 'x'), 'a missing secret ' +
                                                        'answers false');
