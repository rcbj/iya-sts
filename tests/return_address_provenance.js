...
function run(t) {
  ...
  const entry = function () { return applications.get(spId); };
  const acsValues = function () {
    return [].concat(((entry() || {}).fields || {})[ACS] || []);
  };
  const marksOf = function () {
    return applications.observedReturnAddresses(entry() || {}).map(function (row) {
  ...
  const resolveFor = function (requested) {
    ...
    return returnAddress.resolve({
      requested: requested, registered: known.registered, unconfirmed: known.unconfirmed,
  ...
  withMode(config, 'development', function () {
    const created = applications.createApplication({
      ...
      fields: { samlEntityId: spId, samlAssertionConsumerService: [REGISTERED] } });
    t.check(created && created.ok, 'a fixture service provider was registered with one ACS URL',
    ...
    applications.seen({ identifier: spId, kind: 'saml2-service-provider', counts: false,
                        fields: { samlAssertionConsumerService: LEARNT } });
    t.check(acsValues().indexOf(LEARNT) >= 0,
            'development still writes the ACS URL a request named', acsValues());
    ...
    t.check(marksOf().indexOf(ACS + ' ' + REGISTERED) < 0,
            'a sighting that repeats a REGISTERED address does not demote it', marksOf());

    const dev = applications.returnAddressesOf(entry(), ACS);
    t.check(dev.registered.indexOf(LEARNT) >= 0 && dev.unconfirmed.length === 0,
            'development believes every address, marked or not — its behaviour is unchanged',
            JSON.stringify(dev));
    t.equal(resolveFor(LEARNT).ok, true,
            'and the resolver delivers to the learnt address in development');
    const shown = (entry().returnAddressesObserved || []).filter(function (row) {
      return row.value === LEARNT;
    })[0];
    t.check(shown && shown.trusted === true && shown.held === true,
            'the application view lists it, trusted in development', JSON.stringify(shown));
    t.check(!applications.normaliseFields({ appReturnAddressObserved: 'x y' }).ok,
  ...
  withMode(config, 'product', function () {
    const known = applications.returnAddressesOf(entry(), ACS);
    t.check(known.registered.indexOf(LEARNT) < 0 && known.unconfirmed.indexOf(LEARNT) >= 0,
            'returnAddressesOf() withholds the marked address in product', JSON.stringify(known));
    ...
    t.equal(refused.ok, false, 'a request naming the observed address is REFUSED in product');
    t.equal(errorCodes.codeOf(refused), 'STS-REG-0049', 'with STS-REG-0049');
    t.check(/confirm-address/.test(refused.why || '') && /DEVELOPMENT/.test(refused.why || ''),
            'and a sentence saying it was learnt in development and how to confirm it',
            refused.why);
    t.equal(resolveFor(REGISTERED).ok, true, 'a registered address is still delivered to');
    t.equal(resolveFor('').url, REGISTERED,
            'a request naming none gets the last REGISTERED address, never the observed one');
    const shown = (entry().returnAddressesObserved || []).filter(function (row) {
      return row.value === LEARNT;
    })[0];
    t.check(shown && shown.trusted === false,
            'and the view says product does not trust it', JSON.stringify(shown));
  });
  ...
  withMode(config, 'development', function () {
    applications.createApplication({ identifier: onlyId, kind: 'saml2-service-provider',
                                     fields: { samlEntityId: onlyId } });
    applications.seen({ identifier: onlyId, kind: 'saml2-service-provider', counts: false,
                        fields: { samlAssertionConsumerService: LEARNT } });
  });
  withMode(config, 'product', function () {
    const known = applications.returnAddressesOf(applications.get(onlyId), ACS);
    const none = returnAddress.resolve({
      requested: '', registered: known.registered, unconfirmed: known.unconfirmed,
    ...
    t.check(!none.ok && errorCodes.codeOf(none) === 'STS-REG-0049' &&
            ...
            'an entry holding ONLY an observed address refuses a request naming none with ' +
            'STS-REG-0049 and the way to confirm it', JSON.stringify([none.why, errorCodes.codeOf(none)]));
  });
  ...
  // -------------------------------------------------------------------------
  const refusedAbsent = applications.confirmReturnAddress(spId, { attribute: ACS, value: REGISTERED });
  t.check(!refusedAbsent.ok && errorCodes.codeOf(refusedAbsent) === 'STS-REG-0050',
          'confirming an address that is NOT marked is refused by name (STS-REG-0050)',
  ...
  t.equal(errorCodes.codeOf(refusedAttr), 'STS-REG-0051',
          'an attribute that is not a return address is refused (STS-REG-0051)');
  t.equal(errorCodes.codeOf(applications.confirmReturnAddress(spId, { attribute: ACS })),
          'STS-REG-0052', 'a confirm naming no address is refused (STS-REG-0052)');
  const confirmed = applications.confirmReturnAddress(spId, { attribute: ACS, value: LEARNT });
  t.check(confirmed.ok, 'confirm-address succeeds on a marked address',
          JSON.stringify(confirmed.errors));
  t.check(marksOf().indexOf(ACS + ' ' + LEARNT) < 0 && acsValues().indexOf(LEARNT) >= 0,
          'the MARK is gone and the ADDRESS stays', JSON.stringify([marksOf(), acsValues()]));
  withMode(config, 'product', function () {
  ...
  t.check(marksOf().indexOf(ACS + ' ' + SECOND) >= 0, 'a second sighting is marked', marksOf());
  const discarded = applications.discardReturnAddress(spId, { attribute: ACS, value: SECOND });
  t.check(discarded.ok, 'discard-address succeeds', JSON.stringify(discarded.errors));
  t.check(marksOf().indexOf(ACS + ' ' + SECOND) < 0 && acsValues().indexOf(SECOND) < 0,
          'BOTH the mark and the address are gone', JSON.stringify([marksOf(), acsValues()]));
  withMode(config, 'product', function () {
    const gone = resolveFor(SECOND);
    t.check(!gone.ok && errorCodes.codeOf(gone) !== 'STS-REG-0049',
            'product refuses it as an address that is not on the entry, not as an observed one',
            gone.why);
  });
  t.check(!applications.discardReturnAddress(spId, { attribute: ACS, value: REGISTERED }).ok &&
  ...
  withMode(config, 'development', function () {
    applications.seen({ identifier: spId, kind: 'saml2-service-provider', counts: false,
                        fields: { samlAssertionConsumerService: [THIRD, FOURTH] } });
  });
  const added = applications.updateApplication(spId, { attribute: ACS, mode: 'add', value: THIRD });
  t.check(added.ok && added.changed,
          'an explicit `add` of an address already there as observed is a CHANGE',
          JSON.stringify(added));
  t.check(marksOf().indexOf(ACS + ' ' + THIRD) < 0 && acsValues().indexOf(THIRD) >= 0,
          'and it CONFIRMS the address', JSON.stringify([marksOf(), acsValues()]));
  applications.updateApplication(spId, { attribute: ACS, mode: 'remove', value: FOURTH });
  t.check(marksOf().indexOf(ACS + ' ' + FOURTH) < 0 && acsValues().indexOf(FOURTH) < 0,
          'a `remove` takes the mark with the value, so no mark names a missing address',
          JSON.stringify([marksOf(), acsValues()]));
  withMode(config, 'product', function () {
    const before = acsValues().slice(0);
    applications.updateApplication(spId,
      { attribute: ACS, mode: 'add', value: 'https://flagged.rap.test/acs', observed: true });
    t.check(marksOf().indexOf(ACS + ' https://flagged.rap.test/acs') < 0,
            'in product an `observed` flag can mark NOTHING — product learns no address',
            JSON.stringify([before, marksOf()]));
    applications.updateApplication(spId,
      { attribute: ACS, mode: 'remove', value: 'https://flagged.rap.test/acs' });
  });

  const clientId = 'rap-client-' + suffix;
  const CB = 'https://client.rap.test/cb';
  applications.register(clientId, { redirect_uris: ['https://client.rap.test/first'],
                                    client_secret: 'rap-secret' });
  withMode(config, 'development', function () {
    applications.updateApplication(clientId,
      { attribute: 'oauthRedirectUri', mode: 'add', value: CB, observed: true });
  });
  t.check(applications.observedReturnAddresses(applications.get(clientId))
            .some(function (row) { return row.value === CB; }),
          'a redirect URI added with the observed flag in development is marked');
  applications.updateRegistration(clientId,
    { redirect_uris: ['https://client.rap.test/first', CB], client_secret: 'rap-secret' });
  t.check(!applications.observedReturnAddresses(applications.get(clientId))
             .some(function (row) { return row.value === CB; }),
          'and an RFC 7592 update naming it CONFIRMS it — a registration is explicit');
  ...
  const cleanup = function () {
    if ([].concat((applications.get(CLIENT) || { fields: {} }).fields.oauthRedirectUri || [])
          .indexOf(learntCb) >= 0) {
      applications.updateApplication(CLIENT, { attribute: 'oauthRedirectUri', mode: 'remove',
                                               value: learntCb,
  ...
  try {
    withMode(config, 'development', function () {
      const started = oidcRp.beginSignIn(fakeReq(host), fakeRes(), 'admin', { returnTo: '/admin' });
      t.check(started.ok, 'development learns the callback as it always did', started.why);
    });
    ...
    withMode(config, 'product', function () {
      ...
      t.check(cfg.redirect_uris.indexOf(learntCb) < 0 &&
              ...
              'clientConfigOf() withholds it in product, so RFC 9700 mode does not believe it either',
              JSON.stringify(cfg.unconfirmed_redirect_uris));
      const refused = oidcRp.beginSignIn(fakeReq(host), fakeRes(), 'admin', { returnTo: '/admin' });
    ...
    t.check(ok.ok, 'the learnt callback can be confirmed', JSON.stringify(ok.errors));
    withMode(config, 'product', function () {
      const cfg = applications.clientConfigOf(CLIENT);
      const again = oidcRp.ensureRedirectUri(oidcRp.surfaceOf('admin'), cfg, learntCb);
      t.check(again.ok && !again.learnt,
              'and once confirmed product uses it without learning anything', JSON.stringify(again));
    });
  } finally {
    cleanup();
  }
  ...
  [['saml/saml2_sso.js', saml2, /applications\.returnAddressesOf\(known, 'samlAssertionConsumerService'\)/],
   ['saml/saml11_sso.js', saml11, /applications\.returnAddressesOf\(\s*early\.id/],
   ['ws-federation/wsfed.js', wsfed, /applications\.returnAddressesOf\(realmEntry/]]
    .forEach(function (one) {
      t.check(one[2].test(one[1]), one[0] + ' asks applications.returnAddressesOf()');
    });
  t.check(!/registered:\s*known\.samlAssertionConsumerService/.test(saml2) &&
          ...
          'and none of the three hands the resolver the raw attribute any more');
  t.check(/unconfirmed:\s*acsKnown\.unconfirmed/.test(saml2) &&
          ...
          'each passes the withheld addresses on, so the refusal can name them');
  const cfgBlock = registry.slice(registry.indexOf('function clientConfigOf('),
                                  registry.indexOf('function clientConfigOf(') + 3200);
...
module.exports = {
  ...
  describe: 'a return address a development sighting wrote is marked observed, ' +
            'withheld in product until confirmed or discarded, and every ' +
            'return-address check asks applications.returnAddressesOf()',
  run: run
};
