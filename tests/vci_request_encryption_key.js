...
// loaded, handed to request workers in `process.env.STS_VCI_REQUEST_ENC_KEY_PEM`,
// persisted in no mode, and SHARED by every trust realm in a pooled process —
// so a realm's issuer decrypted requests encrypted to another realm's published
// key. `common/mode.js` carried it as NOT_YET (`vci-request-encryption-key`).
//
...
function nullStore() {
  return { loadKeys: function () { return Promise.resolve([]); },
...
function checkPerRealm(t) {
  ...
  function create(id, overrides) {
    ...
    if (!r.ok) {
      t.bad('could not create the realm "' + id + '"', (r.errors || []).join(' '));
      return null;
    }
    made.push(id);
    return r.realm;
  ...
  try {
    ...
    const b = create('vci-key-b', { 'oid4vci.requestEncryptionKeyBits': '3072' });
    if (!a || !b) {
      return;
    }
    ...
    const kA = realms.run(a, function () { return helpers.requestEncryptionKeyFor(); });
    const kB = realms.run(b, function () { return helpers.requestEncryptionKeyFor(); });

    t.check(!!(kDefault && kA && kB && kDefault.publicJwk && kA.publicJwk && kB.publicJwk),
            'the default realm and two realms each answer with a request-encryption key');
    t.check(kA.publicJwk.kid !== kB.publicJwk.kid &&
            kA.publicJwk.kid !== kDefault.publicJwk.kid &&
            kB.publicJwk.kid !== kDefault.publicJwk.kid,
            'THREE REALMS, THREE KEYS. In a pooled process this was one key the ' +
            'pool handed down, shared by every realm',
    ...
    t.check(helpers.stsKeysFor.of('vci-key-a').vciRequestEncKey.publicJwk.kid ===
            kA.publicJwk.kid,
            'the key IS the realm\'s key set\'s member, which is the whole of the ' +
            'change: every property of the set reaches it');
    t.check(/^sts-req-enc-/.test(kA.publicJwk.kid) &&
            ...
            'what a wallet sees is unchanged: the kid prefix, alg RSA-OAEP-256, ' +
            'use enc and key_ops encrypt, which OID4VCI section 10 requires',
            JSON.stringify({ kid: kA.publicJwk.kid, alg: kA.publicJwk.alg,
                             use: kA.publicJwk.use, key_ops: kA.publicJwk.key_ops }));
    ...
    t.equal(modulusBits(kA.publicJwk), 2048,
            'a realm with no override gets oid4vci.requestEncryptionKeyBits\'s default');
    t.equal(modulusBits(kB.publicJwk), 3072,
            'A REALM CARRYING THE SETTING GETS THE SIZE IT ASKED FOR — the key set ' +
            'is made inside the realm it is for, so the setting is read there');
    ...
    t.check(!!(readInA && readInA.plaintext && JSON.parse(readInA.plaintext).probe === 'a'),
    ...
    t.check(/kid/.test(refusedByKid),
            'REALM B REFUSES IT — on the kid, which is the first thing the issuer ' +
            'checks and names the wrong issuer rather than a corrupt request',
            refusedByKid);
    ...
    const forged = stsCrypto.encryptJweCompact(JSON.stringify({ probe: 'forged' }), {
    ...
    t.check(refusedByKey !== '' && !/kid/.test(refusedByKey),
            'AND WITH B\'S KID FORGED ONTO IT, B\'S KEY STILL CANNOT OPEN IT — the ' +
            'refusal is the unwrap, which is the property the shared key did not have',
            refusedByKey);
    ...
    if (again) {
      ...
      t.check(kAgain.publicJwk.kid !== kA.publicJwk.kid,
              'a realm removed and created again under the same id gets a NEW ' +
              'key — the key set\'s purge reaches the new member',
...
function checkChannel(t) {
  ...
  try {
    ...
    t.check(!!(offer && offer.blob && offer.blob.vciRequestEncKey),
            'THE BLOB A PROCESS OFFERS CARRIES THE KEY — serialise() writes the ' +
            'new member',
    ...
    t.check(!!(offer && offer.blob.vciRequestEncKey.privateKeyPem &&
            ...
            'as a PEM, which is what survives JSON and the fork\'s IPC channel');
    ...
    t.check(!!(adoptedSet.vciRequestEncKey && adoptedSet.vciRequestEncKey.publicJwk &&
               adoptedSet.vciRequestEncKey.publicJwk.kid === front.publicJwk.kid),
            'THE SET BUILT FROM A SIBLING\'S BLOB CARRIES THE KEY ITSELF',
            adoptedSet.vciRequestEncKey ? adoptedSet.vciRequestEncKey.publicJwk.kid : '(absent)');
    const worker = realms.run(r.realm, function () {
      return helpers.requestEncryptionKeyFor();
    });
    t.equal(worker && worker.publicJwk.kid, front.publicJwk.kid,
            'THE ADOPTING PROCESS PUBLISHES THE SAME KEY — plainKeySet() puts the ' +
            'member back on a sibling\'s set rather than leaving it to be generated');
    t.check(!!(worker && worker.privateKey &&
               nodeCrypto.createPublicKey(worker.privateKey).export({ format: 'jwk' }).n ===
               front.publicJwk.n),
            'and holds the matching PRIVATE key, so a request encrypted to what one ' +
            'process published is decrypted by the other');
  } finally {
    keystore.reset();
    helpers.resetStsKeys();
...
async function checkStore(t) {
  ...
  try {
    ...
    const opened = JSON.parse(stsCrypto.decryptWithKek(kekText, row, 'signing-keys'));
    t.check(!!(opened.vciRequestEncKey && opened.vciRequestEncKey.publicJwk &&
            ...
            'AND THE ROW CARRIES THE KEY — this is what "persisted in neither mode" ' +
            'becomes',
            opened.vciRequestEncKey ? opened.vciRequestEncKey.publicJwk.kid : '(absent)');

    await restart();
    const restoredSet = helpers.stsKeysFor.of('default');
    t.check(!!(restoredSet.vciRequestEncKey && restoredSet.vciRequestEncKey.publicJwk &&
               restoredSet.vciRequestEncKey.publicJwk.kid === first.publicJwk.kid),
            'THE SET RESTORED FROM THE STORE CARRIES THE KEY ITSELF — lazyKeySet() ' +
            'puts the member back, which is the post-quantum half\'s defect not ' +
            'made a second time');
    const restored = helpers.requestEncryptionKeyFor();
    t.equal(restored.publicJwk.kid, first.publicJwk.kid,
            'THE SAME KEY COMES BACK AFTER A RESTART, so a wallet that cached the ' +
            'issuer metadata before it can still encrypt to this issuer after it');
    ...
    t.check(publicOnly === first.publicJwk.kid &&
            ...
            'PUBLISHING THE KEY DECRYPTS NOTHING — the public JWK is resident on the ' +
            'set and the private half is a getter, as the curve keys are',
    ...
    store.rows.set('default', stsCrypto.encryptWithKek(kekText, JSON.stringify(legacy),
                                                        'signing-keys'));
    await restart();
    t.equal(helpers.STS.vciRequestEncKey, undefined,
            'a set restored from a row written before the key existed has no member');
    t.equal(keystore.requestEncryptionKeyHeldFor('default'), null,
            'and nothing this service holds names one, so the backfill must make one');
    const backfilled = helpers.requestEncryptionKeyFor();
    t.check(!!(backfilled && backfilled.publicJwk && backfilled.privateKey),
            'THE LEGACY SET IS BACKFILLED on first use rather than failing the issuer');
    await settle();
    ...
    t.check(!!(rewritten.vciRequestEncKey &&
            ...
            rewritten.vciRequestEncKey ? rewritten.vciRequestEncKey.publicJwk.kid : '(absent)');
    t.equal(rewritten.privateKeyPem, legacy.privateKeyPem,
            'without replacing the signing key it was added beside');
    await restart();
    t.equal(helpers.requestEncryptionKeyFor().publicJwk.kid, backfilled.publicJwk.kid,
    ...
    t.equal(helpers.stsKeysFor.of('default'), stale,
            'is handed a sibling\'s backfill without its cached set being dropped');
    const adopted = helpers.requestEncryptionKeyFor();
    t.equal(adopted.publicJwk.kid, backfilled.publicJwk.kid,
            'AND USES THE SIBLING\'S KEY RATHER THAN MAKING A SECOND ONE — the ' +
            'backfill asks requestEncryptionKeyHeldFor() before it generates, or ' +
            'this process would publish a key no other process can decrypt for');
  } finally {
    ...
    try {
      ...
      process.stderr.write('vci key test: could not remove ' + dir + ': ' + e.message + '\n');
    }
  }
}
...
function checkEnrichment(t) {
  ...
  const withVci = Object.assign({}, base, { vciRequestEncKey: { publicJwk: {} } });
  const withPq = Object.assign({}, base, { pqKeys: [{}, {}] });
  const withBoth = Object.assign({}, withPq, { vciRequestEncKey: { publicJwk: {} } });
  ...
  t.equal(keystore.enriches(withVci, withPq), false,
          'BUT A SET THAT GAINED ONE MEMBER AND LACKS THE OTHER IS NOT — it would ' +
          'replace a held blob with one that has lost its post-quantum keys');
  t.equal(keystore.enriches(withPq, withVci), false,
          'and the same the other way round: post-quantum keys offered against a ' +
          'held blob that already has the request-encryption key would lose the ' +
          'key — which the one-member rule this replaced accepted');
  ...
  t.equal(keystore.enriches(Object.assign({}, withBoth, { certB64: 'OTHER' }), base), false,
...
function checkSource(t) {
  ...
  t.check(code(pool).indexOf('STS_VCI_REQUEST_ENC_KEY_PEM') < 0 &&
          ...
          'the front process no longer generates a key and hands it down the fork');
  ...
  t.check(/keystore\.enriches\(/.test(code(pool)),
          'the arbitrating process applies keystore.enriches() rather than a copy ' +
          'of the rule with one member in it');
}
...
function childMain() {
  ...
  try {
    ...
    note(pubA && pubB && pubA.kid !== pubB.kid,
         'the issuer metadata of two realms publishes two keys in credential_request_encryption.jwks',
         (pubA && pubA.kid) + ' / ' + (pubB && pubB.kid));
    const compact = stsCrypto.encryptJweCompact(JSON.stringify({ credential_identifier: 'x' }), {
      alg: 'RSA-OAEP-256', enc: 'A256GCM', jwk: pubA });
    let inA = null;
    try {
      inA = realms.run(a, function () { return issuer.decryptJweRequest(compact); });
    } catch (e) {
      inA = { error: e.message };
    }
    note(inA && inA.credential_identifier === 'x',
         'realm A\'s issuer decrypts a Credential Request encrypted to realm A\'s published key',
         JSON.stringify(inA));
    let inB = '';
    try {
    ...
    note(inB !== '',
         'REALM B\'S ISSUER REFUSES THE SAME JWE — the defect this closes', inB);
    const forged = stsCrypto.encryptJweCompact('{"credential_identifier":"y"}', {
      alg: 'RSA-OAEP-256', enc: 'A256GCM', jwk: Object.assign({}, pubA, { kid: pubB.kid }) });
    let forgedInB = '';
    try {
    ...
    const fakeReq = function (body, type) {
      return { path: '/oid4vci/credential', body: body,
               get: function (name) { return /content-type/i.test(name) ? type : ''; } };
    };
    realms.run(a, function () {
      return issuer.readPossiblyEncryptedRequest(fakeReq(compact, 'application/jwt'));
    });
    const lastA = realms.run(a, function () { return issuer.lastCredentialRequest(); });
    const lastB = realms.run(b, function () { return issuer.lastCredentialRequest(); });
    const lastDefault = issuer.lastCredentialRequest();
    note(lastA.seen === true && lastA.encrypted === true && lastA.kid === pubA.kid,
         '/oid4vci/last_request in realm A reports the encrypted request it received',
         JSON.stringify(lastA));
    note(lastB.seen === false && lastDefault.seen === false,
         'AND NEITHER REALM B NOR THE DEFAULT REALM REPORTS IT — it was one `let` for the process',
...
function checkIssuer(t) {
  ...
  const out = path.join(os.tmpdir(), 'sts-vci-key-' + process.pid + '-' + Date.now() + '.json');
...
module.exports = {
  ...
  describe: 'the OpenID4VCI request-encryption key is a per-realm member of the key set',
  run: run
};
