...
function inAChild(env, body) {
  ...
  const script =
    ...
    'Promise.resolve((async function () {' + body + '})()).then(function (report) {' +
...
const DIRECTORY = 'const d = require(R + "/ldap/ldap_server.js");' +
  'const out = {};' +
  'd.entries.forEach(function (e, k) { out[k] = { attributes: Object.keys(e.attributes).sort(),' +
  ' member: e.attributes.member || [], description: e.attributes.description || [] }; });';
...
function productDirectoryHoldsNoDemoData(t) {
  ...
  t.check(!has(r, 'cn=remote-pep-1,ou=users,dc=example,dc=com') &&
          ...
          'NEITHER PRIVILEGED IDENTITY is seeded — a predictable common name printed ' +
          'in this repository would otherwise be admitted to the documents this ' +
          'service enforces its own access with');
  ...
  t.check(!!peps && !!users && !peps.member.length && !users.member.length,
          'the two role groups ARE seeded, EMPTY, so admitting somebody is one member ' +
          'added', JSON.stringify({ peps: peps, users: users }));
  ['dc=example,dc=com', 'ou=users,dc=example,dc=com', 'ou=groups,dc=example,dc=com',
   'ou=applications,dc=example,dc=com'].forEach(function (dn) {
    t.check(has(r, dn), 'the structural container ' + dn + ' is there');
  });
  t.check(!/Every bind succeeds/.test(JSON.stringify(r.entries['dc=example,dc=com'].description)),
...
function roleGroupsFollowTheSettings(t) {
  t.log.info('=== the seeded role groups are roles.remotePepGroup and roles.xacmlUserGroup ===');
  ...
  t.check(!!gate && gate.member.indexOf('cn=remote-pep-1,ou=users,dc=example,dc=com') >= 0,
          'the REMOTE_PEPS group seeded is the one `roles.remotePepGroup` names — the ' +
          'literal cn=remote-peps granted nothing once the setting was changed',
          JSON.stringify(gate || null));
  t.check(!!users && users.member.indexOf('cn=xacml-user-1,ou=users,dc=example,dc=com') >= 0,
...
// ---------------------------------------------------------------------------
// 3. PRODUCT MODE INVENTS NO CLAIM VALUE, AND THE BIND AUDIT SAYS WHAT HAPPENED.
// ---------------------------------------------------------------------------
function productInventsNothingAndAuditsTruthfully(t) {
  t.log.info('=== no invented values, and a verified bind is audited as verified ===');
  const run = inAChild({ STS_MODE: 'product' },
    'const d = require(R + "/ldap/ldap_server.js");' +
    'const credentials = require(R + "/common/credentials.js");' +
    'const audit = require(R + "/common/audit.js");' +
    'const made = d.createUser("probe-person", { origin: "test" });' +
    'const entry = d.entries.get("uid=probe-person,ou=users,dc=example,dc=com");' +
    'const swept = d.populateVcAttributes();' +
    'const pw = credentials.setPassword("probe-person", "Correct-Horse-9-Battery");' +
    ...
    'const bind = d.performOperation("bind", { dn: "uid=probe-person,ou=users,dc=example,dc=com",' +
    ' channel: "ldaps", credentials: "Correct-Horse-9-Battery" });' +
    'const row = audit.list().filter(function (e) { return e.action === "directory.bind"; })[0] || null;' +
    'return { made: made && made.ok, attributes: entry ? Object.keys(entry.attributes).sort() : null,' +
    ' swept: swept, pw: pw && pw.ok, pwErrors: pw && pw.errors, bind: bind.ok, row: row };');
  const r = run.report || {};
  ...
  }), 'a SCIM-shaped create (invent defaulted to TRUE) puts NONE of the five persona ' +
      'attributes on the entry in product mode — no "(mock)" displayName, no invented ' +
      'mail', JSON.stringify(r.attributes));
  t.check(r.swept && r.swept.examined === 0 && /product mode/.test(String(r.swept.skipped)),
          'and the credential-attribute sweep does not run, saying why',
          JSON.stringify(r.swept));
  t.check(r.pw === true && r.bind === true, 'a real password verifies over a bind',
          JSON.stringify({ pw: r.pw, errors: r.pwErrors, bind: r.bind }));
  const detail = (r.row && r.row.detail) || {};
  t.check(detail.passwordVerified === true && !/no password was checked/.test(String(detail.note)),
          'THE AUDIT ROW SAYS THE PASSWORD WAS VERIFIED — it said "no password was ' +
          'checked" on every successful bind, including the ones product mode verified',
...
function developmentIsUnchanged(t) {
  ...
  const run = inAChild({},
    'const d = require(R + "/ldap/ldap_server.js");' +
    'const audit = require(R + "/common/audit.js");' +
    'd.createUser("dev-probe", { origin: "test" });' +
    'const entry = d.entries.get("uid=dev-probe,ou=users,dc=example,dc=com");' +
    'd.performOperation("bind", { dn: "uid=alice,ou=users,dc=example,dc=com", credentials: "x" });' +
    'const row = audit.list().filter(function (e) { return e.action === "directory.bind"; })[0] || null;' +
    'return { displayName: entry && entry.attributes.displayname, alice: !!d.entries.get(' +
    '"uid=alice,ou=users,dc=example,dc=com"), pep: !!d.entries.get("cn=remote-pep-1,ou=users,dc=example,dc=com"),' +
    ' row: row && row.detail };');
  const r = run.report || {};
  t.check(r.alice === true && r.pep === true, 'development still seeds alice and the PEP identity',
          run.output.slice(-300));
  t.check(Array.isArray(r.displayName) && / \(mock\)$/.test(r.displayName[0]),
          'and still invents a persona, "(mock)" and all', JSON.stringify(r.displayName));
  t.check(r.row && r.row.passwordVerified === false && /no password was checked/.test(r.row.note),
...
function listenersHonourTheBindAddress(t) {
  ...
  const run = inAChild({ STS_HOST: '127.0.0.1', LDAP_PORT: '0', LDAPS_PORT: '0',
                         STS_TLS_PORT: '0', STS_MTLS_PORT: '0', KRB5_KDC_PORT: '0',
                         KRB5_SERVICE_PORT: '0' },
    'const kdc = require(R + "/kerberos/krb5_kdc.js");' +
    'const svc = require(R + "/kerberos/krb5_service.js");' +
    'const tls = require(R + "/tls/tls_server.js");' +
    'const ldap = require(R + "/ldap/ldap_server.js");' +
    'const k = kdc.listen(0); const kr = await k.whenReady;' +
    'const s = svc.listen(0); await new Promise(function (ok) { s.listening ? ok() : s.once("listening", ok); });' +
    'const l = ldap.listen(); await l.whenReady;' +
    'const tr = tls.listen(); await tr.whenReady;' +
    'return { kdcTcp: kr.tcp.address().address, kdcUdp: kr.udp.address().address,' +
    ' service: s.address().address, ldap: l.server.address().address,' +
    ' ldaps: l.secureServer && l.secureServer.address().address };');
  const r = run.report || {};
  t.check(r.kdcTcp === '127.0.0.1' && r.kdcUdp === '127.0.0.1',
          'the KDC binds global.host on TCP and UDP', JSON.stringify(r) + run.output.slice(-300));
  ...
  const off = inAChild({ LDAP_PLAIN_LISTENER: 'false', LDAP_PORT: '0', LDAPS_PORT: '0',
                         STS_HOST: '127.0.0.1' },
    'const ldap = require(R + "/ldap/ldap_server.js");' +
    'const l = ldap.listen(); const ready = await l.whenReady;' +
    'return { port: ready.port, plain: ready.plainListener, bound: !!(l.server.server && l.server.server.listening),' +
    ' ldaps: ready.ldapsListening };');
  const o = off.report || {};
  t.check(o.port === null && o.plain === false && o.bound === false && o.ldaps === true,
          'ldap.plainListener off leaves 389 unbound and LDAPS answering, and the ' +
          'directory still counts as started', JSON.stringify(o) + off.output.slice(-300));

  const warned = inAChild({ STS_MODE: 'product', LDAP_PORT: '0', LDAPS_PORT: '0',
                            STS_HOST: '127.0.0.1', LOG_LEVEL: 'warn' },
    'const ldap = require(R + "/ldap/ldap_server.js");' +
    'const l = ldap.listen(); await l.whenReady; return { ok: true };');
  t.check(/IN THE CLEAR/.test(warned.output) && /ldap\.plainListener/.test(warned.output),
          'product mode with the plain listener on WARNS, naming the setting that ends it',
...
function httpProbe(env, method, pathname, body) {
  return inAChild(env,
    'const app = require(R + "/common/app");' +
    'require(R + "/tls/tls_server.js");' +
    ...
    '    headers: { "content-type": "text/plain", accept: "application/json" } }, function (res) {' +
    '    let text = ""; res.on("data", function (c) { text += c; });' +
    '    res.on("end", function () { ok({ status: res.statusCode, body: text }); });' +
    '  });' +
...
function theTruststoreIsNotATestControlInProduct(t) {
  ...
  t.check(rr.status === 403 && /tls\.trustAnchorsFile/.test(rr.body),
          'a product service REFUSES a trust anchor from whoever reaches the port, and ' +
          'names the setting that is the door instead', JSON.stringify(rr).slice(0, 400));
  ...
  t.equal((accepted.report || {}).status, 200,
          'while development mode still takes the anchor, which is what the launchers use');
  ...
  try {
    const loaded = inAChild({ STS_TLS_TRUST_ANCHORS_FILE: file },
      'const tls = require(R + "/tls/tls_server.js");' +
      'return { count: tls.anchorCount(), report: tls.trustAnchorsFileLoaded(),' +
      ' ca: tls.clientTruststoreOptions().ca.length };');
    const lr = loaded.report || {};
    t.check(lr.count === 1 && lr.ca === 1 && lr.report && lr.report.file === file,
            'tls.trustAnchorsFile fills the truststore at startup, before the listeners ' +
            'are created from it', JSON.stringify(lr) + loaded.output.slice(-300));
  } finally {
    try {
      fs.unlinkSync(file);
    } catch (e) {
  ...
  const missing = inAChild({ STS_TLS_TRUST_ANCHORS_FILE: '/nonexistent/anchors.pem' },
  ...
  t.check(missing.status === 1 && missing.report === null &&
          ...
          'a trust-anchor file that cannot be read STOPS the service, naming the setting',
          'exit ' + missing.status);
}
...
async function theProtocolPolicyIsApplied(t) {
  ...
  t.check(defaults.minVersion === 'TLSv1.2' && defaults.ciphers === undefined,
          'the defaults are node\'s own written down — TLSv1.2, and no cipher list',
          JSON.stringify(defaults));
  t.equal(tlsServer.clientTruststoreOptions().minVersion, 'TLSv1.2',
          'and they ride in the context every listener is built from and re-keyed with');

  const bad = inAChild({ STS_TLS_CIPHERS: 'NOT-A-CIPHER-AT-ALL' },
    'require(R + "/tls/tls_server.js"); return { started: true };');
  t.check(bad.status === 1 && bad.report === null && /tls\.ciphers/.test(bad.output),
          'a cipher list matching nothing STOPS the service at startup, naming the setting',
          'exit ' + bad.status);

  const floor = inAChild({ STS_TLS_MIN_VERSION: 'TLSv1.3', STS_TLS_PORT: '0',
                           STS_MTLS_PORT: '0', STS_HOST: '127.0.0.1' },
    'const tlsServer = require(R + "/tls/tls_server.js");' +
    'const ready = await tlsServer.listen().whenReady;' +
    'const tls = require("tls");' +
    'function attempt(max) { return new Promise(function (ok) {' +
    '  const s = tls.connect({ host: "127.0.0.1", port: ready.tlsPort, maxVersion: max,' +
    '    rejectUnauthorized: false }, function () { const v = s.getProtocol(); s.destroy(); ok(v); });' +
    '  s.on("error", function (e) { ok("refused: " + e.code); }); }); }' +
    'return { twelve: await attempt("TLSv1.2"), thirteen: await attempt("TLSv1.3") };');
  const f = floor.report || {};
  t.check(/^refused/.test(String(f.twelve)) && f.thirteen === 'TLSv1.3',
          'with tls.minVersion TLSv1.3 a TLS 1.2 client is REFUSED on 8443 and a 1.3 ' +
          'client is served — the setting reaches the handshake, not only a report',
          JSON.stringify(f) + floor.output.slice(-300));
}

module.exports = {
  name: 'ldap_tls_product_mode',
  describe: 'the directory and TLS literals the 2026-09-12 audit found, in both modes',
