...
function childMain() {
      ...
      const req = http.request({ host: '127.0.0.1', port: port, path: urlPath,
        ...
        res.on('end', function () {
          ...
          try {
            ...
            // Not JSON — an HTML page or plain text; the raw body is kept.
            parsed = null;
          }
          resolve({ status: res.statusCode, headers: res.headers, text: text, json: parsed });
        });
      });
      req.end(body);
    });
  ...
  (async function () {
    ...
    ldap.createUser('hc-alice', { invent: false, attributes: {
      cn: 'Alice Liddell', givenName: 'Alice', sn: 'Liddell', mail: 'alice@hc.example' } });
    ldap.createUser('hc-bare', { invent: false });
    credentials.setPassword('hc-alice', PASSWORD);
    credentials.setPassword('hc-bare', PASSWORD);
    applications.createApplication({ identifier: 'hc-client', protocols: ['oauth2'],
    ...
    let r = await request(port, 'POST', '/oauth2/token', { form: Object.assign({
      grant_type: 'password', username: 'hc-alice', password: 'not-the-password' }, client) });
    note(r.status === 200, '1a. development: the password grant still accepts ANY password',
         r.status + ' ' + r.text.slice(0, 120));
    r = await request(port, 'POST', '/oauth2/token', { form: Object.assign({
      grant_type: 'password', username: 'hc-alice', password: 'invalid' }, client) });
    note(r.status === 400 && r.json && r.json.error === 'invalid_grant',
         '1b. development: the reserved password "invalid" is still refused', r.status);

    config.setOverride('global.mode', 'product');
    r = await request(port, 'POST', '/oauth2/token', { form: Object.assign({
      grant_type: 'password', username: 'hc-alice', password: 'not-the-password' }, client) });
    note(r.status === 400 && r.json && r.json.error === 'invalid_grant',
         '1c. PRODUCT: a wrong password is refused invalid_grant', r.status + ' ' + r.text.slice(0, 160));
    note(r.json && /Authentication failed for user hc-alice\.$/.test(r.json.error_description || ''),
         '1d. and the description is the one protocol answer, naming no reason (no enumeration)',
    ...
    note(r.status === 200 && r.json && r.json.access_token,
         '1e. PRODUCT: the right password is issued tokens', r.status + ' ' + r.text.slice(0, 160));
    ...
    if (r.json && r.json.id_token) {
      ...
      note(idt.name === 'Alice Liddell' && idt.given_name === 'Alice' &&
           ...
           '2a. PRODUCT: the ID Token\'s name, given_name, family_name and email are the ' +
           'person\'s own cn, givenName, sn and mail', JSON.stringify(idt));
      note(!('email_verified' in idt),
           '2b. and it asserts NO email_verified, because nothing verified that mailbox',
           idt.email_verified);
      note(idt.sub === 'urn:sts:user:hc-alice', '2c. and sub is unchanged', idt.sub);
    } else {
      note(false, '2a. PRODUCT: an ID Token came back to inspect', r.text.slice(0, 160));
    }
    if (r.json && r.json.access_token) {
      const info = await request(port, 'GET', '/oauth2/userinfo',
        { headers: { authorization: 'Bearer ' + r.json.access_token } });
      note(info.status === 200 && info.json && info.json.family_name === 'Liddell' &&
           info.json.email === 'alice@hc.example' && !('email_verified' in info.json),
           '2d. PRODUCT: UserInfo answers profile and email from the directory, without ' +
    ...
    if (r.json && r.json.id_token) {
      ...
      const invented = ['name', 'given_name', 'family_name', 'email', 'email_verified']
        .filter(function (k) { return k in bare; });
      note(invented.length === 0,
           '2e. PRODUCT: a person whose entry holds none of them gets NO profile claims — ' +
           'absent rather than invented, and never "undefined"', JSON.stringify(bare));
    } else {
      note(false, '2e. PRODUCT: a token for the bare person came back', r.text.slice(0, 160));
    }
    ...
    credentials.mechanismsFor = function (name) {
      ...
      return Object.assign({}, out, { mfaRequired: true, secondFactor: 'totp' });
    };
    r = await request(port, 'POST', '/oauth2/token', { form: Object.assign({
      grant_type: 'password', username: 'hc-alice', password: PASSWORD }, client) });
    credentials.mechanismsFor = realMechanisms;
    note(r.status === 400 && /second factor/.test((r.json || {}).error_description || ''),
         '1f. PRODUCT: a person holding a second factor is refused the password grant, ' +
         'with the door that does ask for it named', r.status + ' ' + r.text.slice(0, 200));

    // ======================================================================
    // 3. POST /dpop/nonce-mode — PRODUCT REFUSES, DEVELOPMENT IS PER REALM
    // ======================================================================
    r = await request(port, 'POST', '/dpop/nonce-mode', { json: { required: true } });
    note(r.status === 403 && /oauth2\.dpopNonceRequired/.test(r.text),
         '3a. PRODUCT: POST /dpop/nonce-mode is refused and names the setting to use instead',
         r.status + ' ' + r.text.slice(0, 160));
    config.clearOverride('global.mode');

    r = await request(port, 'POST', '/realm/hcrealm/dpop/nonce-mode', { json: { required: true } });
    const inRealm = await request(port, 'GET', '/realm/hcrealm/dpop/nonce-mode');
    const inDefault = await request(port, 'GET', '/dpop/nonce-mode');
    note(r.status === 200 && inRealm.json && inRealm.json.nonces_required === true,
         '3b. development: the switch turns nonces on in the realm it was reached in',
         inRealm.text.slice(0, 120));
    note(inDefault.json && inDefault.json.nonces_required === false,
         '3c. and NOT in the default realm — it was one switch for the whole process',
         inDefault.text.slice(0, 120));
    await request(port, 'POST', '/realm/hcrealm/dpop/nonce-mode', { json: { required: false } });
    ...
    note(r.status === 201 && r.json && /^hc-dyn-/.test(r.json.client_id),
         '4a. development: registration is open and the client_id takes the configured prefix',
         r.status + ' ' + (r.json && r.json.client_id));
    note(r.json && r.json.client_secret_expires_at > Math.floor(Date.now() / 1000),
         '4b. and client_secret_expires_at follows oauth2.registeredSecretLifetimeS',
    ...
    note(readNoToken.status === 401,
         '4c. an RFC 7592 read with no registration access token is refused', readNoToken.status);

    config.setOverride('global.mode', 'product');
    r = await request(port, 'POST', '/oauth2/register',
      { json: { redirect_uris: ['https://rp.hc.example/cb'] } });
    note(r.status === 403 && /oauth2\.openRegistration/.test(r.text),
         '4d. PRODUCT: registration is refused and names oauth2.openRegistration',
         r.status + ' ' + r.text.slice(0, 160));
    let meta = await request(port, 'GET', '/.well-known/oauth-authorization-server');
    note(meta.json && !('registration_endpoint' in meta.json),
         '4e. PRODUCT: and registration_endpoint is not advertised', meta.json && meta.json.registration_endpoint);
    config.setOverride('oauth2.openRegistration', true);
    r = await request(port, 'POST', '/oauth2/register',
      { json: { redirect_uris: ['https://rp.hc.example/cb'], token_endpoint_auth_method: 'client_secret_basic' } });
    meta = await request(port, 'GET', '/.well-known/oauth-authorization-server');
    note(r.status === 201 && meta.json && meta.json.registration_endpoint,
         '4f. PRODUCT with oauth2.openRegistration on: registration answers and is advertised',
         r.status + ' ' + r.text.slice(0, 120));
    ...
    const fakeReq = { protocol: 'http', headers: { host: '127.0.0.1:' + port },
                      get: function (n) { return String(n).toLowerCase() === 'host' ? '127.0.0.1:' + port : undefined; } };
    config.setOverride('oid4vci.txCodeLength', 8);
    let built = offers.buildCredentialOffer(fakeReq, ['IdentityCredential'], 'cross-device');
    config.clearOverride('oid4vci.txCodeLength');
    note(/^[1-9][0-9]{7}$/.test(built.txCode),
         '5a. the Transaction Code has oid4vci.txCodeLength digits and no leading zero', built.txCode);
    ...
    note(!!offers.preAuthorizedCodes.get(built.preAuthorizedCode),
         '5b. development: six wrong codes do NOT spend the pre-authorized code');

    config.setOverride('global.mode', 'product');
    config.setOverride('oid4vci.txCodeMaxAttempts', 3);
    built = offers.buildCredentialOffer(fakeReq, ['IdentityCredential'], 'cross-device');
    const first = await wrongOnce(built.preAuthorizedCode);
    note(first.status === 400 && /2 attempt\(s\) remain/.test(first.text),
         '5c. PRODUCT: a wrong code says how many attempts remain', first.text.slice(0, 200));
    await wrongOnce(built.preAuthorizedCode);
    const third = await wrongOnce(built.preAuthorizedCode);
    note(/has been spent/.test(third.text) && !offers.preAuthorizedCodes.get(built.preAuthorizedCode),
         '5d. PRODUCT: the wrong code that reaches oid4vci.txCodeMaxAttempts SPENDS the code',
         third.text.slice(0, 200));
    const afterSpent = await request(port, 'POST', '/oauth2/token', { form: Object.assign({
      grant_type: 'urn:ietf:params:oauth:grant-type:pre-authorized_code',
      'pre-authorized_code': built.preAuthorizedCode, tx_code: built.txCode }, client) });
    note(afterSpent.status === 400,
         '5e. and the RIGHT code afterwards is refused, because the code no longer exists',
    ...
    note(r.status === 302 && /\/authn\//.test(String(r.headers.location || '')),
         '6a. PRODUCT: a pre-authorized offer page with no session sends the browser to sign in',
         r.status + ' ' + r.headers.location);
    r = await request(port, 'GET', '/issuer/offer?wallet=' + encodeURIComponent('https://evil.hc.example'));
    note(r.status === 400 && /oid4vci\.allowedWalletUrls/.test(r.text),
         '6b. PRODUCT: an unregistered wallet URL is refused as an open redirect', r.status + ' ' + r.text.slice(0, 160));
    config.setOverride('oid4vci.allowedWalletUrls', 'https://evil.hc.example');
    r = await request(port, 'GET', '/issuer/offer?wallet=' + encodeURIComponent('https://evil.hc.example'));
    config.clearOverride('oid4vci.allowedWalletUrls');
    note(r.status === 302 && /^https:\/\/evil\.hc\.example\/vc-issuance-1\.html\?/.test(String(r.headers.location)),
         '6c. PRODUCT: a wallet URL listed in oid4vci.allowedWalletUrls is accepted', r.headers.location);
    r = await request(port, 'GET', '/oid4vp/start?wallet=' + encodeURIComponent('https://evil.hc.example'));
    note(r.status === 400 && /oid4vp\.allowedWalletUrls/.test(r.text),
         '6d. PRODUCT: the Verifier refuses an unregistered wallet URL too', r.status + ' ' + r.text.slice(0, 160));
    config.clearOverride('global.mode');
    r = await request(port, 'GET', '/issuer/offer?wallet=' + encodeURIComponent('https://evil.hc.example'));
    note(r.status === 302 && /^https:\/\/evil\.hc\.example\//.test(String(r.headers.location)),
         '6e. development: any wallet URL is still accepted', r.status + ' ' + r.headers.location);
    r = await request(port, 'GET', '/issuer/offer?mode=cross-device');
    note(r.status === 200 && /tx_code/.test(r.text),
         '6f. development: the cross-device offer page still needs no sign-in', r.status);
    ...
    note(plain.json && plain.json.id === 'did:web:127.0.0.1%3A' + port,
         '7a. the default realm\'s DID is byte-for-byte the Host-header form it always was',
         plain.json && plain.json.id);
    const realmDoc = await request(port, 'GET', '/realm/hcrealm/.well-known/did.json');
    note(realmDoc.json && realmDoc.json.id === 'did:web:127.0.0.1%3A' + port + ':realm:hcrealm',
    ...
    note(pathForm.status === 200 && pathForm.json && pathForm.json.id === (realmDoc.json || {}).id,
         '7c. and resolves at <base>/did.json, where the did:web method looks for it',
         pathForm.status);
    const bareForm = await request(port, 'GET', '/did.json');
    note(bareForm.status === 404,
         '7d. a DID with no path is NOT also served at /did.json', bareForm.status);
    ...
    applications.createApplication({ identifier: 'hc-broker', protocols: ['oauth2'] });
    applications.updateApplication('hc-broker', { attribute: 'oauthAssertionIssuer',
                                                  mode: 'add', value: ISS });
    const hierarchy = pki.hasChain() ? { ok: true } : await pki.start({});
    const issued = hierarchy.ok
      ? await pki.issueSigningKeyPair(undefined, { identifier: 'hc-broker', purpose: 'jwt' })
      : null;
    if (!issued || !issued.issued) {
      note(false, '8. a broker signing key pair was issued', JSON.stringify(hierarchy.errors || issued));
    } else {
      applications.updateApplication('hc-broker', { attribute: 'oauthAssertionJwks', mode: 'set',
                                                    value: JSON.stringify(issued.issued.jwks) });
      const sign = function (payload) {
        const alg = issued.issued.jwsAlg;
        const head = Buffer.from(JSON.stringify({ alg: alg, typ: 'JWT', kid: issued.issued.kid }))
          .toString('base64url');
        const body = Buffer.from(JSON.stringify(payload)).toString('base64url');
        const digest = /384/.test(alg) ? 'sha384' : (/512/.test(alg) ? 'sha512' : 'sha256');
        const options = /^ES/.test(alg)
          ? { key: issued.issued.privateKeyPem, dsaEncoding: 'ieee-p1363' }
          : issued.issued.privateKeyPem;
        return head + '.' + body + '.' +
          nodeCrypto.sign(digest, Buffer.from(head + '.' + body), options).toString('base64url');
      };
      const now = Math.floor(Date.now() / 1000);
      const noIat = await assertionGrant.verify({ audiences: [AUD], assertion: sign({
        iss: ISS, sub: 'hc-someone', aud: AUD, exp: now + 86000, jti: 'hc-noiat' }) });
      note(!noIat.ok && /oauth2\.jwtBearerMaxLifetimeS/.test(noIat.description || ''),
           '8a. a day-long grant assertion with NO iat is refused by the ceiling (it used to ' +
           'skip it entirely)', noIat.description);
      const shortNoIat = await assertionGrant.verify({ audiences: [AUD], assertion: sign({
        iss: ISS, sub: 'hc-someone', aud: AUD, exp: now + 60, jti: 'hc-short' }) });
      note(shortNoIat.ok, '8b. while a short one with no iat is accepted — the control',
    ...
    findings.push({ ok: false, what: 'the child ran to the end', detail: e && e.stack });
    require('fs').writeFileSync(OUT, JSON.stringify(findings));
    process.exit(0);
  });
}
...
function inAChild(t) {
  ...
  const result = childProcess.spawnSync(process.execPath,
    ['-e', '(' + childMain.toString() + ')()'], {
      env: Object.assign(clean, { LOG_LEVEL: 'fatal', HC_ROOT: ROOT, HC_OUT: out }),
...
async function clientAssertions(t) {
  t.log.info('=== client assertions: exp, the lifetime ceiling, a full replay cache ===');
  ...
  const verify = function (claims) {
    ...
    return clientAuth.verify({
      ...
      assertion: hs256(SECRET, Object.assign({ iss: 'hc-assert', sub: 'hc-assert', aud: AUD,
                                               jti: 'hc-' + process.pid + '-' + n }, claims))
    });
  };
  try {
    let v = await verify({ iat: now });
    t.check(v.ok, 'development: a client assertion with NO exp is still accepted', v.description);
    v = await verify({ iat: now, exp: now + 3600 });
    t.check(v.ok, 'development: an hour-long client assertion is still accepted — the ' +
            'parent suite signs its post-quantum ones for an hour', v.description);

    config.setOverride('global.mode', 'product');
    v = await verify({ iat: now });
    t.check(!v.ok && /MUST carry an `exp`/.test(v.description || ''),
            'PRODUCT: a client assertion with no exp is refused (RFC 7523 section 3 claim 4)',
            v.description);
    v = await verify({ iat: now, exp: now + 3600 });
    t.check(!v.ok && /oauth2\.jwtBearerMaxLifetimeS/.test(v.description || ''),
            'PRODUCT: a client assertion longer than oauth2.jwtBearerMaxLifetimeS is refused',
            v.description);
    v = await verify({ exp: now + 3600 });
    t.check(!v.ok && /oauth2\.jwtBearerMaxLifetimeS/.test(v.description || ''),
            'PRODUCT: and leaving out iat does not get round the ceiling — it is measured from now',
            v.description);
    v = await verify({ iat: now, exp: now + 60 });
    t.check(v.ok, 'PRODUCT: a short-lived client assertion is accepted', v.description);
    config.clearOverride('global.mode');
    ...
    t.check(lastOk !== null && refused !== null &&
            ...
            'a replay cache full of UNEXPIRED client assertions refuses the next one rather ' +
            'than forgetting a live one', refused && refused.description);
  } finally {
...
function personaGate(t) {
  ...
  try {
    ...
    t.check(dev.report.some(function (row) { return row.source === 'generated'; }),
            'development: a person with no entry is given generated claim values',
            dev.report.length + ' row(s)');
    config.setOverride('global.mode', 'product');
    const product = vcClaims.subjectClaimsFor(nobody, {});
    t.equal(product.report.filter(function (row) { return row.source === 'generated'; }).length, 0,
            'PRODUCT: no claim value is generated for a signed credential');
    t.equal(Object.keys(vcClaims.generatedFor(nobody)).length, 0,
            'PRODUCT: and the populate sweep is handed nothing to write onto an entry');
  } finally {
    config.clearOverride('global.mode');
  }
}
...
function dpopWindows(t) {
  t.log.info('=== DPoP: the windows are settings, and the export stays the default ===');
  const config = require('../common/config');
  const dpop = require('../oauth-oidc/dpop');
  try {
    t.equal(dpop.IAT_SKEW_SECONDS, 300, 'the old export still names the default');
    config.setOverride('oauth2.dpopIatSkewS', 42);
    t.equal(dpop.state().iat_skew_seconds, 42, 'the live window follows oauth2.dpopIatSkewS');
  } finally {
...
function didParts(t) {
  ...
  t.equal(deep.host + '|' + deep.segments.join('/'), 'idp.example.com|sts/realm/acme',
...
module.exports = {
  ...
  describe: 'the 2026-09-12 sweep: password grant, directory profile claims, ' +
            'Transaction Code, replay caches, DPoP switch, registration, did:web',
  run: run
};
