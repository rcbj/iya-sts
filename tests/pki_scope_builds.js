...
// first was "not signed by" an Issuing CA of the same name. The two callers were
// ordinary: `realms.create()` fires the module's realm watcher, which builds
// without being awaited, and the code that created the realm asks for the branch
// next. `tests/revocation_status.js` met it under `npm test` and had to move into
// a child process to get away from it.
//
...
//   2. Concurrent `ensureScope()` calls produce ONE branch: every caller reports
//      the same Intermediate, exactly one of them built it, and it is still the
//      branch the store holds afterwards. A deliberate `buildScope()` queued
//      behind them replaces it, which is what it is for.
...
function childScript() {
  return [
    ...
    "const realms = require(" + JSON.stringify(path.join(ROOT, 'common/realms')) + ");",
    ...
    "});"
                       ...
                       "delete process.env.CONFIG_FILE; const fs = require('fs');");
}

function run(t) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'pki-scope-builds-'));
  const outFile = path.join(dir, 'out.json');
  const env = Object.assign({}, process.env, { LOG_LEVEL: 'fatal', PROBE_OUT: outFile });
  ...
  t.equal(out.rootBuiltCount, 1, 'exactly one of them built a Root', JSON.stringify(out.rootSerials));
  t.check(out.rootSerials.every(function (s) { return s && s === out.rootSerials[0]; }),
          'and all three were told about the SAME Root', JSON.stringify(out.rootSerials));

  t.log.info('=== 2. four callers, one realm ===');
  t.check(out.realmOk && out.branchesOk, 'the realm exists and every ensureScope() answered ok');
  t.equal(out.branchBuiltCount, 1, 'exactly one caller built the branch; the other three found it',
          JSON.stringify(out.branchSerials));
  t.check(out.branchSerials.every(function (s) { return s && s === out.branchNow; }),
          'every caller reported the Intermediate the store STILL holds — none was replaced ' +
          'under the caller that made it', JSON.stringify({ reported: out.branchSerials, now: out.branchNow }));
  t.check(out.issuingReported.every(function (list) {
            return JSON.stringify(list) === JSON.stringify(out.issuingNow);
          }),
          'and the same for every Issuing CA under it, which is what a leaf is signed by');
  t.equal(out.queuedEnsureSaw, out.branchNow,
          'an ensureScope() queued beside a deliberate rebuild answers the branch that was there');
  t.check(out.rebuiltTo && out.rebuiltTo !== out.branchNow,
          'and the deliberate buildScope() behind it still REPLACES the branch, which is what it is for',
          JSON.stringify({ before: out.branchNow, after: out.rebuiltTo }));

  t.log.info('=== 3. the realm watcher ===');
  t.equal(out.createdBuilt, true, 'a realm created in this process gets its branch from the watcher');
  t.equal(out.restoredBuilt, false,
          'a realm that arrived RESTORED — from the store or another process — does not: that ' +
          'branch is the creating process\'s to build, and building it here too was the same race ' +
          'across processes');
}

module.exports = {
  name: 'pki_scope_builds',
  describe: 'a realm\'s certificate branch and the service Root are built once however many callers ask at once',
  run: run
};
