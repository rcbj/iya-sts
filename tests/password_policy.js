...
function checkThePolicy(t, who) {
  ...
  t.check(devShort.ok,
          ...
          'nothing there verifies one, so a rule about it would be a rule about ' +
          'a credential nothing reads', JSON.stringify(devShort.errors));
  t.check(credentials.setPassword(who, 'x').ok,
          'and sets the SAME password again: history is not enforced there either');
  t.check(attributeOf(who, 'pwdHistory').length === 1 &&
          ...
          'but the history IS RECORDED, in draft-behera\'s time#oid#length#data ' +
          'form holding the previous HASH — so a realm switched to product mode ' +
          'starts with one', attributeOf(who, 'pwdHistory')[0]);
  ...
  withSettings({ 'global.mode': 'product' }, function () {
    ...
    t.check(!credentials.setPassword(who, 'Abcdefghij1').ok,
            'eleven characters with an uppercase letter, a digit and no symbol ' +
            'is refused');
    t.equal(credentials.passwordProblem('Abcdefghijk1'),
            'That password does not meet this realm\'s password policy, which ' +
            'asks for at least 1 symbol (it has 0). The rules are on ' +
    ...
    t.check(credentials.passwordProblem('ΑβγδεζηθικΛ1!') === '',
            'Greek uppercase is an uppercase letter: the classes are Unicode\'s, ' +
            'not ASCII\'s');
    t.check(credentials.passwordProblem('😀😀😀😀😀😀😀😀😀😀A1') === '' &&
            credentials.passwordProblem('😀😀😀😀😀A1') !== '',
            'twelve code points are twelve characters — an emoji is one, and it ' +
            'is a symbol — and seven are seven, not fourteen UTF-16 units');
    ...
    t.check(!again.ok && again.reason === 'password-history',
            'the CURRENT password is refused as a new one', JSON.stringify(again));
    ...
    t.equal(attributeOf(who, 'pwdHistory').length, 5,
            'five previous passwords are remembered under the default of five, ' +
            'and no more — the oldest fall off');
    ...
    t.check(!partial.ok && partial.errors.length === 5,
            'a save that leaves fields out is refused by NAME for each, rather ' +
            'than resetting them — a save that quietly loosened a rule nobody ' +
            'mentioned is the mistake nobody sees', JSON.stringify(partial.errors));
    ...
    t.check(!passwordPolicy.save('default', Object.assign({},
              passwordPolicy.DEFAULTS, { minSymbols: 12, generatedLength: 20 })).ok,
            'a generated length that cannot reliably carry the symbol count is ' +
            'refused');
    ...
    t.check(stored.stored && /^cn=default,ou=passwordPolicies,/.test(stored.dn) &&
            stored.sources.minLength === 'directory' && stored.minLength === 8 &&
    ...
    t.check(['pwdMinLength', 'pwdInHistory', 'stsPwdMinSymbols',
            ...
            'and the entry is handed out in the schema\'s own spelling rather ' +
            'than the lower case the store keeps — which is what an ldapsearch ' +
            'and /admin/ldap/directory show, and what learnName() is given the ' +
            'schema for', entryNames.join(', '));
    t.check(credentials.setPassword(who, 'eightch1').ok,
            'the stored profile is what is ENFORCED: eight characters, a digit, ' +
            'nothing else');
    ...
    try {
      ...
      t.equal(weak.failure && weak.failure.code, 19,
              ...
              'CONSTRAINT VIOLATION (19), which is what a ppolicy server answers',
              weak.failure && weak.failure.message);
      t.equal(attributeOf('password-policy-ldap-probe', 'userPassword').length, 0,
              'and the modify is atomic: nothing was written');
      const strong = asOperator(probe.dn,
                                [['replace', 'userPassword', 'Ldap-Passw0rd!']]);
      t.check(!strong.failure && strong.ended, 'a password that meets it is accepted',
              strong.failure && strong.failure.message);
      const hashed = attributeOf('password-policy-ldap-probe', 'userPassword')[0];
      t.check(/^\$scrypt\$/.test(hashed) &&
              crypto.verifySecret('Ldap-Passw0rd!', hashed),
              'and STORED AS A HASH — this door used to write the value verbatim, ' +
              'in the clear', String(hashed).slice(0, 20));
      t.check(credentials.verify('password-policy-ldap-probe',
                                 'Ldap-Passw0rd!').ok,
              'which the sign-in verifier accepts, so an LDAP-set password works');
      const unchanged = asOperator(probe.dn, [['replace', 'userPassword', hashed]]);
      t.check(!unchanged.failure,
              'writing the stored HASH back unchanged is not a password change, ' +
              'and is allowed even though a hash is otherwise refused',
              unchanged.failure && unchanged.failure.message);
      const reuse = asOperator(probe.dn,
                               [['replace', 'userPassword', 'Ldap-Passw0rd!']]);
      t.equal(reuse.failure && reuse.failure.code, 19,
              'but the same password sent in the CLEAR is a change to the current ' +
              'password, and is refused as one');
      ...
      t.equal(back.failure && back.failure.code, 19,
              'and the first one, sent in the clear again, meets its own HISTORY ' +
              'through the socket', back.failure && back.failure.message);
      const history = asOperator(probe.dn, [['delete', 'pwdHistory']]);
      t.equal(history.failure && history.failure.code, 19,
              'pwdHistory cannot be deleted over LDAP in product mode — a history ' +
              'anybody can empty is not a history');
      ...
      t.equal(prehashed.failure && prehashed.failure.code, 19,
              ...
              'against a policy');
      const two = asOperator(probe.dn, [['add', 'userPassword', 'Another-Passw0rd!']]);
      t.equal(two.failure && two.failure.code, 19,
              'an ADD of a second userPassword is refused: two values would be an ' +
              'entry where the old password still works');
    } finally {
  ...
  t.check(!ldapModify(devProbe.dn, [['replace', 'userPassword', kept]]).failure &&
  ...
  t.check(!ldapModify(devProbe.dn, [['replace', 'userPassword', 'x']]).failure &&
          /^\$scrypt\$/.test(
            attributeOf('password-policy-ldap-dev', 'userPassword')[0]),
          'and a clear value is still HASHED there — storing a password in the ' +
          'clear is a storage defect, not a permissiveness');
  ...
  const made = adminActions.usersAction({ action: 'create',
                                          ...
                                          invent: 'no' }, { via: 'api' });
  t.check(made.ok && made.credential === 'generate' && made.generated === true &&
    ...
    return withSettings({ 'security.passwordHashLogN': 14 }, function () {
      return crypto.hashSecretAsync('async-cheaper').then(function (cheaper) {
        t.check(/^\$scrypt\$16384\$8\$1\$/.test(cheaper) &&
                ...
                'and the job the worker pool is handed carries the configured ' +
                'parameters, encoded as the ones the derivation really used',
                cheaper.slice(0, 24));
      });
    });
  });
...
module.exports = {
  ...
  describe: 'a minimum password length enforced where a password is verified, ' +
            'and the scrypt cost of a NEW hash as settings that leave every ' +
            'stored hash verifying',
  run: run
};
