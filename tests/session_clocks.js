...
function noBrowser() {
  ...
  return {
    headers: headers,
    set: function (name, value) { headers.push(value); },
    req: null
  };
}
...
function run(t) {
  ...
  withSettings({ 'authn.sessionLifetimeS': 120 }, function () {
    const short = authn.startSession(noBrowser(), 'clock-bob', ['pwd'], '1', 'Test');
    ...
    t.check(near(plain.expires, plain.authTime * 1000 + 3600000, 10000),
            'and leaves a session that already exists with the lifetime it was ' +
            'issued with');
    ...
    const touched = authn.startSession(noBrowser(), 'clock-scim', ['pwd'], '1',
                                       'SCIM', { key: 'k-clock', cookie: false });
    t.check(touched.id === keyed.id && near(touched.expires, Date.now() + 120000),
            'and an API session touched by a call is extended by the SETTING, ' +
            'not by the hour it used to be');
  });

  // -----------------------------------------------------------------------
  t.log.info('=== 2. the idle timeout: zero is none, and it is checked at read ===');
  ...
  t.check(!!authn.sessionOf(reqWith({ sts_session: cookie })),
          'so a session unused for two hours is still honoured, as it always was');
  t.check(authn.sessions.get(plain.id).lastSeenAt < Date.now() - 3600000,
          'and reading it wrote NOTHING — with no idle timeout a read is not a ' +
          'write to a persisted store');
  ...
  withSettings({ 'authn.sessionIdleTimeoutS': 60 }, function () {
    t.equal(authn.sessionEnded({ expires: Date.now() + 60000, chosen: false,
            ...
            'an ARRIVAL session is exempt — it has an inactivity window of its ' +
            'own on the sign-in screen\'s clock');
    ...
    const fresh = authn.startSession(noBrowser(), 'clock-carol', ['pwd'], '1', 'Test');
    ...
    t.equal(authn.sessionEnded(freshStored), '',
            'a session used thirty seconds ago is live under a sixty-second timeout');
    ...
    const parent = authn.startSession(noBrowser(), 'clock-dave', ['pwd'], '1', 'Test');
    ...
    t.check(Date.now() - authn.sessions.get(parent.id).lastSeenAt < 5000,
            'and reading it TOUCHED the sign-on session it came from — without ' +
            'that, somebody working in the console presents only the console\'s ' +
            'cookie, the sweep idles the parent out, and the cascade signs them ' +
            'out of the page they are using');
  });
  ...
  withSettings({ 'authn.pendingTtlS': 45, 'authn.mfaStepTtlS': 40 }, function () {
    const url = authn.beginAuthentication({ returnTo: '/admin', protocol: 'Test' });
    ...
    t.equal(authn.mfaStepTtlMs(), 40000, 'and the step clock follows its setting');
  });
  ...
  withSettings({ 'authn.sessionLifetimeS': 1800,
    ...
    t.check(/30 minutes after the\s+last call/.test(logout.SESSION_EXPIRY_RULES.api),
  ...
  const erin = authn.startSession(noBrowser(), 'clock-erin', ['pwd'], '1', 'Test');
  const erinRes = noBrowser();
  const erinAgain = authn.startSession(erinRes, 'clock-erin', ['pwd'], '1', 'Test');
  const erinCookie = cookieOf(erinRes, authn.SESSION_COOKIE);
  t.check(!!erin && !!erinAgain && !!erinCookie, 'a signed-in person to sign out');

  const devNamed = logout.subjectOf(reqWith({}, { username: 'clock-frank' }), null);
  t.check(devNamed.named && devNamed.username === 'clock-frank',
          'development mode, anonymous, naming somebody: honoured as it always ' +
          'was — nothing checks a password there', JSON.stringify(devNamed));

  withSettings({ 'global.mode': 'product' }, function () {
    const anonymous = logout.subjectOf(reqWith({}, { username: 'clock-frank' }), null);
    t.check(anonymous.refused && !anonymous.key,
            'PRODUCT mode, anonymous, naming somebody: REFUSED — this was an ' +
            'unauthenticated way to end any person\'s sessions and revoke their ' +
            'tokens', JSON.stringify(anonymous));
    const other = logout.subjectOf(
      reqWith({ sts_session: erinCookie }, { username: 'clock-frank' }), null);
    t.check(other.refused,
            'and a signed-in person naming SOMEBODY ELSE is refused too — the ' +
            'operator\'s door is /admin/logout', JSON.stringify(other));
  ...
  withSettings({ 'logout.anyUser': 'false' }, function () {
    t.check(logout.subjectOf(reqWith({}, { username: 'clock-frank' }), null).refused,
            'and in development logout.anyUser=false still closes it, as before');
  });
}

module.exports = {
  name: 'session_clocks',
  describe: 'the session lifetime and idle timeout as settings checked where a ' +
            'session is read, the two waiting clocks, the sentence a session ' +
            'row carries, and an anonymous /logout?username= refused in product',
  run: run
};
