...
//
// ---------------------------------------------------------------------------
// WHO MAY WRITE THE DIRECTORY OVER ITS OWN SOCKET, IN PRODUCT MODE (2026-09-12).
//
...
//
// **THE ASSERTION THAT MATTERS MOST IS THE ESCALATION.** `admin-ui/admin_rbac.js`
// reads a person's own `memberOf` when it decides whether they hold a console
// role, so before this change `memberOf: cn=admin-write,…` written on your own
// entry made you an administrator of the service. Section 3 writes exactly that
// and then asks the role check, rather than trusting the refusal alone.
//
...
function del(boundDn, dn) {
  return ldap.performOperation('del', { dn: dn, boundDn: boundDn, channel: 'ldaps' });
}
...
function refusedWith50(t, result, code, target, what) {
  t.check(result.ok === false && result.errorName === 'InsufficientAccessRightsError',
          what + ' — refused with insufficientAccessRights (50)',
          JSON.stringify({ ok: result.ok, errorName: result.errorName, error: result.error }));
...
function run(t) {
  ...
  try {
    ...
    const devWrite = modify('', otherDn, [['replace', 'mail', 'dev@example.test']]);
    t.check(devWrite.ok === true,
            'in development an anonymous connection may still modify somebody else\'s entry',
            JSON.stringify(devWrite));

    product(function () {
      // ---------------------------------------------------------------------
      t.log.info('=== 2. the empty-roster rule does not make anybody an administrator here ===');
      const roles = adminRbac.rolesOf(PERSON);
      t.check(roles.write === true && roles.open === true,
              'precondition: with no role group member, the console treats this person ' +
              'as holding Admin Write',
              JSON.stringify({ write: roles.write, open: roles.open }));
      refusedWith50(t, modify(personDn, otherDn, [['replace', 'mail', 'x@example.test']]),
                    'STS-LDAP-0053', otherDn,
                    'and still may not modify somebody else\'s entry over LDAP');

      // ---------------------------------------------------------------------
      t.log.info('=== 3. anonymous, and the escalation through memberOf ===');
      refusedWith50(t, modify('', personDn, [['replace', 'telephoneNumber', '1']]),
                    'STS-LDAP-0052', personDn,
                    'an anonymous connection may not modify even an allowlisted attribute');
      ...
      const own = modify(personDn, personDn,
        [['replace', 'telephoneNumber', '+1 555 0100'], ['replace', 'displayName', 'Probe']]);
      t.check(own.ok === true, 'a person may change allowlisted attributes on their own entry',
              JSON.stringify(own));
      t.equal(attributeOf(PERSON, 'telephoneNumber').join(), '+1 555 0100',
              'and the value is stored');
      const mixed = modify(personDn, personDn,
        [['replace', 'telephoneNumber', '+1 555 0199'], ['replace', 'mail', 'me@example.test']]);
      refusedWith50(t, mixed, 'STS-LDAP-0054', personDn,
                    'a modify mixing an allowlisted attribute with mail');
      t.equal(attributeOf(PERSON, 'telephoneNumber').join(), '+1 555 0100',
              'and it is atomic: the allowlisted change in it was not applied either');
      withSettings({ 'ldap.selfWritableAttributes': 'mail' }, function () {
        const widened = modify(personDn, personDn, [['replace', 'MAIL', 'me@example.test']]);
        t.check(widened.ok === true,
                'the list is the setting, matched case-insensitively: naming mail lets it through',
                JSON.stringify(widened));
        refusedWith50(t, modify(personDn, personDn, [['replace', 'telephoneNumber', '2']]),
                      'STS-LDAP-0054', personDn,
                      'and an attribute the edited list no longer names is refused');
      });

      // ---------------------------------------------------------------------
      t.log.info('=== 5. a person may not add, delete or rename, nor touch another entry ===');
      refusedWith50(t, modify(personDn, otherDn, [['replace', 'telephoneNumber', '3']]),
                    'STS-LDAP-0053', otherDn, 'a person modifying somebody else\'s entry');
      const newDn = dnOf('authz-new-' + STAMP);
      refusedWith50(t, add(personDn, newDn, { objectClass: ['top', 'person'], sn: 'x' }),
      ...
      refusedWith50(t, rename(personDn, personDn, 'uid=renamed-' + STAMP),
                    'STS-LDAP-0053', personDn, 'a person renaming their own entry');
      t.check(!!ldap.objectFor(PERSON), 'and their entry is still where it was');
      ...
      try {
        const weak = modify(personDn, personDn, [['replace', 'userPassword', 'short']]);
        t.check(weak.ok === false && weak.errorName === 'ConstraintViolationError',
                'a self-service password that breaks the policy is still a constraint ' +
                'violation, not an authorization refusal', JSON.stringify(weak));
        t.equal(announced.length, 0, 'and a refused password is announced to nobody');
        const strong = modify(personDn, personDn,
          [['replace', 'userPassword', 'Self-Service-Passw0rd!' + STAMP]]);
        t.check(strong.ok === true, 'a person may set their own password over LDAP',
        ...
        t.check(announced.length === 1 && announced[0].name === PERSON &&
                ...
                'and it is announced once, after the commit, for the Kerberos key register',
      ...
      t.check(granted, 'precondition: the probe administrator was granted Admin Write',
              JSON.stringify(grant));
      const adminModify = modify(adminDn, otherDn, [['replace', 'mail', 'set@example.test']]);
      t.check(adminModify.ok === true, 'an administrator may modify somebody else\'s mail',
              JSON.stringify(adminModify));
      const adminAdd = add(adminDn, newDn, { objectClass: ['top', 'person'], sn: 'x' });
      t.check(adminAdd.ok === true, 'an administrator may add an entry', JSON.stringify(adminAdd));
      ...
      refusedWith50(t, modify(personDn, otherDn, [['replace', 'mail', 'y@example.test']]),
                    'STS-LDAP-0053', otherDn,
                    'a person is still refused once the roster has a member');

      // ---------------------------------------------------------------------
      t.log.info('=== 7. the same name in another realm is not an administrator ===');
      const created = realms.create({ id: REALM, name: REALM });
      realmMade = created && created.ok !== false;
      t.check(realmMade, 'precondition: a throwaway realm was created', JSON.stringify(created));
      ...
      t.check(foreignDn !== adminDn && /dc=authz-/.test(foreignDn),
              'precondition: that realm holds an entry with the administrator\'s name',
              foreignDn);
      refusedWith50(t, modify(foreignDn, otherDn, [['replace', 'mail', 'z@example.test']]),
                    'STS-LDAP-0053', otherDn,
                    'a DN in another realm bearing an administrator\'s name writes nothing in the default');
    });
  } finally {
    if (granted) {
...
module.exports = {
  ...
  describe: 'product mode: who may add, modify, rename or delete a directory entry over LDAP',
  run: run
};
