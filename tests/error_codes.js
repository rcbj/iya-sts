...
const VENDORED_FILES = ['kerberos/krb5_asn1.js', 'kerberos/krb5_crypto.js',
                        ...
                        'kerberos/krb5_primitives.js', 'kerberos/krb5_spnego.js'];
...
const FAILURE_PATTERNS = [
  ...
  { re: /\bgnapError\(\s*res\b/, before: 4, after: 2,
    what: 'the GNAP gnapError(res, status, code, description) error response (RFC 9635 section 3.6)' },
  ...
  { re: /\b(?:rejection|bareVerdict|tokenVerdict)\(\s*door\b/, before: 2, after: 12,
    what: 'a SPNEGO verdict that is not an acceptance' },
  // ----- OAUTH -----------------------------------------
  { re: /\breturn fail\(/, before: 4, after: 2,
    what: 'the authorization endpoint\'s fail() and the DPoP verifier\'s fail()' },
  ...
  { re: /\bsendKeysPage\(\s*res\s*,\s*[45]\d\d\b/, before: 4, after: 2,
    what: 'the portal\'s sendKeysPage(res, status, html) with a failure status' },
  ...
  { re: /status: [45]\d\d, entry:/, before: 4, after: 2,
    what: 'an internal SSF receiver refusing a push (ssf_receivers.js accept())' },
  // ----- STORE -----------------------------------------
...
function isExemptShape(line) {
  ...
  if (/^(?:async\s+)?function\s+[A-Za-z_$][\w$]*\s*\(/.test(trimmed)) return true;
  return false;
}
...
function checkTable(t) {
  ...
  errorCodes.CODES.forEach(function (row) {
    ...
    if (!row.summary || String(row.summary).trim().length < 12) noSummary.push(row.code);
  ...
  t.check(!noSummary.length, 'every code has a summary a person can read',
          listed(noSummary));
  t.check(!unknownSubsystem.length, 'every code belongs to a declared subsystem',
          listed(unknownSubsystem));
  t.check(!duplicates.length, 'no code is registered twice — a duplicate is two ' +
          'conditions sharing one name', listed(duplicates));
  ...
  t.check(!disorder.length, 'the table is in subsystem order and ascending within ' +
          'each subsystem', listed(disorder));
  const ids = {};
...
function checkOffTheWire(t) {
  ...
  app.use(function (req, res, next) {
    res.on('finish', function () {
      audit.recordHttp(req, res, { route: '/probe', matched: true, durationMs: 0 });
    });
    next();
  });
    ...
    const server = app.listen(0, '127.0.0.1', function () {
      ...
      get('/probe').then(function (probe) {
        ...
        t.check(probe.body === JSON.stringify({ error: 'invalid_request',
                                                error_description: 'a probe refusal' }),
                'the body is exactly what the protocol helper sends', probe.body);
        ...
        const probeRow = rows.filter(function (r) { return r.target === '/probe'; })[0];
        ...
        t.check(!!probeRow && probeRow.summary.indexOf('[STS-HTTP-0002]') === 0,
                'the row\'s summary leads with the code', probeRow && probeRow.summary);
...
function checkFallbacks(t) {
  ...
  t.equal(JSON.stringify(res), '{}',
          'a mark is not enumerable, so serialising the response object shows none');
}
...
function run(t) {
  ...
  t.log.info('=== every code used is registered, and every registered code used ===');
  const result = scan();
  t.check(!result.unregistered.length, 'every code in the source is in the table',
          listed(result.unregistered));
  t.check(!result.unused.length, 'every code in the table is raised somewhere ' +
          '(retire a row rather than leaving it unused)', listed(result.unused));
  ...
  t.check(page === errorCodes.markdown(), 'docs/error-codes.md is exactly what the ' +
          'table generates', page ? 'stale — run: node common/error_codes.js --docs'
                                  : 'missing — run: node common/error_codes.js --docs');
  const config = fs.readFileSync(path.join(ROOT, 'docs', '_config.yml'), 'utf8');
  t.check(config.indexOf('error-codes.md') >= 0,
          'the page is in the site navigation', 'docs/_config.yml header_pages');
...
module.exports = {
  name: 'error_codes',
  describe: 'every failure has a registered, documented error code that never reaches a client',
  run: run,
