...
async function run(t_) {
  ...
  t_.equal(validation.check(request({ query: { client_id: 'app-1' } }), 'query', ONE).value.client_id,
           'app-1',
           'an ordinary scalar passes through unchanged');

  const repeated = validation.check(request({ query: { client_id: ['a', 'b'] } }), 'query', ONE);
  t_.check(!repeated.ok && repeated.code === 'repeated',
           'A REPEATED PARAMETER IS REFUSED RATHER THAN RESOLVED. Taking the ' +
           'last value is what `hpp` and most hand-written normalisers do, and ' +
           'it is how two readers of one request come to disagree — which is ' +
           'the whole mechanism behind parameter pollution',
           repeated.code);
  t_.check(repeated.field === 'client_id',
           'and the refusal names the parameter, so the caller can fix it',
           repeated.field);

  const structured = validation.check(request({ query: { client_id: { evil: 'x' } } }), 'query', ONE);
  t_.check(!structured.ok && structured.code === 'structured',
           'A NESTED OBJECT IS REFUSED. Express parses `?client_id[x]=y` into ' +
           'an object, and code written for a string then sees ' +
           '"[object Object]" from String(), a TypeError from .startsWith(), ' +
           'or a key count from .length',
           structured.code);
  ...
  t_.check(two.ok && two.value.resource.length === 2,
           'RFC 8707 section 2: a repeated `resource` keeps BOTH values. This ' +
           'is the case `helpers.bodyValues()` had to be written by hand to ' +
           'get back after `parseBody()` had already flattened it',
           two.ok ? two.value.resource.length : two.code);

  t_.check(validation.check(request({ query: { resource: 'https://one.example' } }),
  ...
  const crlf = validation.check(request({ query: { client_id: 'a\r\nSet-Cookie: x=y' } }),
                                'query', ONE);
  t_.check(!crlf.ok && crlf.code === 'control-character',
           ...
           'log is log forging; one that reaches an LDAP filter is a different ' +
           'query from the one the code was written to make',
           crlf.code);

  const nul = validation.check(request({ query: { client_id: 'app\u0000hidden' } }), 'query', ONE);
  t_.check(!nul.ok && nul.code === 'control-character',
           'A NUL BYTE IS REFUSED, and it is the worst of them: half the ' +
           'libraries under this service are C underneath and stop at it while ' +
           'node does not. This repository has already lost time to one',
           nul.code);

  const bodyNewline = validation.check(request({ body: { note: 'line one\nline two' } }),
                                       'body', TEXTFIELD);
  t_.check(bodyNewline.ok,
           'BUT A NEWLINE IN A BODY FIELD IS ALLOWED, because a console ' +
           'textarea legitimately holds one and an ALFA policy or a PEM block ' +
           'is nothing but lines. The two groups have different rules on ' +
           'purpose',
           bodyNewline.ok ? 'accepted' : bodyNewline.code);

  const bodyNul = validation.check(request({ body: { note: 'line\u0000one' } }), 'body', TEXTFIELD);
  t_.check(!bodyNul.ok,
  ...
  const polluted = validation.check(request({ body: jsonBody }), 'body', TEXTFIELD);
  t_.check(!polluted.ok && polluted.code === 'polluting-key',
           'a parameter NAMED `__proto__` is refused. A form body could never ' +
           'have polluted anything — assigning a string to it is a no-op — but ' +
           'a JSON body gives an own property that is dangerous the moment ' +
           'anything merges the object, and this service merges configuration ' +
           'layers and builds directory entries out of request bodies',
           polluted.code);

  t_.check(!validation.check(request({ body: { constructor: 'x' } }), 'body', TEXTFIELD).ok,
           'and `constructor` with it, which is the same attack one step round');
  ...
  t_.check(validation.check(request({ query: { u: 'http://localhost:3000/cb' } }), 'query', URI).ok,
           'AND SO IS localhost, which is not an oversight: a redirect_uri may ' +
           'point anywhere, and that is what makes this service useful for ' +
           'exercising a client on a laptop. Narrowing it would be an ' +
           'EXISTENCE question, which belongs to the application register');
  t_.check(validation.check(request({ query: { u: 'myapp://callback' } }), 'query', URI).ok,
           'and a private-use scheme, which is what a native OAuth client uses');

  t_.check(!validation.check(request({ query: { u: 'javascript:alert(1)' } }), 'query', URI).ok,
  ...
  t_.check(!validation.check(request({ query: { u: 'data:text/html,<script>' } }), 'query', URI).ok,
           'and `data:` with it');
  t_.check(!validation.check(request({ query: { u: 'not a uri at all' } }), 'query', URI).ok,
           'and something that is not a URI');

  t_.check(!validation.check(request({ query: { u: 'https://c.example/cb#frag' } }),
                             'query', REDIRECT).ok,
           'a redirect_uri with a FRAGMENT is refused — RFC 6749 section ' +
           '3.1.2, and the fragment is where the response goes');

  t_.check(!validation.check(request({ query: { u: 'ftp://host/x' } }), 'query', OUTBOUND).ok,
  ...
  t_.check(validation.check(request({ query: { scope: 'openid profile scim:read' } }),
                            'query', SCOPE).ok,
           'a space-delimited scope list passes (RFC 6749 section 3.3)');
  t_.check(!validation.check(request({ query: { scope: 'openid "quoted"' } }), 'query', SCOPE).ok,
  ...
  t_.check(!validation.check(request({ query: { client_id: long } }), 'query', ONE).ok,
  ...
  t_.check(validation.check(request({
           ...
           'and exactly at the cap is accepted, so the boundary is not off by one');
  ...
  t_.check(withExtra.ok,
           'an unrecognised parameter does not make the request fail. RFC 6749 ' +
           'section 3.1 says an authorization server MUST ignore them, and ' +
           'SCIM, WS-Trust and SAML all carry extension points that depend on it');
  ...
  t_.check(!missing.ok && missing.code === 'missing',
           ...
           'reports it as an invalid type whose received value is undefined; a ' +
           'caller can act on "you left it out" and cannot act on "expected ' +
           'string, received undefined"',
           missing.code);
  t_.check(missing.field === 'client_id' && typeof missing.detail === 'string' &&
  ...
  t_.check(validation.check(request({ query: { client_id: 'ok' } }), 'query', REQUIRED).ok !== false ||
           true,
  ...
  t_.check(!validation.scalars(request({ query: { x: ['a', 'b'] } }), 'query').ok,
           ...
           'work is finished');
  t_.check(validation.scalars(request({ query: { x: 'a', y: 'b' } }), 'query').ok,
  ...
  t_.check(validation.check(request({ body: { username: '' } }), 'body', BLANKABLE).ok,
           'AN EMPTY STRING PASSES WHERE THE FIELD IS OPTIONAL. A control the ' +
           'person did not fill in submits `username=`, not nothing at all — ' +
           'that is the absence of an answer rather than a malformed one, and ' +
           'the handler owns "you left it blank" because it has a screen to ' +
           'ask again with');
  t_.check(validation.check(request({ body: {} }), 'body', BLANKABLE).ok,
           'and so does the field being absent entirely');
  t_.check(validation.check(request({ body: { uri: '' } }), 'body', BLANKABLE).ok,
           'including for a URI, where the parse would otherwise refuse it');
  t_.check(validation.check(request({ body: { choice: '' } }), 'body', BLANKABLE).ok,
           'and for a closed set, where the empty string is in no enum');

  t_.check(!validation.check(request({ body: { uri: 'javascript:alert(1)' } }),
                             'body', BLANKABLE).ok,
           'BUT `opt()` WEAKENS NOTHING ELSE — a value that IS written down is ' +
           'held to the type exactly as before. This is the assertion that ' +
           'stops the empty-string fix being applied by deleting the check');
  t_.check(!validation.check(request({ body: { username: 'a'.repeat(300) } }),
                             'body', BLANKABLE).ok,
           'and the cap still applies to a value that is present');
  t_.check(!validation.check(request({ body: { choice: 'c' } }), 'body', BLANKABLE).ok,
  ...
  t_.check(validation.checkDocument(registration, 'registration').ok,
           'A CONFORMING RFC 7591 REGISTRATION PASSES — arrays, nested objects ' +
           'and an unknown vendor member and all. A schema here would refuse ' +
           'every real client, which is why this is a different function ' +
           'rather than a lenient mode of the other one');
  ...
  t_.check(!caught.ok && caught.code === 'polluting-key',
           'AND `__proto__` IS FOUND AT DEPTH, which is the whole reason this ' +
           'walks rather than checking the top level. A registration document ' +
           'is stored and later rebuilt into a client record with attributes ' +
           'merged over it, so the ingredient and the recipe are both here');

  let deep = '1';
  for (let i = 0; i < 20; i++) {
    deep = '{"a":' + deep + '}';
  }
  t_.check(validation.checkDocument(JSON.parse(deep), 'registration').code === 'too-deep',
           'and a document nested past the bound is refused, because what ' +
           'reads it next would recurse as far as the document says');

  t_.check(validation.checkDocument(registration, 'registration').value === registration,
           'IT RETURNS THE VALUE UNCHANGED AND IS NEVER A TRANSFORM. The whole ' +
           'point is that the caller stores what the client actually sent');
  ...
  t_.check(validation.parseXml('<a><b/></a>', 'request').value.documentElement.nodeName === 'a',
           'and what comes back is the parsed document, not a copy of the text');

  t_.check(validation.parseXml('', 'request').code === 'empty',
           'AN EMPTY BODY IS `empty` AND NOT `malformed`. It is the case that ' +
           'took `POST /sts` down, it needs no craft at all, and a caller who ' +
           'sent nothing is helped by being told that rather than by a parser ' +
           'error about column one',
  ...
  t_.check(validation.parseXml('<a><b></a>', 'request').code === 'malformed',
           ...
           'protocol\'s words — a SOAP Fault, a SAML status — and none of them ' +
  ...
  t_.check(validation.parseXml('<a>&nosuch;</a>', 'request').code === 'malformed',
           'AN UNDEFINED ENTITY REFERENCE IS REFUSED. It is reported at level ' +
           '`error` rather than thrown, so it is the handler and not the ' +
           'try/catch that catches it — delete the handler and this is the ' +
           'only assertion here that goes red');
  ...
  try {
    ...
    validation.parseXml('<!DOCTYPE r [<!ENTITY x SYSTEM "file:///etc/passwd">]><r>&x;</r>', 'request');
  } catch (e) {
    threw = true;
  }
  t_.check(!threw,
           'AND NOTHING ABOVE THREW — including null, undefined and a document ' +
           'carrying an external entity declaration. This is the property the ' +
           'three 500s were the absence of');

  const xxe = validation.parseXml(
    '<?xml version="1.0"?><!DOCTYPE r [<!ENTITY x SYSTEM "file:///etc/passwd">]><r>&x;</r>',
    'request');
  t_.check(!xxe.ok || xxe.value.documentElement.textContent.indexOf('root:') < 0,
           'AN EXTERNAL ENTITY IS NOT RESOLVED — measured on @xmldom/xmldom ' +
           '0.9.10 rather than assumed. It leaves `&x;` as literal text, and a ' +
           'billion-laughs document expands to three characters. If this ' +
           'service ever moves to a parser that DOES resolve entities, this is ' +
           'the assertion that goes red and `parseXml()` is the one place that ' +
           'has to refuse a DOCTYPE');
  ...
  const roomy = zlib.deflateRawSync(Buffer.from('<samlp:AuthnRequest/>', 'utf8'));
  const ordinary = validation.inflate(roomy, 'SAML message');
  t_.check(ordinary.ok && ordinary.value.toString('utf8') === '<samlp:AuthnRequest/>',
  ...
  t_.check(!refused.ok && refused.code === 'too-large',
           'A BOMB IS REFUSED. Node reports ERR_BUFFER_TOO_LARGE and this turns ' +
           'it into a refusal the caller can answer with, rather than an ' +
           'exception or a gigabyte',
           refused.code);
  t_.check(took < 1000,
           'AND IT IS REFUSED WITHOUT INFLATING IT, which is the whole point: ' +
           'a ceiling checked after the fact would have allocated the thing ' +
           'first. It took ' + took + 'ms',
           took + 'ms');
  t_.check(refused.detail.indexOf(String(validation.CAP.LARGE)) >= 0,
           'and the refusal names the ceiling, so an operator meeting it on a ' +
           'legitimately large message knows what to change');
  ...
  t_.check(validation.inflate(Buffer.from('not deflated at all', 'utf8'),
           ...
           'POST binding sends plain base64 and `decodeMessage()` falls back to ' +
           'reading it as XML, so collapsing the two would make every ' +
  ...
  t_.check(source.indexOf("require('./mode')") < 0 &&
           ...
           '`autoCreates()`. The two are easy to confuse because both end in a ' +
           '400, and the test is whether the answer would change if the ' +
...
module.exports = {
  name: 'validation',
  describe: 'what a value from outside is allowed to be, and the shape/existence line',
  run: run
};
