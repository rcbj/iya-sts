...
| `random_values.js` | **Random values from node's generator, drawn uniformly** (#65): `common/crypto.js` section 13's `randomString()` over GNAP's 31-character alphabet against a chi-square bound, **with the old `byte % 31` computed beside it as the control** — a distribution test that cannot fail the code it replaced asserts nothing; the refusals (under 128 bits, a repeated character); `genId()`'s shape; and the service's source read, comments and strings blanked, for `Math.random(`, `forge.random` and an index taken modulo an alphabet's length. No allow list. The detector is tested on the exact GNAP line #65 removed. |
