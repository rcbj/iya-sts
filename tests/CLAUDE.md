...
* **Each job, per realm.** It makes a development and a product realm (a
  ...
  identity provider metadata this service publishes for it (and the TLS
  anchor for its back channel); and then walks the scenarios as a browser
...
* **Shibboleth's trust of the back channel** is a `StaticPKIX` engine over the
  service's anchor beside the metadata's explicit keys — the SP's own
  documented choice for an identity provider whose TLS certificate is not a
  metadata key. SimpleSAMLphp has no such choice (see the exceptions on
  #191); pysaml2 and Keycloak take a CA file.
