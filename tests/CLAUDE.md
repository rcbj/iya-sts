...
| `oid4vp_x509_client_id.js` | **THE x509_san_dns AND x509_hash CLIENT IDENTIFIERS** (#230, 2026-09-26), against the real certificate authority, with a wallet's check written from OpenID4VP 1.0 section 5.9.3 rather than asked of the module: the Verifier certified for the Response URI's host; the Request Object typed, ES256, signed by the `x5c` leaf, carrying leaf, Issuing CA and Intermediate and NOT the Root, chaining link by link to the service Root; the leaf's one dNSName the Client Identifier; x509_hash the base64url SHA-256 of the same leaf's DER; the prefix per request over the realm's; one certificate per name and key; the refusals (a name that is not the host, an IP host — where x509_hash signs under a nameless certificate — a wildcard, the sixteen-per-realm limit, a realm with no key); product mode never taking the Host header; a renewal keeping the dNSName; and the sign-in's DC API request under the same certificate |
...
| `vc_jsonld.js` | The CLOSED JSON-LD loader (#194-#196): every vendored context held to its SHA-256, the VC 2.0 context to the VCDM's own relatedResource digest, an unknown context refused with its URL (nothing fetched), a protected term's redefinition refused, RDFC-1.0 reproducing the ECDSA specification's canonical document and proof configuration, and a directed language value canonicalized. Reads the ECDSA TestVectors from `STS_VC_SUITES_DIR` (the tests image fetches them: they carry private keys) |
| `vc_rdfc_suites.js` | `eddsa-rdfc-2022`, `ecdsa-rdfc-2019` and `ecdsa-sd-2023` (#195, #196): the specification's combined hashes and signed credentials, round trips over Ed25519, P-256 and P-384 for VC 2.0 and 1.1, proof sets and chains, a chain to a missing proof refused; `ecdsa-sd-2023` against the specification's intermediate values — its HMAC-relabelled statements and mandatory group, a derived proof byte for byte its vector, its derived credential verifying and a changed one not, a base proof refused by a verifier — and the JCS default kept for the sign-in |
| `vc_data_model.js` | The VC Data Model MUSTs `oid4vc/vc_data_model.ts` refuses for (#194), one row per rule, beside the positives (2.0, 1.1, enveloped, a presentation) |
| `vc_jose_cose.js` | VC-JOSE-COSE (#198): every form round-tripping by its kid and by a caller-named key; a contradicting type header, a `vc` claim, an expired exp, another key's signature, a private key named, a hex COSE value and an unknown data: type refused; a missing typ accepted and a string iat a warning; RFC 9901 section 7.1's refusals |
| `vc_api.js` | The VC-API adapter's gate (#194-#199): 404 `STS-VC-0100` with test controls closed and no token asked for, each token refusal `STS-VC-0101` with its status, an admitted token; the issuer's own refusals before a key is touched; DID resolution and dereferencing per DID Core section 7; the two scopes protected |
...
| `ldap_diagnostic_message.js` | **AN LDAP REFUSAL'S TEXT REACHES THE CLIENT** (#261, 2026-09-26). node-ldapjs put a refusal's message in `res.errorMessage`, which `@ldapjs/messages` never encodes, so every result left with an empty diagnosticMessage (RFC 4511 section 4.1.9); the `encodeErrorMessage` option in the `rcbj/node-ldapjs` fork sends it. On two loopback servers of its own: the CONTROL (no diagnostic message without the option), the option's server sending the refusal's text and an uncaught exception as `internal error`, and as source that `ldap_server.js` creates both servers with the option and asks each whether it took. `tests/vendored/sts_credential_signals.js` section c is the over-LDAPS half |
...
| `audit_vocabulary.js` | **EVERY AUDIT ACTION THE SERVICE RECORDS IS A ROW OF `audit.ACTIONS`** (2026-09-26). Since #86 `GET /admin-api/audit?action=` is held to that table, and 74 recorded actions were never listed, so a query for any of them was refused 400 — `sts_portal_backup_codes` found it on `admin.mfa.backup-codes.cleared`. Reads every literal dotted `action:` in the service directories, the four actions built by concatenation with their suffixes, and every row's category against `CATEGORIES`. Against the table before the fix the scan found 64 literals missing |
...
| `lingering_close.js` | **AN EARLY ANSWER TO A LARGE UPLOAD REACHES THE CLIENT** (2026-09-26): a server refusing on the headers while 24 MB is still arriving, directly and through `request_pool.proxy()` with a unix-socket worker — every answer read whole, `Connection: close`, and a client that goes on sending after it NOT reset; the control (a plain `Connection: close`) resets that client 8 of 8 (`write EPIPE`), which is what proves the check can fail. And `arm()` leaving a socket alone once the body has arrived |
...
| `tls_protocol_policy.js` | **WHAT tlsfuzzer FOUND IN THE TLS POLICY** (#212, 2026-09-26). `protocolOptions()` carries the post-quantum-first groups with no finite-field group, the signature list with no DSA or SHA-224 and brainpool omitted by policy, and `SSL_OP_NO_RENEGOTIATION`, and `secureContextOptions()` carries all of it. REAL handshakes against a server built from it: SecP384r1MLKEM1024 served, ffdhe2048 refused, a TLS 1.2 renegotiation refused. THE NIST-CURVE GUARD, in a child process: a server whose handshake lets any curve through closes a client certificate whose EC key is on a curve other than P-256/P-384/P-521 — as the leaf, as the issuer, and in a chain read whole — with `refuseNonNistCurveCertificatesOn()` installed, and stays up, while a P-256 one is read. And the SPIFFE listeners' own list omits brainpool by policy |
...
| `device_signals.js` | **#164 PHASES 3 AND 4, IN PROCESS** (a child process, a THROWAWAY REALM — it revokes real certificates and ends real sessions), every SET read back off a poll stream: the device index (lookups never copy the register; a key added is found, a key removed and another realm's are not); `device-compliance-change` with CAEP's two values (`unknown` sent as `not-compliant`, so unknown → not-compliant is no event), `initiating_entity` by source and the complex `{ user, device }` subject with the device as `iss_sub`; `risk-level-change` with `principal` `DEVICE` and `previous_level` only where there was one; `credential-change` for a device key and a Native SSO secret (this service's two URN types); a COMPROMISE ending the session the device authenticated (its `session-revoked` naming the device) and not the person's other one, revoking the EST certificate for keyCompromise and the secret, risk HIGH, RISC `credential-compromise` and `sessions-revoked` naming account and device; a removal revoking for cessationOfOperation; SSF section 8.1.3.1's subject matching on a device and its three examples; `device:compliance` protected in both modes and the DEVICE_COMPLIANCE role; the MDM feed's batch; the test control's mode split |
| `device_risk.js` | **#164 PHASE 5, IN PROCESS** (a child process, a THROWAWAY REALM): the registered device in risk scoring — `byCredentialId()` and `holdsAny()` from the index (an owner moved is followed); `unregistered-device` not for a person who registered nothing, not before `risk.minimumHistory`, yes for a device owner signing in with none or with somebody else's, yes for anybody under `devices.expectRegistered`; `non-compliant-device`; `compromised-device` HIGH on its own and never "compliant"; the two lowering factors (×0.8, ×0.5) scoring below the same sign-in and never making an UNSCORED one scored; the device feature `registered:<id>` and no `new-device` for the person's own device; the device's level following the sign-in's with CAEP `risk-level-change` (DEVICE) only on a move, and a compromised device's HIGH held; a WebAuthn sign-in assessed with its linked device; the device on Monitoring → Risk |
| `device_policy.js` | **#164 PHASE 6, IN PROCESS** (a child process, a THROWAWAY REALM): the device's facts in the issuance request (recognised, id, via, owner matches, owner kind, compliance, attestation, status, the realm's requirement), none when the question was not asked; with the rules at their defaults a not-compliant device and no device issued and a COMPROMISED one refused (and let through with `devices.refuseCompromised` off, and refused with no application named); with `devices.requireCompliantDevice` on no device, a not-compliant one, somebody else's and a self-asserted one under `compliantDeviceAttested` refused, the person's own compliant (attested) one issued, the console and portal exempt, a waived role question and a session's device read as it stands NOW after an MDM report; `startSession()` refusing and a gated door asked the device alone (`STS-DEVICE-0037`); `urn:sts:acr:compliant-device` published, met only by the person's own compliant device and neither meeting nor met by `mfa`, sent round once then `unmet_authentication_requirements`; `device_id` in the access token and ID Token with one `cnf.jkt` naming the device key, absent with none or somebody else's, sector-derived for pairwise and absent for ephemeral, and SSF's device member rewritten the same way |
| `device_enrolment.js` | **#164 PHASE 2, IN PROCESS**: the enrolment challenge (one per session and purpose, another session refused, expiry, answered once); a JWK proof self-asserted in development and refused in product; an Android Key Attestation, an Apple App Attest statement and a TPM key attestation (draft-ietf-lamps-csr-attestation) BUILT AT RUN TIME with `webauthn_attestation_kit.js` against configured test roots — attested, and each spoiled way refused; the WebAuthn platform-credential link; EST and SCEP's `device` profile (a new device, renewal over the same key, the identity rule, ACME / a server key / a re-enrolment refused, two attributes refused); recognition by x509 (a revoked certificate not), webauthn, jwk and native-sso, a compromised device still recognised, a second device kept as the conflict, and the counters |
...
| `caep_claims_doors.js` | **CAEP FROM THE DOORS THAT ARE NOT A DIRECTORY ATTRIBUTE** (#238, #243, 2026-09-26), in a child process: `assuranceOf()` (`none`, `verified`, a stated level in `urn:sts:ial`, `NIST-IAL` only for `nist_800_63A` with IAL1–3, the newest stated level deciding); a role's members moving told to the account observer as `roles` once per person (a named user, a named group's member; nobody for a description; everybody on a delete); `rolesMoved` on a membership only when a role names the group; `claimsChangeFor()`'s roles claim, `null` for nobody's, `email_verified` from the address proved, and a notice naming its own claims; `writeMailFlag('stsMailVerified')` told to the observer; `identity_assurance.ts` sending `verified_claims` without evidence and the IAL with previous level and direction, and nothing for a sign-in rewriting its own record; a claims-provider unlink sending `_claim_names`/`_claim_sources` gone and never the source's token; the fan-out's live holders, a claim set changed, a protected scope withdrawn, a scope added (nothing) and a roles claim renamed |
| `credential_signals.js` | **WHAT A DETECTED COMPROMISE AND EVERY PERSON CREDENTIAL SAY** (#231, #236, #237; 2026-09-26), in two children, with `ssf.ts`'s two emitters replaced on its loaded exports by a recorder: the emailed factor (create, code-to-link update, an administrator's delete, and at the failure limit a `system` delete with RISC `credential-compromise` under the email URN and one mail), a SIOP subject, an EAB key, a SCEP challenge (`password`, named) and a HOBA key (create, then update for the same kid) with their URN types; a cloned key's counter in `spendAssertion()` (RISC `fido2-roaming`, standing HIGH `authenticator-compromised`) and `clonedKeyVerdict()`; `keyCompromise` on a TLS client and an enrolled certificate (and nothing for cessation); a replayed one-time code under its own failures door, scored `totp-replay` and not `account-failures`, no SET; LDAP credential attributes refused STS-LDAP-0111 in development, a pre-hashed `userPassword` update, a delete revoke, `pwdReset` once; the CIBA user code as `pin`; a bootstrap password. In the PRODUCT child, a password on a stubbed Pwned Passwords list at the sign-in screen sends `credential-compromise` and `account-credential-change-required` once and holds the sign-in at the change step |
...
| `risc_identifiers_recovery.js` | **RISC IDENTIFIERS AS SETS, AND THE RECOVERY ADDRESS** (#234, #235, 2026-09-26), in a child process: every value of `mail`, `telephoneNumber` and `mobile` compared as a set — a removal is `identifier-changed` without `new-value` and is released, so recycled; a second address or a `mobile` beside a `telephoneNumber` is seen and the row keeps its first; a number moving between the two attributes or an address changing case is nothing; every arrived value is checked for recycling and a purge releases every value; the recovery channel (first `mail`, verified or not) added, changed, verified or removed is `recovery-information-changed`, a create or a second address is not; and `writePersonFlag()` hands a write of `stsMailVerified` to the account observer |
...
| `client_attestation.js` | **OAUTH 2.0 ATTESTATION-BASED CLIENT AUTHENTICATION, IN PROCESS** (#229, 2026-09-26, draft-ietf-oauth-attestation-based-client-auth-11). `oauth-oidc/client_attestation.ts` against requests built here, every key made at run time and every JWT signed by the file's own signer (ES256 r\|\|s, RS256, ML-DSA-44 through the vendored engine). Trust: nobody trusted (0724, and empty metadata), a configured JWKS, a wrong kid and a forged signature (0726), an x5c leaf under a configured anchor, a self-signed leaf that IS the anchor and a stranger's chain (0725). The attestation's typ, HMAC alg, missing sub/exp/cnf, expiry, a future iat, a private cnf key and another client's sub, each by code, and 400 `use_fresh_attestation` past the max age. The PoP's typ, another key's signature, missing jti/aud, the token endpoint or two audiences, an iat too old or ahead; a one-member aud array accepted; an ML-DSA-44 instance key end to end. Challenges: required by default, 400 `use_attestation_challenge` with a fresh one, an unknown one, a fresh one on a success, one answer per request, the PoP replayed and a spent challenge refused, and a failed request leaving its challenge unspent. Duplicate and non-token68 fields, no proof, no attestation. The combined mode unverified, with another key, accepted, and each declared method held to its own proof. `requestRefusal()` (declared, additional signal), `bindingRefusal()`, the metadata, and `client_auth.js`, `oauth2_bcp.js` and FAPI 2.0's `oauth2.fapiAllowClientAttestation`. The over-HTTP half is `sts_client_attestation.js` |
...
| `account_delete.js` | **A DELETED PERSON'S SESSIONS END WITH THEM** (#241, 2026-09-26), in a CHILD PROCESS with a relying party of its own — `account_disable.js`'s arrangement. A SCIM `DELETE` ends the person's sign-on session: the old cookie no longer signs in (the authorization endpoint sends it to the screen), the session is gone from the store, the relying party is POSTed a back-channel Logout Token and the session observer is told `revoked`. The CATCH-UP: with the delete's consequence lost (as on a node the delete was not made on), the next request presenting the session is sent to the screen and ends it through `dropSession()`. And what is ended is what the person held AT THE DELETE: somebody made again under the same name before the deferred step runs keeps their session. Mutation-tested: without the directory's hand-off and `sessionOf()`'s subject check, eight of twelve fail |
| `realm_removal_signals.js` | **A TRUST REALM IS REMOVED ALOUD** (#232, 2026-09-26), in a CHILD PROCESS with a real RFC 8935 push receiver. Removing a realm through the console's action pushes, before the realm goes, `session-revoked` (`initiating_entity: admin`) for a live session, `account-purged` for every person, and LAST `stream-updated` `disabled`; the answer no longer says the directory stayed. The wait is BOUNDED: a receiver that never answers holds `realms.retire()` for `realms.removalDeliveryTimeoutS` (1 s in the test) and the realm goes anyway, reported late. A poll stream nobody collects is counted undelivered. The control: `realms.remove()` alone — what a replicated removal runs — announces nothing |
...
| `ca_hierarchy_signals.js` | **A change to the certificate hierarchy tells the people under it** (#244), with `ssf/account_signals.ts` stood in for: a person's TLS client certificate (a register slot) and ACME enrolment are held, an application's is not; `reissue-use-case` on `tls-client` re-mints the slot (`update`, the new serial), on `acme` orphans the enrolment (`revoke`, the old serial — enrolments are never re-minted), once; a revoked Issuing CA (`cACompromise`) walks down to its holder with RISC `credential-compromise`, a revoked Intermediate reaches every use case with none for `superseded`; `/admin/pki`'s `build-scope` orphans the TLS client certificate and says so; 120 people's events go out 50 at a time with a turn between batches. |
| `service_key_signals.js` | **The other keys a relying party pins, and a deleted stream** (#245), in a child with the whole stack: the three new `sts` events (always offered, #42's required members, a validating payload); `serviceKeyChanged()` to the stream that asked and no other; OpenID Federation's scheduled rotation, emergency (naming the revoked next key) and a retired key revoked as compromised; a SPIFFE JWT authority rotation (`bundle_changed` true); the listener's first certificate over its bootstrap NOT announced and its replacement after a rebuilt process branch announced to every realm; since #264, E2–E7: the replacement recorded in `tls.listenerAnnounced`, `listen()` announcing a certificate other than the recorded one as `restarted` and a repeat as nothing, a self-signed, supplied or handed-in certificate never recorded, a unit with no record recorded silently, and `restarted` valid for `tls-certificate-changed` only; a stream deleted from the console and one deleted for inactivity each sent `stream-updated` (`disabled`, with a reason) while it still exists. |
| `listener_certificate_restart.js` | **A listener certificate re-issued at RESTART is announced** (#264). Three child processes, one after another, over one file-backed stub minted driver (`minted_persistence.js`'s five functions), in product mode with a file KEK and the keys kept in a second file. The first start records its certificate and announces nothing. The second has the same Root and a new listener key: it restores the record and announces `tls-certificate-changed` once, from `listen()` and not before, to every realm, `restarted`, from the first start's fingerprint, and the record moves. A start with no store announces nothing. In-process because no HTTP job can restart the service it talks to. A mutant that drops `persist` from the store fails three assertions. |
...
| `tests/vendored/sts_admin_geolocation.js` **(ours)** | **MONITORING → GEOLOCATION, OVER HTTP, IN EVERY MODE** (#255, 2026-09-26): `GET /admin-api/geolocation` in the live window counting this job's own console sign-in; every window and level in its shape, each count at or over `risk.geoMinimumCount` or null and suppressed; the four refusals (400); the page with the map inline and no script, the zoom trail, the Natural Earth credit, and `?format=json` equal to the API's answer (rule 7). It LOADS NO DATASET — the service's geolocation datasets are real ones on an AWS target — so it asserts shapes and the suppression rule. `tests/geolocation_map.js` is the in-process half, on seeded assessments. |
...
| `tests/risk_install_connection.js` | **THE INSTALL-TIME LOADER DIALS THE DATABASE THE WAY THE SERVICE DOES** (#213, 2026-09-26): with the password in a file provider and none in `STS_DATABASE_URL`, `RiskInstall.driverOptions()` hands the driver the URL with it injected, read back through `pg`'s own parser for a password of `%`, `&`, `+`, `@`, `:`, `/`, `#` and `?`; `verifyTls` off and on with `persistence.databaseTlsRejectUnauthorized`, and `run()` PASSING both to `persistence_postgres.create()` (a stand-in caught at that call); a URL's own password kept with no provider; an unreadable provider `STS-RISK-0040` with no driver built; no database named `STS-RISK-0012`. **The loader against a real OpenBao-backed database is `run-tests.sh`'s `riskInstallCheck()`**, before the runner in every postgres mode: `docker exec` with no extra environment, the container's URL asserted password-free and `PGPASSWORD` unset, a synthetic two-row operator ALLOW list in 198.19.213.0/24 (the allow list because a deny list would be compared with `sts_admin_risk.js`'s and trip the shrink limit) imported and then found already recorded (`tests/report/<mode>-98-risk-install.log`; `STS_TEST_RISK_INSTALL=0` skips it) |
...
| `tests/vendored/sts_oid4vp_x509.js` **(ours)** | **THE VERIFIER'S x509 CLIENT IDENTIFIERS, CHECKED AS A WALLET CHECKS THEM** (#230, 2026-09-26), in a throwaway realm: `/oid4vp/verifier-certificate` naming both Client Identifiers, the `x5c` and the anchor, `no-store` (in product mode x509_san_dns refused there until `oid4vp.x509DnsName` names the host, which the job then sets); for each prefix, `/oid4vp/start?client_id_prefix=` followed to the Request Object, whose signature is checked against the `x5c` leaf, whose chain is checked link by link to the service Root fetched from the Intermediate's own caIssuers, and whose Client Identifier is checked against the leaf's dNSName and the Response URI's host, or the SHA-256 of its DER — all with node's `crypto`; the same check refusing another host, another hash and the other prefix's value; the realm's own `oid4vp.clientIdPrefix`; and the Verifier refusing a name that is not the Response URI's host while x509_hash still answers. The expected name is the Response URI's host as the document reports it, so a pinned `global.publicBaseUrl` is followed. Floor of thirteen checks |
...
| `tests/vendored/sts_devices.js` **(ours)** | **THE DEVICE REGISTER THROUGH THE MANAGEMENT API** (#164 phase 1, #218, 2026-09-26), in a throwaway realm: a person's device with a JWK and an APPLICATION's device registered by an administrator; the list filtered by owner kind and key kind, paged with a page past the end clamped, and one device by id (`found: false` for an unknown one); the label and platform edited, a second key added, a key another device holds and a private JWK refused, a key removed; an unknown action refused in the seven-action sentence, an unknown owner, and a person at `devices.maxPerPerson` refused with nothing evicted; Monitoring → Devices' counts and timeline, Device registration's methods and settings, and `/admin-api/ldap/devices`; removal counted as an event. **And #164 phase 2**: the portal's JSON doors (a challenge; a `device-key-proof+jwt` registering a portal-enrolled, self-asserted device; the challenge refused a second time; a form body refused 415), EST's `device` profile (a new device owned by the person, the certificate naming only `urn:sts:device:<id>`, kept on it as an x509 key), recognition at the token endpoint by a DPoP key and by the EST device certificate over mutual TLS (counted on Monitoring → Devices), and product mode on the realm refusing an unattested key proof. **And #164 phases 3 and 4**: `device:compliance` issued only to a client that declares it; `POST /admin-api/device-compliance` applying a batch by key thumbprint and by certificate with an unknown device refused alone, and this run's admin:write token refused there; `device-compliance-change` read off a POLL stream (every SET drained is pooled, because a poll ACKs what it reads); the test control open in development and 403 in product; and a device marked compromised — its EST certificate on the realm's EST CRL, RISC `credential-compromise` and `sessions-revoked` naming the account and the device, CAEP `risk-level-change` to HIGH. **And #164 phases 5 and 6**: a `client_credentials` token bound by DPoP to the application's own device key carrying `device_id` with its one `cnf.jkt` the same key, none without the key, introspection repeating it, discovery listing `device_id` and `urn:sts:acr:compliant-device`; `devices.requireCompliantDevice` on in the realm refusing that request from a not-compliant device and issuing it from a compliant one; `acr_values=urn:sts:acr:compliant-device` sent round once and then `unmet_authentication_requirements`; a device owner signing in with none of their devices assessed `unregistered-device`, and Monitoring → Risk listing the five device signals. Floor of thirty-two checks |
...
| `tests/vendored/sts_tlsfuzzer.js` **(ours)**, with `tests/tlsfuzzer_debugger.js` and `tests/tls_protocol_policy.js` in process | **TLSFUZZER AGAINST THE MAIN HTTPS PORT AND LDAPS 636** (#212, 2026-09-26) — every applicable script of tlsfuzzer, pinned and fetched into the tests image, through `tests/tlsfuzzer/sts_adapter.py`, with client certificates made at run time (RSA and EC P-256 from a throwaway realm's CA; Ed25519, RSA-PSS, ML-DSA-65 and a non-NIST EC curve self-signed with `openssl`). The plan — each script run, not applicable or run as a refusal, each exception with its reason — is `tests/vendored/tlsfuzzer_kit.js`; a failed probe the plan does not explain fails the job. See *The TLS fuzzer*, below |
...
| `tests/vendored/sts_vc_bitstring_status_suite.js` **(ours)** | **THE W3C BITSTRING STATUS LIST SUITE** (#197), and the status changes it never makes: issue with a status, the JSON-LD list credential verifying, revocation through `/vc-api/credentials/status` setting the published bit and the verifier refusing, revocation final, suspension set and lifted. Its eight pending tests are `it.skip` upstream |
| `tests/vendored/sts_vc_jose_cose_suite.js` **(ours)** | **THE W3C VC-JOSE-COSE SUITE** (#198): its 35 tests through `tests/vc-suites/jose-cose-cli.js`, answered by `tests/vc-suites/bin/docker` on its PATH (no container is started). Four documented exceptions, each a fixture against a clause |
| `tests/vendored/sts_did_test_suite.js` **(ours)** | **THE W3C DID CORE SUITE** (#199) over fixtures GENERATED from this service: its did:web, the realm's did:keys, a did:jwk from `/did/generate`, and resolver and dereferencer executions through `/vc-api/resolve` and `/vc-api/dereference`, run by the suite's six jest suites. One documented exception (an unsuccessful resolveRepresentation) |
...
| `tests/vendored/sts_caep_claims_doors.js` **(ours)** | **CAEP FROM THE DOORS THAT ARE NOT A DIRECTORY ATTRIBUTE, OVER THE WIRE** (#238, #243, 2026-09-26), in a throwaway realm with a poll stream covering everybody, for a person holding live tokens: `token-claims-change` carrying the roles claim for a role given; the groups and roles claims together for a group a role names; `verified_claims` (framework, level, the claim, never the evidence) and `assurance-level-change` `NIST-IAL` `IAL2` with no previous level for an `nist_800_63A` verification; `email` and `email_verified` for an address SCIM wrote; a custom access-token claim with its value (the fan-out); and `GET /ssf` stating that a tightened authentication policy sends nothing |
| `tests/vendored/sts_ca_hierarchy_signals.js` **(ours)** | **THE HIERARCHY TELLS THE PEOPLE UNDER IT** (#244, 2026-09-26), in a throwaway realm with a poll stream for CAEP `credential-change` and RISC `credential-compromise`: a person's signing key pair (the `assertions` Issuing CA's issued register) is announced, then `reissue-use-case` on that authority ORPHANS it — `credential-change` (`x509`, `revoke`) naming the old serial, and the answer says people were affected; a second key pair, then the new Issuing CA revoked on its Intermediate's list for `keyCompromise` — `revoke` naming that serial AND RISC `credential-compromise` (`x509`). Only the throwaway realm's authorities are touched. |
| `tests/vendored/sts_service_key_signals.js` **(ours)** | **THE OTHER KEYS A RELYING PARTY PINS** (#245, 2026-09-26), in a throwaway realm with Kerberos on and `krb5.retainedKeyVersions` 0, over a poll stream for this service's own key events: `oidfed/rotate-key` sends `federation-key-rotated` (`requested`), an emergency sends it as `emergency` naming the revoked next key; `spiffe/rotate` (`jwt`) sends `spiffe-authority-rotated` with `bundle_changed` true and the trust domain; an ORDINARY `rotate-krbtgt` that keeps nothing sends `kerberos-tickets-invalidated` with reason `nothing-retained`. The listener's `tls-certificate-changed` and a deleted stream's notice are in-process (`service_key_signals.js`): the first rebuilds what every job shares, and a poll receiver cannot read a SET from a stream that no longer exists. |
| `tests/vendored/sts_account_delete_sessions.js` **(ours)** | **A DELETED PERSON'S SESSION ENDS WITH THEM, OVER THE WIRE** (#241, 2026-09-26), in a throwaway realm it leaves standing, with a poll stream taking `session-revoked` and `account-purged` about everybody. A person signs in through the code flow and a second authorization request is answered from the session (the control); SCIM `DELETE` answers 204; the SAME browser's next authorization request is sent to the sign-in screen; and the stream holds `session-revoked` about the person with `initiating_entity: admin` (#242) and `account-purged`. The back-channel Logout Token needs a relying party the service can reach, which the suite has not, and is held in process by `tests/account_delete.js` |
| `tests/vendored/sts_caep_oauth_grants.js` **(ours)** | **AN OAUTH GRANT REVOKED IS A CAEP `session-revoked`, AND THREE MORE DOORS ARE SINGLE SIGN-ON** (#239, #240, 2026-09-26), in a throwaway realm with CIBA and refresh rotation on and a poll stream covering everybody: `/oauth2/revoke` of a refresh token sends ONE `session-revoked` whose session is `oauth-grant:<id>`, about the person, `user`; a rotated refresh token replayed sends `risk-level-change` (`SESSION`, `HIGH`, `refresh-token-replay`) and `session-revoked`, both `policy`, about one grant; `/admin-api/tokens/revoke-user` sends `admin`; a CIBA approval on `/portal/ciba` is `session-presented`, its tokens are listed under the approving sign-on session and their refresh token is inactive once that session ends; in product, a cross-device OpenID4VCI offer is `session-presented` |
...
| `tests/vendored/sts_risc_register_recovery.js` **(ours)** | **RISC: THE HOLDER'S CHOICE, IDENTIFIERS AS SETS, RECOVERY** (#233, #234, #235, 2026-09-26), in a throwaway realm with a poll stream covering everybody: a register reset and clear keep a pending opt-out, which the `risc.opt-out-effective` job still makes effective, and an effective one, with nothing sent to say the state moved; over SCIM, a second address and a `mobile` beside a `telephoneNumber` changed are each `identifier-changed`, a removed address is `identifier-changed` with no `new-value`, and a new account given it is `identifier-recycled`; a first address set by an administrator is `recovery-information-changed`, a changed one is `identifier-changed` and `recovery-information-changed`, an activation link for somebody who exists is `recovery-activated` and `account-credential-change-required`, and a recovery code spent at sign-in is `recovery-information-changed` and `recovery-activated` |
| `tests/vendored/sts_credential_signals.js` **(ours)** | **CREDENTIAL SIGNALS FROM THE DOORS #231, #236 AND #237 CLOSED** (2026-09-26), in a throwaway PRODUCT realm with a poll stream taking CAEP `credential-change` and RISC `credential-compromise` and `account-credential-change-required`: an ACME EAB key made and deleted (`urn:iya:sts:credential-type:acme-eab-key`), a SCEP challenge (`password`, its friendly_name naming it) and a SIOP self-issued subject (`urn:iya:sts:credential-type:self-issued-key`); a person's key pair revoked on `/admin-api/pki` for `keyCompromise` (RISC `credential-compromise` `x509` beside the CAEP revoke); over LDAPS bound as the realm's own administrator, `stsTotpCredential` modified and `stsWebauthnCredential` added are refused 53 (STS-LDAP-0111) with a diagnosticMessage naming the door (`/portal/mfa`, `/portal/keys`; #261), `userPassword` deleted is `password` `revoke` by admin, `pwdReset` set is `account-credential-change-required`. The in-process half is `tests/credential_signals.js` |
...
## THE W3C VC AND DID SUITES (#194-#199, 2026-09-26)

The Verifiable Credentials and DID Working Groups' own suites, each run by a
`local: true` job of its own (the table above) against the VC-API test
adapter (`oid4vc/vc_api.ts`, a development-only test control —
`oid4vc/CLAUDE.md`, *The W3C VC-API adapter*) of a throwaway DEVELOPMENT
realm, so every local mode runs them: a realm's mode is its own.

| Suite | Commit | Licence |
|---|---|---|
| w3c/vc-data-model-2.0-test-suite | `62836458636bd1b8309214626ec8fc85de43f8bd` | W3C 3-clause BSD / W3C test suite |
| w3c/vc-di-eddsa-test-suite | `1e86cc4ca16d77eb76cd15e3905a624bc16c0879` | W3C 3-clause BSD |
| w3c/vc-di-ecdsa-test-suite (+ w3c/vc-di-ecdsa's TestVectors at `59df72ca8cdb275d97eb496086fea6fac4fa7f0f`) | `94dca21d8c1c7ad3dc31f21144d4d957567726b5` | W3C 3-clause BSD |
| w3c/vc-bitstring-status-list-test-suite | `960ab9a3ad275bb5e56dd3f300939f83cba759b7` | W3C 3-clause BSD |
| w3c/vc-jose-cose-test-suite | `cf4c93ddf7e1bfef5a3e5db4b9a798b02ea778a9` | W3C 3-clause BSD |
| w3c/did-test-suite | `939b31d07d5b1699340ac0702ec0fa46ffcdef0a` | W3C |

**FETCHED AT IMAGE BUILD TIME, NEVER COMMITTED** — several carry test private
keys. `tests/vc-suites/fetch-suites.sh`, in the `vc-suites` stage of
`tests/Dockerfile`, downloads each commit's tarball, checks its SHA-256 and
installs it from its own lockfile (`npm ci --ignore-scripts --omit=dev`; the
two GitHub dependencies there are commits too), into `/opt/vc-suites`
(`STS_VC_SUITES_DIR`). **The suites' `pretest` is NOT run**: it reinstalls
vc-test-suite-implementations from `main` at test time, an unpinned
download; the lockfile's commit is used, and the jobs run mocha directly.
The DID suite keeps its scripts, because its lerna postinstall builds its
matcher package. The in-process files `vc_jsonld.js` and
`vc_rdfc_suites.js` read the ECDSA TestVectors from the same directory.

**How a suite meets the service.** `tests/vendored/vc_suites_kit.js` makes
the realm, a client with `vc-api:issue` and `vc-api:verify` declared, and a
token; writes the suite's `localConfig.cjs` (the implementation manifest,
OAuth client credentials with the secret in `VC_API_CLIENT_SECRET`); and
runs the suite's own mocha with its JSON reporter and
`tests/vc-suites/offline-contexts.cjs` required — a `fetch` wrapper that
answers the JSON-LD contexts this service ships from those (hashed) files,
so a run does not depend on w3.org and logs any other URL it did fetch. The
VC-JOSE-COSE suite runs an implementation as `docker compose run`; the job
puts `tests/vc-suites/bin/` first on its PATH, where `docker` is a script
that answers that one command with `tests/vc-suites/jose-cose-cli.js` over
the adapter (the tests image has no daemon, and would not be given one).
The DID suite has no implementation to call: the job GENERATES its fixture
files from the running service and runs its jest suites over them, required
inside the test environment (jest's `globals` fail its `instanceof`-based
matcher across realms).

**Every failure is fixed or a documented exception**, the job's
`EXCEPTIONS` and a comment on the ticket; an exception that did not occur
is reported. `oid4vc/CLAUDE.md` lists what the suites found and the
exceptions.

**By hand**, inside the tests image on the stack's network, with the usual
...
* **Run as a refusal.** The export-cipher script and the script that signs
  its CertificateVerify with a client certificate on a non-NIST curve are
  here, and every probe must be refused. The second is the regression check
  for the NIST-curve guard tlsfuzzer led to (`tls/CLAUDE.md`).
...
FOUND*):
* client certificates on a curve other than P-256/P-384/P-521, now refused;
* `tls.groups`, post-quantum hybrids first and no finite-field groups;
* `tls.signatureAlgorithms`, with no DSA or SHA-224, brainpool omitted by policy;
