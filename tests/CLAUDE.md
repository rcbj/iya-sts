...
| `tests/vendored/sts_saml_interop_pysaml2.js` **(ours)** | **PYSAML2 AS A SCRIPTED SP, NEGATIVE CASES INCLUDED** (#190): every request/response binding pair, an unsigned request, ForceAuthn and IsPassive, NameIDPolicy in four formats, identity-provider-initiated SSO, Single Logout both ways, and the refusals — a bad signature, a wrong Destination, a stale IssueInstant, a replayed ID. `samlPeer: 'pysaml2'` |
| `tests/vendored/sts_saml_interop_simplesamlphp.js` **(ours)** | **SIMPLESAMLPHP AS AN SP** (#191): its metadata validator on the identity provider's metadata, SP-initiated SSO (encrypted), ForceAuthn, IsPassive, NameIDPolicy, identity-provider-initiated SSO and Single Logout both ways, with `debug.validatexml` on so its log names a document that does not validate. `samlPeer: 'simplesamlphp'` |
