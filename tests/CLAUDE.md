...
| `gnap_token_formats.js` | **THE FIVE RFC 9767 TOKEN FORMATS AS LIBRARIES** (2026-09-12): mint, verify and attenuate for jwt-signed, jwt-encrypted, macaroon, biscuit and zcap over one token model, and every refusal — expiry, audience, binding, access, a forged caveat or block. **A zcap audience that is not a URI** is carried as `urn:gnap:rs:<id>`; that case was added after `tests/vendored/sts_gnap_rs.js` found every zcap token for a plainly named resource server refused, because every audience in this file was a URL |
