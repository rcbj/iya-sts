...
| `scheduler_p5_jobs.js` | **What P5 of #49 moved onto the scheduler**, the whole stack in a child: each former timer and piggy-backed purge is a job of the argued kind and owner (the change-log pull a QUIET per-process job at `persistence.pollInterval`, the trim a cluster job, off once replication stops), SPIFFE's authorities rotated only past half their lifetime (D6), and a GNAP biscuit minted before the `jose:EdDSA:Ed25519` unit rotates still verifying, with `/gnap/keys` and the ZCAP controller listing both keys. `scheduler.js` L holds the two scheduler changes (a sub-tick interval, a quiet job). Four mutants caught on 2026-09-22. |
