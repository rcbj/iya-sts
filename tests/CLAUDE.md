...
| `oid4vp_sign_in.js` | **SIGNING IN WITH A WALLET** (2026-09-17, #38), in a CHILD PROCESS on an ephemeral loopback port, with a wallet written in the file: the button and `oid4vp.signIn`, the `forceMfa` refusal, the wait page (no script, a `<meta>` refresh, a server-drawn QR code, the base CSP), the signed request asking for this issuer's SD-JWT VC and `sub`; a credential issued over HTTP and presented with a Key Binding JWT signing in its entry with `amr ["pop"]`/`acr "1"` and completing the pending authentication; another browser refused even with the `response_code`, a wrong `response_code`, a second `direct_post`, a second collection, expiry, a sign-out withdrawing an uncollected sign-in, a different person's session replaced; and — verified or not, each signing nobody in with its own code — no Key Binding JWT, a wrong nonce, audience or key, another realm's credential, **a credential this realm signed on an access token it did not verify** (the case the register exists for), a trusted foreign issuer's, a deleted entry's and a person the issuance policy refuses; the bar door still signs nobody in. Five mutants — the verified-token condition, the binding cookie, the foreign-issuer check, the answered-once refusal, the entry check — all caught. **Not driven by the parent's debugger wallet**, whose handling of a DCQL query naming only `sub` is unverified |
...
| `tests/vendored/sts_dpop.js` | RFC 9449 end to end over HTTP: all twelve section 4.3 checks, the `cnf.jkt` binding on access and refresh tokens, `dpop_jkt`, `jti` replay, and the nonce handshake in both shapes. Almost entirely negatives, because a DPoP server that issues bound tokens and accepts good proofs looks finished and can be worth nothing |
