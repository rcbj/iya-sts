...
| `realm_retiring.js` | **A REALM BEING REMOVED STARTS NOTHING NEW** (#262, 2026-09-26), in two CHILD PROCESSES. The stack child holds `realms.retire()` open with a deliver hook of its own and asks every door while the realm is retiring: the `mark` hooks run before announce, `startSession()` answers null with `STS-CORE-0121`, the issuance gate refuses all nine kinds (`retiring` on the answer), the token endpoint answers `invalid_grant` over HTTP, OpenID4VCI `credential_request_denied`, enrollment 503, the SPIFFE authority refuses a JWT-SVID — each with a control in another realm that is not refused; then the mark is replicated-only and one-way. The store child runs `persistence.js` over a `pg` double: the `retiring_at` upsert and its change-log row come BEFORE the realm's DELETE, and a mark another node wrote is applied here |
