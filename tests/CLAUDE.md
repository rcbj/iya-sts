...
| `typecheck.js` | **THE TYPE CHECKER, AS A TEST (#50, 2026-09-16).** Runs `tsc -p tsconfig.json` and fails on any error, and checks that every non-vendored file in a type-checked directory (`common/` so far) starts with `// @ts-check`, so a new file is checked from its first commit. `tsc` is a dependency of `tests/package.json`, for `.npmrc`'s reason; a missing compiler FAILS naming that file. Adding a directory to the check is a line in `CHECKED_DIRS` and a pattern in the root `tsconfig.json`'s `include` |
