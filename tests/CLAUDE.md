...
| `federation_subject_policy.js` | **WHICH PEOPLE A FEDERATION PARTNER MAY ASSERT** (#109, 2026-09-22), in a CHILD PROCESS per mode — the service provider's realm in development and then in product — through a real OIDC federated sign-in between two realms: `link-at-first-sign-in` (the linking screen with the name fixed and no passwordless or anonymous door, nothing written before the link, a wrong password, Cancel `STS-FED-0099`, the right password recording `<relationship> <iss> <sub>`, a held TOTP factor asked for — development only, since an in-process product realm has no key-encryption key to seal one — a required factor's enrolment step, and a namespaced entry for a subject naming nobody); the administrator refusal `STS-FED-0093` with a link and its override; `pre-linked` `STS-FED-0091` and a link made by the admin action; a link on a second person refused; `jit-namespaced`; `any-existing` as the mode says (`STS-FED-0094`, `STS-FED-0095`); the domain, pattern and group rules; an unlink ending the partner's sessions; SCIM's extension creating, refusing and removing a link; and the two paged views |
