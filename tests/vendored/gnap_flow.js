...
function harness(options) {
  ...
  const self = { base: base, api: api, realm: REALM, realmBase: realmBase, realmApi: realmApi,
                 GRANT: GRANT, FINISH: FINISH, DEMO: DEMO, RS: realmBase + "/gnap/rs/resource",
                 checks: 0 };
  ...
  self.ok = async function (url, body, what) {
    ...
    assert.ok(r.status === 200 && r.body && r.body.ok !== false,
      what + ": " + url + " answered " + r.status + " " + String(r.raw).slice(0, 400));
    return r.body;
  };

  self.setting = async function (key, value) {
    await self.ok(realmApi + "/config/set", { key: key, value: String(value) }, "set " + key);
  };
  ...
  self.refused = function (r, code, what) {
    ...
    assert.strictEqual(got, code, what + " should be refused " + code + "; it answered " + r.status +
                       " " + String(r.text).slice(0, 400));
    assert.ok(r.headers["cache-control"] === "no-store", what + ": section 3 requires no-store");
    return r.json;
  };
  ...
  self.browser = function () {
    const b = {
      ...
      async go(method, url, form) {
        const headers = {};
        const cookie = Object.keys(b.jar).map(function (k) { return k + "=" + b.jar[k]; }).join("; ");
        if (cookie) {
        ...
        const r = await fetch(self.absolute(url), { method: method, redirect: "manual", headers: headers,
                                                    body: body });
        (r.headers.getSetCookie ? r.headers.getSetCookie() : []).forEach(function (one) {
  ...
  self.reachApproval = async function (b, startUrl, who) {
    ...
    assert.ok(r.status === 303 && /\/gnap\/approve\//.test(r.location),
      "an interaction start URI forwards to the approval page; got " + r.status + " " + r.location +
      " " + r.text.slice(0, 200));
    ...
    if (r.status === 303 && /\/authn\/login\?authn=/.test(r.location)) {
      ...
      assert.ok(authnId, "the sign-in screen carries an authn_id: " + r.text.slice(0, 300));
      ...
      assert.ok(r.status === 303, "the sign-in redirects back; got " + r.status + " " + r.text.slice(0, 300));
      r = await b.go("GET", r.location);
    }
  ...
  self.answer = async function (b, approval, action, untick) {
    ...
    if (action === "allow" && ((page.status === 303 && /hash=/.test(page.location)) ||
                               (page.status === 200 && /You approved the request/.test(page.text)))) {
      return page;
    }
    assert.ok(page.status === 200 && /Allow access\?/.test(page.text),
      "the approval page is drawn; got " + page.status + " " + page.text.slice(0, 300));
    const rights = [];
    ...
    let form = "action=" + action + "&csrf_token=" + encodeURIComponent(self.csrfOf(page.text)) +
  ...
  self.finishParams = function (location, response) {
    assert.ok(/^https?:\/\//.test(String(location || "")),
      ...
      (response ? response.status + " " + String(response.text).slice(0, 600) : String(location)));
    const u = new URL(location);
    return { hash: u.searchParams.get("hash"), ref: u.searchParams.get("interact_ref") };
  };
  ...
  self.grantBody = function (client, extra, displayName) {
    return Object.assign({
      ...
      client: { key: client.keyObject(), display: { name: displayName || "GNAP job",
                                                    uri: "https://client.gnap.test/" } },
      interact: { start: ["redirect"], finish: { method: "redirect", uri: FINISH, nonce: "n-" +
                  nodeCrypto.randomBytes(8).toString("hex") } }
    }, extra || {});
  };

  // The whole redirect / redirect-finish dance, returning the released response.
  self.redirectGrant = async function (client, who, extra, opts) {
    ...
    assert.strictEqual(r.status, 200, "the grant request is accepted: " + r.text.slice(0, 400));
    const pending = r.json;
    const b = (opts && opts.browser) || self.browser();
    const approval = await self.reachApproval(b, pending.interact.redirect, who);
    r = await self.answer(b, approval, "allow");
    const finished = self.finishParams(r.location, r);
    const expected = gnap.interactionHash(body.interact.finish.nonce, pending.interact.finish,
                                          finished.ref, url);
    assert.strictEqual(finished.hash, expected, "the interaction hash (section 4.2.3) verifies");
    r = await client.send("POST", pending.continue.uri, { token: pending.continue.access_token.value,
                                                          json: { interact_ref: finished.ref } });
    assert.strictEqual(r.status, 200, "the continuation releases the grant: " + r.text.slice(0, 400));
    return { pending: pending, released: r.json, browser: b, body: body, interactRef: finished.ref };
  };
  ...
  self.ensurePerson = async function (who) {
    const r = await self.apiPost(realmApi + "/users/create", {
      ...
      attributes: { cn: "GNAP " + who, sn: who, givenName: "GNAP", displayName: "GNAP " + who,
                    mail: who + "@gnap.test" } });
    assert.ok(r.status === 200 && r.body.ok, "created " + who + ": " + r.raw.slice(0, 300));
  };

  self.createRealm = async function (name) {
    await self.ok(api + "/realms/create", { id: REALM, name: name }, "created the realm");
  };
