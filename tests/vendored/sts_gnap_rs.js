...
var log = bunyan.createLogger({ name: "sts_gnap_rs", level: appconfig.LOG_LEVEL || "info" });
...
function verifyJws(compact, jwks) {
  ...
  assert.strictEqual(parts.length, 3, "a JWS compact serialization has three parts");
  ...
  if (/^PS/.test(header.alg)) {
    good = nodeCrypto.verify(hash, signed, { key: key, padding: nodeCrypto.constants.RSA_PKCS1_PSS_PADDING,
                                             saltLength: nodeCrypto.constants.RSA_PSS_SALTLEN_DIGEST }, signature);
  } else if (/^ES/.test(header.alg)) {
    good = nodeCrypto.verify(hash, signed, { key: key, dsaEncoding: "ieee-p1363" }, signature);
  } else {
    good = nodeCrypto.verify(hash, signed, key, signature);
  }
  assert.ok(good, "the JWS signature verifies against the JWKS (" + header.alg + ")");
  return { header: header, claims: b64json(parts[1]) };
}

// RFC 7638: the required members in lexicographic order, no whitespace.
function jwkThumbprint(jwk) {
  const members = { EC: ["crv", "kty", "x", "y"], RSA: ["e", "kty", "n"], OKP: ["crv", "kty", "x"] }[jwk.kty];
  const canonical = "{" + members.map(function (m) { return JSON.stringify(m) + ":" + JSON.stringify(jwk[m]); })
    .join(",") + "}";
...
function openJwe(compact, privateKey) {
  const parts = String(compact).split(".");
  assert.strictEqual(parts.length, 5, "a JWE compact serialization has five parts");
  const header = b64json(parts[0]);
  assert.strictEqual(header.alg, "RSA-OAEP-256", "encrypted to the resource server's RSA key");
  ...
  const decipher = nodeCrypto.createDecipheriv("aes-" + bits + "-gcm", cek, Buffer.from(parts[2], "base64url"));
...
function decodeMacaroonV2(bytes) {
  assert.strictEqual(bytes[0], 2, "a V2 macaroon starts with the version byte 2");
  let i = 1;
  function field() {
  ...
  const out = { location: null, identifier: null, caveats: [], signature: null };
  let f = field();
  while (f.type !== 0) {
...
function macaroonSignature(rootKey, decoded) {
  let sig = hmac(hmac(Buffer.from("macaroons-key-generator"), rootKey), decoded.identifier);
...
function ed25519PublicFromRaw(raw) {
  return nodeCrypto.createPublicKey({ key: { kty: "OKP", crv: "Ed25519", x: gnap.b64u(raw) }, format: "jwk" });
}
...
function verifyBiscuit(value, rootRaw) {
  ...
  const good = nodeCrypto.verify(null, Buffer.concat([block, le, nextPublic]), ed25519PublicFromRaw(rootRaw),
                                 signature);
  ...
  if (nextSecret) {
    const derived = nodeCrypto.createPrivateKey({ key: Buffer.concat([
      Buffer.from("302e020100300506032b657004220420", "hex"), nextSecret]), format: "der", type: "pkcs8" });
    const pub = nodeCrypto.createPublicKey(derived).export({ format: "jwk" }).x;
    proofMatches = pub === gnap.b64u(nextPublic);
  }
  return { signatureVerifies: good, proofMatches: proofMatches, algorithm: algorithm, block: block,
...
function introspect(rs, token, extra) {
  return rs.send("POST", h.realmBase + "/gnap/introspect", {
    json: Object.assign({ access_token: token, resource_server: { key: rs.keyObject() } }, extra || {}) });
}
...
async function test() {
  ...
  const rsJweJwk = Object.assign(rsJwe.publicKey.export({ format: "jwk" }), { alg: "RSA-OAEP-256", use: "enc" });
  ...
  await h.ok(h.realmApi + "/applications/create", {
    ...
    fields: { gnapKey: JSON.stringify(rsKey.keyObject()), gnapJweKey: JSON.stringify(rsJweJwk),
              gnapResourceServerUri: RS_URI } }, "registered the resource server");
  ...
  for (const format of FORMATS) {
    const r = await rsKey.send("POST", h.realmBase + "/gnap/resource", { json: {
      access: [{ type: "https://rs.gnap.test/photos", actions: ["read"], locations: [RS_URI] }],
      resource_server: { key: rsKey.keyObject() }, token_formats_supported: [format] } });
    check("registering a resource set that only accepts " + format + " answers a reference", function () {
      assert.strictEqual(r.status, 200, r.text);
      assert.ok(r.json.resource_reference, r.text);
      assert.strictEqual(r.json.introspection_endpoint, h.realmBase + "/gnap/introspect");
    });
    references[format] = r.json.resource_reference;
  }
  let r = await rsKey.send("POST", h.realmBase + "/gnap/resource", { json: {
    access: [{ type: "https://rs.gnap.test/photos", actions: ["read"], locations: [RS_URI] }],
    resource_server: { key: rsKey.keyObject() }, token_formats_supported: ["macaroon"] } });
  ...
  r = await rsKey.send("POST", h.realmBase + "/gnap/resource", { json: {
    access: ["x"], resource_server: { key: rsKey.keyObject() }, token_formats_supported: ["paseto"] } });
  check("a set whose only token format this AS does not issue is invalid_request", function () {
  ...
  r = await rsKey.send("POST", h.realmBase + "/gnap/resource", { json: { access: ["x"] } });
  check("a registration naming no resource server is invalid_resource_server (section 3.2)", function () {
    h.refused(r, "invalid_resource_server", "no resource_server");
  });
  r = await rsKey.send("POST", h.realmBase + "/gnap/resource", { json: {
    access: ["x"], resource_server: { key: rsKey.keyObject() } }, sign: false });
  check("an unsigned registration is refused as the resource server's failure", function () {
    h.refused(r, "invalid_resource_server", "an unsigned registration");
  });
  r = await rsKey.send("POST", h.realmBase + "/gnap/resource", { json: {
    access: ["x"], resource_server: { key: rsKey.keyObject() }, token_introspection_required: "yes" } });
  check("a non-boolean token_introspection_required fails the JSON Schema", function () {
    h.refused(r, "invalid_request", "a string where a boolean belongs");
  });

  const entry = await h.apiGet(h.realmApi + "/applications?application=" + encodeURIComponent(RS_ID));
  let macaroonRoot;
  check("registration wrote the macaroon root key onto the resource server's own entry", function () {
    const values = entry.body.attributes.gnapMacaroonKey || entry.body.attributes.gnapmacaroonkey;
    assert.ok(values && values[0], JSON.stringify(Object.keys(entry.body.attributes)));
  ...
  const material = (await gnap.rawRequest("GET", h.realmBase + "/gnap/keys", {})).json;
  const controllerDoc = await gnap.rawRequest("GET", h.realmBase + "/gnap/zcap/controller", {});
  check("the verification material and the controller document are published no-store", function () {
    assert.ok(material && material.biscuit && material.zcap, JSON.stringify(material));
  ...
  const rootRaw = Buffer.from(String(material.biscuit.root_public_key).replace(/^ed25519\//, ""), "hex");
  const clientJkt = jwkThumbprint(client.key.publicJwk);
  const issued = {};
  for (const format of FORMATS) {
    const done = await h.redirectGrant(client, OWNER, { access_token: { access: [references[format]] } });
    const token = done.released.access_token;
    issued[format] = token.value;
    r = await introspect(rsKey, token.value);
    check(format + ": introspection by its resource server is active with the section 2.1 model", function () {
      assert.strictEqual(r.status, 200, r.text);
      ...
      assert.ok(r.json.key && r.json.key.jwk, "a bound token's key is returned");
      assert.deepStrictEqual(r.json.access, [references[format]]);
    });
    if (format === "jwt-signed") {
      check("jwt-signed: the JWS verifies against /oauth2/jwks here, and cnf.jkt is the client's RFC 7638 " +
            "thumbprint computed here", function () {
      ...
      check("jwt-encrypted: this file's RSA private key opens the JWE, and the JWS inside verifies", function () {
      ...
      check("macaroon: the V2 binary decodes here and the HMAC chain recomputed from the root key on the " +
            "entry equals the signature", function () {
              ...
              const caveats = decoded.caveats.map(function (c) { return c.identifier.toString("utf8"); });
              assert.ok(caveats.indexOf("gnap:aud=" + RS_ID) >= 0, caveats.join(" | "));
              assert.ok(caveats.indexOf("gnap:cnf=jkt:" + clientJkt) >= 0, caveats.join(" | "));
              assert.ok(macaroonSignature(macaroonRoot, decoded).equals(decoded.signature),
                        "the recomputed MAC matches");
              assert.ok(!macaroonSignature(nodeCrypto.randomBytes(32), decoded).equals(decoded.signature),
                        "and a different root key does not");
            });
      check("macaroon: appending a caveat without re-keying breaks the chain (the attenuation rule)", function () {
        const decoded = decodeMacaroonV2(Buffer.from(token.value, "base64url"));
        decoded.caveats.push({ identifier: Buffer.from("gnap:label=forged"), vid: null });
        assert.ok(!macaroonSignature(macaroonRoot, decoded).equals(decoded.signature));
      });
    } else if (format === "biscuit") {
      check("biscuit: the authority block's Ed25519 signature verifies here against the published root key, " +
            "and the proof's next secret is the next key's private half", function () {
              ...
              assert.ok(v.block.toString("latin1").indexOf(clientJkt) >= 0, "the block carries the binding");
              const wrong = verifyBiscuit(token.value, nodeCrypto.randomBytes(32));
              assert.ok(!wrong.signatureVerifies, "a different root key does not verify");
            });
    } else if (format === "zcap") {
      check("zcap: the capability's verification method resolves through the published controller document " +
            "to the published Ed25519 key", function () {
              const cap = JSON.parse(Buffer.from(token.value, "base64url").toString("utf8"));
              const proof = Array.isArray(cap.proof) ? cap.proof[0] : cap.proof;
              assert.strictEqual(proof.type, "Ed25519Signature2020");
              assert.ok(/^z/.test(proof.proofValue) && base58btc(proof.proofValue.slice(1)).length === 64,
              ...
              const method = methods.filter(function (m) {
                return m && typeof m === "object" && m.id === proof.verificationMethod;
              })[0];
              assert.ok(method, "the controller document names " + proof.verificationMethod);
              const multikey = base58btc(String(method.publicKeyMultibase).slice(1));
              assert.strictEqual(multikey.slice(0, 2).toString("hex"), "ed01", "an Ed25519 multikey");
              assert.strictEqual(gnap.b64u(multikey.slice(2)), material.biscuit.jwk.x,
                                 "the same key /gnap/keys publishes");
            });
    }
    r = await client.send("GET", h.RS, { token: token.value });
    check(format + ": a token issued for one resource server is refused at another (the demonstration RS)",
          function () {
  ...
  check("another resource server introspecting the token is told only active: false", function () {
    assert.strictEqual(r.status, 200, r.text);
    assert.deepStrictEqual(r.json, { active: false });
  });
  r = await introspect(rsKey, issued["jwt-signed"], { proof: "mtls" });
  check("naming a proof method the token is not bound with is active: false", function () {
    assert.deepStrictEqual(r.json, { active: false });
  });
  ...
  r = await introspect(rsKey, issued["jwt-signed"], { access: ["something-else"] });
  check("asking about access the token does not carry is active: false", function () {
    assert.deepStrictEqual(r.json, { active: false });
  });
  ...
  r = await rsKey.send("POST", h.realmBase + "/gnap/introspect", { json: {
    access_token: issued.macaroon, resource_server: { key: rsKey.keyObject() } }, sign: false });
  check("an unsigned introspection call is invalid_resource_server", function () {
    h.refused(r, "invalid_resource_server", "an unsigned introspection");
  });
  r = await rsKey.send("POST", h.realmBase + "/gnap/introspect", { json: {
    resource_server: { key: rsKey.keyObject() } } });
  check("an introspection call with no access_token is invalid_request", function () {
    h.refused(r, "invalid_request", "no access_token");
  });
  r = await rsKey.send("POST", h.realmBase + "/gnap/introspect", { json: {
    access_token: 7, resource_server: { key: rsKey.keyObject() } } });
  check("an access_token that is not a string fails the JSON Schema", function () {
  ...
  const upstream = await h.redirectGrant(client, OWNER, { access_token: { access: [references["jwt-signed"]] } });
  ...
  check("the resource server derives a token from the one it was handed, with no interaction", function () {
    assert.strictEqual(r.status, 200, r.text);
  ...
  check("the derived token names the resource server as its client and the same resource owner", function () {
    assert.strictEqual(r.json.active, true, r.text);
    assert.strictEqual(r.json.instance_id, RS_ID);
    assert.ok(/gnap-rs-owner|urn:sts:user:/.test(String(r.json.sub)), String(r.json.sub));
  });
  r = await rsKey.send("POST", h.GRANT, { json: {
    client: { key: rsKey.keyObject() }, existing_access_token: upstreamValue,
    access_token: { access: [references["jwt-signed"], { type: "https://rs.gnap.test/admin", actions: ["delete"] }] } } });
  ...
  r = await stranger.send("POST", h.GRANT, { json: {
    ...
    access_token: { access: [references["jwt-signed"]] } } });
  check("a resource server the token was not issued for cannot derive from it", function () {
  ...
  r = await rsKey.send("POST", h.GRANT, { json: {
    ...
    access_token: { access: [references["jwt-signed"]] } } });
  check("deriving from a token that is not active is invalid_request", function () {
    h.refused(r, "invalid_request", "an inactive existing token");
  });
  r = await rsKey.send("POST", h.GRANT, { json: {
    client: { key: rsKey.keyObject() }, existing_access_token: 42,
    access_token: { access: [references["jwt-signed"]] } } });
  check("an existing_access_token that is not a string fails the JSON Schema", function () {
  ...
  r = await rsKey.send("POST", h.GRANT, { json: {
    client: { key: rsKey.keyObject() }, existing_access_token: upstreamValue,
    access_token: { access: [references["jwt-signed"]] } } });
  check("with gnap.tokenDerivation off a derivation is request_denied", function () {
  ...
  r = await client.send("DELETE", manage.uri, { token: manage.access_token.value });
  ...
  check("…and the resource server's introspection now says active: false", function () {
    assert.deepStrictEqual(r.json, { active: false });
  });
  r = await rsKey.send("POST", h.GRANT, { json: {
    client: { key: rsKey.keyObject() }, existing_access_token: upstreamValue,
  ...
  const mtlsClient = new gnap.Client({ key: { alg: "RS256" }, proof: "mtls",
    tls: { cert: certPem, key: keyPem, certDer: certDer, rejectUnauthorized: false } });
  const mtlsFlow = await h.redirectGrant(mtlsClient, OWNER);
  r = await mtlsClient.send("GET", h.RS, { token: mtlsFlow.released.access_token.value });
  check("a certificate key proved by mutual TLS obtains a token and presents it at the RS", function () {
    assert.strictEqual(r.status, 200, r.text);
    assert.strictEqual(r.json.method, "mtls");
  });
  r = await gnap.rawRequest("GET", h.RS, { Authorization: "GNAP " + mtlsFlow.released.access_token.value });
  check("the same token on a connection with no client certificate is refused", function () {
    assert.strictEqual(r.status, 401, r.text);
  });
  ...
  r = await gnap.rawRequest("GET", h.RS, { Authorization: "GNAP " + mtlsFlow.released.access_token.value },
    null, { cert: forge.pki.certificateToPem(otherCert), key: forge.pki.privateKeyToPem(otherPair.privateKey),
            rejectUnauthorized: false });
  check("…and on a connection presenting a DIFFERENT certificate", function () {
    assert.strictEqual(r.status, 401, r.text);
  });
  r = await gnap.rawRequest("POST", h.GRANT, { "Content-Type": "application/json" },
                            Buffer.from(JSON.stringify(h.grantBody(mtlsClient))));
  check("a grant request naming a certificate key proved by mtls, on a connection with no certificate, " +
        "is invalid_client", function () {
  ...
  check("with gnap.introspection off the document drops the endpoint and the endpoint refuses", function () {
    assert.strictEqual(offDoc.introspection_endpoint, undefined, JSON.stringify(offDoc));
  ...
  check("with gnap.resourceRegistration off registration answers 404", function () {
    assert.strictEqual(r.status, 404, r.text);
  });
  await h.setting("gnap.resourceRegistration", true);

  assert.ok(h.checks >= 40, "only " + h.checks + " checks ran; a section has stopped being called.");
...
program
  .name("sts_gnap_rs")
  .description("GNAP resource server connections (RFC 9767): the five token formats verified by this " +
    "file's own code, introspection active and inactive, resource set registration, token derivation, " +
    "revocation reaching introspection, and a key proved by mutual TLS.")
