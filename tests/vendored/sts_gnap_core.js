...
var log = bunyan.createLogger({ name: "sts_gnap_core", level: appconfig.LOG_LEVEL || "info" });
...
const h = require("./gnap_flow.js").harness({ base: base, realm: REALM, password: PASSWORD, log: log });
var api = h.api;
...
async function redirectGrant(client, who, extra, opts) {
  return h.redirectGrant(client, who, Object.assign({ client: grantBody(client).client }, extra || {}), opts);
}
...
async function test() {
  ...
  await ok(api + "/realms/create", { id: REALM, name: "GNAP core" }, "created the realm");
  ...
  check("OPTIONS on the grant endpoint answers the section 9 document", function () {
    assert.strictEqual(r.status, 200, r.text);
    assert.strictEqual(r.json.grant_request_endpoint, GRANT);
    ["redirect", "app", "user_code", "user_code_uri"].forEach(function (mode) {
      assert.ok(r.json.interaction_start_modes_supported.indexOf(mode) >= 0, mode);
    });
    assert.deepStrictEqual(r.json.key_proofs_supported.slice().sort(), ["httpsig", "jws", "jwsd", "mtls"]);
    assert.strictEqual(r.json.key_rotation_supported, true);
  });
  r = await gnap.rawRequest("GET", realmBase + "/.well-known/gnap-as-rs", {});
  check("the RS-facing discovery document names the five token formats and both RS endpoints",
        function () {
          assert.strictEqual(r.status, 200, r.text);
          assert.deepStrictEqual(r.json.token_formats_supported.slice().sort(),
                                 ["biscuit", "jwt-encrypted", "jwt-signed", "macaroon", "zcap"]);
          assert.strictEqual(r.json.introspection_endpoint, realmBase + "/gnap/introspect");
          assert.strictEqual(r.json.resource_registration_endpoint, realmBase + "/gnap/resource");
        });
  r = await gnap.rawRequest("GET", RS, {});
  check("the demonstration RS answers an unauthenticated request with the RS-first challenge " +
        "(section 9.1)", function () {
          assert.strictEqual(r.status, 401);
          assert.ok(/^GNAP /.test(r.headers["www-authenticate"]), r.headers["www-authenticate"]);
          assert.ok(r.headers["www-authenticate"].indexOf("as_uri=\"" + GRANT + "\"") >= 0);
        });
  ...
  check("an unsigned grant request is invalid_client (section 7.3)", function () {
    refused(r, "invalid_client", "an unsigned request");
    assert.strictEqual(r.status, 401);
  });
  r = await es.send("POST", GRANT, { json: grantBody(es), mutate: function (m) {
    return { body: Buffer.from(m.body.toString().replace("GNAP core job", "GNAP core jab")) };
  } });
  check("content changed after signing fails the Content-Digest check", function () {
    refused(r, "invalid_client", "a tampered body");
  });
  r = await es.send("POST", GRANT, { json: grantBody(es), httpsig: { tag: "not-gnap" } });
  check("a signature without tag=gnap is refused (section 7.3.1)", function () {
    refused(r, "invalid_client", "the wrong tag");
  });
  r = await es.send("POST", GRANT, { json: grantBody(es), httpsig: { created: Math.floor(Date.now() / 1000) - 3600 } });
  check("a signature created an hour ago is refused", function () {
    refused(r, "invalid_client", "a stale created");
  });
  r = await es.send("POST", GRANT, { json: grantBody(es), httpsig: { alg: "ecdsa-p256-sha256" } });
  check("a signature carrying the alg parameter is refused (section 7.3.1)", function () {
    refused(r, "invalid_client", "an explicit alg parameter");
  });
  r = await es.send("POST", GRANT, { json: grantBody(es), httpsig: { keyid: "someone-else" } });
  ...
  r = await es.send("POST", GRANT, { json: grantBody(es), httpsig: { nonce: replayNonce } });
  assert.strictEqual(r.status, 200, r.text);
  r = await es.send("POST", GRANT, { json: grantBody(es), httpsig: { nonce: replayNonce } });
  ...
  r = await es.send("POST", GRANT, { json: grantBody(es, { access_token: { access: readAccess(), label: 42 } }) });
  ...
  r = await es.send("POST", GRANT, { json: grantBody(es, { client: { key: es.keyObject(),
    class_id: "evil\u0007class" } }) });
  check("a control character in class_id is refused by the schema (sanitisation)", function () {
    refused(r, "invalid_request", "a control character");
  });
  r = await es.send("POST", GRANT, { json: grantBody(es, { access_token: { access: readAccess(),
    flags: ["bearer", "bearer"] } }) });
  ...
  r = await es.send("POST", GRANT, { json: grantBody(es, { access_token: { access: readAccess(),
    flags: ["durable"] } }) });
  check("asking for the response-only durable flag is invalid_flag", function () {
  ...
  check("a multiple-token request with an unlabelled token is invalid_request (2.1.2)", function () {
    refused(r, "invalid_request", "a missing label");
  });
  r = await es.send("POST", GRANT, { json: grantBody(es, { access_token: [
    { label: "a", access: readAccess() }, { label: "a", access: readAccess() }] }) });
  ...
  check("a request for neither access nor subject is invalid_request", function () {
    refused(r, "invalid_request", "an empty request");
  });
  const withPrivate = JSON.parse(JSON.stringify(es.keyObject()));
  withPrivate.jwk.d = "AAAA";
  r = await es.send("POST", GRANT, { json: grantBody(es, { client: { key: withPrivate } }) });
  check("a JWK carrying a private member is invalid_client (section 7.1)", function () {
    refused(r, "invalid_client", "a private key by value");
  });
  const twoFormats = Object.assign({}, es.keyObject(), { "cert#S256": gnap.b64u(gnap.sha("sha256", Buffer.from("x"))) });
  r = await es.send("POST", GRANT, { json: grantBody(es, { client: { key: twoFormats } }) });
  check("a key in two formats is invalid_client (section 11.35)", function () {
    refused(r, "invalid_client", "two key formats");
  });
  r = await es.send("POST", GRANT, { json: grantBody(es, { client: undefined }) });
  check("a request with no client is invalid_client (section 2.3)", function () {
    refused(r, "invalid_client", "no client");
  });
  r = await es.send("POST", GRANT, { json: grantBody(es, { client: "no-such-instance" }) });
  check("an unknown instance identifier is invalid_client (2.3.1)", function () {
    refused(r, "invalid_client", "an unknown instance");
  });
  r = await es.send("POST", GRANT, { json: grantBody(es, { interact: undefined }) });
  check("a request needing approval with no interaction is invalid_interaction (2.5)", function () {
    refused(r, "invalid_interaction", "no interaction");
  });
  r = await es.send("POST", GRANT, { json: grantBody(es, { interact: { start: ["telepathy"] } }) });
  ...
  r = await es.send("POST", GRANT, { json: grantBody(es, { interact_ref: "abc" }) });
  check("interact_ref on a new request is invalid_request (5.1)", function () {
    refused(r, "invalid_request", "an interaction reference at the grant endpoint");
  });
  ...
  check("the pending response carries a redirect, a finish nonce, expires_in and a continuation " +
        "with a wait (sections 3.1 and 3.3)", function () {
  ...
  check("the released token carries the value, the access, expires_in and a manage URI", function () {
    assert.ok(/^[A-Za-z0-9._~+/-]+=*$/.test(token.value), "token68");
    assert.deepStrictEqual(token.access, readAccess());
    assert.ok(token.expires_in > 0);
    assert.ok(token.manage && token.manage.uri && token.manage.access_token.value);
    assert.notStrictEqual(token.manage.access_token.value, token.value);
    assert.ok(!token.flags || token.flags.indexOf("bearer") < 0, "bound, not bearer");
  });
  r = await es.send("GET", RS, { token: token.value });
  check("the demonstration RS accepts the bound token with an HTTP signature", function () {
    assert.strictEqual(r.status, 200, r.text);
  ...
  check("the same token signed with another key is refused at the RS", function () {
    assert.strictEqual(r.status, 401, r.text);
  });
  r = await es.send("GET", RS, { token: token.value, sign: false });
  check("the bound token with no proof is refused at the RS (section 7.2)", function () {
    assert.strictEqual(r.status, 401, r.text);
  });
  r = await es.send("GET", RS, { token: flow.released.continue.access_token.value });
  check("a CONTINUATION token is never an access token at the RS (section 5)", function () {
    assert.strictEqual(r.status, 401, r.text);
  });
  r = await es.send("POST", flow.released.continue.uri, {
    token: flow.released.continue.access_token.value, json: { interact_ref: flow.interactRef } });
  check("the interaction reference presented again is too_many_attempts (section 5.1)", function () {
    refused(r, "too_many_attempts", "a replayed interact_ref");
  });
  r = await es.send("POST", flow.released.continue.uri, { token: flow.released.continue.access_token.value });
  check("…and the grant is finalized: a further continuation is invalid_continuation", function () {
  ...
  check("the used redirect URI shows an error and never redirects to the client (4.1.1)", function () {
  ...
  check("the same client asking the same person for the same rights again skips the approval " +
        "page and finishes at once (remembered in the consent register)", function () {
  ...
  check("a denial still enacts the finish method with a valid hash (section 4.2)", function () {
    assert.strictEqual(params.hash, gnap.interactionHash(body.interact.finish.nonce,
      pending.interact.finish, params.ref, GRANT));
  });
  r = await denier.send("POST", pending.continue.uri, { token: pending.continue.access_token.value,
                                                        json: { interact_ref: params.ref } });
  check("the continuation says user_denied, with a continue member to try again (5.1)", function () {
  ...
  body = grantBody(naming, { user: { sub_ids: [{ format: "uri", uri: "urn:sts:user:" + STRANGER }] } });
  ...
  r = await naming.send("POST", pending.continue.uri, { token: pending.continue.access_token.value,
                                                        json: { interact_ref: params.ref } });
  check("a request naming one person approved by another is unknown_user (section 2.4)", function () {
  ...
  pending = r.json;
  check("a code-and-app request gets a user code, a user_code_uri and an app URI but no finish",
        function () {
          assert.ok(/^[A-Z0-9]{6,8}$/.test(pending.interact.user_code), pending.interact.user_code);
          assert.strictEqual(pending.interact.user_code_uri.code, pending.interact.user_code);
          assert.strictEqual(pending.interact.user_code_uri.uri, realmBase + "/gnap/code");
          assert.ok(/\/gnap\/app\//.test(pending.interact.app));
          assert.ok(!pending.interact.finish);
        });
  r = await poller.send("POST", pending.continue.uri, { token: pending.continue.access_token.value });
  check("a poll before approval answers only a new continuation (5.2)", function () {
    assert.strictEqual(r.status, 200, r.text);
    assert.ok(r.json.continue && !r.json.access_token);
    assert.notStrictEqual(r.json.continue.access_token.value, pending.continue.access_token.value);
  });
  let cont = r.json.continue;
  r = await poller.send("POST", cont.uri, { token: pending.continue.access_token.value });
  check("the previous continuation token stopped working when a new one was issued (5)", function () {
    refused(r, "invalid_continuation", "a superseded continuation token");
  });
  b = browser();
  r = await b.go("POST", "/realm/" + REALM + "/gnap/code", { code: "NOTACODE" });
  check("an unknown user code is an error on the page, not a redirect (4.1.2)", function () {
    assert.strictEqual(r.status, 400);
  ...
  check("a user code typed in lower case with spaces and a dash is accepted (4.1.2)", function () {
    assert.strictEqual(r.status, 303, r.text.slice(0, 300));
  ...
  r = await answer(b, approval, "allow");
  check("with no finish method the RO is told to return to the device", function () {
  ...
  check("the app URI of a grant already approved through a code is refused (4.1)", function () {
    assert.strictEqual(r.status, 400, r.text.slice(0, 200));
  });
  r = await poller.send("POST", GRANT, { json: grantBody(poller, { interact: { start: ["app"] } }) });
  pending = r.json;
  b = browser();
  approval = await reachApproval(b, pending.interact.app, OWNER);
  r = await answer(b, approval, "allow");
  r = await poller.send("POST", pending.continue.uri, { token: pending.continue.access_token.value });
  check("the app start mode reaches the same approval and polling releases the token", function () {
    assert.strictEqual(r.status, 200, r.text);
    assert.ok(r.json.access_token);
  });
  ...
  const listener = http.createServer(function (req, res) {
    ...
    req.on("end", function () {
      pushed.push({ method: req.method, type: req.headers["content-type"], body: text });
      res.writeHead(204);
      res.end();
    });
  });
  await new Promise(function (resolve) { listener.listen(0, "0.0.0.0", resolve); });
  const callbackHost = process.env.GNAP_PUSH_HOST || "localhost";
  const pushUri = "http://" + callbackHost + ":" + listener.address().port + "/gnap-push";
  body = grantBody(es, { interact: { start: ["redirect"], finish: { method: "push", uri: pushUri,
                                                                    nonce: "push-nonce-1" } } });
  r = await es.send("POST", GRANT, { json: body });
  pending = r.json;
  b = browser();
  approval = await reachApproval(b, pending.interact.redirect, OWNER);
  r = await answer(b, approval, "allow");
  check("after approval the browser is shown a page, not redirected (push)", function () {
    assert.strictEqual(r.status, 200, r.text.slice(0, 300));
  });
  check("the AS POSTed {hash, interact_ref} as JSON to the push URI with a verifiable hash",
        function () {
          ...
          assert.strictEqual(message.hash, gnap.interactionHash("push-nonce-1", pending.interact.finish,
                                                                message.interact_ref, GRANT));
          pending.pushedRef = message.interact_ref;
        });
  r = await es.send("POST", pending.continue.uri, { token: pending.continue.access_token.value,
                                                    json: { interact_ref: pending.pushedRef } });
  ...
  const multi = await redirectGrant(es, OWNER, {
    ...
    subject: { sub_id_formats: ["opaque", "iss_sub", "email"], assertion_formats: ["id_token", "saml2"] }
  });
  const tokensByLabel = {};
  check("a multiple-token request is answered with an array, one per label (3.2.2)", function () {
  ...
  r = await gnap.rawRequest("GET", RS, { Authorization: "Bearer " + tokensByLabel.carry.value });
  check("the bearer token works with the Bearer scheme and no proof (7.2)", function () {
    assert.strictEqual(r.status, 200, r.text);
  });
  r = await es.send("GET", RS, { token: tokensByLabel.carry.value });
  check("…and is refused with the GNAP scheme (7.2 names RFC 6750 for bearer tokens)", function () {
    assert.strictEqual(r.status, 401, r.text);
  });
  r = await gnap.rawRequest("GET", RS, { Authorization: "Bearer " + tokensByLabel.bound.value });
  check("the bound token is refused with the Bearer scheme", function () {
    assert.strictEqual(r.status, 401, r.text);
  });
  const subject = multi.released.subject;
  let opaque;
  check("subject information carries the requested sub_ids, both assertions and updated_at (3.4)",
        function () {
          assert.ok(subject, JSON.stringify(multi.released));
          const formats = subject.sub_ids.map(function (one) { return one.format; }).sort();
          assert.deepStrictEqual(formats, ["email", "iss_sub", "opaque"]);
          opaque = subject.sub_ids.filter(function (one) { return one.format === "opaque"; })[0].id;
          assert.ok(subject.sub_ids.filter(function (one) { return one.format === "email"; })[0].email
                    .indexOf(OWNER) === 0);
          const formatsA = subject.assertions.map(function (one) { return one.format; }).sort();
  ...
  check("the id_token assertion verifies against the realm JWKS and names the resource owner",
        function () {
          const idToken = subject.assertions.filter(function (one) { return one.format === "id_token"; })[0].value;
          const parts = idToken.split(".");
          const header = JSON.parse(Buffer.from(parts[0], "base64url"));
          const jwk = jwks.keys.filter(function (k) { return k.kid === header.kid; })[0];
          const ok1 = nodeCrypto.verify("sha256", Buffer.from(parts[0] + "." + parts[1]),
            nodeCrypto.createPublicKey({ key: jwk, format: "jwk" }), Buffer.from(parts[2], "base64url"));
          assert.ok(ok1, "the ID Token signature verifies");
          const claims = JSON.parse(Buffer.from(parts[1], "base64url"));
          assert.strictEqual(claims.sub, "urn:sts:user:" + OWNER);
        });
  check("the saml2 assertion is unpadded base64url of a SAML 2.0 Assertion naming the owner",
        function () {
          const value = subject.assertions.filter(function (one) { return one.format === "saml2"; })[0].value;
          assert.ok(!/=/.test(value) && /^[A-Za-z0-9_-]+$/.test(value));
          const xml = Buffer.from(value, "base64url").toString("utf8");
          assert.ok(/<(saml2?:)?Assertion\b/.test(xml) && xml.indexOf(OWNER) >= 0, xml.slice(0, 300));
        });
  const hinted = new gnap.Client({ key: gnap.newKey("ES256") });
  r = await hinted.send("POST", GRANT, { json: grantBody(hinted, { user: opaque }) });
  check("the opaque identifier is usable as a user reference (2.4.1)", function () {
    assert.strictEqual(r.status, 200, r.text);
  });
  r = await hinted.send("POST", GRANT, { json: grantBody(hinted, { user: "not-a-reference" }) });
  ...
  check("narrowing the access issues a new token with no interaction (5.3)", function () {
    assert.strictEqual(r.status, 200, r.text);
    assert.deepStrictEqual(r.json.access_token.access, [{ type: DEMO, actions: ["read"] }]);
    assert.notStrictEqual(r.json.access_token.value, firstValue);
  });
  const narrowed = r.json;
  r = await es.send("GET", RS, { token: firstValue });
  check("the earlier non-durable token was revoked by the modification", function () {
    assert.strictEqual(r.status, 401, r.text);
  });
  r = await es.send("POST", RS, { token: narrowed.access_token.value, json: { x: 1 } });
  check("the narrowed token no longer grants write at the RS", function () {
    assert.strictEqual(r.status, 403, r.text);
  });
  r = await es.send("PATCH", narrowed.continue.uri, { token: narrowed.continue.access_token.value,
    json: { access_token: { access: [{ type: DEMO, actions: ["read", "write", "delete"] }] } } });
  check("widening the access with no interaction offered is request_denied", function () {
  ...
  check("DELETE on the continuation URI revokes the grant with 204 (5.4)", function () {
  ...
  r = await es.send("POST", current.manage.uri, { token: current.manage.access_token.value });
  check("POST to the manage URI rotates the token, keeping its access (6.1)", function () {
    assert.strictEqual(r.status, 200, r.text);
  ...
  r = await es.send("POST", rotated.manage.uri, { token: current.manage.access_token.value });
  ...
  r = await es.send("POST", rotated.manage.uri, { token: rotated.manage.access_token.value,
    json: { key: newKey.keyObject() } });
  check("a key rotation proved only by the old key is invalid_rotation (6.1.1)", function () {
    refused(r, "invalid_rotation", "a rotation without the new key's signature");
  });
  r = await es.send("POST", rotated.manage.uri, { token: rotated.manage.access_token.value,
    json: { key: newKey.keyObject() }, rotateTo: newKey });
  check("a key rotation proved by both keys binds the token to the new key (7.3.1.1)", function () {
    assert.strictEqual(r.status, 200, r.text);
  });
  ...
  r = await newKey.send("DELETE", rekeyed.manage.uri, { token: rekeyed.manage.access_token.value });
  check("DELETE on the manage URI revokes the token with 204 (6.2)", function () {
    assert.strictEqual(r.status, 204, r.text);
  });
  r = await newKey.send("GET", RS, { token: rekeyed.value });
  ...
  const carrying = await redirectGrant(es, OWNER, { access_token: { access: readAccess(), flags: ["bearer"] } });
  r = await es.send("POST", carrying.released.access_token.manage.uri, {
    ...
    json: { key: newKey.keyObject() }, rotateTo: newKey });
  check("rotating the key of a bearer token is invalid_rotation (6.1.1)", function () {
  ...
  r = await es.send("POST", off.released.access_token.manage.uri, {
    ...
    json: { key: newKey.keyObject() }, rotateTo: newKey });
  check("with key rotation switched off the answer is key_rotation_not_supported", function () {
  ...
  for (const method of ["jwsd", "jws"]) {
    const signer = new gnap.Client({ key: gnap.newKey(method === "jws" ? "EdDSA" : "RS256"), proof: method });
    const done = await redirectGrant(signer, OWNER);
    r = await signer.send("GET", RS, { token: done.released.access_token.value });
    check(method + ": a grant, its continuation and an RS call all prove the key", function () {
      assert.strictEqual(r.status, 200, r.text);
      assert.strictEqual(r.json.method, method);
    });
    r = await signer.send("POST", GRANT, { json: grantBody(signer), jwsTyp: "gnap-binding-wrong" });
    check(method + ": the wrong typ is refused", function () {
      refused(r, "invalid_client", method + " with the wrong typ");
    });
    const next = new gnap.Client({ key: gnap.newKey(method === "jws" ? "EdDSA" : "RS256"), proof: method });
    const mng = done.released.access_token.manage;
    r = await signer.send("POST", mng.uri, { token: mng.access_token.value, json: { key: next.keyObject() },
                                             rotateTo: next });
  ...
  r = await mixed.send("POST", GRANT, { json: grantBody(mixed), proof: "httpsig" });
  check("a key declared jwsd and proved with an HTTP signature is refused", function () {
  ...
  await ok(realmApi + "/applications/create", {
    ...
    fields: { gnapKeyReference: REF, gnapSymmetricKey: secret.toString("base64url"),
              gnapKeyProof: "httpsig", gnapSymmetricAlg: "hmac-sha256" } },
    "registered a client with a shared secret");
  const hmacClient = new gnap.Client({ key: gnap.secretKey(REF, secret, "HS256") });
  r = await hmacClient.send("POST", GRANT, { json: grantBody(hmacClient) });
  check("a key reference to a shared secret proves an HMAC HTTP signature (7.1.1)", function () {
    assert.strictEqual(r.status, 200, r.text);
  });
  const wrongSecret = new gnap.Client({ key: gnap.secretKey(REF, nodeCrypto.randomBytes(32), "HS256") });
  r = await wrongSecret.send("POST", GRANT, { json: grantBody(wrongSecret) });
  check("the same reference with the wrong secret is invalid_client", function () {
    refused(r, "invalid_client", "a wrong shared secret");
  });
  r = await es.send("POST", GRANT, { json: grantBody(es, { client: flow.pending.instance_id }) });
  check("the dynamic instance_id is accepted by reference with the same key (2.3.1)", function () {
    assert.strictEqual(r.status, 200, r.text);
  });
  r = await thief.send("POST", GRANT, { json: grantBody(thief, { client: flow.pending.instance_id }) });
  ...
  await ok(realmApi + "/applications/create", {
    identifier: "gnap-trusted-" + REALM, kind: "gnap-client", protocols: ["gnap"],
    fields: { gnapKey: JSON.stringify(trustedKey.keyObject()), gnapSkipInteraction: "TRUE" } },
    "registered a trusted client");
  r = await trustedKey.send("POST", GRANT, { json: { client: { key: trustedKey.keyObject() },
                                                     access_token: { access: readAccess() } } });
  check("a registered trusted client gets a token with no interaction (2.3.3)", function () {
    assert.strictEqual(r.status, 200, r.text);
  ...
  check("a named authorization server has its own grant endpoint in discovery", function () {
    assert.strictEqual(r.json.grant_request_endpoint, asGrant);
  });
  await ok(realmApi + "/authorization-servers/set", { profile: AS, member: "interaction_start_modes_supported",
                                                      value: JSON.stringify(["user_code"]) },
           "narrowed the start modes");
  r = await gnap.rawRequest("OPTIONS", asGrant, {});
  check("the profile's GNAP override is published by that server's discovery", function () {
    assert.deepStrictEqual(r.json.interaction_start_modes_supported, ["user_code"]);
  });
  r = await gnap.rawRequest("GET", realmBase + "/" + AS + "/.well-known/oauth-authorization-server", {});
  check("…and does not leak into its OAuth metadata", function () {
    assert.ok(!r.json || r.json.interaction_start_modes_supported === undefined, r.text.slice(0, 200));
  });
  r = await es.send("POST", asGrant, { json: grantBody(es) });
  check("…and is ENFORCED: redirect is no longer offered there, so invalid_interaction", function () {
  ...
  check("a grant at the named server records it: tokens will say iss = that grant endpoint", function () {
  ...
  r = await poller.send("POST", GRANT, { json: grantBody(poller, { interact: { start: ["user_code"] } }) });
  pending = r.json;
  check("the continuation response states the wait", function () {
    assert.strictEqual(pending.continue.wait, 30);
  });
  r = await poller.send("POST", pending.continue.uri, { token: pending.continue.access_token.value });
  check("continuing before the wait is too_fast, with a continuation to use later", function () {
  ...
  r = await es.send("GET", base + "/gnap/rs/resource", { token: isolated.released.access_token.value });
  check("a token from this realm is refused by the default realm's resource server", function () {
    assert.strictEqual(r.status, 401, r.text);
  });
  const foreignCont = isolated.released.continue.uri.replace("/realm/" + REALM, "");
  r = await es.send("POST", foreignCont, { token: isolated.released.continue.access_token.value });
  check("this realm's continuation token finds no grant in the default realm", function () {
    refused(r, "invalid_continuation", "a continuation token in another realm");
  });

  assert.ok(h.checks >= 90, "only " + h.checks + " checks ran; a section has stopped being called.");
...
program
  .name("sts_gnap_core")
  .description("GNAP (RFC 9635) against the running service: every interaction start mode and " +
    "finish method, the four key proofing methods, continuation, modification, revocation, token " +
    "rotation and key rotation, subject assertions, key references, instance identifiers, " +
    "authorization server profiles, too_fast, realm isolation, and the refusals of each.")
