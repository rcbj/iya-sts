...
class SpiffeId {
  ...
  byteLength(text) {
    ...
    log.debug("Leaving SpiffeId.byteLength().");
  ...
  // READING ONE.
  //
  // Returns an object rather than throwing, and it always returns an object:
  // the callers are protocol handlers that have to answer with a specific error
  // code (`InvalidArgument` on the SPIRE Server API, a refusal on the console,
  // a 400 on the bundle endpoint) and each of them wants the REASON in its own
  // words. A thrown Error would make every caller write the same try/catch and
  // would lose the distinction between "this is not a SPIFFE ID" and "this is a
  // SPIFFE ID belonging to somebody else".
  //
  ...
  parse(value) {
    ...
    if (!text) {
      log.debug('Leaving SpiffeId.parse(). Empty.');
      return { ok: false,
               reason: 'A SPIFFE ID is required and none was given.' };
    }
    if (this.byteLength(text) > MAX_ID_BYTES) {
      log.debug('Leaving SpiffeId.parse(). Too long.');
      return { ok: false, reason: 'A SPIFFE ID may be at most ' + MAX_ID_BYTES +
                                  ' bytes; this one is ' +
                                  this.byteLength(text) +
                                  '.' };
    }
    // The scheme, case-insensitively, and then everything after it is handled
    // as raw text. Deliberately NOT `new URL()`: that parser lower-cases the
    // host, normalises `.` and `..` out of the path, accepts percent-encoding
    // and accepts a port — four defects it would hide rather than report.
    if (text.slice(0, PREFIX.length).toLowerCase() !== PREFIX) {
      log.debug('Leaving SpiffeId.parse(). Wrong scheme.');
    ...
    // of the thing is.
    if (!authority) {
      log.debug('Leaving SpiffeId.parse(). No trust domain.');
      return { ok: false, reason: 'A SPIFFE ID names a trust domain between ' +
                                  PREFIX + ' and the first /; this one names ' +
                                           'none.' };
    }
    if (authority.indexOf('@') !== -1) {
      log.debug('Leaving SpiffeId.parse(). Userinfo.');
      return { ok: false, reason: 'A SPIFFE ID carries no userinfo; the ' +
                                  'trust domain is the whole authority.' };
    }
    if (authority.indexOf(':') !== -1) {
      log.debug('Leaving SpiffeId.parse(). Port.');
      return { ok: false, reason: 'A SPIFFE ID carries no port; the trust ' +
                                  'domain is the whole authority.' };
    }
    if (this.byteLength(authority) > MAX_TRUST_DOMAIN_BYTES) {
      log.debug('Leaving SpiffeId.parse(). Trust domain too long.');
      return { ok: false, reason: 'A trust domain name may be at most ' +
                                  MAX_TRUST_DOMAIN_BYTES +
                                  ' bytes; this one is ' +
                                  this.byteLength(authority) + '.' };
    }
    if (!TRUST_DOMAIN_CHARS.test(authority)) {
      ...
      log.debug('Leaving SpiffeId.parse(). Bad trust domain characters.');
      return { ok: false, reason: upper
        ? 'A trust domain name is lower-case: ' + authority +
          ' is not a valid trust domain, and it is not another spelling of ' +
    ...
    if (path) {
      if (path.indexOf('?') !== -1 || path.indexOf('#') !== -1) {
        log.debug('Leaving SpiffeId.parse(). Query or fragment.');
        return { ok: false, reason: 'A SPIFFE ID carries no query and no ' +
                                    'fragment.' };
      }
      if (path.indexOf('%') !== -1) {
        log.debug('Leaving SpiffeId.parse(). Percent-encoding.');
        return { ok: false,
                 reason: 'A SPIFFE ID path is not percent-encoded; ' +
      ...
      for (let i = 1; i < parts.length; i++) {
        ...
        if (!segment) {
          log.debug('Leaving SpiffeId.parse(). Empty segment.');
          return { ok: false,
                   reason: 'A SPIFFE ID path has no empty segment, ' +
                                      'so no trailing slash and no //.' };
        }
        if (segment === '.' || segment === '..') {
          log.debug('Leaving SpiffeId.parse(). Relative segment.');
          return { ok: false, reason: 'A SPIFFE ID path has no relative ' +
                                      'segment: . and .. are not permitted.' };
        }
        if (!PATH_SEGMENT_CHARS.test(segment)) {
          log.debug('Leaving SpiffeId.parse(). Bad path characters.');
    ...
    log.debug('Leaving SpiffeId.parse(). trustDomain=' + authority + ', path=' +
              (path || '(none)'));
  ...
  isValid(value) {
    ...
    log.debug("Leaving SpiffeId.isValid().");
    return this.parse(value).ok;
  }
  ...
  // '/ns/default/sa/web')` are the same call: the leading slash is supplied
  // where it is missing, because half the callers here have a path that came
  // off a protobuf field (which carries it with the slash) and half are
  // building one from parts. It THROWS on something that is not a valid
  // identifier, which is the opposite of parse() and is deliberate — a caller
  ...
  make(trustDomain, path) {
    ...
    const parsed = this.parse(PREFIX + domain + tail);
    if (!parsed.ok) {
      log.debug('Leaving SpiffeId.make(). Invalid.');
      throw new Error('Cannot build a SPIFFE ID from trust domain "' + domain +
                      '" and path "' + path + '": ' + parsed.reason);
    }
    log.debug('Leaving SpiffeId.make(). id=' + parsed.id);
    return parsed.id;
  }

  // The trust domain as an identifier in its own right —
  ...
  trustDomainId(trustDomain) {
    ...
    log.debug("Leaving SpiffeId.trustDomainId().");
    return PREFIX +
  ...
  trustDomainOf(value) {
    ...
    log.debug("Leaving SpiffeId.trustDomainOf().");
  ...
  isMemberOf(value, trustDomain) {
    ...
    const parsed = this.parse(value);
    if (!parsed.ok) {
      log.debug("Leaving SpiffeId.isMemberOf().");
      return false;
    }
    log.debug("Leaving SpiffeId.isMemberOf().");
  ...
  // believed. `spiffe_registry.ts` refuses one; nothing else needs to.
  //
  ...
  isReservedPath(value) {
    ...
    const parsed = this.parse(value);
    if (!parsed.ok) {
      log.debug("Leaving SpiffeId.isReservedPath().");
      return false;
    }
    log.debug("Leaving SpiffeId.isReservedPath().");
  ...
  serverId(trustDomain) {
    ...
    log.debug("Leaving SpiffeId.serverId().");
    return this.make(trustDomain, '/spire/server');
  }

  // An attested agent's identifier. The shape is SPIRE's —
  // `/spire/agent/<attestor name>/<attestor-specific suffix>` — because an
  ...
  agentId(trustDomain, attestorName, suffix) {
    ...
    log.debug("Leaving SpiffeId.agentId().");
  ...
  isAgentId(value) {
    ...
    const parsed = this.parse(value);
    log.debug("Leaving SpiffeId.isAgentId().");
  ...
  isServerId(value) {
    ...
    const parsed = this.parse(value);
    log.debug("Leaving SpiffeId.isServerId().");
  ...
  toProto(value) {
    ...
    const parsed = this.parse(value);
    if (!parsed.ok) {
      log.debug("Leaving SpiffeId.toProto().");
      return null;
    }
    log.debug("Leaving SpiffeId.toProto().");
  ...
  fromProto(message) {
    ...
    if (!message) {
      log.debug('Leaving SpiffeId.fromProto().');
      return '';
    }
    ...
    // A caller that put the whole identifier in trust_domain is a real and
    // common mistake, and one this service should NAME rather than silently
    // repair: the result of repairing it is that their code works here and
    // against nothing else. So the scheme is stripped for the purpose of
    // building the identifier — otherwise the result is not parseable at all
    // and the error says nothing useful — but parse() then reports on what was
    // actually built.
    const bare = domain.slice(0, PREFIX.length).toLowerCase() === PREFIX
      ? domain.slice(PREFIX.length).split('/')[0]
      : domain;
    const path = String(message.path || '');
    const parsed = this.parse(PREFIX + bare +
