...
// in-process test can require it and `acme/acme.ts` can be the only file that
...
// What `AcmeJws` needs from the rest of the service: the modules this file
...
class AcmeJws {
  ...
  refusal(type, status, code, detail, extra?) {
    ...
    log.debug("Leaving AcmeJws.refusal().");
  ...
  zodSentence(error, what) {
    ...
    if (!issue) {
      log.debug("Leaving AcmeJws.zodSentence(). No issue.");
      return 'the ' + what + ' did not validate.';
    }
    const where = (issue.path || []).length ? issue.path.join('.') : what;
    log.debug("Leaving AcmeJws.zodSentence().");
  ...
  isJoseJson(contentType) {
    ...
    log.debug("Leaving AcmeJws.isJoseJson(). type=" + type);
    return type === MEDIA_TYPE;
  }

  // ---------------------------------------------------------------------------
  // STRICT BASE64URL (RFC 7515 section 2: no padding, no whitespace, the
  // URL-safe alphabet). Canonical as well as well-formed: the decoded bytes
  // must encode back to exactly what was sent, so two spellings of one value
  ...
  decodeB64url(text, allowEmpty) {
    ...
    if (!value.length) {
      log.debug("Leaving AcmeJws.decodeB64url(). Empty.");
      return allowEmpty ? Buffer.alloc(0) : null;
    }
    if (!B64URL.test(value) || value.length % 4 === 1) {
      log.debug("Leaving AcmeJws.decodeB64url(). Not the alphabet.");
      return null;
    }
    const bytes = Buffer.from(value, 'base64url');
    if (bytes.toString('base64url') !== value) {
      log.debug("Leaving AcmeJws.decodeB64url(). Not canonical.");
      return null;
    }
    log.debug("Leaving AcmeJws.decodeB64url().");
    return bytes;
  }
  ...
  b64u(bytes) {
    ...
    log.debug("Leaving AcmeJws.b64u().");
  ...
  readJsonObject(bytes, what) {
    ...
    try {
      ...
      log.debug("Caught in AcmeJws.readJsonObject(): " +
                ((e && e.message) || e));
      log.debug("Leaving AcmeJws.readJsonObject(). Not JSON.");
      return { ok: false, detail: 'the ' + what + ' is not JSON.' };
    }
    if (!value || typeof value !== 'object' || Array.isArray(value)) {
      log.debug("Leaving AcmeJws.readJsonObject(). Not an object.");
      return { ok: false, detail: 'the ' + what + ' is not a JSON object.' };
    }
    const walked = validation.checkDocument(value, what, { maxDepth: 8,
                                                           maxKeys: 1024 });
    if (!walked.ok) {
      log.debug("Leaving AcmeJws.readJsonObject(). The document was refused.");
      return { ok: false, detail: String(walked.detail || walked.code) };
    }
    log.debug("Leaving AcmeJws.readJsonObject().");
  ...
  parseFlattenedObject(value, what) {
    ...
    if (!value || typeof value !== 'object' || Array.isArray(value)) {
      log.debug("Leaving AcmeJws.parseFlattenedObject(). Not an object.");
      return this.refusal('malformed', 400, 'STS-ACME-0012',
                          'The ' + what + ' is not a flattened JWS JSON ' +
                          'object (RFC 7515 section 7.2.2).');
    }
    const walked = validation.checkDocument(value, what, { maxDepth: 4,
                                                           maxKeys: 16 });
    if (!walked.ok) {
      log.debug("Leaving AcmeJws.parseFlattenedObject(). Refused by the " +
                "walker.");
      return this.refusal('malformed', 400, 'STS-ACME-0012',
                          'The ' + what + ' was ' +
    ...
    if (!parsed.success) {
      log.debug("Leaving AcmeJws.parseFlattenedObject(). Refused by the " +
                "schema.");
      return this.refusal('malformed', 400, 'STS-ACME-0012',
                          'The ' + what + ' must carry exactly "protected", ' +
                          '"payload" and "signature" (RFC 8555 section 6.2 ' +
                          'allows no unprotected header): ' +
                          this.zodSentence(parsed.error, what));
    }
    const header = this.decodeB64url(parsed.data.protected, false);
    const payload = this.decodeB64url(parsed.data.payload, true);
    const signature = this.decodeB64url(parsed.data.signature, false);
    if (!header || !payload || !signature) {
      log.debug("Leaving AcmeJws.parseFlattenedObject(). Not strict " +
                "base64url.");
      return this.refusal('malformed', 400, 'STS-ACME-0013',
                          'A member of the ' +
                          what +
                          ' is not strict base64url: the URL-safe alphabet, ' +
                          'no padding and no whitespace (RFC 7515 section 2).');
    }
    log.debug("Leaving AcmeJws.parseFlattenedObject().");
  ...
  parseBody(text) {
    ...
    const read = this.readJsonObject(Buffer.from(String(text == null ? '' :
                                                        text),
                                                 'utf8'), 'request body');
    if (!read.ok) {
      log.debug("Leaving AcmeJws.parseBody(). Unreadable.");
      return this.refusal('malformed', 400, 'STS-ACME-0012',
                          'The request body is ' +
                          'not a flattened JWS JSON object: ' + read.detail);
    }
    log.debug("Leaving AcmeJws.parseBody().");
  ...
  parseProtectedHeader(parts) {
    ...
    const read = this.readJsonObject(parts.headerBytes, 'protected header');
    if (!read.ok) {
      log.debug("Leaving AcmeJws.parseProtectedHeader(). Unreadable.");
      return this.refusal('malformed', 400, 'STS-ACME-0014',
                          'The JWS protected ' +
    ...
    if (refused.length) {
      log.debug("Leaving AcmeJws.parseProtectedHeader(). A refused member.");
      return this.refusal('malformed', 400, 'STS-ACME-0014',
                          'The protected header carries "' + refused[0] +
                          '". An ACME request names its key with "jwk" or ' +
                          '"kid" and nothing else (RFC 8555 section 6.2), ' +
                          'and this server understands no critical extension.');
    }
    const parsed = HEADER.safeParse(read.value);
    if (!parsed.success) {
      log.debug("Leaving AcmeJws.parseProtectedHeader(). Refused by the " +
                "schema.");
      return this.refusal('malformed', 400, 'STS-ACME-0014',
                          'The JWS protected ' +
                          'header is not acceptable: ' +
                          this.zodSentence(parsed.error, 'protected header'));
    }
    log.debug("Leaving AcmeJws.parseProtectedHeader().");
  ...
  checkAlgorithm(alg) {
    ...
    if (ACCOUNT_ALGS.indexOf(String(alg)) < 0) {
      log.debug("Leaving AcmeJws.checkAlgorithm(). Unsupported.");
      return this.refusal('badSignatureAlgorithm', 400, 'STS-ACME-0015',
                          'The JWS ' +
                          'is signed with "' + String(alg).slice(0, 32) +
                          '", which ' +
                          'this server does not accept for an account key. ' +
                          'It accepts ' + ACCOUNT_ALGS.join(', ') + '.',
                          { algorithms: ACCOUNT_ALGS.slice() });
    }
    log.debug("Leaving AcmeJws.checkAlgorithm().");
    return { ok: true };
  }

  // ---------------------------------------------------------------------------
  // AN ACCOUNT KEY (a `jwk` header). Checked for shape, for private members,
  // for agreement with `alg`, and loaded — node refuses an EC point that is not
  // on its curve — before its thumbprint is taken. What is kept is the PUBLIC
  ...
  checkAccountKey(jwk, alg) {
    ...
    if (!jwk || typeof jwk !== 'object') {
      log.debug("Leaving AcmeJws.checkAccountKey(). No key.");
      return this.refusal('malformed', 400, 'STS-ACME-0021',
                          'The "jwk" header is ' +
                          'not a JSON Web Key.');
    }
    ...
    if (privateMember) {
      log.debug("Leaving AcmeJws.checkAccountKey(). A private member.");
      return this.refusal('badPublicKey', 400, 'STS-ACME-0021',
                          'The "jwk" header ' +
                          'carries the private member "' + privateMember +
                          '". An account key is sent as its PUBLIC half only.');
    }
    let publicJwk = null;
    const kty = String(jwk.kty || '');
    const stringOk = function (value) {
      return typeof value === 'string' && value.length > 0 &&
             value.length <= 4096 && !!self.decodeB64url(value, false);
    };
    ...
    if (!publicJwk) {
      log.debug("Leaving AcmeJws.checkAccountKey(). The key does not fit the " +
                "alg.");
      return this.refusal('badPublicKey', 400, 'STS-ACME-0021',
                          'The "jwk" header ' +
                          'is not a ' + String(alg).slice(0, 32) +
                          ' public key: an RS or PS algorithm takes an RSA ' +
                          'key, ES256/ES384/ES512 an EC key on ' +
                          'P-256/P-384/P-521, and EdDSA an Ed25519 key.');
    }
    let key = null;
    try {
    ...
    if (!key) {
      log.debug("Leaving AcmeJws.checkAccountKey(). The key does not load.");
      return this.refusal('badPublicKey', 400, 'STS-ACME-0021',
                          'The "jwk" header ' +
    ...
    if (kty === 'RSA') {
      ...
      if (bits < MIN_RSA_BITS) {
        log.debug("Leaving AcmeJws.checkAccountKey(). RSA too small.");
        return this.refusal('badPublicKey', 400, 'STS-ACME-0021',
                            'The RSA account key is ' + bits +
                            ' bits; this server requires at ' +
                            'least ' + MIN_RSA_BITS + '.');
      }
    }
    let thumbprint = '';
    ...
    if (!thumbprint) {
      log.debug("Leaving AcmeJws.checkAccountKey(). No thumbprint.");
      return this.refusal('badPublicKey', 400, 'STS-ACME-0021', 'No RFC 7638 ' +
                          'thumbprint can be taken of the "jwk" header.');
    }
    log.debug("Leaving AcmeJws.checkAccountKey().");
  ...
  algorithmFitsKey(alg, jwk) {
    ...
    log.debug("Leaving AcmeJws.algorithmFitsKey(). fits=" + fits);
    return fits;
  }
  ...
  verifyFlattened(parts, key, alg) {
    ...
    try {
      ...
      log.debug("Caught in AcmeJws.verifyFlattened(): " +
                ((e && e.message) || e));
      log.debug("Leaving AcmeJws.verifyFlattened(). It does not verify.");
      return this.refusal('malformed', 400, 'STS-ACME-0024',
                          'The JWS signature ' +
                          'does not verify with the account key.');
    }
    log.debug("Leaving AcmeJws.verifyFlattened().");
    return { ok: true };
  }
  ...
  readPayload(parts) {
    ...
    if (!parts.payloadB64.length) {
      log.debug("Leaving AcmeJws.readPayload(). POST-as-GET.");
      return { ok: true, value: null };
    }
    const read = this.readJsonObject(parts.payloadBytes, 'payload');
    if (!read.ok) {
      log.debug("Leaving AcmeJws.readPayload(). Unreadable.");
      return this.refusal('malformed', 400, 'STS-ACME-0025',
                          'The JWS payload is unreadable: ' + read.detail);
    }
    log.debug("Leaving AcmeJws.readPayload().");
  ...
  checkPayload(value, schema, what) {
    ...
    if (!parsed.success) {
      log.debug("Leaving AcmeJws.checkPayload(). Refused.");
      return this.refusal('malformed', 400, 'STS-ACME-0025',
                          'The ' + what + ' ' +
                          'payload is not acceptable: ' +
                          this.zodSentence(parsed.error, what));
    }
    log.debug("Leaving AcmeJws.checkPayload().");
  ...
  nonceSecret() {
    ...
    log.debug("Leaving AcmeJws.nonceSecret().");
    return held;
  }
  ...
  mintNonce(realmId, lifetimeS) {
    ...
    const nonce = this.b64u(Buffer.concat([head, random,
                                           this.nonceMac(realmId, expiresS,
                                               random)]));
    log.debug("Leaving AcmeJws.mintNonce().");
    return nonce;
  }
  ...
  checkNonce(nonce, realmId) {
    ...
    if (!bytes || bytes.length !== 37 || bytes.readUInt8(0) !== NONCE_VERSION) {
      log.debug("Leaving AcmeJws.checkNonce(). Malformed.");
    ...
    if (expiresS <= Math.floor(Date.now() / 1000)) {
      log.debug("Leaving AcmeJws.checkNonce(). Expired.");
      return { ok: false, reason: 'expired', expiresS: expiresS };
    }
    log.debug("Leaving AcmeJws.checkNonce().");
  ...
  parseEab(eab, expectedUrl, accountThumbprint) {
    ...
    const parts = this.parseFlattenedObject(eab, 'externalAccountBinding');
    if (!parts.ok) {
      log.debug("Leaving AcmeJws.parseEab(). Not a flattened JWS.");
      return this.refusal('malformed', 400, 'STS-ACME-0032', parts.detail);
    }
    const read = this.readJsonObject(parts.headerBytes,
                                     'externalAccountBinding protected header');
    if (!read.ok) {
      log.debug("Leaving AcmeJws.parseEab(). The header is unreadable.");
      return this.refusal('malformed', 400, 'STS-ACME-0032', 'The ' +
                          'externalAccountBinding header is unreadable: ' +
                          read.detail);
    }
    const header = EAB_HEADER.safeParse(read.value);
    if (!header.success) {
      log.debug("Leaving AcmeJws.parseEab(). The header is not acceptable.");
      return this.refusal('malformed', 400, 'STS-ACME-0032', 'The ' +
                          'externalAccountBinding protected header must be ' +
                          'exactly "alg", "kid" and "url" — no nonce (RFC ' +
                          '8555 section 7.3.4): ' +
                          this.zodSentence(header.error, 'EAB header'));
    }
    if (EAB_ALGS.indexOf(header.data.alg) < 0) {
      log.debug("Leaving AcmeJws.parseEab(). Not a MAC.");
      return this.refusal('badSignatureAlgorithm', 400, 'STS-ACME-0032', 'An ' +
                          'external account binding is MACed with ' +
                          EAB_ALGS.join(', ') + ', and this one names "' +
                          header.data.alg + '".',
                          { algorithms: EAB_ALGS.slice() });
    }
    if (header.data.url !== expectedUrl) {
      log.debug("Leaving AcmeJws.parseEab(). The url differs.");
      return this.refusal('malformed', 400, 'STS-ACME-0032', 'The ' +
                          'externalAccountBinding "url" must be the ' +
                          'newAccount URL ' +
                          'the outer JWS was sent to.');
    }
    const keyRead = this.readJsonObject(parts.payloadBytes,
    ...
    if (keyRead.ok) {
      ...
      const key = this.checkAccountKey(keyRead.value, alg);
      bound = key.ok ? key.thumbprint : '';
    }
    if (!bound || bound !== accountThumbprint) {
      log.debug("Leaving AcmeJws.parseEab(). It binds a different key.");
      return this.refusal('malformed', 400, 'STS-ACME-0032', 'The ' +
                          'externalAccountBinding payload must be the ' +
                          'account\'s own public key, the same one the outer ' +
                          'JWS carries in "jwk".');
    }
    log.debug("Leaving AcmeJws.parseEab().");
  ...
  verifyEabMac(parsed, hmacKey) {
    ...
    try {
      ...
      log.debug("Caught in AcmeJws.verifyEabMac(): " + ((e && e.message) || e));
      verified = false;
    }
    log.debug("Leaving AcmeJws.verifyEabMac(). verified=" + verified);
    return verified;
  }
  ...
  checkContacts(list) {
    ...
    for (let i = 0; i < values.length; i++) {
      ...
      if (!/^mailto:/i.test(one)) {
        log.debug("Leaving AcmeJws.checkContacts(). Unsupported scheme.");
        return this.refusal('unsupportedContact', 400, 'STS-ACME-0037',
                            'Contact "' +
                            one.slice(0, 80) +
                            '" is not a mailto: URI, the only ' +
      ...
      if (!/^[A-Za-z0-9.!#$%&'*+/=^_`{|}~-]{1,64}@[A-Za-z0-9]([A-Za-z0-9-]{0,61}[A-Za-z0-9])?(\.[A-Za-z0-9]([A-Za-z0-9-]{0,61}[A-Za-z0-9])?)+$/
        ...
        log.debug("Leaving AcmeJws.checkContacts(). Invalid address.");
        return this.refusal('invalidContact', 400, 'STS-ACME-0037',
                            'Contact "' +
                            one.slice(0, 80) +
                            '" is not one mailto: address with ' +
                            'no header fields.');
      }
    }
    log.debug("Leaving AcmeJws.checkContacts().");
  ...
  serialContentBytes(serialHex) {
    ...
    log.debug("Leaving AcmeJws.serialContentBytes().");
    return bytes;
  }
  ...
  certIdOf(akiBytes, serialHex) {
    ...
    log.debug("Leaving AcmeJws.certIdOf().");
    return this.b64u(akiBytes) + '.' +
  ...
  parseCertId(text) {
    ...
    if (pieces.length !== 2 || value.length > 512) {
      log.debug("Leaving AcmeJws.parseCertId(). Not two parts.");
      return null;
    }
    const aki = this.decodeB64url(pieces[0], false);
    const serial = this.decodeB64url(pieces[1], false);
    if (!aki || !serial) {
      log.debug("Leaving AcmeJws.parseCertId(). Not base64url.");
      return null;
    }
    log.debug("Leaving AcmeJws.parseCertId().");
  ...
  // ---------------------------------------------------------------------------
  // WHAT A CERTIFICATE SAYS, read with pkijs: the serial, the AKI
  // keyIdentifier, the SubjectPublicKeyInfo DER and the validity. For
  // revokeCert (whose
  ...
  asArrayBuffer(bytes) {
    ...
    log.debug("Leaving AcmeJws.asArrayBuffer().");
  ...
  certificateFacts(der) {
    ...
    if (!bytes.length) {
      log.debug("Leaving AcmeJws.certificateFacts(). Empty.");
      return null;
    }
    try {
      const parsed = asn1js.fromBER(this.asArrayBuffer(bytes));
      ...
      const spki = Buffer.from(cert.subjectPublicKeyInfo.toSchema()
                               .toBER(false));
      log.debug("Leaving AcmeJws.certificateFacts().");
      return {
        der: bytes,
      ...
      log.debug("Caught in AcmeJws.certificateFacts(): " +
                ((e && e.message) || e));
      log.debug("Leaving AcmeJws.certificateFacts(). Unreadable.");
      return null;
    }
  }
  ...
  pemToDer(pem) {
    ...
    const match = /-----BEGIN CERTIFICATE-----([\s\S]*?)-----END CERTIFICATE-----/
      .exec(String(pem || ''));
    log.debug("Leaving AcmeJws.pemToDer().");
  ...
  normalIdentifier(type, value) {
    ...
    log.debug("Leaving AcmeJws.normalIdentifier().");
    return out;
  }
}
...
//
// **SINGLE USE is the other half and is a store** (`acme_store.ts`): the
