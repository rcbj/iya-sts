...
//   * **The OpenAPI document is built from that table** (admin_api_spec.ts), so
...
import adminScope = require('../admin-ui/admin_scope');
// ---------------------------------------------------------------------------
...
import debuggerAdmin = require('../debugger/debugger_admin');
...
// — and `admin_api_spec.ts` publishes it verbatim in the OpenAPI document.
...
// published document changes: `admin_api_spec.ts` still emits `requestBody`
...
interface AdminApiDeps {
  ...
  loadApiExplorer(): typeof import('../admin-ui/api_explorer');
  loadGnapConsole(): typeof import('../gnap/gnap_console');
...
class AdminApi {
  ...
  structureOnly(node) {
    const { log } = this.deps;
    const self = this;
    log.debug("Entering AdminApi.structureOnly().");
    if (Array.isArray(node)) {
      log.debug("Leaving AdminApi.structureOnly().");
      return node.map(this.structureOnly.bind(this));
    }
  ...
  validatorKeyOf(route, action) {
    ...
    log.debug("Leaving AdminApi.validatorKeyOf().");
    return route + '\u0000' + (action || '');
  }
  ...
  compileRequestSchemas() {
    ...
    log.debug("Leaving AdminApi.compileRequestSchemas(). " + built +
              " validator(s).");
    return built;
  }
  ...
  errorsFromAjv(errors) {
    ...
    log.debug("Leaving AdminApi.errorsFromAjv(). " + out.length +
              " message(s).");
  ...
  //
  // **`action` IS REMOVED BEFORE VALIDATING, and that is not a loophole.**
  // Every action resource here is `/<resource>/:action`, so the action is a
  // PATH segment and the schemas describe the body WITHOUT it — they carry
  // `additionalProperties: false`, so leaving it in would refuse every request
  // on this API. A caller that also puts `action` in the body is ignored
  ...
  checkRequestBody(req) {
    ...
    if (req.__adminApiHandlerOwnsBody) {
      ...
      // the route table.
      log.debug("Leaving AdminApi.checkRequestBody(). The handler owns this " +
                "body.");
      return { ok: true };
    }
    const validate = validators.get(this.validatorKeyOf(route, action)) ||
                     validators.get(this.validatorKeyOf(route, ''));
    if (!validate) {
      // No schema for this operation — a GET, an unknown action the handler is
      // about to refuse by name, or a row that carries none. Not this
      // function's business to invent one.
      log.debug("Leaving AdminApi.checkRequestBody(). No schema for this " +
                "operation.");
      return { ok: true };
    }
    ...
    if (validate(subject)) {
      log.debug("Leaving AdminApi.checkRequestBody(). Accepted.");
      return { ok: true };
    }
    log.debug("Leaving AdminApi.checkRequestBody(). Refused with " +
              (validate.errors || []).length + " error(s).");
    return { ok: false, errors: this.errorsFromAjv(validate.errors) };
  }

  // Every reply here is JSON, is never cached, and is pretty-printed. The last
  ...
  // over whatever the body carried so that a body copied from the console's
  // form — which does carry `action` — cannot mean something other than the URL
  // it was sent to.
  // ---------------------------------------------------------------------------
  // A CREATE CLAIMS ITS NAME ACROSS NODES FIRST (2026-09-14, #46 section 3),
  // for `POST /admin-api/users/create` and `/admin-api/groups/create` —
  ...
  claimForCreate(what) {
    ...
    try {
      ...
      log.debug("Caught in AdminApi.claimForCreate(): " +
                ((e && e.message) || e));
    }
    if (!directory || typeof directory.claimCreate !== 'function') {
      log.debug("Leaving AdminApi.claimForCreate(). No directory in this " +
                "process.");
      return Promise.resolve({ ok: true, settle: function () {} });
    }
    log.debug("Leaving AdminApi.claimForCreate().");
    return directory.claimCreate(what);
  }

  // ---------------------------------------------------------------------------
  // RUN AN ACTION, CLAIMING ITS NAME FIRST WHEN THERE IS ONE TO CLAIM. `what`
  // is null for an action that creates nothing, and then — and wherever a
  // create cannot race (`createClaims.active()` false, which is every
  // single-process service) — `run` is called SYNCHRONOUSLY, exactly as the
  // handler did before. Otherwise the claim is awaited, a refusal answered in
  ...
  runClaimed(res, what, run) {
    ...
    const self = this;
    log.debug("Entering AdminApi.runClaimed().");
    const idle = { ok: true, settle: function () {} };
    if (!what || !createClaims.active()) {
      log.debug("Leaving AdminApi.runClaimed(). Nothing to claim.");
      return run(idle);
    }
    log.debug("Leaving AdminApi.runClaimed(). Claiming first.");
    return this.claimForCreate(what).then(function (held) {
      if (!held.ok) {
        errorCodes.mark(res, held.code);
        self.sendJson(res, held.reason === 'store' ? 503 : 409,
                      { ok: false,
      ...
      try {
        ...
        self.sendJson(res, 500, {
          ok: false,
          errors: ['The create failed: ' + ((e && e.message) || e)] });
      }
    });
  }
  ...
  withAction(req, body) {
    ...
    log.debug("Leaving AdminApi.withAction().");
  ...
  // and has to be, because a drill-down answers with several lists at once —
  // five on /users, two on /groups. One `page` would move all of them together,
  // so each gets a parameter named after itself and `per` above stays shared.
  //
  // Every one is clamped the way `page` is, and every one is answered: the
  // reply carries a `<name>Paging` object beside the array, with the same
  // member names the flat lists put at the top level. A caller walks these
  // exactly as it walks /tokens, one list at a time. ONE NAME PER LIST: the
  // parameter is the reply array's own name with `Page` on the end, and the
  ...
  detailPagingParameters(lists) {
    ...
    log.debug("Leaving AdminApi.detailPagingParameters().");
    ...
    const rows = [
      { action: 'add', operationId: family.ids.add,
        ...
        description: 'Every ' + family.carrier + ' of that kind issued from ' +
                     ...
                     'changes.\n\nADDITIVE ONLY. What the protocol puts in ' +
                     'is never displaced — an ID Token\'s `sub`, a SAML 2.0 ' +
                     'assertion\'s `name`, a WS-Federation assertion\'s ' +
                     'whole identity claim list.' +
                     (family.reserved
                       ? ' A name this service sets itself is REFUSED rather ' +
                         'than allowed to win, because every one of those is ' +
                         'load-bearing — a settable `exp` would produce ' +
                         'tokens that fail to verify with nothing pointing ' +
                         'back at the call that caused it. `GET ' +
                       ...
                       : ' There is NO reserved list here: those names are ' +
                         ...
                         'called `exp` collides with ' +
                         'nothing. An entry with no ' +
                         'name, and two entries of one name, are still ' +
                         'refused.'),
        ...
        requestBody: {
          ...
          properties: {
            ...
            namespace: { type: 'string',
                         description: 'The SAML 1.1 set only. Defaults to ' +
                                      'the WS-Federation claim ' +
                                      'namespace, which is ' +
                                      'what every relying party already ' +
                                      'reads.' }
          },
          required: ['set', 'name'],
      ...
      { action: 'attributes', operationId: family.ids.attributes,
        ...
        description: 'The array REPLACES the selection for that set. An ' +
                     ...
                     'call rather than being skipped: the catalogue is ' +
                     'fixed, so an unknown name is either a hand-written ' +
                     'request that deserves an answer or a rename that left ' +
                     'a caller behind. `attributeCatalogue` in the GET ' +
                     'beside this is the list — and it is ' +
                     'ONE catalogue for all four sets, so ' +
                     ...
                     'before it is discovered on the wire. An ID Token ' +
                     'always carries name, given_name, family_name, ' +
      ...
      { action: 'attributes-all', operationId: family.ids.all,
        ...
        description: 'Every attribute type in the catalogue, which is a ' +
                     ...
                     'the whole list of names that has to be updated ' +
      ...
      { action: 'attributes-clear', operationId: family.ids.none,
        ...
        description: 'The TYPED ' + noun +
                     's on that set are untouched — this ' +
                     'is the other half. Clearing both takes this and ' +
                     '`clear`.\n\nNothing is deleted from the directory: ' +
                     'what was written onto an entry stays there, because an ' +
                     'operator may have set it and nothing here has the ' +
                     'standing to remove it.',
        requestBodyRequired: true,
        requestBody: {
          type: 'object',
          properties: { set: setField },
  ...
  // ---------------------------------------------------------------------------
  // THE TABLE. Express registration and the OpenAPI document are both built
  // from it, which is what makes them incapable of disagreeing.
  //
  // A row is either a plain operation (`path` + `handler`) or an action
  // resource (`route` with `:action` in it + `handler` + `actions`), where each
  // action is one documented operation at its own concrete URL.
  // ---------------------------------------------------------------------------
  // ---------------------------------------------------------------------------
  ...
  buildProtocolSettingsOperations(): any[] {
    const { log, admin } = this.deps;
    const self = this;
    log.debug("Entering AdminApi.buildProtocolSettingsOperations().");
    const PROTOCOL_SETTINGS_OPERATIONS: any[] = [
      { path: '/oauth2', console: '/admin/oauth2', tag: 'OAuth 2.0 / OIDC',
        ...
        description: 'The `oauth2.*` settings: the issuer identifier, ' +
                     ...
                     'draft-ietf-oauth-v2-1-16, which implies RFC 9700 ' +
                     'mode), the registered redirect URIs and the ' +
                     'loopback port wildcard, Front-Channel Logout, the ' +
                     'refresh token idle timeout, whether a sign-out revokes ' +
                     'refresh tokens, the client assertion clock skew, the ' +
                     ...
                     'does not, which is how one process answers ' +
                     'permissively at /oauth2/authorize and enforces the ' +
      ...
      { path: '/oid4vci-settings', console: '/admin/oid4vci', tag: 'OpenID4VCI',
        ...
        description: 'The nine `oid4vci.*` settings: the wallet an offer ' +
                     ...
                     'encrypted, and the two that decide whether the SD-JWT ' +
                     'VC and `ldp_vc` issuers name themselves by `did:web` ' +
                     'or by URL.\n\nThose two are restart-only and they ' +
                     'change what a VERIFIER has to resolve — a key fetched ' +
                     'from a DID document rather than from JWKS. What a ' +
                     'credential CONTAINS is `GET /credential-claims`.' },
      ...
      // `/admin/webauthn` arrived on the console and owe an operation in the
      // same change. Both replies carry the page's `status` block — the
      // MECHANISM, read from the module that performs it — beside the settings,
      // which is why they are worth fetching rather than reading `GET /config`.
      { path: '/totp', console: '/admin/totp', tag: 'TOTP MFA',
        ...
        description: 'The eight `totp.*` settings — the HMAC digest, the ' +
                     'digits, the seconds in a step, the steps of clock skew ' +
                     'forgiven, the shared secret length, the label an app ' +
                     'shows, whether new enrolments are offered at all, and ' +
                     'how long an unconfirmed one lives — with the RFC 6238 ' +
                     'algorithm table in `status`, read from ' +
                     '`common/totp.ts` rather than written down ' +
                     'here.\n\n**CHANGING THE DIGEST, THE DIGITS OR THE ' +
                     'PERIOD AFFECTS NEW ENROLMENTS ONLY.** An existing ' +
                     'secret is verified with the parameters it was enrolled ' +
                     'under — the ones the QR code told the app — because ' +
                     'this service cannot change them retrospectively. ' +
                     '`totp.window` is the exception and applies to ' +
                     'everybody.\n\n**Codes are verified FOR REAL in both ' +
                     'modes**, which almost nothing else in this service is. ' +
                     'Who holds an enrolment is `GET /users` (or `GET ' +
                     '/mfa`), and clearing one is `POST ' +
                     '/users/clear-totp`.\n\nThese eight were on `GET ' +
                     '/admin-api/mfa` until ' +
                     '2026-09-10, when the console page ' +
                     'that drew them split into a mechanism page and a ' +
                     'roster.' },
      ...
      { path: '/backup-codes', console: '/admin/backup-codes',
        ...
        description: 'The four `backupCodes.*` settings — whether a set is ' +
                     'issued at all, how many codes are in one, how long ' +
                     'each is, and how it is broken up for reading — with ' +
                     'the mechanism itself in `status`, read from ' +
                     '`common/backup_codes.ts` rather than written down ' +
                     'here.\n\n**THIS IS THE ONLY MECHANISM IN THIS SERVICE ' +
                     'THAT NO SPECIFICATION DEFINES.** There is no RFC for a ' +
      ...
      { path: '/webauthn', console: '/admin/webauthn', tag: 'WebAuthn',
        ...
        description: 'The thirteen `webauthn.*` settings, in three kinds. ' +
                     ...
                     'the browser in the `PublicKeyCredential` options. ' +
                     '**CTAP2**: the authenticator attachment, whether the ' +
                     'credential is discoverable (a resident key), and ' +
                     'whether `credProps` is asked for. **POLICY**: whether ' +
                     'a key may be a primary credential, whether it may be a ' +
                     'second factor, and how many one person may hold — ' +
                     'which are not WebAuthn at all but what THIS service ' +
                     'does with a key.\n\n**NOT ONE OF THESE EXISTED UNTIL ' +
                     '2026-09-10.** Every ceremony parameter was a literal ' +
                     'in a string in `authn/authn.ts`, and this service said ' +
                     'there was nothing an operator could usefully turn — ' +
                     'true of the cryptography and false of the ' +
                     'ceremony.\n\n**ONE IS ENFORCED AND THE REST ARE ' +
                     'REQUESTS.** `webauthn.userVerification` is sent to the ' +
                     'browser AND checked against the UV flag when the ' +
                     'ceremony returns, because that flag is inside the ' +
                     'bytes the authenticator signed. Nothing signed says ' +
                     'what the browser was asked about attestation, the ' +
      ...
      { path: '/kerberos', console: '/admin/kerberos', tag: 'Kerberos',
        ...
        description: 'The nineteen `krb5.*` settings: the realm, the two raw ' +
                     'ports, the clock skew and the deliberate clock OFFSET, ' +
                     'the one password every user account shares, the names ' +
                     'that stay unknown, the long-term keys behind krbtgt ' +
                     'and the inter-realm trust, `s2kparams`, and the two ' +
                     'that decide whether a ticket presented at ' +
                     '/authn/spnego may start a browser session.\n\nMOST OF ' +
                     'THEM ARE RESTART-ONLY, and one fact is why: the ' +
                     'principal database — every long-term key in it — is ' +
                     'built from these when the process starts, so a realm ' +
                     'or a password changed at runtime would leave every ' +
                     'existing ticket undecryptable by the service that ' +
                     'issued it.\n\nTWO OF THEM EXIST TO MAKE FAILURES ' +
                     'REACHABLE. `krb5.unknownUsers` names the only ' +
                     'principals that can produce ' +
      ...
      { path: '/ldap', console: '/admin/ldap', tag: 'LDAP',
        ...
        description: 'The six `ldap.*` settings: the two raw ports, the base ' +
                     'DN, whether a name seen for the first time gets an ' +
                     'entry, and the two ceilings that keep a mock from ' +
                     'being filled up.\n\nNOTHING HERE REFUSES A BIND, and ' +
                     'no setting is missing: any DN with any password, and ' +
                     'anonymous, are accepted on 389 and 636 alike. There is ' +
                     'no such behaviour to turn on.\n\nThe base DN is the ' +
                     'DEFAULT realm\'s directory and every other realm is a ' +
                     'subtree beneath it, because a socket has no path to ' +
                     'put a realm segment in and a DN is the ' +
                     'one name a client can carry. What is IN the ' +
                     'directory is `GET /users` and `GET /groups`.' },
      // The newest of these, 2026-08-27, and the only one whose reply carries a
      // `status` member: a persistence setting that is SET and a persistence
      // store that is WORKING are two different facts, and the gap between them
      // is the whole failure mode this feature has. See admin.js's
      // persistenceStatusBlock().
      { path: '/persistence', console: '/admin/persistence', tag: 'Persistence',
        operationId: 'getPersistenceSettings',
        summary: 'What survives a restart, where it is written, and whether ' +
                 'that is working',
        description: 'The `persistence.*` settings AND — unlike every other ' +
                     'operation in this group — a `status` member saying ' +
                     'what the store is actually doing: which mode is in ' +
                     'force, whether it FELL BACK to memory because it could ' +
                     'not be opened, where it writes, how many entries and ' +
                     'realms it holds, when it last wrote, and the error if ' +
                     'the last write failed.\n\nTHREE THINGS PERSIST when a ' +
                     'store is on: the embedded LDAP directory (which is ' +
                     'also the applications registry, the federation ' +
                     'register and the SPIFFE registry — they are directory ' +
                     'entries and nothing else), the trust realm registry, ' +
                     'and the runtime appconfig overrides that `POST ' +
                     '/admin-api/config/set` writes.\n\nAND, IN PRODUCT MODE ' +
                     'ON A POSTGRES STORE SINCE 2026-09-06, WHAT THIS ' +
                     'SERVICE MINTS: sessions, access tokens, ID Tokens, ' +
                     'refresh tokens, authorization codes, pre-authorized ' +
                     'codes, SAML artifacts, Kerberos principals and ' +
                     'tickets, the replay caches, the statistics and the ' +
                     'audit log — each row encrypted under the same ' +
                     'key-encryption key that protects the signing keys, so ' +
                     'a dump of the table is not a set of usable ' +
                     'credentials. `status.minted` reports it.\n\nIN ' +
                     'DEVELOPMENT MODE NONE OF THAT PERSISTS, and the reason ' +
                     'is the one the rule always rested on: the signing key ' +
                     'is regenerated on every start there, so a token ' +
                     'restored from a disk would verify against nothing. ' +
                     'Product mode keeps its keys — which is why it requires ' +
                     'a store — and that single fact is what makes restoring ' +
                     'the rest of it honest. The ldif store holds no minted ' +
                     'state in either mode: it writes whole files per flush, ' +
                     'which is right for a directory somebody types into and ' +
                     'wrong for a session table that changes on every ' +
                     'request.\n\nPROCESSES AGAINST ONE POSTGRES STORE ' +
                     'COORDINATE SINCE 2026-09-06, and this paragraph said ' +
                     'the opposite before it. Every change is written to a ' +
                     'monotonic log inside the transaction that made it, and ' +
                     'each process applies what the others committed — the ' +
                     'directory, the realms, the settings and the minted ' +
                     'rows alike. A LISTEN/NOTIFY nudge only makes that ' +
                     'prompt: the LOG is the contract, so a missed ' +
                     'notification costs latency and never a change. ' +
                     '`status.coordinates` and `status.replication` report ' +
                     'it; `persistence.coordinate` turns it off.\n\nTHE ' +
                     'DATABASE PASSWORD NEED NOT BE IN THE CONNECTION STRING ' +
                     'SINCE 2026-09-12. ' +
                     '`persistence.databasePasswordProvider` reads it from ' +
                     'the five places the key-encryption key comes from — a ' +
                     'mounted file, AWS Secrets Manager, Google Secret ' +
                     'Manager, Azure Key Vault, HashiCorp Vault — and by ' +
                     'default out of the SAME file or secret, told apart by ' +
                     'a field. `status.database.passwordFrom` says WHERE it ' +
                     'came from and never what it is, which is the rule this ' +
                     'whole reply follows about the connection string: the ' +
                     'host, port, database and user are parsed out of it and ' +
                     'the string itself is never returned.\n\nIT SHARES ' +
                     'STATE AND NOT SOCKETS. The KDC, both LDAP listeners, ' +
                     'the main port and SPIFFE\'s four are bound per ' +
                     'process. And the replay caches and DPoP jti sets ' +
                     'CONVERGE rather than synchronise: between a write in ' +
                     'one process and its arrival in another there is a ' +
                     'window the size of persistence.pollInterval in which a ' +
                     'proof one process refused is accepted by ' +
                     'another.\n\nFIVE OF THE SIX SETTINGS ARE RESTART-ONLY, ' +
                     'because the store is opened and read before the HTTP ' +
                     'listener binds. `persistence.databaseUrl` is never ' +
                     'echoed back in `status` — it carries a password, so ' +
                     'the host, port, database and user are parsed out of it ' +
                     'and reported instead.' },
      // THE CLUSTER (2026-09-14, #46). Like `/persistence`, a `status` member
      // beside the settings, because `cluster.mode` SET and a cluster WORKING
      // are two facts. See admin.js's clusterStatusBlock().
      { path: '/cluster', console: '/admin/cluster', tag: 'Cluster',
        ...
        description: 'The five `cluster.*` settings AND a `status` member: ' +
                     ...
                     'came from (never the value); and the read barrier\'s ' +
                     'counters.\n\nIN ACTIVE-PASSIVE MODE ONE NODE SERVES ' +
                     'and every other node waits before it restores or binds ' +
                     'anything, so a standby never answers this operation — ' +
                     ...
                     'CLUSTERED WRITE IS FENCED by the node\'s membership ' +
                     '(and in active-passive mode by the service lease\'s ' +
                     'token), and a node that has lost either exits. A ' +
                     'node\'s settings fingerprint is never ' +
                     'returned — only whether it agrees ' +
      ...
      { path: '/tls', console: '/admin/tls', tag: 'TLS',
        ...
        description: 'The `tls.*` settings: the hostnames and IP addresses ' +
                     'that go into the self-signed certificate this service ' +
                     'mints on every start, and what it makes of a ' +
                     'CLIENT\'s.\n\nTHE CERTIFICATE ONES ARE RESTART-ONLY: ' +
                     'it is minted before anything is listening. One ' +
                     'certificate serves LDAPS 636 and — when `global.https` ' +
                     'is on — the main port, so a caller trusts this service ' +
                     'once rather than twice.\n\nTHERE ARE NO PORTS HERE ' +
                     'SINCE 2026-09-16, when `tls.port` (8443) and ' +
                     '`tls.mutualPort` (9443) were removed with the two ' +
                     'listeners they named: the main port already asks every ' +
                     'connection for a client certificate and requires none ' +
                     'of any of them.\n\nWhether the MAIN port is HTTPS is ' +
                     '`global.https`, which is on `GET /config` with the ' +
                     'rest of the process\'s own settings: it is a fact ' +
                     'about the process rather than about this certificate, ' +
                     'and it defaults to whatever `oauth2.rfc9700` is.' }
    ].map(function (row) {
      return { method: 'GET', path: BASE + row.path, tag: row.tag,
               ...
               description: row.description +
                 ...
                 'gap: every form on the console page this mirrors posts ' +
                 '`set-many` to /admin/config, so ' +
                 '`POST /admin-api/config/set-many` is ' +
                 'already the operation for it. One store, one action, two ' +
                 'doors.',
    ...
    return PROTOCOL_SETTINGS_OPERATIONS;
  }
  ...
  // REFUSES AGAINST.
  //
  // `/token-lifetimes/set` and `/saml-assertions/set` exist to give a caller
  // one refusal the wide `/config/set-many` cannot: a key outside their own
  // list is refused BY NAME rather than ignored. That makes the list
  // load-bearing in the document — a caller reads it to know what may be sent —
  // and it was written out here BY HAND beside a list held in admin.js. Both
  // had drifted: this document named four token-lifetime settings against six,
  // and THREE assertion settings against sixteen. So it is derived, the way
  // every operation in this file is derived from the table that registers it.
  //
  ...
  // descriptions above are: a hand-written list of what an operation accepts is
  // a second definition of a table, and it goes wrong silently in exactly the
  ...
  familyScopeNote() {
    ...
    if (!scoped.length) {
      log.debug("Leaving AdminApi.familyScopeNote(). Nothing is " +
                "family-scoped.");
      return '';
    }
    ...
    log.debug("Leaving AdminApi.familyScopeNote(). " + scoped.length +
              " attribute(s).");
    return '**' + (scoped.length === 1 ? 'One attribute is' : scoped.length +
           ...
           'entry no request could ever name they would read as a policy ' +
           'that was in force. Declare the family first (`protocols` on the ' +
           'create, or add to `appAllowedProtocol`). ' +
           'Clearing one is never refused, ' +
    ...
    const ROUTES: any[] = [
      { method: 'GET', path: BASE, tag: 'Service',
        ...
        description: 'The index. Every operation with the console control it ' +
                     'mirrors, and where the OpenAPI document and the ' +
                     'explorer are. Nothing here changes anything.',
        mirrors: 'GET /admin',
        ...
        handler: function (req, res) {
          ...
          self.sendJson(res, 200, {
            ...
            // build it just made" wants the build number on its own, and a
            // report saying which commit an instance is on wants the commit —
            // splitting them here is the difference between a client reading a
            // field and a client writing a regular expression over `version`.
            //
            // `stamped` is the one that is easy to leave out and worth most:
            // false means this process computed its own number at startup
            // because nothing stamped an artifact, so the build number is the
            // moment it STARTED and comparing it with another instance's says
            // nothing.
      ...
      { method: 'GET', path: BASE + '/openapi.json', tag: 'Service',
        ...
        description: 'Built from the same table that registers the routes, ' +
                     'so it describes what is actually there. ' +
                     '`servers[0].url` is this service as the request ' +
                     'reached it, so a document fetched through a proxy or a ' +
                     'published port names the address the caller can use.',
      ...
      // ---------------------------------------------------------------------
      // THE CRYPTO REPORT. It calls `adminViews.cryptoView()` and computes
      // nothing of its own, which is rule 7 read strictly: the page and this
      // operation must not be able to disagree about what this service's
      // cryptography is, and the way to make that impossible is for there to be
      // one function.
      //
      ...
      { method: 'GET', path: BASE + '/crypto', tag: 'Service',
        ...
        summary:
          'Every algorithm this service signs, verifies and encrypts with',
        description: 'What this service does with cryptography, for every ' +
                     'identity service it advertises: which digest, which ' +
                     'signature algorithm, which cipher, which key, and ' +
                     'which higher-level envelope each is wrapped in — JOSE, ' +
                     'XMLDSIG and XML Encryption, WS-Security, COSE, X.509, ' +
                     'Kerberos.\n\nEVERY ALGORITHM LIST IN THE REPLY IS READ ' +
                     'FROM THE MODULE THAT PERFORMS THE ALGORITHM, the way ' +
                     'GET /admin/sts-metadata reads its endpoint list off ' +
                     'the live express router — so it cannot claim something ' +
                     'this service does not do, and it reports drift against ' +
                     'that page\'s own family list in both ' +
                     'directions.\n\nThe `postQuantum` member is the one to ' +
                     'read before quoting this reply: the signatures are ' +
                     'partly post-quantum and the key establishment is ' +
                     'entirely classical, and those are reported separately ' +
                     'because a signature is checked when it is presented ' +
                     'while captured ciphertext can be kept and opened ' +
        ...
        handler: function (req, res) {
          ...
          self.sendJson(res, 200, report);
          log.debug("Leaving the management API crypto metadata endpoint.");
        } },

      // ---------------------------------------------------------------------
      // THE ENCRYPTION REPORT. `encryptionAdmin.encryptionView()` and nothing
      // of its own, which is rule 7 read the same strict way the crypto report
      // above it is: the console page and this operation must not be able to
      // report different NUMBERS for the same counters, and one function is the
      // only way to make that impossible.
      //
      // **IT NEEDS NO 503 BRANCH, unlike the operation above it**, and the
      // difference is worth stating rather than looking like an omission: that
      // one reaches its reporter across an inverted hook that a process may not
      // have filled, and this one reaches a module it requires directly. There
      // is no state in which the route exists and the function does not.
      // ---------------------------------------------------------------------
      { method: 'GET', path: BASE + '/encryption', tag: 'Service',
        operationId: 'getEncryption',
        summary: 'What this service encrypts at rest, and how much of it it ' +
                 'has done',
        description: 'The at-rest half of this service\'s cryptography: ' +
                     'which data is sealed, where it lives, under which key ' +
                     'and which algorithm, and how many encryptions and ' +
                     'decryptions have happened in this process.\n\nIT IS ' +
                     'NOT GET /admin-api/crypto WITH FEWER FIELDS. That one ' +
                     'answers what this service DOES when it signs or ' +
                     'encrypts — per protocol family, read out of the module ' +
                     'that performs each algorithm — and reads identically ' +
                     'on a service that started a second ago. This one is ' +
                     'TRAFFIC: `accounting` goes up while something is ' +
                     'happening, counted at the one funnel both operations ' +
                     'pass through rather than at the call sites, because a ' +
                     'total assembled from call sites is wrong the first ' +
                     'time somebody adds another one and is wrong ' +
                     ...
                     '`workers.requestCount` each request worker keeps its ' +
                     'own, so a dispatched service answers this from ' +
                     'whichever worker took the call.\n\nNO CIPHERTEXT AND ' +
                     'NO PLAINTEXT IS IN THE REPLY, and there is no ' +
                     'operation anywhere that opens a sealed value on ' +
                     'request: a sealed value is a private key, an ' +
                     'authenticator\'s shared secret or somebody\'s recovery ' +
                     'codes. `key.where` names the PROVIDER the ' +
                     'key-encryption key is read from and never the ' +
                     'key.\n\n**`boundaries` IS THE PART TO READ BEFORE ' +
                     'ACTING ON THE REST**, and it is in this reply rather ' +
                     'than in a document because a machine reader has no ' +
                     ...
                     'leaves plaintext in the WAL, in spilled sorts, in a ' +
                     'pg_dump, on replicas and in query logs) — that layer ' +
                     'is the operator\'s and docs/encryption-at-rest.md is ' +
                     'the write-up; and the key must not live on ' +
                     'the volume it protects, which is what the ' +
        ...
        responseSchema: { type: 'object',
          description: 'The encryption report: `mode`, `key` (present, ' +
                       ...
                       'two limits of everything else in the reply, as ' +
      ...
      // than discovering it: the members under `probes.*.row` and
      // `probes.*.rows` are PostgreSQL's own columns, so they differ between
      // major versions. A client reading a named column should treat its
      // absence as ordinary.
      // ---------------------------------------------------------------------
      { method: 'GET', path: BASE + '/database', tag: 'Service',
        ...
        description: 'Twenty probes against PostgreSQL\'s catalog views, ' +
                     'each run, timed and caught SEPARATELY: the server and ' +
                     'its uptime, the database size, every counter in ' +
                     '`pg_stat_database`, the backends and locks, the ' +
                     'background writer, the checkpointer, the write-ahead ' +
                     'log and the archiver, then per-table and per-index ' +
                     'statistics, sizes, columns and constraints for the ' +
                     'schema this service owns.\n\n**THE COLUMNS ARE THE ' +
                     'SERVER\'S AND NOT THIS API\'S.** Every statement is a ' +
                     '`SELECT *`, because PostgreSQL moves these views ' +
                     'between major versions — `pg_stat_bgwriter` has eleven ' +
                     'columns on 16 and four on 17 and later, when the ' +
                     'checkpoint counters moved to a view that does not ' +
                     'exist before 17. So a client reading a named column ' +
                     'must treat its absence as ordinary rather than as an ' +
                     'error, and the reply is the right place to learn what ' +
                     'this server actually has.\n\n**A PROBE THAT FAILED IS ' +
                     'A MEMBER AND NOT AN ABSENCE**: `probes.<id>.ok` is ' +
                     'false and `code` carries PostgreSQL\'s SQLSTATE, ' +
                     'because 42P01 (no such relation — an older server) and ' +
                     '42501 (insufficient privilege — this service does not ' +
                     'hold `pg_monitor`) are completely different things to ' +
                     'do something about. `failed` lists them.\n\n`derived` ' +
                     'carries four RATIOS PostgreSQL deliberately does not ' +
                     'keep — cache hit, rollback share, dead-tuple share, ' +
                     'and indexes nothing has ever scanned — all of them ' +
                     'cumulative since `stats_reset`, which is in the reply ' +
                     'beside them. `schemaDrift` is the one ASSERTION here ' +
                     'rather than a measurement: the objects the driver ' +
                     'declares against the ones the server actually has, a ' +
                     'check nothing else in this service makes.\n\n**IT ' +
                     'ANSWERS 200 WITH `available: false` WHEN THERE IS NO ' +
                     'DATABASE**, which is the ordinary case: ' +
                     '`persistence.mode` defaults to `memory`. That is not ' +
                     'an error — the question was answerable and the answer ' +
                     'is that there is nothing to report — and `why` says ' +
                     'which of three reasons it is.\n\nNOTHING HERE CHANGES ' +
                     'ANYTHING and no connection string is in the reply: ' +
                     '`target` names the host, port, database and user, ' +
                     'parsed by the one function in this service that ' +
                     'already does that without printing the password.',
        mirrors: 'GET /admin/database',
        responseDescription: 'The whole report.',
        responseSchema: { type: 'object',
          description: 'The database report: `available` and `why`, ' +
                       '`target`, `pool` (this process\'s client-side pool, ' +
                       'sampled before the page borrows a connection), ' +
                       '`probes` keyed by probe id with the ' +
                       'server\'s own columns inside, `derived`, ' +
                       '`schemaDrift` and `failed`.' },
        handler: function (req, res) {
          log.debug("Entering the management API database report endpoint.");
          // **AWAITED, AND THE HANDLER CATCHES.** Express 4 does not look at
          // what a handler returns, so a rejection here would be an unhandled
          // rejection and a request that never gets an answer — the same trap
          // the token endpoint's wrapper exists for. This is the only operation
          // on this API that talks to a database, so it is the only one that
          // can reject for a reason outside this process.
      ...
      // which is rule 7 read the strict way: the page and this operation must
      // not be able to report a different state of the same store.
      //
      // **NOTHING IN THE REPLY IS A SECRET AND NOTHING IN IT CAME FROM READING
      // ONE.** Every probe behind it is a metadata read — a stat of the key
      // file, a `sys` endpoint, a KV version history, a `DescribeSecret`, a
      // listing of version properties — and `common/secrets.js` deletes a
      // deny-list of member names from whatever a provider hands back before it
      // leaves that module. This operation is `admin:read`, like every other
      // read here, and there is deliberately no write beside it: a rotate would
      // destroy everything sealed under the key.
      // ---------------------------------------------------------------------
      { method: 'GET', path: BASE + '/secrets', tag: 'Service',
        operationId: 'getSecrets',
        summary: 'Where the key-encryption key and the database password ' +
                 'come from, and what the store holding them is doing',
        description: 'The two primordial secrets this service READS and ' +
                     'never writes: the key-encryption key everything it ' +
                     'seals is sealed under, and the database password. For ' +
                     'each — the provider, the location, the field taken out ' +
                     'of a JSON value, whether it is sharing the other ' +
                     'one\'s location, and whether THIS PROCESS has actually ' +
                     'read it, with the instant and the error if the last ' +
                     'attempt failed. That last one is the fact no settings ' +
                     'row can carry: a development-mode service never asks ' +
                     'for the key, so a perfectly broken configuration and a ' +
                     'working one look identical until the mode ' +
                     'changes.\n\n`stores` is one member per STORE rather ' +
                     'than per secret, because two secrets kept in one place ' +
                     'share its state. What is in it depends on the ' +
                     'provider: `file` answers the path, mode, owner, mtime ' +
                     'and which members the file holds if it is JSON; ' +
                     '`vault` answers the seal status, the health summary, ' +
                     'the leader, the certificate this service presents and ' +
                     'when it expires, the token the login produced, **what ' +
                     'that identity may actually do — asked of the store ' +
                     'rather than quoted from a policy file** — and the ' +
                     'engines it can see; `aws`, `gcp` and `azure` publish ' +
                     'their metadata against the secret itself.\n\n**NO ' +
                     'PROBE FETCHES A SECRET VALUE AND NONE IS IN THE ' +
                     'REPLY.** Every one is a metadata read, and the module ' +
                     'they live in deletes a deny-list of member names — ' +
                     '`value`, `data`, `token`, `id` among them — from ' +
                     'whatever a provider answers before it leaves.\n\n**A ' +
                     'PROBE THAT FAILED IS A MEMBER AND NOT AN ABSENCE**, ' +
                     'with `ok: false` and, where the store gave one, the ' +
                     'HTTP status in `status`. Here half of them are ' +
                     'SUPPOSED to fail: the identity this service holds is ' +
                     'bound to two read paths, so a 403 against anything ' +
                     'else is the policy working. Each is bounded ' +
                     'by `keys.storeProbeTimeoutMs` and they run in ' +
                     'parallel, so an unreachable store costs that bound once.',
        mirrors: 'GET /admin/secrets',
        responseDescription: 'The whole report.',
        responseSchema: { type: 'object',
          description: 'The secret-store report: `secrets` (one per secret, ' +
                       'with `configured`, `provider`, `where`, `field`, ' +
                       '`shared`, `lastRead` and its own `probes`), `stores` ' +
                       '(one per store, with `probes`), `mode`, ' +
                       '`persistingKeys`, `timeoutMs` and `failed`.' },
        handler: function (req, res) {
          log.debug("Entering the management API secret store report " +
                    "endpoint.");
          ...
          secretsAdmin.secretsView().then(function (report) {
            self.sendJson(res, 200, report);
            log.debug("Leaving the management API secret store report " +
                      "endpoint.");
          }).catch(function (e) {
      ...
      // ---------------------------------------------------------------------
      // THE EMBEDDED PROTOCOL DEBUGGER (2026-09-13).
      // `debuggerAdmin.debuggerView()` and nothing else — rule 7 — and
      // read-only: the debugger's settings are `config/set` like every other
      // row, and who may USE it is the two console roles, granted at
      // `rbac/grant`. Pinned to the front process, which is the only one
      // holding the listener and the api child.
      // ---------------------------------------------------------------------
      { method: 'GET', path: BASE + '/debugger', tag: 'Service',
        operationId: 'getDebugger',
        summary: 'Whether the identity protocol debugger is embedded, where ' +
                 'it answers, and what its api process is doing',
        description: 'The embedded identity protocol debugger: `embedded` ' +
                     'and the setting and mode that decided it, the ' +
                     'listener\'s `port` and whether it is `listening`, the ' +
                     'client (`clientId`), the resource server (`resource`), ' +
                     'the permission its api requires (`permission`) and the ' +
                     'audience a token for it carries (`audience`), and ' +
                     'under `api` the child process: its `state` ' +
                     '(`starting`, `running`, `restarting`, `given-up`, ' +
                     '`not-installed`, `stopped`), process id, socket, ' +
                     'starts, consecutive failures, last exit and last ' +
                     'error, and — in product mode — the `allowedRanges` it ' +
                     'may dial. `settings` is the `Protocol debugger` group ' +
                     'as `/admin/debugger` draws it.\n\nNothing here opens ' +
                     'the debugger: its gate is the permission, ' +
                     'which the authorization server issues to ' +
        ...
        handler: function (req, res) {
          ...
          try {
            self.sendJson(res, 200, debuggerAdmin.debuggerView());
          } catch (e) {
      ...
      // ---------------------------------------------------------------------
      // THE KEY PAIRS. Two operations, because LISTING what this process holds
      // and HANDING A KEY OVER are different acts and only the second is a
      // write.
      // ---------------------------------------------------------------------
      { method: 'GET', path: BASE + '/keys', tag: 'Service',
        operationId: 'getKeys',
        summary: 'Every key pair this process generated at start, and what ' +
                 'it is for',
        description: 'A LIST and never key material. The signing keys are ' +
                     'per trust realm and the TLS certificate belongs to the ' +
                     'process, and each row says which. `formats` is what ' +
                     'that key can be exported as, computed rather than ' +
                     'listed: a key with no certificate cannot be a PKCS#12, ' +
                     'and a post-quantum key has no PKCS#8 encoding at all, ' +
                     'so the answer differs per key for two different ' +
                     'reasons.\n\nNothing here is key material. POST ' +
                     '/admin-api/keys/export is the operation that hands one ' +
                     'over.',
      ...
      // has heard of and requires a 400 naming the ones that exist. A literal
      // `/keys/export` looked like that resource and was not one, so the probe
      ...
      { method: 'POST', route: BASE + '/keys/:action', tag: 'Service',
        ...
        handler: function (req, res) {
          ...
          if (body.action !== 'export') {
            ...
            // console action has an operation here. A handler that stopped
            // writing it would turn that check off with nothing failing.
          ...
          pending.then(function (result) {
            ...
            self.sendJson(res, 400,
                          { ok: false, errors: ['The export failed: ' +
                                                e.message] });
        ...
        actions: [
          { action: 'export', operationId: 'exportKey',
            ...
            description:
              'THIS OPERATION RETURNS PRIVATE KEY MATERIAL. It is the ' +
                     'API half of the one page in this console where reading ' +
                     'is taking.\n\nIt is defensible because of what these ' +
                     'keys are in development mode: generated at start, held ' +
                     'only in memory, dead when the process exits, and ' +
                     'protecting nothing — this service checks no password ' +
                     'and validates no token it did not mint. In product ' +
                     'mode the signing keys are kept in the persistence ' +
                     'store, and this operation hands over one that outlives ' +
                     'the call. It needs an access token carrying ' +
                     '`admin:write` (unless `adminApi.authRequired` is off, ' +
                     'when anybody who can reach this port can call it), ' +
                     'like every other write here — stated again because ' +
                     'this one returns a key.\n\n`format` is one of `pem`, ' +
                     '`der`, `jwk` or `pkcs12`. A password is REQUIRED for ' +
                     '`pkcs12` and optional for the rest, where it encrypts ' +
                     'the private half. The reply carries the file ' +
                     'base64-encoded rather than raw, because this API ' +
                     'answers JSON everywhere else ' +
                     'and a caller that suddenly ' +
                     'got octets would have to special-case one operation.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
                ...
                format: { type: 'string', enum: ['pem', 'der', 'jwk', 'pkcs12'],
                          description: 'PKCS#12 is offered only for a key ' +
                                       'this service holds a certificate for ' +
                                       '— the signing key and the TLS key.' },
                password: { type: 'string',
                            description: 'REQUIRED for `pkcs12`; optional ' +
                                         'for the other three, where it ' +
                                         'encrypts the private half. ' +
                                         'Empty means the private ' +
                                         'key comes out in the clear.' }
              },
              required: ['key', 'format'],
              // THE EXAMPLE IS WHAT DRIVES THIS OPERATION IN THE SUITE.
              // `sts_admin_api_operations.js` walks every GET and every POST
              // that carries one; an operation with no example is covered by
              // nothing and reported by nothing, which is what its ledger
              // refuses. `pem` with no password is chosen deliberately — it is
              // the one combination that needs nothing set up and hands back a
              // file every tool reads.
      ...
      // written to be opened in a browser had become the one page a browser
      // could not open. The console linked to it and the link answered 401.
      //
      // **THE OPERATION BELOW IS WHAT RULE 7 ASKS FOR NOW.** A console page
      // gets an operation here that names it, and this is that page's — it
      // reports what the explorer is reading and what the caller's roles would
      // let them do, without repeating the document, which `GET
      // /admin-api/openapi.json` above already is.
      //
      // Two things did NOT move and are worth saying so nobody goes looking:
      // `admin_api_docs.ts` and `admin_api_explorer.js` are still in this
      ...
      { method: 'GET', path: BASE + '/api-explorer', tag: 'Service',
        ...
        description: 'The explorer is a page of the ADMIN CONSOLE at ' +
                     ...
                     '2026-09-09, when this API began requiring an access ' +
                     'token a browser cannot carry. This operation reports ' +
                     'where the document is, how many paths and operations ' +
                     'it describes, and the audience a ' +
                     'token for this API must name. It does ' +
                     'NOT repeat the document: `GET ' + BASE +
                     '/openapi.json` ' +
                     'is the document.\n\nThe `scope` member is what the ' +
                     'CONSOLE SESSION\'s roles would grant — it is what the ' +
                     'page puts in the token it mints for the person reading ' +
                     'it, so a reader holding Admin Read alone sees ' +
                     '`admin:read` and knows before pressing anything that a ' +
                     'write would be refused. **It is EMPTY when this ' +
                     'operation is called with an access token rather than ' +
                     'read off the page**, which is the ordinary case here: ' +
                     'there is no console session on such a request, and ' +
                     'reporting the token\'s own scopes back to the caller ' +
                     'that sent them would be telling somebody what they ' +
                     'just said.',
        mirrors: 'GET /admin/api-explorer',
        responseDescription: 'Where the explorer reads from, and what the ' +
                             'caller may drive.',
        responseSchema: { type: 'object', properties: {
          page: { type: 'string', description: 'The console page.' },
          api: { type: 'string', description: 'The API it drives.' },
          document: { type: 'string',
                      description: 'Where the page reads the OpenAPI ' +
                                   'document from — a console ' +
                                   'path, so that it arrives on ' +
          ...
          audience: { type: 'string',
                      description: 'What a token for this API must name in ' +
                                   '`aud`. Computed outside any realm, ' +
                                   'because the credential is service-wide.' },
          tokenInReply: { type: 'boolean',
                          description: 'Always false, and named so that its ' +
                                       'absence is a statement rather than ' +
                                       'an omission: the console page is ' +
                                       'handed a token because it has ' +
                                       'already authenticated ' +
                                       'the person reading it, and ' +
        ...
        handler: function (req, res) {
          ...
          // module is loaded at 19a — after this one — because it needs the
      ...
      { method: 'GET', path: BASE + '/status', tag: 'Service',
        ...
        description: 'The issuer, when it started, and the running totals. ' +
                     'The cheapest call here and the one to poll: it takes a ' +
      ...
      { method: 'GET', path: BASE + '/metrics', tag: 'Metrics',
        ...
        description: 'Endpoint calls by matched route and status class, ' +
                     'tokens and artifacts by kind with the state of each, ' +
                     'and sessions counted BOTH ways — the browser sign-on ' +
                     'sessions this process holds, and ' +
                     'the sessions implied by what it has ' +
                     'issued. The two disagree on purpose; the schema says ' +
                     'why.',
        ...
        handler: function (req, res) {
          ...
          self.sendJson(res, 200, adminViews.metricsJson());
      ...
      { method: 'GET', path: BASE + '/users', tag: 'Users',
        ...
        summary:
          'Everyone this service has authenticated, or one of them in full',
        description: 'Without `user` it is the list. With it, one identity: ' +
                     'the names they were seen under, how they authenticated ' +
                     'each time, every sign-on session they hold with the ' +
                     'tokens issued ON each of those, and their LDAP entry. ' +
                     'A name this service has never seen answers 200 with ' +
                     '`known: false` rather than 404 — it is an answer about ' +
                     'the identity, not about the route.\n\nThe identity is ' +
                     'a QUERY PARAMETER and not a path segment on purpose: ' +
                     'the identities here contain the characters a path is ' +
                     'made of (a Kerberos service principal is `HTTP/host`, ' +
                     'a subject is a `urn:`), so /users/HTTP/web.example.com ' +
                     'would be a two-segment path naming nobody.\n\nONE PAGE ' +
                     'PARAMETER IS NOT IN THE LIST BELOW, because its name ' +
                     'is data: each session\'s own token list is moved by ' +
                     '`session-<the session id>Page`, so ' +
                     '`?user=alice&session-8Qk3...Page=2` moves that block ' +
                     'and no other, and each session in the reply answers ' +
                     'with its own `tokensPaging`. It is named after the ' +
                     'session rather than numbered so that the link still ' +
                     'moves the same session after the list around it has ' +
                     'changed, and it is paged at all because one browser ' +
                     'session can hold most of the tokens this service ' +
                     'remembers. OpenAPI cannot spell a parameter whose name ' +
                     'is built at runtime, and a `session-{id}Page` in the ' +
                     'list would generate a client that sends a literal ' +
                     '`{id}` — so it is here in words instead.',
        mirrors: 'GET /admin/users',
        parameters: [
          { name: 'user', in: 'query', required: false,
            schema: { type: 'string' },
            description: 'One identity, as the list\'s `key` names them. ' +
                         'Returns the drill-down instead of the list.' },
      ...
      // RULE 7 FOR /admin/users/new, and it earns its place beyond the parity
      // for the same reason `getNewApplicationForm` does one resource along:
      // what it answers is the CLOSED CATALOGUE `createUser()` validates
      // `attributes` against. A caller that reads this cannot construct a
      // create the service will refuse with "a person here does not have an
      // attribute called ...", and it learns the list from the service rather
      // than from a copy of it in a document.
      //
      ...
      // nothing — it fills a FORM in for a person to edit, and the values it
      ...
      { method: 'GET', path: BASE + '/users/new', tag: 'Users',
        ...
        description: 'The ATTRIBUTE CATALOGUE a create takes in `attributes` ' +
                     ...
                     'carry, each naming the claim it reaches in an issued ' +
                     'token or credential and the document its name comes ' +
                     'from — plus the container DN the entry would land in, ' +
                     'the realm, and the four `credential` options.\n\n**It ' +
                     'creates nobody**: the create is `POST ' +
                     '/admin-api/users/create`. This is the list that call ' +
                     'validates against, and an attribute name that is not ' +
                     'on it is REFUSED rather than dropped — so a caller ' +
                     'that reads this first cannot be told afterwards that ' +
                     'half of what it sent was ignored.\n\n**`uid` and ' +
                     '`userPassword` are deliberately not on it.** `uid` is ' +
                     'the username, sent as `username`, and a second way to ' +
                     'set it would allow an entry at `uid=alice` whose uid ' +
                     'attribute says `bob`. A password goes through ' +
                     '`credential`, so that `credentials.js` hashes it — an ' +
                     'attribute door that took `userPassword` would write ' +
                     'one in the clear.\n\n**The container is THIS ' +
                     'REALM\'S.** The embedded directory is per trust realm, ' +
                     'so `/realm/acme/admin-api/users/new` answers with ' +
      ...
      { method: 'POST', route: BASE + '/users/:action', tag: 'Users',
        // TWO CONSOLE PATHS, AND BOTH ARE NAMED. One resource legitimately
        // mirrors several controls — /admin-api/xacml/{action} names three —
        // and this action switch is reached from the Users list and from
        // /admin/users/new, which posts to itself rather than to the list so
        ...
        actions: [
          { action: 'issue-activation', operationId: 'issueActivationLink',
            summary:
              'Issue a one-time activation link for a provisioned person',
            description: 'How somebody created through POST ' +
                         '/admin-api/users/create, SCIM or an LDAP add comes ' +
                         'to have a way in. They are provisioned with no ' +
                         'credential; this mints a single-use, time-limited ' +
                         'URL at which they choose a password, a security ' +
                         'key, or both.\n\n**THE URL IS RETURNED ONCE AND ' +
                         'NEVER AGAIN.** What is stored is a scrypt hash, so ' +
                         'this service cannot produce it a second time — ' +
                         'only replace it, which is what calling this again ' +
                         'does. Treat it as the credential it is: anybody ' +
                         'holding it can complete the account setup, so a ' +
                         'leaked link is an account takeover.\n\nIt expires ' +
                         'after `security.activationTtlMinutes` and is spent ' +
                         'the moment the setup FINISHES — not when the link ' +
          ...
          { action: 'create', operationId: 'createUser',
            ...
            description: 'An entry under `ou=users` usually appears because ' +
                         'somebody AUTHENTICATED — at either sign-in screen, ' +
                         'on a password grant, with a `UsernameToken`, in a ' +
                         'Kerberos AS-REQ. This is how to get one in ahead ' +
                         'of that, which is what a client testing claims ' +
                         'from the directory needs: the entry carries the ' +
                         'invented person behind that name, so a credential ' +
                         'issued for them and an `ldapsearch` for the entry ' +
                         'say the same thing from the start.\n\n**One entry ' +
                         'per person, and this is one of three doors onto ' +
                         'that rule.** A username already here is refused ' +
                         'with the DN that holds it — whatever protocol ' +
                         'brought them, and whichever attribute their entry ' +
                         'is named by, since a person whose entry was ' +
                         'created by a client certificate is at ' +
                         '`cn=<name>,ou=users` rather than ' +
                         '`uid=<name>,ou=users`. An `ldapadd` under ' +
                         '`ou=users` gets the same refusal as ' +
                         'LDAP_ENTRY_ALREADY_EXISTS (68), because all three ' +
                         'call one function.\n\n**No password is set unless ' +
                         'one is ASKED FOR** through `credential` below. ' +
                         'That default is what this operation has always ' +
                         'done and is right in development mode, where no ' +
                         'password is checked here in this protocol or any ' +
                         'other; in product mode a person with no credential ' +
                         'cannot sign in, and `activation` is how they are ' +
                         'given one. Creating the entry does not put the ' +
                         'name in `GET /admin-api/users`: that lists ' +
                         ...
                         '/admin/users/new form posts. `attributes` are ' +
                         'checked against the catalogue `GET ' +
                         '/admin-api/users/new` publishes and an unknown ' +
                         'name is REFUSED rather than dropped; `invent` ' +
                         'decides whether the rest are made up, and ' +
                         'it DEFAULTS TO TRUE so that a caller written ' +
            ...
            requestBody: {
              ...
              properties: {
                username: { type: 'string',
                            description: 'The name they will authenticate ' +
                                         'under — the same string that ' +
                                         'appears in a token\'s `sub` and on ' +
                                         '/admin/users. Not a DN and not a ' +
                                         '`did:`, and it may not carry a ' +
                                         'character RFC 4514 reserves in a ' +
                                         'DN: those name entries that ' +
                                         'get here by being presented ' +
                                         'rather than by being created.' },
                note: { type: 'string',
                        description: 'Optional. What the entry\'s ' +
                                     '`description` says about why it ' +
                                     'exists; the default says it was ' +
                                     'created by hand rather than by ' +
                                     'authenticating. An ' +
                                     '`attributes.description` wins over it, ' +
                                     'because an operator\'s own sentence ' +
                                     'about a person is the more useful one ' +
                                     'and two values would be the entry ' +
                ...
                attributes: {
                  ...
                  description: 'What is known about them, as `{attribute: ' +
                               ...
                               'document\'s own spelling, and they are the ' +
                               'names the entry carries — so an `ldapsearch` ' +
                               'shows exactly what was sent.\n\n**A NAME ' +
                               'THAT IS NOT ON THE CATALOGUE IS REFUSED and ' +
                               'the whole create fails**, rather ' +
                               'than the value being ignored: ' +
                               'silently dropping it would answer "created" ' +
                               'to a ' +
                               'request asking for something this did not ' +
                               'do. `userPassword` is refused by that rule — ' +
                               'use `credential` — and so is `uid`, which is ' +
                               '`username`.\n\nAn empty string is the same ' +
                               'as sending nothing: the attribute is absent ' +
                               'from the entry rather than present and empty.',
                ...
                invent: {
                  ...
                  description: 'Whether to MAKE UP the attributes not sent. ' +
                               ...
                               'invents a consistent person per username, so ' +
                               'the entry and any credential issued for them ' +
                               'agree from the start.\n\nSend `false` for an ' +
                               'entry carrying ONLY what you sent — its ' +
                               'object classes, its uid, a description and ' +
                               'your attributes. That is what the console\'s ' +
                               'form does. It is not a promise the entry ' +
                               'stays that way: the Populate button on ' +
                               '/admin/vc fills every missing ' +
                               'SELECTED attribute on every ' +
                               'person, and does not know which were typed.'
                },
                credential: {
                  type: 'string',
                  description: 'How they first get in. **`generate` is the ' +
                               'DEFAULT since 2026-09-12**: a password drawn ' +
                               'to satisfy this realm\'s password policy ' +
                               '(`GET /admin-api/policies`), set, and ' +
                               'RETURNED ONCE in `password` — this service ' +
                               'stores a scrypt hash and cannot produce it ' +
                               'again. It costs one hash per create, about ' +
                               '70ms, so a bulk load that wants nobody ' +
                               'holding anything sends `none`. `none` was ' +
                               'the default before, and in development mode ' +
                               'it is enough to sign in, since no password ' +
                               'is checked there; `password`, hashing the ' +
                               '`password` field onto the entry, and refused ' +
                               'by the password policy in product mode when ' +
                               'it does not meet it; `activation`, issuing a ' +
                               'single-use link and returning it ONCE in ' +
                               '`activationUrl` for you to send them, at ' +
                               'which they choose a password, a security key ' +
                               'or both.\n\n**A CREDENTIAL STEP THAT FAILS ' +
                               'DOES NOT UNDO THE CREATE.** A password can ' +
                               'only be written onto an entry that exists, ' +
                               'so the person is there either way; the reply ' +
                               'is `ok: true` with `credentialError` set and ' +
                               'says so, because answering `ok: ' +
                               'false` would send a caller to ' +
                ...
                password: { type: 'string',
                            description: 'Read only when `credential` is ' +
                                         ...
                                         'logged in the clear; nothing in ' +
                                         'this service can show it again.' },
                passwordConfirm: { type: 'string',
                                   description: 'Optional, and CHECKED WHERE ' +
                                                'SENT: the console\'s form ' +
                                                'always sends it, because a ' +
                                                'mistyped password nobody ' +
                                                'can read back is a person ' +
                                                'who cannot sign in and ' +
                                                'nobody who can say why. An ' +
                                                'API caller with one ' +
            ...
            responseDescription: 'The entry as created, in `entry`, with its ' +
                                 '`dn`; `typed` naming the attributes you ' +
                                 'sent and `invented` whether the rest were ' +
                                 'made up. A generated password is in ' +
                                 '`password` and an activation link in ' +
                                 '`activationUrl` — EACH RETURNED ONCE, ' +
                                 'because what is stored is a hash and this ' +
                                 'service cannot produce either again.' },
          ...
          // twice — in the sentence a refused sign-in gets, and in the banner
          // the product-mode bootstrap prints telling an operator to change the
          ...
          { action: 'set-password', operationId: 'setUserPassword',
            ...
            description: 'Hashed with scrypt by `credentials.js`, which is ' +
                         'the one place in this service a password is ever ' +
                         'verified or set, and written to `userPassword` on ' +
                         'their directory entry. **It cannot be read back by ' +
                         'anything** — not this API, not the console, not an ' +
                         '`ldapsearch`, which sees the hash — so a lost ' +
                         'password is replaced rather than ' +
                         'recovered.\n\nSend `password`, or `generate: true` ' +
                         'to have one made up and RETURNED ONCE. The person ' +
                         'must already exist; this creates nobody.\n\n**IN ' +
                         'DEVELOPMENT MODE THIS CHANGES ALMOST NOTHING AND ' +
                         'IS STILL WORTH DOING.** Nothing here checks a ' +
                         'password in development — every one is accepted — ' +
                         'so setting one does not make a sign-in work that ' +
                         'would otherwise fail. What it does is put the ' +
                         'attribute on the entry, which is what an LDAP ' +
                         'client reads, what `hasPassword()` ' +
                         'counts, and what product ' +
                         'mode would need. In PRODUCT mode it is the ' +
                         'credential.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
                ...
                username: { type: 'string',
                            description: 'Accepted for `user`.' },
                password: { type: 'string',
                            description: 'The password to set. Required ' +
                                         'unless `generate` is true.' },
                passwordConfirm: { type: 'string',
                                   description:
                                     'Optional, checked where sent.' },
                generate: { type: 'boolean',
                            description: 'Make one up instead — 32 bytes of ' +
                                         'randomBytes, base64url, the same ' +
                                         'generator the product-mode ' +
                                         'bootstrap account uses. ' +
                                         'It is RETURNED ONCE in ' +
            ...
            responseDescription: 'Whether it was set, and — only where it ' +
                                 'was GENERATED — the password, once. A ' +
                                 'password you sent is never echoed back: ' +
                                 'you already hold it, and returning it ' +
          ...
          // API that handed one out would be an administrative door that mints
          // a working second factor for any account — which is not a second
          // factor at all. A WebAuthn ceremony happens in the person's own
          // browser against their own authenticator, which no API can stand in
          // for. Both are `/portal`, or `/portal/activate` with a link that is
          // itself a credential.
          // -----------------------------------------------------------------
          { action: 'clear-totp', operationId: 'clearUserAuthenticatorApp',
            summary: 'Clear somebody\'s authenticator app enrolment',
            description: '**THE ONLY WAY BACK FOR SOMEBODY WHO HAS LOST ' +
                         'THEIR PHONE.** The shared secret lives on that ' +
                         'device and this service cannot reach it, and there ' +
                         'is deliberately no self-service reset anywhere — a ' +
                         'second factor anybody can remove is not a second ' +
                         'factor.\n\nIt CANNOT lock anybody out: a one-time ' +
                         'code is never a primary credential here, so ' +
                         'clearing one drops the account to one factor ' +
                         'rather than to none. The person sets a new one up ' +
                         'at `/portal/mfa`.\n\nClearing an enrolment nobody ' +
                         'holds answers 400 rather ' +
                         'than 200: the caller asked ' +
                         'to clear a specific thing and it was not there.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
          ...
          //
          // There is no `issue-backup-codes` beside it and there will not be,
          // for the reason the two above have no `enrol`: a set is created by
          // the ACT of enrolling a second factor and by nothing else, and a
          // management API that minted one would be an administrative door
          // handing a working second factor to any account. What this operation
          // does is DELETE a set, which re-arms the automatic issue — the next
          // second factor that person enrols creates a new one.
          // -----------------------------------------------------------------
          { action: 'clear-backup-codes', operationId: 'clearUserBackupCodes',
            summary: 'Clear somebody\'s recovery codes',
            description: '**THE ONLY ROUTE TO A SECOND SET.** A set is ' +
                         'issued automatically, and ONCE, the first time ' +
                         'somebody asks for one from the user portal; ' +
                         'does nothing while a set exists, so clearing is ' +
                         'what lets the next enrolment issue one.\n\n**It ' +
                         'cannot lock anybody out** — a recovery code is ' +
                         'never a way in on its own — but it removes the way ' +
                         'BACK, so somebody whose set is cleared and who ' +
                         'then loses their phone needs an operator ' +
                         'again.\n\n**There is no operation that READS the ' +
                         'codes and there will not be.** They are a working ' +
                         'second factor; `GET /users` reports how many ' +
                         'remain and never what they are. The person reads ' +
                         'their own set back on `/portal/mfa`, which is the ' +
                         'only place in this service that shows ' +
                         'one.\n\nClearing a set nobody holds answers 400 ' +
                         'rather than 200, for `clear-totp`\'s reason.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
          ...
          { action: 'reset-password', operationId: 'resetUserPassword',
            summary:
              'Reset somebody\'s password to a generated one, returned once',
            description: 'Generates a password under this realm\'s password ' +
                         'policy, sets it, and **returns it ONCE** in ' +
                         '`password` — the entry holds a scrypt hash and ' +
                         'nothing can show it ' +
                         'again. Then three things the ordinary ' +
                         '`set-password` does not do: `pwdReset` is set, so ' +
                         'the person must choose their own password at the ' +
                         'sign-in screen before anything is signed in; any ' +
                         'password reset link outstanding is spent; and they ' +
                         'are **signed out of everything** through the same ' +
                         'function `POST /admin-api/logout/global` ' +
                         'calls.\n\n**Shared Signals**: a CAEP ' +
                         '`credential-change` (`password`, `update`) and a ' +
                         'RISC `account-credential-change-required`, plus a ' +
                         'CAEP `session-revoked` for each session the ' +
                         'sign-out ends. They are sent after the reply, and ' +
                         '`caep.autoEmitTypes` ' +
                         'and `risc.autoEmitTypes` decide whether they go at ' +
                         'all.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
            ...
            responseDescription: 'The generated password, ONCE, whether the ' +
                                 'forced change was ' +
                                 'recorded (`forcedChange`) ' +
                                 'and what the sign-out ended (`signedOut`).' },

          { action: 'issue-password-reset',
            operationId: 'issuePasswordResetLink',
            summary:
              'Revoke somebody\'s password and issue a one-time reset link',
            description: 'Mints a single-use link to ' +
                         '`/portal/reset-password`, **returned ONCE** in ' +
                         '`resetUrl` (absolute, on the address this request ' +
                         'arrived at, realm prefix included), valid for ' +
                         '`security.passwordResetTtlMinutes`. The token in ' +
                         'it is stored as a hash; issuing another replaces ' +
                         'it.\n\n**The password the person had is REMOVED** ' +
                         'and they are ' +
                         '**signed out of everything**, so until the link is ' +
                         'used ' +
                         'they cannot sign in with a password. At the link ' +
                         'they choose a new one under the realm\'s password ' +
                         'policy; the removed password is in the history, so ' +
                         'it cannot simply be chosen again where the history ' +
                         'is enforced.\n\n**Shared Signals**: a CAEP ' +
            ...
            responseDescription: 'The reset link, ONCE, when it expires, ' +
                                 'whether a password was removed ' +
                                 '(`passwordRevoked`) and ' +
                                 'what the sign-out ended.' },

          { action: 'disable-primary-keys', operationId: 'disableUserPasskeys',
            summary: 'Remove every primary security key, so a passkey no ' +
                     'longer signs somebody in',
            description: 'Removes every WebAuthn credential enrolled in the ' +
                         '`primary` role — the ones that sign the person in ' +
                         'with no password. A key in the `mfa` role is ' +
                         'untouched.\n\n**Refused where it would leave no ' +
                         'way in**: a person with no password whose primary ' +
                         'keys are their only credential. Reset their ' +
                         'password or issue a reset link first. Refused too ' +
                         'for somebody holding no primary key.\n\n**Shared ' +
                         'Signals**: a CAEP `credential-change` ' +
                         '(`fido2-roaming`, `delete`) per key, ' +
            ...
            responseDescription: 'The keys removed — id, role, label and ' +
                                 'when enrolled; never a public key.' },
          ...
          { action: 'disable-mfa', operationId: 'disableUserSecondFactors',
            ...
            description: 'Removes the authenticator app enrolment, every ' +
                         'security key in the `mfa` role and the recovery ' +
                         'codes, in one act. It cannot lock anybody out, ' +
                         'because none of ' +
                         ...
                         'a CAEP `credential-change` (`delete`) for the ' +
                         'authenticator app (`app`) and for each key ' +
                         '(`fido2-roaming`), and a RISC ' +
                         '`recovery-information-changed` where recovery ' +
                         'codes were removed.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
            ...
            responseDescription:
              'What was removed, and whether a second factor ' +
          ...
          { action: 'require-mfa', operationId: 'requireUserSecondFactor',
            ...
            description: 'Sets `stsMfaRequired` on the person\'s entry. At ' +
                         'the sign-in screen they are then asked for a ' +
                         'second factor — and, while they hold none, shown a ' +
            ...
            responseDescription: 'The requirement as it now stands, by ' +
                                 'account and by realm.' },
          ...
          { action: 'clear-key', operationId: 'clearUserSecurityKey',
            ...
            description: 'Goes through the same `removeKey()` the person\'s ' +
                         'own portal calls, which is what carries the ' +
                         'refusal that matters: **it will not remove the ' +
                         'last way in.** A person with no password whose ' +
                         'only PRIMARY key this is would be locked out by an ' +
                         'operator\'s call, and an operator must not be able ' +
                         'to do what the owner is stopped from doing.\n\nThe ' +
                         'credential id is the base64url one ' +
                         'on this person\'s `factors.keys`, on ' +
      ...
      //
      // Both read `logout/logout.ts`, which is the one model of what a live
      // session is — and the POST here is `terminate()` with a selection of
      // one, the SAME function `POST /admin-api/logout/selective` calls. Two
      // operations over one termination, which is the arrangement rule 7 asks
      // for: the console grew a button, so this grew the operation that mirrors
      // it.
      { method: 'GET', path: BASE + '/sessions', tag: 'Sessions',
        operationId: 'getSessions',
        summary: 'Every session this service is holding right now, across ' +
                 'the three protocols that have one',
        description: 'WHAT IS LIVE, as opposed to what has been ISSUED — ' +
                     'which is `GET /admin-api/tokens`, and everything in ' +
                     'that list outlives everything in this one.\n\nA ' +
                     'session is state THIS SERVICE holds that makes ' +
                     'somebody currently authenticated. Three protocols here ' +
                     'have one and the other seven do not:\n\n* the ' +
                     '**browser sign-on session** from `/authn/login`, which ' +
                     'OAuth 2.0 / OIDC, WS-Federation, SAML 2.0, SAML 1.1 ' +
                     'and the admin console all share — so one row may be ' +
                     'carrying relying parties, realms and service providers ' +
                     'at once, and `protocol` says which protocol the ' +
                     'sign-in came THROUGH while `carries` says what is ' +
                     'riding on it;\n* the **Kerberos ticket-granting ' +
                     'ticket**, because a TGT is the Kerberos session and a ' +
                     'service ticket is one use of it;\n* the **LDAP ' +
                     'connection**, because RFC 4511 section 4.2 makes a ' +
                     'Bind the authorization state of a CONNECTION.\n\n**THE ' +
                     'EXPIRY IS WORKED OUT DIFFERENTLY IN EACH AND ' +
                     '`expiryRule` SAYS HOW.** A browser session expires at ' +
                     'an absolute instant fixed when it was created ' +
                     '(`authn.sessionLifetimeS`) and is NOT extended by use; ' +
                     '`authn.sessionIdleTimeoutS`, off by default, ends one ' +
                     'earlier when it goes unused. A TGT expires at the ' +
                     '`endtime` the KDC sealed into the ticket, which ' +
                     'nothing can move. An LDAP connection has NO EXPIRY and ' +
                     'answers `expiresAt: 0`, which is not an expiry of the ' +
                     'epoch.\n\nEvery row carries the `key` and `id` that ' +
                     '`POST /admin-api/sessions/revoke` takes, and — ' +
                     'for a browser session — the `sessionId` that `GET ' +
                     '/admin-api/tokens?session=` takes.',
        mirrors: 'GET /admin/sessions',
        parameters: [
          { name: 'q', in: 'query', required: false, schema: { type: 'string' },
            description: 'Narrows to rows whose username, subject, handle, ' +
                         'identity key, kind or protocol contains this. One ' +
                         'box over all of them, because a reader arrives ' +
                         'holding exactly one and does not know which.' },
          { name: 'protocol', in: 'query', required: false,
            schema: { type: 'string' },
            description: 'Only sessions started through this protocol. The ' +
                         '`protocol` values in the reply are what there is ' +
                         'to ask for; they are read off the live rows rather ' +
                         'than written down, because a federated sign-in ' +
                         'names the relationship it came through.' }
        ...
        handler: function (req, res) {
          ...
          self.sendJson(res, 200, adminViews.sessionsView(req).json);
        ...
        actions: [
          { action: 'revoke', operationId: 'revokeSession',
            ...
            description: 'The Revoke button on every row of /admin/sessions, ' +
                         'driven without a browser. It is `terminate()` with ' +
                         'a selection of one — the SAME function a global ' +
                         'logout goes through — so it writes the same audit ' +
                         'row, honours the same two settings and gives the ' +
                         'same refusals.\n\n**IT IS NOT ONE ACT ACROSS THE ' +
                         'THREE KINDS AND THE DIFFERENCE MATTERS.** On a ' +
                         'browser session it ends that session and ' +
                         'everything hanging off it: the relying parties are ' +
                         'notified, the refresh tokens issued on it are ' +
                         'revoked. On an LDAP row it closes the socket, ' +
                         'which the client sees as its connection dropping ' +
                         'mid-conversation. On a Kerberos row it does MORE ' +
                         'than the row it names — it stamps a sign-out ' +
                         'instant on the PRINCIPAL, after which every ' +
                         'ticket-granting ticket that principal ' +
                         'authenticated before now is refused ' +
                         'KDC_ERR_TGT_REVOKED (20), because Kerberos has no ' +
                         'per-ticket revocation. It still reaches no service ' +
                         'ticket already in a cache: accepting one never ' +
                         'contacts this KDC. Every row\'s `why` says which ' +
                         'of the three it is before it is pressed.\n\nA row ' +
                         'that names nothing any more — it ended between the ' +
                         'read and the write — answers 400 saying so ' +
                         'rather than 200 having done nothing, and the ' +
              ...
              properties: {
                key: { type: 'string',
                       description: 'Whose session it is: the normalised ' +
                                    ...
                                    '/admin-api/sessions carries as `key`. ' +
                                    'The termination is keyed ' +
                                    'on an identity, which ' +
                                    'is why this is needed beside the id.' },
                select: { type: 'string',
                          description: 'Which session, in the form the list ' +
                                       'gives it as `id`: `session:…`, ' +
                                       '`ldap:…` or `krb5:…@REALM`. It is ' +
                                       'the identifier POST ' +
                                       '/admin-api/logout/selective takes ' +
            ...
            responseDescription: 'What was ended, or why it could not be. ' +
      ...
      { method: 'GET', path: BASE + '/logout', tag: 'Sign-out',
        ...
        description: 'The protocol-independent view of a person\'s live ' +
                     'state. Without `user` it is the FAMILY LIST — what a ' +
                     'logout can reach, what it cannot, and why — which is ' +
                     'the same prose /logout and /admin/logout print, read ' +
                     'off one table in `logout/logout.ts` rather than copied ' +
                     'here.\n\nWith `user` it is that identity: every ' +
                     'browser sign-on session, every relying party, realm ' +
                     'and service provider signed into on one, every token ' +
                     'still revocable, every outstanding authorization and ' +
                     'pre-authorized code, every directory connection bound ' +
                     'as them, and the Kerberos sign-out instant.\n\n**The ' +
                     'rows that CANNOT be ended are in the reply on ' +
                     'purpose** and carry `terminable: false` with a `why`. ' +
                     'A SAML assertion already issued, a Kerberos service ' +
                     'ticket already in a cache and an X509-SVID already ' +
                     'minted cannot be recalled by this service or by a real ' +
                     'one — nothing consults the issuer when they are ' +
                     'presented — and a reply that omitted them would make a ' +
                     'global logout look complete when it is not.\n\nThe ' +
                     'identity is a QUERY PARAMETER and not a path segment ' +
                     'for the reason /admin-api/users gives: the identities ' +
                     'here contain the characters a path is made of.',
        mirrors: 'GET /admin/logout',
        parameters: [
          { name: 'user', in: 'query', required: false,
            schema: { type: 'string' },
            description: 'The identity to look at, as typed. It is ' +
                         'normalised the way every other door here ' +
                         'normalises one, so `alice`, `alice@REALM` and a ' +
                         '`urn:` subject are one answer.' },
        ...
        handler: function (req, res) {
          ...
          self.sendJson(res, 200, adminViews.logoutJson(req).json);
      ...
      { method: 'POST', route: BASE + '/logout/:action', tag: 'Sign-out',
        ...
        handler: function (req, res) {
          ...
          const body = parseBody(req);
          const result = adminActions.logoutAction(self.withAction(req, body));
          if (!result.ok) {
        ...
        actions: [
          { action: 'global', operationId: 'globalLogout',
            ...
            description: 'The default a person gets from `POST /logout` with ' +
                         'an empty body, driven by name. Every browser ' +
                         'sign-on session, every relying party, realm and ' +
                         'service provider on one, every revocable token, ' +
                         'every outstanding code, every directory connection ' +
                         'bound as them, and a Kerberos sign-out instant ' +
                         'after which a TGS-REQ carrying an older ticket is ' +
                         'refused KDC_ERR_TGT_REVOKED (20).\n\n**What it ' +
                         'cannot do is reported rather than skipped.** The ' +
                         'reply\'s `skipped` array names every live thing ' +
                         'that survived and says why.\n\n**What it cannot do ' +
                         'FROM HERE is different and is also in the reply.** ' +
                         'A front-channel logout notification is an iframe ' +
                         'in the signed-out person\'s own browser and a ' +
                         'WS-Federation cleanup is an image in it; neither ' +
              ...
              properties: {
                user: { type: 'string',
                        description: 'The identity to sign out, as typed. ' +
                                     'Normalised the way the GET normalises ' +
                                     'one.' }
              },
              required: ['user'],
              examples: [{ user: 'alice' }],
          ...
          { action: 'end', operationId: 'endLiveSessions',
            ...
            description: 'The selective half. `select` carries row ids from ' +
                         'the GET — `session:<id>`, `token:<jti>`, ' +
                         '`wsfed-rp:<session>|<realm>`, and so on.\n\n**An ' +
                         'empty `select` is REFUSED here and is a global ' +
                         'logout at `POST /logout`**, which is the one place ' +
                         'the two doors differ and is deliberate: an empty ' +
                         'selection arriving at this operation is a caller ' +
                         'that built a list and got nothing, where an empty ' +
                         'body at /logout is a caller asking for everything. ' +
                         'Same absence, opposite intent.\n\nIds are ' +
                         're-resolved against what is live NOW rather than ' +
                         'trusted, so an id that has since been redeemed or ' +
                         'expired ends nothing and is answered in `unknown` ' +
                         'or `skipped` rather than ending something else.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
                user: { type: 'string',
                        description: 'The identity to act on.' },
          ...
          { action: 'restore-token', operationId: 'restoreLoggedOutToken',
            ...
            description: '**No authorization server could offer this.** RFC ' +
                         '7009 defines no such operation and a real ' +
                         'deployment could not have one, because a resource ' +
                         'server may already have cached the refusal. It is ' +
                         'here for the reason `POST ' +
                         '/admin-api/tokens/restore` is — it is the same ' +
                         'function against the same revocation set — and ' +
                         'that reason is that restarting this service to get ' +
                         'back to a working token turns a two-second test ' +
      ...
      { method: 'GET', path: BASE + '/groups', tag: 'Groups',
        ...
        description: 'Without `group` it is the list. With it, one group: ' +
                     'every attribute it holds, operational ones included, ' +
                     'and every member resolved.\n\nTwo answers are easy to ' +
                     'mistake for failures and are neither. A process with ' +
                     'no directory loaded answers 200 with `directory: ' +
                     'false`. A group that is not there answers 200 with ' +
                     '`found: false`, because a client can delete or rename ' +
                     'one through the protocol between one call and the ' +
                     'next, and that is the interesting case rather than a ' +
                     'routing problem.\n\nA GROUP HERE GRANTS NOTHING, with ' +
                     'two exceptions: no endpoint in this service decides ' +
                     'anything on a group, and the only two that do are ' +
                     '`admin.readGroup` and `admin.writeGroup`, which say ' +
                     'who may use the console. A token CAN carry one — ' +
                     '`groups.claim` is on by default and puts the ' +
                     'subject\'s groups in every access ' +
                     'token, ID Token and SAML assertion ' +
        ...
        parameters: [
          ...
          { name: 'members',
            description: 'The membership values of the group named by ' +
                         '`group`, resolved. `group.memberCount`, ' +
                         '`presentCount` and `danglingCount` beside them are ' +
                         'counts of the WHOLE list and not of the page — a ' +
                         'group whose seven members resolve to five is the ' +
                         'fact this resource exists to report, and a ' +
                         'per-page count would not be an answer to it.' },
          { name: 'claimed',
            description: 'Entries whose own memberOf names this group while ' +
                         'it does not list them back.' }
        ])),
        ...
        handler: function (req, res) {
          ...
          self.sendJson(res, 200, adminViews.groupsJson(req));
          log.debug("Leaving the management API groups endpoint.");
        } },

      // -----------------------------------------------------------------------
      // AND THE TWO WRITES (2026-09-06), WHICH CLOSED A HOLE RATHER THAN ADDING
      // A FEATURE.
      //
      // Until today `/admin-api/groups` was a READ and so was `/admin/groups`,
      // and the only two doors onto a group in this directory were an `ldapadd`
      // on the raw socket and `POST /scim/v2/Groups`. So this API could put a
      // PERSON in the directory (`/users/create`) and could not put them in a
      // GROUP, and the console could report a dangling member, a claimed
      // membership and the two groups that decide who may use it without being
      // able to create any of them.
      //
      // **RULE 7 COULD NOT HAVE CAUGHT IT AND THAT IS THE INTERESTING PART.**
      // That rule is a parity check between the console and this API — every
      // control there has an operation here, every operation here names a
      // control there — and it is satisfied exactly when both are missing. It
      // reports drift, not absence. What found this was a load test that had to
      // reach for SCIM to make fifty groups on a service whose own management
      // API creates users five thousand at a time.
      //
      // THE ACTION SWITCH IS IN `adminActions.groupsAction()` and not here,
      // exactly as the users one is: two doors onto one action must not be two
      // readings of what was sent.
      // -----------------------------------------------------------------------
      { method: 'POST', route: BASE + '/groups/:action', tag: 'Groups',
        ...
        actions: [
          { action: 'create', operationId: 'createGroup',
            ...
            description: 'An entry under `ou=groups`, as a `groupOfNames` — ' +
                         'so it is counted as a group by BOTH of the rules ' +
                         '/admin/groups applies, its placement and its ' +
                         'object class, and stays one if a client moves ' +
                         'it.\n\n**The name becomes the `cn` AND the RDN**, ' +
                         'so it is refused if it carries ' +
                         'a character RFC 4514 section 2.4 ' +
                         'reserves in a DN (one of `, = + < > # ; " \\`) — ' +
                         'the ' +
                         'same rule a username is refused by, and refused ' +
                         'rather than escaped for the same reason: an ' +
                         '`ldapadd` can still create such an entry with the ' +
                         'escaping written out. A DN sent here is refused ' +
                         'too, because what it would create is a group whose ' +
                         'name is another group\'s DN.\n\n**A member that ' +
                         'names nothing is WRITTEN, not refused.** This ' +
                         'directory does no referential integrity in either ' +
                         'direction — deleting a person leaves their DN in ' +
                         'every group that listed them — so a create that ' +
                         'refused a dangling member would make the state ' +
                         '/admin/groups exists to report impossible to ' +
                         ...
                         'management API stricter than SCIM about the same ' +
                         'store would be two doors disagreeing about what ' +
                         'this directory holds.\n\n**IT GRANTS NOTHING.** No ' +
                         'endpoint here decides anything on a group. The two ' +
                         'that do are `admin.readGroup` and ' +
                         '`admin.writeGroup`, and they are granted at POST ' +
                         '/admin-api/rbac rather than by creating a group ' +
                         'with the right name — though creating one with the ' +
                         'right name and adding somebody to it does the ' +
                         'same thing, because those two ARE ordinary groups ' +
                         'in this directory.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
                group: { type: 'string',
                         description: 'The `cn`. Not a DN — this puts it ' +
                                      'under `ou=groups` in the realm the ' +
                                      'call is made in.' },
                displayName: { type: 'string',
                               description: 'Accepted for `group`, spelt the ' +
                                            'way SCIM spells it.' },
                note: { type: 'string',
                        description: 'What the entry\'s `description` says ' +
                                     'about why it exists. Defaults to a ' +
                                     'sentence saying it was created by hand ' +
                                     'rather than by a directory client.' },
                members: {
                  description: 'Who is in it, as an array of user names or ' +
                               'DNs — a group can hold another group, and no ' +
                               'user name names one. A form sends one ' +
                               'string, split on newlines and commas. A bare ' +
                               'name resolves to that person\'s OWN entry ' +
                               'wherever it is, because somebody seeded by a ' +
                               'client certificate is at ' +
                               '`cn=<name>,ou=users` and a value written in ' +
                               'the `uid=` form would dangle beside the ' +
                               'entry it meant to name.',
          ...
          { action: 'add-member', operationId: 'addGroupMember',
            ...
            description: 'One membership value onto one group. `group` is ' +
                         'its `cn` or its whole DN; `member` is a user name ' +
                         'or any DN.\n\n**IT IS IDEMPOTENT.** Adding ' +
                         'somebody already listed answers `ok: true` with ' +
                         '`changed: false` rather than an error, so a script ' +
                         'that adds on every run does not fail on its second ' +
                         'one. Membership is asked across `member`, ' +
                         '`uniqueMember` and `memberUid` together, which is ' +
                         'how /admin/groups and the groups claim ask it — an ' +
                         'add that could not see a `memberUid` would write a ' +
                         'second value for one membership.\n\n**It writes ' +
                         'onto `member`** whatever else the entry carries, ' +
                         'rather than extending whichever convention the ' +
                         'group already uses: this service\'s groups claim, ' +
                         'the console and RFC 4519 all read `member` first, ' +
                         'and guessing which of three attributes was meant ' +
                         'would be this operation deciding something the ' +
                         'caller did not say.\n\n**It does not create the ' +
                         'group as a side effect.** A typo in the name would ' +
                         'then be a new group rather than an error. And it ' +
                         'writes nothing onto the PERSON: `memberOf` is ' +
                         'maintained by nothing here — it is not even a ' +
                         'standard attribute — and a value written there is ' +
                         'one no other door in this service can take ' +
            ...
            requestBody: {
              ...
              properties: {
                ...
                username: { type: 'string',
                            description: 'Accepted for `member`.' }
              },
              required: ['group', 'member'],
              examples: [{ group: 'developers', member: 'alice' }],
              additionalProperties: false
            },
            responseDescription: 'Whether anything changed, the value ' +
                                 'written, and whether it resolves to an ' +
                                 'entry this directory holds.' }
        ] },

      // --- The directory itself, entry by entry
      // --------------------------------
      //
      // FIVE OPERATIONS ADDED ON 2026-09-01, ONE PER PAGE, AND THEY EXIST
      // BECAUSE OF RULE 7 RATHER THAN BECAUSE ANYBODY ASKED FOR THEM.
      //
      // `/ldap`, `/ldap/directory`, `/ldap/applications`, `/ldap/federations`
      // and `/ldap/spiffe` were five HTML pages outside the console until that
      // day. They are `/admin/ldap/*` now — console pages, in the console's
      // shell, behind its gate — and the rule this API is written under is that
      // every page of that console has an operation here that mirrors it. So
      // here they are, and the parity check in the suite is what would have
      // noticed if they were not.
      //
      // THE GATE IS THE POINT OF THEM AND NOT AN INCIDENTAL DIFFERENCE. Those
      // pages print `oauthClientSecret` and `fedClientSecret` in the clear,
      // which is why moving them behind the console's gate was the right half
      // of the change; this API takes an access token rather than a browser
      // session, which is what keeps a test able to read the directory without
      // signing a browser in. (It said "deliberately NOT gated" until
      // 2026-09-09, when this API began requiring a token — see the top of this
      // file.)
      //
      ...
      { method: 'GET', path: BASE + '/ldap/directory', tag: 'LDAP',
        ...
        description: 'The whole store, DN by DN, with where each entry came ' +
                     'from — `seed`, an LDAP `add`, or an authentication — ' +
                     'and every attribute with every value.\n\nIT IS NOT AN ' +
                     'LDAP SEARCH. This is the service showing its own ' +
                     'store, which is how a caller tells an empty directory ' +
                     'from a filter that matched nothing, and it is why the ' +
                     'operational attributes are here: a search withholds ' +
                     '`createTimestamp` and `modifyTimestamp` unless they ' +
                     'are asked for by name (RFC 4511 §4.5.1.8) and this is ' +
                     'not a search.\n\n`q` matches the DN, any attribute ' +
                     'NAME and any attribute VALUE, case-insensitively — ' +
                     'values because the caller who needs this most often ' +
                     'has a thumbprint or a secret in hand and no idea which ' +
                     'entry carries it.\n\nTHE REPLY IS THIS REALM\'S ' +
                     'DIRECTORY AND NO OTHER. Since 2026-08-25 each trust ' +
                     'realm has a subtree of its own; reach another ' +
                     'realm\'s through its own path prefix.',
        mirrors: 'GET /admin/ldap/directory',
        parameters: [
          { name: 'q', in: 'query', required: false, schema: { type: 'string' },
            description: 'Substring of the DN, of an attribute name, or of ' +
                         'an attribute value. Case-insensitive.' },
          { name: 'origin', in: 'query', required: false,
            schema: { type: 'string' },
            description: 'Only entries that came from here. The values ' +
                         'actually present are in `origins`.' }
        ].concat(this.pagingParameters()),
        responseDescription:
          'The page of entries, with the filter and the paging.',
        responseSchema: { $ref: '#/components/schemas/DirectoryEntryList' },
        handler: function (req, res) {
          log.debug("Entering the management API directory entries endpoint.");
          self.sendJson(res, 200,
                        adminViews.directoryPageJson('directory', req));
      ...
      { method: 'GET', path: BASE + '/ldap/applications', tag: 'LDAP',
        ...
        summary: 'The application registry as the directory holds it, and ' +
                 'its schema',
        description: 'One entry per identifier under `ou=applications`, ' +
                     ...
                     '/admin-api/applications`. That operation is the ' +
                     'registry as the console works with it: the counters, ' +
                     'the drill-down, the writes. This is the registry as ' +
                     'the DIRECTORY holds it, and the vocabulary is the half ' +
                     'a client reading an entry back over 389 actually needs ' +
                     '— this directory is schemaless, so an entry carrying ' +
                     'thirty invented attribute names is otherwise ' +
                     'guesswork.\n\nTHESE ENTRIES ARE THE REGISTRY rather ' +
                     'than a copy of one. Nothing caches them, so an ' +
                     '`ldapmodify` of `oauthRedirectUri` changes which ' +
                     'redirect URI RFC 9700 mode accepts on the next ' +
                     'request.\n\nTwo attributes hold CREDENTIALS in the ' +
                     'clear, for the reason `/krb5/principals` ' +
                     'prints the Kerberos passwords. They are ' +
                     'never written to the audit log.',
        mirrors: 'GET /admin/ldap/applications',
        parameters: [
          { name: 'q', in: 'query', required: false, schema: { type: 'string' },
            description: 'Substring of the identifier, the name, the DN or ' +
                         'any attribute value. Case-insensitive.' }
        ].concat(this.pagingParameters()),
        responseDescription: 'The page of entries, the kinds and the schema.',
        responseSchema:
          { $ref: '#/components/schemas/DirectoryApplicationList' },
        handler: function (req, res) {
          log.debug("Entering the management API directory applications " +
                    "endpoint.");
          self.sendJson(res, 200,
                        adminViews.directoryPageJson('applications', req));
          log.debug("Leaving the management API directory applications " +
                    "endpoint.");
        } },
      ...
      { method: 'GET', path: BASE + '/ldap/federations', tag: 'LDAP',
        ...
        description: 'The application registry\'s twin, for `ou=federations` ' +
                     ...
                     'LDAPMODIFY IS A SECURITY CHANGE. Everywhere else an ' +
                     'edit changes what this service HANDS OUT; ' +
                     '`fedSigningCertificate` decides whose assertions it ' +
                     'will BELIEVE and `fedEnabled` turns a partner ' +
                     'on.\n\nIt is a container of its own rather than a ' +
                     'corner of `ou=applications` because half its entries ' +
                     'are FOREIGN IDENTITY PROVIDERS, which ask this service ' +
                     'for nothing at all.\n\nThe schema carries a column the ' +
                     'applications one has no need of: which DIRECTION each ' +
                     'attribute is for.\n\n`fedClientSecret` is REDACTED in ' +
                     '`relationships` and present in the entry\'s own ' +
                     'attributes, which is the same split the page makes: ' +
                     'what a script reads is redacted, and a ' +
                     'page claiming to say what the directory ' +
                     'holds may not hide a value an `ldapsearch` shows.',
        mirrors: 'GET /admin/ldap/federations',
        parameters: [
          { name: 'q', in: 'query', required: false, schema: { type: 'string' },
            description: 'Substring of the relationship id, the DN, the ' +
                         'protocol or the direction. Case-insensitive.' }
        ].concat(this.pagingParameters()),
        responseDescription: 'The page of relationships, the roles, the ' +
                             'protocols and the schema.',
        responseSchema:
          { $ref: '#/components/schemas/DirectoryFederationList' },
        handler: function (req, res) {
          log.debug("Entering the management API directory federations " +
                    "endpoint.");
          self.sendJson(res, 200,
                        adminViews.directoryPageJson('federations', req));
          log.debug("Leaving the management API directory federations " +
                    "endpoint.");
        } },
      ...
      { method: 'GET', path: BASE + '/ldap/spiffe', tag: 'LDAP',
        ...
        summary: 'The two SPIFFE containers as the directory holds them, and ' +
                 'their schema',
        description: 'THE TWO CONTAINERS HOLD DIFFERENT KINDS OF THING, ' +
                     'which is why they are two. `ou=entries` is ' +
                     'CONFIGURATION — which SPIFFE ID a workload gets, under ' +
                     'which parent, matching which selectors — and ' +
                     '`ou=agents` is a RECORD of what has attested, which is ' +
                     'why nothing about an agent is editable ' +
                     'anywhere.\n\nTHE ENTRIES ARE THE REGISTRY: nothing ' +
                     'caches them, so an `ldapmodify` of `spiffeX509SvidTtl` ' +
                     'changes the lifetime of the next SVID the Workload API ' +
                     'hands out.\n\nTHIS IS THE ONE DIRECTORY OPERATION WITH ' +
                     'TWO LISTS IN IT, so it pages the way the console\'s ' +
                     'drill-downs do: `entriesPage` and `agentsPage` move ' +
                     'one list each and `per` is shared, with an ' +
                     '`entriesPaging` and an `agentsPaging` ' +
                     'object in the reply. `entries` and `agents` ' +
        ...
        parameters: [
          { name: 'entryq', in: 'query', required: false,
            schema: { type: 'string' },
            description:
              'Substring of a registration entry\'s SPIFFE ID or DN.' },
      ...
      // by analogy: `ou=roles`, `ou=policies` and `ou=peps` each gained a
      // console page that day, and every page of the console has an operation.
      // What made the pages worth writing is that all three modules already
      // PUBLISHED a schema whose comment claimed a page under `/admin/ldap/*`
      // and none of them had one — `common/roles.js` from that afternoon,
      // `xacml/xacml_store.ts` from XACML phase two,
      // `xacml/xacml_pep_registry.ts` from phase five.
      // ---------------------------------------------------------------------
      { method: 'GET', path: BASE + '/ldap/roles', tag: 'LDAP',
        operationId: 'getDirectoryRoles',
        summary:
          'The role entries as the directory holds them, and their schema',
        description: 'THIS CONTAINER IS HALF THE FEATURE. A role has two ' +
                     'relations and they are stored apart on purpose: ' +
                     'MEMBERSHIP — who holds it — is here on the role entry, ' +
                     'and the REQUIREMENT — which roles an application ' +
                     'demands before anything is issued for it — is ' +
                     '`appRequiredRole` on the APPLICATION entry, under a ' +
                     'different container. So nothing in this reply refuses ' +
                     'anybody by itself, and a caller looking for the reason ' +
                     'somebody was turned away wants `GET /admin-api/roles`, ' +
                     'which resolves both halves.\n\n**THE SIX BUILT-IN ' +
                     'ROLES ARE IN NO CONTAINER.** They are computed from ' +
                     'the context of the decision being made, so `EVERYBODY` ' +
                     'has no entry here and never will — which is why an ' +
                     'empty `roles` array is the ORDINARY state of a service ' +
                     'refusing nobody rather than a sign that the feature is ' +
                     'not loaded.\n\nTHE ENTRIES ARE THE REGISTER: nothing ' +
                     'caches them, so an `ldapmodify` adding a value to ' +
                     '`roleMemberUser` is answered by the very next issuance ' +
                     'decision.',
        mirrors: 'GET /admin/ldap/roles',
        parameters: [
          { name: 'q', in: 'query', required: false, schema: { type: 'string' },
            description: 'Substring of a role name, a DN, or any value on ' +
                         'the entry — which includes its members, so this is ' +
                         'how to find the roles one person holds by name.' }
        ].concat(this.pagingParameters()),
        responseDescription: 'The page of role entries, the six built-in ' +
        ...
        handler: function (req, res) {
          ...
          self.sendJson(res, 200, adminViews.directoryPageJson('roles', req));
      ...
      { method: 'GET', path: BASE + '/ldap/policies', tag: 'LDAP',
        ...
        summary:
          'The policy repository as the directory holds it, and its schema',
        description: '`ou=policies` IS the XACML policy repository, the way ' +
                     '`ou=federations` is the federation register. One entry ' +
                     'per policy or policy set, holding the document itself, ' +
                     'with exactly one of them the root — a PDP evaluates ' +
                     'one document and reaches the rest through ' +
                     'PolicyIdReference.\n\n**A WRITE HERE SKIPS THE ' +
                     'TYPECHECKER, and that is the one thing this operation ' +
                     'says that `GET /admin-api/xacml/policies` does not.** ' +
                     'Every write through the console and through ' +
                     '/admin-api/xacml parses the document and statically ' +
                     'typechecks it, so a policy that does not typecheck is ' +
                     'refused at WRITE time rather than going Indeterminate ' +
                     'on every request. An `ldapmodify` of ' +
                     '`xacmlPolicyDocument` reaches the entry directly and ' +
                     'skips all of it, and nothing caches these entries.',
        mirrors: 'GET /admin/ldap/policies',
        parameters: [
          { name: 'q', in: 'query', required: false, schema: { type: 'string' },
            description: 'Substring of a policy name, a DN, or any value on ' +
                         'the entry — the DOCUMENT included, so this finds ' +
                         'the policy that names a particular resource.' }
        ].concat(this.pagingParameters()),
        ...
        handler: function (req, res) {
          ...
          self.sendJson(res, 200,
                        adminViews.directoryPageJson('policies', req));
      ...
      { method: 'GET', path: BASE + '/ldap/peps', tag: 'LDAP',
        ...
        summary: 'The registered remote PEPs as the directory holds them, ' +
                 'and their schema',
        description: 'ALMOST EVERYTHING IN THIS CONTAINER IS A RECORD rather ' +
                     'than configuration, which is what it has in common ' +
                     'with `ou=agents` next door. A PEP registers itself, ' +
                     'and its identity is taken from the CLIENT CERTIFICATE ' +
                     'it presented and never from the body it sent.\n\nTwo ' +
                     'attributes are not a record and an `ldapmodify` of ' +
                     'either is a real change: `xacmlPepEnabled` is an ' +
                     'administrator\'s decision and a PEP that reconnects ' +
                     'does not clear it, and `xacmlPepNotifyUrl` is one of ' +
                     'the three addresses this service will dial.\n\n**AN ' +
                     'EMPTY `peps` ARRAY IS NOT A FEATURE THAT IS OFF.** A ' +
                     'remote PEP pulls `GET /xacml/pep/policies` and ' +
                     'converges without registering at all; registering is ' +
                     'what buys it the change nudge and a row on the console.',
        mirrors: 'GET /admin/ldap/peps',
        parameters: [
          { name: 'q', in: 'query', required: false, schema: { type: 'string' },
            description: 'Substring of a PEP name, a DN, or any value on the ' +
                         'entry — a certificate subject or a notify URL ' +
                         'included.' }
        ].concat(this.pagingParameters()),
      ...
      // and the base DN are SET to, and this one says what actually happened
      // when the process tried to bind them. On a host where the system's own
      // slapd already holds 389 those two replies disagree, and the
      // disagreement is the whole value of having both.
      { method: 'GET', path: BASE + '/ldap/service', tag: 'LDAP',
        ...
        description: 'The two raw sockets and the store behind them AS THEY ' +
                     'ACTUALLY ARE: whether TCP 389 and LDAPS 636 really ' +
                     'bound and the error if either did not, the base DN and ' +
                     'each realm\'s naming context, what a subtree search ' +
                     'from each one answers about, the bind policy, the four ' +
                     'structural rules this directory does still enforce, ' +
                     'the entry count and the persistence status.\n\nWHY IT ' +
                     'IS NOT `GET /admin-api/ldap`: that operation is the ' +
                     'six `ldap.*` SETTINGS — what the sockets are ' +
                     'configured to be. This one is what happened. A host ' +
                     'whose own slapd already holds 389 makes the two ' +
                     'disagree, and nothing else in this service can report ' +
                     'that: `/admin/sts-metadata` is built by walking the ' +
                     'express router and a raw TCP listener is not on ' +
                     'it.\n\nNO BIND IS EVER REFUSED here, by any setting, ' +
                     'except the one literal password ' +
                     'named in `refusedPassword` ' +
                     '— which exists so a negative test has something to ' +
                     'fail on.',
        ...
        handler: function (req, res) {
          ...
          self.sendJson(res, 200, adminViews.directoryPageJson('service', req));
      ...
      // ---------------------------------------------
      //
      // TWO OPERATIONS, AND THIS RESOURCE MATTERS MORE THAN THE OTHERS RATHER
      ...
      // ANYBODY WHO CAN REACH THIS PORT CAN GRANT THEMSELVES BOTH ROLES — which
      // is exactly what `adminApi.authRequired` closed. What has NOT changed is
      // that the console's gate exists so a client can be driven through 302 /
      // 401 / 403 and a role model, not to make this service safe to expose: in
      // the default development mode no password is checked anywhere and
      // `/oauth2/token` will mint a token for any username asked of it. Do not
      // put this port on a public address.
      { method: 'GET', path: BASE + '/rbac', tag: 'Admin roles',
        ...
        description: 'The two roles — **Admin Read** and **Admin Write** — ' +
                     'with every grant, the settings behind the gate, and ' +
                     'who the CALLER is (`you`).\n\nThe roles are two ' +
                     'ORDINARY GROUPS in the embedded LDAP directory ' +
                     '(`admin.readGroup`, `admin.writeGroup`), not a store ' +
                     'of the console\'s own. So this resource, ' +
                     '`/admin/rbac`, an `ldapmodify` on 389 or 636 and a ' +
                     'SCIM PATCH of the group are four doors onto one ' +
                     ...
                     'of.\n\n`enforced` reports whether the roster decides ' +
                     'anything. It is always true now — the console gate ' +
                     'became unconditional on 2026-09-06 — and the field is ' +
                     'kept because a client reading it should not have to ' +
                     'know that.\n\n**EVERY LIST IN THE REPLY IS PAGED.** ' +
                     '`grants` by `page` and `per`, with `page`, `pages`, ' +
                     '`perPage`, `firstRow`, `lastRow` and `matched` at the ' +
                     'top level; `candidates` by `candidatesPage` and the ' +
                     'same `per`, answered in `candidatesPaging`. `roles` ' +
                     'carries each role\'s counts (`memberCount`, ' +
                     '`presentCount`, `danglingCount`, `claimedCount`) and, ' +
                     'since 2026-09-13, not its `members` and `claimed` ' +
                     'lists: those were every membership unpaged, ' +
                     'and they are the rows `grants` pages ' +
        ...
        parameters: [
          ...
          { name: 'candidatesPage', in: 'query', required: false,
            schema: { type: 'integer', minimum: 1 },
            description: 'Which page of the matching `candidates`, answered ' +
                         'in `candidatesPaging`. Clamped like `page`. The ' +
                         'page size is `per` when given — shared with ' +
                         '`grants`, as on every reply here carrying two ' +
                         'lists — and otherwise twenty, the size of the ' +
                         'console\'s results pane. `candidates` was the ' +
                         'whole list until 2026-09-13, which on a ' +
                         'directory of thousands was thousands of ' +
                         'rows on every read of the roster.' },
          { name: 'personfrom', in: 'query', required: false,
            schema: { type: 'integer', minimum: 0 },
            description: 'The console results pane\'s OFFSET into the ' +
                         'matching candidates, answered as the page it falls ' +
                         'on. Ignored when `candidatesPage` is given, and ' +
                         'when it is past the end.' },
          { name: 'person', in: 'query', required: false,
            schema: { type: 'string' },
            description: 'A username to look up among ALL the candidates, ' +
                         'not only the page shown. ' +
                         '`picked.candidate` is its row, ' +
        ...
        handler: function (req, res) {
          ...
          self.sendJson(res, 200, adminViews.rbacListJson(req).json);
      ...
      { method: 'POST', route: BASE + '/rbac/:action', tag: 'Admin roles',
        ...
        actions: [
          { action: 'grant', operationId: 'grantAdminRole',
            ...
            description: 'Adds them to the role\'s group as a `member`, ' +
                         'creating the group if this is ' +
                         'the first grant — which is why ' +
                         ...
                         'entry, is granted anyway: the membership names the ' +
                         'DN they WILL be at (`uid=<name>,ou=users`) and ' +
                         'dangles until they first sign in or somebody ' +
                         'creates them, and the role counts from that ' +
                         'moment. That is the interesting case for a mock ' +
                         'and is deliberately reachable. What IS refused is ' +
                         'a name carrying a character RFC 4514 reserves in a ' +
                         'DN — the same refusal `POST ' +
                         '/admin-api/users/create` gives, for the same ' +
                         'reason.\n\nGranting a role somebody already holds ' +
                         'answers 200 with `changed: false` rather than 400, ' +
                         'so a script that grants on every run does not fail ' +
                         'on its second one.\n\n**THE FIRST GRANT ON A ' +
                         'SERVICE CLOSES THE DOOR BEHIND IT.** While the ' +
                         'roster is empty anybody who signs in holds both ' +
                         'roles; the moment one grant exists, everybody not ' +
                         'in one of the two groups is refused at every page ' +
                         'of the console. Grant yourself first.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
                username: { type: 'string',
                            description: 'The name they sign in as — the ' +
                                         'same string that ' +
                                         'appears in a token\'s ' +
            ...
            responseDescription: 'What was granted, and where the membership ' +
                                 'was written. `changed: false` means they ' +
                                 'already held it.' },
          ...
          { action: 'revoke', operationId: 'revokeAdminRole',
            ...
            description: 'Removes them from the role\'s group — from EVERY ' +
                         ...
                         'first one found, because a person listed twice by ' +
                         'two clients would otherwise still hold the role ' +
                         'after a revoke that reported success.\n\nRevoking ' +
                         'a role somebody does not hold answers 200 with ' +
                         '`changed: false`, and so does revoking from a ' +
            ...
            requestBody: {
              ...
              properties: {
                username: { type: 'string',
                            description: 'Who to take it from.' },
                role: { type: 'string', enum: rbac.ROLE_IDS }
              },
              required: ['username', 'role'],
            ...
            responseDescription: 'What was removed, and whether the roster ' +
                                 'is now empty.' }
        ] },
      ...
      //
      // Rule 7 says a console control owes an operation. It says nothing about
      // an operation whose page moved, and deleting a working one to tidy a
      // table would be a regression dressed as consistency — the same argument
      // `GET /admin-api/users/new` is kept on. So this stays, `mirrors` points
      // at the page that absorbed it, and `adminViews.mfaRosterJson()` answers
      // OUT OF THAT VIEW so there is one tally rather than two scans that agree
      // until they do not.
      //
      // **THE READ IS A REPORT AND THE WRITE IS A RESET**, and there is
      // deliberately no ENROL operation here. Enrolling means being shown a
      // shared secret, and a management API that handed one out would be an
      // administrative door that mints a working second factor for any account
      // — which is not a second factor at all. Enrolment happens where the
      // person is: `/portal/mfa`, or `/portal/activate` with a link that is
      // itself a credential.
      // ---------------------------------------------------------------------
      { method: 'GET', path: BASE + '/mfa', tag: 'Multi-factor authentication',
        operationId: 'getMultiFactor',
        summary:
          'Who holds a second factor, and how RFC 6238 is configured here',
        description: 'One row per person — this realm\'s directory people ' +
                     'and everybody this service has SEEN, unioned — saying ' +
                     'whether they hold a password, how many security keys ' +
                     'are marked as a SECOND factor, and whether an ' +
                     'authenticator app is enrolled.\n\n**TWO MECHANISMS ' +
                     'COUNT AS A SECOND FACTOR HERE**: a WebAuthn credential ' +
                     'enrolled in the `mfa` role, and an RFC 6238 ' +
                     'authenticator app. A person holding either is asked ' +
                     'for it at the sign-in screen and a password alone will ' +
                     'not sign them in; `secondFactor` says which one they ' +
                     'will be asked for, and somebody holding both is asked ' +
                     'for the key with the code offered as the ' +
                     'alternative.\n\n**`totpUsable: false` IS THE ROW TO ' +
                     'LOOK FOR.** It means an enrolment exists that this ' +
                     'process cannot read — almost always a shared secret ' +
                     'sealed under a key-encryption key that has since been ' +
                     'rotated. Those people are REFUSED at the code step ' +
                     'rather than let through on one factor, so they cannot ' +
                     'sign in at all until the enrolment is ' +
                     'cleared.\n\n**CODES ARE VERIFIED FOR REAL IN BOTH ' +
                     'MODES**, which almost nothing else in this service is. ' +
                     '`totp` carries the algorithm table, read from the ' +
                     'module that performs the algorithm.\n\nThe scan stops ' +
                     'at 5,000 people (`capped`), because reading a ' +
                     'credential per person happens on the one thread that ' +
        ...
        handler: function (req, res) {
          ...
          self.sendJson(res, 200, adminViews.mfaRosterJson(req));
      ...
      { method: 'POST', route: BASE + '/mfa/:action',
        ...
        handler: function (req, res) {
          log.debug("Entering the management API multi-factor action " +
                    "endpoint.");
          const body = parseBody(req);
        ...
        actions: [
          { action: 'clear-totp', operationId: 'clearAuthenticatorApp',
            ...
            description: '**THE ONLY WAY BACK FOR SOMEBODY WHO HAS LOST ' +
                         ...
                         'rather than to none. The person sets a new one up ' +
                         'at `/portal/mfa`.\n\nClearing an enrolment nobody ' +
                         'holds answers 400 rather than 200, because unlike ' +
                         'a role grant there is no idempotent reading of it ' +
                         'that is useful: the caller asked to clear a ' +
                         'specific thing and it was not there.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
                ...
                // ways — and the users resource names the person `user`, as
                // every other operation on it does. A schema that took one
                // spelling here and the other there would refuse a caller that
                // had merely followed the other path's example, with a message
                // about a member of the request rather than about the person.
                user: { type: 'string',
          ...
          { action: 'clear-key', operationId: 'clearSecurityKey',
            ...
            description: 'Goes through the same `removeKey()` the person\'s ' +
                         ...
                         'operator\'s call, and an operator must not be able ' +
                         'to do what the owner is stopped from doing.\n\nThe ' +
                         'credential id is the base64url one on `GET ' +
                         '/admin-api/mfa`\'s rows and on `/portal/keys`.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
                username: { type: 'string' },
                // See `clear-totp` above: one switch, two paths, one body
                // shape.
                user: { type: 'string',
                        description: 'Accepted for `username`.' },
                credentialId: { type: 'string',
      ...
      { method: 'GET', path: BASE + '/tokens', tag: 'Tokens',
        ...
        description: 'One list, newest first, filtered and paged. Claims and ' +
                     'facts only — the signed token, the assertion XML and ' +
                     'the ticket are never kept, and the `jti` is all any ' +
                     'operation here needs.\n\n**AN ENTRY IS ONE ISSUANCE ' +
                     'AND NOT ONE CREDENTIAL, since 2026-09-05.** OAuth 2.0 ' +
                     'and OIDC are the only families here that hand back ' +
                     'several credentials at once — an access token, a ' +
                     'refresh token and an ID Token out of one code ' +
                     'redemption — so those arrive as one entry in `sets` ' +
                     'carrying its `members`. Every other family issues one ' +
                     'credential per act, so a SAML assertion, a Kerberos ' +
                     'ticket and an SVID are each a set of one.\n\n`issued` ' +
                     'is the same credentials flattened out of `sets`, so a ' +
                     'caller written against the older per-credential shape ' +
                     'reads exactly what it read. What changed under it is ' +
                     'the paging: a page is a whole number of replies, so ' +
                     '`page`, `pages`, `matched` and `shown` count SETS ' +
                     'while `held` and `matchedCredentials` count ' +
                     'credentials.\n\nOID4VCI credentials are NOT in this ' +
                     'list. They are counted on /admin-api/metrics ' +
                     'and listed nowhere, which is a gap ' +
                     'rather than a principle and is said here so ' +
                     '"everything ' +
        ...
        parameters: [
          ...
          { name: 'kind', in: 'query', required: false,
            ...
            description: 'One kind. ANDed with `family`, so a kind from ' +
                         ...
                         'MATCHES A SET WHEN ANY MEMBER MATCHES.** Asking ' +
                         'for `id_token` answers with the replies that ' +
                         'CONTAIN one — the access token and the refresh ' +
                         'token that came back with it are still in ' +
                         '`members`, because they are part of the same ' +
                         'reply. A caller that wants only the matching ' +
                         'credentials filters `issued` itself; a filter that ' +
                         'hid the neighbours would be the old per-credential ' +
                         'list wearing this one\'s name.' },
          { name: 'state', in: 'query', required: false,
            ...
            description: 'One state, matched against any member — so a set ' +
                         'holding an expired access token and a valid ' +
                         'refresh token is found by both, and its own ' +
                         '`state` reads `mixed` rather than picking one.' },
          { name: 'session', in: 'query', required: false,
            schema: { type: 'string' },
            description: 'Only what was issued UNDER one browser sign-on ' +
                         'session, matched exactly on the id — the ' +
                         '`sessionId` every row of GET /admin-api/sessions ' +
                         'carries, and the join between the two ' +
                         'resources.\n\nOnly a credential issued under a ' +
                         'session records one, so an assertion, a Kerberos ' +
                         'ticket, a token from either direct grant and ' +
                         'anything an RFC 8693 exchange produced are absent ' +
                         'by construction rather than ' +
                         'missing. That is a fact about ' +
                         'the credential and not a gap in the recording.' }
        ].concat(this.pagingParameters()),
        responseDescription:
          'The matching sets, with the paging that found them ' +
        ...
        handler: function (req, res) {
          ...
          self.sendJson(res, 200, adminViews.tokensView(req.query).json);
      ...
      { method: 'GET', path: BASE + '/tokens/set', tag: 'Tokens',
        ...
        description: 'What `GET /admin-api/tokens` groups, opened up. Every ' +
                     'entry in that reply\'s `sets` carries the `setKey` ' +
                     'this takes, so a caller walks the list and opens one ' +
                     'entry without having to know whether it holds three ' +
                     'credentials or one.\n\nIt is addressed by `setKey` and ' +
                     'never by the issuer\'s `setId`: a set of one has no ' +
                     'issuance id at all, and its key is `one:<row handle>` ' +
                     'so that the key space covers every row of that table. ' +
                     'A key nothing holds answers 200 with `found: false` ' +
                     'rather than 404 — a set dropped to the registry\'s cap ' +
                     'is the ordinary end of a set\'s life and ' +
                     'not a caller\'s mistake, and `why` says ' +
        ...
        responseDescription: 'The set and its members, or `found: false` and ' +
                             'the reason.',
        responseSchema: { $ref: '#/components/schemas/IssuedSetDetail' },
        handler: function (req, res) {
          log.debug("Entering the management API issued-set endpoint.");
          self.sendJson(res, 200, adminViews.tokenSetView(req.query).json);
      ...
      { method: 'POST', route: BASE + '/tokens/:action', tag: 'Tokens',
        ...
        handler: function (req, res) {
          ...
          const body = parseBody(req);
          const result = adminActions.tokenAction(self.withAction(req, body));
          if (!result.ok) {
        ...
        actions: [
          { action: 'revoke', operationId: 'revokeToken',
            ...
            description: 'By `jti`, or by pasting the whole token — what ' +
                         ...
                         'invalidated is the token, not its identifier. A ' +
                         'pasted token\'s SIGNATURE IS NOT VERIFIED and that ' +
                         'is safe rather than sloppy: the only thing read ' +
                         'out of it is the `jti`, which is then looked up in ' +
                         'this service\'s own registry, so a forged token ' +
                         'yields an identifier this service never issued and ' +
                         'revoking one of those invalidates nothing.\n\nThis ' +
                         'is the SAME revocation set RFC 7009\'s ' +
                         '/oauth2/revoke writes to, so the token immediately ' +
                         'introspects as inactive, is refused by UserInfo ' +
                         'with `invalid_token`, and fails the refresh ' +
            ...
            responseDescription: 'Revoked, or already revoked — both are ' +
                                 '`ok`, and the message says which.' },

          { action: 'restore', operationId: 'restoreToken',
            summary: 'Un-revoke one token (NON-SPEC)',
            description: 'NON-SPEC, and named as such wherever it appears: ' +
                         'no real authorization server can undo a ' +
                         'revocation. It is here because ' +
                         'otherwise getting back to a working token ' +
          ...
          { action: 'revoke-artifact', operationId: 'revokeIssuedArtifact',
            ...
            description: '**THIS CHANGES NOTHING OUTSIDE THIS SERVICE AND ' +
                         'THAT IS NOT A DEFECT.** A relying party validates ' +
                         'a SAML assertion\'s signature and its Conditions ' +
                         'and asks nobody; a Kerberos service decrypts a ' +
                         'ticket with a key it already holds; an X509-SVID ' +
                         'chains to a bundle. None of them will ever consult ' +
                         'this service, so the credential goes on working ' +
                         'until it expires and the holder is not ' +
                         'told.\n\nWhat it does is record that THIS IDENTITY ' +
                         'PROVIDER HAS DISOWNED the credential, which is a ' +
                         'different claim and a useful one: it is what a ' +
                         'global sign-out can report, what CAEP transmits to ' +
                         'a receiver that subscribed, and what SAML Single ' +
                         'Logout carries for an assertion issued through a ' +
                         'browser profile. A WS-Trust assertion has neither ' +
                         'channel and the mark is the whole of what exists ' +
                         'for it.\n\nUntil 2026-09-05 this was impossible ' +
                         'and every surface said so. What changed is the ' +
                         'recognition that what this service KNOWS and what ' +
                         'a relying party will HONOUR are two claims, and ' +
                         'only the second was ever out of reach. ' +
                         'Every row of GET /admin-api/tokens ' +
              ...
              properties: {
                artifact: { type: 'string',
                            description: 'The `key` off a row of ' +
                                         'GET /admin-api/tokens. This ' +
                                         'service\'s own handle and NOT the ' +
                                         'protocol\'s — a Kerberos ticket ' +
                                         'carries no identifier anybody ' +
                                         'can quote, so `identifier` ' +
                                         'cannot address every row and this ' +
                                         'can.' },
          ...
          { action: 'restore-artifact', operationId: 'restoreIssuedArtifact',
            ...
            description: 'The opposite of `revoke-artifact`, and NON-SPEC ' +
                         'for the reason every restore here is. It is a ' +
                         'smaller act than the others, ' +
                         'because what it takes back never ' +
                         'reached anybody: the credential was working ' +
                         'throughout.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
          ...
          { action: 'revoke-set', operationId: 'revokeIssuedSet',
            ...
            description: 'One act instead of one call per credential. It ' +
                         'writes NOWHERE NEW: each member goes through the ' +
                         'same revocation `/oauth2/revoke` performs, one at ' +
                         'a time, into the same set of revoked `jti`s — so a ' +
                         'token revoked here immediately introspects as ' +
                         'inactive, is refused by UserInfo with ' +
                         '`invalid_token`, and fails the refresh grant with ' +
                         '`invalid_grant`.\n\nWhat it saves is the mistake ' +
                         'this whole resource was reshaped to prevent: ' +
                         'revoking two credentials of three and believing ' +
                         'the grant is dead, when the refresh token left ' +
                         'behind mints a new access token on request.\n\nTHE ' +
                         'MEMBERS ARE RE-READ AT THE MOMENT OF THE CALL and ' +
                         'never taken from the caller — the body carries a ' +
                         '`setKey` and nothing else — so a list drawn an ' +
                         'hour ago cannot revoke a `jti` that has since been ' +
                         'forgotten to the cap while missing one issued ' +
                         'since.\n\nA set holding ' +
                         'nothing revocable is REFUSED ' +
                         'with 400 rather than answered with "revoked 0": ' +
                         'nothing consults this service about a SAML ' +
                         'assertion, a Kerberos ticket or an SVID, so a ' +
                         'success would be a claim about the world that is ' +
                         'not true. That is the same answer `revoke-kind` ' +
                         'gives for an unrevocable kind.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
            ...
            responseDescription: 'How many were revocable, how many actually ' +
                                 ...
                                 'already revoked is counted as revocable ' +
                                 'and not as moved, so `revoked: 0` on an ' +
                                 '`ok` reply means the set was already dead.' },
          ...
          { action: 'restore-set', operationId: 'restoreIssuedSet',
            ...
            description: 'The opposite of `revoke-set`, and NON-SPEC for the ' +
                         'reason `restore` is: no real authorization server ' +
                         'can undo a revocation, because a resource server ' +
                         'may already have cached the refusal. It is here ' +
                         'because otherwise getting back to a working grant ' +
                         'means restarting the service.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
          ...
          { action: 'revoke-kind', operationId: 'revokeTokensByKind',
            ...
            description: 'Only the three JWT kinds can be revoked. Nothing ' +
                         ...
                         'Kerberos ticket — an assertion is valid because ' +
                         'its signature verifies and its Conditions hold, a ' +
                         'ticket because the service it names can decrypt it ' +
                         '— so naming one of those is refused rather than ' +
                         'silently doing nothing. `GET /admin-api/tokens` ' +
                         'lists the three under `revocableKinds`.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
          ...
          { action: 'revoke-subject', operationId: 'revokeTokensBySubject',
            ...
            description: 'Matches `sub` or `username` EXACTLY. That is the ' +
                         'difference from `revoke-user` beside it: this is ' +
                         'the one to use when you have a string off a token, ' +
                         'and that one is for an ' +
                         'identity the users list names, ' +
          ...
          { action: 'revoke-user', operationId: 'revokeTokensByUser',
            ...
            description: 'The users list\'s `key`, and every spelling of it ' +
                         ...
                         '`alice@STS.MOCK` are one ' +
                         'identity here, so revoking ' +
                         '"for alice" means all of them. Use ' +
                         '`revoke-subject` ' +
      ...
      //
      // First, THIS WHOLE API IS ITSELF REALM-SCOPED. `/admin-api/config` is
      // the default realm's configuration; `/realm/acme/admin-api/config` is
      // `acme`'s, and a `set` posted there sets it on `acme` alone. That is not
      // a special case anybody wrote here — it falls out of the same
      // path-prefix middleware that makes /oauth2/token realm-scoped, and it
      // means every one of the operations below already works per realm. These
      // five are only the ones that manage the REGISTRY.
      //
      // Second, THE REGISTRY ITSELF IS NOT REALM-SCOPED, and it could not
      // sensibly be: there is one list of realms in this process, so `GET
      // /admin-api/realms` answers the same list whichever prefix it is called
      // under. What differs is `current`, which names the realm the CALL
      // arrived in — and `remove` refuses to remove that one, for the reason
      // the console gives.
      //
      // In development mode a realm's SIGNING KEY is regenerated on every
      // start, so a token minted in a realm today verifies against nothing
      // after a restart; product mode keeps the keys in the persistence store
      // (`common/CLAUDE.md`, `keystore.js`). THE REALM ROW ITSELF is written
      // down since 2026-08-27 when `persistence.realms` has a store under it,
      // along with that realm's own directory; see
      // `GET /admin-api/persistence`. In the default memory mode it is not, and
      // a stack that wants its realms back creates them from these operations —
      // which is why `create` is worth having rather than a config file entry:
      // the thing that starts the stack already speaks this API.
      // ---------------------------------------------------------------------
      { method: 'GET', path: BASE + '/realms', tag: 'Trust realms',
        ...
        description: 'A TRUST REALM is a whole logical copy of this service: ' +
                     'its own configuration, its own signing key, and its ' +
                     'own sessions, authorization codes, tokens, credential ' +
                     'offers, service providers, statistics and audit log — ' +
                     'answering on the same sockets as every other realm and ' +
                     'told apart by a segment at the front of the ' +
                     'path.\n\nThe DEFAULT realm has no prefix, cannot be ' +
                     'removed and cannot be renamed: every URL this service ' +
                     'published before realms existed is a URL in it. A ' +
                     ...
                     'everything it holds while issuing it — keys, sessions, ' +
                     'codes, tokens, offers, artifacts, statistics and the ' +
                     'audit log. It does NOT separate the embedded ' +
                     'directory: LDAP answers on a socket with no path to ' +
                     'put a segment in, so there is one set of people, ' +
                     'groups and applications for the whole process — which ' +
                     'means OAuth client registrations, SAML service ' +
                     'provider entries, the SPIFFE registry and the two ' +
                     'admin console roles are shared. Kerberos, the ' +
                     'certificate the main port and LDAPS 636 present, and ' +
                     'SPIFFE\'s four sockets are shared for the same ' +
                     'reason.\n\n`reserved` is the list of ids a realm may ' +
                     'not be called, read off the live router: they are the ' +
                     'first segments of paths this service already serves, ' +
                     'and the refusal stands whatever `realms.pathSegment` ' +
                     'is set to precisely so that clearing that setting ' +
                     'cannot turn an existing realm into a shadow ' +
        ...
        handler: function (req, res, next) {
          ...
          // EVERY ROW SHOWS ITS REALM'S `kid`, and reading one MAKES the key
          // set of a realm this node holds none for — twenty realms created on
          ...
          helpers.prepareKeySets(realms.list().map(function (one) {
            ...
            self.sendJson(res, 200, adminViews.realmsJson(req));
      ...
      { method: 'POST', route: BASE + '/realms/:action', tag: 'Trust realms',
        ...
        actions: [
          { action: 'create', operationId: 'createRealm',
            ...
            description: 'Every HTTP endpoint this service has begins ' +
                         'answering under the new realm\'s prefix ' +
                         'immediately, with a signing key of its own and ' +
                         'nothing issued yet.\n\nThe `id` becomes a PATH ' +
                         'SEGMENT, so it is lower-case letters, digits and ' +
                         'hyphens, starts with a letter or a digit, and is ' +
                         ...
                         'case worth being able to build on a mock.\n\nThe ' +
                         'OAuth issuer is deliberately NOT seeded: it ' +
                         'defaults to naming the base URL a request arrived ' +
                         'on, and that already carries the realm prefix.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
                id: { type: 'string',
                      description: 'Lower-case letters, digits and hyphens. ' +
                                   'It is the path segment.' },
                name: { type: 'string',
          ...
          { action: 'update', operationId: 'updateRealm',
            ...
            description: 'The `id` cannot be changed, because it is the path ' +
                         'segment every client was given. Define a new realm ' +
          ...
          { action: 'set', operationId: 'setRealmSetting',
            ...
            description: 'The value applies to the next request that arrives ' +
                         'under that realm\'s prefix and to nothing else — ' +
                         'above whatever the process as a whole is ' +
                         'configured with, and below nothing.\n\nThis is the ' +
                         'same store `POST /realm/<id>/admin-api/config/set` ' +
                         'writes to, and either is fine; the difference is ' +
                         'only which realm you have to be in to make the ' +
                         'call. A setting whose `editable` is false is ' +
            ...
            responseDescription:
              'The realm and the key, in `realm` and `key`.' },
          ...
          { action: 'unset', operationId: 'unsetRealmSetting',
            ...
            description: 'The realm falls back to whatever this service as a ' +
                         ...
                         'runtime override, and is left alone. A key the ' +
                         'realm does not set is refused rather than treated ' +
                         'as already done, because the two are different ' +
                         'facts and a caller that misspelt a key would ' +
                         'otherwise be told it succeeded.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: { id: { type: 'string' }, key: { type: 'string' } },
          ...
          { action: 'remove', operationId: 'removeRealm',
            ...
            description: 'EVERYTHING IT ACCUMULATED GOES WITH IT: its ' +
                         'sessions, its authorization codes, its tokens, its ' +
                         'refresh families, its credential offers, its ' +
                         'service providers, its statistics, its audit log ' +
                         'and its signing key. That is deliberate rather ' +
                         'than thorough — a realm re-created with the same ' +
                         'id inheriting the last one\'s sessions and tokens ' +
                         'would be the single most surprising thing a ' +
                         're-created realm could do.\n\nNOTHING IS REMOVED ' +
                         'FROM THE DIRECTORY, because nothing there belongs ' +
                         'to a realm: `ou=users`, `ou=groups` and ' +
                         '`ou=applications` are shared by every realm in ' +
                         'this process.\n\nA realm cannot remove ITSELF — a ' +
                         'call to `/realm/acme/admin-api/realms/remove` ' +
                         'naming `acme` is refused. Everything about the ' +
                         'removal would work; what would not is the caller, ' +
                         'which would be talking to a prefix that had ' +
                         'stopped existing. Call it from another ' +
      ...
      { method: 'GET', path: BASE + '/config', tag: 'Configuration',
        ...
        summary:
          'Every setting this service has, and where each value came from',
        description: 'Every setting, grouped by protocol, each with its ' +
                     'effective value and the SOURCE of that value: a ' +
                     'runtime override, an environment variable, the ' +
                     'appconfig file CONFIG_FILE names, or `env/defaults.js` ' +
                     'under it. The source is the part that was not ' +
                     'answerable before this resource existed ' +
                     '— the four are indistinguishable once a ' +
                     'value has been read, and the question "why is the ' +
                     'issuer ' +
                     'that?" used to be a grep. There is no fifth source: a ' +
                     'setting with a value in none of them stops this ' +
                     'service from STARTING rather than falling back to a ' +
                     'constant in a module.\n\nIt also says which settings ' +
                     'can be CHANGED while the service runs. The ones that ' +
                     'cannot were consumed at startup — a bound socket, the ' +
                     'TLS certificate\'s names, the Kerberos ' +
                     'principal database and its long-term keys, ' +
        ...
        handler: function (req, res) {
          ...
          self.sendJson(res, 200, admin.configJson());
      ...
      { method: 'POST', route: BASE + '/config/:action', tag: 'Configuration',
        ...
        handler: function (req, res) {
          log.debug("Entering the management API configuration action " +
                    "endpoint.");
          const body = parseBody(req);
          const result = adminActions.configAction(self.withAction(req, body));
          if (!result.ok) {
            errorCodes.mark(res, errorCodes.codeOf(result) || 'STS-API-0038');
          }
          self.sendJson(res, result.ok ? 200 : 400, result);
          log.debug("Leaving the management API configuration action " +
                    "endpoint.");
        },
        actions: [
          { action: 'set', operationId: 'setSetting',
            ...
            description: 'A RUNTIME OVERRIDE — the top of config.js\'s five ' +
                         ...
                         'leave a test\'s forgotten change behind ' +
                         'permanently. The durable copy goes to the ' +
                         'persistent store, which is not a place anything is ' +
                         'checked in from.\n\nThe change applies to the next ' +
                         'token, assertion, ticket or search. Nothing ' +
                         'already issued changes, because a token is a ' +
                         'signed document.\n\nA setting whose `editable` is ' +
                         'false is REFUSED with the reason rather than ' +
                         'accepted, and a value that does not fit the ' +
            ...
            requestBody: {
              ...
              properties: {
                ...
                value: { description: 'A string is always accepted and is ' +
                                      'coerced to the setting\'s type, so ' +
                                      'the environment spelling works here ' +
                                      'too; a JSON number, ' +
            ...
            responseDescription:
              'The setting as it now stands, in `setting`.' },

          { action: 'set-many', operationId: 'setSettings',
            summary: 'Change a whole section at once',
            description: 'What the console\'s per-section Save posts, and it ' +
                         'is not a convenience: a section is how a person ' +
                         'changes configuration, and one call per field ' +
                         'would make a partly-applied section the ordinary ' +
                         'outcome of a mistake in any one of ' +
                         'them.\n\nALL-OR-NOTHING. Every value is checked ' +
                         'before any is written, so a body with one bad ' +
                         'field changes nothing and names it. A key this ' +
                         'service does not know is ignored rather than ' +
                         'refused, because a form posts fields this resource ' +
                         'never declared; `applied` says which were taken ' +
                         'and `changed` which actually differed from what ' +
          ...
          { action: 'reset', operationId: 'resetSetting',
            ...
            description: 'The setting falls back to whatever it would have ' +
                         'used had nothing ever been set here — its ' +
                         'environment variable, the appconfig file, or ' +
                         '`env/defaults.js`. A setting with no override is ' +
                         'refused rather than treated as already done, ' +
                         'because the two are different facts and a ' +
                         'caller that misspelt a key would otherwise ' +
          ...
          { action: 'reset-all', operationId: 'resetAllSettings',
            ...
            description: 'Returns the whole service to the configuration it ' +
                         'started with, without restarting it. This is what ' +
                         'a test should call to put the service back: the ' +
                         'mock\'s admin state survives ' +
                         'between jobs, so a setting left ' +
                         'changed here changes what every later job sees.',
            requestBodyRequired: false,
            requestBody: { type: 'object', properties: {},
                           additionalProperties: false },
            responseDescription:
              'The keys that were cleared, in `cleared`.' } ] },
      ...
      // — that is the rule as written. What it does NOT get is a second store:
      // the handler calls `adminActions.tokenLifetimesAction`, which writes
      // through `config.setOverride()`, which is the same function `POST
      // /config/set` calls against the same override map. So these two
      // operations and the Configuration ones are two doors onto one thing,
      // deliberately, in the way `/admin/rbac` and `ldapmodify` are four doors
      // onto one membership.
      //
      ...
      // ignores what it does not know, which is right for a form posting a
      // whole section and wrong for a test that means to set a lifetime — a
      // misspelt key there succeeds and changes nothing. This one refuses
      // anything that is not one of the six, by name.
      ...
      { method: 'GET', path: BASE + '/token-lifetimes', tag: 'Token lifetimes',
        ...
        description: 'The three lifetimes — access token, ID Token, refresh ' +
                     ...
                     '`GET /config` too; `settings` here is the same row ' +
                     'shape, carrying each one\'s bounds, its source and its ' +
                     'default. `lifetimes` beside it is just the three ' +
                     'lifetimes and the skew, for a caller that wants the ' +
                     'value rather than the provenance.\n\nIt also reports ' +
                     'WHAT IS ALREADY OUT THERE, per kind, counted against ' +
                     'the same clock the endpoints use — the skew is applied ' +
                     'to that count, so a token this calls expired is one ' +
                     'POST /oauth2/introspect will report inactive.\n\nA ' +
                     'LIFETIME IS STAMPED INTO A TOKEN WHEN IT IS SIGNED, so ' +
                     'changing one reaches the next token and ' +
                     'nothing already issued. To take an issued token ' +
                     'out of circulation, revoke it under Tokens.',
        mirrors: 'GET /admin/token-lifetimes',
        responseDescription: 'The six settings, and what has been issued ' +
                             'under them.',
        responseSchema: { $ref: '#/components/schemas/TokenLifetimes' },
        handler: function (req, res) {
          log.debug("Entering the management API token lifetimes endpoint.");
          self.sendJson(res, 200, adminViews.tokenLifetimesJson());
      ...
      { method: 'POST', route: BASE + '/token-lifetimes/:action',
        ...
        // EXISTS BESIDE `/config/set-many`**, which ignores one on purpose — so
        // a narrow door that stopped refusing would be two operations over one
        ...
        // answers "not a member of this request" and stops. Letting ajv go
        // first would replace a refusal a caller can act on with a worse one
        // and switch off the assertion that guards it, which is the same
        // judgement `structureOnly()` makes about `enum` and `required` one
        // level up.
        // ---------------------------------------------------------------------
        ...
        handler: function (req, res) {
          ...
          const body = parseBody(req);
          const result = adminActions.tokenLifetimesAction(self.withAction(req,
              body));
          if (!result.ok) {
        ...
        actions: [
          { action: 'set', operationId: 'setTokenLifetimes',
            ...
            description: 'A RUNTIME OVERRIDE, like every other change made ' +
                         ...
                         '`POST /config/set-many`, a property that is not ' +
                         'one of the six is REFUSED rather than ignored. ' +
                         'That door is for a form posting a whole section, ' +
                         'where an unknown field is ordinary; this one is ' +
                         'for a caller that means to set a lifetime, where a ' +
                         'misspelt key that succeeded and changed nothing is ' +
                         'the worst possible answer.\n\nEvery lifetime must ' +
                         'be a whole number of THIRTY-SECOND units, between ' +
                         '30 and 2592000 (thirty days). The skew is 0 to 300 ' +
                         'in the same units. The idle timeout is whole ' +
                         'seconds, 0 turning it off, and revocation on ' +
                         'sign-out is a boolean. Those bounds are on each ' +
                         'setting\'s row in the GET, so a client can render ' +
                         'them rather than repeat them.\n\nThe change ' +
                         'applies to the NEXT token signed. Nothing ' +
                         'already issued is affected: a lifetime ' +
            ...
            requestBody: {
              ...
              description: 'One property per setting, named by its dot path. ' +
                           ...
                           'from the one this action refuses against rather ' +
                           'than typed beside it: the door refuses anything ' +
                           'outside it BY NAME, which is the whole reason it ' +
                           'exists beside `POST /config/set-many` — that one ' +
                           'IGNORES a key it does not know, which is right ' +
                           'for a form posting a section and wrong for a ' +
                           'caller who misspelt a lifetime. A document short ' +
                           'by one is then a caller refused for following it.',
              properties:
                this.narrowDoorProperties(adminActions.tokenLifetimeKeys()),
          ...
          { action: 'defaults', operationId: 'resetTokenLifetimes',
            ...
            description: 'Clears the runtime override on THESE SIX ONLY, so ' +
                         ...
                         'not `POST /config/reset-all`, which would also ' +
                         'drop an override somebody set on an unrelated ' +
                         'page. A test that changed only the lifetimes ' +
                         'should call this to put the service back; one that ' +
                         'changed more should call that one.\n\nA setting ' +
                         'that was not overridden is skipped ' +
                         'rather than refused, because this means ' +
      ...
      // config.setOverride() against the same override map POST /config/set
      // writes to, and the same map the two identity provider pages' own forms
      // write to. Four doors, one thing.
      //
      ...
      { method: 'GET', path: BASE + '/saml-assertions', tag: 'SAML assertions',
        ...
        summary: 'How long an issued assertion is valid, and the clock skew '
                 + 'written into it',
        description: 'The two assertion lifetimes — SAML 2.0 and SAML 1.1 — ' +
                     'and the clock skew added to BOTH ENDS of each, which ' +
                     'is what this service writes into ' +
                     '`Conditions/NotBefore` and `NotOnOrAfter`, beside the ' +
                     'signing, NameID, artifact and SAML 2.0 encryption ' +
                     'choices and the WS-Federation assertion lifetime — ' +
                     'sixteen settings in all.\n\nAll of them are ordinary ' +
                     'configuration settings and appear in `GET /config` ' +
                     'too; `settings` here is the same row shape, carrying ' +
                     'each one\'s bounds, its source and its default. ' +
                     '`assertions` beside it is the numbers, and includes ' +
                     '`saml2WindowS` and `saml11WindowS` — the WHOLE width ' +
                     'of the stated window, which is the lifetime plus TWICE ' +
                     'the skew and is the figure a caller actually has to ' +
                     'reason about. No single setting states it.\n\nTHE ' +
                     'LIFETIMES ARE PER PROFILE AND THE SKEW IS NOT. SAML ' +
                     '2.0 and SAML 1.1 are separate implementations here, ' +
                     'consumed differently, so each has its own lifetime; ' +
                     'the skew is a fact about the clocks in the estate this ' +
                     'service issues into, which a deployment decides once. ' +
                     'The settings reach WS-Trust and WS-Federation as well ' +
                     '— their assertions come out of the same two builders — ' +
                     'but a WS-Federation sign-in, which carries a SAML 1.1 ' +
                     'assertion, has a lifetime of its own, ' +
                     '`wsfed.assertionLifetimeMin`.\n\n`saml.clockSkewS` IS ' +
                     'NOT `oauth2.clockSkewS`. This one is written INTO a ' +
                     'document this service issues. That one is the ' +
                     'tolerance applied wherever this service READS one ' +
                     'back, including an inbound federation partner\'s ' +
                     'assertion, and it is on `GET /token-lifetimes`.\n\nIt ' +
                     'also reports what has already been issued, per ' +
                     'profile, counted against this service\'s own clock ' +
                     'with no allowance applied.\n\nA WINDOW IS STAMPED INTO ' +
                     'AN ASSERTION WHEN IT IS SIGNED, so changing one ' +
                     'reaches the next assertion and nothing already issued.',
        ...
        handler: function (req, res) {
          ...
          self.sendJson(res, 200, adminViews.samlAssertionsJson());
      ...
      { method: 'POST', route: BASE + '/saml-assertions/:action',
        ...
        handler: function (req, res) {
          ...
          const body = parseBody(req);
          const result = adminActions.samlAssertionsAction(self.withAction(req,
              body));
          if (!result.ok) {
        ...
        actions: [
          { action: 'set', operationId: 'setSamlAssertions',
            ...
            description: 'A RUNTIME OVERRIDE, like every other change made ' +
                         ...
                         'bad field changes nothing and names it.\n\nUNLIKE ' +
                         '`POST /config/set-many`, a property that is not ' +
                         'one of the sixteen is REFUSED rather than ignored, ' +
                         'for the reason `POST /token-lifetimes/set` ' +
                         'gives.\n\nTHE TWO LIFETIMES ARE MINUTES AND THE ' +
                         'SKEW IS SECONDS (the artifact lifetimes are ' +
                         'seconds too). That is not a formatting accident: a ' +
                         'lifetime is set to a number of minutes to watch an ' +
                         'assertion go stale, and a skew is a handful of ' +
                         'seconds covering the difference between two ' +
                         'machines. The skew is 0 to 300 — five minutes is ' +
                         'what Kerberos allows here (`krb5.clockSkew`), and ' +
                         'wider than that the window has stopped being a ' +
                         'tolerance. The bounds are on each setting\'s row ' +
                         'in the GET, so a client can render them rather ' +
                         'than repeat them.\n\nThe change applies to the ' +
                         'NEXT assertion signed. Nothing already issued ' +
            ...
            requestBody: {
              ...
              description: 'One property per setting, named by its dot path, ' +
                           ...
                           'rather than typed here — it named three of them ' +
                           'for months while the action accepted sixteen. ' +
                           'The lifetimes are in MINUTES and the skew and ' +
                           'the artifact lifetimes are in SECONDS; each row ' +
                           'of the GET carries its own unit and bounds.',
              properties:
                this.narrowDoorProperties(adminActions.samlAssertionKeys()),
          ...
          { action: 'defaults', operationId: 'resetSamlAssertions',
            ...
            description: 'Clears the runtime override on THESE SIXTEEN ONLY, ' +
                         'so each falls back to its environment variable, ' +
                         'the appconfig file, or `env/defaults.js` — for the ' +
      ...
      { method: 'GET', path: BASE + '/claims', tag: 'Custom claims',
        ...
        description: 'The four sets — OAuth 2.0 access token, OIDC ID Token, ' +
                     'SAML 2.0 Attribute, SAML 1.1 Attribute — with the ' +
                     'rules that govern them: the claim names this service ' +
                     'sets itself and will not let you override, and the ' +
                     'placeholders a value may use.\n\nEach set has TWO ' +
                     'HALVES and they are configured by different ' +
                     'operations. `claims` are TYPED: a name and a value ' +
                     'somebody wrote, the same for everybody except where a ' +
                     '${placeholder} carries the sign-in. `attributes` are ' +
                     'LDAP ATTRIBUTE TYPES chosen from `attributeCatalogue`, ' +
                     'whose value is read off that person\'s entry under ' +
                     'ou=users — so an `ldapmodify` changes the next token, ' +
                     'and an LDAP client and an OIDC client pointed at this ' +
                     'service are shown the same ' +
                     'person.\n\n`attributeClaims` is what the current ' +
                     'selection would actually put in each set for the ' +
                     'previewed person, built by the same ' +
                     'function the issuance path calls. A ' +
                     'caller with no browser has no other way to ask "what ' +
                     'would ' +
                     'this issue".',
        ...
        parameters: [
          { name: 'user', in: 'query', required: false,
            ...
            description: 'Whose attribute values to preview. Defaults to a ' +
                         ...
                         'preview shows real values on a fresh process. ' +
                         'Somebody with no entry gets generated values — the ' +
                         'same invented person every ' +
                         'time, seeded from the name — and ' +
                         '`preview.entryFound` says which of the two ' +
                         'happened.' }
        ],
      ...
      { method: 'POST', route: BASE + '/claims/:action', tag: 'Custom claims',
        ...
        handler: function (req, res) {
          ...
          // credential-claims row below takes it. The other six actions ignore
          // it.
          const names = self.namesOf(req, body, 'attribute', 'attributes');
          // The THIRD argument is the family, and it is what makes this
          // resource the mirror of /admin/claims rather than of the store: a
          // `set` of `saml2` here is refused by name and sent to
          // /admin-api/saml-attributes, exactly as the console's own form post
          // is. The action function, the store and the audit row are the same
          // ones either way.
        ...
        actions: this.claimSetActions(JWT_CLAIM_FAMILY) },

      // -----------------------------------------------------------------------
      // THE USERINFO HALF OF THE SAME STORE, and the half a client can add to.
      //
      // Its own resource for the reason the SAML one has its own: rule 7 is
      // about the CONTROL, and /admin/userinfo-claims is its own page with its
      // own forms. But it carries something neither of the others does — the
      // OIDC Core section 5.5 vocabulary, in `claimsRequest` — because that is
      // the half of this endpoint's behaviour an administrator does NOT decide,
      // and a caller with no browser has no other way to learn what a claims
      // request may name.
      // -----------------------------------------------------------------------
      { method: 'GET', path: BASE + '/userinfo-claims', tag: 'UserInfo claims',
        operationId: 'getUserInfoClaims',
        summary: 'What every UserInfo response will carry, and what a client ' +
                 'may ask it for',
        description: 'The `userinfo` claim set — the fifth of the five, and ' +
                     'the only one whose subject is not something this ' +
                     'service ISSUES.\n\n**A UserInfo response is built on ' +
                     'EVERY call.** An access token, an ID Token and both ' +
                     'SAML assertions are signed documents: a claim added to ' +
                     'one of those sets reaches a client at its next sign-in ' +
                     'and never reaches what it already holds. A claim added ' +
                     'here reaches the next `GET /oauth2/userinfo` from a ' +
                     'client that signed in an hour ago and has done nothing ' +
                     'since. That is the whole reason it is configured ' +
                     'separately from the ID Token set rather than being the ' +
                     'same list under two names.\n\nThe set has the same TWO ' +
                     'HALVES as every other and they are configured by ' +
                     'different operations. `claims` are TYPED: a name and a ' +
                     'value somebody wrote, the same for everybody except ' +
                     'where a ${placeholder} carries the sign-in. ' +
                     ...
                     'REQUIRED in this response (OIDC Core 5.3.2, and a ' +
                     'client MUST check it against the ID Token\'s), and ' +
                     'when a client has registered a ' +
                     '`userinfo_signed_response_alg` the whole response is a ' +
                     'JWT carrying `iss`, `aud` and ' +
                     '`exp`.\n\n`claimsRequest` is the half no operation ' +
                     'here sets: OIDC Core section 5.5 lets a CLIENT name ' +
                     'individual claims in the `claims` request parameter, ' +
                     'and this service answers them off the same catalogue. ' +
                     'It lists every name a request may use, the four layers ' +
                     'of precedence, what is carried and NOT ' +
                     'enforced (`essential`, `value`, `values`), ' +
                     'and the non-spec way to send one straight to the ' +
                     'endpoint.',
        mirrors: 'GET /admin/userinfo-claims',
        parameters: [
          { name: 'user', in: 'query', required: false,
            schema: { type: 'string', default: 'alice' },
            description: 'Whose attribute values to preview. The same ' +
                         'parameter, the same cap and the same default GET ' +
                         '/admin-api/claims takes, deliberately: the three ' +
                         'replies preview one person unless asked otherwise. ' +
                         '`preview.entryFound` says ' +
                         'whether the directory holds ' +
                         'them or the values were invented from the ' +
                         'username.' },
          { name: 'request', in: 'query', required: false,
            schema: { type: 'string' },
            description: 'A section 5.5 claims request, as the JSON a client ' +
                         'would send — for example ' +
                         '`{"userinfo":{"birthdate":null,"address":null}}`. ' +
                         'The ' +
                         'reply\'s `claimsRequest.preview` then says exactly ' +
                         'what that request would return for `user`, ' +
                         'computed by the two functions the UserInfo ' +
                         'endpoint itself calls. A MALFORMED one is reported ' +
                         'in `claimsRequest.preview.error` and does NOT fail ' +
                         'this call: what it shows is the `invalid_request` ' +
                         'a client would be given, which is the thing a ' +
      ...
      { method: 'POST', route: BASE + '/userinfo-claims/:action',
        ...
        handler: function (req, res) {
          log.debug("Entering the management API UserInfo claims action " +
                    "endpoint.");
          const body = parseBody(req);
          const names = self.namesOf(req, body, 'attribute', 'attributes');
          const result = adminActions.claimsAction(self.withAction(req, body),
          ...
          self.sendJson(res, result.ok ? 200 : 400, result);
          log.debug("Leaving the management API UserInfo claims action " +
                    "endpoint.");
        },
        actions: this.claimSetActions(USERINFO_CLAIM_FAMILY) },

      // -----------------------------------------------------------------------
      // THE SAML HALF OF THE SAME STORE, mirroring the console page that
      // carries it. Two resources rather than one taking four sets, because
      // rule 7 is about the CONTROL: /admin/saml-attributes is its own page
      // with its own forms, and an API that answered for it under a name
      // promising tokens would leave a caller reading `getClaims` to find out
      // what an assertion will carry.
      // -----------------------------------------------------------------------
      { method: 'GET', path: BASE + '/saml-attributes', tag: 'Custom SAML ' +
        ...
        description: 'The two SAML sets — SAML 2.0 Attribute and SAML 1.1 ' +
                     ...
                     'them. The 2.0 set reaches every assertion WS-Trust ' +
                     'issues with a 2.0 token type; the 1.1 set reaches the ' +
                     '1.1 ones, which is what WS-Federation\'s passive ' +
                     'requestor profile carries, so the 1.1 half is the one ' +
                     'a browser sign-in exercises.\n\nEach set has TWO ' +
                     'HALVES and they are configured by different ' +
                     'operations. `claims` are TYPED: a name and a value ' +
                     'somebody wrote, the same for everybody except where a ' +
                     '${placeholder} carries the sign-in. `attributes` are ' +
                     'LDAP ATTRIBUTE TYPES chosen from `attributeCatalogue`, ' +
                     'whose value is read off that person\'s entry under ' +
                     'ou=users — so an `ldapmodify` changes the next ' +
                     'assertion, and an LDAP client and a SAML relying party ' +
                     'pointed at this service are shown the same ' +
                     'person.\n\nTHERE IS NO `reservedJwtClaims` HERE and ' +
                     'the absence is the answer rather than an omission: ' +
                     'that list is enforced for a JWT set only, because an ' +
                     'assertion attribute called `exp` collides with ' +
                     'nothing. `defaultSaml11Namespace` is the rule that IS ' +
                     'this family\'s — the namespace a 1.1 attribute gets ' +
                     'when the call does not name one.\n\nThe two JWT sets ' +
                     'are at GET /admin-api/claims. One store behind both, ' +
                     'and one audit row per change whichever door made it.',
        ...
        parameters: [
          { name: 'user', in: 'query', required: false,
            ...
            description: 'Whose attribute values to preview. Defaults to a ' +
                         ...
                         'preview shows real values on a fresh process. ' +
                         'Somebody with no entry gets generated values — the ' +
                         'same invented person every time, seeded from the ' +
                         'name — and `preview.entryFound` says which of the ' +
                         'two happened. It is the same parameter, the same ' +
                         'cap and the same default GET ' +
                         '/admin-api/claims takes, deliberately: the ' +
                         'two replies preview one person unless asked ' +
                         'otherwise.' }
        ],
        responseDescription: 'The two SAML sets, the attribute catalogue and ' +
                             'the preview.',
      ...
      { method: 'POST', route: BASE + '/saml-attributes/:action',
        ...
        handler: function (req, res) {
          log.debug("Entering the management API SAML attributes action " +
                    "endpoint.");
          const body = parseBody(req);
          const names = self.namesOf(req, body, 'attribute', 'attributes');
          const result = adminActions.claimsAction(self.withAction(req, body),
          ...
          self.sendJson(res, result.ok ? 200 : 400, result);
          log.debug("Leaving the management API SAML attributes action " +
                    "endpoint.");
        },
      ...
      { method: 'GET', path: BASE + '/credential-claims',
        ...
        description: 'The catalogue, what is selected from it, and a preview ' +
                     'of what one person\'s credential would carry if it ' +
                     'were issued now.\n\nThe catalogue is of LDAP ATTRIBUTE ' +
                     'TYPES and not of claim names, because this service has ' +
                     'a directory and a claim whose value nothing else can ' +
                     'see is half a demonstration: the value of a claim is ' +
                     'the value on that person\'s entry under ou=users.',
        mirrors: 'GET /admin/vc',
        parameters: [
          { name: 'user', in: 'query', required: false,
            schema: { type: 'string', default: 'alice' },
            description: 'Whose credential to preview. Defaults to a person ' +
                         'the directory holds from startup, so the preview ' +
                         'shows real values on a fresh process.' }
        ],
      ...
      { method: 'POST', route: BASE + '/credential-claims/:action',
        ...
        handler: function (req, res) {
          ...
          const body = parseBody(req);
          const names = self.namesOf(req, body, 'attribute', 'attributes');
          const result = adminActions.vcAction(self.withAction(req, body),
                                               names);
          if (!result.ok) {
        ...
        actions: [
          { action: 'select', operationId: 'selectCredentialClaims',
            ...
            description: 'Replaces the selection, and then SWEEPS THE ' +
                         'DIRECTORY: every person under ou=users gains the ' +
                         'selected attributes they are missing, invented ' +
                         'from their username — deterministically, so one ' +
                         'username is one invented person across restarts. ' +
                         'An attribute already there is never overwritten, ' +
                         'so an operator\'s `ldapmodify` and the seeded ' +
                         'people\'s own names survive.\n\nThe sweep is the ' +
                         'point rather than a side effect: without it, ' +
                         'selecting `title` would change every future ' +
                         'credential and change nothing an LDAP client could ' +
                         'see, and the two halves of this service would stop ' +
                         'describing the same people. The reply\'s `sweep` ' +
                         'says what it did, including when there was no ' +
                         'directory to do it to.\n\nThe issuer METADATA is ' +
                         'built from this same selection, so what is ' +
                         'advertised cannot drift from what is minted.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
            ...
            responseDescription: 'What is selected now, what changed, and ' +
                                 'what the sweep did.' },

          { action: 'add', operationId: 'addCredentialClaim',
            summary: 'Add one attribute to the credential claim set',
            description: 'Sweeps the directory afterwards, as `select` does. ' +
                         'An attribute already selected is refused rather ' +
              ...
              properties: {
                ...
                name: { type: 'string',
                        description: 'An alias for `attribute`; `vcAction()` ' +
                                     'reads `body.attribute || body.name`, ' +
                                     'so both spellings have always worked ' +
            ...
            responseDescription:
              'What is selected now, and what the sweep did.' },

          { action: 'remove', operationId: 'removeCredentialClaim',
            summary: 'Remove one attribute from the credential claim set',
            description: 'Future credentials stop carrying it. What was ' +
                         'already written onto a directory entry stays there ' +
                         '— nothing here deletes an attribute value, because ' +
                         'an operator may have set it.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
          ...
          { action: 'defaults', operationId: 'resetCredentialClaims',
            ...
            requestBody: { type: 'object', properties: {}, examples: [{}],
                           additionalProperties: false },
            responseDescription:
              'The default selection, and what the sweep did.' },
          ...
          { action: 'populate', operationId: 'populateDirectory',
            ...
            description: 'For a directory that gained entries after the last ' +
                         'change — or one that was not running when it ' +
                         'happened. Running it twice does nothing the second ' +
                         'time, because the sweep only fills what is absent.',
            requestBodyRequired: false,
            requestBody: { type: 'object', properties: {}, examples: [{}],
                           additionalProperties: false },
      ...
      { method: 'GET', path: BASE + '/verifier-request',
        tag: 'Verifier request',
        operationId: 'getVerifierRequest',
        summary: 'What the mock OID4VP Verifier asks a wallet for',
        description: 'The claims, the credential format, and the ' +
                     '`dcql_query` they build — from the function that ' +
                     'builds the real one, so this is the next Authorization ' +
                     'Request rather than a description of it.\n\nEach ' +
                     'catalogue row also says whether the ISSUER currently ' +
                     'mints that claim. The two settings are deliberately ' +
                     'separate and their disagreeing is a state to report ' +
                     'rather than one to prevent: a Verifier asking for a ' +
                     'claim nothing here issues is the only way to exercise ' +
                     'what a wallet does with a request it cannot satisfy.',
        ...
        handler: function (req, res) {
          ...
          self.sendJson(res, 200, adminViews.vpConfigJson());
      ...
      { method: 'POST', route: BASE + '/verifier-request/:action',
        ...
        handler: function (req, res) {
          ...
          const body = parseBody(req);
          const names = self.namesOf(req, body, 'claim', 'claims');
          const result = adminActions.vpConfigAction(self.withAction(req, body),
                                                     names);
          if (!result.ok) {
        ...
        actions: [
          { action: 'select', operationId: 'selectRequestedClaims',
            ...
            description: 'Reaches the wire as the `claims` member of the ' +
                         'next Authorization Request\'s dcql_query.\n\nAn ' +
                         'EMPTY array is a legitimate call and not an empty ' +
                         'form: DCQL reads an absent `claims` member as the ' +
                         'WHOLE credential, so requesting nothing asks the ' +
                         'wallet for everything — the opposite of what ' +
                         'selective disclosure is for, which is exactly why ' +
                         'being able to ask for it matters.\n\nThe claims a ' +
                         'request asks for are FROZEN onto the ' +
                         'transaction when it is built, so a change ' +
                         'here never re-judges a presentation already in ' +
                         'flight.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
                claims: { type: 'array', items: { type: 'string' },
                          description: 'Top-level claim names. A credential ' +
                                       ...
                                       'claim, so `address` is one unit ' +
                                       'however many attributes feed it.' }
              },
              required: ['claims'],
          ...
          { action: 'add', operationId: 'addRequestedClaim',
            ...
            description: 'A claim NOT in the catalogue is accepted, and is ' +
                         'the point rather than a loose end: nothing here ' +
                         'issues it, so it is the only way to exercise what ' +
                         'a wallet does with a request it cannot satisfy. ' +
                         'The reply says so, and the Verifier ' +
                         'will then refuse the presentation on ' +
            ...
            requestBody: {
              ...
              properties: {
                claim: { type: 'string' },
                name: { type: 'string',
                        description: 'An alias for `claim`; ' +
                                     '`vpConfigAction()` reads `body.claim ' +
                                     '|| body.name`, so both spellings ' +
                                     'have always worked and only one ' +
                                     'was published.' }
              },
          ...
          { action: 'remove', operationId: 'removeRequestedClaim',
            ...
            description: 'A claim that is not being asked for is refused ' +
          ...
          { action: 'defaults', operationId: 'resetRequestedClaims',
            ...
            description:
              'OID4VP_CLAIMS where that was set in the environment, ' +
          ...
          { action: 'format', operationId: 'setRequestFormat',
            ...
            description: 'Applies to a request that does not name a format ' +
                         'itself; the three format links on the Verifier ' +
                         'page name one explicitly and are ' +
                         'unaffected.\n\nThe DCQL PATH DIFFERS BY FORMAT and ' +
                         'the reply shows it: top level for dc+sd-jwt, under ' +
                         'credentialSubject for jwt_vc_json, and under the ' +
                         'vendored JSON-LD context\'s own term for ldp_vc — ' +
                         'which cannot carry every claim at all.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
      ...
      // The authorization server profiles — what each discovery document
      // publishes.
      //
      // RFC 9700 section 2.6 asks a server to publish its metadata so that
      // clients stop hard-coding security capabilities. These operations decide
      // what the published document SAYS, per authorization server, which is
      // the other side of that: a client which reads the metadata can be shown
      // reading it, and one which does not can be shown not to.
      //
      // ANY MEMBER IS ACCEPTED, including one this service has never heard of.
      // That is the difference between this and every other resource here — the
      // applications registry REFUSES an attribute outside its schema, because
      // that schema is a published contract about what an entry carries. This
      // has no schema on purpose: publishing something a client did not expect
      // is half the point of a mock.
      // ---------------------------------------------------------------------
      // THE SAML 2.0 IDENTITY PROVIDER.
      //
      // The resource a caller needs before it can drive the Web Browser SSO
      // profile at all, and the reason is the feature itself: the metadata is
      // PER SERVICE PROVIDER, so "the metadata URL" is not a constant a test
      // can hard-code — it is a per-entityID fact this reply carries. A test
      ...
      // this API takes a token a test can mint rather than a browser session,
      // so these operations are how a TEST configures a federation partner with
      // no browser and no cookie jar — which is the only way the feature can be
      // exercised automatically at all.
      //
      // The consequence is the same one mgmt-api/CLAUDE.md states for /rbac and
      // it is worth restating here because what is at stake is different:
      // anybody holding an `admin:write` token (or reaching this port with
      ...
      // That is not a new hole — the same caller can already grant themselves
      // both admin roles and mint a token for any username — but it is the
      // sharpest form of it, and the honest sentence is better than the
      // omission.
      // ---------------------------------------------------------------------
      { method: 'GET', path: BASE + '/federation', tag: 'Federation',
        ...
        description: 'This service can be EITHER END of a federation ' +
                     ...
                     'WS-Federation 1.2, OpenID Connect and OAuth ' +
                     '2.0.\n\n**This is the one feature here that has to be ' +
                     'configured before it will do anything.** Everywhere ' +
                     'else this service accepts what it is given — any ' +
                     'username, any client_id, any entityID, any LDAP bind. ' +
                     'It cannot do that at an assertion consumer service: ' +
                     'what arrives there is an unauthenticated HTTP request ' +
                     'claiming to be a person, and the session it produces ' +
                     'is the same one `/oauth2/authorize`, `/wsfed`, ' +
                     ...
                     'nothing: every row is an entry under `ou=federations`, ' +
                     'the same one an `ldapsearch` reads.',
        ...
        parameters: [
          ...
          { name: 'q', in: 'query', required: false, schema: { type: 'string' },
            description:
              'Filter the list by id, name, partner or application.' },
          { name: 'role', in: 'query', required: false,
            schema: { type: 'string',
        ...
        responseSchema:
          { $ref: '#/components/schemas/FederationRelationshipList' },
        handler: function (req, res) {
          log.debug("Entering the management API federation endpoint.");
          self.sendJson(res, 200, adminViews.federationJson(req));
        ...
        actions: [
          { action: 'create', operationId: 'createFederationRelationship',
            ...
            description: 'It is created **DISABLED**, whatever this request ' +
                         ...
                         'operation overrides its input, and it is ' +
                         'deliberate: a partner that half-exists and ' +
                         'silently accepts assertions is the failure this ' +
                         'whole register is arranged to prevent, so enabling ' +
                         'is a second act that says the configuration is ' +
                         'finished.\n\n**ONE RELATIONSHIP IS ONE ' +
                         'DIRECTION.** A partner this service both consumes ' +
                         'from and asserts to is two relationships with two ' +
                         'ids, because everything that configures one ' +
                         'differs by direction.\n\nThe reply carries ' +
                         '`readiness.missing` — the fields this protocol and ' +
                         'role still need — so a caller can go straight on ' +
                         'to `set` for each of them without knowing the ' +
                         'schema in advance.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
                id: { type: 'string',
                      description: 'The key, the RDN and a URL segment. It ' +
                                   'has to start with a letter or a digit ' +
                                   'and hold only letters, digits, dot, dash ' +
                                   'and underscore, up to 63 characters.' },
                role: { type: 'string',
                        enum: ['service-provider', 'identity-provider'],
                ...
                name: { type: 'string',
                        description:
                          'What to call the partner on a page. The ' +
                                     'id is the name when this is omitted.' },
                peer: { type: 'string',
                        description: 'The partner\'s own identifier — a SAML ' +
                                     'entityID, an OpenID Connect issuer, a ' +
                                     'WS-Federation wtrealm. On a ' +
                                     'service-provider-side relationship it ' +
                                     'is CHECKED: an assertion whose issuer ' +
                                     'is not this string is refused even ' +
                                     'when the signature verifies.' },
                application: { type: 'string',
                               description: 'On an identity-provider-side ' +
                                            'relationship only: the ' +
                                            'identifier of the partner\'s ' +
                                            'entry in `ou=applications`. Its ' +
                                            'entityID, redirect URIs and ' +
                                            'certificate stay THERE, where ' +
                                            'every protocol module reads ' +
                                            'them — this register holds a ' +
            ...
            responseDescription:
              'The relationship as created, and what it still ' +
          ...
          { action: 'set', operationId: 'setFederationField',
            ...
            description: 'The field must be one of this relationship\'s — a ' +
                         'field belonging to the other ROLE is refused by ' +
                         'name rather than written and ignored, and so is ' +
                         'one that records what HAPPENED (the counters, the ' +
                         'last error).\n\n`fedId`, `fedRole` and ' +
                         '`fedProtocol` are refused too, and that is a third ' +
                         'category rather than an oversight: they are the ' +
                         'relationship\'s identity, and changing one would ' +
                         'leave a SAML relationship carrying a token ' +
                         ...
                         '`editable`, which is exactly the list this ' +
                         'operation accepts, so a caller need not ' +
                         'guess.\n\nThe reply always carries ' +
                         '`readiness`, so setting the last ' +
              ...
              properties: {
                ...
                field: { type: 'string',
                         description: 'The attribute name, e.g. `fedSsoUrl`, ' +
                                      ...
                                      '`fedClientId`.' },
                value: { type: 'string',
                         description: 'The new value. An empty string clears ' +
                                      'it. `fedSigningCertificate` is ' +
                                      'normalised — PEM armour and ' +
                                      'whitespace are stripped, because ' +
                                      'what the schema holds is the ' +
                                      'base64 DER a ds:X509Certificate ' +
                                      'carries.' }
              },
            ...
            responseDescription: 'The relationship as it now stands, and ' +
                                 'whether it is ready.' },
          ...
          { action: 'add-value', operationId: 'addFederationValue',
            ...
            description: 'Two fields take values: `fedAttributeMap` on a ' +
                         'service-provider-side relationship, and ' +
                         '`fedRelease` on an identity-provider-side ' +
                         'one.\n\n**`fedAttributeMap`** is written ' +
                         '`<incoming name>=<LDAP attribute>` and is split at ' +
                         'the FIRST equals sign — which matters, because an ' +
                         'incoming name can be a URL and a URL can hold one. ' +
                         'It is only needed for a partner\'s own inventions: ' +
                         'the ordinary OpenID Connect claims, the SAML ' +
                         '`urn:oid:` names and the AD FS claim URIs are ' +
                         'mapped already.\n\n**`fedRelease`** names an ' +
                         'attribute or claim released to that partner, and ' +
                         'it can only REMOVE — from what /admin/claims, ' +
                         '/admin/saml-attributes and the groups claim would ' +
                         'add, and from nothing else. It cannot touch `sub`, ' +
                         '`iss`, `exp` or a NameID. **NO VALUES MEANS NO ' +
                         'POLICY, not release nothing**: adding the first ' +
                         'value here is what starts the filtering.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
              ...
              examples: [{ id: 'partner-a', field: 'fedAttributeMap',
                           value:
                             'http://partner.example/claims/dept=departmentNumber' }],
              additionalProperties: false
            },
          ...
          { action: 'remove-value', operationId: 'removeFederationValue',
            ...
            description: 'Refused if that value is not there, rather than ' +
                         'succeeding silently: a caller removing a mapping ' +
                         'it thinks exists wants to hear that it does not.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
                ...
                value: { type: 'string',
                         description:
                           'The value to remove, exactly as it stands.' }
              },
              required: ['id', 'field', 'value'],
              additionalProperties: false
            },
          ...
          { action: 'enable', operationId: 'enableFederationRelationship',
            ...
            description: 'The second, deliberate act. **It is allowed on a ' +
                         'relationship that is not fully configured**, and ' +
                         'that is not a gap: a half-configured partner is a ' +
                         'state somebody is passing through, and refusing to ' +
                         'save it would mean configuring everything in one ' +
                         'request with no way back. What happens in that ' +
                         'state is that every endpoint for the relationship ' +
                         'REFUSES and says which fields are ' +
                         'missing — it never half-works — and the ' +
          ...
          { action: 'disable', operationId: 'disableFederationRelationship',
            ...
            description: 'A response arriving for a disabled relationship is ' +
                         ...
                         'configuration, the counters and the mappings all ' +
                         'stay, and the people it authenticated keep their ' +
                         'directory entries and their sessions.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: { id: { type: 'string',
          ...
          { action: 'delete', operationId: 'deleteFederationRelationship',
            ...
            description: 'The entry goes and takes its recorded sign-ins ' +
                         'with it. **The PEOPLE it authenticated keep their ' +
                         'entries under `ou=users`** — nothing is ever ' +
                         'deleted from there — and any session they hold is ' +
                         'unaffected until it expires or is ended, which is ' +
                         'what `POST /admin-api/logout` is for.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: { id: { type: 'string',
            ...
            responseDescription: 'Confirmation, and what was deliberately ' +
                                 'left behind.' }
        ] },
      ...
      { method: 'GET', path: BASE + '/saml2', tag: 'SAML 2.0',
        ...
        responseDescription: 'The service providers with the paging that ' +
                             ...
                             'and its record.',
        responseSchema:
          { $ref: '#/components/schemas/Saml2ServiceProviderList' },
        handler: function (req, res) {
          log.debug("Entering the management API SAML 2.0 endpoint.");
          // The layer decides list-or-detail off `?sp=` exactly as the page
          // does.
          self.sendJson(res, 200, adminViews.saml2Json(req));
        ...
        actions: [
          { action: 'register', operationId: 'registerSaml2ServiceProvider',
            ...
            description: 'OPTIONAL, and it changes nothing about whether a ' +
                         'request is accepted: this identity provider ' +
                         'accepts any entityID, and the first AuthnRequest ' +
                         'or metadata fetch creates the entry anyway. What ' +
                         'registering early buys is a metadata document to ' +
                         'hand somebody before they have sent ' +
                         'anything.\n\nIt is refused for an entityID that is ' +
                         'already in the registry — an identifier ' +
                         'names ONE application here whatever ' +
                         'protocol brought it, so the answer to "it is ' +
                         'already ' +
            ...
            responseDescription: 'The application entry, and where its ' +
                                 'metadata is served.' },
          ...
          { action: 'set-logout-service', operationId: 'addSaml2LogoutService',
            ...
            description: 'A `<samlp:LogoutRequest>` CARRIES NO RETURN ' +
                         ...
                         'as one. This is how to remove the guess.\n\nIt ' +
                         'writes `samlSingleLogoutService` on the ' +
                         'application entry, so an `ldapmodify` ' +
                         'of the same attribute does ' +
                         'exactly this — two doors onto one value, not two ' +
                         'stores.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
          ...
          { action: 'remove-logout-service',
            ...
            description: 'The attribute holds a LIST, so values are removed ' +
                         'by name rather than the list being replaced. ' +
                         'Removing the last one puts the fallback back.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
                sp: { type: 'string' },
                value: { type: 'string',
                         description: 'The exact value to remove.' }
              },
              required: ['sp', 'value'],
              examples: [{ sp: 'https://sp.example.com/saml',
                           value: 'https://sp.example.com/saml/slo' }],
      ...
      { method: 'GET', path: BASE + '/saml11', tag: 'SAML 1.1',
        ...
        summary: 'Every SAML 1.1 relying party, and the endpoints each is ' +
                 'configured from',
        description: 'A full SAML 1.1 identity provider: both browser ' +
                     'profiles — Browser/POST and Browser/Artifact — and the ' +
                     'SAML responder behind the second, which also answers ' +
                     'AttributeQuery and AuthenticationQuery and is ' +
                     'therefore this service\'s attribute authority.\n\n**IT ' +
                     'IS NOT AN OLDER SPELLING OF `GET /admin-api/saml2`.** ' +
                     'SAML 1.1 has no request message, so a relying party ' +
                     'cannot identify itself in the protocol: `identifier` ' +
                     'comes from Shibboleth\'s `providerId` parameter, from ' +
                     'a scoped endpoint\'s path segment, or it is GUESSED ' +
                     'from the origin of the TARGET. It has no Single ' +
                     'Logout, so there is no logout service to declare, and ' +
                     'no request signature to record. It has an attribute ' +
                     'authority, which the 2.0 profile does not.\n\n**Every ' +
                     'relying party gets its own metadata document** and one ' +
                     'is minted for any identifier asked for, so nothing has ' +
                     'to be provisioned before a relying party can be ' +
                     'pointed at this service.\n\nThis resource holds ' +
                     'nothing: every row is an entry in `ou=applications`, ' +
                     'the same one `GET /admin-api/applications` reports — ' +
                     'and the KIND is shared with WS-Federation, because a ' +
                     'relying party handed the same assertion through the ' +
                     'passive requestor profile is the same application. ' +
                     '`profiles` says which of the two browser profiles it ' +
                     'has actually used, and an empty list means it has only ' +
                     'ever been handed a 1.1 assertion through another ' +
                     'door.\n\n`?rp=<identifier>` returns one of them, with ' +
                     'what has been recorded about it — and answers 200 with ' +
                     '`found: false` for an identifier that is not ' +
                     'registered, whose metadata is still served and whose ' +
                     'flow would still be answered.',
      ...
      { method: 'POST', route: BASE + '/saml11/:action',
        ...
        handler: function (req, res) {
          ...
          const body = parseBody(req);
          const result = adminActions.saml11Action(self.withAction(req, body));
          if (!result.ok) {
        ...
        actions: [
          { action: 'register', operationId: 'registerSaml11RelyingParty',
            ...
            description: 'OPTIONAL, and it changes nothing about whether a ' +
                         'flow is accepted: this identity provider accepts ' +
                         'any identifier, and the first flow or metadata ' +
                         'fetch creates the entry anyway.\n\nIt buys two ' +
                         'things here rather than the one it buys on the ' +
                         'SAML 2.0 side. A metadata document to hand ' +
                         'somebody before they have sent anything — and **a ' +
                         'NAME to put in `providerId`**, which matters more ' +
                         'in this protocol than in any other here: with no ' +
                         'name sent, the audience of the assertion is ' +
                         'guessed from the origin of the TARGET, and a ' +
                         'relying party expecting a different audience ' +
                         'refuses the assertion inside a signature check ' +
                         'with nothing saying why.\n\nIt is refused for an ' +
                         'identifier already in the registry — an identifier ' +
                         'names ONE application here whatever protocol ' +
                         'brought it, and a WS-Federation relying party ' +
                         'taking 1.1 assertions is already one of these.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
      ...
      { method: 'GET', path: BASE + '/authorization-servers',
        ...
        summary:
          'Every authorization server profile, and what its document says',
        description: 'One process, several authorization servers. The path ' +
                     'component the two discovery shapes already carry — RFC ' +
                     '8414 section 3.1 INSERTS it after the well-known ' +
                     'segment, OpenID Connect Discovery section 4 APPENDS ' +
                     'the well-known segment to it — now selects a ' +
                     'CONFIGURATION as well as an issuer identifier.\n\n**A ' +
                     'path nobody has configured publishes the document this ' +
                     'service always published**, so nothing that worked ' +
                     'before behaves differently.\n\nEvery reply carries ' +
                     '`drift`: the members whose published value disagrees ' +
                     'with what this service would publish, and the removals ' +
                     'that hide something real. A profile that lies is often ' +
                     'exactly what is wanted — it is how you find out ' +
                     'whether a client reads the metadata — but a mock that ' +
                     'let somebody publish a misleading document QUIETLY ' +
                     'would be a trap.\n\n`?profile=<id>` returns one of ' +
                     'them with every override, every removal and its drift.',
        mirrors: 'GET /admin/authorization-servers',
        parameters: [
          { name: 'profile', in: 'query', required: false,
            schema: { type: 'string' },
            description: 'One profile, by the path component that selects ' +
                         'it. Answers 200 with `found: false` for one that ' +
                         'is not configured — whose discovery URLs still ' +
                         'answer, with this service\'s own document.' }
        ].concat(this.pagingParameters()),
        responseDescription: 'The profiles with the paging that found them, ' +
                             'or one profile with its overrides and drift.',
        responseSchema:
          { $ref: '#/components/schemas/AuthorizationServerList' },
        handler: function (req, res) {
          log.debug("Entering the management API authorization servers " +
                    "endpoint.");
          self.sendJson(res, 200, adminViews.authorizationServersJson(req));
          log.debug("Leaving the management API authorization servers " +
                    "endpoint.");
        } },
      ...
      { method: 'POST', route: BASE + '/authorization-servers/:action',
        ...
        handler: function (req, res) {
          log.debug("Entering the management API authorization servers " +
                    "action endpoint.");
          const body = parseBody(req);
          const result = adminActions.asAction(self.withAction(req, body));
          if (!result.ok) {
        ...
        actions: [
          { action: 'create', operationId: 'createAuthorizationServer',
            ...
            description: 'The `id` is a single URL path segment, because it ' +
                         'has to appear in a discovery URL without being ' +
                         'escaped — one that had to be escaped would be one ' +
                         'nobody could find again. A new profile has no ' +
                         'overrides, so both its documents say ' +
                         'exactly what this service says about ' +
            ...
            requestBody: {
              ...
              properties: {
                id: { type: 'string',
                      description: '1-64 characters of letters, digits, dot, ' +
                                   'dash, underscore or tilde, starting with ' +
                                   'a letter or a digit.' },
                label: { type: 'string',
              ...
              examples: [{ id: 'tenant1', label: 'Tenant One',
                           description: 'advertises plain PKCE, to see what ' +
                                        'a client does' }],
              additionalProperties: false
            },
            responseDescription:
          ...
          { action: 'set', operationId: 'setAuthorizationServerMember',
            ...
            description:
              'The value is read as JSON first and as a plain string ' +
                         'if that fails, so `["S256"]` is a list, `false` is ' +
                         'a ' +
                         'boolean and `https://example.com/token` is a ' +
                         'string.\n\n**Any member name is accepted**, ' +
                         'including one this service has never heard of; the ' +
                         'catalogue in the GET reply is help for whoever ' +
                         'fills the form rather than a constraint. A member ' +
                         'that is also removed stops being removed, or the ' +
                         'call would appear to do nothing.\n\nWhat this does ' +
                         'NOT change is what the endpoints do. Advertise ' +
                         '`code_challenge_methods_supported: ["plain"]` and ' +
                         'the ' +
                         'token endpoint still verifies S256 — which is the ' +
                         'point, and is reported as drift rather than left ' +
                         'to be discovered.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
                ...
                value: { description: 'JSON if it parses as JSON, otherwise ' +
                                      'the string.' },
                id: { type: 'string',
                    description: 'An alias for `profile`; `asAction()` reads ' +
                                 '`body.profile || ' +
                                 'body.id`, so both spellings ' +
          ...
          { action: 'remove', operationId: 'removeAuthorizationServerMember',
            ...
            description: 'DIFFERENT FROM `reset`, and the difference is the ' +
                         'reason both exist: reset undoes an override and ' +
                         'this publishes an ABSENCE. A client that cannot ' +
                         'find `code_challenge_methods_supported` does not ' +
                         'learn that PKCE is unavailable ' +
                         '— it learns nothing, and RFC 9700 ' +
          ...
          { action: 'reset', operationId: 'resetAuthorizationServerMember',
            ...
            description: 'Undoes an override or a removal for a single ' +
                         'member, leaving the rest of the profile alone.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
          ...
          { action: 'delete', operationId: 'deleteAuthorizationServer',
            ...
            description: 'The two discovery URLs go on answering — with this ' +
                         'service\'s own document and the issuer taken from ' +
                         'the path — because an unconfigured path component ' +
                         'has always been served that way rather than 404\'d.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
                profile: { type: 'string' },
                id: { type: 'string',
                      description: 'An alias for `profile`; `asAction()` ' +
                                   'reads `body.profile || body.id`, so both ' +
                                   'spellings have always worked and only ' +
                                   'one was published.' }
              },
              anyOf: [{ required: ['profile'] }, { required: ['id'] }],
      ...
      //
      // The write half is NOT a third store beside the protocol endpoints and
      // LDAP: every action below calls a function in applications.js which does
      // the same read-modify-write against the same ou=applications entries, so
      // a form post and an ldapmodify are one act arriving by two routes. That
      // is what keeps the one-store rule intact with three ways in.
      //
      // What may be changed is DECLARED and not DERIVED — configuration, which
      // is what RFC 9700 mode reads, but never the counters or the sightings,
      // which are what happened. The line is drawn by applications.js's
      // EDITABLE table, so this file offers no opinion about it and the
      // console's selects are built from the same rows these actions validate
      // against.
      { method: 'GET', path: BASE + '/applications', tag: 'Applications',
        operationId: 'getApplications',
        summary: 'Every application this service has been asked about, ' +
                 'filtered and paged',
        description: 'The other side of /admin-api/users. That resource ' +
                     'lists every identity that has authenticated here; this ' +
                     'lists what they authenticated TO — every OAuth client, ' +
                     'OpenID Connect relying party, SAML 2.0 or 1.1 service ' +
                     'provider, WS-Federation application, WS-Trust relying ' +
                     'party, OpenID4VP verifier and Kerberos ' +
                     'service.\n\n**The entries ARE the registry.** They ' +
                     'live under `ou=applications` in the embedded LDAP ' +
                     'directory and nothing caches them, so an `ldapmodify` ' +
                     'is visible here on the next call — and changes what ' +
                     'RFC 9700 mode enforces at the same moment. The RFC ' +
                     '7591 client registrations are those entries ' +
                     'too.\n\n**One entry per identifier, whatever protocol ' +
                     'brought it.** The key is the identifier exactly as it ' +
                     'arrived, so an application appearing under one name in ' +
                     'two protocols is one row with two `kinds` rather than ' +
                     'two rows.\n\n`?application=<id>` returns ONE of them ' +
                     'with every attribute of its directory entry and what ' +
                     'the published schema says each attribute is; that ' +
        ...
        parameters: [
          { name: 'application', in: 'query', required: false,
            ...
            description: 'One application, by its identifier exactly — the ' +
                         ...
                         'principal name. Answers 200 with `found: false` ' +
                         'for one this service has never accepted, which is ' +
                         'a different fact from one it has refused: an entry ' +
                         'appears when an identifier ' +
                         'is ACCEPTED, so a client ' +
          ...
          { name: 'attributesPage', in: 'query', required: false,
            ...
            description: 'Which page of the attribute list, on the ' +
                         '`?application=` reply only. Named rather than the ' +
                         'bare `page` because it moves a list inside the ' +
                         'reply rather than the reply itself.' }
        ].concat(this.pagingParameters()),
        responseDescription: 'The matching applications with the paging that ' +
                             'found them, or one application with its ' +
                             'directory entry when `application` was given.',
        responseSchema: { $ref: '#/components/schemas/ApplicationList' },
        handler: function (req, res) {
          log.debug("Entering the management API applications endpoint.");
          self.sendJson(res, 200, adminViews.applicationsJson(req));
      ...
      // this gives an API client.
      //
      // THERE IS NO POST BESIDE IT, and that is rule 7 read exactly rather than
      // by shape. The rule is about CONTROLS: that page's one control posts
      // `action=create` to /admin/applications, so the operation mirroring it
      // is `createApplication` below and already exists. A second create here
      // would be two operations over one function, which is the thing the
      // parity rule is trying to prevent rather than an instance of it.
      { method: 'GET', path: BASE + '/applications/new', tag: 'Applications',
        ...
        description: 'The eight KINDS and the fourteen PROTOCOL FAMILIES a ' +
                     ...
                     'per-protocol identifiers and the redirect URIs, ' +
                     'deduped by attribute and each naming the families it ' +
                     'serves), plus the container DN a new entry would be ' +
                     'created under and how many that container will ' +
                     'hold.\n\n**It creates nothing** — the create is `POST ' +
                     '/admin-api/applications/create`. This is the list that ' +
                     'call validates against, published so that a caller ' +
                     'learns what it may send from the service rather than ' +
                     'from a copy of the list in a document.\n\n**The ' +
                     'container is THIS REALM\'S.** The embedded directory ' +
                     'is per trust realm, so ' +
                     '`/realm/acme/admin-api/applications/new` answers with ' +
                     'acme\'s `ou=applications` and an application created ' +
                     'there is invisible to every other realm — including in ' +
                     'an `ldapsearch`, which reaches it only under that ' +
                     'realm\'s base DN.\n\n**Declaring a protocol family ' +
                     'grants and refuses nothing.** No endpoint in this ' +
                     'service reads `appAllowedProtocol`: an application ' +
                     'declared for SAML 2.0 alone is still issued an access ' +
                     'token. It is a record of intent on the entry, and it ' +
                     'is deliberately not a permission — a mock that refused ' +
                     'a protocol would remove a test case rather ' +
                     'than add one. The configuration that DOES ' +
        ...
        handler: function (req, res) {
          ...
          // console's newApplicationView(), which built the whole page and
          // threw the markup away — so every call here rendered a form nobody
          // read.
          self.sendJson(res, 200, adminViews.newApplicationJson(req));
          log.debug("Leaving the management API new-application endpoint.");
        } },

      { method: 'POST', route: BASE + '/applications/:action',
        tag: 'Applications',
        // TWO CONSOLE PATHS REACH THIS SWITCH SINCE 2026-09-13: the list page,
        ...
        handler: function (req, res) {
          log.debug("Entering the management API applications action " +
                    "endpoint.");
          const body = parseBody(req);
          // The declared protocol families, in the two spellings the console
          // takes them in — `protocol` repeated, as a checkbox column posts it,
          // and one `protocols` array, as a JSON body carries it. Read through
          // listField() rather than off `body` for the reason namesOf() exists
          // at all: helpers.parseBody() builds a plain object, so a repeated
          ...
          const result = adminActions.applicationsAction(self.withAction(req,
            ...
            // For `issue-software-statement`: the issuer a statement names is
            // the one published at the address this request arrived on.
            base: baseUrlOf(req)
          });
          // `refresh-metadata` is asynchronous — it dials the service
          // provider's metadata URL — and every other action is not. See that
          // action's comment in admin.js for why one promise is cheaper than
          // forty awaits.
          if (result && typeof result.then === 'function') {
            result.then(function (answer) {
              if (!answer.ok) {
                errorCodes.mark(res,
                                errorCodes.codeOf(answer) || 'STS-API-0050');
              }
              self.sendJson(res, answer.ok ? 200 : 400, answer);
        ...
        actions: [
          { action: 'create', operationId: 'createApplication',
            ...
            description: 'An entry usually appears because an identifier was ' +
                         ...
                         'wtrealm on a sign-in response, an SPN on a ' +
                         'TGS-REP. This is how to get one in ahead of that, ' +
                         'which is what RFC 9700 mode needs if a client is ' +
                         'to be judged against its OWN redirect URIs rather ' +
                         'than against the `oauth2.redirectUris` ' +
                         'setting.\n\nIt is created with zero counters and a ' +
                         'description saying it was created by hand, so it ' +
                         'cannot be mistaken for an application that ' +
                         'turned up once and never came back. Give it its ' +
            ...
            requestBody: {
              ...
              properties: {
                identifier: { type: 'string',
                              description: 'The client_id, wtrealm, ' +
                                           'AppliesTo, entityID or service ' +
                                           'principal name. One application ' +
                                           'per identifier whatever ' +
                                           'protocol brings it, so this ' +
                                           'is refused if one is already ' +
                                           'here.' },
                name: { type: 'string',
                ...
                kind: { type: 'string',
                        description: 'Optional, one of the eight. It is a ' +
                                     ...
                                     'recorded.\n\n**No console form offers ' +
                                     'this any more** and it is still taken ' +
                                     'here. It was a select on ' +
                                     '/admin/applications/new beside the ' +
                                     'protocol families, which is two ' +
                                     'vocabularies for one question — eight ' +
                                     'kinds against fourteen families, five ' +
                                     'of them with no kind at all — and it ' +
                                     'is DERIVED rather than declared: a ' +
                                     'kind is written when a protocol ' +
                                     'actually recognises the identifier, so ' +
                                     'a form choosing one ' +
                                     'asserted a sighting that had ' +
                ...
                protocols: {
                  ...
                  description: 'THE PROTOCOL FAMILIES THIS APPLICATION IS ' +
                               ...
                               'publishes. They land on ' +
                               '`appAllowedProtocol`, and one that is not in ' +
                               'that list is REFUSED rather than recorded — ' +
                               'a typo that silently became a new family is ' +
                               'how one application comes to be declared for ' +
                               'two spellings of one thing.\n\n**IT GRANTS ' +
                               'AND REFUSES NOTHING.** Nothing in this ' +
                               'service reads the attribute: an application ' +
                               'declared for `saml2` alone is still issued ' +
                               'an access token at /oauth2/token, and one ' +
                               'declared for nothing is treated exactly as ' +
                               'it would have been. It is a record of ' +
                               'INTENT, kept apart from `appProtocol` — ' +
                               'which is what has actually happened and is ' +
                               'not editable — so the two lists on an entry ' +
                               'can be read against each other.\n\nA ' +
                               'form-encoded body may repeat `protocol` ' +
                ...
                fields: {
                  ...
                  description: 'THE ATTRIBUTES THE ENTRY IS CREATED WITH, ' +
                               'keyed by the schema\'s own attribute name ' +
                               'and valued with a string or an array of ' +
                               'strings. This is where the per-protocol ' +
                               'identifiers and the redirect URIs go — ' +
                               '`oauthClientId`, `samlEntityId`, ' +
                               '`wsfedRealm`, `krb5ServicePrincipalName`, ' +
                               '`oauthRedirectUri`, ' +
                               '`samlAssertionConsumerService`, ' +
                               '`wsfedReplyUrl` and the rest.\n\nGET ' +
                               '/admin-api/applications/new publishes the ' +
                               'list as `declarations`, with the families ' +
                               'each attribute serves and whether it holds a ' +
                               'list; it is the same walk of the protocol ' +
                               'table the console\'s form is drawn from, so ' +
                               'this document and that page cannot offer ' +
                               'different fields. GET ' +
                               '/admin/ldap/applications publishes every ' +
                               'attribute in the schema with an `editable` ' +
                               'member.\n\n**Only DECLARED attributes may be ' +
                               'given.** A derived one — a counter, a ' +
                               'sighting, `appProtocol`, ' +
                               '`appRedirectUriObserved` — is REFUSED by ' +
                               'name rather than written, because an entry ' +
                               'created with one would be asserting a past ' +
                               'it does not have. A single-valued attribute ' +
                               'given several values is refused as well, ' +
                               'rather than truncated to the first: the only ' +
                               'one you are likely to meet is ' +
                               '`oauthTlsClientAuthSubjectDn`, which an RFC ' +
                               '8705 check compares by exact string ' +
                               'equality, and quietly keeping one of two is ' +
                               'exactly the wrong answer there.\n\n**Nothing ' +
                               'given here is CHECKED except the OAuth ' +
                               'redirect URIs, and those only in RFC 9700 ' +
                               'mode.** The rest are recorded, in the way ' +
                               'being in this registry ' +
                               'at all is a record.\n\n' +
          ...
          { action: 'set', operationId: 'setApplicationAttribute',
            ...
            // THE LIST IS READ OFF THE SCHEMA rather than typed here, and this
            // and the one on `add` below were typed until 2026-08-25. Making
            // the per-protocol identifiers multi-valued moved six names from
            // this sentence to that one — and neither sentence noticed, which
            // is the whole argument: a hand-written list of what an operation
            // accepts is a second definition of the EDITABLE table, and it goes
            // wrong silently in the document a caller trusts most.
            description: 'For the attributes that hold ONE value — ' +
                         applications.editableAttributes('set')
                           .map(function (row) {
                           return '`' + row.name + '`';
                         }).join(', ') + '. An empty `value` CLEARS the ' +
                         'attribute.\n\n**What may be changed is DECLARED ' +
                         'and not DERIVED.** Configuration — what this ' +
                         'application is allowed to do, which is what RFC ' +
                         '9700 mode reads — is editable. The counters, the ' +
                         'sightings, the kinds and the protocols are what ' +
                         'HAPPENED, and are refused with a list of what is ' +
                         'not: a call that could rewrite them would make ' +
                         'this registry lie about the service\'s own ' +
                         ...
                         '— refusing them HERE is the difference between ' +
                         'offering an operation and merely not preventing ' +
                         'it.\n\n' + this.familyScopeNote() +
                         'This writes the same entry LDAP ' +
                         'writes, through the same functions, so it takes ' +
                         'effect on the very next authorization request.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
          ...
          { action: 'add', operationId: 'addApplicationValue',
            ...
            description: 'For the attributes that hold a LIST — ' +
                         ...
                         'match on the next authorization request. Note that ' +
                         'it is the REGISTERED list — ' +
                         '`appRedirectUriObserved`, which records ' +
                         'what a client actually used, is not ' +
                         'editable, because "registered" and "used" are ' +
                         'different facts and section 2.1 is entirely about ' +
                         'not confusing them.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
          ...
          { action: 'remove', operationId: 'removeApplicationValue',
            ...
            description: 'The inverse of `add`. Removing the LAST value ' +
                         'takes the attribute with it, which is what the ' +
                         'LDAP modify handler does for every other entry in ' +
                         'this directory and what an operator reading it ' +
                         'with an LDAP client will expect.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
          ...
          // operation here, and both call the one action function that page
          // posts to. `applications.returnAddressesOf()` is the rule they
          // change the answer of.
          { action: 'confirm-address',
            operationId: 'confirmApplicationReturnAddress',
            summary:
              'Confirm a return address a development-mode request recorded',
            description: 'A return address — a SAML ACS URL or `shire` on ' +
                         ...
                         '`oauthRedirectUri` — that a request NAMED while ' +
                         'the realm was in DEVELOPMENT mode is written onto ' +
                         'the entry and MARKED on `appReturnAddressObserved` ' +
                         '(`<attribute> <address>`). **PRODUCT mode refuses ' +
                         'a marked address** exactly as it refuses one that ' +
                         'is not on the entry, so a realm switched from ' +
                         'development to product does not trust what ' +
                         'development learnt.\n\nThis takes the mark OFF and ' +
                         'keeps the address, so product believes it from the ' +
                         'next request. An address that is not marked is ' +
                         'REFUSED by name rather than confirmed silently — ' +
                         'it is already registered, or it is not on the ' +
                         'entry. `add` of the same address confirms it too, ' +
                         'because an explicit write is a ' +
                         'registration.\n\nGET ' +
                         '/admin-api/applications?application=… lists the ' +
                         'marked addresses as `returnAddressesObserved`, ' +
                         'each with `trusted` saying whether THIS realm\'s ' +
                         'mode believes it. Addresses recorded before this ' +
                         'service marked sightings carry no mark and cannot ' +
                         'be told apart from registered ones.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
          ...
          { action: 'discard-address',
            ...
            summary:
              'Discard a return address a development-mode request recorded',
            description: 'The opposite answer to `confirm-address`: the ' +
                         'address was recorded from a request in DEVELOPMENT ' +
                         'mode and is NOT this application\'s, so the mark ' +
                         'on `appReturnAddressObserved` AND the address ' +
                         'itself are taken off the entry. Product mode then ' +
                         'refuses it as an address that is not there; ' +
                         'development records it again, marked, if a request ' +
                         'names it again.\n\nAn address that is not marked ' +
                         'is REFUSED rather than removed — discarding is not ' +
                         'the door for taking a REGISTERED address off an ' +
                         'entry, and `remove` is, which says so.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
          ...
          { action: 'issue-tls-client-certificate',
            ...
            summary: 'Issue an application a TLS client certificate from ' +
                     'this realm\'s CA',
            description: 'Generates a key pair and certifies it from the ' +
                         'realm\'s `tls-client` Issuing CA with ' +
                         '`clientAuth`, the application\'s identifier as the ' +
                         'common name and `urn:sts:application:<identifier>` ' +
                         'as its subjectAltName — the name RFC 8705\'s ' +
                         'IMPLICIT mapping reads, so the certificate ' +
                         'authenticates the application under ' +
                         '`tls_client_auth` with nothing registered, ' +
                         'and every access token issued on a ' +
                         'connection presenting it carries ' +
                         '`cnf["x5t#S256"]`.\n\n' +
                         '**THIS REPLY IS THE ONLY COPY OF THE PRIVATE ' +
                         'KEY.** `files` holds a PKCS#12 (base64), an ' +
                         'encrypted PKCS#8 PEM key and the PEM chain, all ' +
                         'protected by `password`, which is neither stored ' +
                         'nor audited. Refused past ' +
                         '`pki.applicationTlsClientCertificateMax` valid ' +
                         'certificates, for a key algorithm outside ' +
                         '`rsa-2048`, `rsa-3072`, `ec-p256`, `ec-p384`, and ' +
                         'in a realm with no certificate authority.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
                ...
                confirm: { type: 'string' },
                keyAlg: { type: 'string',
                          enum: ['rsa-2048', 'rsa-3072', 'ec-p256',
                                 'ec-p384'] },
            ...
            responseDescription: 'The certificate (serial, subject, ' +
                                 'thumbprint, PEM and chain, ' +
                                 'the implicit name), the three ' +
                                 'files, and the application as it now ' +
                                 'stands.' },
          { action: 'revoke-tls-client-certificate',
            ...
            description: 'Revokes a certificate ' +
                         '`issue-tls-client-certificate` issued to THIS ' +
                         'application — a serial belonging to anybody else ' +
                         'matches nothing — onto the Issuing CA\'s CRL and ' +
                         'OCSP responder. The certificate stops ' +
                         'authenticating the application at the token ' +
                         'endpoint at once. An access token already bound to ' +
                         'it stays usable until it expires: ' +
                         'a resource server checks the ' +
            ...
            responseDescription: 'The revoked certificate, the reason, and ' +
                                 'the application as it now stands.' },
          ...
          { action: 'issue-software-statement',
            ...
            summary:
              'Sign a software statement for an application, as this realm',
            description: 'Signs an RFC 7591 section 2.3 software statement ' +
                         'with this realm\'s key and writes it onto the ' +
                         'entry as `oauthIssuedSoftwareStatement`, replacing ' +
                         'the one before. The application is the software ' +
                         'PUBLISHER: hand the statement to whoever ships the ' +
                         'software, and a client presenting it at `POST ' +
                         '/oauth2/register` is registered with the members ' +
                         'it fixes — they take precedence over the ' +
                         'registration\'s own JSON (section 3.1.1), and ' +
                         'whatever the statement does not fix, the client ' +
                         'chooses.\n\n**WHAT IS SIGNED:** `metadata` (RFC ' +
                         '7591 client metadata, e.g. `redirect_uris`, ' +
                         '`grant_types`, `token_endpoint_auth_method`), with ' +
                         '`software_id` defaulting to the application\'s ' +
                         'identifier; `iss` is the issuer this realm ' +
                         'publishes at the address THIS REQUEST arrived on, ' +
                         '`sub` the application, `iat`, a `jti`, and `exp` ' +
                         'after `lifetimeSeconds` ' +
                         '(`oauth2.softwareStatementLifetimeS` when omitted; ' +
                         '0 for none). The header is `typ: ' +
                         'software-statement+jwt`, which is how this service ' +
                         'tells its statements from its other JWTs. A JWT ' +
                         'claim or a member only registration assigns ' +
                         '(`client_id`, the secret, the registration access ' +
                         'token) is refused, and so is an address a ' +
                         'registration would refuse.\n\n**IT IS A TRUSTED ' +
                         'STATEMENT EVERYWHERE IN THIS REALM**: nothing has ' +
                         'to be declared for it, and where ' +
                         '`oauth2.softwareStatementOpensRegistration` is on ' +
                         'it admits a client to an endpoint closed to ' +
                         'everybody else (product mode with ' +
                         '`oauth2.openRegistration` off). It stops verifying ' +
                         'when it expires or when the realm\'s signing key ' +
                         'is replaced — in development mode, every restart. ' +
                         'NOT A SECRET: a statement ships with the software.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
                application: { type: 'string',
                               description: 'The publisher\'s identifier, ' +
                                            'exactly as the registry holds ' +
                                            'it.' },
                metadata: { oneOf: [{ type: 'object' }, { type: 'string' }],
                            description: 'The client metadata the statement ' +
                                         'fixes, as an object or as JSON ' +
                                         'text.' },
                lifetimeSeconds: { oneOf: [{ type: 'integer' },
                                           { type: 'string' }],
                                   description: 'Seconds until `exp`; 0 for ' +
                                                'no expiry. Defaults to ' +
                                                'oauth2.' +
                                                'softwareStatementLifetimeS' +
                                                '.' }
              },
              required: ['application'],
            ...
            responseDescription: 'The statement in `softwareStatement`, its ' +
                                 'signed `claims`, and the application as it ' +
                                 'now stands.' },
          ...
          { action: 'revoke-registration',
            ...
            description: 'RFC 7592\'s delete reached from here instead of ' +
                         'from the client that holds the registration access ' +
                         'token — the same function, so ' +
                         'the outcome is the same one ' +
                         'rather than a second reading of what ' +
                         '"unregistered" ' +
                         'means.\n\n**The ENTRY stays**, with everything it ' +
                         'had recorded; the `client_secret`, the ' +
                         'registration access token and the registration ' +
                         'document go. Losing that an application was ever ' +
                         'here because its registration was withdrawn would ' +
                         'be losing the fact rather than the ' +
                         'configuration.\n\nAfterwards RFC 9700 mode treats ' +
                         'it as an unregistered, PUBLIC client: PKCE is ' +
                         'required of it, its secret is no longer checked, ' +
                         'and its redirect_uri is judged against the ' +
                         '`oauth2.redirectUris` setting rather than against ' +
                         'its own list.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
          ...
          { action: 'refresh-metadata',
            ...
            description: 'Dials the `samlSpMetadataUrl` ON THE ENTRY — never ' +
                         ...
                         'sign-in waits on somebody else\'s web server.\n\nA ' +
            ...
            requestBody: {
              type: 'object',
              description:
                'The application whose metadata should be refreshed.',
              properties: { application: { type: 'string' } },
              required: ['application'],
              examples: [{ application: 'https://sp.example.com' }],
              additionalProperties: false
            },
            responseDescription: 'What the document said: its entityID, ' +
                                 'which KeyDescriptor the certificate came ' +
                                 'from, and the endpoints it describes.' },

          { action: 'load-resource-metadata',
            operationId: 'loadProtectedResourceMetadata',
            summary: 'Read an RFC 9728 protected resource metadata document ' +
                     'and answer with the application it describes',
            description: 'Give ONE of `document` (the JSON, as text or as an ' +
                         'object) or `url` (where it is published). It is ' +
                         'parsed and checked, compared with this trust ' +
                         'realm, and answered with `plan`: the application ' +
                         'it describes — `name`, `oauthPermissionBaseUri` ' +
                         'and `oauthAudience` from `resource`, one ' +
                         'permission per `scopes_supported` value with the ' +
                         'resource prefix taken off, a client_id generated ' +
                         'at random, and `oauth2` as the declared ' +
                         'family.\n\n**IT CREATES NOTHING.** Create the ' +
                         'application with `create`, passing those values in ' +
                         '`fields` together with `oauthResourceMetadata` ' +
                         '(the document, as JSON) and, where it was fetched, ' +
                         '`oauthResourceMetadataUrl`. Every value may be ' +
                         'changed first — this answer is a ' +
                         'proposal.\n\n`authorizationServers` compares the ' +
                         'document\'s `authorization_servers` with the ' +
                         'issuers this realm\'s authorization servers ' +
                         'publish at the address this request arrived on: ' +
                         '`allMatched` when every one is this realm\'s, ' +
                         ...
                         'link-local or reserved address, and a document ' +
                         'whose `resource` is not the identifier its ' +
                         'well-known URL was built from (RFC 9728 section ' +
          ...
          { action: 'forget', operationId: 'deleteApplication',
            ...
            description: 'THE ONE OPERATION HERE THAT LOSES A FACT, which is ' +
                         'why it is separate from `revoke-registration` ' +
                         'rather than something that one does as well. The ' +
                         'entry goes and takes its counters, its sightings ' +
                         'and its attributes with it — for a client_id ' +
                         'somebody typed wrong, or a realm from a test that ' +
                         'is over.\n\nIt will reappear, EMPTY, the next time ' +
                         'that identifier is accepted by a protocol: ' +
                         'this registry records what this service has ' +
      ...
      // The audit log. READ ONLY, and that is a decision rather than an
      // operation nobody got round to. Every other resource here has a POST
      // beside it because the console control it mirrors is a form; this one
      // mirrors a page with no form on it, because a clear button on an
      // administrative console would make an audit log unable to answer the one
      // question it exists for. There is nothing to change, so there is nothing
      // to document as changeable. --- Shared Signals
      // -----------------------------------------------------
      //
      // A GET and a POST, and unlike SCIM's the POST is not optional:
      // /admin/ssf has FOUR CONTROLS on it — set a status, transmit an event,
      // delete a stream, clear what has been received — and rule 7 is about
      // controls. Every one of them calls the same function the console's form
      // posts to, with `action` taken from the URL instead of from a hidden
      // input.
      //
      ...
      // SERVICE WILL DIAL, and the one place that URL may come from is a
      // receiver that authenticated at `POST /ssf/stream` and asked. A
      // management API that could mint one would be a second door onto the
      // outbound request `ssf/ssf_http.ts` spends its header bounding, and the
      // one with the weaker credential (it read "ungated door" until
      // 2026-09-09) — so the console has no create form either, and the parity
      // holds because there is no control to mirror.
      // ---------------------------------------------------------------------
      ...
      //
      // RULE 7 IS WHY THIS IS IN THE SAME COMMIT AS THE PAGES. Every control
      // the console grows owes an operation here, and the one that is easiest
      // to forget is not the protocol's own endpoint — it is exactly this.
      { method: 'GET', path: BASE + '/xacml', tag: 'XACML',
        operationId: 'getXacml',
        summary: 'The Policy Decision Point: whether it is on, what it ' +
                 'decides with, and where the attributes come from',
        description: 'Everything /admin/xacml draws, as JSON.\n\nTHIS IS THE ' +
                     'ONLY FAMILY ON THIS SERVICE THAT ANSWERS A QUESTION ' +
                     'ABOUT SOMEBODY ELSE\'S BOUNDARY. Every other protocol ' +
                     'here authenticates or provisions a person; this one is ' +
                     'handed a subject who was authenticated somewhere else ' +
                     'and asked whether they may.\n\n`pepBias` is the ' +
                     'embedded Policy ENFORCEMENT point\'s setting and not ' +
                     'the PDP\'s. Deny-biased and permit-biased agree on ' +
                     'every Permit and every Deny and differ ' +
        ...
        handler: function (req, res) {
          ...
          self.sendJson(res, 200, adminViews.xacmlView(req));
      ...
      { method: 'GET', path: BASE + '/xacml/policies', tag: 'XACML',
        ...
        description: 'Everything /admin/xacml/policies draws.\n\nTHE STORE ' +
                     'IS `ou=policies` IN THE EMBEDDED DIRECTORY — that ' +
                     'container IS the repository rather than a copy of one, ' +
                     'so an `ldapmodify` of `xacmlPolicyDocument` changes ' +
                     'what the PDP decides on the next request.\n\nEXACTLY ' +
                     'ONE POLICY IS THE ROOT. A PDP evaluates one document ' +
                     'and reaches the rest through PolicyIdReference, so the ' +
                     'root is where evaluation starts; a repository with ' +
                     'none decides nothing and reports `root: ' +
                     'null`.\n\n`templates` is what POST ' +
                     '/admin-api/xacml/create-from-template will build, with ' +
        ...
        handler: function (req, res) {
          ...
          self.sendJson(res, 200, adminViews.xacmlPoliciesView(req));
      ...
      { method: 'GET', path: BASE + '/xacml/editor', tag: 'XACML',
        ...
        summary: 'One policy as an editable tree, with what may legally be ' +
                 'added at each node',
        description: 'Everything /admin/xacml/editor draws. `?policy=<name>` ' +
                     'chooses one; without it, the root.\n\nEach node in ' +
                     '`tree` carries `options.additions` — the elements ' +
                     'XACML allows AT THAT POINT, computed against the real ' +
                     'function library by the same code that validates the ' +
                     'result. That is what makes this usable as an API and ' +
                     'not only as a page: a caller can walk ' +
                     'the tree, read the legal moves, and POST ' +
                     'one, without a second copy of the grammar.',
        mirrors: 'GET /admin/xacml/editor',
        responseDescription:
          'The policy as a tree of editable nodes, with the ' +
        ...
        handler: function (req, res) {
          ...
          self.sendJson(res, 200, adminViews.xacmlEditorView(req));
      ...
      { method: 'GET', path: BASE + '/xacml/decide', tag: 'XACML',
        ...
        description: 'Everything /admin/xacml/decide draws. `subject`, ' +
                     '`action` and `resource` are query parameters; with ' +
                     'none of them it answers `asked: false` rather than ' +
                     'deciding about nobody.\n\nTHE DECISION AND THE ' +
                     'ENFORCEMENT ARE TWO DIFFERENT ANSWERS and both are ' +
                     'here, which is the whole point of the page: the PDP ' +
                     'says Permit, Deny, NotApplicable or Indeterminate, and ' +
                     'the PEP then applies its bias and the ' +
                     'obligation rule to get to allowed or ' +
                     'refused. When a policy "is not working" it is nearly ' +
                     'always because only one of those was being looked ' +
                     'at.\n\nThis is NOT POST /xacml/pdp. That endpoint ' +
                     'takes a JSON Profile request and is what a PEP calls; ' +
                     'this builds a three-category request out of three ' +
                     'parameters and is what a person asks.',
        mirrors: 'GET /admin/xacml/decide',
        responseDescription: 'The decision, what applied, and what the ' +
                             'embedded PEP would do.',
        responseSchema: { $ref: '#/components/schemas/XacmlDecision' },
        handler: function (req, res) {
          log.debug("Entering the management API XACML decision endpoint.");
          self.sendJson(res, 200, adminViews.xacmlDecideView(req));
      ...
      // repository — what policies exist, what one says, what the PDP would
      // decide about a subject you name. This one answers what is actually
      // HAPPENING, and it is the operation somebody reaches for when
      // authorization is misbehaving rather than when it is being set up.
      //
      // NO POST BESIDE IT, and that is rule 7 read exactly rather than by
      // shape: the page it mirrors has no control. A reset was refused rather
      // than forgotten — a console that could zero its own monitoring would
      // make every number on it a number somebody might have zeroed, and the
      // audit log, which is the durable record, cannot be reset either.
      // -----------------------------------------------------------------------
      // GNAP (RFC 9635 + RFC 9767), 2026-09-12. Three operations mirroring the
      // two pages `gnap/gnap_admin.ts` draws and the one form on the first of
      // them. Both doors call `gnap/gnap_console.ts`, the view and action layer
      // of this family, so a page and its operation cannot disagree (rule 7).
      // REQUIRED LAZILY: this module is 19 in the require order and GNAP is
      // 23d, and every module that file loads is a library — but a lazy require
      // keeps that true by construction rather than by a reading of today's
      // require graph.
      // -----------------------------------------------------------------------
      { method: 'GET', path: BASE + '/gnap', tag: 'GNAP',
        operationId: 'getGnap',
        ...
        description: 'Everything /admin/gnap draws. The endpoints of this ' +
                     ...
                     'RS-facing API (RFC 9767); the section 9 capabilities ' +
                     'of the default authorization server and of every named ' +
                     'one, after that profile\'s GNAP overrides — which are ' +
                     'what each grant endpoint ENFORCES; the five token ' +
                     'formats and the public material that verifies the ' +
                     'self-contained ones; the grants this realm holds ' +
                     '(paged, `grantsPage`, and filtered by `state`); the ' +
                     'registered resource sets (paged, `resourcesPage`); ' +
                     'and the `gnap.*` settings, which are ' +
        ...
        handler: function (req, res) {
          ...
          self.sendJson(res, 200, loadGnapConsole().gnapView(req));
      ...
      { method: 'GET', path: BASE + '/gnap/monitor', tag: 'GNAP',
        ...
        description: 'Everything /admin/gnap/monitor draws: one row per ' +
                     ...
                     'counted — client instances and resource servers — with ' +
                     'the grants it holds by state, its live tokens, and its ' +
                     'counters since the process started: grants requested, ' +
                     'approved and denied, tokens issued by format, ' +
                     'rotations, revocations, failed key proofs, ' +
                     'introspections, registrations, derivations, and the ' +
                     'GNAP error codes it was answered with. ' +
                     'Per trust realm, and with no reset: the ' +
        ...
        handler: function (req, res) {
          ...
          self.sendJson(res, 200, loadGnapConsole().gnapMonitorView(req));
      ...
      { method: 'POST', route: BASE + '/gnap/:action', tag: 'GNAP',
        ...
        handler: function (req, res) {
          ...
          const result = loadGnapConsole().gnapAction(
              self.withAction(req, parseBody(req)),
                                                       { via: 'api',
                                                         req: req });
          if (!result.ok) {
        ...
        actions: [
          { action: 'revoke-grant', operationId: 'revokeGnapGrant',
            summary: 'Revoke a GNAP grant, as its client could (RFC 9635 ' +
                     'section 5.4)',
            description: 'Finalizes the grant and revokes every access token ' +
                         'issued under it, through the same path the ' +
                         'client\'s own DELETE on the continuation URI takes ' +
                         '— so the client sees exactly what it would have ' +
          ...
          { action: 'delete-resource-set', operationId: 'deleteGnapResourceSet',
            ...
            description: 'Removes the resource set, after which its ' +
                         'reference no longer resolves in a grant request. ' +
                         'Tokens already issued keep the rights they carry.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
      ...
      { method: 'GET', path: BASE + '/xacml/monitor', tag: 'XACML',
        ...
        description: 'Everything /admin/xacml/monitor draws: the global ' +
                     'figures — policies, enforcement points, decisions, ' +
                     'allows, declines — and then every PEP with its own ' +
                     'counts, EMBEDDED and REMOTE in one list.\n\n**A ' +
                     'DECISION IS NOT AN ENFORCEMENT.** XACML has four ' +
                     'decisions (Permit, Deny, NotApplicable, Indeterminate) ' +
                     'and a PEP has two outcomes, and what maps between them ' +
                     'is the PEP\'s BIAS: a deny-biased PEP refuses a ' +
                     'NotApplicable that a permit-biased one allows, from ' +
                     'the same decision on the same request. An obligation ' +
                     'the PEP cannot discharge also turns a Permit into a ' +
                     'refusal (section 7.2) — the one enforcement outcome ' +
                     'that looks like a bug from the client side and is the ' +
                     'specification working. So `allowed` is not `permit`, ' +
                     'and both are reported.\n\n**WHAT THIS SERVICE SAW IS ' +
                     'NOT WHAT IT WAS TOLD.** `decisions.here` was counted ' +
                     'by this process as it happened; `decisions.remote` is ' +
                     'what registered PEPs REPORT on their heartbeats, ' +
                     'cumulative in their own memory, and a PEP that ' +
                     'restarts makes it go down. `decisions.combined` adds ' +
                     'the two, which is the figure a deployment wants and is ' +
                     'arithmetic over two kinds of evidence rather than ' +
                     'a measurement.\n\n**THE EMBEDDED PEPS ARE NOT ' +
                     '"REGISTERED" AND CANNOT BE.** They are compiled into ' +
                     'this ' +
                     'process, so their existence is a fact about the build. ' +
                     'A remote PEP registers because it has no other way to ' +
                     'be known about — and even that is not a permission: an ' +
                     'unregistered PEP can pull GET /xacml/pep/policies and ' +
                     'enforce perfectly, and appears here nowhere. This is ' +
                     'every enforcement point the service KNOWS ABOUT, which ' +
                     'is a smaller claim than every one that exists.\n\nThe ' +
                     'counters are IN MEMORY, start with the process ' +
                     '(`since`) and are per trust realm. The durable record ' +
                     'of a refusal is GET /admin-api/audit, which has the ' +
                     'reason as well as the count.',
        mirrors: 'GET /admin/xacml/monitor',
        responseDescription: 'The global figures, and one row per ' +
                             'enforcement point.',
        responseSchema: { $ref: '#/components/schemas/XacmlMonitor' },
        handler: function (req, res) {
          log.debug("Entering the management API XACML monitor endpoint.");
          self.sendJson(res, 200, adminViews.xacmlMonitorView(req));
      ...
      { method: 'GET', path: BASE + '/xacml/peps', tag: 'XACML',
        ...
        description: 'Everything /admin/xacml/peps draws.\n\nA REMOTE PEP ' +
                     'runs in another process, holds its own copy of the ' +
                     'engine and PULLS the enabled policies from GET ' +
                     '/xacml/pep/policies. The pull is the contract; the ' +
                     'nudge this service sends to `notifyUrl` when the ' +
                     'repository changes is an optimisation over the polling ' +
                     'interval and never a replacement for it, so a PEP that ' +
                     'is never nudged still converges.\n\nREGISTERING IS NOT ' +
                     'A PERMISSION. An unregistered PEP can pull and enforce ' +
                     'exactly as well — GET /xacml/pep/policies requires no ' +
                     'credential, because a policy is a rule and a rule ' +
                     'nobody can read is a rule nobody can check. What a row ' +
                     'buys is this listing and an address for the ' +
                     'nudge.\n\n`current` is a COMPARISON this service ' +
                     'performs between `syncToken` on the row and the ' +
                     'repository\'s own, not a claim the PEP makes; `stale` ' +
                     'is how long since it was last heard from against ' +
                     'xacml.pepStaleAfterS and changes nothing this service ' +
                     'does. The decision counts are the PEP\'s own, ' +
                     'cumulative in its process — this service saw none of ' +
                     'those decisions, which is what a remote PEP is — so a ' +
                     'PEP that restarts makes them go down.',
        ...
        handler: function (req, res) {
          ...
          self.sendJson(res, 200, adminViews.xacmlPepsView(req));
      ...
      { method: 'POST', route: BASE + '/xacml/:action', tag: 'XACML',
        ...
        handler: function (req, res) {
          ...
          const body = parseBody(req);
          // SETTLED EITHER WAY: `issue-pep-certificate` answers a promise and
          // the other actions answer a result (see `xacmlAction()`), and a
          // rejection is a defect here rather than something a request can
          // cause — so it is a 500 naming the message rather than an unhandled
          // rejection that hangs the request.
          Promise.resolve(adminActions.xacmlAction(self.withAction(req, body)))
            .then(function (result) {
              if (!result.ok) {
                errorCodes.mark(res,
                                errorCodes.codeOf(result) || 'STS-API-0051');
              }
              // THE PRIVATE KEY IN AN ISSUE REPLY IS THE POINT OF THE CALL,
              // handed over once — and `sendJson()` already answers `no-store`,
              // which is what keeps it out of every cache between here and the
              // caller.
              self.sendJson(res, result.ok ? 200 : 400, result);
              log.debug("Leaving the management API XACML action endpoint.");
            }, function (e) {
              log.error(errorCodes.tag('STS-XACML-0072') +
                        'admin_api: an XACML ' +
        ...
        actions: [
          { action: 'create-from-template', operationId: 'createXacmlPolicy',
            ...
            description: 'A template is a working, valid, evaluable policy ' +
                         'in a shape people actually write — the first ' +
                         'twenty clicks of the editor already made. ' +
                         '`template` names one (see GET ' +
                         '/admin-api/xacml/policies), `name` names the ' +
                         'directory entry, and each parameter is sent as ' +
                         '`p_<name>`; anything omitted takes the template\'s ' +
                         'own default, so a call with only `template` ' +
                         'produces the documented example.\n\n`blank` is the ' +
                         'one template that is not an example: it builds an ' +
                         'EMPTY Policy, or an empty PolicySet with ' +
                         '`p_kind=policyset` — which is the only way to ' +
                         'create one of those without ALFA. An empty ' +
                         'deny-unless-permit document DENIES rather than ' +
                         'answering NotApplicable, so build it before making ' +
                         'it the root.\n\nA template BUILDS THE MODEL and ' +
                         'the writer serializes it, rather than substituting ' +
                         'into XML text — so a role called `a"b` cannot ' +
                         'produce ' +
                         'a document that will not parse.\n\nThe FIRST ' +
                         'policy in an empty repository ' +
                         'becomes the root, because a ' +
                         'repository with a policy and no root decides ' +
                         'nothing.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
                ...
                name: { type: 'string',
                          description: 'Names the DIRECTORY ENTRY. The ' +
                                       'PolicyId inside the ' +
          ...
          { action: 'import-alfa', operationId: 'importXacmlAlfa',
            ...
            description: 'ALFA — the Abbreviated Language For Authorization ' +
                         '— is the readable syntax for XACML: forty lines of ' +
                         'XML are eight of ALFA. Send it as `alfa` and a ' +
                         'name as `name`.\n\nIT IS PARSED, CONVERTED AND ' +
                         'STORED AS XACML XML. The repository holds ONE ' +
                         'representation, because two would be two documents ' +
                         'that could disagree — GET /admin-api/xacml/editor ' +
                         'renders the ALFA back from the model whenever you ' +
                         'want to read it.\n\nEVERY ATTRIBUTE MUST BE ' +
                         'DECLARED BEFORE IT IS USED. That is ALFA\'s own ' +
                         'rule and it is the most useful refusal in the ' +
                         'parser: a typo in an attribute name is otherwise a ' +
                         ...
                         'DRAFT, not a ratified standard — no conformance ' +
                         'suite, no schema, no second implementation to ' +
                         'disagree with. The contract offered is the one ' +
                         'that can be kept: anything this ' +
                         'service EMITS as ALFA it reads back, and ' +
            ...
            requestBody: {
              ...
              properties: {
                alfa: { type: 'string',
                          description: 'The ALFA source. Parsed, converted ' +
                                       'and stored as XACML XML — the ' +
                                       'repository holds ONE representation.' },
                name: { type: 'string',
          ...
          { action: 'disable', operationId: 'disableXacmlPolicy',
            ...
            description: 'The point of having this separate from delete: a ' +
                         'disabled policy is still there to be read, edited ' +
                         'and put back, which is what you want while working ' +
                         'on it — the editor is LIVE, ' +
                         'so a policy being edited is a ' +
          ...
          { action: 'set-root', operationId: 'setXacmlRootPolicy',
            ...
            description: 'Clears the flag on the incumbent first, because ' +
                         'the store refuses a second root. Done in that ' +
                         'order deliberately: a failure then leaves the ' +
                         'repository with NO root rather than with two, ' +
                         'which is the recoverable one of the two bad states.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
          ...
          { action: 'delete', operationId: 'deleteXacmlPolicy',
            ...
            description: 'Deleting the ROOT leaves the repository deciding ' +
                         'nothing, and the reply says so rather than letting ' +
                         'it be discovered by a Deny.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
          ...
          { action: 'edit-policy', operationId: 'editXacmlPolicy',
            summary:
              'Change a policy\'s id, description or combining algorithm',
            ...
            requestBody: {
              ...
              properties: {
                policy: { type: 'string',
                          description: 'The directory entry name of the ' +
                                       'policy to edit — the ' +
                                       '`cn` under ou=policies, ' +
                                       'not the PolicyId inside the ' +
                                       'document.' },
                path: { type: 'string',
          ...
          { action: 'add-rule', operationId: 'addXacmlRule',
            ...
            description: 'Every element this editor adds arrives COMPLETE ' +
                         'AND VALID — a new rule has a Target, an Effect and ' +
                         'an id. An editor that produced half-built elements ' +
                         'would hold a document that could not be saved, and ' +
                         'a document that cannot be saved ' +
                         'cannot be evaluated, which is ' +
          ...
          { action: 'edit-match', operationId: 'editXacmlMatch',
            ...
            description: 'THE DATATYPE FOLLOWS THE FUNCTION. A Match whose ' +
                         'literal is a string and whose designator is an ' +
                         'integer does not type-check, so choosing the ' +
                         'function sets both sides rather than letting them ' +
                         'be picked independently and refused later.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
                ...
                matchId: { type: 'string',
                          description: 'The match function. Defaults to ' +
                                       'string-equal. THE DATATYPE FOLLOWS ' +
                                       'IT on both sides, so choosing the ' +
                                       'function settles the literal\'s type ' +
                                       'and the designator\'s together.' },
                value: { type: 'string',
                ...
                attributeId: { type: 'string',
                          description: 'The attribute to read. A bare name ' +
                                       'or the `urn:sts:xacml:attribute:` ' +
                                       'prefix reaches the ' +
                                       'directory through the PIP; ' +
                                       'anything else must be in the ' +
                                       'request.' },
          ...
          { action: 'remove', operationId: 'removeXacmlNode',
            ...
            description: 'By `path`, which is the address GET ' +
                         '/admin-api/xacml/editor gave you. A path is only ' +
                         'valid against the document it was read from: ' +
                         'remove rule 0 and every path naming ' +
                         'rule 1 now means rule 0. Re-read ' +
          ...
          // same commit, and `tests/vendored/sts_admin_api_operations.js` is
          // the check — it compares the actions this document DECLARES against
          // the ones the handler's refusal sentence NAMES, in both directions,
          // because that sentence is what `tests/vendored/admin_api.js`'s
          // parity check reads to find out what a resource can do. Nineteen
          // actions the sentence named and the document did not meant nineteen
          // console controls that could have lost their operation with nothing
          // failing.
          //
          ...
          // and every path naming rule 1 now means rule 0. Re-read the tree
          // after each edit rather than reusing paths.
          // ---------------------------------------------------------------
          { action: 'add-target-anyof', operationId: 'addXacmlTargetClause',
            summary: 'Add a Target clause to a policy, a rule or a Target',
            description: 'A `<Target>` is a list of clauses and **every one ' +
                         'of them must match** — they are ANDed. Inside a ' +
                         'clause the alternatives are ORed and inside an ' +
                         'alternative the Matches are ANDed again, so the ' +
                         'quantifier flips at each of the three ' +
                         'levels.\n\nThat is why the element names read ' +
                         'backwards from what they do: `AnyOf` holds `AllOf` ' +
                         'holds `Match`, and the `AnyOf`s themselves are ' +
                         'ANDed. Getting it wrong produces a ' +
                         'policy that applies to more or less than ' +
          ...
          { action: 'add-allof', operationId: 'addXacmlAlternative',
            ...
            description: 'ANY of a clause\'s alternatives matching satisfies ' +
                         'it — they are ORed. Within one alternative every ' +
                         'Match must hold.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
          ...
          { action: 'add-match', operationId: 'addXacmlMatch',
            ...
            description: 'One attribute tested against one value. It arrives ' +
                         ...
                         'half-built elements would hold a document that ' +
                         'cannot be saved, and a document that cannot be ' +
                         'saved cannot be evaluated, which ' +
                         'is when you most want to look at ' +
          ...
          { action: 'edit-rule', operationId: 'editXacmlRule',
            ...
            description: 'The EFFECT is the consequential one: it is what ' +
                         'this rule contributes to the combining algorithm ' +
                         'when its Target and Condition hold, and flipping ' +
                         'it between Permit and Deny ' +
                         'changes what the policy decides ' +
          ...
          { action: 'add-condition', operationId: 'addXacmlCondition',
            ...
            description: 'A boolean expression evaluated AFTER the Target ' +
                         'matches. **At most one per rule** — the schema ' +
                         'allows exactly one, and the editor stops offering ' +
                         'this on a rule that has one rather than offering ' +
                         'it and then refusing, because an option that is ' +
                         'offered and refused teaches a caller that the menu ' +
                         'is not to be trusted.\n\nIt must evaluate to ' +
                         'EXACTLY ONE BOOLEAN. A condition returning a bag, ' +
                         'or an integer, is a static type error and the ' +
                         'policy is refused at write time rather than going ' +
                         'Indeterminate on every request.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
          ...
          { action: 'set-expression-apply', operationId: 'setXacmlApply',
            ...
            description: 'Replaces whatever is at `path` with an `<Apply>` ' +
                         'of the function named in `functionId`, with ' +
                         'arguments the function\'s own signature requires. ' +
                         'Every expression slot in a policy takes any of the ' +
                         'four shapes, which is why these are four actions ' +
                         'on one target rather than four kinds of node.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
                ...
                functionId: { type: 'string',
                          description: 'The function to apply. Its arguments ' +
                                       'are PRE-BUILT to the declared arity ' +
                                       'and types, so the expression ' +
                                       'typechecks the moment it exists.' }
              },
              required: ['policy', 'path'],
              examples: [{
                policy: 'my-policy',
          ...
          { action: 'set-expression-value', operationId: 'setXacmlValue',
            ...
            description: 'An `<AttributeValue>` of `dataType` with `lexical` ' +
                         'in it. The lexical form is parsed against the ' +
                         'datatype when the policy is written, so ' +
                         '`2026-13-01` as a date is refused there rather ' +
                         'than becoming an Indeterminate at decision time.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
                ...
                type: { type: 'string',
                          description:
                            'The datatype URI. Defaults to xs:string.' },
                lexical: { type: 'string',
                          description: 'The value, in that datatype\'s ' +
                                       'lexical form.' }
              },
              required: ['policy', 'path'],
              examples: [{
          ...
          { action: 'set-expression-designator',
            ...
            description: 'An `<AttributeDesignator>`: reads an attribute out ' +
                         'of the REQUEST, or out of the directory through ' +
                         'the PIP when the id is a bare name or carries the ' +
                         '`urn:sts:xacml:attribute:` prefix.\n\n**IT ' +
                         'RETURNS A BAG**, always, even when it finds ' +
                         'exactly one value — which is why ' +
                         '`string-one-and-only` exists and why most ' +
                         'functions need it wrapped. Nothing in this engine ' +
                         'ever holds a bare value, so there is no code path ' +
          ...
          { action: 'set-expression-variable', operationId: 'setXacmlVariable',
            ...
            description: 'A `<VariableReference>` to a ' +
                         '`<VariableDefinition>` in the same policy. The ' +
                         'reference is checked at write time, so ' +
                         'one naming a variable that is not there is ' +
            ...
            requestBody: {
              ...
              properties: {
                ...
                variableId: { type: 'string',
                          description: 'The VariableDefinition to reference. ' +
                                       'It must exist in ' +
                                       'the same policy — the ' +
          ...
          { action: 'add-argument', operationId: 'addXacmlArgument',
            ...
            description: 'Offered only on an `<Apply>` — every other ' +
                         'expression is a leaf and may only be REPLACED, ' +
                         'which the four set-expression actions already do. ' +
                         'The argument arrives as a literal of the type the ' +
                         'function expects in that position.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
          ...
          { action: 'edit-apply', operationId: 'editXacmlApply',
            ...
            description: 'THE ARGUMENTS ARE RESHAPED TO THE NEW SIGNATURE. A ' +
                         ...
                         'two integers cannot keep its arguments, and ' +
                         'leaving them would hold a document that does not ' +
                         'typecheck and therefore cannot be saved.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
          ...
          { action: 'edit-value', operationId: 'editXacmlValue',
            ...
            description: 'The lexical form is parsed against the datatype, ' +
                         'so a value that is not of its type is refused here ' +
            ...
            requestBody: {
              ...
              properties: {
                ...
                lexical: { type: 'string',
                          description: 'A new lexical form, parsed against ' +
                                       'that datatype.' }
              },
              required: ['policy', 'path'],
              examples: [{
          ...
          { action: 'edit-designator', operationId: 'editXacmlDesignator',
            ...
            description: 'Its category, attribute id, datatype and ' +
                         ...
                         'surprises people**: with it false an absent ' +
                         'attribute is an empty bag, and with it true the ' +
                         'whole expression is Indeterminate ' +
                         '— which the combining ' +
            ...
            requestBody: {
              ...
              properties: {
                ...
                dataType: { type: 'string',
                          description: 'A new datatype URI.' },
                mustBePresent: { type: 'boolean',
                          description: 'False makes an absent attribute an ' +
                                       'empty bag; true makes the whole ' +
                                       'expression Indeterminate, which the ' +
                                       'combining algorithms treat quite ' +
                                       'differently from a Deny.' }
              },
              required: ['policy', 'path'],
              examples: [{
                policy: 'my-policy',
          ...
          { action: 'add-rule-obligation',
            operationId: 'addXacmlRuleObligation',
            summary: 'Add an obligation to a rule',
            description: 'Fires when this RULE\'s Effect is the decision. An ' +
                         'obligation is the half of a decision that says ' +
                         '"yes, ' +
                         'AND you must also do this", and section 7.2 makes ' +
                         'it ' +
                         'binding: a PEP that cannot discharge one MUST NOT ' +
                         'grant the access. Both PEPs here implement that — ' +
                         'the embedded one at /xacml/protected and the ' +
                         'remote container — and it is the part ' +
              ...
              properties: {
                ...
                on: { type: 'string',
                          description: 'Permit or Deny — the effect this ' +
                                       ...
                                       'effect is silently never discharged.' }
              },
              required: ['policy', 'path'],
              examples: [{
                policy: 'my-policy',
          ...
          { action: 'add-rule-advice', operationId: 'addXacmlRuleAdvice',
            ...
            description: 'Something the PEP MAY do. **It is allowed to ' +
                         'ignore this, and that is the whole difference from ' +
                         'an obligation** — advice a PEP drops is a PEP ' +
                         'behaving correctly, where an obligation it drops ' +
                         'is half a policy enforced and reported as success.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
          ...
          { action: 'add-policy-advice', operationId: 'addXacmlPolicyAdvice',
            ...
            description: 'On the policy\'s decision rather than on one ' +
                         'rule\'s. See addXacmlRuleAdvice for why advice and ' +
                         'an obligation are not the same thing.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
          ...
          { action: 'edit-obligation', operationId: 'editXacmlObligation',
            summary:
              'Change an obligation\'s or advice\'s id, or when it fires',
            description: 'The FULFILL-ON is the consequential field: an ' +
                         'obligation set to fire on Permit contributes ' +
                         'nothing to a Deny, and one attached to the wrong ' +
                         'effect is silently never discharged.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
          ...
          { action: 'add-assignment', operationId: 'addXacmlAssignment',
            ...
            description: 'A value handed to the PEP along with the ' +
                         'obligation — what to log, whom to notify, which ' +
                         'record to stamp. Its expression takes any of the ' +
                         'four shapes the set-expression actions produce.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
          ...
          // REFERENCES.
          //
          // Everything below reaches a part of the XACML syntax the editor
          // could not build before. Four of them are not new controls so much
          // as a repair: `add-policy` exists because a PolicySet used to be
          // offered `add-rule`, which the writer then discarded — an edit
          // accepted, reported as done, and absent from the stored document.
          //
          ...
          { action: 'add-policy', operationId: 'addXacmlPolicyToSet',
            ...
            description: 'A PolicySet holds POLICIES; a Policy holds RULES. ' +
                         'They are not two spellings of one thing, and ' +
                         'asking for a rule on a policy set is refused ' +
                         'rather than written somewhere it will not be ' +
                         'read.\n\nThe new policy arrives with ' +
                         '`deny-unless-permit` and no rules, which decides ' +
                         'Deny. That is deliberate: this editor is LIVE, so ' +
                         'a child added to the running root takes ' +
          ...
          { action: 'add-policyset', operationId: 'addXacmlPolicySetToSet',
            ...
            description: 'There is no depth limit. It arrives with ' +
                         '`deny-unless-permit` — the POLICY-combining ' +
                         'spelling, which is a different URI from the ' +
                         'rule-combining one of the same name.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
          ...
          { action: 'add-policy-reference',
            ...
            description: 'THIS IS HOW A PDP REACHES MORE THAN ONE DOCUMENT. ' +
                         ...
                         'document is loaded — so referencing a policy that ' +
                         'has not been written yet is allowed and is not an ' +
                         'error. An unresolved reference is reported on the ' +
                         'decision instead, which is what keeps the order ' +
                         'two policies are authored in from ' +
                         'mattering.\n\nThe `ref` is the PolicyId INSIDE the ' +
                         'other document, not its directory entry name.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
                ...
                ref: { type: 'string',
                          description:
                            'The PolicyId being referenced. Any URI.' },
                version: { type: 'string',
                          description: 'An optional version constraint. Omit ' +
                                       'for no constraint.' }
              },
              required: ['policy', 'path'],
              examples: [{ policy: 'my-policy-set', path: '',
          ...
          { action: 'edit-reference', operationId: 'editXacmlReference',
            ...
            description: 'An empty `version` means NO version constraint, ' +
                         'which is a different document ' +
                         'from one naming a version — so ' +
                         'it is written as absent rather than as an empty ' +
                         'string.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
                ...
                version: { type: 'string',
                          description:
                            'A version constraint, or empty for none.' }
              },
              required: ['policy', 'path'],
          ...
          { action: 'add-variable', operationId: 'addXacmlVariable',
            ...
            description: 'Names an expression so several rules can share it ' +
                         '— and so it is evaluated ONCE per request rather ' +
                         'than once per use.\n\nITS SCOPE IS THE POLICY IT ' +
                         'IS ON. A sibling policy in the same set cannot ' +
                         'name it (section 5.24), which is why GET ' +
                         '/admin-api/xacml/editor is the only reliable ' +
                         'source of what a VariableReference at a given path ' +
                         'may legally name.\n\nIt arrives holding a ' +
                         'designator, because a VariableDefinition with ' +
                         'no expression is a document that will not load. ' +
              ...
              properties: {
                ...
                variableId: { type: 'string',
                          description: 'The name. Defaults to the next free ' +
                                       ...
                                       'reader rejects a document defining ' +
                                       'one id twice, so allowing it here ' +
                                       'would write a policy this service ' +
                                       'cannot load back.' }
              },
              required: ['policy', 'path'],
          ...
          { action: 'edit-variable', operationId: 'renameXacmlVariable',
            ...
            description: 'EVERY `VariableReference` NAMING IT IS REWRITTEN. ' +
                         'The alternative — refusing to rename one that is ' +
                         'used — is safe and useless, since a variable ' +
                         'nobody references is the only one nobody wants to ' +
                         'rename; and a rename that left the references ' +
                         'behind would produce a document that does not ' +
                         'load, so the write would be refused and ' +
                         'nothing would change. The reply says how ' +
          ...
          { action: 'set-expression-selector',
            ...
            description: 'An XPath over the `<Content>` of a request ' +
                         'category, rather than a named attribute — and it ' +
                         'returns a BAG exactly as a designator does, so ' +
                         'most functions still need a `one-and-only` around ' +
                         'it.\n\nThe namespace bindings its prefixes need ' +
                         'TRAVEL WITH THE DOCUMENT: a prefix in the path ' +
                         'means nothing without one, and an unresolvable ' +
                         'prefix is an empty bag, which is NotApplicable, ' +
                         'which looks exactly like a policy ' +
                         'that decided you may not. Set them with ' +
                         'edit-selector.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
                ...
                category: { type: 'string',
                          description:
                            'The request category whose content the ' +
                                       'path runs over. Defaults to ' +
                                       'resource.' },
                path_xpath: { type: 'string',
                          description: 'Sent as `path` — but `path` is ' +
                                       'already the node address on this ' +
                                       'endpoint, so set the XPath with ' +
                                       'edit-selector immediately ' +
                                       'afterwards. The ' +
                                       'selector arrives with ' +
          ...
          { action: 'edit-selector', operationId: 'editXacmlSelector',
            summary:
              'Change an AttributeSelector\'s path, category or bindings',
            description: 'ONE NAMESPACE BINDING AT A TIME, because the ' +
                         'console this mirrors has no JavaScript and cannot ' +
                         'grow a row: send `namespacePrefix` with ' +
                         '`namespaceUri` to add or change one, and ' +
                         '`namespacePrefix` with an empty `namespaceUri` to ' +
                         'remove it.\n\n`ContextSelectorId` names an ' +
                         'attribute holding the node the path starts from; ' +
                         'absent means the whole content.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
                ...
                path: { type: 'string',
                          description: 'The node\'s address, from GET ' +
                                       ...
                                       'naming rule 1 now means rule 0.' },
                path_xpath: { type: 'string',
                          description: 'Sent as `path` collides with the ' +
                                       'node address; this operation reads ' +
                                       'the XPath from `path` ONLY when it ' +
                                       'is not the address — see the console ' +
                                       'form, which posts them separately.' },
                category: { type: 'string',
                          description:
                            'The category whose content is selected.' },
                dataType: { type: 'string',
                          description: 'The datatype of the bag it returns.' },
                contextSelectorId: { type: 'string',
                          description:
                            'An attribute naming the starting node, ' +
                                       'or empty for the whole content.' },
                mustBePresent: { type: 'string',
                          description: '"true" or "false". Absent KEEPS the ' +
                                       'current value rather than clearing ' +
                                       'it.' },
          ...
          { action: 'set-expression-function',
            ...
            description: 'A function named as a VALUE rather than applied: ' +
                         ...
                         'those wrongly, and it is why choosing a ' +
                         'higher-order function from set-expression-apply ' +
                         'now builds this shape for you rather than an ' +
                         'AttributeValue the validator refuses.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
          ...
          { action: 'edit-function', operationId: 'editXacmlFunctionReference',
            ...
            description: 'The whole content of the element is which function ' +
                         'it names, so this is its only field.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
                ...
                functionId: { type: 'string',
                          description:
                            'Any identifier in the function library.' }
              },
              required: ['policy', 'path'],
              examples: [{ policy: 'my-policy',
                           path: 'rules.0.condition.args.0',
          ...
          { action: 'edit-assignment', operationId: 'editXacmlAssignment',
            ...
            description: 'The three attributes of an ' +
                         ...
                         'and Issuer are OPTIONAL and empty means absent, ' +
                         'not empty: an assignment with no category is a ' +
                         'plain named value handed to the PEP.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
                ...
                category: { type: 'string',
                          description:
                            'An optional category. Empty removes it.' },
                issuer: { type: 'string',
          ...
          { action: 'disable-pep', operationId: 'disableXacmlPep',
            ...
            description: 'IT DOES NOT STOP IT ENFORCING, and the reply says ' +
                         'so rather than letting that be discovered. A ' +
                         'remote PEP holds its own copy of the engine and ' +
                         'its own copy of the policy; nothing in this API ' +
                         'reaches into another process. What changes is that ' +
                         'this service no longer dials it when the ' +
                         'repository changes, so it converges only on its ' +
                         'own polling interval.\n\nNamed with a `-pep` ' +
                         'suffix rather than reusing `disable` because this ' +
                         'endpoint dispatches on the action name across the ' +
                         'whole family, and a second `disable` would be ' +
          ...
          { action: 'enable-pep', operationId: 'enableXacmlPep',
            ...
            description: 'Puts it back into the set this service dials when ' +
                         'the repository changes. A PEP that re-registers ' +
                         'does NOT clear this by itself — a component an ' +
                         'administrator stopped nudging must not be able to ' +
            ...
            requestBody: {
              ...
              properties: {
                name: { type: 'string',
                          description:
                            'The registered PEP\'s name, which is the ' +
          ...
          { action: 'forget-pep', operationId: 'forgetXacmlPep',
            ...
            description: 'Removes the row in ou=peps. THE PROCESS IS ' +
                         ...
                         'again. Use it for a PEP that is genuinely gone; ' +
                         'use disable-pep for one that is not.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
          ...
          { action: 'issue-pep-certificate',
            ...
            summary: 'Issue a remote PEP the certificate and private key for ' +
                     'its HTTPS listener',
            description: 'Generates a key pair and certifies it from the ' +
                         'Remote PEP listeners (`pep-tls`) Issuing CA of THE ' +
                         'REALM THE PEP REGISTERED TO — the realm this call ' +
                         'is made in, because ou=peps is per realm — so a ' +
                         'client that installed this service\'s Root CA ' +
                         'verifies the PEP, and ' +
                         'the chain says which realm vouched for it. A realm ' +
                         'whose branch was built before that Issuing CA ' +
                         'existed gets it added under its existing ' +
                         'Intermediate; nothing already issued is ' +
                         'replaced.\n\nThe PEP must be REGISTERED: its row ' +
                         'is the one record that it exists in this realm. ' +
                         'That is not a permission check on the PEP — ' +
                         'registering is still not what lets it pull or ' +
                         'enforce.\n\nTHE CERTIFICATE NAMES the PEP\'s ' +
                         'registered name (where that is a DNS name) and the ' +
                         'host of its notify URL, plus any `dnsNames` and ' +
                         '`ipAddresses` sent. ' +
                         'It certifies `serverAuth` and nothing ' +
                         'else.\n\n**THE PRIVATE KEY IS IN THIS REPLY AND ' +
                         'NOWHERE ELSE.** This service records the ' +
                         'certificate — its issuer\'s CRL and OCSP responder ' +
                         'answer for it — and keeps no copy of the ' +
                         'key, so nothing can read it back. To put the pair ' +
                         'in the container, write `fullChainPem` (the leaf ' +
                         'followed by its Issuing CA and the realm ' +
                         'Intermediate) and `privateKeyPem` to the files ' +
                         '`PEP_HTTPS_CERT` and `PEP_HTTPS_KEY` name; the PEP ' +
                         'picks up a pair written after it started. ' +
                         '`anchorPem` is the service Root a client of that ' +
                         'listener installs.\n\nIssuing again REPLACES: the ' +
                         'certificate it replaces goes on its ' +
            ...
            requestBody: {
              ...
              properties: {
                ...
                dnsNames: { oneOf: [{ type: 'array',
                                      items: { type: 'string' } },
                                    { type: 'string' }],
                          description: 'DNS names to ADD to the two derived ' +
                                       'from the registration. An array, or ' +
                                       'one string separated by commas or ' +
                                       'whitespace. A `*.` wildcard is ' +
                                       'accepted in the first label.' },
                ipAddresses: { oneOf: [{ type: 'array',
                                         items: { type: 'string' } },
                                       { type: 'string' }],
                          description: 'IPv4 or IPv6 addresses to add, in ' +
      ...
      { method: 'GET', path: BASE + '/ssf', tag: 'Shared Signals',
        operationId: 'getSsf',
        summary: 'The Shared Signals transmitter: its streams, their ' +
                 'subjects, their queues and what a receiver refused',
        description: 'Everything /admin/ssf draws, as JSON. Per stream: the ' +
                     'configuration a receiver agreed, who it is about, what ' +
                     'is waiting to be delivered, the counters, and the ' +
                     'stream\'s own log — which is the only place a REFUSED ' +
                     'PUSH is recorded, because a push a receiver rejected ' +
                     'is invisible from the receiving end by ' +
                     'definition.\n\nSSF IS THE PIPE AND NOT THE VOCABULARY. ' +
                     'It defines how two parties agree a stream, who the ' +
                     'events are about (RFC 9493), what they travel in (RFC ' +
                     '8417) and how they get there (RFC 8935 push, RFC 8936 ' +
                     'poll) — and two events of its own, both about the ' +
                     'pipe. CAEP and RISC are the vocabularies spoken over ' +
                     'it; CAEP is implemented and its own register is at GET ' +
                     '/admin-api/caep, and RISC has one of its own at GET ' +
                     '/admin-api/risc.\n\nTHE RECEIVER\'S ' +
                     '`authorization_header` IS NOT IN THIS REPLY. It is a ' +
                     'credential belonging to somebody else\'s endpoint and ' +
                     'it goes back only to the receiver that set it, at ' +
                     'GET /ssf/stream.',
        ...
        handler: function (req, res) {
          ...
          self.sendJson(res, 200, adminViews.ssfJson(req));
      ...
      // No POST of its own, because the page has no control; the two that act
      // on dead letters are that operation's `revive` and `clear-dead-letters`.
      // ---------------------------------------------------------------------
      { method: 'GET', path: BASE + '/ssf/dead-letters', tag: 'Shared Signals',
        operationId: 'getSsfDeadLetters',
        summary:
          'What the Shared Signals transmitter could not deliver, counted',
        description: 'Everything /admin/ssf/dead-letters draws, as JSON: ' +
                     'every Security Event Token held on a dead-letter queue ' +
                     'in this realm, counted by cause, error code, the ' +
                     'receiver\'s HTTP status and event type, over time ' +
                     'across the retention window, and per stream with each ' +
                     'push stream\'s delivery state — then the letters ' +
                     'themselves, newest first, searched and paged, without ' +
                     'their tokens.\n\n**THE COUNTS ARE THE WHOLE REALM\'S ' +
                     'WHATEVER IS NARROWED.** `dlq`, `dlstream` and ' +
                     '`dlcause` narrow `letters` and `matched` ' +
                     'only.\n\n**`process` IS THE ANSWERING PROCESS\'S.** ' +
                     'Its push cap is shared by ' +
                     'every realm in that process, and ' +
        ...
        parameters: [
          { name: 'dlq', in: 'query', required: false,
            ...
            description: 'Narrows the letters to those whose jti, stream, ' +
                         'reason, error code, status, event name, type URI ' +
                         'or subject contains this.' },
          ...
          { name: 'lettersPage', in: 'query', required: false,
            schema: { type: 'integer', minimum: 1 },
            description: 'Which page of the matched letters. Clamped, like ' +
                         'every page parameter here.' }
        ].concat(this.pagingParameters()),
        responseDescription: 'The dead-letter queues of this realm, counted, ' +
                             'and a page of their letters.',
        responseSchema: { $ref: '#/components/schemas/SsfDeadLetters' },
        handler: function (req, res) {
          log.debug("Entering the management API dead letters endpoint.");
          self.sendJson(res, 200, adminViews.ssfDeadLettersJson(req));
      ...
      { method: 'POST', route: BASE + '/ssf/:action', tag: 'Shared Signals',
        ...
        handler: function (req, res) {
          log.debug("Entering the management API Shared Signals action " +
                    "endpoint.");
          const body = parseBody(req);
          // THE FIRST HANDLER IN THIS FILE THAT AWAITED; PKI, XACML, CAEP, RISC
          // and the database and secret-store reports have since. Transmitting
          // a Security Event Token signs a JWS — which may be ML-DSA or SLH-DSA
          // on the worker pool — and then POSTs it to somebody else's endpoint.
          // Neither can be done synchronously, and answering before either had
          // happened would be this API reporting "sent" about nothing.
          adminActions.ssfAction(self.withAction(req, body))
            .then(function (result) {
            if (!result.ok) {
            ...
            log.error('admin-api: the Shared Signals action threw: ' +
                      e.message);
            self.sendJson(res, 500, { ok: false,
        ...
        actions: [
          { action: 'status', operationId: 'setSsfStreamStatus',
            ...
            description: 'The three values and what separates them: a PAUSED ' +
                         'stream keeps QUEUEING and delivers nothing, so ' +
                         'what happened while it was paused is still there ' +
                         'when it is enabled again; a ' +
                         'DISABLED one DROPS what is waiting. ' +
                         'That is the difference between "I was not ' +
                         'listening" ' +
            ...
            requestBody: {
              ...
              properties: {
                ...
                reason: { type: 'string',
                          description: 'Optional. Why, in words — it rides ' +
                                       'in the stream-updated ' +
                                       'event\'s `reason` ' +
            ...
            responseDescription: 'What happened, and whether the ' +
                                 'stream-updated event went with it.' },

          { action: 'transmit', operationId: 'transmitSsfEvent',
            summary: 'Send a Security Event Token on a stream',
            description: 'Builds the SET, signs it with ' +
                         '`ssf.signingAlgorithm`, queues it on the stream ' +
                         'and — for a PUSH stream — POSTs it to the ' +
                         'receiver\'s delivery endpoint. On a POLL stream it ' +
                         'stays queued until the receiver asks.\n\n**IT IS ' +
                         'THE ONLY WAY AN EVENT HAPPENS HERE.** Nothing in ' +
                         'this service watches a session and emits when it ' +
                         'changes: SSF defines no event about a session, so ' +
                         'a transmitter that invented one would be inventing ' +
                         'a vocabulary. That changes with CAEP.\n\nA failed ' +
                         'push is NOT retried and the event stays on the ' +
                         'queue. A mock that retried would make a ' +
                         'receiver\'s one-shot failure invisible ' +
                         '— a client answering 500 then 202 ' +
              ...
              properties: {
                ...
                type: { type: 'string',
                        description: 'The event type URI. It must be one the ' +
                                     'stream DELIVERS — the intersection of ' +
                                     'what the receiver requested and what ' +
                                     'this transmitter supports — and a ' +
                                     'refusal lists what that is.' },
                payload: { type: 'object',
                           description: 'The event\'s own members. A ' +
                                        'verification event takes an ' +
                                        'optional `state`; a stream-updated ' +
                                        'event takes a required `status` and ' +
                                        'an optional `reason`. An ' +
                                        'unrecognised member is CARRIED with ' +
                                        'a warning rather than refused: an ' +
                                        'event vocabulary extends, and a ' +
                                        'receiver is expected to ignore ' +
                                        'what it does not know.' },
                subject: { type: 'object',
                           description: 'Optional `sub_id` (RFC 9493), ' +
                                        'simple or complex. Neither SSF ' +
                                        'event takes one — both are about ' +
                                        'the STREAM — so this is here for ' +
                                        'the vocabularies that come next. ' +
                                        'Each format\'s member set is CLOSED ' +
                                        'and an extra member is refused ' +
                                        'by name.' },
                txn: { type: 'string',
          ...
          { action: 'delete', operationId: 'deleteSsfStream',
            ...
            description:
              'The stream, its subjects and everything queued on it. ' +
                         'The receiver is NOT told — SSF has no event for ' +
                         '"your ' +
                         'stream is gone", and a stream-updated with status ' +
                         'disabled would be a lie about something that still ' +
                         'exists. A receiver finds out on its next call, ' +
                         'with a 404 naming the stream_id.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
          ...
          { action: 'revive', operationId: 'reviveSsfStream',
            ...
            description: 'A push stream whose pushes have all failed for ' +
                         ...
                         'more is pushed to it, its SETs go to its ' +
                         'dead-letter queue, and a sweep pushes one as a ' +
                         'probe every period. This revives it at once, so ' +
                         'the next SET is pushed. Refused for ' +
                         'a stream that is not dead. The dead ' +
            ...
            requestBody: {
              ...
              properties: {
                ...
                reason: { type: 'string',
                          description:
                            'Optional; written on the stream\'s log.' }
              },
              required: ['stream_id'],
              examples: [{ stream_id: 'ssf-0123456789ab' }],
              additionalProperties: false
          ...
          { action: 'clear-dead-letters', operationId: 'clearSsfDeadLetters',
            ...
            description: 'Empties the stream\'s dead-letter queue: SETs that ' +
                         'could not be delivered, each kept with its reason ' +
                         'for `ssf.deadLetterRetentionS`. ' +
                         'Nothing is sent. GET ' +
                         '/admin-api/ssf lists them per stream as ' +
                         '`deadLetters`.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
            ...
            responseDescription: 'How many were dropped, or a refusal naming ' +
                                 'the stream_id.' },
          ...
          { action: 'clear-received', operationId: 'clearSsfReceived',
            ...
            description: 'Empties the list `POST /ssf/receive` fills — this ' +
                         ...
                         'reversed and what a client acting as the ' +
                         'TRANSMITTER pushes to. It touches no stream and no ' +
                         'queue: those are the transmitter half and have ' +
      ...
      // /admin/caep-sessions carry THREE controls between them — emit an event
      // by hand, reset one session's CAEP state, clear the register — and rule
      // 7 is about controls. All three call the same function the console's
      // forms post to, with `action` taken from the URL instead of from a
      // hidden input.
      //
      // **THERE IS DELIBERATELY NO WAY TO END A SESSION FROM HERE**, and that
      // is the same reading of rule 7 that leaves SSF without a `create`.
      // Emitting a `session-revoked` says a session was revoked; it does not
      // revoke one, and a management API that did both would make "tell the
      // receivers" and "sign this person out" one act — which is exactly the
      // conflation CAEP exists to separate. /logout, /admin/logout and POST
      // /admin-api/logout end sessions, and a sign-out through any of them
      // emits the event on its own.
      { method: 'GET', path: BASE + '/caep', tag: 'CAEP',
        operationId: 'getCaep',
        summary: 'The CAEP session register: what state each session is in ' +
                 'and how many events of which type have been sent about it',
        description: 'Everything /admin/caep and /admin/caep-sessions draw, ' +
                     'as JSON. Per session: who it belongs to, when it was ' +
                     'established, the CAEP state it is in — established, ' +
                     'presented, revoked — its assurance level, its device ' +
                     'compliance, its risk level, the claims that have ' +
                     'changed, and a count per event type.\n\nTHE REGISTER ' +
                     'OUTLIVES THE SESSION AND THAT IS THE POINT. The ' +
                     'session store forgets one the moment it is signed out, ' +
                     'so a row saying `revoked` is the only remaining ' +
                     'evidence that it existed and was revoked.\n\nCAEP IS A ' +
                     'VOCABULARY AND NOT A FAMILY. Its events travel on SSF ' +
                     'streams, are signed by the SSF signer and are ' +
                     'delivered by the two SSF deliveries, so GET ' +
                     '/admin-api/ssf is where the streams themselves are. ' +
                     '`streams` here says only which of them would take a ' +
                     'CAEP event at all, because a session with a count of ' +
                     'zero almost always means nobody asked for that type — ' +
                     'and SSF gives a receiver no other notice of that.',
        ...
        handler: function (req, res) {
          ...
          self.sendJson(res, 200, adminViews.caepJson(req));
          log.debug("Leaving the management API CAEP endpoint.");
        } },

      // THE SECOND CAEP READ, AND IT EXISTS BECAUSE OF RULE 7 RATHER THAN
      // BECAUSE THE DOCUMENT IS DIFFERENT. `/admin/caep-sessions` is a page of
      // the console and had no operation naming it — the GET above mirrors
      // `/admin/caep`, and one operation cannot mirror two pages. That gap was
      // invisible until the parity check in tests/vendored/admin_api.js named
      // it.
      //
      // ONE OPERATION ANSWERS BOTH SHAPES, `?session=` deciding which, for the
      // reason /admin-api/permissions/groups gives: the list and the drill-down
      // are the same question at two scales and the console draws them from one
      // register, so two operations would be two places to disagree about what
      // a session's history is.
      { method: 'GET', path: BASE + '/caep/sessions', tag: 'CAEP',
        ...
        description: 'What /admin/caep-sessions draws. Without `session` it ' +
                     'is the register: one row per session this service has ' +
                     'HELD, including the ones it no longer holds, with its ' +
                     'CAEP state, assurance, device compliance, risk and a ' +
                     'count per event type — searched with `sessq` and paged ' +
                     'with `sessionsPage` and `per`. Beside it, ' +
                     '`applications`: what this transmitter has said to each ' +
                     'RECEIVER across every session, searched with `appq` ' +
                     'and paged with `applicationsPage`. That is the third ' +
                     'question this page answers and the one somebody ' +
                     'arrives with once more than one receiver exists — the ' +
                     'sessions are per SESSION, the streams are per STREAM, ' +
                     'and this is per APPLICATION.\n\nWith `session` it is ' +
                     'that one session opened out: the events actually sent ' +
                     'about it, in order, with the jti and the stream each ' +
                     'went out on and what the register noticed as it was ' +
                     'applied, paged with `eventsPage`.\n\n**THE REGISTER ' +
                     'OUTLIVES THE SESSION AND THAT IS THE POINT.** The ' +
                     'session store forgets one the moment it is signed out, ' +
                     'so a row saying `revoked` is the only remaining ' +
                     'evidence that it existed and was revoked — and it is ' +
                     'why this is not the same list as GET ' +
                     '/admin-api/sessions, which is what is LIVE.\n\nA ' +
                     '`session` that names nothing answers 200 with ' +
                     '`session: null` rather than 404: this register is ' +
                     'capped at `caep.maxSessionsTracked` and drops the ' +
                     'oldest, so an old identifier coming back empty is an ' +
                     'ordinary outcome and not a missing resource.',
        mirrors: 'GET /admin/caep-sessions',
        parameters: [
          { name: 'sessq', in: 'query', required: false,
            schema: { type: 'string' },
            description:
              'Narrows the register to rows whose username, `sub`, ' +
                         'session id, SSF subject or protocol contains this.' },
          { name: 'session', in: 'query', required: false,
            schema: { type: 'string' },
            description: 'One session id. The reply is then that session and ' +
                         'its events rather than the list.' },
          { name: 'sessionsPage', in: 'query', required: false,
            schema: { type: 'integer', minimum: 1 },
            description: 'Which page of the register. Clamped, like every ' +
                         'page parameter here.' },
          { name: 'appq', in: 'query', required: false,
            schema: { type: 'string' },
            description: 'Narrows the per-receiver list to rows whose ' +
                         'application name, identifier or stream `aud` ' +
                         'contains this. It deliberately does NOT match ' +
                         'event types: a receiver that takes none of the ' +
                         'eight is exactly the row somebody is looking for ' +
                         'when they ask why nothing arrived.' },
          { name: 'applicationsPage', in: 'query', required: false,
            schema: { type: 'integer', minimum: 1 },
            description: 'Which page of the per-receiver list. It shares ' +
                         '`per` with the register above ' +
                         'it, which is the arrangement ' +
                         'every page here that carries two lists has.' },
          { name: 'eventsPage', in: 'query', required: false,
            schema: { type: 'integer', minimum: 1 },
            description:
              'Which page of ONE session\'s events, with `session`.' }
        ].concat(this.pagingParameters()),
        ...
        handler: function (req, res) {
          ...
          self.sendJson(res, 200, adminViews.caepSessionsJson(req));
      ...
      { method: 'POST', route: BASE + '/caep/:action', tag: 'CAEP',
        ...
        handler: function (req, res) {
          ...
          // endpoint.
          adminActions.caepAction(self.withAction(req, body))
            .then(function (result) {
            if (!result.ok) {
        ...
        actions: [
          { action: 'emit', operationId: 'emitCaepEvent',
            ...
            description: 'Builds the payload, adds the four claims CAEP ' +
                         'gives every event, composes the SUBJECT from the ' +
                         'session — SSF\'s complex subject, naming the ' +
                         ...
                         'nobody had asked to hear about it" is a different ' +
                         'fact ' +
              ...
              properties: {
                ...
                initiating_entity: { type: 'string',
                  description: 'admin | user | policy | system. It is the ' +
                               ...
                               'did".' },
                payload: { type: 'string',
                  description: 'The event-specific members, as JSON. Empty ' +
                               'for a conforming specimen of the type.' },
                reason_admin: { type: 'string',
                  description: 'Why, for a log. Sent as an object keyed by ' +
                               '`caep.reasonLanguage` — CAEP makes it a ' +
                               'language map and a string there is the ' +
                               'commonest mistake in the profile.' },
          ...
          { action: 'reset-session', operationId: 'resetCaepSession',
            ...
            description: 'Clears the state, the assurance level, the device ' +
                         ...
                         'SIGNS NOBODY OUT. This register is a record of ' +
                         'what has been SAID about a session; resetting it ' +
                         'forgets the record. The session itself is ' +
                         'untouched and every token issued on it is still ' +
                         'good, which is the distinction this whole page ' +
                         'rests on.\n\nIt is a reset and not a delete ' +
                         'because the identity and the sign-in instant are ' +
                         'still true — a delete would take the row off the ' +
                         'page, which reads as the session having gone.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
          ...
          { action: 'clear', operationId: 'clearCaepSessions',
            ...
            description: 'Forgets what has been said about every session. ' +
                         ...
                         'queued event is dropped — those are the SSF half, ' +
                         'at POST /admin-api/ssf/delete and ' +
                         '/clear-received.\n\nIt is worth knowing what this ' +
                         'throws away: rows for sessions this service NO ' +
                         'LONGER HOLDS are the only evidence that ' +
                         'those sessions existed and were revoked, ' +
      ...
      // way to end a session. Emitting an `account-disabled` says an account
      // was disabled; it does not disable one, and a management API that did
      // both would make "tell the receivers" and "deprovision this person" one
      // act — which is exactly the conflation RISC exists to separate. The
      // directory half is POST /admin-api/users and SCIM, and a write through
      // either emits the event on its own when `risc.autoEmit` is on.
      { method: 'GET', path: BASE + '/risc', tag: 'RISC',
        operationId: 'getRisc',
        summary: 'The RISC account register: what state each account is in ' +
                 'and how many events of which type have been sent about it',
        description: 'Everything /admin/risc and /admin/risc-accounts draw, ' +
                     'as JSON. Per account: who it is, the SSF subject a ' +
                     'receiver was sent, and THREE states rather than one — ' +
                     'the lifecycle (active, disabled, purged), the RISC ' +
                     'section 2.8 opt-out state (opt-in, opt-out-initiated, ' +
                     'opt-out) and whether a credential has been reported ' +
                     'compromised. They move independently: an account can ' +
                     'be opted out and perfectly healthy, or compromised and ' +
                     'still enabled.\n\nTHE REGISTER OUTLIVES THE ACCOUNT, ' +
                     'MORE STARKLY THAN THE CAEP ONE OUTLIVES A SESSION. A ' +
                     'purged account is gone from the directory entirely, so ' +
                     'the row whose lifecycle says `purged` is the only ' +
                     'remaining evidence anywhere that this service ever ' +
                     'told anybody it was.\n\nRISC IS A VOCABULARY AND NOT A ' +
                     'FAMILY. Its events travel on SSF streams, are signed ' +
                     'by the SSF signer and are delivered by the two SSF ' +
                     ...
                     'one counter with no CAEP equivalent: events this ' +
                     'transmitter built and did NOT send, because the ' +
                     'account is in the opt-out state and ' +
                     '`risc.honourOptOut` is on. It is the only number here ' +
                     'that says a receiver heard nothing ON PURPOSE.',
        mirrors: 'GET /admin/risc',
        responseDescription: 'The register, the catalogue and the settings.',
        responseSchema: { $ref: '#/components/schemas/Risc' },
        handler: function (req, res) {
          log.debug("Entering the management API RISC endpoint.");
          self.sendJson(res, 200, adminViews.riscJson(req));
      ...
      { method: 'GET', path: BASE + '/risc/accounts', tag: 'RISC',
        ...
        description: 'What /admin/risc-accounts draws. Without `account` it ' +
                     'is the register: one row per account this service has ' +
                     'been told anything about, including ones that no ' +
                     'longer exist, searched with `acctq` and paged with ' +
                     '`accountsPage` and `per`. Beside it, `applications`: ' +
                     'what this transmitter has said to each RECEIVER across ' +
                     'every account, searched with `rappq`.\n\nWith ' +
                     '`account` it is that one account opened out: the ' +
                     'events actually sent about it, in order, with the jti ' +
                     'and the stream each went out on and what the register ' +
                     'noticed as it was applied, paged with ' +
                     ...
                     'address a caller is holding — out of a log, off an ' +
                     'event it is chasing — is routinely the superseded one. ' +
                     'A search that matched only the current spelling would ' +
                     'hide exactly the row somebody came to find.\n\nAn ' +
                     '`account` that names nothing answers 200 with ' +
                     '`account: null` rather than 404: this register ' +
                     'is capped at `risc.maxAccountsTracked` and ' +
                     'drops the oldest.',
        ...
        parameters: [
          { name: 'acctq', in: 'query', required: false,
            ...
            description: 'Narrows the register to rows whose account id, ' +
                         'username, `sub`, email address, telephone number, ' +
                         'SSF subject, directory DN or any FORMER identifier ' +
                         'contains this.' },
          ...
          { name: 'rappq', in: 'query', required: false,
            ...
            description: 'Narrows the per-receiver list to rows whose ' +
                         ...
                         'contains this. It deliberately does NOT match ' +
                         'event types: a receiver that takes none of the ' +
                         'fourteen is exactly the row somebody is looking ' +
                         'for when they ask why nothing arrived.' },
          { name: 'rapplicationsPage', in: 'query', required: false,
            schema: { type: 'integer', minimum: 1 },
            description: 'Which page of the per-receiver list. It shares ' +
                         '`per` with the register above it.' },
          { name: 'eventsPage', in: 'query', required: false,
            schema: { type: 'integer', minimum: 1 },
            description:
              'Which page of ONE account\'s events, with `account`.' }
        ].concat(this.pagingParameters()),
        ...
        handler: function (req, res) {
          ...
          self.sendJson(res, 200, adminViews.riscAccountsJson(req));
      ...
      { method: 'POST', route: BASE + '/risc/:action', tag: 'RISC',
        ...
        handler: function (req, res) {
          ...
          adminActions.riscAction(self.withAction(req, body))
            .then(function (result) {
            if (!result.ok) {
        ...
        actions: [
          { action: 'emit', operationId: 'emitRiscEvent',
            ...
            description: 'Builds the payload, composes the SUBJECT from the ' +
                         'account in the format `risc.subjectFormat` names, ' +
                         'and transmits it on every stream that both ' +
                         'delivers the type and covers that ' +
                         'subject.\n\nELEVEN OF THE FOURTEEN CARRY NO ' +
                         'PAYLOAD MEMBERS AT ALL, so for those the subject ' +
                         'is the entire message and `payload` is left empty. ' +
                         'Only `credential-compromise` has a REQUIRED ' +
                         'member, `credential_type`, which RISC defines by ' +
                         'reference to CAEP\'s `credential-change` — the two ' +
                         'lists are the same list.\n\nTEN OF THE FOURTEEN ' +
                         'ARE ONLY EVER PRODUCED THIS WAY. No breach corpus ' +
                         'is searched by this service and no recovery flow ' +
                         'runs in it. The other four fire on their own when ' +
                         'the DIRECTORY changes and `risc.autoEmit` is on — ' +
                         'a person deleted, `active` going false or true, a ' +
                         'mail address or telephone number moving.\n\nFOUR ' +
                         'OF THEM CHANGE REAL STATE WHEN THEY ' +
                         'GO. RISC section 2.8 defines each ' +
                         'opt-out event as "the account is in this state" ' +
                         'rather ' +
                         'than as a report that it moved, so emitting ' +
                         '`opt-out-effective` here IS the transition — and ' +
                         'the next event about that account is suppressed ' +
                         'while `risc.honourOptOut` is on.\n\nAN ACCOUNT ' +
                         'THIS SERVICE HAS NEVER HELD IS ACCEPTED, which is ' +
                         'the opposite of what emitCaepEvent does with an ' +
                         'unknown session. A session identifier this service ' +
                         'never minted is one it can compose no subject ' +
                         'from; an account is a person, and RISC is aimed ' +
                         'ACROSS providers, so the account a receiver is ' +
                         'warned about is usually one it has never ' +
                         'seen.\n\nA TYPE NO STREAM TAKES IS NOT AN ERROR. ' +
                         'The account\'s state is still updated and the ' +
                         'reply says nothing was sent.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
                ...
                payload: { type: 'string',
                  description: 'The event-specific members, as JSON. Empty ' +
                               'for a conforming specimen of the type, which ' +
                               'is what eleven of the fourteen always are. ' +
                               'Note the HYPHEN in `identifier-changed`\'s ' +
                               '`new-value`: it is the only hyphenated ' +
                               'member name in any of the three ' +
                               'vocabularies, and `new_value` is ' +
                               'carried and silently ignored by a receiver.' },
                reason_admin: { type: 'string',
                  description: 'Why, for a log. Sent as an object keyed by ' +
                               '`risc.reasonLanguage`. It reaches the wire ' +
                               'for ONE of the fourteen types — RISC gives ' +
                               'the reason members to ' +
                               '`credential-compromise` and to nothing else ' +
                               '— and is dropped by the catalogue on any ' +
                               'other, rather than being sent as a member ' +
          ...
          { action: 'reset-account', operationId: 'resetRiscAccount',
            ...
            description: 'Clears the lifecycle, the opt-out state, the ' +
                         ...
                         'entry is untouched, which is the distinction this ' +
                         'whole page rests on — and it is sharper here ' +
                         'than for a session, because a row saying ' +
          ...
          { action: 'clear', operationId: 'clearRiscAccounts',
            ...
            description: 'Forgets what has been said about every account. ' +
                         ...
                         'receivers were told it was deleted. The directory ' +
                         'cannot supply it, because the entry is gone.',
            requestBodyRequired: false,
            requestBody: {
              type: 'object',
              properties: {},
      ...
      { method: 'GET', path: BASE + '/scim', tag: 'SCIM',
        operationId: 'getScim',
        summary: 'The SCIM 2.0 provisioning surface, and what it has been ' +
                 'asked to do',
        description: 'Counters and capabilities in one reply. The counters ' +
                     'say which SCIM operation was performed how many times, ' +
                     'on which resource type, and what was refused with ' +
                     'which `scimType`; every operation ' +
                     'and resource type is listed ' +
                     'INCLUDING the ones at zero, because "does this server ' +
                     'do ' +
                     'PATCH" is otherwise answered by omission. The ' +
                     'capabilities ' +
                     'say what the endpoints are, what SCIM here ' +
                     'deliberately does not do, and which LDAP attribute ' +
                     'each SCIM member is.\n\nTHERE IS NO POST BESIDE THIS ' +
                     'ONE and that is not a gap: everything about SCIM that ' +
                     'can be changed is a configuration row — ' +
                     '`scim.enabled`, the three limits and the ' +
                     'authentication settings — so POST ' +
                     '/admin-api/config/set is already the operation for it. ' +
                     'The console page has no form on it either, which is ' +
                     'the parity rule holding rather than being ' +
                     'broken.\n\nWHAT SCIM WROTE is not here: it went into ' +
                     'the embedded directory, so a person provisioned over ' +
                     'SCIM is on /admin-api/users and their groups are on ' +
                     '/admin-api/groups. There is no second store to report.',
        ...
        handler: function (req, res) {
          ...
          self.sendJson(res, 200, adminViews.scimJson(req));
      ...
      // move and no cycle it could close.
      // -----------------------------------------------------------------------
      { method: 'GET', path: BASE + '/pki', tag: 'PKI',
        operationId: 'getPki',
        summary: 'The certificate authority this trust realm holds, and what ' +
                 'has been issued from it',
        description: 'Everything /admin/pki draws: the three CA tiers with ' +
                     'their subjects, serials, validity, key and signature ' +
                     'algorithms, thumbprints and CERTIFICATES; the key and ' +
                     'signature algorithms a build may ask for; the two ' +
                     'assertion PROFILES a key pair may be issued for, with ' +
                     'the attributes each one writes; and every application ' +
                     'in this realm that holds an issued key pair or ' +
                     'declares an assertion issuer — **ONE ROW PER ' +
                     'APPLICATION AND PER PROFILE**, so an application ' +
                     'holding both an RFC 7523 and an RFC 7522 key pair ' +
                     'appears twice. A single row with a pair of columns was ' +
                     'the first shape of this and was wrong for a reason ' +
                     'worth keeping: every fact on it — the key handle, the ' +
                     'expiry, the declared issuer, whether there is a key ' +
                     'pair to take off — is per profile.\n\n**NO PRIVATE KEY ' +
                     'IS EVER IN THIS REPLY.** `common/pki.js` drops every ' +
                     'one of them on the way out, so a caller here could not ' +
                     'leak the Root\'s key by forgetting. The CERTIFICATES ' +
                     'are in full, because a certificate is the half of a ' +
                     'key pair that is meant to be handed around and the ' +
                     'Root is the one thing a relying party has to be given ' +
                     'out of band. An application\'s own private key is on ' +
                     'its directory entry as `oauthAssertionPrivateKey`, ' +
                     'sealed under the same key-encryption key as the ' +
                     'hierarchy wherever that key outlives the process, and ' +
                     'opened for a caller that holds a credential.\n\n**IT ' +
                     'IS PER REALM.** A trust realm is a logical identity ' +
                     'service with its own signing key and its own ' +
                     'applications, so a CA shared across realms would be ' +
                     'one authority vouching for several services. Reach ' +
                     'this under a realm prefix for that realm\'s ' +
                     'hierarchy.\n\n**AND SO IS `tree`, SINCE 2026-09-11 — ' +
                     'IT NARROWED AND THIS IS THE RECORD OF IT.** It carried ' +
                     'the Root, the process branch and EVERY REALM\'S ' +
                     'Intermediate for a day; it now carries the Root, the ' +
                     'process branch (TLS and SPIFFE, which certify sockets ' +
                     'every realm answers on) and the Intermediate of the ' +
                     'realm THIS REQUEST WAS REACHED IN, with its Issuing ' +
                     'CAs. `revocation` narrowed with it, so a Revoke here ' +
                     'can only name an authority this realm is under. Reach ' +
                     'the resource under another realm\'s prefix for that ' +
                     'realm\'s branch — which is the same answer the console ' +
                     'gives, because this reply and that page are one ' +
                     'function.\n\n**REVOCATION IS PUBLISHED AND NEVER ' +
                     'CONSULTED, SINCE 2026-09-11.** Every authority signs a ' +
                     'CRL (/pki/crl/{scope}/{ca}) and answers OCSP ' +
                     '(/pki/ocsp/{scope}/{ca}); `revocation` in the reply is ' +
                     'the REGISTER — one entry per authority with what it ' +
                     'issued, what is on its list, and its addresses over ' +
                     'http and ldap — and `revocation-certificate` below ' +
                     'puts a serial on one. What this service does NOT do is ' +
                     'consult a list, its own included, so a certificate ' +
                     'revoked here still authenticates here. **AND `revoke` ' +
                     'BELOW IS A DIFFERENT ACT WITH THE SAME WORD IN IT**: ' +
                     'it takes a key pair off an application and puts ' +
                     'nothing on any list. `revocationNote` and `residency` ' +
                     'are the durable statement of what each means.',
        ...
        parameters: this.pagingParameters().filter(function (one) {
          ...
          { name: 'persons',
            description: 'The People table on /admin/pki, paged by PERSON — ' +
                         'a person holding both profiles is one member of ' +
      ...
      // ---------------------------------------------------------------------
      // RULE 7 FOR THE CERTIFICATE DETAILS DIALOG (2026-09-13). `/admin/pki`
      // and `/admin/crypto-metadata` open a certificate's every field and its
      // trust chain over the page; this is the same answer for a machine, from
      // the same view layer — so a certificate cannot be openable on one door
      // and unknown on the other. Without `certificate` it is the LIST a caller
      // finds the fingerprint in.
      // ---------------------------------------------------------------------
      { method: 'GET', path: BASE + '/certificates', tag: 'PKI',
        operationId: 'getCertificates',
        summary: 'Every certificate this trust realm holds, or one of them ' +
                 'with every X.509 field and its trust chain',
        description: 'Without `certificate`: a LIST of every certificate ' +
                     'this service holds in the trust realm the request was ' +
                     'reached in — the service Root, the process branch (TLS ' +
                     'and SPIFFE), this realm\'s Intermediate, Issuing CAs ' +
                     'and what they certified, the signing-key certificates, ' +
                     'the certificate authority workbench, the TLS listener ' +
                     'certificates, the SPIFFE authorities, and the key ' +
                     'pairs issued to applications and people — one row per ' +
                     'certificate with every place it appears.\n\nWith ' +
                     '`certificate`: that certificate described WHOLE — the ' +
                     'tbsCertificate in RFC 5280 section 4.1\'s order ' +
                     '(version, serial, inner signature algorithm, issuer ' +
                     'and subject RDN by RDN, validity with its ASN.1 time ' +
                     'type, the public key with its parameters and bytes, ' +
                     'both unique identifiers, every extension decoded), the ' +
                     'outer signature algorithm and value, both fingerprints ' +
                     '— and `chain`, the path to a self-signed certificate ' +
                     'BUILT from what this service holds by matching each ' +
                     'issuer\'s name AND verifying its signature, with ' +
                     '`chainStatus` saying where and why a path stopped. ' +
                     'Every certificate in the chain is described with the ' +
                     'same fields.\n\n**A certificate is named by its ' +
                     'SHA-256 fingerprint and looked up, never sent.** A ' +
                     'fingerprint of anything this realm does not hold is ' +
                     'refused, so a certificate from another realm is ' +
                     'reached under that realm\'s prefix. Mirrors the ' +
        ...
        parameters: [
          { name: 'certificate', in: 'query', required: false,
            ...
            description: 'A SHA-256 certificate fingerprint, 64 hexadecimal ' +
                         'digits with or without colons — the `fingerprint` ' +
                         'of a row of the list. Returns that certificate\'s ' +
        ...
        handler: function (req, res) {
          ...
          if (!String(req.query.certificate || '').trim()) {
            self.sendJson(res, 200, certificateViews.listView(req));
          ...
          certificateViews.detailsView(req).then(function (view) {
            if (!view.ok) {
              ...
              self.sendJson(res, code === 'STS-ADMIN-0641' ? 404
            ...
            self.sendJson(res, 500, { ok: false, errors: [e.message] });
          });
        } },
      ...
      { method: 'POST', route: BASE + '/pki/:action', tag: 'PKI',
        ...
        // posting somewhere other than its list page has to be named here or
        // that check quietly stops covering it — which is what /admin/users/new
        // records.
        ...
        handler: function (req, res) {
          ...
          // (`/ssf/:action` is the first, and for a related reason: it signs
          // and then POSTs). Express 4 does not look at what a handler returns,
          // so an unhandled rejection here would be a request that never gets
          // an answer.
          pkiAdmin.pkiAction(self.withAction(req, body))
            .then(function (result) {
            if (!result.ok) {
        ...
        actions: [
          { action: 'build', operationId: 'buildPkiChain',
            summary:
              'Build this realm\'s certificate authority — all three tiers',
            description: 'Root CA, Intermediate CA and Issuing CA, generated ' +
                         'and signed in one act.\n\n**ALL THREE OR NONE, and ' +
                         'that is not laziness.** A trust chain is only ' +
                         'worth anything whole: an Issuing CA with no ' +
                         'Intermediate above it is a two-tier chain wearing ' +
                         'a three-tier name, and a half-built hierarchy is ' +
                         'exactly the state in which somebody issues a ' +
                         'certificate that verifies here and nowhere else. A ' +
                         'failure at any tier stores nothing.\n\n**CALLING ' +
                         'IT AGAIN REPLACES WHAT IS THERE**, and everything ' +
                         'issued from the old hierarchy chains to nothing ' +
                         'the moment it does. This service keeps no copy of ' +
                         'what it issued, so none of it can be listed — the ' +
                         'key pairs are on the application entries and go on ' +
                         'SIGNING; what stops is this service being able to ' +
                         'see that it issued them.\n\n**WHERE IT SURVIVES A ' +
                         'RESTART depends on the mode.** In product mode the ' +
                         'hierarchy is written to `sts_keys`, sealed under ' +
                         'the same key-encryption key as the signing keys. ' +
                         'In development — the default — it lives exactly as ' +
                         'long as the process, which is the rule the signing ' +
                         'key follows and for the same reason.',
            requestBodyRequired: false,
            requestBody: {
              type: 'object',
              properties: {
                keyAlg: { type: 'string',
                          description: 'One of the ids in `keyAlgorithms` on ' +
                                       ...
                                       'ec-p384, ec-p521 or ed25519. One ' +
                                       'algorithm for all three tiers — the ' +
                                       'LEAF\'s signature algorithm is ' +
                                       'decided by the Issuing CA\'s key, ' +
                ...
                signatureAlg: { type: 'string',
                                description: 'One of the ids in ' +
                                             ...
                                             'algorithm", which is almost ' +
                                             'always ' +
                                             'what is wanted: an EC key\'s ' +
                                             'digest is decided by its ' +
                                             'CURVE, so naming one here can ' +
                                             'hand a P-521 key SHA-256 — ' +
                                             'legal, verifying, and ' +
                                             'nobody\'s intention. A pair ' +
                                             'whose families ' +
                                             'disagree is refused ' +
                ...
                country: { type: 'string',
                           description: 'The C=, two letters, optional. It ' +
                                        ...
                                        'than taste — several validators ' +
                                        'refuse a UTF8String country and ' +
                                        'report it as a signature problem.' },
                cn_root: { type: 'string',
                           description: 'The Root CA\'s common ' +
                                                        'name.' },
                cn_intermediate: { type: 'string',
                                   description: 'The Intermediate CA\'s ' +
                                                'common name.' },
                cn_issuing: { type: 'string',
                              description: 'The Issuing CA\'s common name.' },
                years_root: { type: 'integer',
                              description: 'How long the Root is valid. ' +
                                           'Defaults to the certificate ' +
                                           'profile\'s own twenty years.' },
                years_intermediate: { type: 'integer',
                                      description: 'Ten by default.' },
                years_issuing: { type: 'integer',
                                 description: 'Five by default.' }
              },
          ...
          { action: 'issue', operationId: 'issuePkiKeyPair',
            summary: 'Issue a signing key pair to an application — or, with ' +
                     ...
                     'Issuing CA',
            description: 'Generates a key pair, signs a leaf certificate for ' +
                         'it with this realm\'s Issuing CA, and writes the ' +
                         'whole lot onto that application\'s directory ' +
                         'entry.\n\n**WHICH ATTRIBUTES DEPENDS ON `purpose`, ' +
                         'AND THE TWO SETS SHARE NONE.** `jwt` (the default, ' +
                         'RFC 7523) writes seven — ' +
                         '`oauthAssertionPrivateKey`, ' +
                         '`oauthAssertionCertificate`, ' +
                         '`oauthAssertionCertificateChain`, ' +
                         '`oauthAssertionJwks` (with `x5c` and `x5t#S256` on ' +
                         'the key), `oauthAssertionKid`, ' +
                         '`oauthAssertionExpiresAt` and ' +
                         '`oauthAssertionKeySource` (`issued`). `saml` (RFC ' +
                         '7522) writes six under `oauthSamlAssertion*`, with ' +
                         'no JWKS among them because SAML has none — what a ' +
                         'party registers for that profile IS a certificate ' +
                         '— and with a `oauthSamlAssertionThumbprint` where ' +
                         'the other has a `kid`, because those are the ' +
                         'handles the two formats actually carry.\n\n**AN ' +
                         'APPLICATION MAY HOLD BOTH KEY PAIRS AND NEITHER ' +
                         'CAN SIGN FOR THE OTHER\'S PROFILE.** No verifier ' +
                         'reads the other set, so issuing one leaves the ' +
                         'other untouched and taking one off leaves the ' +
                         'other working. The SAML leaf also carries the RFC ' +
                         '7522 grant-type URI as a second URI ' +
                         'subjectAltName, so a certificate read out of ' +
                         'context says which profile it was issued ' +
                         'for.\n\n**THIS SERVICE KEEPS NO SECOND COPY OF THE ' +
                         'PRIVATE KEY.** `common/pki.js` hands it over once ' +
                         'and forgets it, so the entry is where it ' +
                         'lives.\n\n**AND IT IS SEALED THERE.** AES-256-GCM ' +
                         'under the key-encryption key, through ' +
                         '`common/keystore.js` — the same mechanism and the ' +
                         'same key that seal this service\'s own signing ' +
                         'keys and the CA hierarchy this leaf was issued ' +
                         'from — wherever that key outlives the process, ' +
                         'which is product mode. So an ldapsearch on TCP 389 ' +
                         'where every bind succeeds, an ldif file, a ' +
                         'database row and a backup of either hold ' +
                         '`$aesgcm$…`. `GET /admin-api/applications` opens ' +
                         'it for you, because it comes through ' +
                         '`common/applications.js` and you are holding an ' +
                         '`admin:read` token; in development mode it is in ' +
                         'the clear, where the key-encryption key is ' +
                         'ephemeral and would not survive the restart the ' +
                         'entry does. Issuing again replaces.\n\n**THE LEAF ' +
                         'IS A `digital-signature` CERTIFICATE and ' +
                         'deliberately carries no extended key usage.** What ' +
                         'it signs is a JWT, not a TLS handshake; giving it ' +
                         '`clientAuth` would make it usable for RFC 8705 as ' +
                         'well, which is a DIFFERENT credential with a ' +
                         'different registration attribute, and one ' +
                         'certificate quietly doing both is how a deployment ' +
                         'ends up unable to revoke either.\n\n**A KEY PAIR ' +
                         'IS NOT A TRUST DECISION.** It lets the application ' +
                         'SIGN, which is all RFC 7523 section 2.2 (client ' +
                         'authentication) needs. For section 2.1 — the ' +
                         'authorization grant — the `iss` it will use must ' +
                         'also be declared on `oauthAssertionIssuer`, ' +
                         'through POST /admin-api/applications/add. RFC 7522 ' +
                         'is the same two acts with its own declaration, ' +
                         '`oauthSamlAssertionIssuer`.\n\n**THE LIFETIME IS ' +
                         'CLAMPED to the Issuing CA\'s own expiry** rather ' +
                         'than refused where it would overshoot: the ' +
                         'ordinary cause is a five-year Issuing CA in its ' +
                         'fifth year.\n\n**AND SINCE 2026-09-11 THE SUBJECT ' +
                         'MAY BE A PERSON.** `target=person` issues to ' +
                         'somebody in `ou=users` instead: the same ' +
                         'hierarchy, the same profile, the same certificate, ' +
                         'written onto their own entry as ' +
                         ...
                         'shares no name with the application\'s and is read ' +
                         'by nothing else, which is the same rule the two ' +
                         'profiles above follow. RFC 7523 section 3 asks ' +
                         'only that `iss` be a unique identifier for the ' +
                         'issuer and says the `sub` of an authorization ' +
                         'grant typically identifies a resource owner, so a ' +
                         'person signing for themselves is the profile read ' +
                         'literally.\n\n**A PERSON MAY ONLY ASSERT ABOUT ' +
                         'THEMSELVES**, and the grant refuses anything else: ' +
                         'a key issued to one resource owner is that ' +
                         'person\'s credential rather than permission to ' +
                         'speak for the others, and the certificate carries ' +
                         '`urn:sts:person:<name>` as a URI subjectAltName so ' +
                         'that the rule holds for an assertion presented on ' +
                         'its `x5c` alone. A party that may assert about ' +
                         'OTHER people is an application with ' +
                         '`oauthAssertionIssuer` declared on it.\n\n**AND ' +
                         'THIS REPLY CARRIES THE PRIVATE KEY, WHICH THE ' +
                         'APPLICATION\'S DOES NOT.** An application\'s is ' +
                         'readable through `GET /admin-api/applications`, ' +
                         'which opens the seal for an `admin:read` token; a ' +
                         'person\'s entry is not drawn through any module ' +
                         'that would open it, so the alternatives were a ' +
                         'page that prints somebody\'s private key on every ' +
                         'visit or a key nobody can ever obtain. ' +
                         '`privateKeyPem` is in the reply to `target=person` ' +
                         'and in no other answer this API gives, and it is ' +
                         'not shown again.\n\n**AND SINCE 2026-09-13 A ' +
                         'PERSON MAY HOLD AN RFC 7522 KEY PAIR TOO**: ' +
                         '`target=person` with `purpose=saml` writes ' +
                         '`stsSamlAssertionCertificate`, its chain, a ' +
                         'thumbprint, the expiry, the provenance and a ' +
                         'sealed `stsSamlAssertionPrivateKey` — a fourth ' +
                         'attribute set sharing no name with the others. The ' +
                         'SAML 2.0 bearer grant reads it under the same ' +
                         'rule: the `<Subject>` must be that person.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
                identifier: { type: 'string',
                              description: 'The application to issue to — or ' +
                                           'the USERNAME, with ' +
                                           '`target=person`. It must ALREADY ' +
                                           'EXIST in this realm — creating ' +
                                           'one here would be this endpoint ' +
                                           'inventing an application, or a ' +
                                           'person, in order to give it a ' +
                                           'credential.' },
                target: { type: 'string', enum: ['application', 'person'],
                          description: 'WHO the key pair is for. Defaults to ' +
                                       '`application`, which is what every ' +
                                       'caller written before this field ' +
                                       'existed sends. `person` writes ' +
                                       '`stsAssertion*` (or ' +
                                       '`stsSamlAssertion*` for ' +
                                       '`purpose=saml`) onto a `ou=users` ' +
                                       'entry instead and returns the ' +
                                       'private key once.' },
                issuer: { type: 'string',
                          description: '`target=person` only: the `iss` (or ' +
                                       'SAML `<Issuer>`) their assertions ' +
                                       'will carry, written as ' +
                                       '`stsAssertionIssuer` or ' +
                                       '`stsSamlAssertionIssuer`. Defaults ' +
                                       'to their username, which the grant ' +
                                       'accepts without anything being ' +
                                       'declared — asking an operator to ' +
                                       'write a name down twice is a ' +
                                       'configuration step with no decision ' +
                                       'in it.' },
                purpose: { type: 'string', enum: ['jwt', 'saml'],
                           description: 'Which assertion profile the key ' +
                                        'pair is for: `jwt` for RFC 7523 and ' +
                                        '`saml` for RFC 7522. Defaults to ' +
                                        '`jwt`, which is what every caller ' +
                                        'written before this field existed ' +
                                        'sends and is why it is the default ' +
                                        'rather than a required field. A ' +
                                        'value this service does not know is ' +
                                        'REFUSED rather than defaulted — a ' +
                                        'caller that asked for a profile ' +
                                        'wants a certificate for something, ' +
                                        'and quietly handing back the other ' +
                                        'one would put a key ' +
                                        'pair on the wrong ' +
                                        'attribute set with nothing saying ' +
                                        'so.' },
                commonName: { type: 'string',
                              description: 'The leaf\'s CN. Defaults to the ' +
                                           'identifier. The identifier also ' +
                                           'goes into a URI subjectAltName ' +
            ...
            responseDescription: 'The `purpose`, the attributes it wrote, ' +
                                 'the key handle (`kid` and `thumbprint` — ' +
                                 'both are returned whichever profile was ' +
                                 'asked for, because both are computed at ' +
                                 'issuance), the expiry and the JWS ' +
                                 'algorithm the issued key signs with. ' +
                                 'The key material itself is on the ' +
                                 'application entry.' },
          ...
          { action: 'revoke', operationId: 'revokePkiKeyPair',
            ...
            description: 'Clears the attributes the issue wrote FOR ONE ' +
                         'PROFILE — seven for `jwt` and six for `saml`. An ' +
                         'application commonly holds both key pairs and this ' +
                         'takes ONE off; clearing both would be an operation ' +
                         'whose name said one thing and did two.\n\n**THIS ' +
                         'OPERATION IS NOT THE ONE THAT REVOKES A ' +
                         'CERTIFICATE, AND SINCE 2026-09-11 THERE IS ONE.** ' +
                         '`revoke-certificate` puts a serial on an issuer\'s ' +
                         'revocation list; this takes a key pair off an ' +
                         'application entry and puts nothing on any list. ' +
                         'After it the certificate is still valid, still ' +
                         'chains to this realm\'s Root, and would still ' +
                         'verify anywhere that trusts that Root. What ' +
                         'changes is that THIS service will no longer accept ' +
                         'an assertion signed with that key, because the key ' +
                         'is no longer registered against the application. ' +
                         'The name is `revoke` because that is the word on ' +
                         'the button; the description is where the claim is ' +
                         'kept honest.\n\n**`target=person` TAKES A ' +
                         'PERSON\'S KEY PAIR OFF INSTEAD**, and it clears ' +
                         'the `stsAssertionIssuer` DECLARATION with it where ' +
                         'the application arm deliberately leaves ' +
                         '`oauthAssertionIssuer` alone. The difference is a ' +
                         'fact about the two: an application may hold a JWKS ' +
                         'it registered itself beside the one this service ' +
                         'issued, and a person may not — everything in ' +
                         '`stsAssertion*` was put there by the issue, so ' +
                         'leaving the declaration would leave somebody ' +
                         'declared as an issuer with no key to issue with. ' +
                         'Since 2026-09-13 it takes ONE PROFILE ' +
            ...
            requestBody: {
              ...
              properties: {
                identifier: { type: 'string',
                              description: 'The application to take the key ' +
                                           'pair off — or the USERNAME, with ' +
                                           '`target=person`.' },
                target: { type: 'string', enum: ['application', 'person'],
                          description: 'WHOSE key pair. Defaults to ' +
                                       '`application`, which is what every ' +
                                       'caller written before this field ' +
                                       'existed sends. `person` clears one ' +
                                       'profile\'s set off a `ou=users` ' +
                                       'entry, the declaration included — ' +
                                       '`stsAssertion*` or, for ' +
                                       '`purpose=saml`, `stsSamlAssertion*`.' },
                purpose: { type: 'string', enum: ['jwt', 'saml'],
                           description: 'Which profile\'s key pair to take ' +
                                        'off. Defaults to `jwt`. The other ' +
            ...
            responseDescription: 'Whether anything was taken off, and the ' +
                                 'sentence saying what that did and did not ' +
                                 'do.' },
          ...
          { action: 'upload-certificate', operationId: 'uploadPkiCertificate',
            summary: 'Replace an application\'s or a person\'s key pair with ' +
                     'a certificate it already holds — issued by this ' +
                     'realm\'s CA, or by another with its full chain',
            description: 'The application generated its own key pair and ' +
                         'brings the CERTIFICATE; this writes it over the ' +
                         'key pair this service manages for one profile, ' +
                         'onto the same attributes an `issue` writes — seven ' +
                         'for `jwt`, six for `saml` — with the private key ' +
                         'attribute CLEARED and ' +
                         '`oauthAssertionKeySource` / ' +
                         '`oauthSamlAssertionKeySource` saying where it came ' +
                         'from. **No private key is taken**: an upload ' +
                         'carrying one is refused and nothing is ' +
                         'stored.\n\n**WHAT THE CHAIN MUST BE DEPENDS ON WHO ' +
                         'ISSUED IT.** A certificate from THIS REALM\'s own ' +
                         'certificate authority may be sent alone — the ' +
                         'service holds every tier above it — and is held to ' +
                         'the path check an `x5c` is: it must pass through ' +
                         'this realm\'s own Intermediate and nothing on it ' +
                         'may be revoked (`uploaded-realm-ca`). A ' +
                         'certificate from ANY OTHER authority must arrive ' +
                         'with its WHOLE chain, every intermediate and a ' +
                         'SELF-SIGNED root, in either field and in any order ' +
                         '(`uploaded-external-ca`). Every link is verified — ' +
                         'signature, issuer name, validity — every issuer ' +
                         'must be a CA permitted keyCertSign within its path ' +
                         'length constraint, the leaf must not be a CA and ' +
                         'must permit digitalSignature, and its revocation ' +
                         'is checked the way a registered certificate is ' +
                         'when it is used. An incomplete chain, an unrelated ' +
                         'extra certificate, a self-signed leaf, and a chain ' +
                         'to this SERVICE\'s Root through ANOTHER realm\'s ' +
                         'Intermediate are each refused by name. The ' +
                         'external root need not be trusted by anything ' +
                         'here: the application is registering a KEY and the ' +
                         'chain is the evidence for it.\n\n**THE KEY MUST BE ' +
                         'ONE THE PROFILE\'s VERIFIER CAN USE**: RSA of at ' +
                         'least 2048 bits, ECDSA on P-256, P-384 or P-521 — ' +
                         'and for `jwt` also secp256k1 or Ed25519. The chain ' +
                         'is stored WITH its root for an external authority, ' +
                         'because revocation checking needs every issuer; ' +
                         'the JWKS written for `jwt` carries it in ' +
                         '`x5c`.\n\nLike `issue`, a key pair is not a trust ' +
                         'decision: the issuer the application will assert ' +
                         'as still has to be declared for section ' +
                         '2.1.\n\n**`target=person` (2026-09-13) REPLACES A ' +
                         'PERSON\'s KEY PAIR** — `stsAssertion*` or ' +
                         '`stsSamlAssertion*` on their `ou=users` entry — ' +
                         'under the same chain rules, with one more: a ' +
                         'certificate this realm issued must name THAT ' +
                         'person (`urn:sts:person:<name>`), so an ' +
                         'application\'s leaf or another person\'s is ' +
                         'refused. For an application, a leaf this realm ' +
                         'issued to a PERSON is refused. A self-signed ' +
                         'certificate is refused for a person with no ' +
                         'by-value alternative, and the grant still holds an ' +
                         'uploaded key to assertions about the person alone.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
                identifier: { type: 'string',
                              description: 'The application, exactly as the ' +
                                           'registry holds it — or the ' +
                                           'USERNAME, with `target=person`.' },
                purpose: { type: 'string', enum: ['jwt', 'saml'],
                           description: 'Which profile\'s key pair to ' +
                                        'replace. Defaults to `jwt`; the ' +
                                        'other is untouched.' },
                certificate: { type: 'string',
                               description: 'The leaf certificate, PEM. It ' +
                                            'may be followed by its chain in ' +
                                            'the same value.' },
                chain: { type: 'string',
                         description: 'The certificates above it, PEM, in ' +
                                      'any order: every intermediate and the ' +
            ...
            responseDescription: 'The source, the subject and issuer, the ' +
                                 'chain it was checked through, the key ' +
                                 'handle and the expiry — or the refusal ' +
                                 'naming what is wrong with the chain.' },

          { action: 'clear', operationId: 'clearPkiChain',
            summary: 'Remove this realm\'s certificate authority entirely',
            description: 'Destructive, and it says so: every certificate ' +
                         'issued from the hierarchy chains to nothing the ' +
                         'moment this returns, and this service keeps no ' +
                         'copy of what it issued, so nothing here can list ' +
                         'what broke. The key pairs stay on the application ' +
                         'entries and go on signing; what stops is this ' +
                         'service being able to see that it issued them.',
            requestBodyRequired: false,
            requestBody: { type: 'object', properties: {}, examples: [{}],
                           additionalProperties: false },
            responseDescription: 'How many certificates had been issued from ' +
                                 'the hierarchy that was removed.' },

          // ===================================================================
          // THE CERTIFICATE & KEY CONFIGURATION PANE (2026-09-10).
          //
          // Eight operations, and every one of them takes and returns THE WHOLE
          // FORM. That is not an API shaped by a page: the pane has a hundred
          // and fifteen fields, `apply-profile` rewrites twenty-two of them,
          // and a caller that had to reconstruct the result itself would be a
          // second implementation of what a profile MEANS. So the reply carries
          // `draft` — the form as it should now be — and a caller does
          // `apply-profile`, edits three fields of what came back, and posts it
          ...
          // form over one encoder; `GET /admin-api/pki` publishes the whole
          // list as `workbench.fields`, so a client reads the vocabulary from
          // the service rather than from a copy of it in a document.
          // ===================================================================
          { action: 'apply-profile', operationId: 'applyPkiProfile',
            summary: 'Fill the certificate form in from a profile',
            description: 'Rewrites the twenty-two extension boxes, the ' +
                         'default validity and the profile’s Common Name ' +
                         'from the profile named in `pki_profile`, and ' +
                         'narrows the two algorithm menus to the approach in ' +
                         '`pki_pq_mode`. Nothing is issued.\n\n**A COMMON ' +
                         'NAME SOMEBODY TYPED IS NEVER OVERWRITTEN** — an ' +
                         'empty one, or one still holding ANY profile’s own ' +
                         'default, is replaced. Picking Intermediate ' +
                         'CA after Root CA would otherwise ' +
                         'issue an intermediate called "RootCA", which ' +
                         'chains ' +
                         'correctly and reads as a bug for as long as it ' +
                         'takes somebody to notice.\n\nPost the whole form ' +
                         'back; anything omitted falls to its default rather ' +
                         'than being kept.',
            requestBodyRequired: false,
            requestBody: { type: 'object', properties: {},
          ...
          { action: 'generate-keys', operationId: 'generatePkiKeyPair',
            summary:
              'Generate a key pair into the form without issuing anything',
            description: 'Puts a fresh pair in `pki_private_key` and ' +
                         '`pki_public_key`, in the algorithm named by ' +
                         '`pki_key_alg`.\n\n**IT EXISTS BECAUSE GENERATION ' +
                         'CAN BE SLOW.** An SLH-DSA pair takes seconds and ' +
                         'this process is single-threaded, so making one and ' +
                         'then issuing four certificates from it (with ' +
                         '`pki_reuse_key`) is the difference between a ' +
                         'usable surface and one that stalls the whole ' +
                         'service four times over.',
            requestBodyRequired: false,
            requestBody: { type: 'object', properties: {},
                           examples: [{ pki_key_alg: 'ec-p384' }],
                           additionalProperties: true },
            responseDescription:
              'The form with the new pair in it, in `draft`.' },

          { action: 'generate-alt-keys', operationId: 'generatePkiAltKeyPair',
            summary: 'Generate the alternative (hybrid) key pair',
            description: 'The second key of a hybrid certificate — ITU-T ' +
                         'X.509 (2019) clause 9.8’s ' +
                         '`subjectAltPublicKeyInfo`. The list is the ' +
                         'post-quantum algorithms that can sign, because a ' +
                         'hybrid certificate whose second key is also RSA is ' +
                         'a certificate signed twice by the same century. ' +
                         'Issuing with `pki_pq_mode=hybrid` generates one ' +
                         'anyway if the boxes are empty.',
            requestBodyRequired: false,
            requestBody: { type: 'object', properties: {},
          ...
          { action: 'issue-certificate', operationId: 'issuePkiCertificate',
            ...
            description: 'The pane’s own button. Generates a key pair ' +
                         ...
                         'builds the certificate from the subject, the ' +
                         'validity and the twenty-two extensions, signs it ' +
                         'with the authority `pki_issuer` names — or with ' +
                         'its own key under a self-signed profile — and ' +
                         'keeps both in this realm’s store.\n\n**THE ' +
                         'SIGNATURE IS CONSTRAINED BY THE ISSUER’S KEY AND ' +
                         'NOT THE SUBJECT’S.** A pair whose families ' +
                         'disagree is refused HERE with the list of what ' +
                         'that key can produce, rather than by the ' +
                         'primitive, which reports it as a key-usage error ' +
                         'naming neither.\n\n**A REFUSAL CARRIES THE FORM ' +
                         'BACK** in `draft`: a hundred and fifteen fields ' +
                         'discarded because one line of a subjectAltName ' +
                         'would not parse is not a refusal anybody can act on.',
            requestBodyRequired: true,
            requestBody: { type: 'object', properties: {},
                           examples: [{ pki_profile: 'tls-server',
                                        ...
                                        pki_save_keys: '1' }],
                           additionalProperties: true },
            responseDescription: 'The stored object (no private key in it), ' +
                                 'and the form with a FRESH SERIAL in it — a ' +
                                 'serial that stayed put would be re-used by ' +
                                 'the next certificate the same authority ' +
                                 'signs, and two certificates from one ' +
                                 'issuer sharing a serial are ' +
                                 'indistinguishable to anything that ' +
                                 'revokes, caches or pins by (issuer, ' +
                                 'serial).' },

          { action: 'use-key', operationId: 'usePkiStoredKey',
            summary: 'Load a stored object’s key pair back into the form',
            description: 'What a CA renewing its own certificate does. It ' +
                         'ticks `pki_reuse_key` itself, because a pair ' +
                         'loaded into the boxes and then silently replaced ' +
                         'by a fresh one at the next issue is ' +
                         'the most confusing thing this surface ' +
          ...
          { action: 'remove-object', operationId: 'removePkiObject',
            ...
            description: 'The key pair is gone. **Anything it ISSUED is ' +
                         'kept** — those certificates ' +
                         'are still valid documents — and ' +
            ...
            requestBody: { type: 'object',
                           ...
                           additionalProperties: true },
            responseDescription: 'The sentence saying what that did and did ' +
                                 'not do.' },

          { action: 'clear-store', operationId: 'clearPkiObjects',
            summary: 'Empty this realm’s object store',
            description: 'Every key pair and certificate the pane issued in ' +
                         'this realm is discarded. **The three-tier ' +
                         'hierarchy is NOT touched** — that is `clear` — and ' +
                         'anything these keys signed is still a valid ' +
                         'document that still chains to whatever signed IT.',
            requestBodyRequired: false,
            requestBody: { type: 'object', properties: {}, examples: [{}],
                           additionalProperties: true },
          ...
          { action: 'export', operationId: 'exportPkiKeyPair',
            ...
            requestBody: { type: 'object',
                           ...
                           additionalProperties: true },
            responseDescription: 'One entry per member of `files`: its name, ' +
                                 'its media type and its bytes as base64. A ' +
                                 'DER export is two files and the console’s ' +
                                 'own door sends only the private one, which ' +
          ...
          // starred names cannot collide with one. Omitted, it means the realm
          // the request arrived in, which is what every other control here
          ...
          { action: 'build-root', operationId: 'buildPkiRoot',
            ...
            description: 'One Root, shared by every realm and by the process ' +
                         'branch — which is what lets an operator install ' +
                         'ONE anchor and have it cover LDAPS 636, the main ' +
                         'port and every token this service ' +
                         'signs.\n\n**REPLACING IT RE-ISSUES EVERY BRANCH IN ' +
                         'THE SAME ACT**, because an Intermediate still ' +
                         'hanging from the old Root chains to nothing — a ' +
                         'new Root with the old branches under it is a ' +
                         'service whose tree does not reach its own anchor, ' +
                         'and every path check fails while the page looks ' +
                         'right.\n\n**ANYTHING TRUSTING THE OLD ROOT STOPS ' +
                         'TRUSTING THIS SERVICE.** That is the cost of one ' +
                         'anchor covering everything. The signing keys ' +
            ...
            requestBody: {
              ...
              properties: {
                ...
                years: { type: 'integer',
                         description:
                           'How long it is valid for. Defaults to the ' +
          ...
          { action: 'build-scope', operationId: 'buildPkiScope',
            summary: 'Build or rebuild one scope’s branch — its Intermediate ' +
                     'and every Issuing CA under it',
            description: 'A scope is a trust realm or the process. **ALL OF ' +
                         'IT OR NONE OF IT**: a branch with an Intermediate ' +
                         'and two of its three Issuing CAs is the state in ' +
                         'which one use case silently has no authority and ' +
                         'its keys come out uncertified, so a failure ' +
                         'anywhere stores nothing.\n\n**THE ROOT IS NOT ' +
                         'TOUCHED** — every other scope hangs from it — and ' +
                         'an IMPORTED authority in this branch is left alone ' +
                         'unless `replaceImported` says otherwise: somebody ' +
                         'who pasted a corporate CA in did not press this to ' +
                         'have it thrown away.\n\nEverything ' +
                         'the old authorities had certified ' +
          ...
          { action: 'reissue-use-case', operationId: 'reissuePkiUseCase',
            ...
            description: 'Re-issues that one authority from its scope’s ' +
                         ...
                         'everything that hung under it. **The other use ' +
                         'cases are untouched**, which is the whole reason ' +
                         'each has an authority of its own rather than ' +
                         'sharing one: an operator who wants to replace what ' +
                         'signs their SAML documents should not thereby ' +
                         'replace what signs their tokens.\n\nIt is NOT the ' +
                         'same as `recertify` — see that one.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
                ...
                useCase: { type: 'string',
                           description: 'One of `jose`, `xml`, `assertions`, ' +
                                        ...
                                        'in the wrong ' +
                                        'scope is refused and the refusal ' +
                                        'says which scope it belongs to.' }
              },
              required: ['useCase'],
              examples: [{ scope: '', useCase: 'jose' }],
              additionalProperties: false },
            responseDescription: 'How many certificates were re-minted under ' +
                                 'the new authority.' },

          { action: 'recertify', operationId: 'recertifyPkiUseCase',
            summary:
              'Renew the certificates under one Issuing CA, keeping the keys',
            description: 'Re-issues every certificate that authority has ' +
                         'minted, from the SAME authority, with fresh ' +
                         'serials and a fresh validity window.\n\n**THE KEYS ' +
                         'ARE UNTOUCHED, WHICH IS WHAT MAKES THIS A ' +
                         'RENEWAL**: the subject public key is read back out ' +
                         'of the certificate being replaced, so nothing that ' +
                         'verifies against the published JWKS stops ' +
                         'verifying. `reissue-use-case` is the other one — ' +
                         'it replaces the authority, and everything it had ' +
                         'signed chains to nothing until it is re-minted.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
          ...
          { action: 'import-ca', operationId: 'importPkiCa',
            summary:
              'Use a certificate authority this service did not generate',
            description: 'Paste a CA certificate and its private key. With ' +
                         ...
                         'otherwise it becomes one scope’s Issuing CA for ' +
                         'one use case, and everything under that authority ' +
                         'is re-minted from it.\n\n**THREE CHECKS HAPPEN ' +
                         'BEFORE ANYTHING IS STORED**, and the middle one is ' +
                         'the one that matters: both halves must be present ' +
                         '(a certificate with no key is a trust anchor ' +
                         'rather than an authority), the KEY MUST BELONG TO ' +
                         'THE CERTIFICATE (an authority whose key is ' +
                         'somebody else’s issues certificates that verify ' +
                         'nowhere, and the failure arrives at a relying ' +
                         'party rather than here), and the certificate must ' +
                         'be a CA at all — `cA:FALSE` means nothing it signs ' +
                         'is accepted by a path validator.\n\n**THE KEY IS ' +
                         'STORED AS THIS SERVICE STORES ITS OWN**: in the ' +
                         'keystore row, sealed under the key-encryption key ' +
                         'wherever that key outlives the process.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
              ...
              additionalProperties: false },
            responseDescription: 'The authority as it was read, and what has ' +
                                 'to happen next.' },
          ...
          { action: 'pin-key', operationId: 'pinPkiKeyPair',
            ...
            description: 'A SLOT is a use case and an algorithm — `jose` / ' +
                         ...
                         'operator actually wants: *what signs my ES384*, ' +
                         'not *the key that happened to be there on ' +
                         'Tuesday*. A `kid` would not do, because it changes ' +
                         'whenever the key does.\n\n**WITH NO CERTIFICATE ' +
                         'THIS SERVICE ISSUES ONE** from that use case’s ' +
                         'Issuing CA, so your key chains to this service’s ' +
                         'Root exactly as a generated one would — which is ' +
                         'what somebody who wants their own key under this ' +
          ...
          // the whole time: the route is `/pki/:action`, so an undeclared
          // action still WORKS — what it does not do is appear in the document,
          // which is the only place a machine driving this API can read the
          // repertoire from.
          // ---------------------------------------------------------------
          { action: 'revoke-certificate', operationId: 'revokePkiCertificate',
            summary: 'Put a serial on one certificate authority\'s ' +
                     'revocation list — which is NOT `revoke`',
            description: 'RFC 5280 revocation: the serial goes on the named ' +
                         'authority\'s CRL and its OCSP responder answers ' +
                         '`revoked` for it from that moment. The list is ' +
                         'rebuilt and signed on demand at ' +
                         '`/pki/crl/{scope}/{ca}`, so the next fetch there ' +
                         'already carries the entry, and the copy published ' +
                         'into the directory under `ou=crl` is rewritten too ' +
                         '— the `ldap://` and `ldaps://` addresses in every ' +
                         'certificate this authority signed point at a ' +
                         'DOCUMENT, and a document goes on saying what it ' +
                         'said before unless somebody rewrites it.\n\n**THIS ' +
                         'IS NOT `revoke`, WHICH IS A DIFFERENT ACT WITH THE ' +
                         'SAME WORD IN IT.** That one takes an issued key ' +
                         'pair off an application\'s directory entry and ' +
                         'puts nothing on any list. This one puts a serial ' +
                         'on a list and takes nothing off anybody. The names ' +
                         'are deliberately not `revoke` and `revoke-key`: ' +
                         'this action list is published and a machine ' +
                         'chooses from it, so renaming the older one to make ' +
                         'room would have broken every caller that already ' +
                         'had it.\n\n**IT IS IDEMPOTENT AND THE EARLIER ' +
                         'ENTRY WINS** — revoking twice answers 200 with ' +
                         '`already: true` and does not move the date, ' +
                         'because a validator is entitled to act on the ' +
                         'first moment it was told about.\n\n**AND THIS ' +
                         'SERVICE DOES NOT CONSULT ITS OWN LISTS.** A client ' +
                         'certificate revoked here still authenticates on ' +
                         'the main port and LDAPS 636, because those check ' +
                         'the anchors on `/tls/trust` and fetch nothing. ' +
                         'What this operation buys is that a relying party ' +
                         'which DOES check can now find out.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
                scope: { type: 'string',
                         description: 'Which branch the issuer is in: a ' +
                                      'realm id, `*service` for the one ' +
                                      'Root, `*process` for the process ' +
                                      'branch, or omitted for the realm this ' +
                                      'request arrived in.' },
                ca: { type: 'string',
                      description: 'The issuing authority, as `GET ' +
                                   '/admin-api/pki` names it in ' +
                                   '`revocation`: `root`, `intermediate`, or ' +
                                   'a use case id for an Issuing CA. **A ' +
                                   'revocation is made BY AN ISSUER** — a ' +
                                   'serial is only unique within one, which ' +
                                   'is why there is a list per CA and ' +
                                   'not one per realm.' },
                serialHex: { type: 'string',
                             description: 'The certificate\'s serial number ' +
                                          'in hex. Case, leading zeros and ' +
                ...
                reason: { type: 'string',
                          ...
                          description: 'RFC 5280 section 5.3.1. Defaults to ' +
                                       '`superseded`, which is what this ' +
                                       'service uses when it rotates or ' +
                                       'reissues something itself. ' +
                                       '`certificateHold` is the only one ' +
                                       '`release-hold` can undo; ' +
                                       '`unspecified` writes no reason code ' +
                                       'at all, which is what that value ' +
                                       'means on the wire.' },
                subject: { type: 'string',
                           description: 'Optional. The subject DN, kept for ' +
                                        'the operator reading ' +
                                        'the list — it is not ' +
                                        'on the CRL, which carries serials.' },
                note: { type: 'string',
                        description: 'Optional. Why, in your own words, for ' +
                                     'the same reader.' }
              },
              ...
              additionalProperties: false },
            responseDescription: 'The entry as it now stands, `already` when ' +
                                 'it was on the list ' +
                                 'before, and the sentence ' +
          ...
          { action: 'release-hold', operationId: 'releasePkiCertificateHold',
            summary: 'Lift a `certificateHold` — the one revocation RFC 5280 ' +
                     'lets you undo',
            description: 'Takes the serial back off the authority\'s list ' +
                         'and bumps the CRL number, so the next CRL and the ' +
                         'next OCSP answer call it good again.\n\n**ONLY A ' +
                         'HOLD.** A serial revoked for any other reason is ' +
                         'refused with the reason named: everything else RFC ' +
                         '5280 defines is PERMANENT, and a validator is ' +
                         'entitled to cache a permanent revocation for as ' +
                         'long as the CRL it read says it is fresh — so ' +
                         'undoing one here would produce a certificate this ' +
                         'service calls good and half the world still calls ' +
                         'revoked.\n\n**AND EVEN A HOLD IS NOT LIFTED ' +
                         'INSTANTLY ANYWHERE BUT HERE**: a validator holding ' +
                         'the previous list goes on refusing until that ' +
            ...
            requestBody: {
              ...
              properties: {
                scope: { type: 'string',
                         description: 'As on `revoke-certificate`: a realm ' +
                                      'id, `*service`, ' +
                                      '`*process`, or omitted for ' +
                                      'the realm this request arrived in.' },
                ca: { type: 'string',
                      description: 'The issuing authority the hold is on. It ' +
                                   'has to be the same one that took it — a ' +
                                   'serial is only unique within an issuer.' },
                serialHex: { type: 'string',
              ...
              examples: [{ ca: 'intermediate', serialHex: '0a1b2c3d' }],
              additionalProperties: false },
            responseDescription: 'The sentence saying the hold is lifted, ' +
                                 'and that a validator which cached the ' +
                                 'earlier list still has it.' }
        ] },

      { method: 'GET', path: BASE + '/scim/monitor', tag: 'SCIM',
        operationId: 'getScimMonitor',
        summary: 'How many SCIM calls there have been, from whom, of what ' +
                 'kind, and how many failed',
        description: 'Everything /admin/scim/monitor draws: the call totals, ' +
                     'one row per operation with its successes, failures, ' +
                     'latency and bytes returned, one row per resource type, ' +
                     'one row per authenticated client, the authentication ' +
                     'schemes with the ones at zero included, what went back ' +
                     'by status class, status and `scimType`, and the last ' +
                     'fifty requests individually.\n\n**A CLIENT IS AN ' +
                     'AUTHENTICATED PRINCIPAL, NOT A CONNECTION.** SCIM is ' +
                     'stateless HTTP — no session, no registration, nothing ' +
                     'to be connected — so `authentication.distinct` is how ' +
                     'many different names have successfully authenticated ' +
                     'since this process started. It never goes down: a ' +
                     'provisioning client that has stopped calling is ' +
                     'indistinguishable from one that is between ' +
                     'calls.\n\n**A REFUSED CALLER IS NOT A CLIENT.** Calls ' +
                     'the gate turned away are counted in ' +
                     '`authentication.refused` and appear in no `clients` ' +
                     'row, even when the credential carried a name — Basic ' +
                     'and Digest both put one on the wire. Attributing ' +
                     'traffic to an identity this service declined to ' +
                     'believe is the one mistake this reply could make that ' +
                     'would matter.\n\n**THE OPERATION COUNTS DO NOT SUM TO ' +
                     '`calls`.** One `POST /scim/v2/Bulk` carrying five ' +
                     'creates is one `bulk` AND five `create`s, because each ' +
                     'of the five really is performed.\n\n**AN ABSENT ' +
                     'MEASUREMENT IS NULL AND NOT ZERO.** `averageMs`, ' +
                     '`maxMs` and `successRate` are null where nothing has ' +
                     'been called: an average over no samples is absent, and ' +
                     'a 100% success rate on zero requests is the most ' +
                     'misleading number here.\n\nThe counters are IN MEMORY, ' +
                     'start with the process (`since`) and are PER TRUST ' +
                     'REALM, like the directory SCIM writes into. The ' +
                     'durable record of what SCIM was asked ' +
                     'to do is GET /admin-api/audit, which has ' +
                     'the actor and the target as well as the count.',
        mirrors: 'GET /admin/scim/monitor',
        responseDescription: 'The call totals, the per-operation, ' +
                             'per-resource, per-client and per-scheme ' +
                             'breakdowns, and the recent requests.',
        responseSchema: { $ref: '#/components/schemas/ScimMonitor' },
        handler: function (req, res) {
          log.debug("Entering the management API SCIM monitor endpoint.");
          self.sendJson(res, 200, adminViews.scimMonitorJson(req));
      ...
      { method: 'GET', path: BASE + '/signals', tag: 'Shared Signals',
        ...
        description: 'This service\'s own admin console is a registered ' +
                     'Shared Signals receiver: it has a stream of its own ' +
                     '(`sts-admin-console`), seeded in EVERY trust realm, ' +
                     'asking for every CAEP and every RISC event type, and ' +
                     'each event is POSTed to it over RFC 8935 push at ' +
                     '/admin/signals/receive with that stream\'s own bearer ' +
                     'token. This is what arrived.\n\n**IT IS SEEDED IN ' +
                     'EVERY REALM AND THE CONSOLE\'S CLIENT ENTRY IS IN ' +
                     'ONE**, and the disagreement is deliberate: a client ' +
                     'entry is about signing somebody IN, and this ' +
                     'console\'s gate reads the DEFAULT realm\'s session ' +
                     'wherever it is reached; a stream is about what ' +
                     'HAPPENED, and events happen in the realm they happen ' +
                     'in.\n\n**READ `status.why` BEFORE CONCLUDING NOTHING ' +
                     'HAS HAPPENED.** An empty `received` has five causes ' +
                     'and only one of them is that: `ssf.enabled` off, ' +
                     '`ssf.internalReceivers` off, the stream deleted (it is ' +
                     'an ORDINARY stream — delete it here or at /admin/ssf ' +
                     'and it stays deleted until a restart), ' +
                     '`ssf.pushDelivery` off, or `caep.enabled` / ' +
                     '`risc.enabled` off under it.\n\n**WHAT A PERSON SEES ' +
                     'IS A DIFFERENT RECEIVER.** The user portal has a ' +
                     'stream of its own and shows each person only the ' +
                     'events whose subject is them; there is no operation ' +
                     'here for that view, because it is a person\'s own page ' +
                     'and not an administrative one.\n\nThe inbox is IN ' +
                     'MEMORY, per trust realm, capped at ' +
                     '`ssf.maxReceivedEvents`. The durable record of ' +
                     'every delivery is GET /admin-api/audit, which ' +
                     'cannot be cleared.',
        mirrors: 'GET /admin/signals',
        parameters: [
          { name: 'sigq', in: 'query', required: false,
            schema: { type: 'string' },
            description: 'Narrows the list to rows whose event name, type ' +
                         'URI, subject, issuer, audience, `jti` or stream id ' +
                         'contains this. One box over all of them, because a ' +
                         'reader arrives holding one and does not know which ' +
                         'column it is in.' },
        ...
        responseDescription: 'This receiver and its stream, and the ' +
                             'delivered events.',
        responseSchema: { $ref: '#/components/schemas/Signals' },
        handler: function (req, res) {
          log.debug("Entering the management API signals endpoint.");
          self.sendJson(res, 200, adminViews.signalsJson(req));
      ...
      { method: 'POST', route: BASE + '/signals/:action', tag: 'Shared Signals',
        ...
        handler: function (req, res) {
          ...
          const body = parseBody(req);
          const result = adminActions.signalsAction(self.withAction(req, body));
          if (!result.ok) {
        ...
        actions: [
          { action: 'clear', operationId: 'clearSignalsInbox',
            ...
            description: 'Drops every delivered event held for the admin ' +
                         ...
                         'TOUCH THE STREAM.** Clearing what a receiver has ' +
                         'been shown and tearing down the agreement to send ' +
                         'it more are two different acts; the second one is ' +
                         'POST /admin-api/ssf/delete-stream. This receiver ' +
                         'goes on taking delivery the moment this ' +
                         'returns.\n\n**AND IT DOES NOT TOUCH THE RECORD.** ' +
                         'Every delivery wrote an `ssf.event.receive` row to ' +
                         'the audit log, which has no clear operation ' +
                         'anywhere — which is what makes it the durable half.',
            requestBodyRequired: false,
            // THE EMPTY EXAMPLE IS WHAT DRIVES IT.
            // `tests/vendored/sts_admin_api_operations.js` replays every POST
            // that carries an example and then asserts that every documented
            // operation was driven by something — so an action taking no
            // parameters and declaring no example is covered by nothing and
            // reported by nothing, which is what this one was until that ledger
            // went red on it. `{}` is the shape `revoke-all` and `clear-store`
      ...
      //
      // Rule 7: `/admin/tls/trust` has two controls, so this resource has the
      // same two, through `truststoreAction()` and `truststoreJson()` in
      // `admin-core/`, which reach `tls/tls_server.js` through
      // `admin.setTruststore()`.
      //
      // **BOTH OPERATIONS ARE PINNED TO THE FRONT PROCESS** in
      // `workers.dispatch` mode (`common/request_pool.js`'s
      // `NEVER_DISPATCHED`): the anchors are the configuration of listeners
      // only that process holds, and a worker changing its own copy would
      // change nothing a handshake reads. They are FANNED out nowhere, answered
      // nowhere else, and carry no realm: the array is the process's, so every
      // realm prefix reads and writes the same one.
      //
      // **THIS IS THE RUNTIME DOOR PRODUCT MODE HAD NONE OF.** `POST
      // /tls/trust` needs no credential and product mode refuses it; this API's
      // own access token is the credential here, `admin:read` to list and
      // `admin:write` to change — the gate below every operation, by method, so
      // nothing in these two rows re-checks it.
      // -----------------------------------------------------------------------
      { method: 'GET', path: BASE + '/tls/trust', tag: 'TLS',
        operationId: 'getTruststore',
        summary:
          'Every client-certificate trust anchor, and where each came from',
        description: 'The anchors LDAPS 636 and the main HTTPS port ' +
                     'verify a CLIENT certificate against. A certificate ' +
                     'that chains to one of these is verified, and a ' +
                     'verified certificate is an identity here — it starts a ' +
                     'sign-on session and it is what admits a remote XACML ' +
                     'PEP to `/xacml/pep/*` — so this list is whose ' +
                     'certificates this service believes.\n\nEach row ' +
                     'carries the subject, issuer, serial, validity, SHA-256 ' +
                     'fingerprint (the colon form `openssl x509 -fingerprint ' +
                     '-sha256` prints), whether it is a CA, the PEM, and ' +
                     '`source`: `file` for one read from ' +
                     '`tls.trustAnchorsFile` at startup, `runtime` for one ' +
                     'added while the process was running, with `persisted` ' +
                     'saying whether it was written down.\n\n**A RUNTIME ' +
                     'ANCHOR IS PERSISTED** in `ou=trustAnchors` in the ' +
                     'default realm\'s directory, so it survives a restart ' +
                     'wherever the directory does (`persistence.mode` ldif ' +
                     'or postgres) and reaches every other process against ' +
                     'the same store. A `file` anchor is not stored there ' +
                     'and comes back at the next start however it was ' +
                     'removed.\n\n**ONE TRUSTSTORE FOR THE PROCESS, NOT PER ' +
                     'REALM.** The sockets that verify a client certificate ' +
                     'are shared by every trust realm, so this answers the ' +
                     'same list under every realm prefix.\n\n**NO ' +
                     'PRIVATE KEY IS IN THIS REPLY** ' +
                     '— the truststore holds certificates and nothing else.',
        mirrors: 'GET /admin/tls/trust',
        parameters: this.pagingParameters(),
        responseDescription: 'The anchors on this page with their paging, ' +
                             'the counts by source, the configured anchors ' +
                             'file, and which doors can change the list.',
        responseSchema: { type: 'object',
        ...
        handler: function (req, res) {
          ...
          self.sendJson(res, json.installed ? 200 : 503, json);
      ...
      { method: 'POST', route: BASE + '/tls/trust/:action', tag: 'TLS',
        ...
        handler: function (req, res) {
          ...
          const body = parseBody(req);
          const result = adminActions.truststoreAction(self.withAction(req,
              body),
                                                       { via: 'api' });
          if (!result.ok) {
        ...
        actions: [
          { action: 'add', operationId: 'addTrustAnchors',
            ...
            description: 'Adds every `-----BEGIN CERTIFICATE-----` block in ' +
                         '`certificates` — the root, or the whole chain ' +
                         'above the leaf — and applies the new truststore to ' +
                         'every listener with `setSecureContext()`. The next ' +
                         'handshake is judged against it; connections ' +
                         'already open keep the truststore they were made ' +
                         'under.\n\n**ALL OR NOTHING ON A BLOCK OPENSSL ' +
                         'CANNOT READ.** If any block does not parse, none ' +
                         'is added: an anchor the listener cannot parse ' +
                         'makes the next truststore change throw on every ' +
                         'listener. A certificate already held is counted in ' +
                         '`duplicates` and not added twice. The truststore ' +
                         'holds at most 32 anchors; an add that fills it ' +
                         'keeps what went in and says why the rest did not ' +
                         'in `warning`.\n\n**PERSISTED** in ' +
                         '`ou=trustAnchors` wherever the directory is; ' +
                         '`persisted` in the reply says whether every anchor ' +
                         'this call added was written down.\n\n**THIS IS THE ' +
                         'GATED TWIN OF `POST /tls/trust`**, which needs no ' +
                         'credential and is refused in product mode. This ' +
                         'operation answers in both modes. The ' +
                         'reply names what was added and carries ' +
            ...
            responseDescription: 'How many anchors were added, how many were ' +
                                 'already held, the new total, and the ' +
                                 'subject and fingerprint of each one added.' },

          { action: 'remove', operationId: 'removeTrustAnchor',
            summary: 'Stop trusting one anchor, named by its fingerprint',
            description: 'Removes the ONE anchor whose SHA-256 fingerprint ' +
                         'is `fingerprint` — colon-separated or plain hex, ' +
                         'either case — and applies the smaller truststore ' +
                         'to every listener.\n\n**THERE IS NO BULK CLEAR ON ' +
                         'THIS API, AND THAT IS DELIBERATE.** A clear is the ' +
                         'one change whose reach is every client certificate ' +
                         'every other caller relies on; `POST ' +
                         '/tls/trust/clear` exists as a development test ' +
                         'control and is refused in product mode. Remove the ' +
                         'rows you mean.\n\n**A `file` ANCHOR MAY BE REMOVED ' +
                         'AND COMES BACK AT THE NEXT START**, when ' +
                         '`tls.trustAnchorsFile` is read again; the reply ' +
                         'says so. A fingerprint this truststore does not ' +
                         'hold is refused and removes nothing.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
                fingerprint: { type: 'string',
                               description: 'The anchor\'s SHA-256 ' +
                                            'fingerprint, as `GET ' +
                                            '/admin-api/tls/trust` lists ' +
      ...
      // opens nothing.
      //
      // **A KDC PER TRUST REALM SINCE 2026-09-15**, so a call under a realm
      // prefix reads and writes THAT realm's principals — the people in its
      // directory and the service principals in its registry — and each reply
      // says `trustRealm`. A realm whose `krb5.enabled` is off has no
      // principals and says so. Until that date there was one KDC for the
      // process and every prefix reached the default realm's.
      // -----------------------------------------------------------------------
      { method: 'GET', path: BASE + '/kerberos/principals', tag: 'Kerberos',
        ...
        description: 'Two lists, paged separately with `?peoplePage=` and ' +
                     ...
                     'their own password when it was set or verified ' +
                     '(PRODUCT MODE): the principal, the kvno, the enctypes, ' +
                     'when and on what event the keys were derived, whether ' +
                     'they are sealed, and `current` — whether they still ' +
                     'match the password the entry holds. A person whose ' +
                     'keys are not current is refused by the KDC until their ' +
                     ...
                     '2026-09-15, so a realm prefix answers that ' +
                     'realm\'s principals.',
        mirrors: 'GET /admin/kerberos/principals',
        // `per` and the two lists' own page parameters — NOT
        // pagingParameters(), whose `page` this resource never reads.
        parameters: this.pagingParameters().filter(function (one) {
          return one.name === 'per';
        }).concat(this.detailPagingParameters([
          { name: 'people',
        ...
        responseDescription: 'Both lists with their paging, whether this is ' +
                             'a product KDC, whether ' +
                             'krb5.personKeys is on, the ' +
                             'enctypes and starting kvno, and the ' +
                             'acceptor\'s SPN.',
        responseSchema: { type: 'object',
                          description:
                            'The stored Kerberos principals, paged.' },
        handler: function (req, res) {
          log.debug("Entering the management API Kerberos principals " +
                    "endpoint.");
          self.sendJson(res, 200, adminViews.kerberosPrincipalsJson(req));
      ...
      { method: 'POST', route: BASE + '/kerberos/principals/:action',
        ...
        actions: [
          { action: 'create-service',
            operationId: 'createKerberosServicePrincipal',
            ...
            description: 'Makes a RANDOM key for every enctype in ' +
                         ...
                         'an MIT keytab (format 0x502) in `keytab`, ' +
                         'base64.\n\n**THE KEYTAB IS IN THIS REPLY AND ' +
                         'NOWHERE ELSE**: it cannot be downloaded again. A ' +
                         'lost one is replaced by `rotate-service`.\n\nThe ' +
                         'KDC issues tickets for that SPN under the stored ' +
                         'key from the next request, in both modes, and this ' +
                         'service\'s own acceptor prefers it when the SPN is ' +
                         '`krb5.servicePrincipal`. Refused for an SPN that ' +
                         'already holds a stored key, for `krbtgt/*`, ' +
                         'for another realm, and for anything that ' +
          ...
          { action: 'rotate-service',
            operationId: 'rotateKerberosServicePrincipal',
            summary: 'Replace a service principal\'s key, and get the new ' +
                     'keytab once',
            description: 'New random keys at the stored kvno PLUS ONE, and ' +
                         'the new keytab in `keytab`, base64.\n\n**THE ' +
                         'VERSION IT REPLACES IS KEPT** — at most ' +
                         '`krb5.retainedKeyVersions` previous versions, each ' +
                         'for `krb5.retainedKeyTtlS` (by default the ticket ' +
                         'lifetime plus the clock skew) — and **the keytab ' +
                         'carries them too**, as MIT\'s `ktadd` leaves one: ' +
                         '`keytabKvnos` lists every version in it and ' +
                         '`retained` says until when each is accepted. A ' +
                         'ticket already issued under a kept version is ' +
                         'still accepted by this KDC and its acceptor; ' +
                         'nothing new is issued under it, and past its ' +
                         'window it is refused KRB_AP_ERR_BADKEYVER. ' +
                         '`drop-previous-service-keys` ends the window at ' +
                         'once. Refused for an SPN with no stored key.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
            ...
            responseDescription: 'The principal, the new kvno, the enctypes ' +
                                 'and the new keytab (base64).' },

          { action: 'delete-service',
            operationId: 'deleteKerberosServicePrincipal',
            summary: 'Delete a service principal\'s stored key',
            description: 'Removes the stored key and its info from the ' +
                         'application entry. The entry itself stays, as ' +
                         'every entry this registry records does. A ticket ' +
                         'for that SPN is then keyed as it was before a key ' +
                         'was stored: from `krb5.servicePassword` ' +
                         'if it is the acceptor\'s ' +
          ...
          { action: 'clear-person-keys', operationId: 'clearKerberosPersonKeys',
            ...
            description:
              'Removes the Kerberos keys derived from `username`\'s ' +
                         'password. Their next AS-REQ is refused with "sign ' +
                         'in ' +
                         'once"; their next verified sign-in derives new ' +
                         'keys ' +
                         'at ' +
                         'the next kvno. Somebody with no keys answers ' +
                         '`cleared: false` rather than a refusal, so a ' +
                         'script may clear on every run. There is no ' +
                         'operation that SETS a person\'s keys: they come ' +
                         'from the person\'s password and from nothing else.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
                username: { type: 'string',
                            description:
                              'The person, as they sign in — in the ' +
          ...
          { action: 'drop-previous-service-keys',
            ...
            description: 'Removes the PREVIOUS key versions a rotation kept ' +
                         'for `spn`, leaving the current key and its keytab ' +
                         'untouched. A ticket issued under a dropped version ' +
                         'is refused KRB_AP_ERR_BADKEYVER from the next ' +
                         'request, rather than when `krb5.retainedKeyTtlS` ' +
                         'would have ended its window — which is what an ' +
          ...
          { action: 'drop-previous-person-keys',
            ...
            summary: 'Stop accepting tickets under a person\'s previous key ' +
                     'versions, now',
            description: 'Removes the PREVIOUS key versions a password ' +
                         'change kept for `username`, leaving their current ' +
                         'keys — and so their sign-in — untouched. A ticket ' +
                         'sealed under a dropped version is refused ' +
                         'KRB_AP_ERR_BADKEYVER from the next request. A ' +
                         'previous version was never a way in: ' +
                         'pre-authentication uses the current keys only, so ' +
      ...
      { method: 'GET', path: BASE + '/audit', tag: 'Audit log',
        ...
        description: 'Every authentication, session, LDAP directory ' +
                     'operation, console interaction, management API call ' +
                     'and protocol endpoint call, newest first.\n\nThis is ' +
                     'HISTORY where the rest of this API is STATE. ' +
                     '/admin-api/metrics can say the directory holds eleven ' +
                     'entries; only this can say a twelfth was created at ' +
                     '14:02 and deleted at 14:03 by somebody bound as ' +
                     '`uid=carol`, over LDAPS.\n\n**NO CREDENTIAL IS EVER IN ' +
                     'A ROW.** Not a password, not a bearer token, not an ' +
                     'assertion, and no request or response body. A modify ' +
                     'names the attributes it changed and never their ' +
                     'values, because a modify is where a `userPassword` ' +
                     'gets set; a compare says whether it matched and not ' +
                     'what was tried; an authorization code in a query ' +
                     'string is replaced with `(redacted)`.\n\n**One act ' +
                     'usually produces several events.** A sign-in writes ' +
                     'three — the HTTP call, the credential being accepted, ' +
        ...
        parameters: [
          ...
          { name: 'outcome', in: 'query', required: false,
            ...
            description: 'Three rather than two on purpose: a `refused` is ' +
                         ...
                         'them would bury the one row worth ' +
                         'paging somebody about under the ' +
                         'fifty that are a client getting its parameters ' +
                         'wrong.' },
          { name: 'actor', in: 'query', required: false,
            schema: { type: 'string' },
            description: 'Substring of either spelling of the actor, ' +
                         'case-insensitive — the normalised key (`alice`) or ' +
                         'the form it was presented in (a bind DN, ' +
                         '`alice@REALM`, an X.509 subject). A substring ' +
                         'because the collapse to one key can ' +
                         'only be done where an identity is ' +
                         'normalised, and a directory row\'s actor is a DN.' },
          { name: 'q', in: 'query', required: false, schema: { type: 'string' },
            description: 'Substring of the summary, the target or the ' +
        ...
        handler: function (req, res) {
          ...
          self.sendJson(res, 200, adminViews.auditView(req.query).json);
          log.debug("Leaving the management API audit endpoint.");
        } },

      // THE ERROR CODES (2026-09-12). A read with no write beside it, which is
      // rule 7 read exactly: the page it mirrors has no control, because the
      // table is source and a code's meaning cannot change at runtime without
      // making every alert rule written against it a statement about something
      // else.
      { method: 'GET', path: BASE + '/error-codes', tag: 'Audit log',
        ...
        description: 'The central table of error codes, ' +
                     '`STS-<SUBSYSTEM>-<NNNN>`, filtered and paged: each ' +
                     'code with the subsystem it belongs to, what failed, ' +
                     'and what the CLIENT is told in its protocol\'s own ' +
                     'vocabulary (`spec`).\n\n**A CODE IS NEVER SENT TO A ' +
                     'CLIENT.** It is recorded on the audit row (`errorCode` ' +
                     'on GET /admin-api/audit, filterable there with ' +
                     '`?code=`) and at the front of the service log line; ' +
                     'every protocol response is exactly what it ' +
                     'was.\n\n`seen` counts the rows the audit log holds IN ' +
                     'THIS REALM right now, so it falls as the log\'s cap ' +
                     'discards the oldest, and it is zero for a failure ' +
                     'recorded only as a log line — a startup refusal, or ' +
                     'anything the remote PEP container records. ' +
                     '`unregisteredSeen` lists codes found on held rows that ' +
                     'the table does not hold, each a failure site whose ' +
                     'code was never registered.\n\nThe ' +
                     'same table is published as ' +
                     'docs/error-codes.md, generated from ' +
        ...
        parameters: [
          ...
          { name: 'q', in: 'query', required: false, schema: { type: 'string' },
            description: 'Substring of the code, the summary or what the ' +
                         'client sees, case-insensitive — a code pasted out ' +
                         'of a log line, or a protocol error name such as ' +
                         '`invalid_grant`.' },
          { name: 'seen', in: 'query', required: false,
            schema: { type: 'string' },
            description:
              '`1` for only the codes on at least one held audit row.' }
        ].concat(this.pagingParameters()),
        ...
        responseSchema: { type: 'object',
          description: 'The error code table: `codes` (this page, each with ' +
                       ...
                       '`seen` and `lastSeenAt`), `subsystems` (each with ' +
                       '`id`, `prefix`, `label`, `where`, `what`, `codes` ' +
                       'and `seen`), `unregisteredSeen`, `registered`, ' +
                       '`retired`, `auditRowsHeld`, `auditRowsWithCode`, ' +
                       '`distinctCodesSeen`, `filter` and the paging ' +
                       'members.' },
        handler: function (req, res) {
          log.debug("Entering the management API error codes endpoint.");
          self.sendJson(res, 200, adminViews.errorCodesView(req.query).json);
          log.debug("Leaving the management API error codes endpoint.");
        } },

      // THE USED-ASSERTION HISTORY (2026-09-13). A read with no write beside
      // it, which is rule 7 read exactly: the page it mirrors has no control,
      // because forgetting a row would make a still-valid assertion usable
      // again. A PROMISE, because on a postgres store the history is the
      // database's.
      { method: 'GET', path: BASE + '/used-assertions', tag: 'Tokens',
        operationId: 'getUsedAssertions',
        summary: 'Every RFC 7523 and RFC 7522 assertion this realm has ' +
                 'accepted and that has not yet expired',
        description: 'The used-assertion history: every RFC 7523 JWT and RFC ' +
                     ...
                     'authorization grant (`assertion`) — and not yet ' +
                     'expired, newest first, filtered and paged.\n\n**AN ' +
                     'ASSERTION IS ACCEPTED ONCE, EVER.** One history for ' +
                     'both uses and both profiles, keyed by format, issuer ' +
                     'and `jti`/`ID`, so a JWT that authenticated a client ' +
                     'is refused as a grant and the reverse. `state` is ' +
                     '`reserved` while the token request it came with has ' +
                     'not finished, and `spent` once that response was a ' +
                     '2xx; a request that failed for another reason releases ' +
                     'the assertion and it is not listed.\n\n`persistent` ' +
                     'says whether the history survives a restart (it does ' +
                     'in the `ldif` and `postgres` stores, in both modes), ' +
                     'and `atomicAcrossProcesses` whether recording a use is ' +
                     'one claim every process agrees on at once (postgres). ' +
                     'A row is kept until `expiresAt` — the assertion\'s own ' +
        ...
        responseDescription:
          'This realm\'s unexpired used assertions, where the ' +
                             'history is held, and the paging that found them.',
        responseSchema: { type: 'object',
          description: 'The used-assertion history: `rows` (this page, each ' +
                       'with `format`, `formatLabel`, `use`, `useLabel`, ' +
                       '`issuer`, `identifier`, `clientId`, `subject`, ' +
                       '`state`, `usedAt`, `spentAt`, `expiresAt` and ' +
                       '`origin`, times in milliseconds), `store`, ' +
                       '`persistent`, `atomicAcrossProcesses`, `storeNote`, ' +
                       '`cap`, `live`, `matched`, `shown`, `filter`, ' +
                       '`formats`, `uses`, `states` and the paging members.' },
        handler: function (req, res) {
          log.debug("Entering the management API used assertions endpoint.");
          adminViews.usedAssertionsView(req.query).then(function (view) {
            self.sendJson(res, 200, view.json);
      ...
      // Delegation. THE SECOND READ-ONLY RESOURCE HERE, and for a related
      // reason to the audit log's: it mirrors a console page with no form on
      // it. Everything that page shows is an observation — an act happened or
      // it did not, and the policy half is the KERBEROS principal database's
      // configuration, which is not settable from anywhere in this service.
      // There is nothing to change, so there is nothing to document as
      // changeable, and rule 7 is satisfied by this GET alone.
      { method: 'GET', path: BASE + '/delegation', tag: 'Delegation',
        operationId: 'getDelegation',
        summary: 'Who acted on whose behalf, through what, to reach what',
        description: 'Every delegation this service has performed or ' +
                     'REFUSED, in one model across three protocol families, ' +
                     'plus the configured policy that decides the Kerberos ' +
                     'ones.\n\nEight mechanisms: Kerberos S4U2Self, ' +
                     'S4U2Proxy (classic and resource-based) and a forwarded ' +
                     'ticket-granting ticket; WS-Trust `OnBehalfOf` and ' +
                     '`ActAs`; RFC 8693 token exchange in both its shapes. ' +
                     'They are recorded against ONE model because the ' +
                     'question is protocol-independent: which hop invented ' +
                     'which identity.\n\n**The axis worth filtering on is ' +
                     '`mode`.** Under a `delegation` the credential CARRIES ' +
                     'the chain — an `act` claim, a composite `ActAs`, ' +
                     '`S4U_DELEGATION_INFO` in the PAC — so the far end can ' +
                     'see who is really asking. Under an `impersonation` ' +
                     'nothing does, which means this endpoint is the ONLY ' +
                     'place that fact is ever visible: no reading of the ' +
                     'token afterwards can recover it.\n\n**Refusals are ' +
                     'here and are most of the value.** A refused act ' +
                     'carries `reason` — the KDC\'s own words, the same ' +
                     'sentence the client was sent — naming the two accounts ' +
                     'and the two attributes and which was missing. A ' +
                     ...
                     '`applications`, every application an act named in ' +
                     'WHATEVER ROLE it played (the console draws one of them ' +
                     'in full at /admin/delegation/application) — and ' +
                     '`policy`, which is who may delegate to whom before ' +
                     'anybody has tried.\n\n**THIS IS THE DELEGATION ' +
                     'REGISTER AND NOT EVERYTHING A PERSON WAS ISSUED.** An ' +
                     'ordinary grant is not a delegation act and is not ' +
                     'here: for one identity END TO END — every credential ' +
                     'with the exact grant or flow that produced it, beside ' +
                     'the acts naming them — the console unions this ' +
      ...
      //
      // A RESOURCE OF ITS OWN RATHER THAN MORE ACTIONS ON `/delegation`, and
      // the reason is the same one the console gives for putting two headings
      // on one page: the acts and the permissions are two registers, and an API
      // that answered both under one path would make a caller tell them apart
      // by the shape of a row.
      //
      // THIS COMMENT SAID `GET /admin-api/delegation` CARRIES THE REGISTER IN
      // AN `allowed` MEMBER, AND IT NEVER HAS (corrected 2026-09-01). Only the
      // CONSOLE route adds that member; this endpoint answers
      // `delegationView(query).json`, which is the ACTS view, the one shared
      // with /admin/delegation/map. The behaviour is right — see that
      // function's header, where folding a second register into it is refused
      // because every caller of the acts view would then pay for a walk of
      // ou=applications it did not ask for — so the sentence went and the code
      // stayed. This resource is where the register is reachable under its own
      // name.
      //
      // THE FIVE ACTION NAMES STUTTER SLIGHTLY UNDER THIS PATH
      // (`/permissions/define-permission`) AND THAT IS DELIBERATE. They are the
      // names in the console's hidden `action` inputs, where the page they sit
      // on is `/admin/delegation` and `define` alone would say nothing about
      // what is being defined — and `remove` and `revoke` are two different
      // things here (one removes a permission somebody exposes, the other takes
      // a grant away from a client). One vocabulary for both doors is worth
      // more than a shorter URL, and rule 7's parity check reads the console's
      // own list.
      // -----------------------------------------------------------------------
      { method: 'GET', path: BASE + '/permissions', tag: 'Delegation',
        ...
        description: 'The CONFIGURED delegation register, in Microsoft Entra ' +
                     ...
                     '/admin-api/delegation`: that one is what HAPPENED — ' +
                     'acts, evidence, one row per exchange — and this one is ' +
                     'INTENT, typed in before anybody asked for ' +
                     'anything.\n\n**How it works.** A RESOURCE application ' +
                     'is given a base URI (`oauthPermissionBaseUri`; Entra ' +
                     'calls it the Application ID URI and spells it ' +
                     '`api://<guid>`, and anything absolute works here) and ' +
                     'permissions on it (`oauthPermission`). A permission is ' +
                     'identified by the two joined — `https://example.com/` ' +
                     '+ `write` = `https://example.com/write` — and a CLIENT ' +
                     'application is granted some of them ' +
                     '(`oauthDelegatedPermission`). All three are ordinary ' +
                     'attributes on ordinary entries in `ou=applications`, ' +
                     'so an `ldapmodify` is a configuration change here ' +
                     'exactly as it is for a redirect URI.\n\n**What the ' +
                     'token then says.** A client asks for a permission as ' +
                     'an ordinary OAuth `scope`, and the access token comes ' +
                     'back AUDIENCED to the base URI with the permission ' +
                     'NAME on its scope claim: `scope=openid ' +
                     ...
                     'create one. `grants[].asked` is whether the client has ' +
                     'ever requested that scope, read off its own ' +
                     '`oauthScope`: evidence rather than proof, and the one ' +
                     'thing here that comes from what happened.\n\nThe ' +
                     '`graph` member is the same picture ' +
      ...
      //
      // A MEMBER ON `GET /permissions` WOULD HAVE BEEN THE SHORTER CHANGE AND
      // IT IS THE WRONG ONE, for the reason that reply's own `graph` member is
      // the boundary rather than an example. `graph` is one document about the
      // whole register and its size is the register's; a list of groups that
      // carried its grants would repeat the register once per group, and one
      // that did not would leave a caller no way to ask for a single group's
      // rows at all. It also needs PAGING — the console's own list of groups is
      // paged, and a member of somebody else's reply has nowhere to put a page
      // number.
      //
      // ONE OPERATION ANSWERS BOTH SHAPES, `?application=` deciding which,
      // because they are the same question at two scales and the console draws
      // them with one function: without it, every group with its counts; with
      // it, the ONE group that application is in, with its grants, its
      // permissions and the graph /admin/delegation/cluster draws. Two
      // operations would have been two places to disagree about what a group
      // is.
      //
      ...
      { method: 'GET', path: BASE + '/permissions/groups', tag: 'Delegation',
        ...
        summary: 'Which applications are joined to each other by delegated ' +
                 'permissions',
        description: 'The configured register PARTITIONED: a group is a set ' +
                     'of applications that can be reached from one another ' +
                     'by following grants, **ignoring which way each grant ' +
                     'points**.\n\nDirection is dropped for MEMBERSHIP and ' +
                     'kept on every line of the picture. Following the ' +
                     'arrows would answer *what can this client eventually ' +
                     'reach*, which is a question about a chain, and a ' +
                     'permission register has no chains in it: holding a ' +
                     'permission on an API grants nobody that API\'s own ' +
                     'permissions. Following a grant either way is the only ' +
                     'reading under which an API and the three front ends ' +
                     'holding permissions on it come out as ONE group rather ' +
                     'than as four.\n\n**The membership universe is what the ' +
                     'CONFIGURED register touches**: every application ' +
                     'carrying a base URI or a permission of its own, and ' +
                     'every application holding a grant. An entry in ' +
                     '`ou=applications` that is neither is in no group, ' +
                     'because this register has nothing to say about ' +
                     'it.\n\nThree states join no two applications, and each ' +
                     'produces a group of ONE rather than being left out: a ' +
                     'DANGLING grant (no application defines the permission, ' +
                     'so there is no far end), a grant an application made ' +
                     'to ITSELF (one application however it is drawn), and a ' +
                     'resource nobody holds anything on — which is the most ' +
                     'interesting group of one there is, since somebody ' +
                     'described an API and nothing may reach it.\n\nA group ' +
                     'is NAMED after the identifier of its members that ' +
                     'sorts first. That is a property of the SET, so adding ' +
                     'a grant inside a group does not rename it.\n\nWith no ' +
                     '`application`, every group with its counts and no ' +
                     'rows, biggest first and paged on `groupsPage` — the ' +
                     'shape /admin/delegation/allowed lists. With ' +
                     '`application`, the ONE group that application is in, ' +
                     'with its permissions paged on `groupPermissionsPage`, ' +
                     'its grants paged on `groupGrantsPage`, and the `graph` ' +
                     '/admin/delegation/cluster draws, in the shape `GET ' +
                     '/admin-api/delegation`\'s `graph` uses. An application ' +
                     'the configured register has never heard of answers 200 ' +
                     'with `group: null`, not 404: having no permissions ' +
                     'configured is the ordinary state of most entries in ' +
                     'this registry, and it is a fact rather than an error. ' +
                     '**The identifier is matched EXACTLY** ' +
                     '— nothing in this service case-folds ' +
                     'one, so `WebApp1` and `webapp1` are two applications.',
        mirrors: 'GET /admin/delegation',
        parameters: [
          { name: 'application', in: 'query',
            description: 'One application\'s identifier. Answers the group ' +
                         'it is in, with its rows and its graph, instead of ' +
                         'the list of every group. Matched exactly.',
            schema: { type: 'string' } },
          { name: 'groupsPage', in: 'query',
            description: 'Which page of the group LIST. The same name the ' +
                         ...
                         'several lists a delegation page can hold and a ' +
                         'bare `page` would move them together. Not read ' +
                         'when `application` is given.',
            schema: { type: 'integer', minimum: 1 } },
          { name: 'groupGrantsPage', in: 'query',
            description: 'Which page of the GRANTS inside one group. Read ' +
                         'only when `application` is given.',
            schema: { type: 'integer', minimum: 1 } },
          { name: 'groupPermissionsPage', in: 'query',
            description: 'Which page of the PERMISSIONS the applications in ' +
                         'one group expose, granted or not. Read only when ' +
                         '`application` is given.',
            schema: { type: 'integer', minimum: 1 } },
          { name: 'per', in: 'query',
            description: 'Rows per page, shared by both lists above — one ' +
                         '`per` per page is this console\'s rule and this ' +
                         'operation follows it.',
            schema: { type: 'integer', minimum: 1 } }
        ],
        responseDescription: 'Every group with its members and counts, or ' +
                             'the one group an application is in with its ' +
                             'grants, its permissions and its graph.',
        responseSchema: { type: 'object',
                          description: 'The configured register partitioned ' +
                                       'into groups of ' +
                                       'applications that can reach ' +
                                       'one another, direction ignored.' },
        handler: function (req, res) {
          log.debug("Entering the management API permission groups endpoint.");
          self.sendJson(res, 200, adminViews.permissionGroupsView(req.query));
      ...
      { method: 'POST', route: BASE + '/permissions/:action', tag: 'Delegation',
        ...
        handler: function (req, res) {
          ...
          const body = parseBody(req);
          const result = adminActions.permissionsAction(self.withAction(req,
              body));
          if (!result.ok) {
        ...
        actions: [
          { action: 'set-permission-base', operationId: 'setPermissionBase',
            summary:
              'Give an application the base URI its permissions hang off',
            description: 'The first step of exposing an API, and the one ' +
                         'that makes every permission on the entry NAMEABLE: ' +
                         'a permission is identified by this value followed ' +
                         'by its name, so an application with permissions ' +
                         'and no base has permissions no client can ever ask ' +
                         'for.\n\nIt must be ABSOLUTE, because it becomes ' +
                         'the `aud` of an access token and an audience that ' +
                         'is not absolute is one nothing can compare ' +
                         'against. A trailing separator is ADDED where there ' +
                         'is none — `https://example.com` becomes ' +
                         '`https://example.com/` — because the identifier is ' +
                         'a plain concatenation and the two would otherwise ' +
                         'join into one word. An `ldapmodify` is not ' +
                         'normalised and means exactly what it ' +
                         'says.\n\nSending an empty value CLEARS it. The ' +
                         'permissions stay on the entry with no identifier, ' +
                         'which `GET /admin-api/permissions` reports, and ' +
                         'grants already made become `dangling` on the ' +
                         'clients holding them. Neither is ' +
                         'tidied up: that would be this operation ' +
            ...
            requestBody: {
              ...
              properties: {
                resource: { type: 'string',
                            description: 'The application that EXPOSES the ' +
                                         'API, by its identifier exactly as ' +
            ...
            responseDescription: 'The application as it now stands, and what ' +
                                 'a permission on it is now called.' },
          ...
          { action: 'define-permission', operationId: 'definePermission',
            ...
            description: 'Entra ID\'s `oauth2PermissionScopes`, one at a ' +
                         ...
                         'token\'s `scope` claim, so it must be a legal ' +
                         'OAuth scope token: any printable ASCII except ' +
                         'space, double quote and backslash (RFC 6749 ' +
                         'section 3.3), and not `|`, which separates the ' +
                         'name from the description in the attribute. The ' +
                         'DESCRIPTION is optional and is stored after the ' +
                         'first `|` in the same value.\n\n**Defining a ' +
                         'permission grants it to nobody.** That is the ' +
                         'ordering this feature is built on and the reason ' +
                         'this operation and `grant-permission` are two: a ' +
                         'permission must exist before anything can be ' +
                         ...
                         'leave the second unreachable. Remove it and define ' +
                         'it again to change the wording.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
                ...
                name: { type: 'string',
                        description: 'The permission name — the word a ' +
                                     'client will send inside a `scope`.' },
                description: { type: 'string',
                               description: 'Optional prose, shown wherever ' +
                                            'the permission is.' }
              },
              required: ['resource', 'name'],
              examples: [{ resource: 'api1', name: 'write',
                           description:
                             'Change widgets on somebody\'s behalf' }],
              additionalProperties: false
            },
          ...
          { action: 'remove-permission', operationId: 'removePermission',
            ...
            description: 'Named by its NAME rather than by the raw attribute ' +
                         'value, because that value is `name|description` ' +
                         'and a caller holding a stale description would ' +
                         'fail to remove anything.\n\n**Grants naming it are ' +
                         'NOT revoked.** They stay on the clients\' entries ' +
                         'and become `dangling`, which `GET ' +
                         '/admin-api/permissions` reports and the reply here ' +
                         'counts. Revoking them would be this operation ' +
                         'writing to entries the caller did not name; ' +
                         'define the permission again and every one of ' +
          ...
          { action: 'grant-permission', operationId: 'grantPermission',
            ...
            description: '**THE DELEGATION RELATIONSHIP ITSELF** — Entra ' +
                         'ID\'s `requiredResourceAccess`, one permission at ' +
                         'a time.\n\nIt lands on the CLIENT\'s entry, as a ' +
                         'value of `oauthDelegatedPermission`, because the ' +
                         'client is the party that will name the permission ' +
                         'in a `scope` — so the entry that answers *may this ' +
                         'request be honoured* is the entry the request ' +
                         'identifies. One client granted three permissions ' +
                         'is three calls and three values; three clients ' +
                         'granted one permission is one value on each of ' +
                         'three entries. That is how one-to-many and ' +
                         'many-to-one both work with no store of their ' +
                         'own.\n\n**The permission must already be ' +
                         'DEFINED**, matched EXACTLY rather than as a prefix ' +
                         'of a registered base — so a client cannot address ' +
                         'a token to somebody\'s API by inventing a word ' +
                         ...
                         'scope, and what the grant changes ' +
            ...
            requestBody: {
              ...
              properties: {
                client: { type: 'string',
                          description: 'The application that WILL ASK — the ' +
                                       'one whose `client_id` ' +
                                       'appears on the token ' +
                                       'request. Not the one exposing the ' +
                                       'API.' },
            ...
            responseDescription: 'The grant, and what an access token asking ' +
                                 'for it will carry.' },
          ...
          { action: 'revoke-permission', operationId: 'revokePermission',
            ...
            description: 'The opposite of `grant-permission`, and with the ' +
                         ...
                         'simply reported as UNGRANTED — which is the state ' +
                         '`oauth2.delegatedPermissionsEnforced` turns into a ' +
                         'refusal.\n\nIt is also how a DANGLING grant is ' +
                         'cleared: send the identifier exactly as it appears ' +
                         'on the entry, and it goes whether or not anything ' +
                         'defines it.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
      ...
      // -----------------------------------------------------------------------
      // CONSENT — the THIRD register in this family and the first whose rows
      // have a PERSON in them.
      //
      // A resource of its own rather than more actions on `/permissions`, for
      // the reason that one is a resource of its own rather than more actions
      // on `/delegation`: a caller that had to tell an act from an intent from
      // a consent by the shape of a row would be told nothing by any of them.
      // The console draws it on a page of its own for the same reason and rule
      // 7's parity check is what keeps the two in step.
      //
      // THE FOUR ACTION NAMES SAY WHICH HALF THEY TOUCH, and the stutter under
      // this path (`/consent/grant-global-consent`) is deliberate exactly as
      // `/permissions/define-permission`'s is. They are the names in the
      // console's hidden `action` inputs, where the page is `/admin/consent`
      // and `grant` alone would not say whether it meant the override or
      // somebody's answer — and those two are the pair a caller most needs kept
      // apart, because removing the wrong one asks the wrong people again.
      // -----------------------------------------------------------------------
      // -----------------------------------------------------------------------
      // ROLES. Three operations against `adminViews.rolesView()`,
      // `adminViews.rolesPreview()` and `adminActions.rolesAction()` — the same
      // three functions the console calls, so rule 7's parity is a property of
      // the wiring rather than of two lists agreeing.
      //
      // **THE PREVIEW GETS AN OPERATION OF ITS OWN AND NOT A QUERY ON THE
      // FIRST**, and that is the one decision here worth arguing. On the
      // console it IS a query parameter, because a `<form method="get">` is
      // what a page with no script has; over HTTP the two are different
      // questions — one reads a register and the other asks the PDP — and a
      // caller that got a decision by adding two parameters to a listing would
      ...
      { method: 'GET', path: BASE + '/roles', tag: 'Roles',
        ...
        description: 'The role register, both relations.\n\n**A role is a ' +
                     'name somebody may hold, and holding one is what an ' +
                     'ISSUANCE is decided on.** Three kinds of thing can be ' +
                     'mapped into one — a person, a GROUP (so every member ' +
                     'of it holds the role, resolved at decision time) and ' +
                     'an APPLICATION, which is what a `client_credentials` ' +
                     'grant is decided on where there is no person at ' +
                     'all.\n\n**`roles` is MEMBERSHIP and `requiring` is ' +
                     'REQUIREMENT, and they are opposite.** Membership is ' +
                     'stored on the ROLE entry under `ou=roles` and is ' +
                     'written through this resource. A requirement is ' +
                     '`appRequiredRole` on an APPLICATION\'s own entry and ' +
                     'is written through `POST /admin-api/applications/add` ' +
                     '— one store, one door that writes it. It is READ here ' +
                     'because this is the only surface that can resolve it: ' +
                     '`requiring[].unknown` names a role an application ' +
                     'demands that NOTHING defines, which refuses everybody, ' +
                     'silently and correctly, and looks exactly like the ' +
                     'application being broken.\n\n**Only NARROWED ' +
                     'applications are in `requiring`.** An application that ' +
                     'names no required role requires `EVERYBODY`, everybody ' +
                     'holds `EVERYBODY`, and nothing is refused — which is ' +
                     'how this service behaved before roles existed and is ' +
                     'what makes the feature off by default without being ' +
                     'absent.\n\n**The six `builtIn` roles are COMPUTED and ' +
                     'in no container.** They cannot be created, edited or ' +
                     'deleted, they have no members, and every one of them ' +
                     'is answered from the CONTEXT of the decision being ' +
                     'made. They are never in the roles claim either: ' +
                     '`EVERYBODY` and `ALL_AUTHENTICATED_USERS` are true of ' +
                     'almost every token this service issues, so carrying ' +
                     'them would tell a relying party nothing it did not ' +
                     'know from holding the token.\n\n`gated: false` means ' +
                     'the XACML family is not loaded in this process, so ' +
                     '`common/issuance_gate.js` has no decider and every ' +
                     'issuance is allowed whatever this register says. ' +
                     '`enforced: false` means `roles.enforceIssuance` is ' +
                     'off, which is the same outcome ' +
                     'by a different route and ' +
                     'is the way back if a policy edit locks something out.',
        mirrors: 'GET /admin/roles',
        responseDescription: 'The six built-in roles, every configured role ' +
                             'with its three membership lists, every ' +
                             'application that has been narrowed, and ' +
                             'whether the decision is being asked for at all.',
        responseSchema: { type: 'object',
                          description: 'The role register, both relations.' },
        handler: function (req, res) {
      ...
      { method: 'GET', path: BASE + '/roles/preview', tag: 'Roles',
        ...
        description: 'Asks the embedded PEP whether this service would issue ' +
                     ...
                     'anything.\n\n**It is the SAME call the nine issuance ' +
                     'sites make** — `common/issuance_gate.check()`, through ' +
                     '`xacml/xacml_role_pep.ts`, against the policy ' +
                     '`xacml.issuancePolicy` names — so a preview that ' +
                     'agreed with the enforcement only by coincidence is ' +
                     'impossible. That is the only reason it is worth ' +
                     'having.\n\nIt is not `POST /xacml/pdp`, which asks the ' +
                     'same engine a different question: an arbitrary request ' +
                     'against the repository ROOT, which is the policy about ' +
                     'somebody else\'s boundary. Two questions, two ' +
                     'documents.\n\n`available: false` means the XACML ' +
                     'family is not loaded in this process: nothing is gated ' +
                     'and every issuance is allowed.\n\n**`application` and ' +
                     '`subject` are both needed and neither is declared ' +
                     'required**, because this is a READ and a read with no ' +
                     'question in it has nothing to refuse — it answers 200 ' +
                     'with `answered: false` and says what was missing, ' +
                     'exactly as a GET of /admin/roles with no parameters ' +
                     'draws the form and no answer. **Read `answered` ' +
                     'first.** There is deliberately no `decision` member on ' +
                     'that reply: the gate ALLOWS a call that names no ' +
                     'application, so an operation that fell through to it ' +
                     'would hand back a Permit meaning "you did not ask".',
        mirrors: 'GET /admin/roles',
        parameters: [
          { name: 'application', in: 'query',
            schema: { type: 'string' },
            description: 'What something would be issued FOR — a ' +
                         '`client_id`, a wtrealm, a SAML entityID, an SPN. ' +
                         'An application no entry names requires EVERYBODY, ' +
                         'because this service registers one on first sight ' +
                         'and refusing the first request from a new client ' +
                         'is precisely the permissiveness it is for.' },
          { name: 'subject', in: 'query',
            schema: { type: 'string' },
            description: 'Who it would be issued to — a username, or a ' +
                         'client_id where `subjectKind` is `application`.' },
          ...
          { name: 'kind', in: 'query',
            ...
            description: 'Which issuance. It becomes the XACML `action-id`, ' +
                         ...
                         'a refresh token. `GET ' +
                         '/admin-api/roles` lists the nine in ' +
                         '`issuanceKinds`; the default is ' +
                         '`issue-access-token`.' }
        ],
        ...
        responseSchema: { type: 'object',
                          description: 'One issuance decision, made and ' +
                                       'thrown away.' },
        handler: function (req, res) {
          log.debug("Entering the management API role preview endpoint.");
          const answer = adminViews.rolesPreview(req.query);
          if (!answer) {
            // NOTHING ASKED IS `answered: false` AND NOT A 400, and that is
            // this operation MIRRORING ITS PAGE rather than being lenient. A
            // GET of /admin/roles with no parameters draws the form and no
            // answer, so this answers the same thing: it is a READ, and a read
            // of this resource with no question in it has nothing to refuse.
            //
            ...
            // is no `decision` member on this reply at all, and why `answered`
            ...
            self.sendJson(res, 200, {
              ...
              why: '`application` and `subject` are both needed. A decision ' +
                   'needs something being issued FOR and somebody it is ' +
                   'being issued TO, and an issuance named with neither is ' +
                   'allowed by definition rather than by policy — so nothing ' +
                   'was asked and there is no decision here to read.' });
            log.debug("Leaving the management API role preview endpoint. " +
                      "Nothing asked.");
            return;
          }
          self.sendJson(res, 200, Object.assign({ answered: true }, answer));
          log.debug("Leaving the management API role preview endpoint.");
        } },
      ...
      { method: 'POST', route: BASE + '/roles/:action', tag: 'Roles',
        ...
        handler: function (req, res) {
          ...
          const body = parseBody(req);
          const result = adminActions.rolesAction(self.withAction(req, body),
                                                  { via: 'api' });
          if (!result.ok) {
        ...
        actions: [
          { action: 'create-role', operationId: 'createRole',
            ...
            description: 'Writes one entry under `ou=roles` with no members. ' +
                         'Adding members is `add-member`; the two are ' +
                         'separate because a role is worth creating before ' +
                         'anybody holds it — an application can be narrowed ' +
                         'to it first, and the register will then say ' +
                         'so.\n\nThe name becomes an LDAP RDN and a value in ' +
                         'a token claim, so it is up to 64 characters of ' +
                         'letters, digits, and `. _ : @ -` or a space. **It ' +
                         'may not be one of the six built-in roles**: ' +
                         'those are computed and answered first, so a ' +
          ...
          { action: 'delete-role', operationId: 'deleteRole',
            ...
            description: 'Deletes the entry. **Applications that still ' +
                         'REQUIRE it are named in the reply and the delete ' +
                         'still happens**, which is deliberate: refusing ' +
                         'would mean a role could not be removed until every ' +
                         'application naming it had been edited, and those ' +
                         'entries are usually the thing somebody is in the ' +
                         'middle of changing. Each of them now requires a ' +
                         'role NOBODY holds and is therefore issued nothing ' +
                         'at all — so the consequence is said at the moment ' +
                         'it is created rather than discovered later as a ' +
                         'service that stopped working.\n\nNothing already ' +
                         'ISSUED is touched. A token minted while ' +
                         'somebody held the role is still valid and ' +
            ...
            responseDescription: 'What was removed, and which applications ' +
          ...
          { action: 'describe-role', operationId: 'describeRole',
            ...
            requestBody: {
              ...
              examples: [{ role: 'staff',
                           description:
                             'Anybody with a desk in the building' }],
              additionalProperties: false
            },
          ...
          { action: 'add-member', operationId: 'addRoleMember',
            ...
            description: 'Adds one value to the role entry. **Which of the ' +
                         'three lists it goes in is `kind`, and the three ' +
                         'are looked up in three different places**, so ' +
                         'naming the wrong one succeeds and writes something ' +
                         'that will never match:\n\n* `user` — a username. ' +
                         'The person need not exist: this service creates a ' +
                         'directory entry for any name on first sight, so a ' +
                         'role can be granted before its holder has ever ' +
                         'signed in.\n* `group` — a group in `ou=groups`. ' +
                         'Every member holds the role, **resolved at ' +
                         'DECISION TIME** rather than expanded on write, so ' +
                         'an `ldapmodify` adding somebody to the group ' +
                         'changes the very next token.\n* `application` — an ' +
                         'application that holds the role AS ITSELF, which ' +
                         'is what a `client_credentials` grant is decided ' +
                         'on.\n\nIt is NOT the same relation as ' +
                         '`appRequiredRole` on an application entry, which ' +
                         'is what that application DEMANDS of others. An ' +
                         'application appears in both and means opposite ' +
                         'things in each.\n\nA BUILT-IN role is refused ' +
                         'by name: those six are computed and have no ' +
                         'membership to edit.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
                ...
                kind: { type: 'string', enum: ['user', 'group', 'application'],
                        description: 'Which of the three lists the member ' +
                                     'goes in.' },
                member: { type: 'string',
                          description: 'The person, group or application. ' +
                                       'Named `member` rather than `name` on ' +
                                       'purpose: `role` is the role\'s own ' +
                                       'name, and a body that ' +
                                       'confused the two would succeed ' +
          ...
          { action: 'remove-member', operationId: 'removeRoleMember',
            ...
            description: 'Removes one value from the role entry. Matched ' +
                         'case-insensitively, for the reason the register ' +
                         'gives: a username here arrives from a login form, ' +
                         'a SAML subject, a Kerberos principal and a ' +
                         '`client_id`, and this service has always treated ' +
                         'those as one identity however they were ' +
                         'typed.\n\n**Nothing already ISSUED is touched**, ' +
                         'exactly as revoking a delegated permission does ' +
                         'not re-judge a grant already made. The next ' +
                         'issuance is decided without the role; a token ' +
            ...
            requestBody: {
              ...
              properties: {
                role: { type: 'string',
                        description: 'The role being taken away.' },
                kind: { type: 'string', enum: ['user', 'group', 'application'],
      ...
      { method: 'GET', path: BASE + '/consent', tag: 'Delegation',
        ...
        description: 'Both halves of the consent register.\n\n**How it ' +
                     'works.** With `oauth2.consentRequired` on — it is ON ' +
                     'by default, and it is the one policy in this service ' +
                     'that is — the authorization endpoint draws ' +
                     '`/oauth2/consent` the first time a given username ' +
                     'signs in to a given `client_id` for a given scope, and ' +
                     'issues nothing until they answer. Allow writes one ' +
                     '`oauthConsent` value per scope onto that person\'s own ' +
                     'entry under `ou=users`; Deny returns `access_denied` ' +
                     'to the client and records nothing.\n\n**`globals` is ' +
                     'CONFIGURATION and `users` is a RECORD**, and the ' +
                     'difference decides what removing a row does. A global ' +
                     'consent is `oauthGlobalConsent` on an APPLICATION\'s ' +
                     'entry, one value per scope: everybody who signs in to ' +
                     'that application skips the prompt for it and nothing ' +
                     'is written about anybody — so revoking it asks ' +
                     'EVERYBODY again, including the people who would have ' +
                     'said yes. A recorded consent is one person\'s answer, ' +
                     'and revoking it asks that one person.\n\n**A delegated ' +
                     'permission is recorded by its WHOLE identifier** — ' +
                     '`https://example.com/write`, never the bare `write` — ' +
                     'because two resources may both expose a permission of ' +
                     'that name and a consent to one must not cover the ' +
                     'other. `globals[].resource` says which application ' +
                     ...
                     'means no directory is installed behind the register, ' +
      ...
      { method: 'POST', route: BASE + '/consent/:action', tag: 'Delegation',
        ...
        handler: function (req, res) {
          ...
          const body = parseBody(req);
          const result = adminActions.consentAction(self.withAction(req, body));
          if (!result.ok) {
        ...
        actions: [
          { action: 'grant-global-consent', operationId: 'grantGlobalConsent',
            summary:
              'Consent a scope for everybody who signs in to an application',
            description: 'Adds one value to `oauthGlobalConsent` on the ' +
                         'application\'s own entry. Nobody is asked about ' +
                         'that scope on that application again, and ' +
                         '**nothing is written about anybody** — this is an ' +
                         'OVERRIDE and not a record.\n\nThat is the whole ' +
                         'difference from a person pressing Allow, and it ' +
                         'decides what removing it does: ' +
                         '`revoke-global-consent` asks everybody again, ' +
                         'because there is no record of who would have ' +
                         'agreed. Somebody who consented the same scope ' +
                         'personally — before or after — still has that on ' +
                         'their entry and is still not asked.\n\n**It is ' +
                         'keyed on the pair, not on the scope.** Consenting ' +
                         '`read` here consents it for THIS application; an ' +
                         'application registered five minutes later that ' +
                         'spells the same word is still asked. There is no ' +
                         'service-wide list of scopes nobody is ever asked ' +
                         'about, deliberately: it would mean an application ' +
                         'nobody has reviewed inheriting a decision made ' +
                         'about a different one.\n\nThe scope must be a ' +
                         'legal RFC 6749 section 3.3 scope token, because a ' +
                         'value with a space in it is two scopes and could ' +
                         'never match one. It need NOT name a permission any ' +
                         'application defines — most scopes are not ' +
                         'permissions, and refusing an unrecognised one ' +
                         'would make it impossible to consent `openid`.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
                client: { type: 'string',
                          description: 'The application people sign in to, ' +
                                       'by its identifier exactly as ' +
                                       '`ou=applications` holds it. This is ' +
                                       'the CLIENT — the party that will ' +
                                       'name the scope in a request — and ' +
                                       'not the resource that exposes it.' },
                scope: { type: 'string',
                         description: 'The scope, exactly as a client puts ' +
                                      'it in a `scope` ' +
                                      'parameter. A delegated ' +
            ...
            responseDescription: 'The application as it now stands, and what ' +
                                 'skipping the prompt for that scope now ' +
                                 'means.' },
          ...
          { action: 'revoke-global-consent', operationId: 'revokeGlobalConsent',
            ...
            description: 'Removes one value from `oauthGlobalConsent`. **The ' +
                         ...
                         'covering, because an override records nothing ' +
          ...
          { action: 'revoke-consent', operationId: 'revokeConsent',
            ...
            description: 'Removes the `oauthConsent` value naming this ' +
                         'person, this application and this scope. They are ' +
                         'asked again the next time that application ' +
                         'requests that scope; nobody else is ' +
                         'affected.\n\n**All three are required and that is ' +
                         'not pedantry.** One person may consent the same ' +
                         'scope to several applications, and revoking the ' +
                         'wrong pair is invisible until somebody is asked ' +
                         'again — which is a week later and looks like a bug ' +
                         'in the screen.\n\nA scope covered by GLOBAL ' +
                         'consent is not on anybody\'s entry, so there is ' +
              ...
              properties: {
                username: { type: 'string',
                            description: 'The person, exactly as ' +
                                         '/admin/users names them. It is ' +
                                         'normalised the same way every ' +
                                         'identity here is, so `alice` and ' +
                                         'her urn:uuid: subject are one ' +
                                         'person.' },
                client: { type: 'string',
                          description:
                            'The application they consented it to.' },
                scope: { type: 'string',
                         description: 'The scope they consented.' }
              },
          ...
          { action: 'forget-user-consent', operationId: 'forgetUserConsent',
            ...
            description: 'Every `oauthConsent` value on one person\'s entry, ' +
                         'in one call, so that every application asks them ' +
                         'again. A separate operation rather than a loop ' +
                         'over `revoke-consent` because being asked again is ' +
                         'the one thing somebody wants after testing this ' +
                         'screen, and doing it a row at a time for a person ' +
                         'with thirty consents is a chore rather than a ' +
                         'control.\n\nIt reaches nothing under GLOBAL ' +
            ...
            requestBody: {
              ...
              properties: {
                username: { type: 'string',
                            description:
                              'The person whose answers are forgotten.' }
              },
              required: ['username'],
              examples: [{ username: 'alice' }],
              additionalProperties: false
      ...
      // -----------------------------------------------------------------------
      // SPIFFE. Three resources, mirroring the three console pages one for one,
      // and each POST calls the SAME action function the console's form posts
      // to — with `action` taken from the URL instead of from a hidden input.
      // Rule 7.
      //
      // There is a fourth SPIFFE surface that is deliberately NOT here: the
      // SPIRE Server API itself. It is gRPC, it already does all of this, and
      // wrapping it in JSON would be a second implementation of forty-two
      // methods that could then disagree with the first. What this API covers
      // is the CONSOLE — the trust domain, its authorities, its federated
      // bundles, the registration entries and the agents — which is the parity
      // rule's actual subject.
      // -----------------------------------------------------------------------
      { method: 'GET', path: BASE + '/spiffe', tag: 'SPIFFE',
        ...
        description: 'What this service is as a SPIFFE issuing authority. ' +
                     'The X.509 and JWT authorities (the ACTIVE one first, ' +
                     'with the retired ones that are still published behind ' +
                     'it), the bundle path and its sequence, every federated ' +
                     ...
                     'key is in this reply. The authority ' +
                     'CERTIFICATE is published, as GET ' +
                     '/tls/server-certificate publishes that one.',
        mirrors: 'GET /admin/spiffe',
        responseDescription: 'The trust domain, its authorities, its ' +
                             'federated bundles and its listeners.',
        responseSchema: { type: 'object',
                          description: 'The SPIFFE trust domain as this ' +
                                       'service holds it.' },
        handler: function (req, res) {
          ...
          self.sendJson(res, 200, adminViews.spiffeJson(req));
      ...
      { method: 'POST', route: BASE + '/spiffe/:action', tag: 'SPIFFE',
        ...
        handler: function (req, res) {
          ...
          adminActions.spiffeAction(self.withAction(req, body))
            .then(function (result) {
            if (!result.ok) {
            ...
            self.sendJson(res, 500, { ok: false, errors: [err.message] });
        ...
        actions: [
          { action: 'rotate', operationId: 'rotateSpiffeAuthority',
            ...
            description: 'A new authority is PREPENDED — everything is ' +
                         'signed with it from that moment — and the old one ' +
                         'stays in the published bundle, so SVIDs already in ' +
                         'the field go on verifying. That is what a bundle ' +
                         'is FOR, and dropping the old one is the difference ' +
                         'between a rotation and an outage.\n\nThe bundle ' +
                         '`spiffe_sequence` changes, which is how a consumer ' +
                         'that polls the bundle endpoint knows to refetch. ' +
                         'At most four authorities are retained; past that ' +
                         'the oldest is dropped and anything it signed stops ' +
                         'verifying at that moment.\n\nThis is also the ONLY ' +
                         'way to add an authority to this trust domain. The ' +
                         'SPIRE Server API\'s AppendBundle and ' +
                         'PublishJWTAuthority are refused, because they ' +
            ...
            responseDescription:
              'What was rotated, and the new bundle sequence.' },

          { action: 'federation-set', operationId: 'setSpiffeFederatedBundle',
            summary: 'Add or replace a foreign trust domain\'s bundle',
            description: '**The bundle is PUSHED here and never PULLED by ' +
                         'this service.** The SPIFFE federation ' +
                         'specification puts a bundle endpoint URL in the ' +
                         'relationship and a real implementation polls it; ' +
                         'this one records the URL and refuses to follow it ' +
                         '— the SPIRE Server API\'s RefreshBundle says so in ' +
                         'terms — because fetching a URL somebody ' +
                         'registered, in order to obtain a key that will ' +
                         'then verify credentials, is a server-side request ' +
                         'forgery with a citation attached. The same refusal ' +
                         'this service gives WS-Federation\'s `wreqptr` and ' +
                         'a client\'s `jwks_uri`.\n\n**A trust domain this ' +
                         'service itself serves is REFUSED** — any realm\'s, ' +
                         'whether or not SPIFFE is on in it (2026-09-12). ' +
                         'The bundles are held per realm, and a federated ' +
                         'entry naming a served domain would let an ' +
                         'authority somebody registered in one realm ' +
                         'authenticate workloads as another realm\'s.\n\nThe ' +
                         'document is CHECKED, which is unusual for this ' +
                         'service: every JWK needs a `use` of `x509-svid`, ' +
                         '`jwt-svid` or `wit-svid`, because a consumer MUST ' +
                         'IGNORE one without it — so a bundle of keys ' +
                         'missing that member is stored happily and then ' +
                         'verifies nothing, with no error anywhere pointing ' +
                         'back here.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
                trustDomain: { type: 'string',
                               description: 'The trust domain NAME — ' +
                                            ...
                                            'service\'s own is refused: a ' +
                                            'trust domain does not federate ' +
                                            'with itself, and ' +
                                            'accepting it would give ' +
                                            'it two bundles that could ' +
                                            'disagree.' },
                document: { type: 'object',
                            description: 'The bundle, as a JWK Set with ' +
                                         '`spiffe_sequence` and ' +
                                         '`spiffe_refresh_hint`. A JSON ' +
                                         'string is accepted ' +
                                         'too, which is what the ' +
                                         'console\'s textarea sends.' },
                bundleEndpointUrl: { type: 'string',
                                     description: 'Recorded and never ' +
                                                  'fetched. It is reported ' +
                                                  'back so an operator ' +
                                                  'can see what the ' +
                                                  'relationship says.' },
                bundleEndpointProfile: { type: 'string',
                                         ...
                                         description:
                                           'Which profile the partner ' +
                                                      'expects. Recorded.' },
                endpointSpiffeId: { type: 'string',
                                    description: 'For `https_spiffe`, the ' +
                                                 'SPIFFE ID the ' +
                                                 'partner\'s endpoint ' +
                                                 'presents. Recorded.' }
              },
          ...
          { action: 'federation-remove',
            operationId: 'removeSpiffeFederatedBundle',
            summary: 'Forget a foreign trust domain\'s bundle',
            description: 'Any registration entry that federates with it ' +
                         'keeps the name and simply contributes no bundle to ' +
                         'its workloads, which is the same state as a ' +
                         'relationship configured before its bundle has ' +
                         'arrived. The entries are left alone deliberately; ' +
                         'the SPIRE Server API\'s BatchDeleteFederatedBundle ' +
                         'is where the three modes ' +
      ...
      { method: 'GET', path: BASE + '/spiffe/entries', tag: 'SPIFFE',
        ...
        description: 'A registration entry says which SPIFFE ID a workload ' +
                     'gets, under which parent, matching which selectors. It ' +
                     'is the most important object in a SPIFFE deployment: ' +
                     'the Workload API answers out of it.\n\n**The entries ' +
                     'ARE the registry.** They live under ' +
                     '`ou=entries,ou=spiffe` in the embedded LDAP directory ' +
                     'and nothing caches them, so an `ldapmodify` is visible ' +
                     'here on the next call and changes what the next SVID ' +
                     'looks like.\n\n**The selectors restrict nothing ' +
                     'here.** They are recorded, reported, and used by the ' +
                     'SPIRE Server API\'s GetAuthorizedEntries — and the ' +
                     'Workload API hands every caller every identity, ' +
                     'because nothing in this service attests a workload.',
        mirrors: 'GET /admin/spiffe/entries',
        parameters: [
          { name: 'entry', in: 'query', required: false,
            schema: { type: 'string' },
            description: 'One entry, by its id — the 32 hex characters this ' +
                         'registry minted, which is what the SPIRE Server ' +
                         'API calls `id`. The reply then carries that entry ' +
                         'with every attribute of its directory entry.' },
          { name: 'q', in: 'query', required: false, schema: { type: 'string' },
            description: 'Substring of the SPIFFE ID, the parent, the entry ' +
          ...
          { name: 'origin', in: 'query', required: false,
            ...
            description: 'How the entry got here. `auto` is one this service ' +
                         ...
                         'telling those from entries somebody meant is the ' +
                         'whole reason this field exists.' }
        ].concat(this.pagingParameters()),
        responseDescription: 'The matching registration entries with the ' +
                             'paging that found them, or one entry with its ' +
                             'directory entry when `entry` was given.',
        responseSchema: { type: 'object',
                          description:
      ...
      { method: 'POST', route: BASE + '/spiffe/entries/:action', tag: 'SPIFFE',
        ...
        handler: function (req, res) {
          log.debug("Entering the management API SPIFFE entries action " +
                    "endpoint.");
          const body = parseBody(req);
          const result = adminActions.spiffeEntriesAction(self.withAction(req,
              body));
          if (!result.ok) {
            errorCodes.mark(res, errorCodes.codeOf(result) || 'STS-API-0063');
          }
          self.sendJson(res, result.ok ? 200 : 400, result);
          log.debug("Leaving the management API SPIFFE entries action " +
                    "endpoint.");
        },
        actions: [
          { action: 'create', operationId: 'createSpiffeEntry',
            summary: 'Register an identity',
            description: 'Three refusals and no others: a SPIFFE ID that is ' +
                         'not one, a SPIFFE ID in ANOTHER trust domain (this ' +
                         'service is the issuing authority for exactly one, ' +
                         'and cannot sign for somebody else\'s — that is ' +
                         'what federation is for), and a SPIFFE ID under the ' +
                         'reserved `/spire` path, which belongs to this ' +
                         'server and the agents it attests.\n\nA DUPLICATE ' +
                         'SPIFFE ID IS ALLOWED. Two entries granting one ' +
                         'identity under different parents is a real ' +
                         'configuration and SPIRE permits it.\n\nThis is the ' +
                         'same function `BatchCreateEntry` on the SPIRE ' +
                         'Server API calls, writing the same directory entry ' +
                         'an `ldapadd` would.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
                ...
                parentId: { type: 'string',
                            description: 'The agent or server it hangs ' +
                                         'beneath. Defaults to this ' +
                                         'server\'s own SPIFFE ID, which is ' +
                                         'what SPIRE uses for an entry ' +
                                         'describing a workload rather ' +
                                         'than a node.' },
                selectors: { type: 'string',
                             description: 'Comma-separated `type:value` ' +
                                          'pairs, split on the FIRST colon ' +
                                          'only — so `docker:label:app:web` ' +
                                          'is type `docker` and value ' +
                                          '`label:app:web`. An entry with NO ' +
                                          'selectors matches every workload, ' +
                                          'which is how a catch-all is ' +
                                          'written and is also the shape of ' +
                                          'one somebody forgot to finish.' },
                dnsNames: { type: 'string',
                            description: 'Comma-separated DNS ' +
                                         'subjectAltNames, added beside the ' +
                                         'SPIFFE ID. What makes an SVID ' +
                                         'usable by TLS software that checks ' +
                                         'a hostname and cannot read a ' +
                                         'SPIFFE ID.' },
                federatesWith: { type: 'string',
                                 description: 'Comma-separated trust domain ' +
                                              'names whose bundles are ' +
                                              'handed to a holder of this ' +
                                              'identity. A name with no ' +
                                              'bundle here contributes ' +
                                              'nothing rather than failing.' },
                x509SvidTtl: { type: 'integer',
                               description:
                                 'Seconds. 0 means spiffe.svidTtl.' },
                jwtSvidTtl: { type: 'integer',
                              description:
                                'Seconds. 0 means spiffe.jwtSvidTtl.' },
                hint: { type: 'string',
            ...
            responseDescription:
              'The entry as it now stands, with its new id.' },

          { action: 'update', operationId: 'updateSpiffeEntry',
            summary: 'Change one field of an entry',
            description: '**What may be changed is DECLARED and not ' +
                         'DERIVED.** The declared half is what the entry may ' +
                         'DO — the SPIFFE ID, the parent, the selectors, the ' +
                         'DNS names, the lifetimes, the hint, the flags — ' +
                         'and it is what the Workload API reads. The derived ' +
                         'half is what HAPPENED: the revision number, the ' +
                         'SVID counter, when it was created. Those are ' +
                         'refused with a list of what is not, because a call ' +
                         'that could rewrite them would make this registry ' +
                         'lie about the service\'s own behaviour in a way ' +
                         'indistinguishable from the recording being ' +
                         'broken.\n\n`ldapmodify` still reaches everything, ' +
                         'which is deliberate: refusing it HERE is the ' +
                         'difference between offering an operation and ' +
                         'merely not preventing it.\n\nThe change applies to ' +
                         'the NEXT SVID issued from this entry. Nothing ' +
                         'already issued changes, and there is nothing to ' +
                         'invalidate — SPIFFE has no revocation.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
                ...
                field: { type: 'string',
                         ...
                         description: 'Which field. Anything else is refused ' +
                                      'naming these.' },
                value: { type: 'string',
                         description: 'A list field takes comma-separated ' +
                                      'values and an empty ' +
                                      'value CLEARS it; a boolean ' +
                                      'takes true or false; a TTL takes ' +
                                      'seconds.' }
              },
          ...
          { action: 'delete', operationId: 'deleteSpiffeEntry',
            ...
            description: 'Anything holding an SVID minted from it keeps that ' +
                         'SVID until it expires. SPIFFE has no revocation — ' +
                         'the answer is a short lifetime and rotation, which ' +
                         'is why the default X509-SVID lifetime here is an ' +
                         'hour and the JWT-SVID one is five minutes.\n\nIf ' +
                         'this was the LAST entry naming that SPIFFE ID, the ' +
                         'identity\'s directory entry under `ou=users` is ' +
                         'marked `spiffeCredentialStatus: revoked` with the ' +
                         'reason on it. The entry is NOT deleted, and that ' +
                         'flag is not a certificate status: nothing reads it ' +
                         'back and no SVID is refused because of it. ' +
                         'Deleting one of several entries that name the same ' +
                         'identity changes nothing there.\n\nA seeded entry ' +
                         'stays deleted until a restart: nothing here is ' +
                         'persisted, but nothing re-creates it either, ' +
                         'because an operator who deleted it meant to.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
      ...
      { method: 'GET', path: BASE + '/spiffe/agents', tag: 'SPIFFE',
        ...
        description: 'An agent appears when it calls `AttestAgent` on the ' +
                     'SPIRE Server API. These entries are a RECORD rather ' +
        ...
        parameters: [
          { name: 'agent', in: 'query', required: false,
            ...
            description: 'One agent, by its SPIFFE ID — always under ' +
                         '`/spire/agent/`. The reply then carries its ' +
                         'directory entry, where the cn is a DIGEST of the ' +
                         'SPIFFE ID and `spiffeAgentId` is the identity.' },
          { name: 'q', in: 'query', required: false, schema: { type: 'string' },
            description: 'Substring of the agent id, the attestation type or ' +
                         'any selector, case-insensitive.' }
        ].concat(this.pagingParameters()),
        responseDescription:
          'The matching agents with the paging that found them.',
        responseSchema: { type: 'object',
      ...
      { method: 'POST', route: BASE + '/spiffe/agents/:action', tag: 'SPIFFE',
        ...
        handler: function (req, res) {
          log.debug("Entering the management API SPIFFE agents action " +
                    "endpoint.");
          const body = parseBody(req);
          const result = adminActions.spiffeAgentsAction(self.withAction(req,
              body));
          if (!result.ok) {
            errorCodes.mark(res, errorCodes.codeOf(result) || 'STS-API-0064');
          }
          self.sendJson(res, result.ok ? 200 : 400, result);
          log.debug("Leaving the management API SPIFFE agents action " +
                    "endpoint.");
        },
        actions: [
          { action: 'ban', operationId: 'banSpiffeAgent',
            summary: 'Refuse this agent at AttestAgent',
            description: 'ONE OF THE FEW REFUSALS IN THIS SERVICE, and it ' +
                         'earns its place: a ban that did not refuse would ' +
                         'make the button a lie. A banned agent gets ' +
                         '`PermissionDenied` from `AttestAgent`.\n\nWhatever ' +
                         'SVID it already holds keeps working until it ' +
                         'expires. There is no revocation in SPIFFE, so a ' +
                         'ban stops the NEXT identity rather than the ' +
                         'current one.\n\nThe agent\'s own entry under ' +
                         '`ou=users` — the one every identity this trust ' +
                         'domain issues a certificate to gets — is marked ' +
                         '`spiffeCredentialStatus: revoked`, and unbanning ' +
                         'marks it active again. It is never deleted, and ' +
                         'nothing reads that flag back.',
            requestBodyRequired: true,
            requestBody: {
              type: 'object',
              properties: {
          ...
          { action: 'delete', operationId: 'deleteSpiffeAgent',
            ...
            description: '**Deleting is forgetting, not revoking.** It ' +
                         'reappears the moment it attests again, because ' +
                         'attestation is not checked here. Ban it if the ' +
      ...
      // CERTIFICATE ENROLLMENT (2026-09-13). Each family's operations are
      // declared beside the family, in a file that registers no route and
      // requires its view model lazily inside each handler — so requiring it
      // here, at 19, moves no route (rule 1), exactly as the GNAP rows above
      // reach `gnap_console.js`.
      ...loadAcmeApi().ROUTES,
      ...loadEstApi().ROUTES,
      ...loadScepApi().ROUTES,
  ...
  // way to bootstrap an administrator. `mgmt-api/CLAUDE.md` has argued that
  // since this API existed, and every word of it is still true of development
  // mode.
  //
  ...
  // operation this service offers, which is a map of the administrative
  // surface; a product deployment that served that to anybody would be handing
  // out the floor plan. The explorer moved to the console on 2026-09-09 and is
  // behind its session now; the HTML-shaped refusal below is kept for a browser
  // that navigates here anyway.
  // ---------------------------------------------------------------------------
  ...
  //
  // What changed is the default. `/admin-api` is a MACHINE surface — no
  // browser, no session, no sign-in screen — so it is reached the way a machine
  // reaches a resource server: an OAuth 2.0 access token this service issued,
  // audienced to this API, carrying `admin:read` for a read and `admin:write`
  // for anything that changes state. Three things are checked and each refuses
  // differently, because they are three different mistakes:
  //
  //   * NO TOKEN, or one this service did not sign, or an expired one — 401
  //     with a `WWW-Authenticate` header naming the scopes, which is what an
  //     OAuth client is built to read.
  ...
  // no token at all and got the "this API requires an access token" 401, which
  // is the least useful thing it could say to a client doing the stricter
  ...
  presentedTokenOf(req) {
    ...
    if (!matched) {
      log.debug("Leaving AdminApi.presentedTokenOf(). Nothing presented.");
      return { token: '', scheme: '' };
    }
    log.debug("Leaving AdminApi.presentedTokenOf(). " + matched[1]);
  ...
  bearerOf(req) {
    ...
    log.debug("Leaving AdminApi.bearerOf().");
  ...
  wantedAudiences(req) {
    ...
    if (pinned && !atDefault) {
      log.debug("Leaving AdminApi.wantedAudiences().");
      return [pinned];
    }
  ...
  wantedAudience(req) {
    ...
    const wanted = this.wantedAudiences(req);
    log.debug("Leaving AdminApi.wantedAudience().");
  ...
  audienceAccepted(claims, req) {
    ...
    log.debug("Leaving AdminApi.audienceAccepted().");
  ...
  // base, and the configured base of this API. A token's `iss` must be an
  // issuer one of this service's authorization servers publishes at one of them
  // — the default one, or a named one beneath it — which is
  // `jwt_access_token.js`'s `isHostedIssuer()`, the same reading the other
  // resource servers here make.
  //
  ...
  // telling the same story. A pinned `adminApi.audience` names no base, which
  // is why the request's own and the configured one are always in the list.
  //
  ...
  issuerAccepted(claims, req) {
    ...
    const self = this;
    log.debug("Entering AdminApi.issuerAccepted().");
    const bases = [];
    ...
    const accepted = realms.run(realms.get(realms.DEFAULT_ID), function () {
      self.wantedAudiences(req).forEach(addBase);
    ...
    log.debug("Leaving AdminApi.issuerAccepted(). accepted=" + accepted);
    return accepted;
  }
  ...
  realmIssuerAccepted(claims, req) {
    ...
    log.debug("Leaving AdminApi.realmIssuerAccepted(). " + accepted);
    return accepted;
  }
  ...
  realmAudienceAccepted(claims, req) {
    ...
    log.debug("Leaving AdminApi.realmAudienceAccepted(). " + accepted);
    return accepted;
  }
  ...
  consoleOperationOf(req) {
    ...
    log.debug("Leaving AdminApi.consoleOperationOf(). " + resource + " " +
              action);
  ...
  realmTokenRefusal(claims, req) {
    ...
    log.debug("Leaving AdminApi.realmTokenRefusal(). " +
  ...
  // an API that requires one. So the options are gathered HERE, beside the
  // table they describe, and a fourth caller gets the same three facts by
  ...
  registerGate(app: RouteApp): void {
    ...
    app.use(BASE, function (req, res, next) {
      if (config.value('adminApi.authRequired')) {
        ...
        // `/realm/<id>/admin-api` it is that realm's — and a token minted at
        // the default realm's token endpoint then fails to verify, which is a
        // 401 on a perfectly good credential.
        //
        // Taking the default realm's key is not a workaround for that; it is
        // the SERVICE credential, and the service roster is the default realm's
        // too. A token a realm's key signed would be believed nowhere else if
        // it were believed here, which is why that case is its own block below:
        // since 2026-09-14 (#32) a realm has administrators of its own, their
        // token is tried only under their realm's prefix, and `admin_scope.js`
        // refuses it every service-wide operation. Believing a realm-minted
        // token AS the service credential would be the hole — anybody who could
        // create a realm minting a token for everything — and this key is what
        // keeps it shut.
        //
        // The AUDIENCE needs no such care: `baseUrlOf()` answers scheme and
        // host with no path, so `<base>/admin-api` is the same string in every
        // realm.
        // ---------------------------------------------------------------------
        let claims = null;
        try {
        ...
        if (!claims) {
          ...
          return self.sendJson(res, 401, { error: 'invalid_token', errors: [
            'That access token was not issued by this service, or its ' +
            'signature does not verify. Tokens are ' +
            'signed with the key at /oauth2/jwks and ' +
        ...
        // to a client named `…/admin-api` would be an administrative credential
        // here.
        ...
        if (!jwtAccessToken.isAccessTokenType(typ)) {
          ...
          return self.sendJson(res, 401, { error: 'invalid_token', errors: [
            ...
            '"at+jwt", and this token\'s is ' +
            (typ ? '"' + typ + '"' : 'absent') +
            '. An ID Token or a refresh token is not an access token, and a ' +
            'token minted before this service issued at+jwt is refused too; ' +
            'ask /oauth2/token for a new one.'] });
        }
        if (tokenRealm === realms.DEFAULT_ID ? !self.issuerAccepted(claims, req)
                                             : !self.realmIssuerAccepted(claims,
                                                 req)) {
          errorCodes.mark(res, 'STS-API-0083');
          res.set('WWW-Authenticate',
                  'Bearer error="invalid_token", scope="' + scopesWanted + '"');
          return self.sendJson(res, 401, { error: 'invalid_token', errors: [
            'RFC 9068 section 4: the iss claim must exactly match an issuer ' +
            'this service publishes, and this token names ' +
            JSON.stringify(claims.iss || null) +
            '. An issuer is an address, so ' +
            'a token minted under one host name is refused under another; ' +
            'mint it at the address you call this API at, or set ' +
            'global.publicBaseUrl.'] });
        }
        const audienceOk = tokenRealm === realms.DEFAULT_ID
          ? self.audienceAccepted(claims, req) :
            self.realmAudienceAccepted(claims, req);
        if (!audienceOk) {
          ...
          return self.sendJson(res, 403, { error: 'forbidden', errors: [
            ...
            self.wantedAudiences(req).map(function (aud) {
              return '"' + aud + '"';
            }).join(' or ') + '. A bearer token minted for another resource ' +
            'server must not be replayable here, which is what `aud` is ' +
            'for.'] });
        }
        ...
        // whose whole point is that holding it is not enough — was accepted
        // here as a bearer token. Every other resource server in this service
        // refuses that through `dpop.presentedAccessToken()`; this gate
        // verifies its own token and so has to ask for itself.
        //
        // It is NOT the two settings: this runs whatever they say, because it
        ...
        if (boundJkt) {
          if (presentation.scheme !== 'dpop') {
            ...
            return self.sendJson(res, 401, { error: 'invalid_token', errors: [
              'That access token is DPoP-bound ' +
              '(it carries cnf.jkt), so it must ' +
              'be sent as "Authorization: DPoP <token>" with a DPoP proof — ' +
              'not ' +
              'as a Bearer token. Presenting it as a bearer token would ' +
          ...
          if (!checked.ok) {
            ...
            return self.sendJson(res, 401, { error: 'invalid_dpop_proof',
        ...
        if (required) {
          ...
          res.set('WWW-Authenticate',
                  (senderConstraints.accessTokenDpopRequired() ? 'DPoP' :
                   'Bearer') +
        ...
        const policy = accessGate.check({
          ...
          // travels in the REQUEST — the `access-control` document is written
          // to take it from there, which is what lets one policy decide for
          // every surface — so naming the role per action is the whole of
  ...
  registerRoutes(app: RouteApp): void {
    ...
    const self = this;
    log.debug("Entering AdminApi.registerRoutes().");
    // -------------------------------------------------------------------------
    ...
    // `parseBody(req)` and refuses in its own words, so a check written inside
    // them would be one check per operation and the next operation's would be
    ...
    ROUTES.forEach(function (entry) {
      ...
      app.post(path, function (req, res) {
        ...
        const checked = self.checkRequestBody(req);
        if (!checked.ok) {
    ...
    log.debug("Leaving AdminApi.registerRoutes().");
  }
}
...
import docs = require('./admin_api_docs');
// `docs` is required for its place in the require order and nothing of it is
// read here; this reference keeps TypeScript from eliding the require.
void docs;
