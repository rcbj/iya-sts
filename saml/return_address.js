...
function confirmHow(spec) {
  return 'Confirm it on the application\'s page under /admin/applications, or ' +
         'with POST /admin-api/applications/confirm-address (application "' +
...
function resolve(spec) {
  log.debug("Entering resolve(). application=" + (spec.application || '(none)'));
  ...
  if (mode.acceptsUnregisteredAddresses()) {
    ...
    return { ok: true, url: String(spec.fallback || ''),
             from: 'this service\'s own mock, because nothing named an address' };
  }
  ...
  if (requested && unconfirmed.indexOf(requested) >= 0) {
    log.debug("Leaving resolve(). Product: the address named is still marked observed.");
    return errorCodes.mark({ ok: false,
             why: 'This realm is in PRODUCT mode, and the ' + spec.parameter + ' "' +
                  requested + '" is on the ' + spec.attribute + ' of "' +
                  (spec.application || '(unnamed)') + '" but was put there by a ' +
                  'request while the realm was in DEVELOPMENT mode, and nobody has ' +
                  'confirmed it. An address a request taught this service is not a ' +
                  'registered one, so it is refused as though it were not on the ' +
                  'entry. ' + confirmHow(spec) }, 'STS-REG-0049');
  }
  if (!registered.length && !requested && unconfirmed.length) {
    log.debug("Leaving resolve(). Product: only observed addresses are on the entry.");
    return errorCodes.mark({ ok: false,
             why: 'This realm is in PRODUCT mode, so a response is delivered only to a ' +
                  'registered address, and every ' + spec.attribute + ' on "' +
                  (spec.application || '(unnamed)') + '" (' + unconfirmed.join(', ') +
                  ') was put there by a request in DEVELOPMENT mode and is still ' +
                  'unconfirmed. The request named no ' + spec.parameter + '. ' +
                  confirmHow(spec) }, 'STS-REG-0049');
  }
  if (!registered.length) {
    log.debug("Leaving resolve(). Product: nothing is registered.");
    return { ok: false,
             why: 'This realm is in PRODUCT mode, so a response is delivered only to an ' +
                  'address registered on the application\'s own entry, and "' +
                  (spec.application || '(unnamed)') + '" has no ' + spec.attribute + '. ' +
                  (requested
                    ? 'The request named ' + requested + ', which is not registered anywhere. '
                    : 'The request named no ' + spec.parameter + ' either. ') +
                  'Register the address on the application (the console\'s application page, ' +
                  'POST /admin-api/applications/add with attribute ' + spec.attribute +
                  ', or an ldapmodify) and send the request again.' };
  }
  if (!requested) {
    log.debug("Leaving resolve(). Product: the request named none; the registered one.");
    return { ok: true, url: registered[registered.length - 1],
             from: 'the ' + spec.attribute + ' on the application entry' };
  }
  if (registered.indexOf(requested) < 0) {
    ...
    return { ok: false,
             why: 'This realm is in PRODUCT mode, and the ' + spec.parameter + ' "' + requested +
                  '" is not one of the ' + registered.length + ' address(es) registered as ' +
                  spec.attribute + ' on "' + (spec.application || '(unnamed)') + '". The ' +
                  'comparison is exact. Register it on the application entry if it is ' +
                  'genuinely that application\'s, or send the request with a registered one.' };
  }
  log.debug("Leaving resolve(). Product: a registered address.");
  return { ok: true, url: requested, from: 'the request, and it is registered' };
}
