...
//
// Separate from wstrust.ts because a SAML assertion is not a WS-Trust concept —
...
// `ws-trust/wstrust.ts` and everything else that took `encryptAssertion` from
// here still does.
// ---------------------------------------------------------------------------
...
// One <Attribute>, as the callers spell it.
...
interface BuildOptions {
  ...
  nameIdFormat?: string;
  nameIdValue?: string | null;
  subjectConfirmation?: { recipient?: string; inResponseTo?: string;
...
class Saml2Assertions {
  ...
  signAssertion(xml: string): string {
    ...
    log.debug("Entering Saml2Assertions.signAssertion().");
    logArtifact('SAML assertion', 'before signing', xml);
    // AFTER the <Issuer>, which is where the SAML 2.0 schema puts a signature
    // on an assertion. The reference is worked out from the root's own `ID` by
    // the signer; passing one here would only be a second place for it to be
    // wrong.
    //
    // THE ALGORITHMS ARE THE CONFIGURED ONES (2026-09-12) —
    // `saml.signatureAlgorithm` and `saml.canonicalizationAlgorithm`. Until
    // then this call passed neither and the signer's defaults decided, which is
    ...
    log.debug("Leaving Saml2Assertions.signAssertion().");
    return signed;
  }
  ...
  // ---------------------------------------------------------------------------
  private attributeValuesOf(a: AttributeRow): string {
    const { log, xmlEscape } = this.deps.helpers;
    log.debug("Entering Saml2Assertions.attributeValuesOf().");
    const values = Array.isArray(a.values) ? a.values : [a.value];
    log.debug("Leaving Saml2Assertions.attributeValuesOf().");
  ...
  // parser is strict about. Omit it and this produces exactly what it always
  // did.
  //
  //   authnContextClassRef  how the End-User authenticated. WS-Federation's
  //                         `wauth` asks for a method and the session records
  //                         which one was actually performed, so the assertion
  //                         has to be able to say something other than the
  //                         default.
  //   attributes            [{ name, nameFormat, value }] replacing the default
  //                         two. A WS-Federation relying party keys off the
  //                         claim URIs from the Microsoft claim namespaces, not
  //                         off `name`.
  //
  // FIVE MORE ARRIVED WITH THE WEB BROWSER SSO PROFILE (saml2_sso.ts), and
  // every one of them is a MUST in saml-profiles-2.0-os section 4.1.4.2 that
  // WS-Trust and WS-Federation genuinely do not need. That is why they are
  // options here rather than a second builder: the element order, the namespace
  // and the signature location are what a service provider's parser is strict
  // about, and there is one place they are decided.
  //
  ...
  //                         transient or persistent format needs an opaque
  //                         value, and the subject is still what the attributes
  //                         say.
  //   subjectConfirmation   { recipient, inResponseTo, notOnOrAfter } becoming
  //                         a <SubjectConfirmationData>. THE PROFILE REQUIRES
  //                         IT: section 4.1.4.2 says the bearer assertion MUST
  //                         carry a Recipient matching the assertion consumer
  //                         service URL and an InResponseTo matching the
  //                         request. A service provider that checks either —
  //                         most do — refuses an assertion without them, and
  //                         the refusal reads as a signature problem to anybody
  //                         who has not met it before.
  //   sessionIndex          the AuthnStatement's SessionIndex. It defaults to
  //                         the assertion's own ID, which is what the two older
  //                         callers got and is fine while nothing logs out;
  //                         Single Logout is the feature that makes it matter,
  //                         because the LogoutRequest names the session by this
  //                         value.
  //   authnInstant          when the person actually authenticated, which is
  //                         the SESSION's authTime and not now. Defaulted to
  ...
  //                         provider that accepts an unsigned assertion has a
  //                         hole in it, and this is how somebody finds that
  ...
  buildSamlAssertion(subject: string, audience?: string | null,
    ...
    log.debug("Entering Saml2Assertions.buildSamlAssertion().");
    opts = opts || {};
    const id = genId();
    const now = iso(0);
    ...
    // them would be a lie about an event rather than an allowance about a
    // clock. Only the Conditions move. With the setting at its default 0 this
    // is byte-for-byte what this function emitted before the setting existed,
    // which is the property that keeps every existing caller and every recorded
    // assertion unchanged.
    ...
    // sign-in (which passes its own claim list) and a WS-Trust Issue (which
    ...
    // an assertion is a list of elements and not an object: a duplicate name
    // does not overwrite anything, it produces two <Attribute> elements with
    // one name, and a relying party reading the first sees whichever this
    // function happened to emit first. It became reachable by ticking a box
    // rather than by typing a name when /admin/claims grew its directory
    // attributes: `cn` becomes the claim `name`, and `name` is one of the two
    // attributes above.
    const names = new Set(attributes.map(function (a) { return a.name; }));
    const configured = custom.filter(function (a) {
      return !names.has(a.name);
    });
    ...
    // the other two callers have no request to answer. Built as an
    // EMPTY-ELEMENT SubjectConfirmation when there is nothing to say, exactly
    // as before, because that is what every existing caller's output has been
    // and a self-closing element is not the same document as one with an empty
    // child.
    ...
    const confirmation = scd
      ? '<saml:SubjectConfirmation ' +
          ...
          (scd.recipient ?
           ' Recipient="' + xmlEscape(scd.recipient) + '"' : '') +
          (scd.inResponseTo ?
    ...
    // both come through this function, so this counts every SAML 2.0 assertion
    // instead of every one somebody remembered to count. `exp` is the
    ...
    const record = stats.recordAssertion('2.0',
                                         { id: id, subject: subject,
                                                  ...
                                                  expiresAt: Date.parse(exp) ||
                                                      0 });
    // `sign: false` is a state, not a failure — see the option's note above —
    // so the record is corrected here for the same reason it is corrected in
    // the catch below: the console shows whether an assertion was signed, and
    // an unsigned one is the single most useful thing that column can say.
    if (opts.sign === false) {
      record.signed = false;
      log.debug("Leaving Saml2Assertions.buildSamlAssertion(). Unsigned, " +
                "because the caller asked for that.");
      return xml;
    }
    try {
      log.debug("Leaving Saml2Assertions.buildSamlAssertion().");
      return this.signAssertion(xml);
    } catch (e) {
      log.debug("Caught in Saml2Assertions.buildSamlAssertion(): " +
                ((e && e.message) || e));
  ...
  //
  // It did not move because it was duplicated — it never was: one
  // implementation with two callers, this file's SSO profile and WS-Trust's
  // `?encrypt=1`. It moved because the four crypto families are now in one
  // module, and leaving the only cipher in the service inside a SAML file would
  // have meant "all of it except that one" — which is the shape of exception
  // that stops being true.
  //
  // **IT WAS NOT REPLACED BY THE VENDORED `encryptXml()`/`decryptXml()`, and
  // that is argued at the top of Section 2 over there rather than here** —
  // briefly, the two produce byte-compatible documents so there was no interop
  // gap to close, and this one answers rather than throwing, checks the
  // unwrapped key's LENGTH, parses the plaintext before calling CBC a success,
  // and tells a NamespaceError apart from a wrong certificate. Those messages
  // are the product.
  //
  ...
  // before-and-after of an encryption is printed exactly as it was, and none of
  ...
  encryptElement(xml: string, certPem: string, opts?: object) {
    ...
    log.debug("Entering Saml2Assertions.encryptElement().");
    log.debug("Leaving Saml2Assertions.encryptElement().");
  ...
  encryptAssertion(assertionXml: string, certPem: string,
    ...
    const { stsCrypto } = this.deps;
    const { log, logArtifact } = this.deps.helpers;
    log.debug("Entering Saml2Assertions.encryptAssertion().");
    log.debug("Leaving Saml2Assertions.encryptAssertion().");
  ...
  decryptElement(xml: string, privateKeyPem: string, opts?: object) {
    const { stsCrypto } = this.deps;
    const { log, logArtifact } = this.deps.helpers;
    log.debug("Entering Saml2Assertions.decryptElement().");
    log.debug("Leaving Saml2Assertions.decryptElement().");
