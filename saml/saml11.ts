...
// SECOND CALLER, `saml11_sso.ts`, which puts the same assertion in front of a
...
// than a flag on saml2.ts is that **SAML 1.1 is a different specification and
...
// offered — see `wsfed.ts`, and `fed:TokenTypesOffered` in the federation
...
//     one explicitly (see verifyAssertionSignature in wsfed.ts) or the
...
// As in saml2.ts: the custom attributes an admin configured, and the register
...
class Saml11Assertions {
  ...
  //
  // **`idAttribute: 'AssertionID'` IS LOAD-BEARING ON THE WAY OUT AS WELL AS
  // THE WAY BACK IN, and this file said the opposite until 2026-08-24.** It
  // used to read "SIGNING does not care that the id attribute has an unusual
  // name; only verification does", which is true of the DIGEST and false of the
  // document. xml-crypto's ensureHasId() looks for the first of `Id`, `ID`,
  // `id` on the node being signed, and when it finds none it INVENTS ONE —
  // `Id="_0"` — and overrides the reference URI passed below with `#_0`. So
  // every SAML 1.1 assertion this service issued carried a bogus `Id` attribute
  // the schema does not have, and a reference naming it instead of the
  // AssertionID.
  //
  // It verified anyway, which is why it went unnoticed for so long: a verifier
  // resolving `#_0` finds the injected attribute. Two things broke it. A
  // relying party that resolves the reference through `AssertionID` — which is
  // what a SAML-aware verifier does, and what wsfed.ts's own
  // verifyAssertionSignature() is told to do — looks for an element with that
  // AssertionID and finds the reference pointing somewhere else. And, the
  // failure that actually surfaced it, **a document carrying TWO signatures
  // gets `Id="_0"` on both**: the saml11_sso.ts browser profiles sign the
  // assertion and the Response around it, and xml-crypto then refuses to verify
  // either, reporting "multiple elements with the same value for the ID / Id /
  // Id attributes" — its signature-wrapping guard, firing on a document nothing
  // malicious ever touched.
  //
  ...
  // real identity provider emits. It is safe to name because `AssertionID` is
  ...
  signSaml11Assertion(xml) {
    ...
    log.debug("Entering Saml11Assertions.signSaml11Assertion().");
    ...
    //
    // Exclusive canonicalization is the shared signer's default and the choice
    // is load-bearing — the assertion is signed as a standalone document and
    // then embedded in an RSTR that declares prefixes of its own, so inclusive
    // c14n would pull those ancestor declarations into the digest at
    // verification time and the signature would fail for every relying party
    // while verifying perfectly here. That argument is now made once, over
    ...
    log.debug("Leaving Saml11Assertions.signSaml11Assertion().");
    return signed;
  }
  ...
  // ---------------------------------------------------------------------------
  private attributeValuesOf(a) {
    const { log, xmlEscape } = this.deps.helpers;
    log.debug("Entering Saml11Assertions.attributeValuesOf().");
    const values = Array.isArray(a.values) ? a.values : [a.value];
    log.debug("Leaving Saml11Assertions.attributeValuesOf().");
  ...
  //
  // SEVEN MORE ARRIVED WITH THE BROWSER PROFILES (saml11_sso.ts), and this is
  // the same growth saml2.ts took for the Web Browser SSO profile — options on
  // the one builder rather than a second builder, for the reason this directory
  // has always given: one assertion writer means ONE place where the element
  // order, the attribute spelling and the signature location are decided, and
  // those are exactly what a relying party's parser is strict about. It also
  // means the custom SAML 1.1 attributes configured on /admin/saml-attributes
  // reach an assertion issued by the browser profiles with no wiring at all.
  //
  ...
  //                       entitled to see one.
  //   nameIdValue         the NameIdentifier's text, when it is not the
  //                       subject.
  //   nameQualifier       the NameIdentifier's NameQualifier, which SAML 1.1
  //                       has and SAML 2.0 spells the same way. A Shibboleth
  //                       service provider keys its attribute resolution off
  //                       it, so the browser profiles set it to the identity
  //                       provider's own providerID; omitted entirely when
  //                       nothing asks, because an empty NameQualifier is not
  //                       the same as no attribute.
  ...
  //                       artifact-profile assertion confirmed as `bearer` says
  //                       the assertion travelled through the browser when it
  //                       did not.
  //   subjectLocality     { ipAddress, dnsAddress } becoming a
  //                       <SubjectLocality> inside the AuthenticationStatement.
  //                       Section 4.2 asks the identity provider to record
  //                       where the browser was, and it is the one element here
  //                       written from the HTTP request rather than from the
  //                       session.
  //   doNotCache          adds <DoNotCacheCondition/>. The Browser/POST
  //                       profile's single-use policy: the assertion travels
  //                       through the browser, so a relying party is told not
  //                       to keep it. It is off for the two older callers,
  //                       whose assertions do not pass through a browser at
  //                       all.
  //   authenticationStatement
  //                       false leaves the <AuthenticationStatement> OUT
  //                       (2026-09-12). Default true, which is every caller
  //                       that existed before it. An answer to an
  //                       AttributeQuery is a statement about ATTRIBUTES;
  //                       writing an authentication statement into it — with a
  //                       method and an instant — told a relying party that
  //                       somebody signed in who may never have been near this
  //                       service. SAML 1.1 lets an assertion carry any
  //                       non-empty set of statements, so leaving it out is a
  //                       smaller document rather than an invalid one.
  //   sign                false returns the assertion UNSIGNED. It is a test
  //                       case rather than a mistake — a relying party that
  ...
  buildSaml11Assertion(opts) {
    ...
    // length above its own two lines: `now` still states when this service
    // issued and when the person authenticated, and only the Conditions move.
    // The setting is shared with SAML 2.0 rather than being a second row
    // because a deployment allowing for a relying party's clock has decided
    // that once, and the two profiles are issued to the same estate.
    ...
    // saml2.ts gives beside its own default: a caller that names no method has
    ...
    // The NameIdentifier, and the one thing to know about the defaults: they
    // are what this service has said for years and nothing consumed, so they
    // stay the defaults rather than becoming what a browser-profile relying
    // party asked for. A caller that was asked for a format passes it.
    const nameIdFormat = opts.nameIdFormat || NAMEID_FORMAT_UNSPECIFIED;
    const nameIdValue = opts.nameIdValue == null ? opts.subject :
                        opts.nameIdValue;
    // OMITTED rather than written empty when nothing asks: `NameQualifier=""`
    // is a qualifier whose value is the empty string, which is not the same
    // document as one with no qualifier, and the difference is inside the
    // signature.
    ...
    // rather than a preference. Both attributes are optional and each is
    // omitted when the caller has nothing to say, for the reason NameQualifier
    // is.
    ...
    const localityEl = locality && (locality.ipAddress || locality.dnsAddress)
      ? '<saml:SubjectLocality' +
        ...
        '/>'
      : '';
    // Appended to what the caller asked for, never substituted for it — the
    // same rule as SAML 2.0, and it matters more here: a WS-Federation relying
    // party keys off the claim URIs in claimsFor(), and displacing one of those
    ...
    // attributes to tick.
    //
    // Keyed on the NAMESPACE AND THE NAME together, which is the only correct
    // key here: SAML 1.1 splits a claim URI into the two, so `name` in the
    // identity claims namespace and `name` in somebody else's are different
    // claims, and a filter on the local name alone would drop a configured
    // attribute that collided with nothing. The one collision it does catch is
    // real — the WS-Federation claim list carries `name` in that namespace, and
    // so does a ticked `cn`.
    ...
    const attributeEls = asked.concat(configured).map(function (a) {
      return '<saml:Attribute AttributeName="' + xmlEscape(a.name) + '"' +
        ...
        self.attributeValuesOf(a) + '</saml:Attribute>';
    }).join('');
    ...
    const xml =
      '<saml:Assertion xmlns:saml="' + SAML11_NS + '"' +
        ...
        (attributeEls
          ...
          : '') +
      '</saml:Assertion>';
    // Counted before the signing attempt, not after: an assertion that failed
    // to sign was still built and still went out (unsigned — see the catch), so
    ...
    const record = stats.recordAssertion('1.1',
                                         { id: id, subject: opts.subject,
                                                  ...
                                                  expiresAt: Date.parse(exp) ||
                                                      0 });
    // `sign: false` is a DELIBERATE unsigned assertion and is recorded as one,
    // so that the console's Signed column tells it apart from a signing FAILURE
    // below. The two look identical on the wire and mean opposite things: one
    // is somebody exercising a relying party's checking, the other is this
    // service being broken.
    if (opts.sign === false) {
      record.signed = false;
      log.debug("Leaving Saml11Assertions.buildSaml11Assertion(). Unsigned " +
                "because the caller " +
                "asked for that.");
      return xml;
    }
    try {
      const signed = this.signSaml11Assertion(xml);
      log.debug("Leaving Saml11Assertions.buildSaml11Assertion(). " +
                "AssertionID " + id + ".");
      return signed;
    } catch (e) {
      log.debug("Caught in Saml11Assertions.buildSaml11Assertion(): " +
                ((e && e.message) || e));
      // Returned unsigned rather than not at all, exactly as saml2.ts does: an
      // unsigned assertion in the response is something a relying party can
      // look at and reject for the right reason, where a 500 here would say
      // nothing about what failed. The log line is the record of which it was —
      // and so, now, is the console's Signed column, which is why the record is
      // corrected here.
      record.signed = false;
...
export = {
  ...
  // exporting them means saml11_sso.ts cannot spell either differently.
