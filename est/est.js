...
function estError(req, res, ctx, status, sentence, headers) {
  log.debug("Entering estError(). status=" + status);
...
// carries the Basic challenge where Basic is accepted; a 429 carries
// Retry-After.
function refuseWith(req, res, ctx, refusal, fallbackCode) {
  log.debug("Entering refuseWith().");
  ...
  log.debug("Leaving refuseWith().");
  errorCodes.mark(res, errorCodes.codeOf(refusal) || fallbackCode ||
                       'STS-EST-0020');
  return estError(req, res, ctx, status, sentence, headers);
}

// A body answered base64, as RFC 8951 section 3 requires of every EST response
// that carries DER.
function sendBase64(res, contentType, der) {
  log.debug("Entering sendBase64().");
  res.status(200)
     .set('Content-Type', contentType)
     .set('Content-Transfer-Encoding', 'base64')
     .set('Cache-Control', 'no-store')
     .send(Buffer.from(codec.base64Lines(der), 'latin1'));
  log.debug("Leaving sendBase64().");
}
...
// ---------------------------------------------------------------------------
function refusedBeforeAuthentication(req, res, ctx) {
  log.debug("Entering refusedBeforeAuthentication(). op=" + ctx.op);
  const query = validation.check(req, 'query', NO_QUERY);
  if (!query.ok) {
    errorCodes.mark(res, 'STS-EST-0019');
    estError(req, res, ctx, 400, 'No EST operation takes a query string: ' +
             query.detail);
    log.debug("Leaving refusedBeforeAuthentication(). A query.");
    return true;
  }
  if (config.value('est.enabled') === false) {
    errorCodes.mark(res, 'STS-EST-0001');
    estError(req, res, ctx, 503, 'EST is turned off in this realm ' +
             '(est.enabled).');
    log.debug("Leaving refusedBeforeAuthentication(). Disabled.");
    return true;
  }
  const transport = core.transportRefusal(req, FAMILY);
  if (transport) {
    refuseWith(req, res, ctx, transport);
    log.debug("Leaving refusedBeforeAuthentication(). Transport.");
    return true;
  }
  ...
  if (label !== undefined) {
    ...
    if (!known) {
      ...
      estError(req, res, ctx, 404, 'There is no EST label "' +
               String(label).slice(0, 40) + '" here. A label is a ' +
               'certificate profile, one of: ' + core.PROFILE_IDS.join(', ') +
               '.');
      log.debug("Leaving refusedBeforeAuthentication(). Unknown label.");
      return true;
    }
    ctx.labelled = true;
  ...
  if (ctx.labelled || (enrolling && ctx.op !== 'simplereenroll')) {
    ...
    if (!profile.ok) {
      refuseWith(req, res, ctx, profile);
      log.debug("Leaving refusedBeforeAuthentication(). Profile.");
      return true;
    }
  }
  log.debug("Leaving refusedBeforeAuthentication(). Passed.");
  return false;
}
...
function refusedBeforeBody(req, res, ctx) {
  ...
  if (type !== MEDIA.pkcs10) {
    ...
    estError(req, res, ctx, 415, 'An EST enrollment body is ' + MEDIA.pkcs10 +
             ' (RFC 7030 section 4.2.1), and this one was "' +
             type.slice(0, 80) + '".');
    log.debug("Leaving refusedBeforeBody(). Media type.");
    return true;
  }
  ...
  if ((Number.isFinite(declared) && declared > max) || bytes > max) {
    ...
    estError(req, res, ctx, 413, 'The request body is larger than the ' + max +
             ' bytes est.maxRequestBytes allows.');
    log.debug("Leaving refusedBeforeBody(). Too large.");
    return true;
  }
  log.debug("Leaving refusedBeforeBody(). Passed.");
  return false;
}
...
// The Authorization header, READ but not believed: which scheme, and for Basic
// the name and password. A malformed Basic credential is reported as such
// rather than as a wrong password.
function basicCredentialOf(req) {
  log.debug("Entering basicCredentialOf().");
  const header = String(req.headers.authorization || '');
  if (!header) {
    log.debug("Leaving basicCredentialOf(). None.");
    return { present: false };
  }
  if (!/^basic(\s|$)/i.test(header)) {
    log.debug("Leaving basicCredentialOf(). Another scheme.");
    return { present: true, basic: false };
  }
  const match = /^basic\s+([A-Za-z0-9+/]+={0,2})\s*$/i.exec(header);
  if (!match || header.length > MAX_AUTHORIZATION ||
      match[1].length % 4 !== 0) {
    log.debug("Leaving basicCredentialOf(). Malformed.");
    return { present: true, basic: true, malformed: true };
  }
  const decoded = Buffer.from(match[1], 'base64');
  if (decoded.toString('base64') !== match[1]) {
    log.debug("Leaving basicCredentialOf(). Not canonical.");
    return { present: true, basic: true, malformed: true };
  }
  ...
  if (colon < 1 || !core.wellFormedId(username)) {
    log.debug("Leaving basicCredentialOf(). No usable name.");
    return { present: true, basic: true, malformed: true };
  }
  log.debug("Leaving basicCredentialOf().");
  return { present: true, basic: true, username: username,
           password: text.slice(colon + 1) };
}

function applicationNamed(clientId) {
  log.debug("Entering applicationNamed().");
  let view = null;
  try {
    view = applications.forClientId(clientId) || applications.get(clientId);
  } catch (e) {
    log.debug("Caught in applicationNamed(): " + ((e && e.message) || e));
    view = null;
  }
  log.debug("Leaving applicationNamed().");
...
async function authenticate(req, ctx) {
  ...
  if (basic.present) {
    if (!basic.basic) {
      log.debug("Leaving authenticate(). Not Basic.");
    ...
    if (config.value('est.basicAuthentication') === false) {
      log.debug("Leaving authenticate(). Basic is off.");
      return core.refuse('STS-EST-0010', 401, 'HTTP Basic is turned off ' +
                         'for EST in this realm (est.basicAuthentication).');
    }
    if (basic.malformed) {
      log.debug("Leaving authenticate(). Malformed Basic.");
      return core.refuse('STS-EST-0011', 401, 'The HTTP Basic credential is ' +
  ...
  if (mtls.peerCertificate(req)) {
    ...
    const byCertificate = await core.authenticateCertificate(req,
                                                             'est-certificate');
    log.debug("Leaving authenticate(). Certificate ok=" + !!byCertificate.ok);
    return byCertificate;
  }
  log.debug("Leaving authenticate(). No credential.");
...
function identityHintOf(req, basic) {
  log.debug("Entering identityHintOf().");
  if (basic.present && basic.username) {
    log.debug("Leaving identityHintOf(). Basic.");
    return basic.username;
  }
  const presented = basic.present ? null : mtls.peerCertificate(req);
  log.debug("Leaving identityHintOf().");
...
function principalLabel(principal) {
  log.debug("Entering principalLabel().");
  log.debug("Leaving principalLabel().");
...
// already answered.
async function enrollmentRequest(req, res, ctx, parseOptions) {
  ...
  const authenticated = await authenticate(req, ctx);
  if (!authenticated.ok) {
    refuseWith(req, res, ctx, authenticated);
    log.debug("Leaving enrollmentRequest(). Not authenticated.");
    return null;
  }
  ctx.principal = authenticated.principal;
  ctx.identity = principalLabel(ctx.principal);
  const decoded = codec.decodeBody(req.body);
  if (!decoded.ok) {
    errorCodes.mark(res, 'STS-EST-0008');
    estError(req, res, ctx, 400, decoded.why);
    log.debug("Leaving enrollmentRequest(). Not base64.");
    return null;
  }
  const csr = await core.parseCsr(decoded.der, parseOptions || {});
  if (!csr.ok) {
    refuseWith(req, res, ctx, csr);
    log.debug("Leaving enrollmentRequest(). The CSR was refused.");
    return null;
  }
  log.debug("Leaving enrollmentRequest(). key=" + csr.keyAlg);
  return csr;
}

// The certificate the core issued, as RFC 7030 section 4.2.3 returns it.
function answerIssued(res, ctx, issued) {
  log.debug("Entering answerIssued().");
  ctx.targetUri = core.entryUri(issued.target);
  counted(ctx, { outcome: 'issued', status: 200,
                 serialHex: issued.record.serialHex });
  sendBase64(res, MEDIA.certsOnly,
             codec.certsOnly([issued.record.certificatePem]));
  log.info('est: ' + ctx.op + ' issued ' + issued.record.profile + ' serial ' +
           issued.record.serialHex + ' for ' + ctx.targetUri + ' to ' +
           ctx.identity);
  log.debug("Leaving answerIssued().");
}
...
// 4.1 — the CA certificates: the EST Issuing CA, this realm's Intermediate and
// the service Root. Unauthenticated, as section 4.1.1 says it MUST be: a client
// needs the anchor before it can check anything else.
async function cacerts(req, res, ctx) {
  log.debug("Entering cacerts().");
  const chain = await core.ensureAuthority(FAMILY);
  if (!chain.ok) {
    errorCodes.mark(res, 'STS-EST-0014');
    estError(req, res, ctx, 503, 'This realm has no certificate hierarchy ' +
             'yet, so there is no EST CA certificate to return. Build one on ' +
             '/admin/pki.');
    log.debug("Leaving cacerts(). No hierarchy.");
    return;
  }
  counted(ctx, { outcome: 'answered', status: 200 });
  sendBase64(res, MEDIA.certsOnly, codec.certsOnly(chain.chainPem));
  log.debug("Leaving cacerts().");
}

// 4.2.1 — a certificate for a key the client holds.
async function simpleenroll(req, res, ctx) {
  log.debug("Entering simpleenroll().");
  const csr = await enrollmentRequest(req, res, ctx);
  if (!csr) {
    log.debug("Leaving simpleenroll(). Answered.");
    return;
  }
  const target = core.targetFromRequest(csr.requested, csr.commonName,
                                        ctx.principal);
  if (!target.ok) {
    refuseWith(req, res, ctx, target);
    log.debug("Leaving simpleenroll(). No target.");
    return;
  }
  ...
  if (!issued.ok) {
    refuseWith(req, res, ctx, issued);
    log.debug("Leaving simpleenroll(). Refused by the core.");
    return;
  }
  answerIssued(res, ctx, issued);
  log.debug("Leaving simpleenroll().");
}
...
function subjectOfCertificate(pem) {
  log.debug("Entering subjectOfCertificate().");
  ...
  try {
    ...
    log.debug("Caught in subjectOfCertificate(): " + ((e && e.message) || e));
    subject = null;
  }
  log.debug("Leaving subjectOfCertificate().");
  return subject;
}
...
function requestedNameSet(requested) {
  log.debug("Entering requestedNameSet().");
  ...
  log.debug("Leaving requestedNameSet().");
  return out.filter(function (one, i) {
    return out.indexOf(one) === i;
  }).sort();
}

function recordNameSet(record) {
  log.debug("Entering recordNameSet().");
  ...
  log.debug("Leaving recordNameSet().");
  return out.filter(function (one, i) {
    return out.indexOf(one) === i;
  }).sort();
}

// Does this request repeat the renewed certificate's subject and names exactly
// (RFC 7030 section 4.2.2)?
function repeats(csr, record) {
  log.debug("Entering repeats().");
  const subject = subjectOfCertificate(record.certificatePem);
  const same = subject !== null && subject === csr.subject &&
    JSON.stringify(requestedNameSet(csr.requested)) ===
    JSON.stringify(recordNameSet(record));
  log.debug("Leaving repeats(). " + same);
  return same;
}
...
// among the target entry's valid EST certificates by the subject and names the
// request repeats. Either way it is superseded on the EST CRL once the new one
// is issued.
async function simplereenroll(req, res, ctx) {
  log.debug("Entering simplereenroll().");
  const csr = await enrollmentRequest(req, res, ctx);
  if (!csr) {
    log.debug("Leaving simplereenroll(). Answered.");
    return;
  }
  let target = null;
  let renewed = null;
  ...
  if (byCertificate) {
    if (!byCertificate.ok) {
      refuseWith(req, res, ctx, byCertificate);
      log.debug("Leaving simplereenroll(). The presented certificate.");
      return;
    }
    ...
    if (!allowed.ok) {
      refuseWith(req, res, ctx, allowed);
      log.debug("Leaving simplereenroll(). Not authorized.");
      return;
    }
    ...
    if (!renewed || !repeats(csr, renewed)) {
      errorCodes.mark(res, 'STS-EST-0016');
      estError(req, res, ctx, 400, 'A re-enrollment request must repeat the ' +
               'subject and subjectAltName of the certificate being renewed ' +
               '(RFC 7030 section 4.2.2), and this one differs from the TLS ' +
               'client certificate presented.');
      log.debug("Leaving simplereenroll(). Differs from the certificate.");
      return;
    }
  } else {
    const named = core.targetFromRequest(csr.requested, csr.commonName,
                                         ctx.principal);
    if (!named.ok) {
      refuseWith(req, res, ctx, named);
      log.debug("Leaving simplereenroll(). No target.");
      return;
    }
    target = named.target;
    ctx.targetUri = core.entryUri(target);
    ...
    const held = core.enrolledOf(target).filter(function (one) {
      return one.family === FAMILY && repeats(csr, one);
    });
    ...
    if (!renewed) {
      ...
      estError(req, res, ctx, 400, held.length
        ? 'The certificate this request renews has expired or been revoked; ' +
          'ask /simpleenroll for a new one.'
      ...
      log.debug("Leaving simplereenroll(). Nothing to renew.");
      return;
    }
  }
  if (renewed.status !== 'valid') {
    errorCodes.mark(res, 'STS-EST-0021');
    estError(req, res, ctx, 400, 'The certificate this request renews has ' +
             'expired or been revoked; ask /simpleenroll for a new one.');
    log.debug("Leaving simplereenroll(). Not valid.");
    return;
  }
  ...
  if (!issued.ok) {
    refuseWith(req, res, ctx, issued);
    log.debug("Leaving simplereenroll(). Refused by the core.");
    return;
  }
  answerIssued(res, ctx, issued);
  log.debug("Leaving simplereenroll().");
}
...
function serverKeyAlgorithm(csr, profile) {
  log.debug("Entering serverKeyAlgorithm().");
  const ids = keyMaterial.keyAlgIds();
  const alg = ids.indexOf(String(csr.keyAlg)) >= 0
    ? String(csr.keyAlg)
    : (profile === 'key-encipherment' ? 'rsa-2048' : 'ec-p256');
  log.debug("Leaving serverKeyAlgorithm(). " + alg);
  return alg;
}
...
async function serverkeygen(req, res, ctx) {
  log.debug("Entering serverkeygen().");
  if (config.value('est.serverKeyGeneration') === false) {
    errorCodes.mark(res, 'STS-EST-0005');
    estError(req, res, ctx, 501, 'Server-side key generation is turned off ' +
             'for EST in this realm (est.serverKeyGeneration).');
    log.debug("Leaving serverkeygen(). Off.");
    return;
  }
  const csr = await enrollmentRequest(req, res, ctx, { template: true });
  if (!csr) {
    log.debug("Leaving serverkeygen(). Answered.");
    return;
  }
  ...
  if (encrypted.length) {
    ...
    estError(req, res, ctx, 501, 'The request asks for the private key to be ' +
             'encrypted (RFC 7030 section 4.4.1.2), which this server does ' +
             'not do; it refuses rather than return the key unencrypted.');
    log.debug("Leaving serverkeygen(). Encryption asked for.");
    return;
  }
  const keyAlg = serverKeyAlgorithm(csr, ctx.profile);
  ...
  if (described.kind === 'pqc' && described.use === 'kem' &&
    ...
    estError(req, res, ctx, 400, 'A ' + keyAlg + ' key is a ' +
             'key-encapsulation key and can be certified only for ' +
             'key-encipherment; this request is for ' + ctx.profile + '.');
    log.debug("Leaving serverkeygen(). A KEM key for a signing profile.");
    return;
  }
  const target = core.targetFromRequest(csr.requested, csr.commonName,
                                        ctx.principal);
  if (!target.ok) {
    refuseWith(req, res, ctx, target);
    log.debug("Leaving serverkeygen(). No target.");
    return;
  }
  ctx.targetUri = core.entryUri(target.target);
  ...
  if (!issued.ok) {
    refuseWith(req, res, ctx, issued);
    log.debug("Leaving serverkeygen(). Refused by the core.");
    return;
  }
  ...
  log.debug("Leaving serverkeygen().");
}
...
function csrAttributesFor(profileId) {
  ...
  log.debug("Leaving csrAttributesFor(). " + items.length + " item(s).");
  return items;
}
...
async function csrattrs(req, res, ctx) {
  log.debug("Entering csrattrs().");
  counted(ctx, { outcome: 'answered', status: 200 });
  sendBase64(res, MEDIA.csrattrs,
             codec.csrAttrs(csrAttributesFor(ctx.profile)));
  log.debug("Leaving csrattrs().");
}

// 4.3 — Full CMC is optional and not implemented.
async function fullcmc(req, res, ctx) {
  log.debug("Entering fullcmc().");
  errorCodes.mark(res, 'STS-EST-0004');
  estError(req, res, ctx, 501, 'Full CMC (RFC 7030 section 4.3) is not ' +
           'implemented here; use /simpleenroll, /simplereenroll or ' +
           '/serverkeygen.');
  log.debug("Leaving fullcmc().");
}
...
function operation(op) {
  log.debug("Entering operation(). op=" + op.name);
  ...
  return function (req, res) {
    ...
    let started = null;
    try {
      if (refusedBeforeAuthentication(req, res, ctx)) {
    ...
    return Promise.resolve(started).catch(function (e) {
      ...
      estError(req, res, ctx, 500, 'The EST server could not answer this ' +
               'request.');
    }).then(function () {
...
function wrongMethod(op) {
  log.debug("Entering wrongMethod(). op=" + op.name);
  log.debug("Leaving wrongMethod().");
  return function (req, res) {
    log.debug("Entering the EST wrong-method answer.");
    errorCodes.mark(res, 'STS-EST-0003');
    estError(req, res, { op: op.name }, 405, 'EST ' + op.name + ' answers ' +
...
[BASE, BASE + '/:label'].forEach(function (base) {
  OPERATIONS.forEach(function (op) {
    ...
    if (op.method === 'GET') {
      app.get(path, operation(op));
    } else {
      app.post(path, operation(op));
    }
  });
});
...
app.use(function (req, res, next) {
  ...
  return wrongMethod(op)(req, res);
});

// The two console pages, required HERE so that the family is one line in
// `common/protocol_stack.js`.
require('./est_admin');
...
module.exports = {
  ...
  basicCredentialOf: basicCredentialOf,
  csrAttributesFor: csrAttributesFor,
  serverKeyAlgorithm: serverKeyAlgorithm,
  requestedNameSet: requestedNameSet,
  recordNameSet: recordNameSet
};
