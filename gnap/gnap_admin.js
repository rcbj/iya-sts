...
app.get('/admin/gnap', function (req, res) {
  ...
  const capabilityRows = json.authorizationServers.length
    ...
    : '<tr><td colspan="7" class="sub">No named authorization server exists ' +
      'in this realm yet; the default one answers at ' +
  ...
  const grantNav = admin.pageNavPair('/admin/gnap', req.query,
                                     Object.assign({}, json.grants.paging,
                                                   { param: 'grantsPage' }));
  const grantRows = json.grants.rows.length ?
    ...
    return '<tr><td><code>' + esc(grant.id) + '</code></td><td>' +
      ...
      (grant.resourceOwner ? '<code>' + esc(grant.resourceOwner) + '</code>' :
       '<span ' +
  ...
  const resourceRows = json.resourceSets.rows.length ?
    ...
    return '<tr><td><code>' + esc(row.reference) + '</code></td><td><code>' +
      esc(row.resourceServer) +
      '</code></td><td><code>' + esc(JSON.stringify(
          row.access)) + '</code></td><td>' +
      list(row.tokenFormats) + '</td><td>' + (row.introspectionRequired ?
                                              'yes' : 'no') + '</td><td>' +
      esc(when(row.createdAt)) + '</td><td><form method="post" ' +
      'action="/admin/gnap"><input type="hidden" name="action" ' +
      'value="delete-resource-set"><input type="hidden" name="reference" ' +
      'value="' + esc(row.reference) + '"><button ' +
      'type="submit" class="danger">Delete</button></form></td></tr>';
  }).join('') : '<tr><td colspan="7" class="sub">No resource server has ' +
    'registered a resource set in this realm (RFC 9767 section 3.4).</td></tr>';
  const material = json.verificationMaterial;
  const inner = (typeof admin.messagesOf === 'function' ?
                 admin.messagesOf(req) : '') +
    admin.note('<strong>GNAP (RFC 9635) and its resource server connections ' +
      '(RFC 9767), one authorization server per trust realm.</strong> A ' +
      'client instance proves a key at the grant endpoint, a resource owner ' +
      'approves in a browser through the one sign-in screen, and the tokens ' +
      'are released at the continuation URI in any of the five RFC 9767 ' +
      'formats. ' +
      (json.enabled ? '' :
       '<strong>GNAP is turned off in this realm.</strong>')) +
    admin.warn('<strong>A key proof is verified in every mode.</strong> An ' +
      'unsigned or wrongly signed request is refused whatever ' +
      '<code>global.mode</code> says, because a token bound to a key nobody ' +
    ...
    '<h2>Endpoints</h2><table class="kv">' + endpointRows + '</table><h2>Authorization ' +
    'servers</h2><p class="sub">Each named authorization server is also a ' +
    'GNAP authorization server at <code>/{id}/gnap</code>. Its discovery ' +
    'members are overridden or removed on <a ' +
    'href="/admin/authorization-servers">Authorization servers</a>, and what ' +
    ...
    'proofs</th><th>Token formats</th><th>Key rotation</th></tr></thead>' +
    '<tbody><tr><td><code>default</code></td><td>' +
    '<code>' + esc(json.capabilities.grant_request_endpoint) +
    ...
    esc(json.capabilities.key_rotation_supported) + '</td></tr>' + capabilityRows + '</tbody></table><h2>Token ' +
    'formats</h2><table class="kv"><tr><th>jwt-signed</th><td>RS256, ' +
    'verified against <code>' + esc(material ? material.jwt.jwks_uri : '') +
    '</code></td></tr><tr><th>jwt-encrypted</th><td>Nested in a JWE: to the ' +
    'resource server\'s <code>gnapJweKey</code>, or to this authorization ' +
    'server (introspect it)</td></tr><tr><th>macaroon</th><td>HMAC-SHA256 ' +
    ...
    'Resource owner</th><th>Server</th><th>Interaction</th><th>Tokens</th>' +
    ...
    'Access</th><th>Formats</th><th>Introspection</th><th>Registered</th><th>' +
    '</th></tr></thead><tbody>' + resourceRows +
    '</tbody></table>' + resourceNav.foot +
    '<h2>Settings</h2>' + admin.configFormsFor('/admin/gnap') +
    '<p class="links"><a href="/admin/gnap?format=json">JSON</a> · <code>GET ' +
...
app.get('/admin/gnap/monitor', function (req, res) {
  ...
  const rows = json.rows.length ? json.rows.map(function (row) {
    ...
    const formats = Object.keys(c.formats || {})
                          .filter(function (f) { return c.formats[f]; })
                          .map(function (f) {
    ...
    return '<tr><td><a href="/admin/applications?application=' +
      ...
      '"><code>' + esc(row.identifier) + '</code></a>' + (row.name ? '<div ' +
          'class="sub">' + esc(row.name) +
      '</div>' : '') + '</td><td>' + esc(row.role) + (row.webApplication ?
                                                      '<div ' +
          'class="sub">web</div>' : '') +
      '</td><td class="num">' + row.grantsHeld.pending + ' / ' +
      row.grantsHeld.approved + ' ' +
          '/ ' +
      row.grantsHeld.finalized + '</td><td class="num">' + row.activeTokens +
      '</td><td ' +
      'class="num">' + c.grants + '</td><td class="num">' + c.approved +
      '</td><td ' +
          'class="num">' + c.denied +
      '</td><td class="num">' + c.tokens + '<div class="sub">' + (formats ||
          '—') + '</div></td><td ' +
      'class="num">' + (c.rotations + c.keyRotations) + '</td><td ' +
          'class="num">' +
      (c.tokenRevocations +
       c.revoked) + '</td><td class="num">' + c.proofFailures + '</td><td ' +
      'class="num">' + c.introspections + ' / ' + c.registrations + ' / ' +
      c.derivations + '</td><td>' + (errors || '<span ' +
          'class="sub">none</span>') + '</td><td>' + esc(
              row.lastAt || '—') + '<div ' +
              'class="sub">' + esc(row.lastEvent || '') + '</div></td></tr>';
  }).join('') : '<tr><td colspan="14" class="sub">No application in this ' +
                'realm uses GNAP yet.</td></tr>';
  const inner = (typeof admin.messagesOf === 'function' ?
                 admin.messagesOf(req) : '') +
    admin.note('<strong>What the applications using GNAP have done</strong>, ' +
               'counted since ' +
      esc(json.since) + ' in this trust realm. An application is listed when ' +
      'it is declared for GNAP, when it has spoken it, or both — so an entry ' +
    ...
    '<h2>Tokens by format</h2><table><thead><tr>' + Object.keys(
      ...
      return '<th>' + esc(f) + '</th>';
    }).join('') + '</tr></thead><tbody><tr>' + formatRow + '</tr></tbody></table><h2 ' +
    'id="list-page">Applications</h2>' + nav.head +
    '<table><thead><tr><th>Application</th><th>Role</th><th>Grants held<div ' +
    'class="sub">pending / approved / finalized</div></th><th>Live ' +
    'tokens</th><th>Requested</th><th>Approved</th><th>Denied</th><th>Tokens ' +
    ...
    'activity</th></tr></thead><tbody>' + rows + '</tbody></table>' + nav.foot +
    admin.note('There is no reset. The counters start with the process and ' +
    ...
    '<code>GET /admin-api/gnap/monitor</code> · <a href="/admin/gnap">GNAP ' +
    'settings</a></p>';
