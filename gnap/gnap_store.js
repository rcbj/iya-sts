...
const STATE = { PROCESSING: 'processing', PENDING: 'pending', APPROVED: 'approved',
                FINALIZED: 'finalized' };

function digest(value) {
  return nodeCrypto.createHash('sha256').update(String(value), 'utf8').digest('base64url');
}
...
function saveGrant(grant, note) {
  ...
  if (note) {
    grant.history = (grant.history || []).concat([{ at: grant.updatedAt, state: grant.state,
...
function issueContinuation(grant) {
  ...
  continuations.set(grant.continuationHash, { grantId: grant.id, issuedAt: nowSec() });
...
function putInstance(instanceId, fields) {
  instances.set(digest(instanceId), Object.assign({ createdAt: nowSec() }, fields));
}
...
function putUserRef(reference, fields) {
  userRefs.set(digest(reference), Object.assign({ createdAt: nowSec() }, fields));
}
...
function putResource(reference, fields) {
  resources.set(reference, Object.assign({ reference: reference, createdAt: nowSec() }, fields));
...
function resourceByCanonical(canonical, rsIdentity) {
  ...
  resources.forEach(function (row) {
    if (!found && row.canonical === canonical && row.rsIdentity === rsIdentity) {
      found = row;
    }
  });
  return found;
...
function prune() {
  ...
  grants.forEach(function (grant, id) {
    ...
    const finalizedLongAgo = grant.state === STATE.FINALIZED && grant.updatedAt < now - 86400;
    const pendingExpired = grant.state !== STATE.APPROVED && grant.state !== STATE.FINALIZED &&
