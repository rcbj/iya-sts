...
//
// **WHY THIS IS A WALK AND NOT A ZOD SCHEMA.** Every other surface here declares
// its input with `validation.types`. That works for flat forms and queries, and
// `checkParsed()` refuses nested values on purpose. A GNAP request is nested
// four deep with members that are "an object OR an array of objects" and "a
// string OR an object" (section 10.3's registry registers `access_token`,
// `client` and `user` twice each, once per type), and the refusal a client
// needs is a GNAP error code naming WHICH member, not a flattened zod path. So
// the document is first passed through `validation.checkDocument()` — the
// shared bound on depth, key count and prototype-polluting names, which is the
// part that must be the same everywhere — then through its JSON SCHEMA
// (`gnap_schemas.js`, ajv: types, lengths, caps, URI formats, no control
// characters), and only then walked here for the semantics.
//
...
// own vocabulary (`issuer_subject_id`, `decentralized_identifier`) where RFC 9493
// and GNAP section 2.2 say `iss_sub` and `did`. Aliasing between the two would
// put a translation on the path of every GNAP subject, and a client that sent
// `issuer_subject_id` to a GNAP AS would be accepted here and refused by every
// other GNAP implementation. So the grammar is written again, in GNAP's words,
// the way `ssf_subjects.js`'s own header argues a grammar should be.
// ---------------------------------------------------------------------------
...
function checkAccess(access, where) {
  ...
  if (!Array.isArray(access)) {
    ...
    return refusal('STS-GNAP-0020', '"' + where + '" must be an array of access rights ' +
                   '(RFC 9635 section 8).');
  }
  for (let i = 0; i < access.length; i++) {
    ...
    if (typeof right === 'string') {
      if (!right) {
        ...
        return refusal('STS-GNAP-0021', 'an access reference in "' + where + '" is empty.');
      }
      continue;
    }
    if (!isObject(right) || typeof right.type !== 'string' || !right.type) {
      log.debug("Leaving checkAccess(). Element " + i + " is not a typed object or string.");
      return refusal('STS-GNAP-0022', 'element ' + i + ' of "' + where + '" must be a ' +
                     'reference string or an object with a string "type" (RFC 9635 section 8).');
    }
    const arrays = ['actions', 'locations', 'datatypes', 'privileges'];
    for (let j = 0; j < arrays.length; j++) {
      if (right[arrays[j]] !== undefined && !isStringArray(right[arrays[j]])) {
        log.debug("Leaving checkAccess(). " + arrays[j] + " is not an array of strings.");
        return refusal('STS-GNAP-0023', '"' + arrays[j] + '" in element ' + i + ' of "' +
    ...
    if (right.identifier !== undefined && typeof right.identifier !== 'string') {
      ...
      return refusal('STS-GNAP-0023', '"identifier" in element ' + i + ' of "' + where +
                     '" must be a string.');
    }
  }
...
function checkTokenRequest(one, index, multiple) {
  ...
  if (!isObject(one)) {
    ...
    return refusal('STS-GNAP-0024', '"' + where + '" must be an object (RFC 9635 section 2.1.1).');
  }
  ...
  if (one.label !== undefined && (typeof one.label !== 'string' || !one.label)) {
    log.debug("Leaving checkTokenRequest(). Bad label.");
    return refusal('STS-GNAP-0025', '"' + where + '.label" must be a non-empty string.');
  }
  if (multiple && typeof one.label !== 'string') {
    ...
    return refusal('STS-GNAP-0026', 'every access token in a request for multiple tokens ' +
                   'must carry a label (RFC 9635 section 2.1.2).');
  }
  ...
  if (!isStringArray(flags)) {
    ...
    return refusal('STS-GNAP-0027', '"' + where + '.flags" must be an array of strings.',
                   'invalid_flag');
  }
  const seen = {};
  for (let i = 0; i < flags.length; i++) {
    if (seen[flags[i]]) {
      ...
      return refusal('STS-GNAP-0028', 'the flag "' + flags[i] + '" appears more than once ' +
                     '(RFC 9635 section 2.1.1 requires invalid_flag).', 'invalid_flag');
    }
    seen[flags[i]] = true;
    if (RESPONSE_ONLY_FLAGS.indexOf(flags[i]) >= 0) {
      log.debug("Leaving checkTokenRequest(). A response-only flag was requested.");
      return refusal('STS-GNAP-0029', 'the flag "' + flags[i] + '" is a response flag and ' +
                     'cannot be requested (RFC 9635 section 10.4).', 'invalid_flag');
    }
    if (REQUEST_FLAGS.indexOf(flags[i]) < 0) {
      log.debug("Leaving checkTokenRequest(). Unknown flag.");
      return refusal('STS-GNAP-0030', 'the flag "' + flags[i] + '" is not one this ' +
                     'authorization server understands.', 'invalid_flag');
    }
  }
...
function checkAccessTokenMember(value) {
  ...
  if (Array.isArray(value)) {
    if (!value.length) {
      ...
      return refusal('STS-GNAP-0031', '"access_token" must not be an empty array.');
    }
    const tokens = [];
    const labels = {};
    for (let i = 0; i < value.length; i++) {
      ...
      if (!one.ok) {
        log.debug("Leaving checkAccessTokenMember(). Element " + i + " refused.");
        return one;
      }
      if (labels[one.token.label]) {
        log.debug("Leaving checkAccessTokenMember(). Duplicate label.");
        return refusal('STS-GNAP-0032', 'the label "' + one.token.label + '" is used by more ' +
                       'than one requested access token (RFC 9635 section 2.1.2).');
      }
      labels[one.token.label] = true;
      tokens.push(one.token);
    }
    log.debug("Leaving checkAccessTokenMember(). " + tokens.length + " tokens.");
...
function checkSubId(subject, where, nested) {
  ...
  if (!isObject(subject) || typeof subject.format !== 'string') {
    ...
    return refusal('STS-GNAP-0033', '"' + where + '" must be a Subject Identifier object with ' +
                   'a "format" (RFC 9493).');
  }
  const members = SUB_ID_FORMATS[subject.format];
  if (!members) {
    log.debug("Leaving checkSubId(). Unknown format.");
    return refusal('STS-GNAP-0034', '"' + where + '" uses the Subject Identifier format "' +
  ...
  const allowed = subject.format === 'aliases' ? ['format', 'identifiers'] : ['format'].concat(members);
  ...
  if (extra.length) {
    ...
    return refusal('STS-GNAP-0036', 'a "' + subject.format + '" Subject Identifier carries "' + extra[0] +
  ...
  if (subject.format === 'aliases') {
    if (nested) {
      ...
      return refusal('STS-GNAP-0035', 'an "aliases" identifier must not contain another ' +
                     '"aliases" identifier (RFC 9493 section 3.2.8).');
    }
    if (!Array.isArray(subject.identifiers) || !subject.identifiers.length) {
      log.debug("Leaving checkSubId(). aliases without identifiers.");
      return refusal('STS-GNAP-0036', '"' + where + '.identifiers" must be a non-empty array.');
    }
    for (let i = 0; i < subject.identifiers.length; i++) {
      const inner = checkSubId(subject.identifiers[i], where + '.identifiers[' + i + ']', true);
      if (!inner.ok) {
  ...
  for (let i = 0; i < members.length; i++) {
    if (typeof subject[members[i]] !== 'string' || !subject[members[i]]) {
      ...
      return refusal('STS-GNAP-0036', 'a "' + subject.format + '" Subject Identifier must ' +
                     'carry a string "' + members[i] + '" (RFC 9493).');
    }
  }
  if (subject.format === 'account' && subject.uri.indexOf('acct:') !== 0) {
    log.debug("Leaving checkSubId(). account without acct: URI.");
    return refusal('STS-GNAP-0036', 'an "account" Subject Identifier\'s uri must use the ' +
                   'acct scheme (RFC 9493 section 3.2.1).');
  }
  if (subject.format === 'phone_number' && !/^\+[1-9][0-9]{1,14}$/.test(subject.phone_number)) {
    log.debug("Leaving checkSubId(). phone_number is not E.164.");
    return refusal('STS-GNAP-0036', 'a "phone_number" Subject Identifier must be E.164 ' +
                   '(RFC 9493 section 3.2.5).');
  }
...
function checkSubIds(value, where) {
  if (!Array.isArray(value)) {
    return refusal('STS-GNAP-0037', '"' + where + '" must be an array of Subject Identifiers.');
  }
  for (let i = 0; i < value.length; i++) {
...
function checkSubjectMember(value) {
  ...
  if (!isObject(value)) {
    ...
    return refusal('STS-GNAP-0038', '"subject" must be an object (RFC 9635 section 2.2).');
  }
  ...
  if (value.sub_id_formats !== undefined) {
    if (!isStringArray(value.sub_id_formats)) {
      ...
      return refusal('STS-GNAP-0038', '"subject.sub_id_formats" must be an array of strings.');
    }
  ...
  if (value.assertion_formats !== undefined) {
    if (!isStringArray(value.assertion_formats)) {
      ...
      return refusal('STS-GNAP-0038', '"subject.assertion_formats" must be an array of strings.');
    }
...
function checkClientMember(value) {
  ...
  if (typeof value === 'string') {
    if (!value) {
      ...
      return refusal('STS-GNAP-0039', '"client" must not be an empty string.', 'invalid_client');
    }
    log.debug("Leaving checkClientMember(). By reference.");
    return { ok: true, client: { reference: value, key: null, classId: null, display: null } };
  }
  if (!isObject(value)) {
    log.debug("Leaving checkClientMember(). Absent or not an object.");
    return refusal('STS-GNAP-0039', '"client" is REQUIRED: an object or an instance ' +
                   'identifier (RFC 9635 section 2.3).', 'invalid_client');
  }
  if (value.key === undefined || value.key === null) {
    log.debug("Leaving checkClientMember(). No key.");
    return refusal('STS-GNAP-0040', '"client.key" is REQUIRED (RFC 9635 section 2.3).',
                   'invalid_client');
  }
  ...
  if (value.display !== undefined) {
    ...
    if (value.display.name !== undefined && typeof value.display.name !== 'string') {
      log.debug("Leaving checkClientMember(). display.name is not a string.");
      return refusal('STS-GNAP-0041', '"client.display.name" must be a string.');
    }
    ...
    const logoProblem = value.display.logo_uri === undefined ? ''
      : absoluteUriProblem(value.display.logo_uri, 'client.display.logo_uri', true);
    if (uriProblem || logoProblem) {
      log.debug("Leaving checkClientMember(). A display URI is not absolute.");
      return refusal('STS-GNAP-0041', (uriProblem || logoProblem) + ' (RFC 9635 section 2.3.2).');
    }
    display = { name: value.display.name || null, uri: value.display.uri || null,
  ...
  return { ok: true, client: { reference: null, key: value.key,
                               classId: value.class_id === undefined ? null : value.class_id,
                               display: display } };
}
...
function checkUserMember(value) {
  ...
  if (typeof value === 'string') {
    if (!value) {
      ...
      return refusal('STS-GNAP-0042', '"user" must not be an empty string.', 'unknown_user');
    }
    log.debug("Leaving checkUserMember(). By reference.");
    return { ok: true, user: { reference: value, subIds: null, assertions: null } };
  }
  if (!isObject(value)) {
    log.debug("Leaving checkUserMember(). Not an object.");
    return refusal('STS-GNAP-0042', '"user" must be an object or a reference string ' +
  ...
  if (value.assertions !== undefined) {
    if (!Array.isArray(value.assertions) || !value.assertions.every(function (one) {
      return isObject(one) && typeof one.format === 'string' && typeof one.value === 'string';
    })) {
      log.debug("Leaving checkUserMember(). assertions malformed.");
      return refusal('STS-GNAP-0043', '"user.assertions" must be an array of objects with ' +
                     'string "format" and "value" (RFC 9635 sections 2.4 and 3.4).');
    }
...
function checkInteractMember(value) {
  ...
  if (!isObject(value)) {
    ...
    return refusal('STS-GNAP-0044', '"interact" must be an object (RFC 9635 section 2.5).');
  }
  if (!Array.isArray(value.start)) {
    log.debug("Leaving checkInteractMember(). start missing.");
    return refusal('STS-GNAP-0045', '"interact.start" is REQUIRED and must be an array ' +
                   '(RFC 9635 section 2.5).');
  }
  const start = [];
  for (let i = 0; i < value.start.length; i++) {
    const one = value.start[i];
    const mode = typeof one === 'string' ? one : (isObject(one) ? one.mode : undefined);
    if (typeof mode !== 'string' || !mode) {
      log.debug("Leaving checkInteractMember(). A start mode has no name.");
      return refusal('STS-GNAP-0046', 'element ' + i + ' of "interact.start" must be a mode ' +
                     'name or an object with a "mode" (RFC 9635 section 2.5.1).');
    }
  ...
  if (value.finish !== undefined) {
    ...
    if (!isObject(f) || typeof f.method !== 'string' || !f.method) {
      ...
      return refusal('STS-GNAP-0047', '"interact.finish.method" is REQUIRED (RFC 9635 ' +
                     'section 2.5.2).');
    }
    if (typeof f.nonce !== 'string' || !f.nonce || !/^[\x21-\x7e]+$/.test(f.nonce)) {
      log.debug("Leaving checkInteractMember(). finish nonce missing or not ASCII.");
      return refusal('STS-GNAP-0048', '"interact.finish.nonce" is REQUIRED and must be an ' +
                     'ASCII string (RFC 9635 section 2.5.2).');
    }
    if (f.method === 'redirect' || f.method === 'push') {
      const problem = absoluteUriProblem(f.uri, 'interact.finish.uri', false);
      if (problem) {
        log.debug("Leaving checkInteractMember(). finish URI refused: " + problem);
    ...
    if (typeof hashMethod !== 'string' || !HASH_METHODS[hashMethod]) {
      ...
      return refusal('STS-GNAP-0050', 'the hash_method "' + hashMethod + '" is not one this ' +
                     'authorization server computes (' + Object.keys(HASH_METHODS).join(', ') +
                     ').');
    }
  ...
  if (value.hints !== undefined) {
    if (!isObject(value.hints) ||
        (value.hints.ui_locales !== undefined && !isStringArray(value.hints.ui_locales))) {
      log.debug("Leaving checkInteractMember(). hints malformed.");
      return refusal('STS-GNAP-0051', '"interact.hints" must be an object whose "ui_locales" is ' +
                     'an array of strings (RFC 9635 section 2.5.3).');
    }
...
function checkEnvelope(body, where, schema, gnapError) {
  ...
  if (!isObject(body)) {
    ...
    return refusal('STS-GNAP-0052', 'the ' + where + ' must be a JSON object.', gnapError);
  }
  const bounded = validation.checkDocument(body, where, { maxDepth: 12, maxKeys: 2000 });
  if (!bounded.ok) {
  ...
  if (schema) {
    ...
    if (!shaped.ok) {
      ...
      return refusal('STS-GNAP-0061', 'the ' + where + ' does not match its schema: ' +
...
function parseGrantRequest(body) {
  ...
  if (body.interact_ref !== undefined) {
    ...
    return refusal('STS-GNAP-0054', '"interact_ref" is only sent to a continuation URI ' +
                   '(RFC 9635 section 5.1).');
  }
  const out = { tokens: [], multiple: false, subject: null, client: null, user: null,
  ...
  if (body.access_token === undefined && body.subject === undefined) {
    ...
    return refusal('STS-GNAP-0055', 'a grant request must ask for something: "access_token" ' +
                   '(section 2.1), "subject" (section 2.2), or both.');
  }
  ...
  if (body.existing_access_token !== undefined) {
    // RFC 9767 section 4.
    if (typeof body.existing_access_token !== 'string' || !body.existing_access_token) {
      log.debug("Leaving parseGrantRequest(). existing_access_token malformed.");
      return refusal('STS-GNAP-0056', '"existing_access_token" must be a non-empty string ' +
                     '(RFC 9767 section 4).');
    }
    out.existingAccessToken = body.existing_access_token;
  }
  log.debug("Leaving parseGrantRequest(). " + out.tokens.length + " token request(s), subject=" +
...
function parseContinuation(body, hadContent) {
  ...
  if (unexpected.length) {
    ...
    return refusal('STS-GNAP-0057', 'a continuation POST carries only "interact_ref"; "' +
                   unexpected.join('", "') + '" belong to a PATCH (RFC 9635 section 5.3).');
  }
  if (body.interact_ref !== undefined &&
      (typeof body.interact_ref !== 'string' || !/^[A-Za-z0-9._~-]+$/.test(body.interact_ref))) {
    log.debug("Leaving parseContinuation(). interact_ref malformed.");
    return refusal('STS-GNAP-0058', '"interact_ref" must be a string of unreserved characters ' +
                   '(RFC 9635 section 4.2).', 'invalid_interaction');
  }
  log.debug("Leaving parseContinuation().");
  return { ok: true, interactRef: body.interact_ref === undefined ? null : body.interact_ref };
}
...
function parseModification(body) {
  ...
  if (body.client !== undefined) {
    ...
    return refusal('STS-GNAP-0059', 'a modification MUST NOT include "client" (RFC 9635 ' +
                   'section 5.3).');
  }
  if (body.interact_ref !== undefined) {
    log.debug("Leaving parseModification(). interact_ref present.");
    return refusal('STS-GNAP-0059', 'a modification MUST NOT include "interact_ref" ' +
                   '(RFC 9635 section 5.3).');
  }
  const out = { tokens: null, multiple: false, subject: null, user: null, interact: null };
  if (body.access_token !== undefined) {
    const tokens = checkAccessTokenMember(body.access_token);
    if (!tokens.ok) {
...
function parseRotation(body, hadContent) {
  ...
  const envelope = checkEnvelope(body, 'rotation request', 'rotation', 'invalid_rotation');
  if (!envelope.ok) {
  ...
  if (unexpected.length || body.key === undefined) {
    ...
    return refusal('STS-GNAP-0060', 'a rotation request carries no content, or only "key" ' +
                   '(RFC 9635 sections 6.1 and 6.1.1).', 'invalid_rotation');
  }
...
function checkResourceServer(value) {
  ...
  return refusal('STS-GNAP-0500', '"resource_server" is REQUIRED: an object with a "key" or ' +
                 'an instance identifier (RFC 9767 section 3.2).', 'invalid_resource_server');
}

// RFC 9767 section 3.3.
function parseIntrospection(body) {
  log.debug("Entering parseIntrospection().");
  const envelope = checkEnvelope(body, 'introspection request', 'introspection');
  if (!envelope.ok) {
  ...
  if (typeof body.access_token !== 'string' || !body.access_token) {
    ...
    return refusal('STS-GNAP-0501', '"access_token" is REQUIRED (RFC 9767 section 3.3).');
  }
  if (body.proof !== undefined && (typeof body.proof !== 'string' || !body.proof)) {
    log.debug("Leaving parseIntrospection(). proof malformed.");
    return refusal('STS-GNAP-0501', '"proof" must be a key proofing method name (RFC 9767 ' +
                   'section 3.3).');
  }
  ...
  return { ok: true, request: { accessToken: body.access_token, proof: body.proof || null,
                                resourceServer: rs.resourceServer, access: access } };
}

// RFC 9767 section 3.4.
function parseRegistration(body) {
  log.debug("Entering parseRegistration().");
  const envelope = checkEnvelope(body, 'resource registration request', 'registration');
  if (!envelope.ok) {
  ...
  if (!access.access.length) {
    ...
    return refusal('STS-GNAP-0502', '"access" must name at least one right to register ' +
                   '(RFC 9767 section 3.4).');
  }
  const rs = checkResourceServer(body.resource_server);
  if (!rs.ok) {
  ...
  if (body.token_formats_supported !== undefined && !isStringArray(body.token_formats_supported)) {
    log.debug("Leaving parseRegistration(). token_formats_supported malformed.");
    return refusal('STS-GNAP-0502', '"token_formats_supported" must be an array of strings.');
  }
  if (body.token_introspection_required !== undefined &&
      typeof body.token_introspection_required !== 'boolean') {
    log.debug("Leaving parseRegistration(). token_introspection_required malformed.");
    return refusal('STS-GNAP-0502', '"token_introspection_required" must be a boolean.');
  }
