...
// `authenticate`, `presentation` and `liveProblem` as FACADES forwarding to
// the instance the composition root builds (#50, R2), for `gnap.ts`,
// `ssf/ssf_auth.ts` and `ssf/ssf_cluster.ts`, which require it by those names.
// `tests/cluster_signout_signals.js` replaces the whole module in
// `require.cache`, which is unaffected. A process that loads this module
...
// ---------------------------------------------------------------------------
const slot = new InstanceSlot<GnapRs>(
  'gnap/gnap_rs',
  () => new GnapRs(GnapRs.defaultDeps()),
  null,
  helpers.log);
...
export = {
  ...
  presentation: slot.forward('presentation'),
  liveProblem: slot.forward('liveProblem')
};
