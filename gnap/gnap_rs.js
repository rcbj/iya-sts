...
async function introspect(req) {
  ...
  if (!body.ok) {
    log.debug("Leaving introspect(). Body refused.");
    return body;
  }
  const parsed = request.parseIntrospection(body.json);
  if (!parsed.ok) {
  ...
  const record = store.tokenByValue(parsed.request.accessToken);
  ...
  const answer = { active: true, access: record.access, iss: record.iss };
  ...
  log.debug("Leaving introspect(). Active.");
...
async function register(req) {
  ...
  const body = proof.readBody(req);
  if (!body.ok) {
    log.debug("Leaving register(). Body refused.");
    return body;
  }
  const parsed = request.parseRegistration(body.json);
  if (!parsed.ok) {
    log.debug("Leaving register(). Request refused.");
    return parsed;
  }
  ...
  log.debug("Leaving register(). reference=" + row.reference);
...
// (gnap_tokens.js's header). The key is DERIVED, so it never has to be stored
// to be correct — but an RS has to be able to READ it, and the place an RS
// operator reads a credential for their application is that application's
// entry (the console's drill-down, `/admin-api/applications`). So it is written
// there once, as `gnapMacaroonKey`: a credential attribute, sealed with the
// process key-encryption key when keys persist (the user's choice, 2026-09-12)
// and withheld from LDAP readers in product mode.
// ---------------------------------------------------------------------------
function ensureMacaroonKey(rs) {
  log.debug("Entering ensureMacaroonKey().");
  if (grants.field(rs, 'gnapMacaroonKey')) {
    log.debug("Leaving ensureMacaroonKey(). Present.");
    return;
  }
  ...
  // `updateApplication()` is the door that keeps derived attributes out of an
...
// record of, the record IS the answer: the format check re-derives facts the
// store already holds.
// ---------------------------------------------------------------------------
function presentation(req) {
  ...
  // header field method defined in [RFC6750]", and a bound token with the GNAP
...
async function authenticate(req, options) {
  ...
  const opts = options || {};
  const presented = presentation(req);
  if (!presented.ok) {
