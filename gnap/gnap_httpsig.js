...
//   * `;sf` and `;key=` re-serialize through `gnap/gnap_sf.js` (2.1.1, 2.1.2)
...
// the error-code registry and `gnap_sf.js`, none of which requires it back.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// THE ONE PLACE A REFUSAL IS MADE. Logged at warn with the code at the front of
// the line — a failed proof is an operator's question before it is anything
...
function bodyBytes(body) {
  log.debug("Entering bodyBytes().");
  if (body === undefined || body === null) {
    log.debug("Leaving bodyBytes(). No content.");
    return Buffer.alloc(0);
  }
  if (Buffer.isBuffer(body)) {
    log.debug("Leaving bodyBytes(). Buffer.");
    return body;
  }
  if (body instanceof Uint8Array) {
    log.debug("Leaving bodyBytes(). Uint8Array.");
    return Buffer.from(body);
  }
  log.debug("Leaving bodyBytes(). String.");
  return Buffer.from(String(body), 'utf8');
}

// `algorithm` may be one name or an array of names, in which case every member
// is computed — section 2's second example, and what a client supporting a
// population of verifiers sends. It THROWS on an unknown name, carrying the
// code: an unsupported algorithm here is the caller's own configuration, not
...
// Section 2 read with GNAP's requirement (RFC 9635 section 7.3.1: "The verifier
// MUST validate this field value"): EVERY member whose algorithm is accepted
// must match, and at least one must be present. "Any one matches" would let a
// client send a correct sha-256 beside a wrong sha-512 and have a verifier that
// prefers sha-512 accept a body the sha-512 does not describe.
function verifyContentDigest(headerValue, body, options) {
  ...
  if (matched.length === 0) {
    log.debug("Leaving verifyContentDigest(). No accepted algorithm.");
...
// A JavaScript value handed in as a parameter, turned into a bare item. A value
// that is already a bare item is kept, so a caller that needs a Token rather
// than a String can say so.
function bareItemOf(value) {
  log.debug("Entering bareItemOf().");
  let bare = null;
  ...
  log.debug("Leaving bareItemOf(). " + (bare ? bare.type : 'not a bare item'));
  return bare;
}

// Parameters from either an ordered [[key, value]] array or a plain object (in
// its insertion order). Duplicate keys are last-wins in the first position,
// which is what a PARSER would have made of the same text (RFC 8941 section
// 4.2.3.2) — so a signer cannot build an identifier no verifier could parse
// back to itself. THROWS on a value that is not a bare item; callers map it.
function paramsFrom(input) {
  log.debug("Entering paramsFrom().");
  ...
  log.debug("Leaving paramsFrom(). " + out.length + " parameter(s).");
  return out;
}
...
function componentItem(component) {
  log.debug("Entering componentItem().");
  let item;
  try {
...
// Two identifiers are the same when the names are equal and the parameters are
...
function fieldLines(message, name) {
  log.debug("Entering fieldLines(). " + name);
  const headers = message && message.headers;
  if (!headers || typeof headers !== 'object') {
    log.debug("Leaving fieldLines(). No headers.");
    return null;
  }
  let raw;
  ...
  if (raw === undefined || raw === null) {
    log.debug("Leaving fieldLines(). Absent.");
    return null;
  }
  const lines = (Array.isArray(raw) ? raw : [raw]).map(String);
  log.debug("Leaving fieldLines(). " + lines.length + " line(s).");
...
// 2.2.6 and 2.2.7 want the path and query "before decoding any percent-encoded
// octets" and a WHATWG URL object re-encodes and resolves dot segments. The URL
// parser is used for what it is right about — that the string is absolute, and
// the host normalization of 2.2.3.
function targetParts(message) {
  log.debug("Entering targetParts().");
  const raw = message && message.targetUri;
  if (typeof raw !== 'string') {
    log.debug("Leaving targetParts(). No target URI.");
    return null;
  }
  const m = /^([A-Za-z][A-Za-z0-9+.-]*):\/\/([^/?#]*)([^?#]*)(?:\?([^#]*))?(?:#.*)?$/.exec(raw);
  if (!m) {
    log.debug("Leaving targetParts(). Not absolute.");
    return null;
  }
  let url;
  try {
    ...
    log.debug("Leaving targetParts(). Unparseable: " + e.message);
    return null;
  }
...
// into `with%20plus%20whitespace`. That set leaves exactly ASCII alphanumerics
// and `*-._` unencoded.
// ---------------------------------------------------------------------------
function percentDecode(text) {
  log.debug("Entering percentDecode().");
  ...
  log.debug("Leaving percentDecode().");
  return Buffer.from(out).toString('utf8');
}

function urlencodedEncode(text) {
  log.debug("Entering urlencodedEncode().");
  const bytes = Buffer.from(text, 'utf8');
  let out = '';
  for (let k = 0; k < bytes.length; k++) {
  ...
  log.debug("Leaving urlencodedEncode().");
  return out;
}
...
function queryParams(query) {
  ...
  const out = [];
  (query || '').split('&').forEach(function (sequence) {
    if (sequence === '') {
      return;
    }
    ...
    out.push([urlencodedEncode(percentDecode(name.replace(/\+/g, ' '))),
              urlencodedEncode(percentDecode(value.replace(/\+/g, ' ')))]);
  });
  log.debug("Leaving queryParams(). " + out.length + " parameter(s).");
  return out;
}
...
function checkParams(name, params, allowed) {
  log.debug("Entering checkParams(). " + name);
...
function derivedValue(message, name, params) {
  log.debug("Entering derivedValue(). " + name);
  ...
  log.debug("Leaving derivedValue(). " + name);
  return { ok: true, value: value };
}

function fieldValue(message, name, params, options) {
  log.debug("Entering fieldValue(). " + name);
  const problem = checkParams(name, params, ['sf', 'key', 'bs', 'req', 'tr']);
  if (problem) {
    log.debug("Leaving fieldValue(). Parameters.");
    return problem;
  }
  ...
  if (bs) {
    log.debug("Leaving fieldValue(). bs.");
    return {
      ok: true,
...
function componentValue(message, component, options) {
  log.debug("Entering componentValue().");
  const normalized = componentItem(component);
  if (isRefusal(normalized)) {
    log.debug("Leaving componentValue(). Identifier.");
    return normalized;
  }
...
function checkSignatureParams(params) {
  log.debug("Entering checkSignatureParams().");
  for (let k = 0; k < params.length; k++) {
...
function signatureBase(message, covered, signatureParams, options) {
  log.debug("Entering signatureBase().");
  let components;
  let paramsInput = signatureParams;
  if (covered && covered.type === 'innerList' && Array.isArray(covered.value)) {
    ...
    log.debug("Leaving signatureBase(). No component list.");
    return refuse('STS-GNAP-0206',
                  'The covered components must be an array or an Inner List.');
  }
  let params;
  try {
    params = paramsFrom(paramsInput);
  } catch (e) {
  ...
  if (paramProblem) {
    log.debug("Leaving signatureBase(). Parameter types.");
    return paramProblem;
  }
  const items = [];
  const lines = [];
  ...
  for (let k = 0; k < components.length; k++) {
    const normalized = componentItem(components[k]);
    if (isRefusal(normalized)) {
      log.debug("Leaving signatureBase(). Identifier.");
      return normalized;
    }
    const identity = identityOf(normalized.item);
    if (seen[identity]) {
    ...
    if (isRefusal(cv)) {
      log.debug("Leaving signatureBase(). Component refused.");
      return cv;
    }
...
function checkKey(name, entry, key, purpose) {
  log.debug("Entering checkKey(). " + name + " " + purpose);
  if (entry.kind === 'hmac') {
    let length = -1;
...
function rawSign(entry, key, data) {
  log.debug("Entering rawSign(). " + entry.kind);
  let out;
  switch (entry.kind) {
    case 'hmac':
  ...
  log.debug("Leaving rawSign().");
  return out;
}

// HTTP_VERIFY, section 3.3. The HMAC comparison is constant-time and a length
// difference is a plain false: `timingSafeEqual` throws on unequal lengths, and
// an exception there would be a different code path an attacker can time.
function rawVerify(entry, key, data, signature) {
  log.debug("Entering rawVerify(). " + entry.kind);
  let ok;
  switch (entry.kind) {
    case 'hmac': {
  ...
  log.debug("Leaving rawVerify(). " + ok);
  return ok;
}
...
//
// `algorithm` may be an HTTP registry name or a JWS name; when it is absent the
// `alg` parameter names it. It is never INVENTED into the parameters: whether
// the signature carries `alg` is the signer's decision (GNAP forbids it), so
// the parameters are exactly what the caller passed, in the caller's order.
function sign(message, options) {
  ...
  if (keyProblem) {
    log.debug("Leaving sign(). Key.");
    return keyProblem;
  }
  const built = signatureBase(message, opts.components || [], params, opts);
  if (isRefusal(built)) {
    log.debug("Leaving sign(). Base.");
    return built;
  }
  let signatureBytes;
  try {
    signatureBytes = rawSign(entry, opts.key, Buffer.from(built.base, 'ascii'));
  } catch (e) {
...
// Signature-Input and Signature fields — the shape of RFC 9635 section 7.3.1.1,
// where the key-rotation signature is added beside the old key's and covers it.
//
// It APPENDS TEXT rather than re-serializing what was there. A later signature
// may cover the whole `signature-input` field without `;key`, and that covers
// its bytes as sent; re-serializing the existing members would change those
// bytes under a signature that has already been made. The existing value is
// still PARSED first, because appending to a malformed field makes one nobody
// can read, and a label already present in either field is refused — section 4
// says a label MUST be unique, and a second member under it would replace the
...
function appendSignature(message, result) {
  ...
  fields.forEach(function (field) {
    const headerKey = headerKeyFor(headers, field[0]);
...
function parseSignatureField(message, name) {
  log.debug("Entering parseSignatureField(). " + name);
  const lines = fieldLines(message, name);
  if (!lines) {
    log.debug("Leaving parseSignatureField(). Absent.");
    return { ok: true, dictionary: [] };
  }
  const duplicates = [];
  let dictionary = [];
  try {
    // Line by line as well as combined: a label repeated on two field lines is
...
function parseSignatures(message) {
  log.debug("Entering parseSignatures().");
  const inputs = parseSignatureField(message, 'signature-input');
  if (isRefusal(inputs)) {
    log.debug("Leaving parseSignatures(). Signature-Input.");
    return inputs;
  }
  const values = parseSignatureField(message, 'signature');
  if (isRefusal(values)) {
    log.debug("Leaving parseSignatures(). Signature.");
    return values;
  }
  ...
  values.dictionary.forEach(function (pair) { labels[pair[0]] = true; });
  const out = [];
...
//   skewS             how far in the FUTURE created may be; defaults to maxAgeS
//   requireCreated    refuse a signature with no created (implied by maxAgeS)
//   requireComponents components that MUST be covered
...
function verify(message, options) {
  ...
  const parsed = parseSignatures(message);
  if (isRefusal(parsed)) {
    log.debug("Leaving verify(). Parse.");
    return parsed;
  }
  ...
  for (let k = 0; k < candidates.length; k++) {
    const result = verifyOne(message, candidates[k], opts, now);
    if (result.ok) {
      verified.push(result);
    } else if (opts.require !== 'any') {
      log.debug("Leaving verify(). " + candidates[k].label + " refused.");
      return result;
  ...
  if (verified.length === 0) {
    log.debug("Leaving verify(). None verified.");
    return firstFailure;
  }
  log.debug("Leaving verify(). " + verified.length + " verified.");
  return { ok: true, verified: verified };
}

function verifyOne(message, parsed, opts, now) {
  log.debug("Entering verifyOne(). " + parsed.label);
  const p = parsed.params;
  const paramProblem = checkSignatureParams(parsed.paramList);
  if (paramProblem) {
    log.debug("Leaving verifyOne(). Parameter types.");
    return paramProblem;
  }
  ...
  for (let k = 0; k < required.length; k++) {
    ...
    if (isRefusal(wanted)) {
      log.debug("Leaving verifyOne(). Required component malformed.");
      return wanted;
    }
  ...
  if (fromParam !== undefined) {
    const paramEntry = algorithmNamed(fromParam);
...
module.exports = {
  ...
  DIGEST_ALGORITHMS: Object.keys(DIGEST_ALGORITHMS),
  KNOWN_FIELD_TYPES: KNOWN_FIELD_TYPES,
