...
// key holds the DESCRIPTOR this module builds rather than the JSON it came from.
//
...
//     stores this subsystem writes are JSON rows (persistence/CLAUDE.md). What is
//     stored is the key object the client sent, and `describe()` rebuilds the
//     descriptor from it on every read — which also means a descriptor can never
//     be stale against the row it was built from.
//
// ---------------------------------------------------------------------------
...
function normaliseProof(proof) {
  ...
  if (typeof proof === 'string') {
    if (PROOF_METHODS.indexOf(proof) < 0) {
      ...
      return refusal('STS-GNAP-0001', 'the key names the proofing method "' + proof +
  ...
  if (!proof || typeof proof !== 'object' || Array.isArray(proof)) {
    ...
    return refusal('STS-GNAP-0002', 'a key\'s "proof" must be a method name or an ' +
                   'object with a "method" member (RFC 9635 section 7.3).');
  }
  if (typeof proof.method !== 'string' || PROOF_METHODS.indexOf(proof.method) < 0) {
    log.debug("Leaving normaliseProof(). Object with no known method.");
    return refusal('STS-GNAP-0001', 'the key\'s proof object names no method this ' +
                   'authorization server implements (' + PROOF_METHODS.join(', ') + ').',
                   'invalid_client');
  }
  if (proof.method !== 'httpsig') {
    log.debug("Leaving normaliseProof(). Object form for a string-only method.");
    return refusal('STS-GNAP-0003', 'the "' + proof.method + '" proofing method is ' +
                   'defined in string form only (RFC 9635 section 10.16).');
  }
  if (typeof proof.alg !== 'string' || !proof.alg ||
      typeof proof['content-digest-alg'] !== 'string' || !proof['content-digest-alg']) {
    log.debug("Leaving normaliseProof(). httpsig object missing alg or content-digest-alg.");
    return refusal('STS-GNAP-0004', 'an httpsig proof in object form must carry both ' +
                   '"alg" and "content-digest-alg" (RFC 9635 section 7.3.1).');
  }
  log.debug("Leaving normaliseProof(). httpsig object form.");
  return { ok: true, method: 'httpsig',
           params: { alg: proof.alg, contentDigestAlg: proof['content-digest-alg'] } };
}
...
function certificateFrom(value) {
  ...
  try {
    const certificate = new nodeCrypto.X509Certificate(Buffer.from(text, 'base64'));
...
function describe(key, options) {
  ...
  if (typeof key === 'string') {
    if (!key) {
      ...
      return refusal('STS-GNAP-0005', 'a key reference must be a non-empty string.');
    }
    const resolved = typeof opts.resolveReference === 'function' ? opts.resolveReference(key) : null;
    if (!resolved) {
      log.debug("Leaving describe(). Unknown key reference.");
      return refusal('STS-GNAP-0006', 'the key reference is not one this authorization ' +
                     'server knows (RFC 9635 section 7.1.1).', 'invalid_client');
    }
    // A resolved reference is either a registered public key by value (which
    // is described like any other) or a SHARED SECRET — the one case section
    // 7.1.1 allows a symmetric key, precisely because it never crosses the wire.
    if (resolved.secret) {
      const proof = normaliseProof(resolved.proof || 'httpsig');
      if (!proof.ok) {
        log.debug("Leaving describe(). The registered reference names a bad proof.");
        return proof;
      }
    ...
    if (!inner.ok) {
      log.debug("Leaving describe(). The registered reference holds a bad key.");
      return inner;
    }
    inner.reference = key;
    inner.value = key;
  ...
  if (!key || typeof key !== 'object' || Array.isArray(key)) {
    ...
    return refusal('STS-GNAP-0005', 'a key must be an object (RFC 9635 section 7.1) or a ' +
                   'reference string (section 7.1.1).');
  }
  ...
  if (present.length !== 1) {
    ...
    return refusal('STS-GNAP-0007', 'a key by value must be presented in exactly one ' +
                   'format (jwk, cert or cert#S256); this one has ' + present.length +
  ...
  if (format === 'cert') {
    ...
    if (!certificate) {
      ...
      return refusal('STS-GNAP-0008', 'the key\'s "cert" member is not a PEM X.509 ' +
                     'certificate.', 'invalid_client');
    }
    const publicKey = certificate.publicKey;
    log.debug("Leaving describe(). Certificate.");
    return { ok: true, proof: proof, format: 'cert', jwk: null, certificate: certificate,
  ...
  if (!/^[A-Za-z0-9_-]{43}$/.test(thumbprint)) {
    log.debug("Leaving describe(). cert#S256 is not a SHA-256 base64url value.");
    return refusal('STS-GNAP-0009', 'the key\'s "cert#S256" member is not a base64url ' +
                   'SHA-256 thumbprint (RFC 8705).', 'invalid_client');
  }
  log.debug("Leaving describe(). Certificate thumbprint.");
  return { ok: true, proof: proof, format: 'cert#S256', jwk: null, certificate: null,
           thumbprint: thumbprint, identity: 'x5t:' + thumbprint, publicKey: null,
...
function describeJwk(key, proof) {
  ...
  if (!jwk || typeof jwk !== 'object' || Array.isArray(jwk)) {
    ...
    return refusal('STS-GNAP-0010', 'the key\'s "jwk" member must be a JSON Web Key ' +
                   'object.', 'invalid_client');
  }
  if (jwk.kty === 'oct' || PRIVATE_MEMBERS.some(function (m) { return jwk[m] !== undefined; })) {
    log.debug("Leaving describeJwk(). Private or symmetric member present.");
    return refusal('STS-GNAP-0011', 'a key by value must be a PUBLIC key; this JWK carries ' +
                   'private or symmetric key material (RFC 9635 sections 7.1 and 7.1.2).',
                   'invalid_client');
  }
  if (typeof jwk.alg !== 'string' || !jwk.alg || jwk.alg === 'none' ||
    ...
    return refusal('STS-GNAP-0012', 'a JWK presented in GNAP must carry "alg" (not "none") ' +
                   'and "kid" (RFC 9635 section 7.1).', 'invalid_client');
  }
  ...
  if (!spec || spec.family === 'hmac' || (spec.kty && spec.kty !== jwk.kty)) {
    ...
    return refusal('STS-GNAP-0013', 'the JWK\'s alg "' + jwk.alg + '" is not an asymmetric ' +
                   'JWS algorithm for a ' + jwk.kty + ' key.', 'invalid_client');
  }
  let publicKey;
  let thumbprint;
  try {
    ...
    return refusal('STS-GNAP-0014', 'the JWK does not import as a public key: ' +
                   e.message, 'invalid_client');
  }
  log.debug("Leaving describeJwk().");
  return { ok: true, proof: proof, format: 'jwk', jwk: jwk, certificate: null,
           thumbprint: thumbprint, identity: 'jkt:' + thumbprint, publicKey: publicKey,
