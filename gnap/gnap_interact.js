...
//
// **`notePresented(session, 'GNAP', req)` is called where an EXISTING session is
// honoured**, below every refusal, as the four browser SSO profiles do — that is
// CAEP's `session-presented` (tests/caep_presented_every_protocol.js reads the
// literal call).
//
// ---------------------------------------------------------------------------
...
//
// **UNLIKE `/oauth2/consent`, THE ANSWER IS CSRF-CHECKED.** The approval page is
// reached from a URI a client chose to send a browser to, which is exactly the
// setup a cross-site form post exploits: a page elsewhere that auto-submits
...
// a `data:` image, which `img-src 'self' data:` allows — an external logo URI is
// shown as a link, because fetching it would be the "client-hosted logo" section
// 11.16 warns about, and CSP would refuse it anyway.
// ---------------------------------------------------------------------------
...
const PAGE_CSS =
  '.card{width:520px}p.app{font-size:.9em;margin:0 0 14px}' +
  'ul.rights{list-style:none;padding:0;margin:0 0 8px}' +
  'ul.rights li{padding:8px 10px;margin:6px 0;border:1px solid #e3e3ea;border-radius:6px;' +
  'background:#fafafd;font-size:.85em}' +
  'ul.rights li label{display:flex;gap:8px;align-items:flex-start}' +
  'ul.rights li span{display:block;color:#666;font-size:.9em;margin-top:3px}' +
  'img.logo{max-width:48px;max-height:48px;float:right;margin:0 0 6px 8px}' +
  'input.code{font-size:1.4em;letter-spacing:.2em;text-transform:uppercase;width:100%}';

function sendPage(res, status, title, inner) {
  const html = '<!DOCTYPE html>\n<html lang="en"><head><meta charset="utf-8"><title>' +
    xmlEscape(title) + ' — GNAP authorization server</title><style>' + authn.CARD_CSS + PAGE_CSS +
    '</style></head><body><div class="card">' + inner + '</div></body></html>\n';
  res.status(status).type('text/html').set('Cache-Control', 'no-store').send(html);
}
...
function interactionError(res, code, heading, detail) {
  ...
  sendPage(res, 400, heading, '<h1>' + xmlEscape(heading) + '</h1><p class="err">' +
           xmlEscape(detail) + '</p><div class="meta"><div>Nothing was approved and nothing ' +
           'was sent to any application. Start again from the application that sent you here.' +
           '</div></div>');
}
...
function startMode(mode) {
  return function (req, res) {
    ...
    const params = validation.checkParsed({ id: req.params.id }, 'params', ID_PARAMS);
    const grant = params.ok ? store.grantByInteraction(mode + ':' + params.value.id) : null;
    if (!activeForInteraction(grant)) {
      ...
      return interactionError(res, 'STS-GNAP-0400', 'This link is not active',
        'The link does not belong to a request that is still waiting for approval — it may ' +
        'have expired, been used already, or been answered.');
    }
    ...
    if (!one || one.used || one.id !== params.value.id) {
      ...
      return interactionError(res, 'STS-GNAP-0401', 'This link has already been used',
...
function codePage(req, res, status, problem) {
  ...
  sendPage(res, status, 'Enter your code',
    '<h1>Enter the code</h1><p class="sub">The code your device or application is showing ' +
    'you.</p>' + (problem ? '<p class="err">' + xmlEscape(problem) + '</p>' : '') +
    '<form method="post" action="/gnap/code">' + websecurity.field(session ? session.id : '') +
    '<input class="code" name="code" autocomplete="one-time-code" autofocus required>' +
    '<div class="row"><button type="submit">Continue</button></div></form>' +
    '<div class="meta"><div>Spaces and dashes are ignored, and letters may be typed in either ' +
    'case (RFC 9635 section 4.1.2).</div></div>');
}
...
app.get('/gnap/code', function (req, res) {
  ...
  if (config.value('gnap.enabled') === false) {
    ...
    return interactionError(res, 'STS-GNAP-0402', 'GNAP is turned off',
                            'This trust realm does not run GNAP (gnap.enabled).');
  }
...
app.post('/gnap/code', function (req, res) {
  ...
  // Section 4.1.2: "If the AS detects too many unrecognized code enter attempts,
  // the interaction component SHOULD display an error". Counted per address,
  // because the code is what is being guessed and has no owner yet.
  ...
  if (!allowed.ok) {
    ...
    return codePage(req, res, 429, 'Too many codes have been tried from here. ' + allowed.detail);
  }
  ...
  const mode = grant && grant.interaction && grant.interaction.modes
    ? (grant.interaction.modes.user_code && grant.interaction.modes.user_code.code === code ? 'user_code'
      : (grant.interaction.modes.user_code_uri && grant.interaction.modes.user_code_uri.code === code
  ...
  if (!activeForInteraction(grant) || !mode) {
    ...
    return codePage(req, res, 400, 'That code is not recognised, or it has expired.');
  }
...
function describeRight(right) {
  if (typeof right === 'string') {
    ...
    return '<code>' + xmlEscape(right) + '</code><span>' + (registered
      ? 'a resource set registered by <code>' + xmlEscape(registered.rsIdentifier) + '</code>: ' +
        xmlEscape(registered.access.map(function (one) {
          return typeof one === 'string' ? one : one.type;
        }).join(', '))
      : 'an access reference this authorization server attaches no further meaning to') +
      '</span>';
  }
  const parts = [];
  ['actions', 'datatypes', 'locations', 'privileges'].forEach(function (dimension) {
...
function approvalPage(req, res, grant, session) {
  ...
  grant.request.tokens.forEach(function (token, t) {
    rows += (grant.request.tokens.length > 1 || token.label
      ? '<li><strong>Token ' + xmlEscape(token.label || String(t + 1)) + '</strong>' +
        (token.bearer ? ' — a bearer token, usable by whoever holds it' : '') + '</li>' : '');
    token.access.forEach(function (right, r) {
      rows += '<li><label><input type="checkbox" name="right" value="t' + t + 'r' + r +
        '" checked><span>' + describeRight(right) + '</span></label></li>';
    });
  });
  const subjectAsked = grant.request.subject &&
    (grant.request.subject.subIdFormats.length || grant.request.subject.assertionFormats.length);
  if (subjectAsked) {
    rows += '<li><label><input type="checkbox" name="subject" value="yes" checked><span>' +
      '<strong>Who you are</strong><span>identifiers (' +
      xmlEscape(grant.request.subject.subIdFormats.join(', ') || 'none') + ') and assertions (' +
      xmlEscape(grant.request.subject.assertionFormats.join(', ') || 'none') + ')</span></span>' +
      '</label></li>';
  }
  const finish = grant.interaction.finish;
  const inner = logo + '<h1>Allow access?</h1>' +
    '<p class="sub">Signed in as <code>' + xmlEscape(session.user.username) + '</code></p>' +
    '<p class="app"><strong>' + xmlEscape(display.name || grant.client.identifier) +
    '</strong> is asking for access on your behalf.' +
    (display.uri ? '<br><a href="' + xmlEscape(display.uri) + '" rel="noreferrer">' +
      xmlEscape(display.uri) + '</a>' : '') + '</p>' +
    '<form method="post" action="/gnap/approve/' + xmlEscape(grant.interaction.approvalId) + '">' +
    websecurity.field(session.id) +
    '<ul class="rights">' + (rows || '<li>Nothing specific — this request asks only to be ' +
      'continued.</li>') + '</ul>' +
    '<div class="row"><button type="submit" id="gnap-allow" name="action" value="allow">Allow' +
    '</button><button type="submit" id="gnap-deny" name="action" value="deny" class="secondary">' +
    'Deny</button></div></form>' +
    '<div class="meta">' +
    '<div>Untick anything you do not want to allow; the application receives only what stays ' +
    'ticked (RFC 9635 section 4).</div>' +
    (finish ? '<div>Afterwards ' + (finish.method === 'redirect'
      ? 'you are sent back to <code>' + xmlEscape(finish.uri) + '</code>.'
      : 'the application is notified directly at <code>' + xmlEscape(finish.uri) + '</code>.') + '</div>'
      : '<div>Afterwards you can close this page and return to the application.</div>') +
    '<div>Client instance: <code>' + xmlEscape(grant.client.identifier) + '</code> · proof: <code>' +
    xmlEscape(grant.client.proof) + '</code>' + (grant.client.classId ? ' · class: <code>' +
    xmlEscape(grant.client.classId) + '</code>' : '') + '</div>' +
    '<div>Authorization server: <code>' + xmlEscape(grant.grantEndpoint) + '</code></div>' +
    '</div>';
...
function afterDecision(res, grant, finished) {
  if (finished.redirect) {
    ...
    // client's callback.
    return res.set('Cache-Control', 'no-store').redirect(303, finished.redirect);
  }
  const approved = grant.decision && grant.decision.approved;
  const name = (grant.client.display && grant.client.display.name) || grant.client.identifier;
  return sendPage(res, 200, approved ? 'Approved' : 'Not approved',
    '<h1>' + (approved ? 'You approved the request' : 'The request was not approved') + '</h1>' +
    '<p class="sub">' + (finished.pushed === true
      ? xmlEscape(name) + ' has been told.'
      : (finished.pushed === false
        ? xmlEscape(name) + ' could not be told directly; it will find out when it next checks.'
        : 'Return to ' + xmlEscape(name) + '; it will pick up the answer.')) + '</p>');
}

function approvalGrant(req, res) {
  const params = validation.checkParsed({ id: req.params.id }, 'params', ID_PARAMS);
  const grant = params.ok ? store.grantByInteraction('approve:' + params.value.id) : null;
  if (!activeForInteraction(grant) || !grant.interaction.started) {
    interactionError(res, 'STS-GNAP-0406', 'This request is not waiting for you',
      'There is no request waiting for approval at this address, or it was already answered or ' +
      'has expired.');
    return null;
  }
  return grant;
...
app.get('/gnap/approve/:id', function (req, res) {
  ...
  if (query.ok && query.value.authn_error) {
    ...
    monitor.record(grant.client.identifier, 'grant.denied', { gnapError: 'user_denied' });
  ...
  if (!session) {
    ...
    return res.set('Cache-Control', 'no-store').redirect(303, authn.beginAuthentication({
      returnTo: '/gnap/approve/' + grant.interaction.approvalId,
      protocol: 'GNAP',
      application: grant.client.identifier,
      hint: grant.userHint || '',
      details: [
        { label: 'Application', value: (grant.client.display && grant.client.display.name) ||
  ...
  if (grants.rememberedFor(grant, session.user.username)) {
    ...
    return grants.decide(req, grant, session, { approve: true, tokens: grant.request.tokens,
                                                subject: false })
...
app.post('/gnap/approve/:id', function (req, res) {
  ...
  if (!csrf.ok) {
    ...
    return interactionError(res, 'STS-GNAP-0407', 'This answer could not be accepted',
      'The form did not come from this page (its anti-forgery token is missing or wrong).');
  }
  ...
  const posted = validation.checkParsed(Object.assign({}, body, { right: ticked }), 'body',
                                        APPROVE_FORM);
  if (!posted.ok) {
    log.debug("Leaving POST /gnap/approve. Malformed: " + posted.detail);
    return interactionError(res, 'STS-GNAP-0408', 'This answer could not be read', posted.detail);
  }
  const approve = posted.value.action === 'allow';
  const tokens = grant.request.tokens.map(function (token, t) {
    return { label: token.label, bearer: token.bearer, access: token.access.filter(function (right, r) {
  ...
  return grants.decide(req, grant, session, {
    ...
    log.error(errorCodes.tag('STS-GNAP-0409') + 'gnap: the approval could not be completed: ' +
              (e && e.stack || e));
