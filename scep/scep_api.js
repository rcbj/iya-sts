...
// at the top would load `scep_console.js` — and through it the SCEP family's
...
function send(res, status, body) {
  log.debug("Entering send(). status=" + status);
...
const ROUTES = [
  { method: 'GET', path: BASE + '/scep', tag: 'SCEP', operationId: 'getScep',
    ...
    description: 'Everything /admin/scep draws, out of the same view ' +
                 'function: the absolute endpoint URLs of this realm\'s SCEP ' +
                 'server (RFC 8894) and GetCACaps\' capabilities; the SCEP ' +
                 'Issuing CA and whether the hierarchy is built; the RA ' +
                 'certificate (subject, serial, key algorithm, expiry, ' +
                 'status — never its private key); the nine profiles with ' +
                 'what each needs, whether scep.allowedProfiles allows it ' +
                 'and its SCEP URL, and the five refused with why; the ' +
                 'challenge passwords (paged, `credentialsPage`; ids and ' +
                 'status, never a secret); the registered host names; the ' +
                 'certificates issued over SCEP (paged, `certificatesPage`); ' +
                 'the documented exceptions; the mode; and the `scep.*` ' +
    ...
    responseSchema: openObject('The SCEP server of this realm, as ' +
                               '/admin/scep draws it.'),
    handler: function (req, res) {
      log.debug("Entering the management API SCEP endpoint.");
      const model = require('./scep_console');
      const query = model.queryOf(req);
      if (!query.ok) {
        errorCodes.mark(res, 'STS-SCEP-0060');
        send(res, 400, { ok: false, errors: [query.detail] });
        log.debug("Leaving the management API SCEP endpoint. Bad query.");
        return;
      }
      send(res, 200, model.scepView(req));
      log.debug("Leaving the management API SCEP endpoint.");
    } },

  { method: 'GET', path: BASE + '/scep/monitor', tag: 'SCEP',
    operationId: 'getScepMonitor',
    summary: 'What the SCEP server has done: requests, issuances, refusals ' +
             'by failInfo and by error code',
    description: 'Everything /admin/scep/monitor draws: the counters since ' +
                 'the process started, per trust realm and added across ' +
                 'processes — requests, issued, refused, revoked, challenges ' +
                 'created; counts by operation (PKIOperation by ' +
                 'messageType), ' +
                 'by RFC 8894 failInfo, by STS error code, by profile and by ' +
                 'principal; and the recent requests (paged). No reset: the ' +
    ...
    responseSchema: openObject('What the SCEP server has done in this ' +
                               'realm, as /admin/scep/monitor draws it.'),
    handler: function (req, res) {
      ...
      send(res, 200, model.scepMonitorView(req));
  ...
  { method: 'POST', route: BASE + '/scep/:action', tag: 'SCEP',
    ...
    handler: function (req, res) {
      ...
      return model.scepAction(body, { via: 'api', actor: 'admin-api',
        ...
        .catch(function (e) {
          log.error(errorCodes.tag('STS-SCEP-0061') + 'scep: an /admin-api ' +
    ...
    actions: [
      { action: 'create-challenge', operationId: 'createScepChallenge',
        summary: 'Create a single-use SCEP challenge password for one entry ' +
                 'and one profile',
        description: 'The challenge authorizes ONE enrollment, AS the entry ' +
                     'it names, for the profile it names (the default ' +
                     'profile when none is given). **It is in this reply ' +
                     'once and never again**: the entry keeps only its ' +
                     'SHA-256. The reply also carries the SCEP URL for its ' +
        ...
        requestBody: {
          ...
          properties: Object.assign({}, ENTRY_PROPERTIES, {
            profile: { type: 'string', maxLength: 64,
                       description: 'One of the nine enrollment profiles.' },
      ...
      { action: 'delete-challenge', operationId: 'deleteScepChallenge',
        ...
        description: 'The challenge stops being redeemable. A certificate ' +
                     'already issued with it is untouched.',
        requestBodyRequired: true,
        requestBody: {
          type: 'object',
      ...
      { action: 'reissue-ra', operationId: 'reissueScepRa',
        ...
        description: 'A new RSA key pair (scep.raKeyAlgorithm) certified by ' +
                     'the SCEP Issuing CA; the certificate it replaces is ' +
                     'put on that CA\'s CRL as superseded. Clients ' +
                     'encrypting to the old one are answered ' +
      ...
      { action: 'revoke-certificate', operationId: 'revokeScepCertificate',
        ...
        description: 'Puts the serial on the SCEP Issuing CA\'s CRL and OCSP ' +
                     'with the RFC 5280 reason given, and marks the record ' +
                     'on the entry revoked.',
        requestBodyRequired: true,
        requestBody: {
          type: 'object',
          properties: {
      ...
      { action: 'add-host-name', operationId: 'addScepHostName',
        ...
        description: 'A dNSName or iPAddress is issued over any enrollment ' +
