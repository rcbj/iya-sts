...
const ACTIONS = [
  ...
  // ---------------------------------------------------------------------
  // ACTIONS RECORDED AND NEVER LISTED (2026-09-26). Each was written by its
  // module and had no row here, so the audit filter could not name it — and
  // since #86 holds `?action=` to this list, `GET /admin-api/audit` REFUSED a
  // query for it. tests/audit_vocabulary.js now fails on the next one.
  // ---------------------------------------------------------------------
  { action: 'admin.aud-sub.set', category: 'admin',
    label: 'A person\'s aud_sub setting was changed by an administrator' },
  { action: 'admin.mfa.backup-codes.cleared', category: 'authentication',
    label: 'An administrator cleared a person\'s recovery codes' },
  { action: 'admin.password.reset-keytab', category: 'admin',
  ...
  { action: 'enrollment.acme.account.create', category: 'protocol',
    label: 'An ACME account was created' },
  { action: 'enrollment.acme.account.deactivate', category: 'protocol',
    label: 'An ACME account was deactivated' },
  { action: 'enrollment.acme.account.key-change', category: 'protocol',
    label: 'An ACME account\'s key was changed' },
  { action: 'enrollment.eab.create', category: 'authentication',
    label: 'An ACME External Account Binding key was made' },
  { action: 'enrollment.eab.delete', category: 'authentication',
    label: 'An ACME External Account Binding key was removed' },
  { action: 'enrollment.challenge.create', category: 'authentication',
    label: 'A SCEP challenge password was made' },
  { action: 'enrollment.challenge.delete', category: 'authentication',
    label: 'A SCEP challenge password was removed' },
  { action: 'enrollment.issue', category: 'protocol',
    label: 'A certificate was issued through ACME, EST or SCEP' },
  { action: 'enrollment.issue.refused', category: 'protocol',
  ...
  { action: 'federation.signout-answered', category: 'session',
    label: 'A federation partner confirmed a sign-out this service sent' },
  { action: 'federation.unlink', category: 'directory',
    label: 'A federation link was removed from a person' },
  { action: 'group.add-member', category: 'directory',
    label: 'A member was added to a group' },
  { action: 'krb5.keytab.person', category: 'authentication',
    label: 'A person\'s keytab was made from a password in hand' },
  { action: 'logout.backchannel.retry', category: 'session',
    label: 'A dead back-channel Logout Token delivery was retried' },
  { action: 'oauth.device-secret.revoke', category: 'protocol',
    label: 'A Native SSO device secret was revoked' },
  { action: 'oauth2.ciba.notify', category: 'protocol',
    label: 'A CIBA ping or push notification was delivered or failed' },
  { action: 'oauth2.claim-source-linked', category: 'authentication',
    label: 'A person linked a Claims Provider' },
  { action: 'oauth2.claims-providers.add-provider', category: 'admin',
    label: 'A Claims Provider was registered' },
  { action: 'oauth2.claims-providers.update-provider', category: 'admin',
    label: 'A Claims Provider was changed' },
  { action: 'oauth2.claims-providers.remove-provider', category: 'admin',
    label: 'A Claims Provider was removed' },
  { action: 'oauth2.claims-providers.revoke-link', category: 'admin',
    label: 'A person\'s Claims Provider link was revoked' },
  { action: 'oauth2.command', category: 'protocol',
    label: 'An OpenID Provider Command was received' },
  { action: 'oauth2.command.tenant', category: 'protocol',
    label: 'An OpenID Provider tenant command was carried out' },
  { action: 'oauth2.grant-revoked', category: 'protocol',
    label: 'A Grant Management grant was revoked' },
  { action: 'oauth2.par.withdraw', category: 'admin',
  ...
  { action: 'portal.app-password.revoked', category: 'authentication',
    label: 'A person revoked an app password' },
  { action: 'portal.app-password.csrf', category: 'authentication',
    label: 'An app password form was refused for its CSRF token' },
  { action: 'portal.certificates.csrf', category: 'authentication',
  ...
  { action: 'portal.kerberos.refused', category: 'authentication',
    label: 'A person\'s keytab request was refused' },
  { action: 'portal.kerberos.csrf', category: 'authentication',
    label: 'A Kerberos form was refused for its CSRF token' },
  { action: 'portal.key.started', category: 'authentication',
    label: 'A person started enrolling a security key' },
  { action: 'portal.key.refused', category: 'authentication',
    label: 'A person\'s security key enrolment was refused' },
  { action: 'portal.mfa.backup-codes.generated', category: 'authentication',
    label: 'A person generated a set of recovery codes' },
  { action: 'portal.mfa.backup-codes.confirmed', category: 'authentication',
    label: 'A person confirmed they kept their recovery codes' },
  { action: 'portal.mfa.backup-codes.discarded', category: 'authentication',
    label: 'A person discarded a generated set of recovery codes' },
  { action: 'portal.risc.optOutInitiated', category: 'signals',
    label: 'A person started opting out of RISC' },
  { action: 'portal.risc.optOutCancelled', category: 'signals',
    label: 'A person cancelled a RISC opt-out' },
  { action: 'portal.risc.optIn', category: 'signals',
    label: 'A person opted back in to RISC' },
  { action: 'portal.signing-key.issued', category: 'authentication',
    label: 'A person issued their own signing key pair' },
  { action: 'portal.signing-key.removed', category: 'authentication',
    label: 'A person took their signing key pair off' },
  { action: 'portal.signing-key.csrf', category: 'authentication',
    label: 'A signing key form was refused for its CSRF token' },
  { action: 'portal.siop.remove', category: 'authentication',
    label: 'A person removed a self-issued subject' },
  { action: 'risk.feedback', category: 'authentication',
    label: 'Feedback on a risk assessment was recorded' },
  { action: 'risk.response', category: 'authentication',
  ...
  { action: 'saml2.artifact.resolve', category: 'protocol',
    label: 'A SAML 2.0 artifact resolution was refused' },
  { action: 'saml2.attribute-query', category: 'protocol',
    label: 'A SAML 2.0 attribute query was answered or refused' },
  { action: 'saml2.metadata.mdq', category: 'protocol',
    label: 'A SAML metadata MDQ lookup was made or refused' },
  { action: 'saml2.metadata.refresh', category: 'protocol',
    label: 'A service provider\'s SAML metadata was refreshed' },
  { action: 'session.reauthenticate', category: 'session',
    label: 'The same person signed in again on a live session' },
  { action: 'session.refuse', category: 'session',
    label: 'A session was refused for its sign-in mechanism' },
  { action: 'session.refused', category: 'session',
    label: 'A hosted surface refused a session' },
  { action: 'ssf.event.act', category: 'signals',
    label: 'A received Security Event was acted on' },
  { action: 'ssf.stream.swept', category: 'signals',
    label: 'A stale receiver stream was swept' },
  { action: 'xacml.pep.enable', category: 'authorization',
    label: 'A remote PEP was enabled' },
  { action: 'xacml.pep.disable', category: 'authorization',
    label: 'A remote PEP was disabled' },
  { action: 'xacml.pep.forget', category: 'authorization',
    label: 'A remote PEP was forgotten' },
  { action: 'xacml.policy.write', category: 'authorization',
    label: 'An XACML policy was written' },
  { action: 'xacml.policy.delete', category: 'authorization',
    label: 'An XACML policy was deleted' }
];
