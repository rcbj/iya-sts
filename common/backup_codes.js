...
// NOT SHARED WITH IT.
//
// `common/totp.js` uses `ABCDEFGHIJKLMNOPQRSTUVWXYZ234567` because the
...
// secret beside them is. `common/credentials.js` does the sealing; this file
...
// realm may issue six long codes while another issues twelve short ones, and
// neither had to be told about the other.
//
// **UNLIKE TOTP, NONE OF THESE IS COPIED ONTO THE RECORD**, and the difference
// is real rather than an inconsistency. A TOTP parameter was TOLD TO AN APP
// that this service cannot reach, so changing it retrospectively would break
// an enrolment somebody already scanned. A recovery code is a string compared
// against a stored string: shortening `backupCodes.length` changes what the
// NEXT set looks like and leaves an existing set matching exactly as it did,
...
// that draws the list, for `totp.offered()`'s reason: a page is markup and an
// endpoint is a door.
//
...
// ONE CODE.
//
// **`randomInt` PER CHARACTER AND NOT `randomBytes` WITH A MODULO.** Thirty-two
// divides 256 exactly, so a modulo would in fact be unbiased here — and that is
// precisely the kind of accident that stops being true the moment somebody
// drops the vowels to stop a code spelling something and leaves twenty-six
// characters behind. `nodeCrypto.randomInt()` is rejection-sampled inside node
// and is correct for every alphabet size, so the property does not depend on a
...
// ---------------------------------------------------------------------------
// A WHOLE SET. Returns plain strings; `common/credentials.js` is what turns
...
// spent, leaves an identical string on the list still marked unused. That is a
// code which works twice, which is the one property a single-use credential
// may not have, and it would be found by nobody.
// ---------------------------------------------------------------------------
function generate(opts) {
  ...
  const codes = [];
  // A bound rather than `while (true)`. At fifty bits per code the body below
  // will not run twice; the bound is what stops a future alphabet of two
  // characters turning a generation into a hang on the one thread that answers
  // every socket this service holds.
  ...
  log.debug('Leaving generate(). ' + codes.length + ' code(s) of ' + length +
            ' character(s).');
  return codes;
}
...
// same person: this service PRINTS the code in groups with a dash between them
// so that it can be transcribed, and somebody transcribing it will type the
// dash they can see. Refusing their own rendering back would be the service
// arguing with its own page.
//
...
//
// **CONSTANT TIME**, through the one comparison this repository has. Unlike the
// six digits of a TOTP code, a recovery code has fifty bits in it and a timing
// oracle is not the attack anybody would reach for — the reason it is here is
// that `===` is right for everything else in a JavaScript file, so the wrong
// one is what gets written by reflex. `common/credentials.js` has a whole
// header about that.
// ---------------------------------------------------------------------------
...
//
// **THIS REVERSES THIS MODULE'S OLDEST DECISION AND THE HEADER RECORDS WHAT IT
// COST.** A set used to be stored ENCRYPTED, on the argument that `crypto.js`
// states — *a secret this service VERIFIES is hashed, a secret it must PRESENT
// cannot be* — and a recovery code was both, because `/portal/mfa` let a
// person read their remaining codes back months later.
//
...
//
// **IT IS `crypto.hashSecret()` AND NOT A DIGEST OF THIS MODULE'S OWN**, which
// is rule 3r: there is one place this service hashes a secret it will later
// check, and `userPassword` and `stsActivationToken` already go through it.
// scrypt at N=2^15 is expensive on purpose — and for a code with fifty bits of
// CSPRNG entropy in it, that is belt AND braces rather than the whole defence:
// SHA-256 over fifty bits is about 2^50 guesses, which is hours of GPU time
// per code, and scrypt turns that into something nobody finishes.
//
// **THE COST IS REAL AND IT IS WHY THERE IS AN ASYNC DOOR.** Measured on this
// machine: one hash is 72ms, and a WRONG code has to be compared against every
// code in the set — ten by default — which measured **906ms of blocked event
// loop**. That is not a price a sign-in path can pay on one thread, so
// `common/credentials.js` checks a presented code through the WORKER POOL and
// in parallel. The synchronous door is kept for `npm test` and for
// `workers.count = 0`, which is a supported configuration.
//
// A hash carries its own random salt, so two identical codes in two sets — or
// in one — hash differently, and there is no shortcut that would let a
...
// with both halves in its hand uses, and this compares a code against a STORED
// HASH, which is all a running service ever has. Folding them into one
// function that sniffed the stored form would be a function that silently did
...
// password typed into the code box is refused with a sentence about the shape
...
// ---------------------------------------------------------------------------
// WHAT `/admin/backup-codes` AND `/admin/crypto-metadata` DRAW. Read from this
// module rather than written down over there, which is that page's whole
...
module.exports = {
  ...
  matchesHashAsync: matchesHashAsync,
  isHash: isHash,
