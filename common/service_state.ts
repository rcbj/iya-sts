...
// shape: `ServiceState` takes the store, the keystore, the certificate
...
// What a `ServiceState` needs from the rest of the service, each named for
// the module that supplies it today.
interface ServiceStateDeps {
...
class ServiceState {
  ...
  // ---------------------------------------------------------------------------
  // ONE KEY SET PER REALM BEFORE ANYTHING IS SERVED (2026-09-14, #46 section
  // 1).
  //
  ...
  // realm has (`keystore.js`, first writer wins) — so between the generation
  // and the commit a process may hold a set it is about to be told to drop. For
  // a realm made at runtime that window is the one this service always had
  // inside a container. For a COLD START it would be every node's first
  // seconds: N nodes against an empty store, N sets, each node signing with its
  // own until its write lands.
  //
  ...
  settleSigningKeys(): Promise<any> {
    ...
    if (typeof keystore.arbitrates !== 'function' || !keystore.arbitrates()) {
      log.debug("Leaving ServiceState.settleSigningKeys(). The store does " +
                "not arbitrate.");
      return Promise.resolve(null);
    }
    const ids = cluster.isActiveActive()
      ? realms.list().map(function (one: any) {
    ...
    log.debug("Leaving ServiceState.settleSigningKeys(). " + ids.length +
              " realm(s).");
    ...
    return helpers.prepareKeySets(ids).then(function () {
      ids.forEach(function (id) {
        try {
          ...
          log.debug("Caught in ServiceState.settleSigningKeys(): " +
                    ((e && e.message) || e));
        }
      });
      return keystore.settleAll();
    }).then(function (outcomes: any[]) {
      const adopted = (outcomes || []).filter(function (one: any) {
        return one && one.adopted;
      }).length;
      log.info('service_state: the signing keys of ' + ids.length +
               ' realm(s) are settled with the store before anything is ' +
               'served' +
               (adopted ? '; ' + adopted + ' set(s) another node had ' +
                          'written first were adopted' : '') + '.');
      return outcomes;
    });
  }
  ...
  start(): Promise<any> {
    ...
    // the first one is: reading a key-encryption key from AWS, GCP, Azure or
    // Vault is a network call, and `helpers.js` builds a key set inside a
    // PROPERTY READ that cannot await. So everything asynchronous happens here,
    // and what is left is a synchronous map lookup.
    //
    ...
    return keystore.start().then(function (keys) {
      ...
      //
      // AGREEMENT: this node's keyed fingerprint of the settings every node
      // must share is compared with every live node's. A node with a different
      ...
      // one that was persisting has lost them, silently, and the only place
      // that would ever show is somebody being signed out for no reason. It
      // does nothing at all in development mode, in memory mode and on an ldif
      ...
      return persistence.restoreMinted().then(function (mintedResult) {
        ...
        // ABOVE**, and the asymmetry is the argument: a process that cannot
        // read its own signing keys is lying about what it issues, and a
        // process that cannot read the change log is simply the process this
        // service was before 2026-09-06 — correct about its own copy, and alone
        // with it. It starts, says so loudly, and keeps trying on its timer.
        // ---------------------------------------------------------------------
        return persistence.coordinate().then(function (coordinating) {
          return self.settleSigningKeys().then(function () {
            return coordinating;
          });
        ...
        // row family. **BEFORE the listener, because every key pair this
        // service generates is a leaf of it** — a request answered before the
        // tree exists would be answered with a self-signed certificate that a
        // moment later stops being the published one.
        //
        ...
        // — which is exactly what this service did for its whole life until
        // this date — where a service that cannot read its signing keys is
        // lying about what it issues. `pki.start()` says so loudly and returns.
        // ---------------------------------------------------------------------
        ...
        return pki.start({ realmIds: realms.list().map(function (one: any) {
          return one.id === realms.DEFAULT_ID ? '' : one.id;
        }),
          ...
          // in-process caller of helpers. This is the one place that knows
          // both, so this is where they meet.
          keySetFor: function (realmId: string) {
          ...
          // four processes, four key sets, arbitrated away afterwards.
          // `.existing()` is the cache itself, so this answers the question
          // without filling it. `pki.js`'s watcher carries the measurement.
          keySetHeldFor: function (realmId: string) {
