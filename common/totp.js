...
//
// `common/credentials.js`'s `ROLES` has `primary` and `mfa` for a security key
...
function base32Encode(buffer) {
  ...
  log.debug('Leaving base32Encode(). ' + out.length + ' character(s).');
  return out;
}

function base32Decode(text) {
  log.debug('Entering base32Decode().');
  ...
  for (let i = 0; i < cleaned.length; i++) {
    ...
    if (index < 0) {
      // NAMED rather than skipped. A character that is not in the alphabet is
      // a typo or a paste of something that is not a secret at all, and
      ...
      // right".
      throw new Error('"' + cleaned[i] + '" is not a base32 character. A ' +
                      'shared secret is the letters A to Z and the digits 2 ' +
                      'to 7.');
    }
    value = (value << 5) | index;
    bits += 5;
  ...
  log.debug('Leaving base32Decode(). ' + out.length + ' byte(s).');
...
//
// Every one of them is read through `config.value()`, which answers out of the
// AMBIENT REALM — so a realm may run six-digit SHA-1 codes while another runs
// eight-digit SHA-512, and neither had to be told about the other. That falls
// out of the realm design rather than being arranged here.
//
// **THE PARAMETERS ARE COPIED ONTO THE RECORD WHEN A SECRET IS ENROLLED**, and
// this function is what an enrolment reads. Verification reads the RECORD and
// not this — see `verify()`, where the reason is argued. A setting changed
...
// page that draws the enrolment form, for the reason `authn.js` gives about
// `authn.unauthenticatedSessions`: a page is markup and an endpoint is a door,
// and a form posted by hand while the setting is off must not enrol anybody.
//
...
// R6 recommendation and the size every authenticator expects for HMAC-SHA-1.
//
// `randomBytes` and nothing derived from the username, the time or a counter:
// a secret guessable from a name is a list of accounts whose second factor is
// no factor.
// ---------------------------------------------------------------------------
function generateSecret(opts) {
  log.debug('Entering generateSecret().');
  const options = opts || {};
  const bytes = Number(options.bytes || settings().secretBytes);
  const secret = base32Encode(nodeCrypto.randomBytes(bytes));
  log.debug('Leaving generateSecret(). ' + bytes + ' byte(s).');
  return secret;
}
...
//
// **THE ANSWER IS THE COUNTER AND NOT A BOOLEAN**, because the caller needs it:
// RFC 6238 section 5.2 requires that a code be accepted ONCE, and the only way
// to enforce that is to remember which step was spent and refuse anything at or
// below it next time. `common/credentials.js` stores it on the record; this
// function has no store and deliberately none, so that it can be called from a
// test, from an enrolment confirmation and from a sign-in with the same
// meaning.
//
// **THE PARAMETERS COME FROM THE RECORD**, which is the argument `settings()`
// above defers to. The digits, the period and the algorithm are what the QR
// code told the authenticator app when it was scanned, and they are the app's
// now — this service cannot change them retrospectively. An operator who edits
// `totp.digits` therefore changes what the NEXT enrolment is, and every
// existing one goes on working. The alternative was measured against the only
// thing that matters here: an operator adjusting a setting must not silently
// lock out everybody who already enrolled.
//
// **THE SKEW WINDOW IS SYMMETRIC AND IT IS A REAL WINDOW.** RFC 6238 section
// 5.2 recommends at most one step either side, which is what the default of 1
// gives — a code is good for its own 30 seconds plus 30 either side, so a phone
// whose clock is a little off still works and a code lives at most 90 seconds.
// It is settable because a mock is run on laptops that have been asleep.
//
...
// alternative is the `===` this repository's credential module was written to
// abolish.
// ---------------------------------------------------------------------------
function verify(record, presented, opts) {
  ...
  const window = options.window === undefined
    ? settings().window : Math.max(0, Number(options.window));
  ...
  // depend on WHICH step matched, which is a far better oracle than the digit
  // comparison this file bothers to make constant-time. The cost is at most a
  // handful of HMACs.
  let matched = null;
...
// rather than an RFC. Three things about it are worth stating because each has
...
//     service will check, and an app that ignores them is no worse off — which
//     is the state Google Authenticator is in, and the reason `/admin/totp`
//     warns against changing the algorithm.
//
// **THE ISSUER DEFAULTS TO THIS REALM'S HOST** rather than to a constant. Two
// realms of one process are two identity providers, and an authenticator
...
function otpauthUri(spec) {
  log.debug('Entering otpauthUri(). account=' + (spec && spec.account));
  const options = spec || {};
...
// ON THE SERVER.** Every page of the user portal is `script-src 'none'` — see
// `common/app.js`, whose comment argues that the absence of script is what
// makes a family of reflected-content problems moot rather than merely
...
// already in the policy for the two OID4VC offer pages, which draw their codes
// exactly this way and with the same library.
//
...
// match, because changing a page that works to make a file consistent is how
// the two QR renderings would come to be maintained as one thing.
//
...
// ---------------------------------------------------------------------------
function qrSvgDataUri(uri) {
  log.debug('Entering qrSvgDataUri().');
  // REQUIRED HERE AND NOT AT THE TOP, which is the one lazy require in this
  // file. `qrcode` pulls in a chain this module has no other use for, and this
  // module is loaded by `credentials.js` — which is on the path of every
...
// table lives with the code that performs the algorithm, so the report cannot
...
module.exports = {
  BASE32_ALPHABET: BASE32_ALPHABET,
