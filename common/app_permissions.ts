...
//
// `AppPermissions` takes the logger, the application registry and the
...
class AppPermissions {
  ...
  //
  // One read of `ou=applications` answers every question this feature has, so
  // it is done once and the two views are built from it. Two functions each
  // walking the registry would be two linear reads per page and — the reason
  // that actually matters — two chances to disagree about which entries count
  // as resources.
  //
  // **A RESOURCE IS AN ENTRY THAT CARRIES A BASE URI OR A PERMISSION**, not one
  // that carries both. An entry with a base and no permissions is an API
  // somebody has started describing; an entry with permissions and no base is
  // one whose base was removed from under them, and those permissions have no
  // identifier and can never be asked for. Both are states worth SEEING, and an
  ...
  register() {
    ...
    rows.forEach(function (row) {
      ...
      const resource = {
        ...
        // `ldapmodify` is not normalised (the attribute's own row says so), so
        // an entry can hold `https://example.com` while every identifier
        // computed from it carries the `/` this module added. Showing only one
        // of the two would make the console disagree with an `ldapsearch`.
    ...
    // labelled `3` would be the page deciding that a permission is a detail of
    // a pair of applications. It is the other way round: the permission is the
    ...
    rows.forEach(function (row) {
      ...
      // this registry that a grant has ever been USED, and it is worth having
      // on the page precisely because the interesting reading of a configured
      ...
      list.forEach(function (value) {
        ...
        const grant = {
          ...
          // NO APPLICATION DEFINES THIS PERMISSION. Not an error and not
          // hidden: the resource's entry may have been deleted, the permission
          // removed from under the grant, or an `ldapmodify` may have written a
          // grant that never resolved — and `updateApplication()` refuses to
          // CREATE one through either console door, so a dangling row is always
          // something that happened outside them. The same three-state honesty
    ...
    const answer = {
      ...
      counts: {
        ...
        // Grants nobody has used, and permissions nobody holds. These two are
        // the whole reason to draw a configured register beside an observed
        // one.
    ...
    log.debug("Leaving AppPermissions.register(). " +
              answer.counts.permissions + " permission(s), " +
  ...
  forApplication(identifier) {
    const { log } = this.deps;
    const self = this;
    log.debug("Entering AppPermissions.forApplication(). identifier=" +
              identifier);
    const wanted = String(identifier == null ? '' : identifier).trim();
    const all = self.register();
    const answer = {
      identifier: wanted,
      exposes: all.permissions.filter(function (one) {
        return one.resource === wanted;
      }),
      holds: all.grants.filter(function (one) {
        return one.client === wanted;
      }),
      ...
      // about the application as a whole — *who can reach me* — and answering
      // it by asking the reader to fold up a column would be the page doing
      // arithmetic on their behalf.
      grantedToOthers: all.grants.filter(function (one) {
        return one.resource === wanted;
      })
    };
    log.debug("Leaving AppPermissions.forApplication(). exposes=" +
              answer.exposes.length + ", holds=" + answer.holds.length);
    return answer;
  }
  ...
  // attribute editor and the management API's generic `update` operation also
  // go through them, and what is missing there is only that its messages are
  // about an ATTRIBUTE (`added "https://example.com/write" to
  // oauthDelegatedPermission`) where a reader of this feature is thinking about
  // a RELATIONSHIP.
  //
  // They return `applications.updateApplication()`'s shape unchanged — `{ ok,
  // changed, application, message, errors }` — so the console's
  ...
  setBaseUri(resource, value, actor) {
    ...
    if (!result.ok) {
      log.debug("Leaving AppPermissions.setBaseUri(). Refused.");
      return result;
    }
    const normalised = applications.permissionBaseOf(asked);
    log.debug("Leaving AppPermissions.setBaseUri(). ok.");
    return Object.assign({}, result, {
      message: asked
        ? '"' + resource + '" exposes its permissions under ' + normalised +
          ...
          'A permission called `write` on it is now `' + normalised +
          'write`, ' +
  ...
  definePermission(resource, name, description, actor) {
    ...
    if (!result.ok) {
      log.debug("Leaving AppPermissions.definePermission(). Refused.");
      return result;
    }
    const base = applications.permissionBaseOf(
      ((result.application || {}).fields || {}).oauthPermissionBaseUri);
    log.debug("Leaving AppPermissions.definePermission(). ok.");
    return Object.assign({}, result, {
      message: '"' + resource + '" now exposes the permission `' + leaf +
               '`, identified by `' + base + leaf + '`. NOTHING HOLDS IT ' +
               'YET — defining a permission grants it to nobody, which is ' +
               'the ordering this feature is built on. Grant it to a client ' +
               'and a request carrying `' + base + leaf + '` in its `scope` ' +
               'produces an access token audienced to ' + base + ' with `' +
               leaf + '` on its scope claim.'
    });
  }

  // REMOVE ONE. The value has to match what is on the entry exactly —
  ...
  removePermission(resource, name, actor) {
    ...
    const self = this;
    log.debug("Entering AppPermissions.removePermission(). resource=" +
              resource + ", name=" + name);
    const leaf = String(name == null ? '' : name).trim();
    const entry = applications.get(resource);
    if (!entry) {
      log.debug("Leaving AppPermissions.removePermission(). No such " +
                "application.");
      return errorCodes.mark({ ok: false, errors: ['There is no application ' +
        'called "' + resource + '" in this registry.'] }, 'STS-REG-0021');
    }
    ...
    if (!found) {
      log.debug("Leaving AppPermissions.removePermission(). No such " +
                "permission.");
      return errorCodes.mark({ ok: false, errors: ['"' + resource +
        '" defines no permission called "' + leaf + '". It defines: ' +
    ...
    if (!result.ok) {
      log.debug("Leaving AppPermissions.removePermission(). Refused.");
      return result;
    }
    // WHO STILL HOLDS IT, counted AFTER the removal, because that is the fact
    // the person needs and it is not obvious: removing a permission does NOT
    // revoke the grants naming it. They stay on the clients' entries and become
    // DANGLING, which the console reports. Silently tidying them would be this
    // action writing to entries the person did not name.
    const stranded = self.register().grants.filter(function (one) {
      return one.permissionId === found.id;
    });
    log.debug("Leaving AppPermissions.removePermission(). ok, " +
      ...
      message: '"' + resource + '" no longer exposes `' + leaf + '`. ' +
        (stranded.length
          ? '<strong>' + stranded.length + ' grant(s) still name `' + found.id +
            ...
            stranded.map(function (one) {
              return one.client;
            }).join(', ') +
            '. They were NOT removed, deliberately: revoking them would be ' +
            'this action writing to entries you did not name. Revoke each ' +
            'one, or define the permission again and they resolve exactly ' +
            'as before.'
  ...
  grant(client, permissionId, actor) {
    ...
    if (!result.ok) {
      log.debug("Leaving AppPermissions.grant(). Refused.");
      return result;
    }
    const defines = applications.forPermission(id);
    log.debug("Leaving AppPermissions.grant(). ok.");
    return Object.assign({}, result, {
      message: '"' + client + '" is granted `' + id + '`' +
        (defines ? ', which "' + defines.identifier + '" exposes' : '') +
        '. A request from it carrying that string in `scope` produces an ' +
  ...
  revoke(client, permissionId, actor) {
    ...
    if (!result.ok) {
      log.debug("Leaving AppPermissions.revoke(). Refused.");
      return result;
    }
  ...
  // keep.
  //
  // **EVERY BOX IS AN APPLICATION AND THERE IS NO PERSON ON THIS PICTURE**,
  // which is the whole visual difference from the acts diagram and is why the
  // two must not be drawn on one canvas. A delegation ACT has three layers and
  // the first of them is a PERSON — a stick figure, someone on whose behalf
  // something happened. A configured permission has no person in it at all: it
  // says `webapp1 may reach https://example.com/ as whoever is signed in`, and
  // there is no whoever yet. Merging the two would put a picture of what may
  // happen and a picture of what did happen in one frame with no way to tell a
  // box that has been used from a box that has been described.
  //
  // **THIS SERVICE IS NOT ON IT EITHER**, and that is the second difference.
  // The hexagon is on the acts picture because every line there exists because
  // this service issued or refused something; not one line here has been issued
  // at all. A hexagon would be a box with no edges, which is a drawing of a
  // claim nobody made.
  //
  ...
  // mechanisms joining the same boxes: `webapp1 -> api1` for `read` and the
  // same pair for `write` are two different arrangements, and one line labelled
  ...
  graph(rows?) {
    const { log, applications } = this.deps;
    const self = this;
    log.debug("Entering AppPermissions.graph().");
    const source = rows || self.register().grants;
    const nodes = new Map();
    const edges = new Map();
    ...
    function nodeFor(identifier, role) {
      ...
      if (!node) {
        node = {
          ...
          // reaching for a stick figure. See that function — the shape falls
          // out of these two members and nothing else.
          key: '', presented: identifier, application: identifier,
          what: '',
          roles: { initial: 0, intermediary: 0, target: 0 },
          protocols: ['OAuth 2.0 / OIDC'],
          // `acts` MUST STAY ZERO ON EVERY BOX. `edgeLook()` colours an edge
          // RED when `acts && !issued`, which is its way of saying "this was
          // tried and refused" — and every line here has been tried nought
          // times. A configured grant drawn in the refusal colour would be the
          // picture asserting the one thing it cannot know.
          acts: 0, issued: 0, refused: 0,
          firstAt: 0, lastAt: 0,
          selfTarget: false,
          // This file's own two, read by the console's node table rather than
          // by the renderer.
    ...
    source.forEach(function (row) {
      // A DANGLING GRANT HAS NO RESOURCE BOX TO REACH, so it draws no line. It
      // is still on the tables and in the counts — the picture is not where
      // that state is reported, because a line to nowhere is not a drawing of a
      ...
      if (row.client === row.resource) {
        ...
        // refuses an application granting itself — and an `ldapmodify` can
        // still write it. Recorded on the box the way `delegation.graph()`
        // records S4U2Self, because an arrow leaving a box and returning to it
      ...
      if (!edges.has(id)) {
        edges.set(id, {
          ...
          // THE RELATION THIS FILE ADDS TO THE RENDERER, and the third one
          // added by a caller after `user_graph.ts`'s two. It is deliberately
          // NOT `reaches`: that word is the acts picture's claim that a
          // credential was issued FOR something, and this line says only that
          // one may be.
          relation: 'may-reach',
          // No mode, no mechanism and no spec, because a configured permission
          // is none of those. `edgeLabelLines()` and `edgeLook()` are given a
      ...
      // WHETHER THE CLIENT HAS EVER ASKED FOR IT — the one thing on this
      // picture that comes from what happened rather than from what was
      // configured. It is carried so the line can be drawn SOLID when it has
      // been used and dashed when it has not, which is the single most useful
      // thing a configured picture can say: these are the grants nobody needed.
    ...
    const answer = {
      ...
      // either to the renderer and to `?format=json` without asking which it
      // has.
      counts: {
        nodes: nodes.size,
        edges: edges.size,
    ...
    log.debug("Leaving AppPermissions.graph(). " + answer.counts.nodes +
              " box(es), " + answer.counts.edges + " line(s).");
    return answer;
  }
  ...
  //
  // `graph()` above answers *what may reach what* and it answers it for the
  // whole registry at once. That is the right picture for a service with five
  // applications in it and it is the wrong one for a service with eighty: the
  // interesting reading of a permission register is almost never the whole of
  // it, it is **which applications are joined to each other at all** — the API
  // and the three front ends that hold permissions on it, the batch job that
  // reaches two of them, and the twelve applications elsewhere in the registry
  // that have nothing whatever to do with any of it.
  //
  // So this function partitions the register into GROUPS. A group is a
  // connected component of the grant graph, and the whole of the definition is
  ...
  // and the picture draws that with a round end and an arrowhead precisely
  // because the two ends are not interchangeable. Following the arrows would
  // answer *what can this client eventually reach*, which is a question about a
  // chain; a permission register has no chains in it, because holding a
  // permission on an API does not grant that API's own permissions to anybody.
  // Following a grant EITHER WAY answers the question a reader actually brings
  // to this page — *which applications are in the same conversation as this
  // one* — and it is the only reading under which the API and the three front
  // ends holding permissions on it come out as one group rather than as four.
  // The PICTURE still draws every line with its direction on it, so nothing is
  // lost: what is dropped is direction as a criterion for MEMBERSHIP, not
  // direction as a fact.
  //
  // Three states join no two applications and each is a different reason:
  //
  //   * **A DANGLING grant** names a permission no application defines, so
  //     there is no far end to be in a group with. Its client is a member of
  //     whatever group its other grants put it in, and of a group of its own
  //     if it has none — which is the honest drawing of *this application has
  //     been granted something that does not exist*.
  //   * **A SELF-GRANT** (client and resource the same entry, which only an
  //     `ldapmodify` can write — see `graph()`) is one application, so it is
  //     one group of one. `graph()` already refuses to draw an arrow from a box
  //     back to itself; there is nothing for that arrow to connect either.
  //   * **A RESOURCE NOBODY HOLDS ANYTHING ON** is a group of one, and it is in
  //     this answer rather than left out of it. An API with permissions
  //     defined and no grants against them is the most interesting group of
  //     one there is: somebody described an API and nothing may reach it.
  //     Leaving it out would have made this list a list of grants wearing a
  //     different hat.
  //
  // The MEMBERSHIP UNIVERSE is therefore every application the configured
  // register touches at all: every resource `register()` found (an entry
  // carrying a base URI or a permission), and every client holding a grant. An
  // entry in `ou=applications` that is neither is not in any group, because
  // this register has nothing to say about it — `/admin/applications` is where
  // those live.
  //
  // **IT TAKES THE REGISTER RATHER THAN READING ONE.** `register()` is a walk
  // of `ou=applications` and the console already has its answer in hand when it
  // asks for this; a second walk would be a second linear read per page and —
  ...
  clusters(reg?) {
    const { log, applications } = this.deps;
    const self = this;
    log.debug("Entering AppPermissions.clusters().");
    const all = reg || self.register();
    ...
    all.grants.forEach(function (one) {
      add(one.client);
      // A dangling grant has no resource — see the header. `row.resource` is
      // the empty string there rather than a name nothing answers to, which is
    ...
    // The root is whichever identifier the joins happened to leave on top, so
    // it moves when a grant is added anywhere in the group — which would make
    // every link to a group on the console a link that goes stale for a reason
    // nobody could see. The first member is a property of the SET, so a group
    // keeps its name until its membership changes.
    const keyOf = {};
    ...
    // grant is filed by its CLIENT, which is enough: its resource is in the
    // same group by construction where it has one, and where it has not the
    // grant is dangling and belongs to the client alone.
    const grantsOf = {};
    all.grants.forEach(function (one) {
    ...
    const list = Object.keys(membersOf).map(function (root) {
      ...
      // WHAT THE PICTURE WILL ACTUALLY DRAW, counted here rather than by the
      // page that draws it. `graph()` draws no line for a dangling grant and
      // none for a self-grant, and a group whose table said `3 grants` above a
      // diagram with one line on it would be the console disagreeing with
      // itself about the same three rows.
      ...
      return {
        ...
        counts: {
          ...
          lines: drawn.length,
          permissions: permissions.length,
          dangling: grants.filter(function (one) {
            return one.dangling;
          }).length,
          selfGrants: grants.filter(function (one) {
            return !one.dangling && one.resource && one.resource === one.client;
          }).length,
          // The two readings the whole configured register exists for, per
          // group: a grant somebody has asked for at least once, and one nobody
          // has ever needed. Dangling rows are in neither, because `asked` on a
          // grant that resolves to nothing is not evidence about a
          // relationship.
    ...
    // worth looking at and a list of forty groups of one with the interesting
    // one at position thirty-one is the same problem the whole-register picture
    // had. Ties broken by the number of lines and then by the key, so that two
    // runs of the same service produce the same page — `register()` sorts for
    ...
    log.debug("Leaving AppPermissions.clusters(). " + answer.counts.clusters +
              " group(s) over " +
  ...
  // The one group an application is in, or null where the configured register
  // has never heard of it. Exact equality on the identifier, for the reason
  ...
  clusterFor(identifier, reg) {
    const { log } = this.deps;
    const self = this;
    log.debug("Entering AppPermissions.clusterFor().");
    const wanted = String(identifier == null ? '' : identifier).trim();
    log.debug("Entering AppPermissions.clusterFor(). identifier=" + wanted);
    const all = reg || self.clusters();
    const key = all.memberOf[wanted];
    const found = key === undefined
      ? null
      : all.clusters.filter(function (one) { return one.key === key; })[0] ||
        null;
    log.debug("Leaving AppPermissions.clusterFor(). " +
