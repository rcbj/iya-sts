...
// the problem rather than solving it — the ciphertext is only as safe as the
// key that opens it — so the KEK itself is never stored by this service, never
...
function missingModule(pkg, provider, err) {
  ...
  log.debug("Leaving missingModule().");
  return errorCodes.mark(new Error(
    'the "' + provider + '" key-encryption-key provider needs the ' + pkg +
    ' package and it is not installed. It is deliberately NOT a dependency ' +
    'of this service — it is a mock first, and four cloud SDKs nobody uses ' +
    'would be carried by every install. Run `npm ' +
    'install ' + pkg + '` in this ' +
...
function pick(raw, spec, borrowed) {
  ...
  try {
    ...
    log.debug("Caught in pick(): " + ((e && e.message) || e));
  ...
  if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
    ...
    if (borrowed) {
      throw errorCodes.mark(new Error('what is stored for ' + spec.label + ' ' +
                      'is not a JSON object, and it is the same place ' +
...
const fileProvider = {
  ...
  read: function (spec) {
    ...
    const path = at.where;
    if (!path) {
      throw errorCodes.mark(new Error(secret.provider + ' is "file" and ' +
                      secret.file +
                      ' names no path. Set it to a file holding ' +
                      secret.label +
    ...
    try {
      ...
      throw errorCodes.mark(new Error('the file "' + path + '" holding ' +
                      secret.label +
                      ' could not be read: ' + e.message + '. In product ' +
                      'mode this service will not start without it, because ' +
                      'the alternative is generating a new signing key and ' +
...
const awsProvider = {
  ...
  read: async function (spec) {
    ...
    if (!secretId) {
      throw errorCodes.mark(new Error(secret.provider + ' is "aws" and ' +
                      secret.ref +
    ...
    const value = answer.SecretString !== undefined &&
                  answer.SecretString !== null
      ? answer.SecretString
...
const gcpProvider = {
  ...
  read: async function (spec) {
    ...
    let name = at.where;
    if (!name) {
      throw errorCodes.mark(new Error(secret.provider + ' is "gcp" and ' +
                      secret.ref + ' ' +
                      'names no secret. Set it to ' +
                      'projects/<project>/secrets/<name> (a /versions/<n> ' +
                      'suffix is optional).'), 'STS-KEYS-0046');
    }
    ...
    if (!payload) {
      throw errorCodes.mark(new Error('the secret version "' + name + '" ' +
          'carries no payload'),
                            'STS-KEYS-0048');
    }
...
// elsewhere. `keys.kekVault` is the vault URL and `keys.kekRef` the secret
// name.
// ---------------------------------------------------------------------------
const azureProvider = {
  id: 'azure',
  ...
  read: async function (spec) {
    ...
    const name = at.where;
    if (!vault || !name) {
      throw errorCodes.mark(new Error(secret.provider + ' is "azure" and it ' +
                                                        'needs both ' +
    ...
    if (!answer || !answer.value) {
      throw errorCodes.mark(new Error('the secret "' + name + '" in ' + vault +
          ' ' +
          'has no value'),
                            'STS-KEYS-0048');
    }
...
function certAuthMount() {
  ...
  if (!raw || !/^[A-Za-z0-9._\-\/]+$/.test(raw) ||
      /(^|\/)\.\.?(\/|$)/.test(raw)) {
    throw errorCodes.mark(new Error('keys.vaultCertAuthMount is "' + raw +
                    '", ' +
                    'which is not a mount path this service will put into a ' +
                    'request. Use letters, digits, "-", "_", "." and "/" ' +
                    'only — the path the cert auth method was enabled at, ' +
                    '`cert` by default.'),
                          'STS-KEYS-0052');
  }
  log.debug("Leaving certAuthMount().");
  return raw;
}
...
async function vaultConnect(spec) {
  ...
  if (!issued) {
    throw errorCodes.mark(new Error('the secret store accepted the client ' +
                    'certificate and returned no token, which is a store ' +
                    'with the `cert` auth method enabled at ' +
                    'auth/' + mount + ' and no ' +
...
const vaultProvider = {
  ...
  read: async function (spec) {
    ...
    const path = at.where;
    if (!path) {
      throw errorCodes.mark(new Error(secret.provider + ' is "vault" and ' +
                      secret.ref +
    ...
    if (!data) {
      throw errorCodes.mark(new Error('the Vault path "' + path + '" ' +
          'returned no data'),
                            'STS-KEYS-0048');
    }
    // KV v2 nests: { data: { data: { value: ... } } }. KV v1 does not.
    const holder = (data.data && typeof data.data === 'object') ? data.data :
                    data;
    const value = holder[field];
    if (value === undefined || value === null || value === '') {
      throw errorCodes.mark(new Error('the Vault path "' + path + '" has no "' +
                      field + '" ' +
                      'field. It holds: ' + Object.keys(holder).join(', ') +
                      '. Set ' + secret.field + ' to one of those.'),
                            'STS-KEYS-0048');
    }
...
function providerFor(id) {
  ...
  log.debug("Leaving providerFor().");
  return PROVIDERS.filter(function (one) { return one.id === wanted; })[0] ||
         null;
}
...
function tagReadFailure(spec, err) {
  ...
  try {
    ...
    log.debug('tagReadFailure(): the message could not be tagged: ' +
              e.message);
  }
...
async function readDatabasePassword() {
  ...
  if (!text) {
    throw new Error(errorCodes.tag('STS-KEYS-0053') +
                    ...
                    config.value(DATABASE_PASSWORD.provider) + '" and what ' +
                    'it read is empty. An empty password is not a password, ' +
                    'and this service will not dial a database with one ' +
                    'rather than tell you the secret is blank.');
  }
...
function probeTimeoutMs() {
  log.debug("Entering probeTimeoutMs().");
  const ms = Number(config.value('keys.storeProbeTimeoutMs'));
  log.debug("Leaving probeTimeoutMs().");
  return (ms > 0) ? ms : 5000;
}
    ...
    return [
      ...
      runProbe('members', 'Whether the file holds a JSON object, and which ' +
        ...
        try {
          ...
          log.debug("Caught in a callback in store(): " +
                    ((e && e.message) || e));
    ...
    return [
      runProbe('field', 'Whether the member this secret is configured to ' +
        ...
        try {
          ...
          log.debug("Caught in a callback in secret(): " +
                    ((e && e.message) || e));
  ...
  secret: function (spec, session) {
    ...
    return [
      runProbe('versions',
        ...
        const client = (await connect(session, spec)).client;
        const answer = await client.request({ path: '/' + meta,
                                              method: 'GET' });
        const d = (answer && answer.data) || {};
        return {
          metadataPath: meta,
...
function metadataPathFor(readPath) {
  log.debug("Entering metadataPathFor().");
  const path = String(readPath || '');
  if (path.indexOf('/data/') < 0) {
    log.debug("Leaving metadataPathFor().");
    return null;
  }
  log.debug("Leaving metadataPathFor().");
...
function connectionKey(spec) {
  log.debug("Entering connectionKey().");
  log.debug("Leaving connectionKey().");
...
function connectUnauthenticated(session, spec) {
  log.debug("Entering connectUnauthenticated().");
  ...
  log.debug("Leaving connectUnauthenticated().");
...
function storeKeyFor(spec, provider) {
  log.debug("Entering storeKeyFor().");
  if (provider.id === 'file') {
    log.debug("Leaving storeKeyFor().");
...
function storeLabelFor(spec, provider) {
  log.debug("Entering storeLabelFor().");
  if (provider.id === 'file') {
    log.debug("Leaving storeLabelFor().");
