...
function maxRequested() {
  log.debug("Entering maxRequested().");
  const count = Number(config.value('oid4vp.maxRequestedClaims'));
  log.debug("Leaving maxRequested().");
...
function expectedVct() {
  log.debug("Entering expectedVct().");
  log.debug("Leaving expectedVct().");
...
// here rather than in vc_verifier.js because the query is built here and the
...
// vc_claims.js's, and the failure when they drifted would be silent: the
...
function titleFor(claimName) {
  log.debug("Entering titleFor().");
  const words = String(claimName).split('_');
  log.debug("Leaving titleFor().");
...
// put in a credential (/admin/vc, vc_claims.js's selection). Asked by the page
// and answered here rather than there, so that admin.js keeps rendering and
// deciding nothing: "you are asking for a claim this issuer is not currently
// minting" is a statement about two configurations, and both of them live on
// this side.
...
const FORMATS = [
  { id: 'dc+sd-jwt', label: 'SD-JWT VC', claimsAt: 'top',
    ...
    get identifierText() {
      log.debug("Entering identifierText().");
      log.debug("Leaving identifierText().");
      return expectedVct();
    },
...
// character a query string spells a space with, so `?format=dc+sd-jwt` reaches
// a handler as "dc sd-jwt" — every link that names that format has to
// percent-encode it, and the ones on the bar door did not. That was invisible
// while an unrecognised format fell back to a constant that happened to be this
// one; it stopped being invisible the moment the fallback became configuration,
// since a button saying "present an SD-JWT VC" would have asked for whatever
// the default was. The links are fixed AND this accepts the un-encoded
// spelling, because a QR code somebody saved, or a wallet that built the link
// itself, is not something this service can go back and correct.
function formatById(id) {
  log.debug("Entering formatById().");
  const wanted = String(id == null ? '' : id).trim().split(' ').join('+');
  log.debug("Leaving formatById().");
...
// target of Reset rather than the catalogue's defaults, so that a deployment
// that configured the variable gets ITS list back and not somebody else's. A
// FUNCTION, because oid4vp.claims is settable at runtime: Reset has to mean
// "back to what this deployment configured", and a constant captured at require
// time would mean "back to what it configured when the process started" — which
// stops being the same sentence the moment /admin/config is used. It returns a
// fresh array each call, so no caller can mutate it.
function defaultRequested() {
  log.debug("Entering defaultRequested().");
  log.debug("Leaving defaultRequested().");
...
function requestedRows() {
  log.debug("Entering requestedRows().");
  log.debug("Leaving requestedRows().");
...
function isRequested(claimName) {
  log.debug("Entering isRequested().");
  log.debug("Leaving isRequested().");
...
function setRequested(names) {
  log.debug("Entering setRequested(). " + (names || []).length + " name(s) " +
      "offered.");
  const errors = [];
  const wanted = [];
  const seen = new Set();
  parseNames(names).forEach(function (name) {
    ...
    if (!CLAIM_NAME.test(name)) {
      errors.push('"' + name + '" is not a claim name this service will put ' +
                  'in a DCQL path. Letters, digits, hyphen and underscore, ' +
                  'starting with a letter or an underscore — and no dots, ' +
                  'because the unit of disclosure is the top-level claim and ' +
                  'a nested path would name a claim no credential here ' +
                  'carries.');
      return;
    }
    if (seen.has(name)) {
      // Silently, because a duplicate is not a mistake worth an error message:
      // two identical DCQL paths would be one request asking for the same claim
      // twice.
      return;
    }
    seen.add(name);
  ...
  if (errors.length) {
    log.debug("Leaving setRequested(). " + errors.length + " error(s); " +
        "nothing changed.");
  ...
  const added = state.requested.filter(function (
      name) { return before.indexOf(name) < 0; });
  const removed = before.filter(function (name) {
  ...
  log.debug("Leaving setRequested(). " + state.requested.length + " claim(s) " +
      "requested.");
  return { ok: true, requested: requestedClaims(), added: added,
           removed: removed };
}
...
function removeRequested(name) {
  ...
  const result = setRequested(state.requested.filter(
      function (claim) { return claim !== wanted; }));
...
function defaultFormatId() {
  log.debug("Entering defaultFormatId().");
  log.debug("Leaving defaultFormatId().");
  return state.format;
}
...
// written out in three places and the fallback is now configuration rather than
// a constant.
...
// only two claims this Verifier could ask for before this page existed — and
// does not for `birthdate` (`birthDate` in the context) or for `address`, which
// is not a term at all: an ldp_vc credential carries `streetAddress`,
// `locality`, `region` and `country` flat, because that is how the context
// defines them.
//
...
// ldpOmitted() is what the page and the Verifier's own text say it with, since
// a request that silently asked for less than the page showed would make
// selective disclosure look broken in the one format whose selective disclosure
...
function pathsFor(format, claimName) {
  ...
  if (!row) {
    ...
    // as written, under credentialSubject: this is the "ask for a claim that is
    // not there" case, and the honest thing is to ask for exactly what was
    // typed.
    log.debug("Leaving pathsFor().");
    return [['credentialSubject', claimName]];
  }
  log.debug("Leaving pathsFor().");
  return row.ldpTerms.map(function (term) {
...
// cannot be two implementations that drift. vc_verifier.js's vpDcqlQuery() is
...
function dcqlQuery(formatId) {
  ...
  // and the difference is the reason this is an `if` rather than an assignment:
  // DCQL reads an absent `claims` as "the whole credential", which is the
  // opposite of what this Verifier is for — so a configuration naming no claims
  // sends no member, and every page that shows this says so in as many words
...
// path" at all: a path is not a property of the claim, it is a property of the
// claim AND the format, and a column that quietly picked one format would be
// wrong two-thirds of the time.
function dcqlPathsFor(formatId, claimName) {
  log.debug("Entering dcqlPathsFor().");
  const format = formatById(formatOf(formatId));
  log.debug("Leaving dcqlPathsFor().");
...
function dcqlClaims(formatId) {
  ...
  log.debug("Leaving dcqlClaims(). " + out.length + " path(s) for " +
      state.requested.length + " " +
      "claim(s).");
  return out;
}

// The requested claims an ldp_vc query cannot carry: in the catalogue, and with
// no term in the vendored context. Not the same as "unknown" — an unknown claim
// is asked for as written (see pathsFor), because that is the negative somebody
// configured on purpose.
function ldpOmitted() {
  log.debug("Entering ldpOmitted().");
  log.debug("Leaving ldpOmitted().");
...
module.exports = {
  ...
  get MAX_REQUESTED() {
    log.debug("Entering MAX_REQUESTED().");
    log.debug("Leaving MAX_REQUESTED().");
    return maxRequested();
  },
