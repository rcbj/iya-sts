...
//
// Kept apart from vc_issuer.ts because it is asked about from both directions —
...
// vc_configs.ts to decide per configuration and is read by vc_issuer.ts in
// turn, which is why the configuration registry is its own module: with the
// registry inside vc_issuer.ts these two would require each other.
// ---------------------------------------------------------------------------
...
interface VcDidDeps {
  ...
  VCI_CONFIGS: Record<string, { format?: string; issuerDid?: boolean }>;
}
...
class VcDid {
  ...
  //     fetched the DEFAULT realm's document and the signature failed against
  //     the wrong key — or, worse, two realms claimed one DID.
  //   * `global.publicBaseUrl` and `global.trustProxy` were ignored, so behind
  //     a proxy the DID named the last hop while the issuer identifier beside
  //     it named the public origin.
  //   * A base URL WITH A PATH has a did:web of its own shape. The did:web
  //     method specification turns path segments into `:`-separated components
  //     —
  ...
  stsDid(req?: any): string {
    ...
    if (!req || !req.get) {
      log.debug("Leaving VcDid.stsDid().");
      return 'did:web:' + ('localhost:' + PORT).replace(/:/g, '%3A');
    }
    const parts = this.didWebPartsOf(baseUrlOf(req));
    log.debug("Leaving VcDid.stsDid().");
    return 'did:web:' +
  ...
  didWebPartsOf(base: unknown): { host: string; segments: string[] } {
    ...
    log.debug("Leaving VcDid.didWebPartsOf().");
    return {
  ...
  private didSigner(): { alg: string; key: any; kid: string } {
    const { log, config, STS, signingKeyFor } = this.deps;
    log.debug("Entering VcDid.didSigner().");
    const alg = String(config.value('oid4vci.credentialSigningAlgorithm') ||
                       'RS256');
    if (alg === 'RS256') {
      log.debug("Leaving VcDid.didSigner(). RS256.");
      return { alg: 'RS256', key: STS.privateKey, kid: STS.kid };
    }
    const signer = signingKeyFor(alg);
    log.debug("Leaving VcDid.didSigner(). " + alg + ".");
  ...
  // which is then a third method, below) and bbs-2023 Data Integrity proofs
  ...
  async stsDidDocument(req?: any): Promise<any> {
    ...
    const did = this.stsDid(req);
    // The same RSA public key the JWKS publishes, read out of the certificate
    // the same way — one source of truth for what this issuer signs with, so a
    // DID document and a JWKS can never describe different keys.
    const pub: any = forge.pki.certificateFromPem(STS.certPem).publicKey;
    ...
    // whose `oid4vci.credentialSigningAlgorithm` names a curve algorithm signs
    // its DID-named credentials and its Domain Linkage Credential with that
    // curve key, and a verifier resolving this DID must find it here. Its
    // public JWK is read off the realm's key set — the same object
    ...
    if (signer.alg !== 'RS256' && signer.kid !== STS.kid) {
      ...
      if (extra) {
        ...
        log.error(errorCodes.tag('STS-VC-0043') +
                  ...
                  'is not in this realm\'s key set, so the DID document ' +
                  'cannot publish it.');
      }
    }
    try {
      ...
      log.debug("Caught in VcDid.stsDidDocument(): " +
                ((e && e.message) || e));
      // The BBS half is optional here: an ldp_vc issued while it is
      // unavailable would fail earlier and louder than a missing verification
      // method.
  ...
  // Is this request's DID one with a PATH — a trust realm, or a pinned base
  // URL with a path in it? Such a DID resolves to `<base>/did.json` rather
  // than to the well-known location.
  private didHasPath(req: any): boolean {
    const { log, baseUrlOf } = this.deps;
    log.debug("Entering VcDid.didHasPath().");
    log.debug("Leaving VcDid.didHasPath().");
    return this.didWebPartsOf(baseUrlOf(req)).segments.length > 0;
  }

  // did:web resolution is a plain GET of this document. no-store for the same
  // reason the JWKS is: the keys it describes are regenerated on every start
  // in development mode, so a cached copy outlives them — and every document
  // that describes a key is no-store in either mode (root CLAUDE.md).
  private async sendDidDocument(req: any, res: any): Promise<void> {
    const { log, logArtifact } = this.deps;
    log.debug("Entering VcDid.sendDidDocument(), the did:web document " +
              "endpoint.");
    const doc = await this.stsDidDocument(req);
    logArtifact('DID Document', 'as served', doc);
    res.set('Cache-Control', 'no-store');
    res.status(200)
       .type('application/did+json')
       .send(JSON.stringify(doc, null, 2));
    log.debug("Leaving VcDid.sendDidDocument(), the did:web document " +
              "endpoint. " +
        doc.verificationMethod.length + " method(s).");
  }

  // ---------------------------------------------------------------------------
  // Well Known DID Configuration (DIF) — /.well-known/did-configuration.json
  //
  // This is the document that answers the question the other two cannot. The
  // DID document says "this DID has these keys". The credential says "I was
  // issued by did:web:sts%3A8081". Neither says why anyone should believe that
  // DID is the same entity as the https issuer the wallet discovered, and for
  // did:web the circularity is the trap: the DID resolves by fetching the very
  // origin whose claim is in question, so "the DID document says so"
  // establishes nothing extra.
  //
  ...
  // assertionMethod, and require that credentialSubject.origin is the origin
  // the document came from. On success the origin's controller and the DID's
  ...
  // verify it; and the LD form would need a JsonWebSignature2020 over
  // URDNA2015 canonicalization, which is real work for no additional teaching.
  //
  ...
  //
  // Note the origin here is whatever this container is reached at, http
  // included. The spec assumes https; the stacks here are https by default,
  ...
  async domainLinkageCredential(req?: any): Promise<string> {
    ...
    // credential is self-issued by definition — nobody else is in a position
    // to say which origin a DID controls. `id` is deliberately absent at the
    ...
    // The configured credential algorithm — see didSigner() — with the kid
    // still a DID URL into this DID's own document, which is what the DIF
    // specification requires of the JWT form.
    ...
    const token = stsCrypto.signJws({ iss: did, sub: did, nbf: now, exp: exp,
      ...
      noTimestamp: true,
      header: { alg: signer.alg, kid: did + '#' + signer.kid,
                typ: undefined }
    });
    logArtifact('Domain Linkage Credential', 'after signing (JWT form)',
                token);
    log.debug("Leaving VcDid.domainLinkageCredential(). did=" + did +
              ", origin=" + origin);
    return token;
  }
  ...
  // something it signed" and "does an origin vouch for this DID" — and both
  // need an artifact to check. Handing back a DID with no credential signed by
  // it would make a test that could only assert the document parsed.
  //
  // Two methods, because they exercise genuinely different code in the wallet:
  //
  //   jwk  the identifier IS the key, so it resolves with NO network call.
  //        Generated fresh per request from a new P-256 key, and the
  //        credential is signed with that key (ES256) — so verifying it
  //        exercises the local-decode path end to end and cannot pass by
  //        accident against this service's RSA key.
  //   web  this service's own did:web, whose document is served at
  //        /.well-known/did.json and whose domain linkage is at
  //        /.well-known/did-configuration.json. The credential is signed with
  //        the RS256 key that document publishes.
  //
  // did:key is deliberately absent: encoding one needs multicodec varints and
  // base58btc, which live in the wallet (client/src/did.js) and would be a
  // second implementation here — exactly the kind of duplication that lets two
  // encoders agree with each other and with nobody else.
  // ---------------------------------------------------------------------------
  private generatedDidJwk(): any {
    ...
    // bytes, so a different order is a different DID for the same key.
    ...
    log.debug("Leaving VcDid.generatedDidJwk(). did=" + did.slice(0, 40) +
              "…");
  ...
  // An SD-JWT VC naming `issuerDid` as its iss and signed with the key that
  // DID publishes. Minimal on purpose — one Disclosure and a cnf — because
  // what is being tested is the SIGNATURE against a resolved document, not the
  // credential's shape. `privateKey` is whatever jsonwebtoken will sign with:
  // the ONE caller that names the STS key passes the parsed KeyObject that
  // helpers.js holds, and the one that generates a throwaway did:jwk key
  // passes that key's PEM, because it has never been parsed and is used
  // exactly once. Both are valid here and the parameter is named for the pair
  // rather than for either.
  private credentialSignedBy(issuerDid: string, privateKey: any, alg: string,
                             kid?: string): string {
    ...
    const header: Record<string, unknown> = { alg: alg, typ: 'dc+sd-jwt' };
    if (kid) {
      header.kid = kid;
    }
    ...
    log.debug("Leaving VcDid.credentialSignedBy().");
    return signed + '~' + disclosure + '~';
  }

  // The DID this issuer should be named by for a given CONFIGURATION, or ""
  // when it should keep the https identifier it has always used — in which
  // case every builder behaves exactly as it did before any of this existed.
  //
  ...
  // IdentityCredentialLdpVcDid) always uses one: that is the point of it, it
  // is advertised in the metadata, and a wallet asks for it by name. The
  // startup flags are the other way, and they apply to the PLAIN
  // configurations: they are how a whole deployment says "name me by DID
  // throughout", which is what a real issuer that had gone over to DIDs would
  ...
  issuerDidFor(configId: string, req?: any): string {
    ...
    if (config.issuerDid) {
      log.debug("Leaving VcDid.issuerDidFor().");
      return this.stsDid(req);
    }
    if (config.format === 'ldp_vc') {
      log.debug("Leaving VcDid.issuerDidFor().");
      return LDP_VC_ISSUER_DID ? this.stsDid(req) : '';
    }
    log.debug("Leaving VcDid.issuerDidFor().");
  ...
  registerRoutes(app: any): void {
    ...
    app.get('/.well-known/did.json', function (req, res) {
      return self.sendDidDocument(req, res);
    });

    // -------------------------------------------------------------------------
    // THE did:web LOCATION FOR A DID WITH A PATH (2026-09-12). The method
    // specification resolves `did:web:example.com:realm:acme` to
    // `https://example.com/realm/acme/did.json`, so a realm's DID — see
    // stsDid() — is only resolvable if its document is there. The realm
    // middleware strips the prefix, so this route is `/did.json` and it
    // answers in the realm the request named.
    //
    // Where the DID has NO path this answers 404 with the location that does
    // resolve, rather than serving the document at a second address: a DID of
    // the bare-host shape is resolved at the well-known path and nowhere else,
    // and a document served where no resolver looks is a document that
    // disagrees with nothing until the day it does.
    // -------------------------------------------------------------------------
    app.get('/did.json', async function (req, res) {
      log.debug("Entering the path-form did:web document endpoint.");
      if (!self.didHasPath(req)) {
        log.debug("Leaving the path-form did:web document endpoint. This DID " +
                  "has no path.");
        res.set('Cache-Control', 'no-store');
        errorCodes.mark(res, 'STS-VC-0045');
        return res.status(404).type('application/json').send(JSON.stringify({
          error: 'not_found',
          error_description: 'This DID (' + self.stsDid(req) + ') has no ' +
                             'path, so did:web resolves it at ' +
            baseUrlOf(req) + '/.well-known/did.json.'
        }, null, 2));
      }
      log.debug("Leaving the path-form did:web document endpoint. Handing " +
                "over.");
      return self.sendDidDocument(req, res);
    });

    app.get('/.well-known/did-configuration.json',
            async function (req, res) {
      log.debug("Entering the DID Configuration endpoint.");
      const doc = {
        '@context': DID_CONFIGURATION_CONTEXT,
        linked_dids: [await self.domainLinkageCredential(req)]
      };
      logArtifact('DID Configuration', 'as served', doc);
      // no-store for the same reason as the DID document: the key that signed
      // this is regenerated on every start in development mode, so a cached
      // copy verifies against nothing.
      res.set('Cache-Control', 'no-store');
    ...
    app.get('/did/generate', async function (req, res) {
      ...
      if (method !== 'jwk' && method !== 'web') {
        ...
        res.status(400).type('application/json').send(JSON.stringify({
          ...
          error_description: 'method must be jwk or web. did:key is not ' +
            'generated here: encoding one needs multicodec and base58btc, ' +
            'which live in the wallet, and a second implementation would be ' +
            'free to agree only with itself.'
        }, null, 2));
        log.debug("Leaving the DID generator endpoint. Refused method " +
                  method + ".");
        return;
      }
      ...
      let body;
      if (method === 'jwk') {
        const generated = self.generatedDidJwk();
        // ---------------------------------------------------------------------
        // The DID this endpoint just minted, recorded — which is what gives it
        // a directory entry, through the observer admin_stats.js installs for
        // ldap_server.js.
        //
        // It is the weakest of the recording sites in this service and it is
        // worth being explicit about why it is here at all: nothing was
        // PRESENTED and nobody was authenticated. What happened is that this
        // service created an identity, which is the same act as seeding a
        // Kerberos principal on first sight, and an identity this service
        // created that appears in no directory is the gap this endpoint had.
        // The method says plainly that it was generated.
        //
        // The `web` branch below gets NO such call, and that is a difference
        // rather than an omission: the DID there is this service's OWN — the
        // issuer's identity, already published at /.well-known/did.json — and
        // a directory entry for it would file the issuer among the people,
        // with an invented given name and a fictional mailbox attached to the
        // thing that signs every credential.
        // ---------------------------------------------------------------------
        stats.recordAuthentication({
          presented: generated.did,
          protocol: 'W3C DID Core',
          method: 'did:jwk generated at /did/generate',
          note: 'Minted by this service on request. Nothing was presented ' +
                'and nobody was authenticated; the key pair behind it was ' +
                'generated here and is returned to the caller.'
        });
        body = {
          method: 'jwk',
          did: generated.did,
          document: generated.document,
          // The document resolves from the identifier itself, so there is
          // nothing to fetch and nothing for a caller to point a URL at.
          documentUrl: '',
          verificationMethod: generated.did + '#0',
          credential: self.credentialSignedBy(generated.did,
                                              generated.privateKeyPem,
                                              'ES256',
                                              generated.did + '#0')
        };
      } else {
        const did = self.stsDid(req);
        const signer = self.didSigner();
        body = {
          method: 'web',
          did: did,
          document: await self.stsDidDocument(req),
          // Where did:web actually resolves this DID: the well-known path for
          // a bare host, `<base>/did.json` for one with a path (a trust
          // realm).
          documentUrl: baseUrlOf(req) +
                       (self.didHasPath(req) ? '/did.json' :
                         '/.well-known/did.json'),
          didConfigurationUrl: baseUrlOf(req) +
                               '/.well-known/did-configuration.json',
          origin: baseUrlOf(req),
          verificationMethod: did + '#' + signer.kid,
          credential: self.credentialSignedBy(did, signer.key, signer.alg,
                                              signer.kid)
        };
      }
      logArtifact('generated DID', 'as returned',
                  { method: body.method, did: body.did });
      // no-store for the same reason as every other document describing these
      // keys: a jwk DID is new on every call, and the web one describes a key
      // regenerated at each start in development mode.
      res.set('Cache-Control', 'no-store');
      res.status(200).type('application/json')
