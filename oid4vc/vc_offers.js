...
function deferredTokenKey(token) {
  log.debug("Entering deferredTokenKey().");
  log.debug("Leaving deferredTokenKey().");
...
const deferredAccessTokens = {
  ...
  get size() {
    ...
    return deferredAccessTokenStore.size;
  }
};
...
function deferredReadyMs() {
  log.debug("Entering deferredReadyMs().");
  log.debug("Leaving deferredReadyMs().");
  return config.value('oid4vci.deferredReadyMs');
}

function deferredIntervalS() {
  log.debug("Entering deferredIntervalS().");
  log.debug("Leaving deferredIntervalS().");
...
function offerTtlMs() {
  log.debug("Entering offerTtlMs().");
  const seconds = Number(config.value('oid4vci.offerTtlS'));
  log.debug("Leaving offerTtlMs().");
  ...
  if (asked && asked !== configured && !mode.acceptsUnregisteredAddresses()) {
    ...
    if (allowed.indexOf(asked) < 0) {
      ...
      return { error: 'The wallet URL "' + asked + '" is neither ' +
                      ...
                      'send an offer to an address the request named. Add it ' +
...
function vciOfferUsername() {
  log.debug("Entering vciOfferUsername().");
  log.debug("Leaving vciOfferUsername().");
...
//
// `options.user` (2026-09-12) is WHO a pre-authorized offer is for. Absent, it
// is `oid4vci.offerUsername` — the H.2 story, where the issuer identified the
...
function buildCredentialOffer(req, configurationIds, mode, options) {
  ...
  if (mode === 'cross-device' || mode === 'deferred') {
    ...
    offer.grants = {
      'urn:ietf:params:oauth:grant-type:pre-authorized_code': {
        ...
        tx_code: {
          ...
          description: 'Type the ' + txCodeValue.length + '-digit code shown ' +
              'by the issuer.'
        },
        // `oid4vci.preAuthorizedPollIntervalS`, 5 by default (2026-09-12).
        interval: Number(config.value('oid4vci.preAuthorizedPollIntervalS')) ||
                  5
      }
    };
  } else {
    ...
    issuerStates.set(issuerState,
                     { configurationIds: configurationIds, expires: expires });
    offer.grants = { authorization_code: { issuer_state: issuerState } };
  }

  logArtifact('OID4VCI Credential Offer', 'as built', offer);
  log.debug("Leaving buildCredentialOffer(). mode=" + mode + ", issuer_state=" +
            issuerState +
...
app.get('/issuer', function (req, res) {
  ...
  const page = '<!DOCTYPE html>\n<html lang="en"><head><meta ' +
    ...
    'diploma</title><style>body{font-family:system-ui,-apple-system,"Segoe ' +
    ...
    '#d5d5dd;border-radius:10px;padding:30px 34px;width:520px;box-shadow:0 ' +
    ...
    '</head><body><div class="card"><h1>Mock ' +
    'University</h1><p>Congratulations on your graduation. Your diploma is ' +
    'available as a digital credential ' +
    '(<code>' + xmlEscape(configId) + '</code>) that you can keep in your ' +
    'wallet.</p><p><a class="cta" href="/issuer/offer">Request your digital ' +
    'diploma</a><a class="cta secondary" ' +
    'href="/issuer/offer?by=reference">Request it (offer by ' +
    'reference)</a></p><p class="alt">On a different device? These show a QR ' +
    'code to scan with your wallet instead:<br><a class="cta secondary" ' +
    'href="/issuer/offer?mode=cross-device">Show a QR code ' +
    '(cross-device)</a><a class="cta secondary" ' +
    'href="/issuer/offer?mode=deferred">Show a QR code (issuance takes a ' +
    'while)</a></p><div class="meta">This is the Credential Issuer\'s web ' +
    'page in OID4VCI Appendix H. The first two links build a Credential ' +
    'Offer and send you to your wallet at ' +
    '<code>' + xmlEscape(walletBaseUrl()) + '</code> ' +
    '(H.1, same device). The other two hand the offer over by QR code and a ' +
    'Transaction Code instead — H.2, and H.3 where the issuer needs time to ' +
    'produce the credential. The issuer is ' +
...
app.get('/issuer/offer', function (req, res) {
  ...
  if (preAuthorized && !mode.opensTestControls()) {
    ...
    if (!session || !session.user || !session.user.username) {
      log.debug("Leaving the credential offer endpoint. A sign-in is needed " +
                "first.");
    ...
    if (session.authenticated === false) {
      log.debug("Leaving the credential offer endpoint. The session is not " +
                "authenticated.");
      errorCodes.mark(res, 'STS-VC-0027');
      return res.status(403).type('text/plain').send(
        'A pre-authorized Credential Offer is a credential about the person ' +
        'it is made for, and this browser has not signed in — it chose to ' +
        'continue without doing so. Sign in as the person the credential ' +
        'should describe and load this page again.\n');
    }
  ...
  const now = Date.now();
  credentialOffers.forEach(function (v, k) {
    if (v.expires < now) credentialOffers.delete(k);
  });
  issuerStates.forEach(function (v, k) {
    if (v.expires < now) issuerStates.delete(k);
  });
  preAuthorizedCodes.forEach(function (v, k) {
    if (v.expires < now) preAuthorizedCodes.delete(k);
  });
  deferredTransactions.forEach(function (v, k) {
  ...
  if (String(req.query.by || '') === 'reference') {
    ...
    credentialOffers.set(id,
                         { offer: built.offer, expires: now + offerTtlMs() });
  ...
  if (built.mode !== 'cross-device' && built.mode !== 'deferred') {
    ...
    log.debug("Leaving the credential offer endpoint. Sent the End-User to " +
              wallet + ".");
    return;
  }
  ...
  log.debug("Leaving the credential offer endpoint. Displayed a QR code for " +
            "the wallet to scan.");
});
...
function renderOfferQrPage(res, opts) {
  ...
  qrcode.toDataURL(opts.offerUri,
    ...
    .then(function (dataUrl) {
      ...
      const page = '<!DOCTYPE html>\n<html lang="en"><head><meta ' +
        ...
        '.txcode{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;' +
        'font-size:2.1em;letter-spacing:.28em;font-weight:700;color:#12107c;' +
        'background:#f0f0fa;border-radius:8px;padding:12px 6px;margin:6px 0 ' +
        '2px}.uri{word-break:break-all;font-family:ui-monospace,' +
        'SFMono-Regular,Menlo,monospace;font-size:.72em;color:#555;' +
        'background:#fafafa;border:1px solid #eee;border-radius:6px;' +
        'padding:8px;text-align:left}.meta{margin-top:20px;padding-top:14px;' +
        'border-top:1px solid #eee;font-size:.78em;color:#777;' +
        'text-align:left}code{font-family:ui-monospace,SFMono-Regular,Menlo,' +
        'monospace}</style></head><body><div class="card"><h1>Scan this with ' +
        'your wallet</h1><p>Your digital diploma is ready to be claimed' +
        (deferred ? ', though issuing it will take us a little time once you ' +
                    'ask.' : '.') + '</p><img ' +
        'class="qr" id="offer_qr" alt="Credential Offer QR code" ' +
        'src="' + dataUrl + '"><p>Then ' +
        'type this Transaction Code into your wallet:</p><div class="txcode" ' +
        ...
        'straight to the token endpoint — there is no authorization request. ' +
  ...
  log.debug("Leaving renderOfferQrPage().");
}
...
app.get('/oid4vci/credential-offer/:id', function (req, res) {
  ...
  if (!record || record.expires < Date.now()) {
    ...
    return vciError(res, 404, 'invalid_request', 'No such Credential Offer, ' +
                                                 'or it has expired.');
  }
