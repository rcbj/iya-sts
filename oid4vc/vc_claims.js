...
const VC_ATTRIBUTES = [
  ...
  { ldap: 'schacDateOfBirth', claim: ['birthdate'], label: 'Date of birth',
    ...
    toClaim: function (value) {
      log.debug("Entering toClaim().");
      const digits = String(value).replace(/[^0-9]/g, '');
      if (digits.length < 8) {
        log.debug("Leaving toClaim().");
        return String(value);
      }
      log.debug("Leaving toClaim().");
...
function personField(name) {
  log.debug("Entering personField().");
  log.debug("Leaving personField().");
...
// and any name works. An ldp_vc credential is JSON-LD, signed over
// CANONICALIZED RDF, and bbs2023.js canonicalizes with `safe: true` — so a term
// the vendored context does not define is not dropped quietly, it THROWS, and
// the throw arrives at issuance time inside a cryptosuite rather than on this
// page.
//
// The vendored context (contexts/idptools_identity_v1.json) is signed over, so
// it cannot be edited to add a term without invalidating every credential
// issued against the old one — which is exactly why it is vendored rather than
// fetched. So each row names the term to use in that format or null, and
// vc_issuer.js filters the subject through the context it actually loaded. A
...
// different credentials in different formats" is a surprise if it is discovered
// rather than stated.
// ---------------------------------------------------------------------------
function ldpTermFor(row) {
  log.debug("Entering ldpTermFor().");
  log.debug("Leaving ldpTermFor().");
  return row.ldpTerm || '';
}
...
function setSelection(names) {
  log.debug("Entering setSelection(). " + (names || []).length + " name(s) " +
      "offered.");
  const errors = [];
  ...
  log.info('vc: the credential claim set is now ' +
           (selectedNames().join(', ') || '(empty)') +
           '. Added: ' + (added.join(', ') || 'nothing') + '. Removed: ' +
           (removed.join(', ') || 'nothing') + '.');
  log.debug("Leaving setSelection(). " + wanted.size +
            " attribute(s) selected.");
  return { ok: true, selected: selectedNames(), added: added,
           removed: removed };
}
...
// lines and a dependency whose only job is to be deterministic is a dependency
// whose version bump silently changes every generated value.
// ---------------------------------------------------------------------------
function randomFor(seedText) {
  log.debug("Entering randomFor().");
  const digest = crypto.createHash('sha256')
                       .update(String(seedText), 'utf8')
                       .digest();
  let state = digest.readUInt32LE(0);
  log.debug("Leaving randomFor().");
  return function () {
...
function digits(random, count) {
  log.debug("Entering digits().");
  let out = '';
  for (let i = 0; i < count; i++) {
    out += String(Math.floor(random() * 10));
  }
  log.debug("Leaving digits().");
  return out;
}
...
// consistent with each other: the email is built from the given and family
// names that are also in the entry, the postal code belongs to the country, and
// the display name is the two names in the order the entry's cn uses.
//
...
function personaFor(name) {
  ...
  // certain: there are 256 name pairs and a mock can easily see more users than
  // that.
  const mailbox = (given + '.' + family).toLowerCase() + '.' + digits(random,
                                                                      4);
  const persona = {
    username: username,
    given: given,
    ...
    // writes it. The day is capped at 28 so that no invented person is born on
    // the 30th of February — a date a strict consumer rejects, which would make
    // this service look broken for a reason that has nothing to do with
    // credentials.
    ...
    // reserved for exactly this), so no invented person is given a working
    // line.
...
function generatedFor(name) {
  ...
  log.debug("Leaving generatedFor(). " + Object.keys(out).length +
            " value(s).");
  return out;
}
...
// wallet a credential, and a store it consults is not allowed to prevent that.
// ---------------------------------------------------------------------------
let directory = null;
...
// it has to be able to say what happened: "nothing to do" and "the directory is
// not loaded" look identical from the outside and are entirely different
// answers.
  ...
  if (!directory || typeof directory.populate !== 'function') {
    ...
    return { ok: false, loaded: false, examined: 0, changed: 0, values: 0,
             errors: ['The embedded LDAP directory is not loaded, so there ' +
                      'are no entries to populate. Nothing else is affected: ' +
                      'the claims below still reach every credential, ' +
                      'generated per user.'] };
  }
  try {
    const result = directory.populate() || {};
    log.debug("Leaving populateDirectory(). " + (result.changed || 0) + " " +
        "entry/entries changed.");
...
//      is a statement this service already made about the person — it came from
//      the sign-in or from the custom claims page — and a credential that
//      contradicted the token that authorised it would be indefensible.
...
//      person has no entry yet, or when the entry does not carry that attribute
//      — and ONLY IN DEVELOPMENT MODE since 2026-09-12. A product realm
...
// selected claim that silently did not arrive would be indistinguishable, at
// the wallet, from a selection that never took effect. Product mode takes the
// other side of that trade on purpose: an absent claim is a thing a wallet
// handles, and an invented one in a SIGNED credential is a thing it believes.
// ---------------------------------------------------------------------------
...
// decoration: it is what /admin/vc shows in its preview, and "this came from
// the entry" versus "this was invented just now" is the difference between a
...
function valueFor(row, name, tokenClaims, attributes, persona) {
  ...
  // object of this shape, and a token that carried a partial one would produce
  // an address assembled out of two sources — which is worse than either.
  ...
  if (stored && stored.length && String(stored[0]) !== '') {
    ...
    // picking the first is the only rule that does not depend on insertion
    // order being meaningful — which it is not, in this store or in a real
    // directory.
  ...
  // ---------------------------------------------------------------------
  if (!mode.inventsClaimValues()) {
    log.debug("Leaving valueFor(). Nothing on the entry, and this realm " +
              "invents nothing.");
    return null;
  }
...
// with where each came from. `rows` is which of the selected rows to build, and
// it exists for one caller: a wallet that asked for a SUBSET of the claims in
// its authorization_details (OID4VCI section 5.1.1). Absent means all of them,
// which is what every other caller wants and what an authorization carrying no
// `claims` member means.
...
function metadataClaims(prefix) {
  ...
  log.debug("Leaving metadataClaims(). " + out.length +
            " claim(s) advertised.");
  return out;
}

// The same, for ldp_vc: only the rows whose term the vendored context defines,
// and FLAT — the context defines `streetAddress` and `locality` as terms of
// their own, not as members of an `address` object, so that is where they go.
function ldpMetadataClaims() {
  ...
  log.debug("Leaving ldpMetadataClaims(). " + out.length + " claim(s) " +
      "advertised.");
  return out;
}
...
// when this format cannot carry it at all (ldp_vc, whose context defines no
// term for it). The mirror image of advertisedClaims(), and the two must agree:
// a row whose path here is absent from the metadata would be requestable and
// never issued.
...
function rowsForPaths(paths, format) {
  log.debug("Entering rowsForPaths(). " + (paths || []).length + " path(s), " +
      "format=" + format);
...
function unknownPaths(paths, format) {
  ...
  const advertised = new Set(advertisedClaims(format).map(
      function (c) { return pathKey(c.path); }));
  const out = (paths ||
               []).filter(function (path) {
                 return !advertised.has(pathKey(path));
               });
...
// any term its loaded context turns out not to define — see the LDP note above;
// the list here is derived from that file by hand and the caller checks it
...
function ldpSubjectFrom(claims) {
  ...
  log.debug("Leaving ldpSubjectFrom(). " + Object.keys(out).length + " " +
      "member(s).");
  return out;
}
