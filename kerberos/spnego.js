...
function page(title, inner) {
  ...
  return '<!DOCTYPE html>\n<html lang="en"><head><meta charset="utf-8">' +
    ...
    '18px}table{border-collapse:collapse;width:100%;margin:.5rem 0 ' +
    ...
    'margin-bottom:12px}pre{background:#f4f4f8;border:1px solid #e2e2ea;' +
...
app.get('/spnego', function (req, res) {
  ...
  if (String(req.query.format || '').toLowerCase() === 'json') {
    ...
    return res.status(200).json({
      ...
      // host matches one of these, and the KDC registers it on first sight, so
      // a client's own guess works. NOTHING IN SPNEGO CARRIES THIS: it is an
      ...
      spnHostRule: 'a host matches when it IS one of acceptsAnySpnForHosts ' +
        'or ends with a dot and one of them',
  ...
  const inner = '<h1>A SPNEGO-protected page lives here</h1><p ' +
    'class="sub">Kerberos over HTTP: RFC 4559 carries RFC 4178, which ' +
    'carries RFC 4121, which carries the AP-REQ.</p><p><a ' +
    'href="/spnego/protected"><strong>/spnego/protected</strong></a> answers ' +
    '<code>401</code> with <code>WWW-Authenticate: Negotiate</code> to ' +
    'anyone who asks for it without a ticket. Your browser will most likely ' +
    'fail that: Chrome and Firefox use Negotiate only for hosts on an ' +
    ...
    'shows you both halves.</p><h2>And the same handshake as a ' +
    'sign-in</h2><p><a ' +
    'href="/authn/spnego"><strong>/authn/spnego</strong></a> performs ' +
    ...
    'request, a <code>wsignin1.0</code>, a SAML <code>AuthnRequest</code> ' +
    'and the admin console all complete without anybody typing anything, ' +
    'because every protocol here reads the one session.</p><p>That is the ' +
    'difference between the two doors, and it is the only difference: this ' +
    'one proves a handshake and stops, that one acts on it. It is available ' +
    'to every application and registered for none &mdash; a button on the ' +
    'sign-in screen, <code>appAuthnMechanism: spnego</code> on an ' +
    'application entry, or <code>fedAuthnMechanism: spnego</code> on a ' +
    ...
    (config.value('krb5.spnegoAuthentication')
       ...
       : '<strong>It is switched off right now</strong> ' +
         ...
         'says so. ') +
    'The three knobs below are this page&rsquo;s alone: a sign-in door with ' +
    'a query parameter that makes it fail in an instructive way is a footgun ' +
    'rather than a lesson.</p><h2>What a client needs before it can get ' +
    'in</h2><table><tr><th>Thing</th><th>Value</th></tr><tr><td>Service ' +
    'principal name</td><td><code>' +
...
const HEADINGS = {
  'no-authorization':        ['Authentication required', '401 &mdash; ' +
      'authentication required'],
  'wrong-scheme':            ['Wrong scheme', '401 &mdash; wrong ' +
                                              'authentication scheme'],
  'empty-token':             ['Empty token', '401 &mdash; an empty Negotiate ' +
                                             'token'],
  'no-mech-token':           ['Send a token', '401 &mdash; send the ' +
                                              'mechanism&rsquo;s token'],
  'ticket-refused':          ['Ticket refused', '401 &mdash; the ticket was ' +
                                                'refused'],
  'request-mic':             ['Send the MIC',
                              '401 &mdash; send the mechListMIC'],
  ...
  switch (verdict.code) {
    case 'no-authorization':
      html = '<p>This resource is protected with <code>Negotiate</code>. The ' +
        'challenge above carries <strong>no token</strong>, which is RFC ' +
        '4559 section 4: the server says only that it will negotiate, and ' +
      ...
      break;
    case 'wrong-scheme':
      html = '<div class="err">This resource speaks <code>Negotiate</code>. ' +
        'The request offered ' +
        '<code>' + xmlEscape(verdict.scheme) + '</code>.</div>';
      break;
    case 'empty-token':
    ...
    case 'undecodable':
      html = '<p>The bytes after <code>Negotiate </code> must be a SPNEGO ' +
        'negotiation token (RFC 4178) or a bare Kerberos GSS token (RFC ' +
        '4121).</p>';
      break;
    case 'no-common-mechanism':
      html = '<p>The client offered ' + verdict.offered.length + ' ' +
          'mechanism(s): ' +
      ...
      break;
    case 'no-mech-token':
      html = '<p>The <code>NegTokenInit</code> carried a mechanism list and ' +
        'no <code>mechToken</code>, so this reply selects <code>' +
        xmlEscape(spnego.mechName(verdict.selected)) + '</code> and asks for ' +
        'one: <code>accept-incomplete</code>. That is the round trip the ' +
        '&ldquo;optimistic&rdquo; token exists to avoid.</p>';
      break;
    case 'non-kerberos-mechanism':
      html = '<p>Selected <code>' + xmlEscape(verdict.selected) +
             '</code>.</p>';
      break;
    case 'ticket-refused':
      html = '<div class="err">The negotiation selected Kerberos and the ' +
        'AP-REQ did not pass.</div><p>The reason is in the ' +
        '<code>responseToken</code> of the <code>WWW-Authenticate</code> ' +
        'header, as a <code>KRB-ERROR</code>. SPNEGO itself carries no error ' +
        'detail at all, so a client that reads only <code>negState</code> ' +
        'learns nothing but &ldquo;no&rdquo;.</p>';
      break;
    case 'bad-mech-list-mic':
      ...
      html = verdict.continuation
        ? '<p>Computed over the DER of the <code>MechTypeList</code> &mdash; ' +
          'the SEQUENCE, not the <code>[0]</code> wrapper (RFC 4178 section ' +
          '5).</p>'
        : '<p>The client&rsquo;s ticket was perfectly good. What failed is ' +
          'the integrity check over the <em>mechanism list</em> &mdash; so ' +
          'either the list was altered in transit, or the client computed ' +
          'the MIC over the wrong bytes. The commonest cause of the second ' +
          'is signing <code>[0] MechTypeList</code> rather than ' +
          '<code>MechTypeList</code>: two bytes, and RFC 4178 section 5 ' +
          'spells the distinction out because implementations kept getting ' +
          'it wrong.</p>';
      break;
    case 'mic-required':
    ...
    case 'request-mic':
      html = '<p>The ticket was accepted, and this reply is ' +
        ...
        'acceptor&rsquo;s first reply (RFC 4178 section 4.2.2).</p><p>Answer ' +
        'with a bare <code>NegTokenResp</code> carrying only the ' +
        '<code>mechListMIC</code>, computed over the DER of the ' +
        '<code>MechTypeList</code> with the subkey from your ' +
        'Authenticator.</p>';
      break;
    case 'no-pending-continuation':
      html = '<p>A bare <code>NegTokenResp</code> arrived, but nothing here ' +
        'is waiting for one. A real acceptor keeps this state on the ' +
    ...
    case 'continuation-no-mic':
      html = '<p>The previous reply was <code>request-mic</code>, so the ' +
        'only thing this token needed was the MIC.</p>';
      break;
    default:
      html = '';
      break;
...
function acceptedPage(verdict) {
  ...
  const inner = '<h1>' + HEADINGS.accepted[1] + '</h1>' +
    ...
    '<tr><td>Mutual authentication</td><td>' + (verdict.mutual
      ...
      : verdict.mutualOff
        ? '<span class="fail">none</span> &mdash; <code>?mutual=off</code> ' +
          'is set, so nothing has proved this server is who it claims to be'
...
function refusalPage(verdict) {
  ...
  const inner = '<h1>' + heading[1] + '</h1>' +
    ...
    (verdict.checks ? '<h2>What this service checked</h2>' +
      checksTable(verdict.checks) : '') +
    (rejected
      ? '<p class="sub">SPNEGO&rsquo;s <code>reject</code> carries no reason ' +
        'of its own &mdash; the structure has no field for one. Everything ' +
        'above this line is out of band, and a real server tells you none of ' +
        'it.</p>'
      : '');
