...
// so a key made from an OLD password is never used after the password changes,
// however the change arrived. An LDAP modify of `userPassword` included, which
// never passes through the observer at all and is caught by the stamp at the
// next AS-REQ and repaired at the next sign-in.
//
...
// carrying the NAME, the REALM, the KVNO, the password STAMP and every enctype's
// key — SEALED as a whole under the key-encryption key wherever that key
// outlives the process (`keystore.persists()`, which is `writeTotpRecord()`'s
// rule and for its reason: development's key is ephemeral, and a directory
// attribute sealed under it is permanent garbage after a restart).
//
...
//
// The KDC's sockets and `krb5.realm` are PROCESS-WIDE: a raw Kerberos socket has
// no path to put a trust-realm segment in, and the realm name inside the
// protocol is the KERBEROS realm. So the KDC answers in no trust realm, and this
// file reads and writes the **DEFAULT trust realm's directory** — the
...
//
// A LIBRARY (rule 3): it registers no route. It requires the principal database,
// the codec, the keytab writer and six `common/` leaves — none of which requires
// it back. Two slots point INTO it and two OUT of it:
//
...
function setDirectory(hooks) {
  ...
  if (missing.length) {
    ...
    log.error(errorCodes.tag('STS-KRB-0110') +
              ...
              missing.join(', ') + ', so it was refused whole. No person will ' +
              'get Kerberos keys and no service principal can be stored.');
...
function openRecord(value) {
  ...
  try {
    ...
    if (!record || record.v !== RECORD_VERSION || typeof record.keys !== 'object') {
...
//   * ONE WRITE. A password change retires the old current key and stores the new
//     one atomically; two attributes would have a moment holding the new key and
//     no previous version, or the reverse.
//   * NO SCHEMA CHANGE. `stsKrb5Keys` and `krb5ServiceKeys` are already withheld
//     everywhere, sealed, and on the schema rows a sighting preserves; a third and
//     fourth attribute would have to be added to all of that and would be the
//     one a future change forgot. `RECORD_VERSION` stays 1: `previous` is
//     optional, a record without it has none, and an older reader ignores it.
//
// **A PREVIOUS VERSION IS NEVER A WAY IN.** It is handed to the KDC for ONE
// purpose — decrypting a ticket already sealed under it — and pre-authentication
// reads the current keys only (`krb5_principals.js`'s `directoryUser()` puts
// those, and only those, in the key cache). An old password cannot sign in.
//
// **THE BOUNDS ARE APPLIED ON READ AS WELL AS ON WRITE.** A version's expiry is
// the EARLIER of the one stamped when it was retired and its retirement plus the
// lifetime in force NOW, so shortening the setting ends windows at once and
// lengthening it never resurrects one; the count is re-applied too. What is past
// either bound is never used and never listed, and the next write of that key
...
function withinBounds(list, nowMs) {
  ...
  return list.filter(function (entry) {
    return entry && Number.isFinite(Number(entry.kvno)) && retainedUntilMs(entry) > nowMs;
  }).sort(function (a, b) {
...
function retire(outgoing, newKvno, nowMs) {
  ...
  if (!outgoing || Number(outgoing.kvno) === Number(newKvno) || !retainedVersionsLimit()) {
    return kept;
  }
  const entry = {
...
function retainedRows(infoRetained, nowMs) {
  return withinBounds(infoRetained, nowMs).map(function (entry) {
    return {
      kvno: Number(entry.kvno),
      etypes: (Array.isArray(entry.etypes) ? entry.etypes : []).map(function (etype) {
...
function personKeys(name) {
  ...
  if (record.name !== name || record.realm !== principals.REALM) {
    ...
    log.debug('Leaving personKeys(). Bound to ' + record.name + '@' + record.realm + '.');
    return { state: 'unknown', detail: 'the stored keys are bound to another name' };
  }
...
function serviceKeys(spn) {
  ...
  return keys.length
    ? { kvno: record.kvno, keys: keys, retained: retainedForKdc(record, Date.now()) }
    : null;
}
...
function observePassword(name, password, info) {
  ...
  const next = previous.then(function () {
    ...
    log.error(errorCodes.tag('STS-KRB-0107') + 'krb5-keys: deriving the ' +
              'Kerberos keys for ' + name + ' failed: ' + (e.stack || e.message));
...
async function derive(name, password, event) {
  ...
  if (event !== 'set' && record && record.name === name && record.stamp === stamp &&
      wanted.every(function (etype) { return typeof record.keys[etype] === 'string'; })) {
  ...
  if (!again || again.passwordHash !== current.passwordHash) {
    log.info('krb5-keys: the password for ' + name + ' changed while its ' +
             'Kerberos keys were being derived, so these were thrown away; the ' +
  ...
  // THE OUTGOING RECORD IS THE ONE ON THE ENTRY NOW, not the one read before the
  // derivation: an operator's "drop previous versions" may have landed in
  ...
  if (!sealed) {
    log.error(errorCodes.tag('STS-KRB-0108') + 'krb5-keys: the Kerberos keys ' +
              'for ' + name + ' could not be sealed, so they were NOT stored. ' +
              'Storing them in the clear would put a password-equivalent key in ' +
              'every directory dump.');
    audit.failure('STS-KRB-0108', {
      action: 'service.failure', protocol: 'Kerberos', channel: 'internal',
      target: name + '@' + principals.REALM, outcome: 'error',
      summary: 'Kerberos keys for ' + name + ' could not be sealed and were not stored'
    });
    log.debug('Leaving derive(). Not sealed.');
    return { derived: false, why: 'seal' };
  }
  const infoValue = JSON.stringify({ kvno: kvno, etypes: wanted, derivedAt: derivedAt,
  ...
  if (!directory.writePerson(name, sealed.value, infoValue)) {
    ...
    audit.failure('STS-KRB-0109', {
      action: 'service.failure', protocol: 'Kerberos', channel: 'internal',
      target: name + '@' + principals.REALM, outcome: 'error',
      summary: 'Kerberos keys for ' + name + ' could not be written to the directory'
    });
  ...
  audit.audit({
    ...
    summary: 'Kerberos keys for ' + name + ' were derived from the password just ' +
  ...
  log.info('krb5-keys: ' + name + '@' + principals.REALM + ' now has Kerberos ' +
           'keys at kvno ' + kvno + ' (' + (sealed.sealed ? 'sealed' : 'clear, ' +
           'development key store') + '), derived from the password just ' + event +
           (previous.length ? '; keeping previous kvno ' + previous.map(function (one) {
             return one.kvno;
...
function normaliseSpn(raw) {
  ...
  if (components.length < 2 || components.some(function (c) {
    ...
    return { ok: false, error: '"' + text + '" is not a service principal name: ' +
             'one is two or more non-empty components separated by "/", like ' +
             'HTTP/web.example.com, with no spaces and no "@" inside it.' };
  }
  if (components[0].toLowerCase() === 'krbtgt') {
    log.debug('Leaving normaliseSpn(). krbtgt.');
    return { ok: false, error: 'krbtgt is the ticket-granting key every TGT in ' +
             'the realm is sealed under, and it is set by krb5.krbtgtPassword ' +
...
//
// `outgoing` is the opened record a ROTATE replaces (null for a create): its key
// becomes a previous version, and **THE KEYTAB CARRIES EVERY VERSION STILL
...
function mintServiceKeys(spn, kvno, act, context, outgoing) {
  ...
  kept.forEach(function (version) {
    ...
    keyPairs(version.keys).forEach(function (pair) {
      entries.push({ realm: principals.REALM, components: spn.components,
                     ...
                     kvno: Number(version.kvno), etype: pair[0], key: pair[1] });
    });
  });
  const previous = parseInfo((directory.readService(spn.identifier) || {}).info);
  const sealed = sealRecord({ v: RECORD_VERSION, spn: spn.spn, realm: principals.REALM,
                              kvno: kvno, createdAt: now.toISOString(), keys: keys,
                              previous: kept });
  if (!sealed) {
    ...
    return refusal('STS-ADMIN-0607', 'The new key for ' + spn.spn + ' could not ' +
                   'be encrypted under the key-encryption key, so nothing was ' +
                   'stored and no keytab was made.');
  }
  ...
  if (!directory.writeService(spn.identifier, sealed.value, JSON.stringify(info))) {
    log.debug('Leaving mintServiceKeys(). Not written.');
    return refusal('STS-ADMIN-0607', 'The key for ' + spn.spn + ' could not be ' +
                   'written to its application entry, so no keytab was made.');
  }
  const bytes = keytab.writeKeytab(entries);
  audit.audit({
    action: 'admin.krb5.service.' + act, actor: String((context || {}).actor || ''),
    protocol: 'Kerberos', channel: 'internal', target: spn.identifier,
    summary: 'The Kerberos service principal ' + spn.identifier + ' was ' + act +
             ' with a random key (kvno ' + kvno + ') through the ' +
             String((context || {}).via || 'console'),
    detail: { kvno: kvno, etypes: etypes, via: String((context || {}).via || 'console'),
  ...
  const retained = retainedRows(retainedInfo(kept, now.getTime()), now.getTime());
  return {
    ok: true, act: act, spn: spn.spn, principal: spn.identifier, kvno: kvno,
    etypes: etypes, sealed: sealed.sealed, retained: retained,
    keytabKvnos: [kvno].concat(kept.map(function (one) { return Number(one.kvno); })),
    keytabFilename: spn.spn.replace(/[^A-Za-z0-9.-]+/g, '_') + '.kvno' + kvno + '.keytab',
    keytab: bytes.toString('base64'),
    message: 'The Kerberos service principal ' + spn.identifier + ' was ' + act +
             ' at kvno ' + kvno + '. Its keytab is in this answer and is not ' +
             'kept: it cannot be downloaded again, only replaced by rotating.' +
             (kept.length
               ? ' The keytab also carries the previous kvno ' + kept.map(function (one) {
                   return one.kvno;
                 }).join(', ') + ', which this KDC and its acceptor still accept ' +
                 'for tickets already issued under it until ' +
                 retained.map(function (one) { return one.expiresAt; }).join(', ') +
...
function createServicePrincipal(raw, context) {
  ...
  if (!directory) {
    ...
    return refusal('STS-ADMIN-0609', 'There is no directory in this process, so ' +
                   'there is nowhere to store a service principal\'s key.');
  }
  ...
  if (existing && existing.keys) {
    ...
    return refusal('STS-ADMIN-0604', spn.identifier + ' already holds a stored ' +
                   'key. Rotate it to get a new keytab; creating it again would ' +
                   'silently strand every service holding the current one.');
  }
  if (!existing) {
    ...
    const created = realms.run(realms.DEFAULT_REALM, function () {
      return applications.createApplication({
        identifier: spn.identifier, kind: 'kerberos-service', protocols: ['krb5'],
    ...
    if (!created.ok || !directory.readService(spn.identifier)) {
      ...
      return refusal('STS-ADMIN-0606', 'No application entry could be made for ' +
                     spn.identifier + ': ' + ((created.errors || []).join(' ') ||
                     'the directory did not hold it afterwards') + '.');
    }
  }
  const result = mintServiceKeys(spn, Number(config.value('krb5.kvno')), 'created',
                                 context);
...
function rotateServicePrincipal(raw, context) {
  ...
  if (!existing || !existing.keys) {
    ...
    return refusal('STS-ADMIN-0605', spn.identifier + ' holds no stored key to ' +
                   'rotate. Create it first.');
  }
  ...
  const outgoing = opened.ok && opened.record.spn === spn.spn ? opened.record : null;
...
function deleteServicePrincipal(raw, context) {
  ...
  audit.audit({
    action: 'admin.krb5.service.deleted', actor: String((context || {}).actor || ''),
    protocol: 'Kerberos', channel: 'internal', target: spn.identifier,
    summary: 'The stored key for the Kerberos service principal ' + spn.identifier +
             ' was deleted through the ' + String((context || {}).via || 'console'),
  ...
  return { ok: true, spn: spn.spn, principal: spn.identifier,
           message: 'The stored key for ' + spn.identifier + ' is gone. The ' +
                    ...
                    'records does; a ticket for that SPN is now keyed as it was ' +
                    'before a key was stored — from krb5.servicePassword if it ' +
                    'is the acceptor\'s own name, and not at all in product mode ' +
                    'otherwise.' };
}
...
function clearPersonKeys(name, context) {
  ...
  if (!current) {
    ...
    return refusal('STS-ADMIN-0608', 'There is nobody called "' + who + '" in ' +
                   'the default trust realm\'s directory, which is the one this ' +
                   'KDC reads.');
  }
  if (!current.keys && !current.info) {
    log.debug('Leaving clearPersonKeys(). Nothing held.');
    return { ok: true, cleared: false, username: who,
             message: who + ' holds no Kerberos keys, so there was nothing to clear.' };
  }
  if (!directory.writePerson(who, null, null)) {
    log.debug('Leaving clearPersonKeys(). Not written.');
    return refusal('STS-ADMIN-0607', 'The Kerberos keys on ' + who + '\'s entry ' +
                   'could not be removed.');
  }
  audit.audit({
    action: 'admin.krb5.keys.cleared', actor: String((context || {}).actor || ''),
    protocol: 'Kerberos', channel: 'internal', target: who + '@' + principals.REALM,
  ...
  return { ok: true, cleared: true, username: who,
           message: 'The Kerberos keys of ' + who + ' are cleared, previous ' +
                    'versions included. Their next AS-REQ is refused with "sign ' +
                    'in once"; their next verified sign-in derives new keys at ' +
                    'the next kvno.' };
}
...
//
// It REWRITES the record with `previous` empty and the current key untouched, so
// the person still signs in and the service keeps its current keytab. Somebody
// holding nothing to drop is answered `dropped: 0` rather than refused, for
// `clearPersonKeys()`'s reason. A record this process cannot OPEN is refused:
// rewriting it would mean writing a current key it cannot read.
//
...
function dropPrevious(kind, label, current, context, write) {
  ...
  if (!current || !current.keys) {
    ...
    return refusal('STS-ADMIN-0605', label + ' holds no stored Kerberos key, so ' +
                   'there are no previous versions to drop.');
  }
  const opened = openRecord(current.keys);
  if (!opened.ok) {
    log.debug('Leaving dropPrevious(). Unreadable.');
    return refusal('STS-ADMIN-0607', 'The stored Kerberos key for ' + label +
                   ' cannot be opened on this service (' + opened.why + '), so its ' +
                   'previous versions cannot be dropped without rewriting a key it ' +
                   'cannot read. Clear or rotate it instead.');
  }
  const nowMs = Date.now();
  const stored = Array.isArray(opened.record.previous) ? opened.record.previous : [];
  ...
  const info = Object.assign({}, parseInfo(current.info) || {}, { retained: [] });
  ...
  audit.audit({
    action: 'admin.krb5.previous.dropped', actor: String((context || {}).actor || ''),
    protocol: 'Kerberos', channel: 'internal', target: label,
    summary: 'The previous Kerberos key versions of ' + label + ' (' +
             (kvnos.length ? 'kvno ' + kvnos.join(', ') : 'none still in their window') +
             ') were dropped through the ' + String((context || {}).via || 'console'),
    detail: { kind: kind, kvnos: kvnos, via: String((context || {}).via || 'console') }
  });
  log.info('krb5-keys: dropped the previous key versions of ' + label + ' (' +
           (kvnos.join(', ') || 'only expired ones were left') + '); a ticket under ' +
           'any of them is now refused KRB_AP_ERR_BADKEYVER.');
  ...
  return { ok: true, dropped: kvnos.length, kvnos: kvnos, principal: label,
           ...
           message: kvnos.length
             ? 'Dropped previous kvno ' + kvnos.join(', ') + ' of ' + label + '. A ' +
               'ticket issued under ' + (kvnos.length === 1 ? 'it' : 'any of them') +
               ' is now refused KRB_AP_ERR_BADKEYVER; kvno ' + opened.record.kvno +
               ' is untouched.'
             : label + ' kept only previous versions already past their window; ' +
               'they are removed from storage now.' };
}
...
function dropPreviousPersonKeys(name, context) {
  ...
  if (!who || personNameProblem(who)) {
    ...
    return refusal('STS-ADMIN-0608', 'Name the person whose previous Kerberos key ' +
                   'versions to drop, as `username`.');
  }
  const current = directory.readPerson(who);
  if (!current) {
  ...
  const result = dropPrevious('person', who + '@' + principals.REALM, current, context,
...
function listServices() {
  ...
  log.debug('Leaving listServices(). ' + rows.length + ' service principal(s).');
  return rows;
}
...
function withheldValues(attribute, values) {
  ...
  if (!WITHHELD_ATTRIBUTES.some(function (one) { return one.toLowerCase() === lower; })) {
    return values;
  }
...
if (typeof credentials.setPasswordObserver === 'function') {
  ...
  log.warn('krb5-keys: common/credentials.js offers no setPasswordObserver(), so ' +
           'no person will get Kerberos keys from a password. That is the older ' +
           'credential store and is not an error.');
}
if (typeof principals.setKeySource === 'function') {
  principals.setKeySource({ personKeys: personKeys, serviceKeys: serviceKeys });
} else {
  log.warn('krb5-keys: kerberos/krb5_principals.js offers no setKeySource(), so ' +
           'stored Kerberos keys are never read. That is the older principal ' +
