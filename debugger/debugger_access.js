...
//   * **ON EVERY CALL THE GATE FORWARDS.** `debugger/debugger_server.js` asks
...
// ---------------------------------------------------------------------------
function isAdministrator(subject) {
  log.debug("Entering isAdministrator().");
  const who = subject || {};
  const name = String(who.name || '').trim();
  if (who.kind && who.kind !== 'user') {
    log.debug("Leaving isAdministrator(). Not a person.");
  ...
  if (!name) {
    log.debug("Leaving isAdministrator(). Nobody named.");
    return { allowed: false, code: 'STS-DBG-0001', roles: [],
             why: 'nobody is named as the subject' };
  }
  if (who.authenticated === false) {
    log.debug("Leaving isAdministrator(). Not authenticated.");
    return { allowed: false, code: 'STS-DBG-0001', roles: [],
  ...
  try {
    ...
    log.debug("Caught in isAdministrator(): " + ((e && e.message) || e));
    held = null;
  }
  ...
  if (held && held.open === true) {
    log.debug("Leaving isAdministrator(). The roster is empty.");
    return { allowed: false, code: 'STS-DBG-0024', roles: [],
             why: name + ' holds no console role and uses the console only ' +
                  ...
                  'role — and the debugger does not open that way. Grant ' +
                  'a role on /admin/rbac (or POST /admin-api/rbac/grant) ' +
                  'first' };
  }
  ...
  if (!holdsOne) {
    log.debug("Leaving isAdministrator(). Holds no console role.");
  ...
  if (!policy.allowed) {
    log.debug("Leaving isAdministrator(). The policy refused.");
    return { allowed: false, code: 'STS-DBG-0009', roles: roles,
             why: policy.why };
  }
  log.debug("Leaving isAdministrator(). Allowed.");
  return { allowed: true, roles: roles, why: '' };
}

// Whether a space-delimited scope carries the permission.
function asksForPermission(scope) {
  log.debug("Entering asksForPermission().");
  const asked = String(scope || '').split(/\s+/).indexOf(PERMISSION_ID) >= 0;
  log.debug("Leaving asksForPermission(). " + asked);
  return asked;
}
...
// ---------------------------------------------------------------------------
function narrowScope(scope, subject, context) {
  log.debug("Entering narrowScope().");
  if (!asksForPermission(scope)) {
    log.debug("Leaving narrowScope(). The permission was not asked for.");
  ...
  const answer = inDefault
    ...
    : { allowed: false, code: 'STS-DBG-0001',
        why: 'the debugger permission exists in the default realm only, and ' +
             'this request is in the realm "' + realms.currentId() + '"' };
  if (answer.allowed) {
    log.debug("Leaving narrowScope(). Granted.");
    return String(scope);
  }
  ...
  log.debug("Leaving narrowScope(). Taken off.");
  return kept;
}
...
module.exports = {
  ...
  asksForPermission: asksForPermission,
