...
//      (`debugger_api_process.js`), and this file forwards `/api/*` to it with
...
//   4. **PEOPLE ONLY, CONSOLE ADMINISTRATORS ONLY** — `debugger_access.js`.
//
// ---------------------------------------------------------------------------
...
function authorizationBaseOf(req) {
  log.debug("Entering authorizationBaseOf().");
  const pinned = helpers.pinnedBaseUrl();
  if (pinned) {
    log.debug("Leaving authorizationBaseOf(). Pinned.");
    return pinned;
  }
  const host = hostHeaderOf(req).replace(/:[0-9]{1,5}$/, '');
  log.debug("Leaving authorizationBaseOf().");
...
function wantsJson(req) {
  ...
  const api = req.path === API_PREFIX || req.path.indexOf(API_PREFIX + '/') ===
                                         0;
...
function sendPage(res, status, title, inner) {
  ...
  res.status(status).set('Content-Type', 'text/html; charset=utf-8')
     ...
     .send('<!DOCTYPE html><html lang="en"><head><meta charset="utf-8">' +
           ...
           '</style></head><body><h1>' + esc(title) + '</h1>' + inner +
           '</body></html>');
  log.debug("Leaving sendPage().");
}

function refuse(req, res, status, code, error, why, challenge) {
  ...
  if (wantsJson(req)) {
    ...
    sendPage(res, status, status === 403 ? 'Not permitted' : 'Refused',
             '<p>' + esc(why) + '</p><p><a class="btn" href="' +
             esc(ACCOUNT_PATH) + '">Your debugger session</a></p>');
  }
...
function verifyAccessToken(token, req, opts) {
  ...
  try {
    ...
    log.debug("Caught in verifyAccessToken(): " + ((e && e.message) || e));
    claims = null;
  }
  if (!claims) {
    log.debug("Leaving verifyAccessToken(). Did not verify.");
    return { ok: false, status: 401, code: 'STS-DBG-0003',
             error: 'invalid_token',
             why: 'That access token was not issued by this service, or its ' +
                  'signature does not verify.' };
  }
  const now = Math.floor(Date.now() / 1000);
  if (claims.exp && Number(claims.exp) <= now) {
    log.debug("Leaving verifyAccessToken(). Expired.");
  ...
  if (!jwtAccessToken.isAccessTokenType(typ)) {
    log.debug("Leaving verifyAccessToken(). Not an access token.");
  ...
  const bases = [authorizationBaseOf(req)];
  const pinned = helpers.pinnedBaseUrl();
  ...
  if (!issuerOk) {
    log.debug("Leaving verifyAccessToken(). Wrong issuer.");
    return { ok: false, status: 401, code: 'STS-DBG-0006',
             ...
             why: 'That access token names the issuer ' +
                  ...
                  'accepts tokens from this service\'s default authorization ' +
                  'server at ' + bases.join(' or ') + '.' };
  }
  ...
  if (audiences.indexOf(access.PERMISSION_BASE) < 0) {
    log.debug("Leaving verifyAccessToken(). Wrong audience.");
  ...
  if (scopes.indexOf(access.PERMISSION_NAME) < 0) {
    ...
    return { ok: false, status: 403, code: 'STS-DBG-0008',
             ...
             why: 'That access token does not carry the debugger ' +
                  'permission. It is issued to console administrators only, ' +
                  'so the authorization server leaves it off for anybody ' +
                  'else.' };
  }
  ...
  if (!answer.allowed) {
    log.debug("Leaving verifyAccessToken(). Not an administrator now.");
    return { ok: false, status: 403, code: answer.code || 'STS-DBG-0009',
             error: 'access_denied', why: answer.why + '.' };
  }
  log.debug("Leaving verifyAccessToken(). Accepted.");
...
function isLanding(req) {
  log.debug("Entering isLanding().");
  if (UI_LANDINGS.indexOf(req.path) >= 0) {
    log.debug("Leaving isLanding(). A UI landing.");
    return true;
  }
  if (req.path.indexOf(API_PREFIX + '/') !== 0) {
    log.debug("Leaving isLanding(). Not the api.");
    return false;
  }
  ...
  log.debug("Leaving isLanding(). " + hit);
  return hit;
}
...
// fetch and post to anything, because reaching another party's endpoints from
// the browser is what several of its workflows are.
app.use(function securityHeaders(req, res, next) {
  res.set('Content-Security-Policy',
          "default-src 'self'; script-src 'self' 'unsafe-inline'; " +
          "style-src 'self' 'unsafe-inline'; img-src 'self' data: https: " +
          "http:; font-src 'self' data:; connect-src 'self' https: http:; " +
          "form-action 'self' https: http:; frame-src 'self' https: http:; " +
...
app.get(CALLBACK_PATH, function (req, res) {
  ...
  oidcRp.handleCallback(req, res, SURFACE, {
    authorizationBase: authorizationBaseOf(req),
    poolPin: pin
    ...
    if (!answer || !answer.ok) {
      ...
      sendPage(res, 400, 'Signing in did not complete',
               '<p>' + esc((answer && answer.why) || 'no reason given') +
    ...
    if (!res.headersSent) {
      ...
      sendPage(res, 500, 'Signing in did not complete',
...
// ---------------------------------------------------------------------------
function landingTarget(req) {
  log.debug("Entering landingTarget().");
  log.debug("Leaving landingTarget().");
  return debuggerBaseOf(req) + '/oauth2_oidc_2.html';
}

app.get('/callback', function (req, res) {
  log.debug("Entering GET /callback.");
  const query = String(req.originalUrl || '').split('?')[1] || '';
  res.redirect(303, landingTarget(req) + (query ? '?' + query : ''));
...
app.post('/callback',
  ...
  res.redirect(303, landingTarget(req) + (fragment ? '#' + fragment : ''));
...
app.use(function gate(req, res, next) {
  ...
  if (req.path === CALLBACK_PATH || isLanding(req)) {
  ...
  if (presented) {
    ...
    if (!verdict.ok) {
      ...
      refuse(req, res, verdict.status, verdict.code, verdict.error,
             verdict.why,
  ...
  if (!session) {
    ...
    refuse(req, res, 401, 'STS-DBG-0002', 'unauthorized',
           'The debugger needs a signed-in console administrator: open it ' +
           'in a browser, or send an access token carrying ' +
  ...
  const verdict = verifyAccessToken(tokens.accessToken || '', req);
  if (!verdict.ok) {
    // THE AUDIENCE IS ONE OF THEM. The session's token was asked for with the
    // permission, so a token addressed anywhere else is the authorization
    // server having taken the permission off (and with it the only audience
    // the request named) — the same answer as a missing scope.
    if (verdict.code === 'STS-DBG-0007' || verdict.code === 'STS-DBG-0008' ||
        verdict.code === 'STS-DBG-0009' || verdict.code === 'STS-DBG-0024') {
      ...
      if (wantsJson(req)) {
        ...
        sendPage(res, 403, 'The debugger is for administrators',
                 (why.code === 'STS-DBG-0024'
                   ? '<p><strong>Nobody is an administrator yet.</strong> ' +
                     'While neither console role group has a member the ' +
                     'admin console opens to everybody so that the first ' +
                     'role can be granted; the debugger does not. Grant ' +
                     'somebody Admin Read or Admin Write on ' +
                     '<code>/admin/rbac</code>, then sign in again.</p>'
                   : '') +
                 '<p>You are signed in as <code>' +
                 esc(session.user.username) + '</code>. The identity ' +
                 'protocol debugger is available to console administrators — ' +
                 'members of the Admin Read or Admin Write group — ' +
                 (reason.allowed
                   ? 'and this account IS one now, but its token was issued ' +
                     'before it was, without the debugger permission. Sign ' +
                     'out and sign in again.</p>'
                   : 'and this account is neither, so the authorization ' +
                     'server issued its token without the debugger ' +
                     'permission.</p>') +
                 '<p class="muted">' + esc(why.why) + '</p>' +
                 signOutForm(session));
      }
    ...
    // verifyAccessToken().
    refuse(req, res, verdict.status, verdict.code, verdict.error, verdict.why);
...
// ---------------------------------------------------------------------------
function signOutForm(session) {
  log.debug("Entering signOutForm().");
  log.debug("Leaving signOutForm().");
  return '<form method="post" action="' + esc(SIGNOUT_PATH) + '">' +
...
app.get(ACCOUNT_PATH, function (req, res) {
  ...
  sendPage(res, 200, 'Identity protocol debugger',
    '<p>Signed in as <code>' + esc(req.debuggerUser) + '</code> through ' +
    'this service\'s authorization server, as the client ' +
    '<code>' + esc(access.UI_CLIENT_ID) + '</code>. The api at ' +
    '<code>/api</code> is reached with an access token carrying ' +
    '<code>' + esc(access.PERMISSION_ID) + '</code>, which is issued to ' +
    'console administrators only.</p>' +
    '<p>The api process is <strong>' + esc(status.state) + '</strong>' +
    (status.allowList
      ? ', allowed to dial ' + esc(String(status.allowedRanges.length)) +
    ...
    (session ? signOutForm(session) : '<p class="muted">This request ' +
      'presented a bearer token, so there is no session to sign out of.</p>'));
...
app.post(SIGNOUT_PATH,
  ...
  if (!session || !csrf.ok) {
    refuse(req, res, 403, 'STS-DBG-0021', 'csrf',
           'That sign-out did not come from a page this debugger drew. ' +
           ((csrf && csrf.detail) || ''));
  ...
  const signOnEnded = parent
    ? !!authn.endSessionById(parent, 'the Sign out button on the protocol ' +
                                     'debugger')
    : false;
  if (signOnEnded) {
  ...
  sendPage(res, 200, 'Signed out',
           '<p>You are signed out of the debugger' +
           (signOnEnded ? ', and of the sign-on session it was built on' : '') +
...
app.get('/tls/server-certificate', function (req, res) {
  ...
  if (!config.value('global.https')) {
    ...
    res.status(404).json({ error: 'This listener is not serving TLS, so it ' +
                                  'has no certificate to publish.',
                           code: 'ENOTLS' });
...
// the one it was started with — see `debugger_api_process.js`'s
// updateAnchor(). A replacement answers this call with the not-ready 502 below
// rather than forwarding it to a child that is exiting.
function checkAnchor() {
  log.debug("Entering checkAnchor().");
  const now = Date.now();
  if (!config.value('global.https') ||
      now - anchorCheckedAt < ANCHOR_CHECK_MS) {
    log.debug("Leaving checkAnchor(). Not due.");
    return false;
  }
  anchorCheckedAt = now;
  let pem = '';
  try {
    ...
    log.debug("Caught in checkAnchor(): " + ((e && e.message) || e));
    pem = '';
  }
  log.debug("Leaving checkAnchor().");
  return apiProcess.updateAnchor(pem);
}

function forward(req, res) {
  log.debug("Entering forward(). " + req.method + " " + req.originalUrl);
  checkAnchor();
  if (!apiProcess.ready()) {
    const status = apiProcess.status();
    refuse(req, res, 502, 'STS-DBG-0010', 'api_unavailable',
           'The debugger api is ' + status.state + '. ' +
           (status.lastError ? 'Last failure: ' + status.lastError : ''));
    log.debug("Leaving forward(). Not ready.");
    return;
  }
  const max = Number(config.value('debugger.maxRequestBytes'));
  const declared = Number(req.headers['content-length'] || 0);
  if (declared > max) {
    refuse(req, res, 413, 'STS-DBG-0012', 'request_too_large',
           'That request body is ' + declared + ' bytes and the debugger ' +
           'forwards at most ' + max + ' (debugger.maxRequestBytes).');
    log.debug("Leaving forward(). Too large.");
    return;
  }
  ...
  upstream.on('error', function (err) {
    log.warn(errorCodes.tag('STS-DBG-0011') + 'debugger: forwarding ' +
             req.method + ' ' + target + ' to the api failed: ' + err.message);
    if (!res.headersSent) {
      const timedOut = /timed out/.test(err.message);
      refuse(req, res, timedOut ? 504 : 502, 'STS-DBG-0011',
             'api_unavailable',
  ...
  req.pipe(upstream);
  log.debug("Leaving forward().");
}

app.use(API_PREFIX, forward);
...
function servedText(file, stat, stsUrl) {
  log.debug("Entering servedText().");
  const key = file + '\n' + stat.mtimeMs + '\n' + stsUrl;
  if (substitutedCache.has(key)) {
    log.debug("Leaving servedText(). Cached.");
...
app.use(function staticSite(req, res) {
  ...
  if (SUBSTITUTED.indexOf(ext) >= 0) {
    res.send(servedText(file, stat, authorizationBaseOf(req)));
...
function listen() {
  ...
  if (!mode.embedsProtocolDebugger()) {
    startProblem = 'debugger.enabled is ' +
                   ...
                   ' and this service is in ' + mode.current() + ' mode, so ' +
                   'the debugger is not embedded';
    log.debug("Leaving listen(). Not embedded.");
    return { whenReady: Promise.resolve({ port: null, why: startProblem }) };
  }
  const uiProblem = fs.existsSync(path.join(uiDirectory(), 'index.html'))
    ? '' : 'the debugger UI is not installed: ' +
           path.join(uiDirectory(), 'index.html') + ' does not exist ' +
  ...
  if (problem) {
    ...
    log.debug("Leaving listen(). Not installed.");
...
function status() {
  ...
  return {
    ...
    scheme: scheme(),
    publicBaseUrl: String(config.value('debugger.publicBaseUrl') || '') || null,
...
module.exports = {
  ...
  isLanding: isLanding,
  authorizationBaseOf: authorizationBaseOf,
  debuggerBaseOf: debuggerBaseOf,
