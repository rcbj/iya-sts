...
// own rather than fifty lines inside `federation_sp.ts` for one reason: the
...
// function is where this service decides it. A plain require in the ordinary
// direction — `admin_stats.js` requires `federation.js`, not this file, so
// there is no cycle — and it registers no route, so nothing moves.
import stats = require('./../common/admin_stats');

// One mapping: an incoming name, the LDAP attribute it becomes, and where
// that was decided.
interface MapRow {
  incoming: string;
  ldap: string;
...
// The one catalogue field shape this module reads.
...
// The relationship members this module reads.
interface RelationshipLike {
  fedId?: string;
  fedAttributeMap?: unknown[];
  fedUsernameSource?: string;
  [member: string]: any;
}

interface UsernameChoice {
  username: string;
  raw: string;
  from: string;
  prefixed: boolean;
...
interface FederationMapDeps {
  log: typeof helpers.log;
  config: { value(key: string): any };
  identityKeyOf(value: string): string;
}
...
// object. A row whose first member is `address` is the compound one —
// `street` is `address.street_address` rather than a claim called `address` —
// and the second member is the name to match on. That shape is read here
...
// LAYER 2, PART FOUR's prefixes — see `buildDefaults()`.
...
class FederationMap {
  ...
  private static addDefault(log: FederationMapDeps['log'], incoming: string,
                            ldap: string, where: string): void {
    log.debug("Entering FederationMap.addDefault().");
    const key = String(incoming).toLowerCase();
    // FIRST WINS. The catalogue is walked before the hand-written rows below
    // it, so a name in both keeps the catalogue's attribute — which is the one
    // the rest of this service already uses for it.
    if (seenIncoming.has(key)) {
      log.debug("Leaving FederationMap.addDefault().");
      return;
    }
  ...
  static buildDefaults(log: FederationMapDeps['log'],
    ...
    const addDefault = function (incoming: string, ldap: string,
                                 where: string): void {
      FederationMap.addDefault(log, incoming, ldap, where);
    };

    vcAttributes.forEach(function (row) {
      ...
      if (claims[0] === 'address') {
        ...
        // ordinary case and is what is matched; one sending the whole
        // `address` object is handled by flatten() below, which turns it into
        // the same flat names before anything gets here.
        if (claims[1]) {
          addDefault(claims[1], row.ldap, 'OpenID Connect Core 1.0 ' +
                                          'address claim');
        }
        return;
      }
    ...
    // Shibboleth, SimpleSAMLphp and most European federations emit, and the
    // OIDs are X.500's — so an attribute name here is the SAME NAME as the
    // LDAP attribute, said in the other of the two ways a directory says it.
    // Which is why this table is a translation rather than a mapping and why
    // every row of it can be checked against RFC 4519 rather than against
    // somebody's product.
    [
      ...
      // eduPerson and SCHAC. A research federation sends these and nothing
      // else, and eduPersonPrincipalName is its USERNAME rather than one of
      // its attributes — which is why the note says so: somebody federating
      // with an InCommon or GEANT partner sets fedUsernameSource to this name.
      ['urn:oid:1.3.6.1.4.1.5923.1.1.1.6', 'uid',
       'eduPerson 2020-01 eduPersonPrincipalName — usually the USERNAME'],
      ['urn:oid:1.3.6.1.4.1.5923.1.1.1.1', 'employeeType',
    ...
    //
    // `http://schemas.xmlsoap.org/ws/2005/05/identity/claims/…` is what AD FS,
    // and therefore most of the WS-Federation deployments anybody will point
    // this service at, puts in a SAML 1.1 assertion. `saml11.ts` already EMITS
    // several of these; this is the same list read in the other direction, and
    // the two are deliberately not shared — that module's list is what this
    // service asserts and this one is what it will accept, and they are
    // allowed to differ.
    [
    ...
    log.debug('federation_map: ' + DEFAULT_MAP.length + ' default ' +
              'mappings, ' + vcAttributes.length + ' of them derived from ' +
              'the credential catalogue.');
    log.debug("Leaving FederationMap.buildDefaults().");
  }
  ...
  // `=` splits it, and the rest of the value is the attribute name — which
  // matters, because an incoming name can be a URL and a URL can hold an `=`.
  // Splitting on the last one, or on all of them, produces a mapping that
  ...
  relationshipMap(record?: RelationshipLike | null): Map<string, MapRow> {
    ...
    ((record && record.fedAttributeMap) || []).forEach(function (value) {
      ...
      if (at <= 0) {
        log.warn('federation: "' + text + '" is not a mapping on ' +
                 (record.fedId || '?') + '. A mapping is <incoming ' +
                 'name>=<LDAP attribute>, split at the FIRST equals sign. It ' +
                 'was ignored.');
        return;
      }
    ...
    log.debug("Leaving FederationMap.relationshipMap(). " + map.size +
  ...
  private resolve(name: string, own: Map<string, MapRow>): MapRow | null {
    const { log } = this.deps;
    log.debug("Entering FederationMap.resolve().");
    const key = String(name || '').toLowerCase();
    if (own.has(key)) {
      log.debug("Leaving FederationMap.resolve().");
      return own.get(key);
    }
  ...
  //
  // An ID Token is JSON and can nest; a SAML AttributeStatement cannot. The
  // one nested shape that matters is OpenID Connect's `address`, whose members
  // are claims in their own right — so it is flattened into `street_address`,
  // `locality`, `region`, `postal_code`, `country` and `formatted`, which is
  // exactly the set the catalogue's compound rows match on.
  //
  // Everything else that is an object is turned into its JSON text and mapped
  // as one value, rather than being dropped or walked. A partner sending a
  // nested object under a name somebody mapped has said something; showing it
  ...
  flatten(bag?: Record<string, any> | null): Record<string, string[]> {
        ...
        if (typeof one === 'object') {
          try {
            ...
            log.debug("Caught in a callback in put(): " +
                      ((e && e.message) || e));
            // A cycle, which JSON cannot represent. It cannot come off the
            // wire — JSON.parse never produces one — so this can only be a
            // caller's own object, and String() of it is more use than
            // throwing here.
            return String(one);
          }
        }
    ...
    log.debug("Leaving FederationMap.flatten(). " + Object.keys(out).length +
              ' name(s).');
    return out;
  }
  ...
  // THE USERNAME.
  //
  // `fedUsernameSource` names the incoming value to use; empty means the
  // subject the protocol itself carried — a SAML NameID, or `sub`.
  //
  ...
  // was pointed at another instance of this service. A foreign `sub` is an
  // opaque string, and that partner's happened to be `urn:sts:user:alice` —
  // this service's OWN subject format at the time, because the partner IS this
  // service. The raw value went to `startSession()`, `userFor()` put the
  // prefix on again, and every downstream token carried
  // `sub: urn:sts:user:urn:sts:user:alice`.
  //
  // The doubling is the symptom and not the bug. The bug is that the identity
  // funnel ALREADY normalises — `recordAuthentication()` runs `presented`
  // through `identityOf()`, which is what makes `alice`, `urn:sts:user:alice`
  // and `alice@REALM` one person and one directory entry — so an unnormalised
  // name reaching the SESSION means the session and the directory disagree
  // about who signed in. `/admin/users` said `alice` while the tokens said
  // something else, and that split is exactly what the one-entry-per-person
  // rule exists to prevent. It would have happened with any partner whose
  // subject carried an `@`, not only with one that shares this service's URN
  // format.
  //
  // **THE PREFIX IS APPLIED AFTER NORMALISATION**, which is what makes it work
  // at all: `fed-alice` separates a federated person from a local one, and
  ...
  usernameFor(record: RelationshipLike | null | undefined,
    ...
    const { log, config, identityKeyOf } = this.deps;
    log.debug("Entering FederationMap.usernameFor(). source=" +
    ...
    if (source) {
      ...
      if (!raw) {
        ...
        // stopped sending the attribute the username comes from would
        // otherwise start creating a second set of entries named by NameID,
        // and the two sets would look like two populations rather than one
        // bug.
        log.warn('federation: ' + (record.fedId || '?') + ' is configured ' +
                 'to take the username from ' +
                 '"' + source + '", which this assertion did not ' +
                 'carry. Falling back to the subject, which means this ' +
                 'sign-in may land on a different entry from the last one.');
        raw = String(subject || '');
    ...
    //
    // **EXCEPT A `urn:uuid:` SUBJECT, WHICH IS NOT RESOLVED HERE
    // (2026-09-14).** That is this service's own subject form now
    // (`urn:uuid:<entryUUID>`), and `identityOf()` answers it by looking the
    // UUID up in THIS realm's directory. A partner's subject is a value in the
    // PARTNER's namespace: it names an entry over there, and resolving it here
    // would either find nobody (and file the person under a raw URN nothing
    // may create an entry for) or, worse, find a local person who happens to
    // hold that value. So it becomes a local name of its own — `sub-<uuid>`,
    // stable for as long as the partner's subject is, and in no namespace a
    // local person can be in.
    const normalised = /^urn:uuid:/i.test(raw)
      ? 'sub-' + raw.slice('urn:uuid:'.length).toLowerCase()
      : identityKeyOf(raw);
    if (normalised !== raw) {
      log.debug('usernameFor(): the partner\'s subject "' + raw + '" is ' +
                'filed here as "' +
    ...
    log.debug("Leaving FederationMap.usernameFor(). username=" + username +
              ' from ' + from);
  ...
  //
  // Returns everything a caller needs to both ACT and EXPLAIN, because both
  // the directory write and the relationship page read this one result and a
  // second pass over the bag would be a second answer:
  //
  //   username     what to file them under
  //   attributes   { ldapName: [values] }, ready for the directory
  //   mapped       [{ incoming, ldap, where, values }] — what happened, in
  //                order
  //   unmapped     [{ incoming, values }] — layer 3, the useful half
  //
  // TWO INCOMING NAMES CAN MAP TO ONE ATTRIBUTE and the values are
  // CONCATENATED rather than one overwriting the other. That is the honest
  ...
  mapIncoming(record: RelationshipLike | null | undefined,
              ...
              subject?: unknown) {
    const { log } = this.deps;
    log.debug("Entering FederationMap.mapIncoming(). id=" +
              ((record && record.fedId) || '?'));
    const flat = this.flatten(bag);
    const own = this.relationshipMap(record);
    ...
    const who = this.usernameFor(record, flat, subject);
    if (unmapped.length) {
      // At INFO rather than DEBUG: this is the line somebody reads when a
      // federated entry came out emptier than they expected, and it names
      // every attribute the partner sent that this service threw away.
      log.info('federation: ' + ((record && record.fedId) || '?') + ' sent ' +
               unmapped.length + ' attribute(s) nothing maps: ' +
               unmapped.map(function (one) {
                 return one.incoming;
               }).join(', ') +
               '. They are NOT written to the directory — add a mapping on ' +
               'the relationship to keep one.');
    }
    log.debug("Leaving FederationMap.mapIncoming(). " + mapped.length +
              ' mapped, ' + unmapped.length + ' unmapped, username=' +
              who.username);
    return {
