...
interface FederationGraphDeps {
  log: typeof helpers.log;
  realms: typeof realms;
  federation: typeof federation;
}

// What `graph()` is narrowed by: the list page's own three filters.
...
class FederationGraph {
  ...
  private partyId(kind, name) {
    const { log } = this.deps;
    log.debug("Entering FederationGraph.partyId().");
    log.debug("Leaving FederationGraph.partyId().");
  ...
  // them.
  private brokerKey(application, relationship) {
    const { log } = this.deps;
    log.debug("Entering FederationGraph.brokerKey().");
    log.debug("Leaving FederationGraph.brokerKey().");
  ...
  // calls into the register (`readinessOf`, `isEnabled`) so the two pages
  // cannot disagree about whether a partner is usable. What this adds is the
  ...
  describe(record) {
    ...
    const protocolRow: any = federation.protocolRow(record.fedProtocol) || {};
    const roleRow: any = federation.roleRow(record.fedRole) || {};
    ...
    const row = {
      ...
      // ---------------------------------------------------------------------
      // WHAT THE IDENTITY-PROVIDER SIDE IS CONFIGURED TO DO ABOUT
      // AUTHENTICATING SOMEBODY, which is the half of this feature no table has
      // ever shown.
      //
      ...
      //                                         relationship
      //   * a mechanism that is not one of ours somebody typed something,
      //                                         and the relationship will
      //                                         refuse
      //   * NOTHING AT ALL                      and this is not "unknown".
      //                                         It is a decision with a
      //                                         name: the
      //                                         sign-in screen, which checks no
      //                                         password in development mode.
      //                                         Every relationship created
      //                                         before the attribute existed is
      //                                         in this state, so a picture
      //                                         that left it blank would be
      //                                         hiding the commonest answer
      //                                         there is.
      //
      // `authenticationFor()` resolves all four and this reads its answer
      // rather than deriving a second one, so the picture and the sign-in path
      // cannot come to disagree about what a relationship will do.
      // ---------------------------------------------------------------------
      ...
      // is declared here so that every row carries the field whether or not it
      // is brokered: a caller reading the JSON should not have to tell an
      // absent member from a zero.
      brokeredUse: null,
    ...
    if (record.fedRole === 'identity-provider') {
      ...
      if (resolved) {
        ...
        if (resolved.mechanism === 'federation') {
          ...
          // names nothing usable. BOTH are kept, because the picture has to
          // draw the intent even when it cannot draw the destination — a broker
          // pointing at a disabled partner otherwise looks exactly like one
          // configured to use the sign-in screen, which is the fallback the
    ...
    log.debug("Leaving FederationGraph.describe(). " + row.id + ', ' +
              row.roleLabel + '.');
    return row;
  }
  ...
  //
  // TWO SOURCES AND A FULL OUTER JOIN, and each of the three resulting states
  // is something somebody needs to see:
  //
  //   * CONFIGURED AND USED         the ordinary case, and the number the
  //                                 picture was asked for.
  //   * CONFIGURED, NEVER USED      an application pointed at a partner
  //                                 that has never authenticated anybody for
  //                                 it. This is
  ...
  //                                 dropped, because a count that vanished when
  //                                 a pointer was removed would leave the
  //                                 totals on this page not adding up with
  //                                 nothing to say why.
  //
  ...
  // ---------------------------------------------------------------------------
  private applicationRows(record, row) {
    const { log, federation } = this.deps;
    log.debug("Entering FederationGraph.applicationRows(). id=" + row.id);
    // Only the service-provider side has a per-application split. See the
    // schema row for `fedApplicationUse`: an identity-provider-side
    // relationship names exactly ONE application, so its per-application count
    // IS its own count and a second list holding the same number is the copy
    // that comes to disagree.
    if (row.role !== 'service-provider') {
      log.debug("Leaving FederationGraph.applicationRows(). Not the " +
                'service-provider side.');
      return [];
    }
    const byName = new Map();
    federation.applicationsUsing(row.id).forEach((one) => {
    ...
    federation.applicationUse(record).forEach((use) => {
    ...
    // Busiest first, then by name — the order `applicationUse()` already
    // returns, restated here rather than inherited because this list has
    // members that one does not and a half-sorted list is worse than an
    // unsorted one.
    rows.sort((a, b) => {
    ...
    log.debug("Leaving FederationGraph.applicationRows(). " + rows.length +
              ' application(s).');
    return rows;
  }
  ...
  // THE GRAPH.
  //
  // `wanted` narrows it, and it narrows the WHOLE model rather than the
  // drawing: the tables under the picture are built from what this returns, so
  // a filter that hid boxes and left the rows standing would be two answers to
  // one question. The three filters are the list page's own — role, protocol
  // and a text match — because narrowing the table and narrowing the picture
  // has to be one control rather than two that can take different values.
  //
  // IT MUST NOT THROW ON AN EMPTY REGISTER. A realm with no federation
  // configured is the ordinary state of this service, and it comes back as the
  // hexagon alone with `empty: true` — which is what lets the page say *this
  ...
  graph(wanted) {
    ...
    records.forEach((record) => {
      const row: any = this.describe(record);
      ...
      row.applications = this.applicationRows(record, row);
      row.applicationCount = row.applications.filter((one) => {
      ...
      //
      //   * somebody used the generic partner buttons at the foot of the
      //     sign-in screen, which offer every usable relationship and belong to
      //     no application;
      ...
      // per-application numbers add up to less than the total two columns to
      // its left. A reader who spots that and cannot account for it has found a
      // discrepancy in a page about counting, so the difference is a FIGURE
      // with a name rather than an absence.
      //
      // CLAMPED AT ZERO. These are attributes in a directory and `ldapmodify`
      // is a door onto them like any other, so `fedAuthentications` can be
      // edited down below the sum of the rows underneath it. A negative here
      // would be a second wrong number reported as confidently as the first.
      // -------------------------------------------------------------------
      const attributed = row.applications.reduce((n, one) => {
    ...
    // register is an answer, and a realm is a whole logical copy of this
    // service so the picture of one realm is not the picture of another. It is
    // the same decision `delegation_map.js` made about its own hexagon, and for
    // the same reason.
    addNode({
      id: STS_ID, kind: 'sts',
      label: 'This service',
    ...
    // Drawing both would put two arrows between the same pair of boxes saying
    // two true things that a reader reads as one thing said twice.
    //
    // So the identity-provider side is drawn first and remembers which (party,
    // onward relationship) pairs it has covered, and the loop below skips
    // exactly those — carrying the COUNTS onto the arrow that was drawn rather
    // than losing them, because they are that pair's counts wherever the arrow
    // ends up.
    // ---------------------------------------------------------------------
    const brokered = new Map();
    rows.filter((row) => { return row.role === 'identity-provider'; })
        .forEach((row) => {
      ...
      const id = this.partyId('partner-sp', name);
      const node = addNode({
      ...
      edges.push(edge);
      if (row.brokersTo) {
        brokered.set(this.brokerKey(name, row.brokersTo), edge);
      }
    });

    rows.filter((row) => { return row.role === 'service-provider'; })
        .forEach((row) => {
      const name = row.peer || row.id;
      const id = this.partyId('partner-idp', name);
      const node = addNode({
      ...
      row.applications.forEach((use) => {
        const owner = brokered.get(this.brokerKey(use.application, row.id));
        if (owner) {
          owner.use = use;
          ...
          // increments `fedAuthentications` on this side, since what it counts
          // is assertions CONSUMED and this side ISSUES them
          // (`federation/CLAUDE.md`, "What it deliberately does not do"). A
          // table printing a bare 0 there asserts that nobody has ever signed
          // in for this partner, which for a brokered one is false — the
          // sign-ins happened and were counted against the relationship they
          // went THROUGH. This is the only route by which the identity-provider
          // side can report a real number, so it goes on the ROW as well as on
          // the edge rather than being left for whichever of the two happens to
          // be rendered.
          owner.row.brokeredUse = use;
          return;
        }
        const partyNodeId = this.partyId('application', use.application);
        addNode({
    ...
    const result = {
      ...
      // can tell "nothing is configured" from "nothing matches". Two states,
      // two different things to say, and only one of them is a problem.
      empty: records.length === 0,
      filtered: records.length !== rows.length,
      counts: {
        relationships: rows.length,
        serviceProvider: rows.filter((r) => {
          return r.role === 'service-provider';
        }).length,
        identityProvider: rows.filter((r) => {
          return r.role === 'identity-provider';
        }).length,
        usable: rows.filter((r) => { return r.usable; }).length,
        applications: nodes.filter((n) => {
          return n.kind === 'application';
        }).length,
        partners: nodes.filter((n) => {
          return n.kind === 'partner-sp' || n.kind === 'partner-idp';
        }).length,
        authentications: rows.reduce((n, r) => {
          return n + r.authentications;
        }, 0)
      }
    };
    log.debug("Leaving FederationGraph.graph(). " + nodes.length +
              ' box(es), ' + edges.length +
