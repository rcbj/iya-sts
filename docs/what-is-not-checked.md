...
| Verify an access token at the OpenID4VCI endpoints — **in development mode** | OpenID4VCI lets the authorization server be somebody else, so in development the credential, deferred credential and notification endpoints read a token this realm cannot verify unverified. **Product mode refuses it**, and a token this realm revoked, with `invalid_token` (2026-09-18). Every other door that takes a token verifies it in both modes: UserInfo, `/admin-api`, SCIM's and Shared Signals' OAuth schemes, the step-up resource, introspection, and the token endpoint's token exchange and JWT and SAML client authentication and grants |
...
| ~~Turn a verified presentation into a sign-on~~ — **reversed 2026-09-17** | "Sign in with a wallet" on the sign-in screen (`/authn/wallet`, `oid4vp.signIn`) starts a session for the directory entry a **holder-bound credential this service issued** was issued for — on an access token this service verified and nobody has disowned — after a FRESH holder proof verifies against the key the credential is bound to: a Key Binding JWT (`dc+sd-jwt`), a Verifiable Presentation JWT (`jwt_vc_json`) or a Data Integrity proof by the credential's `did:jwk` (`ldp_vc`). The request is offered through the W3C **Digital Credentials API**, whose cross-device path proves the wallet is near the browser; the plain QR code, which can be relayed to a victim, is **off by default in both modes** (`oid4vp.signInCrossDevice`). Any other presentation — a trusted partner's credential, another realm's, one issued on a foreign access token, one whose entry is gone, one this service has disowned or whose status list says revoked — still verifies and signs nobody in, and the page says why. The Verifier at `/oid4vp/verifier` still says yes on a web page and stops |
| ~~Publish any status for a credential it issued~~ — **reversed 2026-09-17** | Every credential carries a status reference — an IETF **Token Status List** claim, a W3C **Bitstring Status List** entry, or both — and this realm serves the lists at `/oid4vci/status-lists*`. A global sign-out, an administrator's revocation on `/admin/tokens` and `/admin/vc-status` each set the bit. The Verifier consults them: this realm's from its own store, a trusted foreign issuer's by fetching the list, and a credential whose status cannot be established is **refused** |
| One factor is one factor, and a wallet is one — **qualified 2026-09-17** | A presentation proves possession of ONE key, so a session says `amr ["pop"]` and `acr "1"`. A request that demands two factors is now answered by the presentation **and a second factor afterwards** (an authenticator app, a security key, or the person's password), and a wallet may itself be the second factor after a password. Where the issuer verified a **key attestation** saying the key is guarded by the person's own authentication, the sign-in says `acr "mfa"` on its own — and nothing a presentation says about ITSELF is believed: only what the issuer recorded when it issued the credential |
...
| ~~Deactivate anybody on SCIM `active: false`~~ — **reversed 2026-09-17** | `active: false` DISABLES the account: it writes the password-policy lock `pwdAccountLockedTime` on the entry — the same state the **Disable** button on `/admin/users` writes — every door then refuses that person (a password anywhere, any sign-in, a Kerberos AS-REQ, every token grant, the management API), and everything they hold is ended at once. `active: true` enables them again |
...
| ~~Verify a SAML `AuthnRequest`'s signature, or consume a service provider's metadata~~ — **reversed 2026-09-17 (#37); this row said both were *recorded, neither checked*** | A signed `AuthnRequest`, `LogoutRequest`, `LogoutResponse` or `ArtifactResolve` is **verified in every mode** — the HTTP Redirect binding's query signature, the POST-SimpleSign binding's, or an enveloped one; RSA, ECDSA, EdDSA, DSA, ML-DSA or SLH-DSA; SHA-1 only with `saml.allowSha1Signatures` — against the service provider's **registered** certificates (`samlSigningCertificate`, from its metadata or the SAML 2.0 page), never the certificate the request carries, and **refused** when it does not verify. The certificate a request carries is recorded as *observed* and trusted only once you **Confirm** it on `/admin/saml2` (`POST /admin-api/saml2/confirm-signing-certificate`). An **unsigned** request is accepted in development and **refused in product** — `saml2.requireSignedAuthnRequests`, `auto` by default — and refused in either where the service provider's metadata says `AuthnRequestsSigned="true"`; `WantAuthnRequestsSigned` in `/saml2/metadata` says which. A service provider's **metadata is consumed** by the refresh on its application page or an upload on `/admin/saml2`: its endpoints become its registered return addresses, its certificates are registered, its `NameIDFormat`s restrict a `NameIDPolicy`, and `WantAssertionsSigned` is honoured in product. Since the #37 follow-up the document's effective `validUntil` is **enforced** after consumption (every request from that service provider is refused once it passes), a document past its `cacheDuration` is fetched again in the background, a document must verify against the realm's `saml2.metadataTrustAnchors` when any is set, and an artifact is resolved only for the service provider it was issued to — authenticated, where signed requests are required. What is still NOT checked: a MAC, MD5 or stateful hash-based signature (refused as not checkable, never accepted) |
...
this service issued and has not disowned — a presentation made at the Verifier's own pages is
