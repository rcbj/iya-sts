...
**The attestation statement is verified (#105)**, under
...
(`STS-AUTHN-0206`) — enrolling one for anybody who named an existing person
holding no key would give that account to whoever asked. In development it still enrols on first use, and the screen says the
...
**Node attestation is verified or refused, in every mode.**
...
A client is issued only the scopes it declared (#110).
...
not be merged. **The Unix socket attests its caller (#40)** the
