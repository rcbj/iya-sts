...
| `tls.signatureAlgorithms` | `STS_TLS_SIGALGS` | OpenSSL's list without DSA and SHA-224: ML-DSA, ECDSA, EdDSA, RSA-PSS, RSA PKCS #1 v1.5 with SHA-256/384/512 | **restart** — the TLS contexts are built when the listeners are created | The signature schemes the same listeners sign with and accept from a client certificate, and what a CertificateRequest advertises (node's `sigalgs`). **Empty means OpenSSL's default, which advertises DSA and SHA-224 in TLS 1.2** and so a `dss_sign` certificate type. A list that builds no context stops the service at startup. |
