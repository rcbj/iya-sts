...
| `oauth2.clientAttestationTrustAnchors` | `STS_OAUTH2_CLIENT_ATTESTATION_TRUST_ANCHORS` | `(empty)` | yes | PEM certificates, concatenated: the trust anchors a Client Attestation's `x5c` chain must lead to (`attest_jwt_client_auth`, draft-ietf-oauth-attestation-based-client-auth-11 section 10.8). The signing certificate may not be self-signed (HAIP 1.0 section 4.4.1). Empty, with `oauth2.clientAttestationTrustedKeys` empty too, trusts no attester: the two attestation methods are then not advertised and every attestation is refused. |
| `oauth2.clientAttestationTrustedKeys` | `STS_OAUTH2_CLIENT_ATTESTATION_TRUSTED_KEYS` | `(empty)` | yes | A JWKS of client attester public keys: an attestation carrying no `x5c` must verify under one of them, narrowed by its `kid`. Symmetric and private keys are refused; a MAC-protected attestation is not accepted. Post-quantum (AKP) keys are accepted. |
| `oauth2.clientAttestationChallengeRequired` | `STS_OAUTH2_CLIENT_ATTESTATION_CHALLENGE_REQUIRED` | `true` | yes | Require every Client Attestation PoP to carry a challenge this server issued (`POST /oauth2/challenge`, or the `OAuth-Client-Attestation-Challenge` header on the previous response), each good for one request. A PoP without one is answered 400 `use_attestation_challenge` with a fresh challenge. **Turning it off is weaker and not recommended**: the PoP's `jti` and `iat` are then the only replay protection. The DPoP combined mode uses DPoP nonces instead. |
| `oauth2.clientAttestationChallengeTtlS` | `STS_OAUTH2_CLIENT_ATTESTATION_CHALLENGE_TTL_S` | `300` | yes | How long a challenge this server hands out is accepted in a Client Attestation PoP. |
| `oauth2.clientAttestationChallengeCacheSize` | `STS_OAUTH2_CLIENT_ATTESTATION_CHALLENGE_CACHE_SIZE` | `10000` | yes | The most unexpired challenges a realm holds; at the bound the oldest is dropped, and a client presenting it is given a fresh one. |
| `oauth2.clientAttestationMaxAgeS` | `STS_OAUTH2_CLIENT_ATTESTATION_MAX_AGE_S` | `86400` | yes | How long after its `iat` a Client Attestation is still fresh enough; an older one is answered `use_fresh_attestation`. |
| `oauth2.clientAttestationPopMaxAgeS` | `STS_OAUTH2_CLIENT_ATTESTATION_POP_MAX_AGE_S` | `300` | yes | How long after its `iat` a Client Attestation PoP is accepted, plus `oauth2.clientAssertionSkewS`; its `jti` is remembered for the same window. |
| `oauth2.fapiAllowClientAttestation` | `STS_OAUTH2_FAPI_ALLOW_CLIENT_ATTESTATION` | `false` | yes | Under a FAPI 2.0 profile, accept `attest_jwt_client_auth` and `attest_jwt_client_auth_dpop` beside mTLS and `private_key_jwt`, as HAIP 1.0 section 4 allows. Off, FAPI 2.0 section 5.3.2.1 item 6 is held to as written; FAPI 1.0 never accepts them. |
...
| `oauth2.codeReplayIdempotent` | `STS_OAUTH2_CODE_REPLAY_IDEMPOTENT` | `false` | yes | **Weaker than the specification — leave it off.** With it on, an IDENTICAL repeat of a Token Request for an authorization code already redeemed is answered with the tokens it already got, for the rest of the code's own lifetime. RFC 6749 section 4.1.2 says a code used twice MUST be refused; off (the default) it is, and everything the first redemption bought is revoked (section 10.5). RFC 9700, OAuth 2.1 and FAPI mode ignore it. It exists for the parent project's development-mode job that still asserts the old courtesy, which `env/docker-tests.js` turns it on for. |
| `oauth2.refreshTokenRotation` | `STS_OAUTH2_REFRESH_TOKEN_ROTATION` | `false` | yes | Issue a NEW refresh token on every refresh, refuse the one that was spent, and treat a replay as a compromise — the whole token family is revoked, not just the token replayed (OAuth 2.1 section 4.3.1, RFC 9700 section 4.14.2). **RFC 9700 mode and OAuth 2.1 mode already do this for every client**, so this row is how to have it with both modes off; turning it off while a mode is on changes nothing, because the mode is the stricter answer. What it does NOT bring with it is the rest of RFC 9700 section 2.2.2 — the idle timeout and the refusal of a refresh naming no client stay behind `oauth2.rfc9700`, because neither is what asking for rotation asked for. (A token presented by a DIFFERENT client, and a refresh asking for more scope than its grant, are RFC 6749 section 6's own MUSTs and refused in every mode.) A refresh token minted before it was turned on carries no family and is rotated from its next use, so there is nothing to migrate. |
...
| `oauth2.requestUriFragmentCheck` | `STS_OAUTH2_REQUEST_URI_FRAGMENT_CHECK` | `true` | yes | OpenID Connect Core section 6.2 gives a request_uri's fragment as the base64url SHA-256 of its content, a signal that a cached copy is out of date. ON (the default) also treats a fragment of that shape as an integrity check, and refuses content that does not hash to it. The section does not require an OP to verify it, and the OpenID conformance suite's request_uri modules send a fragment hashed from random bytes, so its OpenID Connect realms turn this off. Off, the fragment only names a version of the content for `oauth2.requestUriCacheS`. |
...
| `oauth2.deviceAuthorization` | `STS_OAUTH2_DEVICE_AUTHORIZATION` | `false` | yes | Answer RFC 8628 at /oauth2/device_authorization: a device with no browser is given a device code and a user code, the person approves on /portal/device, and the device polls the token endpoint with the device_code grant (#150). OFF by default: a new way in is something a realm turns on, and section 5.4 describes the phishing a device flow invites. |
| `oauth2.deviceCodeLifetimeS` | `STS_OAUTH2_DEVICE_CODE_LIFETIME_S` | `600` | yes | How long a device code and its user code stay valid (RFC 8628 expires_in). Short, because the user code is short. |
| `oauth2.deviceCodeIntervalS` | `STS_OAUTH2_DEVICE_CODE_INTERVAL_S` | `5` | yes | The minimum wait between a device's polls (RFC 8628 interval); a poll sooner is answered slow_down and the interval grows by five seconds. |
...
| `saml2.unsolicitedSso` | `STS_SAML2_UNSOLICITED_SSO` | `true` | yes | Whether /saml2/unsolicited[/{sp}] sends an UNSOLICITED Response (saml-profiles-2.0-os section 4.1.5) — a sign-in started at this identity provider, for a service provider that did not ask: `providerId` (or the path segment) names it, `shire` one of its registered assertion consumer services, `target` the RelayState. The service provider must be registered in product, the address must be one its metadata or its entry registered, and the issuance policy is asked as for any sign-in. The assertion carries no InResponseTo, so a service provider decides for itself whether it accepts one; turn this off for a realm whose providers should only ever be answered. |
...
| `saml11.doNotCacheCondition` | `STS_SAML11_DO_NOT_CACHE_CONDITION` | `false` | yes | Put a <saml:DoNotCacheCondition/> in the Conditions of an assertion sent on the Browser/POST profile. OFF by default (#189): the profile does not ask for one — its single-use policy (oasis-sstc-saml-bindings-1.1 section 4.1.2) is the RELYING PARTY's to keep. WARNING: turning it on makes the Shibboleth SP refuse every such assertion with its stock security-policy.xml; turn it on only for relying parties known to honour the condition. |
...
| `tls.groups` | `STS_TLS_GROUPS` | `X25519MLKEM768:SecP256r1MLKEM768:SecP384r1MLKEM1024 / X25519:P-256 / X448:P-384:P-521` | **restart** — the TLS contexts are built when the listeners are created | The key-exchange groups of the same listeners, as an OpenSSL groups list (node's `ecdhCurve`). Post-quantum hybrids first; `/` separates tuples, and a client that supports a group in an earlier tuple than the key share it sent is asked (HelloRetryRequest) for one there. No finite-field group. **Empty means node's `auto`**, which offers only X25519MLKEM768 of the hybrids and adds ffdhe2048 and ffdhe3072. A list that builds no context stops the service at startup. |
| `tls.signatureAlgorithms` | `STS_TLS_SIGALGS` | OpenSSL's list without DSA and SHA-224, brainpool omitted by policy: ML-DSA, ECDSA (NIST curves), EdDSA, RSA-PSS, RSA PKCS #1 v1.5 with SHA-256/384/512 | **restart** — the TLS contexts are built when the listeners are created | The signature schemes the same listeners sign with and accept from a client certificate, and what a CertificateRequest advertises (node's `sigalgs`). **Empty means OpenSSL's default, which advertises DSA and SHA-224 in TLS 1.2** and so a `dss_sign` certificate type, and brainpool in TLS 1.3 (a client certificate whose EC key is on a curve other than the NIST ones is then closed after the handshake, `STS-TLS-0035`). A list that builds no context stops the service at startup. |
...
| `oid4vp.clientIdPrefix` | `OID4VP_CLIENT_ID_PREFIX` | `pre-registered` | yes | How a wallet is to authenticate a SIGNED request (OpenID4VP section 5.9): pre-registered, decentralized_identifier, verifier_attestation, openid_federation, x509_san_dns or x509_hash. An unsigned request always uses redirect_uri. |
| `oid4vp.x509DnsName` | `OID4VP_X509_DNS_NAME` | *(empty)* | yes | The DNS name of the x509_san_dns Client Identifier, carried in the Verifier's certificate. It must be the Response URI's host. Empty: the host of `global.publicBaseUrl`; with neither, development takes the request's host and product refuses. |
| `oid4vp.x509SigningAlgorithm` | `OID4VP_X509_SIGNING_ALGORITHM` | `ES256` | yes | The JWS algorithm of an x509_san_dns or x509_hash Request Object, and so the realm key the Verifier's certificate is over. |
...
| `scim.inventOnCreate` | `SCIM_INVENT_ON_CREATE` | `true` | yes | Development mode only: whether a person a SCIM client creates is filled in with the invented persona and the /admin/vc attributes the client did not send. Off, a create writes exactly what was sent. Product invents nothing either way. |
...
| `ssf.foreignPollS` | `STS_SSF_FOREIGN_POLL_S` | `30` | yes | How often the ssf.foreign-poll scheduler job polls every foreign transmitter this realm registered with a poll stream (RFC 8936) (#153). |
| `ssf.foreignPollMaxEvents` | `STS_SSF_FOREIGN_POLL_MAX_EVENTS` | `50` | yes | The maxEvents this realm asks a foreign transmitter for in one RFC 8936 poll. |
| `ssf.foreignPollMaxRounds` | `STS_SSF_FOREIGN_POLL_MAX_ROUNDS` | `5` | yes | How many polls one run makes while the transmitter says moreAvailable, before leaving the rest to the next run; one more acknowledges what the last received. |
| `ssf.foreignMaxTransmitters` | `STS_SSF_FOREIGN_MAX_TRANSMITTERS` | `20` | yes | The most foreign transmitters one realm may register. |
| `ssf.foreignInboxMax` | `STS_SSF_FOREIGN_INBOX_MAX` | `500` | yes | The most Security Event Tokens from foreign transmitters kept per realm, the oldest dropped first. What is kept is also what a replayed jti is recognised by. |
| `ssf.foreignTimeoutMs` | `STS_SSF_FOREIGN_TIMEOUT_MS` | `10000` | yes | How long one request to a foreign transmitter — discovery, its token endpoint, stream management, a poll — may take. |
...
| `risk.geoMinimumCount` | `STS_RISK_GEO_MINIMUM_COUNT` | `3` | yes | The fewest distinct people a place is numbered with on Monitoring → Geolocation and `GET /admin-api/geolocation` (#255). A continent, country or total with fewer is shaded but carries no number; a city with fewer is neither drawn nor listed, and is counted in its country's "other cities" line. 1 numbers every place, which can identify a person by where they signed in from. Monitoring → Risk. |
...
| `devices.eventsKept` | `STS_DEVICES_EVENTS_KEPT` | `5000` | yes | How many device creations, removals, evictions and compliance changes this realm keeps for Monitoring → Devices. The oldest is dropped when a new one is recorded past it. |
| `devices.complianceFeedMaxReports` | `STS_DEVICES_COMPLIANCE_FEED_MAX_REPORTS` | `500` | yes | The most device reports one POST /admin-api/device-compliance (the MDM or posture feed, under the device:compliance scope) may carry. A larger batch is refused whole (STS-DEVICE-0034) rather than applied in part, so a feed knows to split it. |
...
| `devices.expectRegistered` | `STS_DEVICES_EXPECT_REGISTERED` | `false` | yes | On, risk scoring's unregistered-device signal (x2) fires for ANY person whose sign-in no registered device of theirs proved. Off — the default — only for a person who has registered a device. It waits for risk.minimumHistory, as new-device does. |
