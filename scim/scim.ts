...
//
// ---------------------------------------------------------------------------
// SCIM 2.0 — SYSTEM FOR CROSS-DOMAIN IDENTITY MANAGEMENT (RFC 7642, 7643,
// 7644).
//
// The fifteenth protocol family here, and the first one whose whole purpose is
// to WRITE. Every other family in this service answers a question about
// somebody who is already there — issue this person a token, tell me who signed
// in, seal this ticket. SCIM is how an identity provider PUTS somebody there in
// the first place, and what it provisions into is the embedded LDAP directory,
// entry for entry, with no store of its own.
//
...
// which is where every hand-rolled SCIM server is subtly wrong — `emails[type
// eq "work"].value` is a path, and treating it as a property name is the defect
...
// for a mock, and would have been wrong in exactly the places a client is
// trying to test. This is the opposite case from Swagger UI, where 11.7 MB and
// a telemetry dependency bought a familiar look for an API with no
// authentication.
//
...
// `registeredRoutes()` in sts_metadata.js walks `app._router.stack` skipping
// any layer with no `.route` — so every SCIM endpoint would have been INVISIBLE
// to the drift check, silently, which is the one thing that page exists to
// prevent. And its constructor REQUIRES an authentication scheme and a handler
// — ONE of each, where RFC 7644 section 2 names six schemes and this service
// now offers all of them, and where the handler it wanted would have had to be
// a single function answering for a bearer token, a Digest nonce exchange and a
// HOBA signature at once. When that argument was first written the objection
// was simpler (there was no authentication at all here, so what it would have
...
// provider exposes, because it creates and DELETES accounts. So `scim_auth.ts`
...
// shape: `Scim` takes scimmy, the logger, the settings, the directory,
// `scim_auth` and the other modules it reads through its constructor as
// `ScimDeps`. Its seventeen SCIM routes, the HOBA registration pair and
// `GET /scim` are registered by `registerRoutes(app)`; the two scimmy
// resource declarations are `declareResources()`; and the capability
...
import scimmyModule = require('scimmy');
const SCIMMY: any = scimmyModule;
...
// WHO IS ASKING. A library like scim_map.ts — it registers nothing and never
...
interface ScimDeps {
  ...
  audit: { audit: Fn };
  directory: Record<'groupsOfUser' | 'resourceIdOfDn' | 'usernameOfEntry' |
                    'claimCreate' | 'readPerson' | 'usersDn' | 'allPersons' |
                    'existingUserEntry' | 'normalizeDn' | 'createUser' |
                    'writePerson' | 'maxEntries' | 'dnForResourceId' |
                    'populateVcAttributesAt' | 'deletePerson' |
  ...
  errorCodes: {
    ...
    codeOf(value: unknown): string | null | undefined;
  };
  // The console, for its reader slot only.
  adminConsole: { setScimReader?: Fn };
}

// An express request as this module reads it, with the two members the gate
// stashes on it.
interface ScimRequest {
  ...
  scimStartedAt?: number;
  scimAuth?: any;
  __scimCreateClaimed?: boolean;
  [member: string]: any;
}

// What a sender is told about the call it answers.
interface CallInfo {
  operation: string;
  resourceType: string;
  need?: string;
...
// A route handler, as `handle()` wraps one.
type ScimHandler = (req: ScimRequest, res: any) => any;

// The base path. NOT a setting: it is baked into every `meta.location` and
// every Location header, a client stores those, and a base path that could move
// at runtime would leave a provisioning client holding URLs that stopped
// resolving with nothing to point at. `/scim/v2` is what every implementation
// uses.
const BASE = '/scim/v2';
...
//     (2026-09-12, `registerHobaKey()` in scim_auth.ts). Either way the
...
class Scim {
  ...
  private maxResults(): number {
    const { log, config } = this.deps;
    log.debug("Entering Scim.maxResults().");
    log.debug("Leaving Scim.maxResults().");
    return config.value('scim.maxResults');
  }

  private bulkMaxOperations(): number {
    const { log, config } = this.deps;
    log.debug("Entering Scim.bulkMaxOperations().");
    log.debug("Leaving Scim.bulkMaxOperations().");
    return config.value('scim.bulkMaxOperations');
  }

  private bulkMaxPayloadSize(): number {
    const { log, config } = this.deps;
    log.debug("Entering Scim.bulkMaxPayloadSize().");
    log.debug("Leaving Scim.bulkMaxPayloadSize().");
  ...
  //
  // SCIMMY.Config is what builds the ServiceProviderConfig document, and it is
  // the ONE place those capabilities are stated — the handlers below carry no
  // second opinion about whether filtering works. Same arrangement
  // `authorization_servers.js` has for the OAuth metadata and there for the
  // same reason: a document that IS the server cannot drift from it.
  //
  ...
  // again at the top of the ServiceProviderConfig handler, so that a change
  // made at /admin/config reaches the published document. Without the second
  // call `scim.maxResults` would be enforced live (queryParams() reads it per
  // request, which is the rule for a runtime setting) while the document went
  // on advertising the number this process started with — a captured `const` in
  ...
  applyCapabilities(base?: string): void {
    ...
    SCIMMY.Config.set({
      patch: true,
      bulk: { supported: true, maxOperations: this.bulkMaxOperations(),
              maxPayloadSize: this.bulkMaxPayloadSize() },
      filter: { supported: true, maxResults: this.maxResults() },
    ...
    //
    // `authenticationSchemes` is the one configuration property scimmy treats
    // as CUMULATIVE: `set` PUSHES onto the array, and only an empty value
    // clears it. This function is deliberately called more than once — at
    // require time and again at the top of the ServiceProviderConfig handler,
    // so that a scheme turned off at /admin/config disappears from the
    // published document — so without the reset the array would grow by four
    // every time somebody read the document, and a client would be handed the
    // same scheme fourteen times with nothing in the code looking wrong.
    //
    ...
    // see schemesForConfig() in scim_auth.ts.
    // -------------------------------------------------------------------------
    ...
    log.debug("Leaving Scim.applyCapabilities(). " + schemes.length +
              " canonical scheme(s).");
  }
  ...
  // baseUrlOf(), which honours a forwarded header only when global.trustProxy
  // says to. Do not pin a scheme or a host here: this service answers correctly
  // as localhost, as `sts` on a compose network and through a published port
  // because nothing in it is told which, and a SCIM `meta.location` pointing at
  // the wrong one is a URL a provisioning client will store and keep using.
  // ---------------------------------------------------------------------------
  private locationPrefix(req: ScimRequest, type: string): string {
    const { log, baseUrlOf } = this.deps;
    log.debug("Entering Scim.locationPrefix().");
    log.debug("Leaving Scim.locationPrefix().");
    return baseUrlOf(req) + BASE + '/' + type + '/';
  }

  // How long this request took, in milliseconds, or null when nothing stamped
  // it. NULL AND NOT ZERO: admin_stats.js counts a call with no duration
  // towards the call total and towards nothing else, so an unstamped path shows
  // up as a smaller latency sample rather than as a fast request. The only
  // unstamped path today would be a sender called from outside handle(), which
  // is a thing this file does not do and this function is what keeps honest if
  // it ever does.
  private elapsedFor(req: ScimRequest): number | null {
    const { log } = this.deps;
    log.debug("Entering Scim.elapsedFor().");
    const started = req && req.scimStartedAt;
    log.debug("Leaving Scim.elapsedFor().");
  ...
  // status AS A STRING, and an optional `scimType` from a closed list. The
  // status being a string is the detail everybody gets wrong and every client
  // checks.
  //
  // It is built through SCIMMY.Messages.ErrorResponse rather than by hand so
  // that the shape comes from the same library that produced the error, and the
  // counting happens HERE — one place, at the moment an answer goes out —
  // rather than at each handler, which is the reasoning that keeps signJwt()
  // the single token counter.
  // ---------------------------------------------------------------------------
  // The code for the condition a SCIM error below was thrown for, carried on
  // the Types.Error itself under the non-enumerable Symbol errorCodes.mark()
  // uses. scimmy rethrows a Types.Error untouched and ErrorResponse serialises
  // only schemas, status, scimType and detail, so the body is unchanged.
  private readonly coded = (code: string, error: any): any => {
    const { log, errorCodes } = this.deps;
    log.debug("Entering Scim.coded().");
    log.debug("Leaving Scim.coded().");
    return errorCodes.mark(error, code);
  };

  private sendScimError(req: ScimRequest, res: any, info: CallInfo,
                       ex: any): void {
    const { log, stats, errorCodes, SCIMMY } = this.deps;
    const { coded } = this;
    log.debug("Entering Scim.sendScimError(). status=" + (ex && ex.status));
    let error = (ex instanceof SCIMMY.Types.Error) ? ex
      : coded('STS-SCIM-0020',
              new SCIMMY.Types.Error(500, null,
                                     String((ex && ex.message) || ex)));
    // THIS FUNCTION MUST NOT THROW (2026-09-13). ErrorResponse refuses any
    // status outside RFC 7644 section 3.12's list and any scimType that status
    // does not allow, and this is called from a promise's `.catch()` — so a
    // throw here is an unhandled rejection and node ends the PROCESS. A
    // directory write answered 507 did exactly that: one full directory took
    // every protocol on every socket down. A status the section does not list
    // is this service's defect, so it goes out as 500 with the same detail and
    // its own code.
    let body = null;
    try {
      body = new SCIMMY.Messages.ErrorResponse(error);
    } catch (e) {
      log.debug("Caught in Scim.sendScimError(): " + ((e && e.message) || e));
    ...
    stats.recordScim({ operation: info.operation,
                       ...
                       // whatever the caller attempted — the status and
                       // scimType tables beside it already say what happened.
                       ...
                       ms: this.elapsedFor(req), bytes: Buffer.byteLength(text),
    ...
    res.status(error.status)
       ...
       .end(text);
    log.debug("Leaving Scim.sendScimError(). " + error.status + " " +
  ...
  // ADVERTISES `etag: {supported: false}` in its ServiceProviderConfig. A
  // document that says there is no version control, on responses carrying a
  // version, is exactly the drift building the document out of SCIMMY.Config
  // was meant to prevent — and a client that noticed the header and started
  // sending `If-Match` would get its precondition ignored, which is the worst
  // of the three possible behaviours. `app.set('etag', false)` would have fixed
  // it too and would have turned the header off for the whole service, which is
  // not this module's call to make.
  // ---------------------------------------------------------------------------
  private sendScim(req: ScimRequest, res: any, info: CallInfo,
                   status: number, body: unknown, location?: string): void {
    const { log, stats } = this.deps;
    log.debug("Entering Scim.sendScim(). status=" + status);
  ...
  //
  // Three things happen here and each would otherwise be repeated seventeen
  // times, which is how one of them comes to be missing from the eighteenth:
  //
  ...
  //     Resource#read() and #write() CATCH anything that is not a Types.Error
  //     and re-throw it as a 404 "Resource not found", so an ordinary
  //     programming mistake inside an egress handler surfaces to the client as
  //     a missing user. Logging the original here is what makes that findable.
  //   * the counting, which happens in the two senders above.
  //   * AUTHENTICATION, added last and belonging here for exactly the reason
  //     the other three do. Every SCIM route goes through this function, so
  //     this is one gate and not eighteen — and the eighteenth is the one that
  //     would have been missed. `info.need` says what the operation costs:
  //     'read', 'write', or 'none' for the discovery endpoints, which are open
  ...
  // credential for an endpoint that would refuse it anyway — "the feature is
  // off" and "you are not authenticated" send somebody to two different places.
  // ---------------------------------------------------------------------------
  private handle(info: CallInfo, fn: ScimHandler): ScimHandler {
    const { log, scimAuth, errorCodes, SCIMMY } = this.deps;
    const { coded } = this;
    log.debug("Entering Scim.handle().");
    log.debug("Leaving Scim.handle().");
    return (req, res) => {
      log.debug("Entering the SCIM " + info.operation + " handler for " +
                info.resourceType + ".");
      ...
      // /admin/scim/monitor; a request with no stamp is counted with NO
      // duration rather than with a zero, because a zero would pull the mean
      // down and look healthy.
      req.scimStartedAt = Date.now();
      if (!this.enabled()) {
        this.sendScimError(req, res, info,
                           coded('STS-SCIM-0001',
                                 new SCIMMY.Types.Error(501, null,
          'SCIM is turned off on this service (scim.enabled). The routes are ' +
          'registered, which is why this is a 501 and not a 404. Turn it ' +
          'back on at /admin/scim or with SCIM_ENABLED=true.')));
      ...
      // headers it hands back are set either way: on a refusal they are the
      // WWW-Authenticate challenges RFC 7644 section 2 makes a SHALL (and, in
      // the DPoP nonce handshake, the DPoP-Nonce a wallet needs to retry), and
      // on success they can be the RFC 7616 Authentication-Info that lets a
      // client authenticate this server back.
      //
      // **`authenticateSpent()` SINCE 2026-09-14 (#46 section 5)**: the same
      // decision, with a Digest nonce count or a HOBA signature spent across
      // the cluster before it is accepted — which is a round trip, so the rest
      // of this gate runs when it is in.
      scimAuth.authenticateSpent(req, info.need || 'none')
        .then((decision) => {
          Object.keys(decision.headers || {}).forEach((name) => {
          ...
          if (!decision.ok) {
            // scim_auth.ts names the condition on the decision it refused with.
            this.sendScimError(req, res, info,
        ...
        .catch((ex) => {
          ...
          if (!(ex instanceof SCIMMY.Types.Error)) {
            ...
            // that reached it into a 404 and the real message is the only way
            // to tell a genuine "no such user" from a defect in this file.
            log.error(errorCodes.tag('STS-SCIM-0020') +
                      'scim: the ' + info.operation + ' handler for ' +
                      info.resourceType + ' threw: ' +
  ...
  private scimBody(req: ScimRequest): any {
    const { log, validation, errorCodes, SCIMMY } = this.deps;
    const { coded } = this;
    log.debug("Entering Scim.scimBody(). content-type=" +
              (req.headers['content-type'] || '(none)'));
    const raw = typeof req.body === 'string' ? req.body : '';
    if (!raw.trim()) {
      log.debug("Leaving Scim.scimBody(). There was no body.");
      return undefined;
    }
    let parsed;
    try {
      ...
      log.debug("Caught in Scim.scimBody(): " + ((e && e.message) || e));
      log.debug("Leaving Scim.scimBody(). It was not JSON.");
  ...
  // Number.isInteger() — so `?count=5` is silently DROPPED and the client gets
  // the default page size while believing it asked for five. Parsed here, and a
  // value that is not a number is refused rather than ignored, because a client
  // that asked for a page size and got a different one has no way to find out.
  //
  ...
  // ---------------------------------------------------------------------------
  private queryParams(req: ScimRequest): Record<string, any> {
    const { log, SCIMMY } = this.deps;
    const { coded } = this;
    log.debug("Entering Scim.queryParams().");
    ...
    // answering none.
    ['filter', 'attributes', 'excludedAttributes', 'sortBy',
     'sortOrder'].forEach((name) => {
      ...
      if (value !== undefined && String(value) !== '') {
        ...
        if (text.length > SCIM_QUERY_MAX) {
          throw coded('STS-SCIM-0004',
                      new SCIMMY.Types.Error(400, 'invalidFilter',
            'The "' + name + '" parameter is ' + text.length + ' characters ' +
            'and the limit ' +
            'is ' + SCIM_QUERY_MAX + '. A filter is an expression ' +
            'this service parses rather than a value it compares, so its ' +
            'cost is a property of its length.'));
        }
        params[name] = text;
      }
    });
    ['startIndex', 'count'].forEach((name) => {
      const raw = query[name];
      const value = Array.isArray(raw) ? raw[0] : raw;
      if (value === undefined || String(value) === '') {
        return;
    ...
    if (params.count === undefined || params.count > this.maxResults()) {
      params.count = this.maxResults();
    }
    log.debug("Leaving Scim.queryParams(). " + Object.keys(params).length +
              " parameter(s).");
    return params;
  }
  ...
  // `excludedAttributes` are MULTI-VALUED — a JSON array — while on a query
  // string they are one comma-separated string, and scimmy's Resource
  // constructor wants the query-string spelling. `String(["a","b"])` is
  // `"a,b"`, which is exactly right, and that is why this is written as a
  // String() rather than left to happen: an array reaching that constructor
  // unconverted is `["a","b"] pr` in a filter, which parses as nothing and
  // silently returns every attribute.
  private searchParams(body: any): Record<string, any> {
    const { log, SCIMMY } = this.deps;
    const { coded } = this;
    log.debug("Entering Scim.searchParams().");
    const source = body || {};
    const params: Record<string, any> = {};
    ['filter', 'attributes', 'excludedAttributes', 'sortBy',
     'sortOrder'].forEach((name) => {
    ...
    log.debug("Leaving Scim.searchParams(). " + Object.keys(params).length +
              " parameter(s).");
    return params;
  }
  ...
  // AROUND.
  //
  // `Resource#read()` does NOT apply the filter it parsed. It parses it, hands
  // the resource instance (carrying `.filter`) to the egress handler and wraps
  ...
  //
  // **THE DEFECT.** `SCIMMY.Types.Filter#match()` in scimmy 1.3.5 handles a
  // nested attribute by diving into it — `new
  // Filter([expressions]).match([actual])` — without first checking that
  // `actual` is there. When it is not, the recursive call reaches
  // `Object.entries(undefined)` and THROWS. So a filter naming any
  // sub-attribute (`emails.value co "@example.com"`, `name.familyName sw "Sm"`)
  // blows up on the first resource that lacks that member, which in a directory
  // is the ordinary case and not an edge one. The exception surfaces as a 400
  // `invalidValue` saying "Cannot convert undefined or null to object", which
  // points at nothing.
  //
  // It is routed around in `scim_map.ts`'s toScimUser(), which pads every
  // multi-valued and complex member so that the value is always at least an
  // empty array or object, and `prune()` takes the padding off before the
  // resource goes on the wire. That is the same kind of workaround
  // `toSearchEntry()` uses for ldapjs's SearchResponse.send(), and it is
  // documented in both places for the same reason: it is two lines that look
  // like a stylistic choice.
  //
  // The try/catch here is the belt to that braces. A filter this service cannot
  // evaluate is refused as `invalidFilter` — an honest answer a client can act
  ...
  private applyFilter(filter: any, values: any[]): any[] {
    ...
    try {
      ...
      log.debug("Caught in Scim.applyFilter(): " + ((ex && ex.message) || ex));
      log.warn(errorCodes.tag('STS-SCIM-0006') +
               'scim: a filter could not be evaluated (' + ex.message + '). ' +
               'This is refused rather than answered with an empty list, ' +
               'because "no results" and "I could not read your filter" are ' +
               'different answers and a client can only act on the second.');
  ...
  // them — the schema, the coercion, the filter grammar, PATCH, the
  // ListResponse — is scimmy's; everything below them is ldap_server.js's; and
  // this is the boundary, which is why it is short.
  // ---------------------------------------------------------------------------

  // The groups a person is in, in the shape scim_map.ts wants. Read through the
  ...
  private groupsOf(dn: string): any[] {
    const { log, directory } = this.deps;
    log.debug("Entering Scim.groupsOf().");
    const answer = directory.groupsOfUser(dn);
    log.debug("Leaving Scim.groupsOf().");
  ...
  private userResourceFor(entry: any, req: ScimRequest): any {
    ...
    const resource = scimMap.toScimUser(entry, {
      ...
      // matches a name against. What is DONE with it is scim_map.ts's decision
  ...
  // A CREATE THAT ARRIVES INSIDE A BULK REQUEST CLAIMS ITS NAME TOO
  // (2026-09-14, #46 follow-up).
  //
  // `createHandler()` below claims a userName or displayName across nodes
  ...
  // 409 `uniqueness` (`STS-LDAP-0092`), or 500 when the store could not be
  // asked (`STS-LDAP-0093`) — inside a Bulk response, that operation's own
  // status. Inert anywhere a create cannot race.
  // ---------------------------------------------------------------------------
  private claimingIngress(type: string, handler: Fn): Fn {
    const { log, directory, createClaims, SCIMMY } = this.deps;
    const { coded } = this;
    log.debug("Entering Scim.claimingIngress(). " + type);
    ...
    return async (resource, data, ctx) => {
      ...
      try {
        ...
        log.debug("Caught in Scim.claimingIngress(): " +
                  ((e && e.message) || e));
        held.settle(false);
        throw e;
      }
  ...
  // back from GET /Groups is every group by BOTH of ldap_server.js's rules —
  // under ou=groups, OR carrying a group objectClass wherever it sits. So a
  // group a client added under ou=people IS a SCIM Group here. That is not a
  // leak: it is this service having exactly one answer to "what is a group",
  // which the console and the groups claim already use, and a SCIM view with a
  // third opinion would be the second definition that eventually disagrees.
  // ---------------------------------------------------------------------------
  private groupResourceFor(entry: any, req: ScimRequest): any {
    const { log, directory, scimMap } = this.deps;
    log.debug("Entering Scim.groupResourceFor().");
    log.debug("Leaving Scim.groupResourceFor().");
  ...
  declareResources(): void {
    ...
    // carried employeeNumber, department, organization, division and manager
    // since SCIM arrived here, but an attribute scimmy's schema definition does
    // not know about is dropped by its coercion in both directions — so those
    // five went out of a POST and came back as nothing, and /Schemas listed two
    // documents while this service claimed three. Declaring it puts the URN in
    // /Schemas and in the User resource type's schemaExtensions, and makes the
    // namespaced member on the wire the one the mapping writes.
    //
    // `required` is left false: RFC 7643 section 4.3 defines the extension, and
    // a User carrying none of it is an ordinary User rather than an invalid
    // one.
    SCIMMY.Resources.declare(SCIMMY.Resources.User)
      ...
      .egress((resource, ctx) => {
        ...
        if (resource.id) {
          ...
          if (!entry) {
            ...
            // Resource#read() turns a null into "Unexpected empty value
            // returned by egress handler" — a 500 — where this is a perfectly
            // ordinary 404.
        ...
        // it stops being useful. See scim_map.ts's toScimUser().
        return matched.map(scimMap.prune);
      })
      .ingress(claimingIngress('User', (resource, data, ctx) => {
        log.debug("Entering the SCIM User ingress handler. id=" +
                  (resource.id || '(a create)'));
        const req = (ctx || {}).req;

        // TRIMMED ONCE, AND WRITTEN BACK ONTO THE RESOURCE, which is the whole
        // point of doing it here rather than in three places. The name is read
        // three times — to build the DN, to check uniqueness, and to become the
        // `uid` attribute — and trimming it for the first two and not the third
        // produced an entry whose DN said `uid=lead` while its `uid` attribute
        // held `" lead"`. That is one name with two spellings in one entry: the
        // DN a client is handed as its id does not name what the entry says it
        // is, `existingUserEntry()` finds it by one and not the other, and the
        // uniqueness check below stops matching, so the same person can be
        // created twice.
        ...
        if (userName.toLowerCase() === REFUSED_USERNAME) {
          // The reachable negative. See the header: a SCIM client's error
          // handling is built around scimType codes that a permissive server
          // never produces.
          throw coded('STS-SCIM-0008',
                      new SCIMMY.Types.Error(400, 'invalidValue',
            'The userName "' + REFUSED_USERNAME + '" is refused on purpose, ' +
            'so that a negative test has something to fail on — the same ' +
            'reserved value the password grant, WS-Trust, the WS-Federation ' +
            'sign-in screen and every LDAP bind here refuse. Nothing else ' +
            'about this request was wrong.'));
        }
        ...
        // DOOR, AND THIS IS THE ONE THING IN THIS FILE MOST WORTH NOT UNDOING.
        //
        // SCIM is the FOURTH way a person can be put in this directory — after
        // an `ldapadd`, the console's form and POST /admin-api/users/create —
        // and the last three already share one function so that "creating a
        // user" cannot come to mean three things. This one used to build its
        // own DN, run its own uniqueness scan and apply its own name-syntax
        // rule, and each of the three was subtly weaker:
        //
        //   * THE DN. createUser() applies namePlan(), which FOLDS a new name
        //     onto an entry that is already this person's under a different
        //     naming attribute — a client certificate's `cn=rcbj,ou=users`,
        //     say. Building `uid=rcbj,ou=users` directly would have created a
        //     second object for one person, which is precisely what that fold
        //     exists to prevent.
        //   * UNIQUENESS. createUser() asks existingUserEntry(), which matches
        //     on the `uid` attribute AND on the RDN value. The scan here
        //     compared only `uid`, so a person whose entry a certificate had
        //     named by `cn` was invisible to it and SCIM would happily create
        //     them a second time — the "one entry per person at every door"
        //     rule broken at the newest door.
        //   * THE NAME. Both refused RFC 4514's reserved characters and the two
        //     lists had already drifted by one (`#` anywhere, versus only
        //     leading). There is now one regex, `nameUsableInDn()`, and all
        //     three doors read it.
        //
        ...
        // status and a `scimType`. That translation is this module's job and
        // stays here, which is the same split oauth2_bcp.js has with oauth2.js
        // — one decides, the other says it in its own protocol.
        // ---------------------------------------------------------------------
        let dn;
        if (existing) {
          // An update keeps the DN it has. A rename is an LDAP modrdn rather
          // than a PUT — and since 2026-09-14 it would not change the SCIM id
          // either, which is the entry's `entryUUID`.
          dn = existing.dn;

          // UNIQUENESS on an update, which createUser() cannot answer because
          // nothing is being created. The same lookup it uses, so both doors
          // agree about what "taken" means — and compared by DN, so that
          // leaving somebody's userName as it was is not a conflict with
          // themselves.
          const clash = directory.existingUserEntry(userName);
          if (clash &&
              directory.normalizeDn(clash.dn) !== directory.normalizeDn(
                  existing.dn)) {
            throw coded('STS-LDAP-0005',
                        new SCIMMY.Types.Error(409, 'uniqueness',
              'There is already a user called "' + userName + '" here, at ' +
              clash.dn +
              '. RFC 7643 section 4.1.1 makes userName unique, and this ' +
              'directory keeps one entry per person at every door.'));
          }
        } else {
          const made = directory.createUser(userName, {
            ...
            // Now there is a credential, so the row can say whose act this was;
          ...
          if (!made.ok) {
            // `existing` on the refusal is how createUser() reports a name that
            // is taken, and it is the only one of its refusals that is a 409
            // rather than a 400 — a client retries a conflict differently from
            // the way it retries a bad value. The directory's own code for
            // which refusal it was, or this one.
        ...
        // Read back rather than reusing what came in, because on a create the
        // entry createUser() just wrote is what the SCIM attributes are merged
        // OVER — objectClass, the description saying where it came from, and
        // the invented persona values it filled. The window rule then does the
        // rest: what SCIM sent replaces what it maps, and everything else
        // stays.
        ...
        // `manager` holds (2026-09-14); a value naming no entry is kept as
        // sent.
        ...
        if (!written.ok) {
          ...
          throw coded(errorCodes.codeOf(written) || 'STS-SCIM-0011',
            new SCIMMY.Types.Error(written.reason === 'full' ? 500 : 400,
              ...
              written.reason === 'full'
                ? 'The directory holds its maximum of ' +
                  directory.maxEntries() +
                  ' entries (ldap.maxEntries). Nothing was written.'
                : 'The entry could not be written at ' + dn + ' (' +
                  written.reason + ').'));
        }

        // The credential-claim fill, FOR THE PERSON THIS REQUEST CREATED.
        // createUser() already ran applyVcAttributes() on the entry it made —
        // but the write just above REPLACED the attribute set with SCIM's
        // window merged over it, so a mapped attribute the client did not send
        // (a `street` the persona had invented, say) is gone again. This puts
        // back only what is ABSENT, so nothing the SCIM client sent is touched,
        // and /admin/vc's selection reaches a provisioned person exactly as it
        // reaches one who signed in.
        //
        // **IT WAS `populateVcAttributes()` — THE WHOLE-REALM SWEEP — UNTIL
        // 2026-09-07, AND THAT MADE THIS DOOR QUADRATIC.** That function walks
        // every entry in the realm twice, and this called it once per person,
        // to fix up the one entry named on the line above. Five thousand
        // creates over SCIM took 254.6s and slowed from 8.79ms each to 101.54ms
        // as the directory filled; `/admin-api` and LDAP, which never called
        // it, did the same five thousand in 4.1s and 2.2s and stayed flat. See
        ...
        // `user.create` row — naming SCIM, because it now takes a protocol —
        // and a second row from this module would be one act counted twice at
        // the SAME layer, which is the double-recording rule 3c warns about.
        // The HTTP call is a different layer and is recorded by app.js either
        // way.
        if (existing) {
          this.auditScim('user.update', dn, converted.attributes, req);
        }
      ...
      .degress((resource, ctx) => {
        ...
        if (!removed.ok && removed.reason === 'protected') {
          // THE DEFAULT REALM'S BOOTSTRAP ADMINISTRATOR (2026-09-13), which no
          // door may delete — see ldap_server.js's
          // isBootstrapAdministratorEntry(). RFC 7644 section 3.12 lists 403
          // for a request the server refuses to perform on an otherwise valid
          // resource.
        ...
        if (!removed.ok) {
          throw coded(errorCodes.codeOf(removed) || 'STS-SCIM-0012',
            ...
            removed.reason === 'notLeaf'
              ? 'The entry at ' + resource.id + ' has children, and this ' +
                'directory refuses a delete of anything that is not a leaf ' +
                '(RFC 4511 section 4.8). Delete what is under it first.'
              : 'There is no entry at ' + resource.id + ' under ' +
                directory.usersDn() + '.'));
        }
    ...
    SCIMMY.Resources.declare(SCIMMY.Resources.Group)
      .egress((resource, ctx) => {
        log.debug("Entering the SCIM Group egress handler. id=" +
                  (resource.id || '(a list)'));
        const req = (ctx || {}).req;
        if (resource.id) {
          ...
          if (!entry) {
            throw coded('STS-SCIM-0013', new SCIMMY.Types.Error(404, null,
              'There is no group at ' + resource.id + '. An entry that ' +
              'exists and is not a group answers the same way: it is not a ' +
              'Group resource either.'));
          }
      ...
      .ingress(claimingIngress('Group', (resource, data, ctx) => {
        log.debug("Entering the SCIM Group ingress handler. id=" +
                  (resource.id || '(a create)'));
        const req = (ctx || {}).req;
        // Trimmed and written back for the reason the User handler's userName
        // is: this becomes both the RDN value and the `cn` attribute, and
        // trimming one of the two is one group with two names.
        const displayName = String(data.displayName || '').trim();
        if (data.displayName !== undefined) {
          data.displayName = displayName;
        }
        const existing = resource.id ?
          directory.readGroupEntry(resource.id) : null;
        if (resource.id && !existing) {
          throw coded('STS-SCIM-0013',
            new SCIMMY.Types.Error(404, null,
                                   'There is no group at ' + resource.id +
                                   '.'));
        }

        const converted = scimMap.fromScimGroup(data,
        ...
        // The same DN-syntax rule createUser() applies to a username, read from
        // the one place it is written (see nameUsableInDn() in ldap_server.js).
        // A group has no createUser() of its own to defer to — nothing but an
        // `ldapadd` and this creates one — so the CHECK is shared even though
        // the door is not. Only on a create: an update keeps the DN it has, so
        // a displayName that could not have been an RDN moves nothing, and
        // refusing it would make a group an `ldapadd` put there with an awkward
        // name un-editable over SCIM.
        if (!existing && !directory.nameUsableInDn(displayName)) {
          throw coded('STS-LDAP-0046',
                      new SCIMMY.Types.Error(400, 'invalidValue',
            'This displayName carries a character RFC 4514 section 2.4 ' +
            'reserves in a DN (one of , = + < > # ; " \\). A group here is ' +
            'stored at a its entry\'s DN, so such a name would produce a ' +
            'group that cannot be read back. Refused rather than escaped, ' +
            'for the reason createUser() refuses the same characters in a ' +
            'username: an `ldapadd` can still create it, with the escaping ' +
            'written out by the client.'));
        }
        ...
        // A MEMBER THAT NAMES NOTHING IS NOT REFUSED, and that is deliberate
        // rather than an omission. This directory does no referential integrity
        // — a delete leaves the DN behind in every group that listed it — so
        // refusing a member here would make it impossible to reproduce the
        // dangling-member state that /admin/groups exists to report, and would
        // be this service enforcing in one direction what it explicitly does
        // not enforce in the other. Logged, so it is visible rather than
        // silent. A MEMBER ARRIVES AS A SCIM ID AND IS STORED AS A DN
        // (2026-09-14): a `member` value is a DN in LDAP, and every reader of a
        // group here — the groups claim, /admin/groups, an ldapsearch — reads
        // it as one. A value that names no entry is kept as it was sent, for
        // the reason below.
        ...
        (converted.attributes.member || []).forEach((value) => {
          if (!directory.readPerson(value) &&
              !directory.readGroupEntry(value)) {
            log.info('scim: ' + dn + ' lists ' + value + ' as a member and ' +
                     'nothing is stored there. It is written anyway — this ' +
                     'directory does no referential integrity, and a ' +
                     'dangling member is a state worth being able to produce.');
          }
        });
        ...
        this.auditScim(written.created ? 'group.create' : 'group.update', dn,
                  converted.attributes, req);

        log.debug("Leaving the SCIM Group ingress handler. The entry was " +
                  (written.created ? 'created.' : 'updated.'));
        return scimMap.prune(
          this.groupResourceFor(directory.readGroupEntry(dn), req));
      }))
      .degress((resource, ctx) => {
        ...
        this.auditScim('group.delete', resource.id, {}, (ctx || {}).req);
        log.debug("Leaving the SCIM Group degress handler.");
      });
    log.debug("Leaving Scim.declareResources().");
  }

  // ---------------------------------------------------------------------------
  // THE AUDIT ROW FOR A SCIM WRITE.
  //
  // It uses the DIRECTORY vocabulary — `user.create`, `group.update` and the
  // rest — rather than a set of SCIM actions of its own, and that is the
  // decision worth recording. A SCIM POST and an `ldapadd` are the same act
  // arriving by two routes; giving them different action names would mean a
  // reader filtering /admin/audit for "a user was created" seeing only half the
  // creations, which is the blind spot rule 3c warns about. The PROTOCOL column
  // is what says which door it came through, and the channel is `http` because
  // it did.
  //
  ...
  // audit page already says happens and is why this does not try to suppress
  // it.
  // ---------------------------------------------------------------------------
  private auditScim(action: string, dn: string, attributes: any,
                    req: ScimRequest): void {
    const { log, audit } = this.deps;
    log.debug("Entering Scim.auditScim().");
    audit.audit({
      action: action,
      // The authenticated caller, which this surface has had since the SCIM
      // endpoints started requiring one. It is passed in rather than resolved
      // by audit.js's actor resolver, because that resolver reads the browser
      // session and a provisioning client has none — the whole point of the
      // schemes in scim_auth.ts is that a caller can be somebody without one.
      actor: (req && req.scimAuth && req.scimAuth.principal) || '',
      protocol: 'SCIM',
      channel: 'http',
      target: dn,
      summary: 'SCIM ' + (action.indexOf('.create') > 0 ? 'created' :
                          action.indexOf('.delete') > 0 ? 'deleted' :
                          'updated') +
               ' ' + dn,
      detail: { attributes: Object.keys(attributes || {}).sort().join(', ') }
    });
    log.debug("Leaving Scim.auditScim().");
  }
  ...
  // the ServiceProviderConfig, authenticates, and is told there is nobody to
  // be.
  //
  ...
  // write path, and therefore nothing that can come to disagree about what a
  // User resource is.
  //
  ...
  // there. A caller whose credential names somebody with no directory entry
  // gets a 404 instead — the alias resolved and there is nothing at the end of
  // it, which is a different sentence.
  //
  // The COMMON case for that 404 is worth knowing: a client_credentials token
  // has no person behind it, and a client certificate authenticates a DN. Both
  // are perfectly good credentials for provisioning somebody else and neither
  // is anybody /Me could be.
  // ---------------------------------------------------------------------------
  private meSubject(req: ScimRequest): string {
    const { log, stats, directory, SCIMMY } = this.deps;
    const { coded } = this;
    log.debug("Entering Scim.meSubject().");
    const decision = req.scimAuth || {};
    if (!decision.ok || decision.anonymous || !decision.principal) {
      log.debug("Leaving Scim.meSubject(). Nobody authenticated.");
      throw coded('STS-SCIM-0015', new SCIMMY.Types.Error(501, null,
        '/Me is an alias for the subject the request authenticated as (RFC ' +
        '7644 section 3.11), and this request authenticated as nobody — ' +
        'authentication is turned off here, so there is no subject to alias. ' +
        'Present a credential, or ask for the user by id. This is a 501 ' +
        'rather than a 404 because the alias is unavailable, not because the ' +
        'resource is missing.'));
    }
    // Through the identity normalisation every other reader of this directory
    // uses, so that `alice`, `urn:uuid:<entryUUID>` and `alice@REALM` reach ONE
    // entry — the same fold recordAuthentication() applies, and the reason this
    // is not a lookup by the raw principal. objectFor() then handles all three
    // identity shapes, including the DN of a client certificate and a DID,
    ...
    if (!located || !located.found) {
      log.debug("Leaving Scim.meSubject(). " + key + " has no entry.");
      throw coded('STS-SCIM-0016', new SCIMMY.Types.Error(404, null,
        'This request authenticated as "' + decision.principal + '", and ' +
        'there is no entry for them ' +
        'under ' + directory.usersDn() + ' — so the alias resolves to ' +
        'nothing. That is the ordinary answer for a client_credentials token ' +
        'or a client certificate, neither of which has a person behind it: ' +
        'both are good credentials for provisioning somebody else, and ' +
        'neither is anybody /Me could be. Create the entry first, or use ' +
        '/Users.'));
    }
    log.debug("Leaving Scim.meSubject(). " + located.dn);
    return located.dn;
  }

  private listHandler(type: string, Resource: any): ScimHandler {
    const { log } = this.deps;
    log.debug("Entering Scim.listHandler().");
    log.debug("Leaving Scim.listHandler().");
    return async (req, res) => {
      const list = await new Resource(this.queryParams(req))
        .read({ req: req });
      this.sendScim(req, res, { operation: 'list', resourceType: type }, 200,
                    list);
    };
  }

  private searchHandler(type: string, Resource: any): ScimHandler {
    const { log, SCIMMY } = this.deps;
    const { coded } = this;
    log.debug("Entering Scim.searchHandler().");
    log.debug("Leaving Scim.searchHandler().");
    return async (req, res) => {
      const body = this.scimBody(req) || {};
      // The schema URN is REQUIRED on a SearchRequest (section 3.4.3) and is
      // checked, unlike most things here: a POST to .search carrying an
      // ordinary resource body is a client that meant to create something, and
      // answering it as an empty search would be the most confusing possible
      // reply.
      const schemas = Array.isArray(body.schemas) ? body.schemas : [];
      if (schemas.indexOf(SEARCH_REQUEST_URN) < 0) {
        throw coded('STS-SCIM-0018',
                    new SCIMMY.Types.Error(400, 'invalidSyntax',
          'A .search body must carry schemas: ["' + SEARCH_REQUEST_URN +
          '"] (RFC 7644 section 3.4.3).'));
      }
      const list = await new Resource(this.searchParams(body))
        .read({ req: req });
      this.sendScim(req, res, { operation: 'search', resourceType: type }, 200,
                    list);
    };
  }

  private readHandler(type: string, Resource: any): ScimHandler {
    const { log } = this.deps;
    log.debug("Entering Scim.readHandler().");
    log.debug("Leaving Scim.readHandler().");
    return async (req, res) => {
      const one = await new Resource(String(req.params.id),
                                     this.queryParams(req)).read({ req: req });
      this.sendScim(req, res, { operation: 'read', resourceType: type }, 200,
                    one);
    };
  }

  private createHandler(type: string, Resource: any): ScimHandler {
    const { log, directory, createClaims, SCIMMY } = this.deps;
    const { coded } = this;
    log.debug("Entering Scim.createHandler().");
    log.debug("Leaving Scim.createHandler().");
    return async (req, res) => {
      const body = this.scimBody(req);
      // -----------------------------------------------------------------------
      // THE NAME IS CLAIMED FIRST WHEN SEVERAL NODES WRITE ONE STORE
      // (2026-09-14, #46 section 3). Two creates of one userName on two nodes
      // each found nobody and each answered 201 with an id, and the store could
      ...
      if (!held.ok && held.reason === 'store') {
        ...
        // same reading `scim_auth.ts` makes of its own store failure.
        throw coded('STS-LDAP-0093', new SCIMMY.Types.Error(500, null,
          createClaims.refusalMessage(held)));
      }
      if (!held.ok) {
      ...
      let created;
      try {
        created = await new Resource(this.queryParams(req))
          .write(body, { req: req });
      } catch (e) {
        log.debug("Caught in Scim.createHandler(): " + ((e && e.message) || e));
        held.settle(false);
        throw e;
      }
      held.settle(true);
      this.sendScim(req, res, { operation: 'create', resourceType: type }, 201,
                    created,
                    this.locationPrefix(req, type + 's') +
                      encodeURIComponent(created.id));
    };
  }

  private replaceHandler(type: string, Resource: any): ScimHandler {
    const { log } = this.deps;
    log.debug("Entering Scim.replaceHandler().");
    log.debug("Leaving Scim.replaceHandler().");
    return async (req, res) => {
      const body = this.scimBody(req);
      const updated = await new Resource(String(req.params.id),
                                         this.queryParams(req))
        .write(body, { req: req });
      this.sendScim(req, res, { operation: 'replace', resourceType: type }, 200,
                    updated);
    };
  }

  // PATCH is the one operation this file does not implement at all: scimmy's
  // PatchOp reads the resource through egress, applies the operations against
  // the schema, and writes the result back through ingress. So the two handlers
  // above are what makes PATCH work, and there is no third code path to keep in
  // step.
  //
  ...
  private modifyHandler(type: string, Resource: any): ScimHandler {
    ...
    return async (req, res) => {
      ...
      if (patched === undefined) {
        this.sendScim(req, res, { operation: 'modify', resourceType: type },
                      204, undefined);
        return;
      }
      this.sendScim(req, res, { operation: 'modify', resourceType: type }, 200,
                    patched);
    };
  }

  private deleteHandler(type: string, Resource: any): ScimHandler {
    const { log } = this.deps;
    log.debug("Entering Scim.deleteHandler().");
    log.debug("Leaving Scim.deleteHandler().");
    return async (req, res) => {
      await new Resource(String(req.params.id)).dispose({ req: req });
      this.sendScim(req, res, { operation: 'delete', resourceType: type }, 204,
                    undefined);
    };
  }
  ...
  //
  // The same shape /admin/ldap/service and /tls have: a page that says what the
  // protocol surface is, what it will and will not do, and where to point a
  // client — plus ?format=json so a test can read the same facts. It is NOT a
  // SCIM endpoint and says so; a real SCIM server publishes none of this.
  // ---------------------------------------------------------------------------
  private pageShell(title: string, inner: string): string {
    const { log, xmlEscape } = this.deps;
    log.debug("Entering Scim.pageShell().");
    log.debug("Leaving Scim.pageShell().");
    return '<!DOCTYPE html>\n<html lang="en"><head><meta charset="utf-8">' +
      ...
      'rgba(0,0,0,.08)}h1{font-size:1.3em;margin:0 ' +
      '0 4px;color:#12107c}h2{font-size:1em;margin:1.4em 0 ' +
      '.4em}p.sub{color:#666;font-size:.85em;margin:0 0 ' +
      '18px}.warn{background:#fff8e1;border:1px solid #ffe082;padding:9px ' +
      '12px;border-radius:5px;font-size:.82em;margin:0 0 ' +
      '16px}table{border-collapse:collapse;width:100%;margin:.5rem 0 ' +
  ...
  description(req: ScimRequest): any {
    ...
    const out = {
      ...
      baseUrl: base + BASE,
      specifications: ['RFC 7642', 'RFC 7643', 'RFC 7644', 'RFC 6750',
                       'RFC 7617', 'RFC 7616', 'RFC 7486', 'RFC 9449'],
      // Every scheme, whether it is on, what it costs a caller, and the access
      // control policy behind it — from scim_auth.ts's table, which is the same
      ...
      store: {
        what: 'The embedded LDAP directory in this process. There is no ' +
              'second store and no cache: a SCIM POST and an ldapadd write ' +
              'the same entry, and a person provisioned here appears on ' +
              '/admin/users, in an ldapsearch, and in the attributes their ' +
      ...
      identifiers: {
        ...
        why: 'RFC 7643 section 3.1 asks for an opaque, server-assigned, ' +
             'unique identifier that is never reassigned. It was the ' +
             'entry\'s DN until 2026-09-14, and a rename reassigned it. The ' +
             'entryUUID is assigned when the entry is created, survives a ' +
             'rename, and is the same value a person\'s token carries in ' +
             '`sub` as urn:uuid:<value>. A DN presented as an id still ' +
             'resolves, for a client that stored one before the change. ' +
             'Member and manager values are ids too, and are stored as the ' +
             'DNs LDAP holds.',
      ...
      endpoints: [
        ...
        { method: 'GET', path: BASE + '/ResourceTypes',
          what: 'User and Group.' },
        { method: 'GET', path: BASE + '/Schemas',
          what: 'The core User and Group schemas, and the enterprise User ' +
                'extension.' },
        { method: 'GET', path: BASE + '/Users',
          what: 'The list, with ?filter, ?sortBy, ?startIndex, ?count, ' +
                '?attributes.' },
        { method: 'POST', path: BASE + '/Users', what: 'Create.' },
        { method: 'POST', path: BASE + '/Users/.search',
          what:
            'The same query as a POST body, for a filter too long for a URL.' },
        ...
        { method: '(the same seven)', path: BASE + '/Groups',
          what: 'Groups, whose members are resolved from member, ' +
                'uniqueMember and memberUid alike.' },
        { method: 'POST', path: BASE + '/.search',
          what: 'A query across BOTH resource types at once.' },
        { method: 'POST', path: BASE + '/Bulk',
          what: 'Up to ' + this.bulkMaxOperations() +
        ...
        { method: 'POST', path: '/.well-known/hoba/register',
          what: 'Register a HOBA public key (RFC 7486 section 7). ' +
                'Unauthenticated on purpose — it is how a caller gets a ' +
                'credential. GET describes it.' }
      ],
      ...
      doesNotDo: [
        ...
        'apply at these endpoints only — no other surface here reads a ' +
        'scope, and holding one confers nothing beyond /scim/v2. Note also ' +
        'that only the OAuth schemes carry scopes at all: Basic, Digest, ' +
        'HOBA, a cookie and a client certificate authenticate a caller who ' +
        'may then do both, so a caller who cannot get a scope can simply use ' +
        ...
        'one-second resolution would be a concurrency control a client ' +
        'trusts and that is wrong; and no password in this service is ' +
        'checked, so there is none to change.',

        'A MEMBER THAT NAMES NOTHING IS ACCEPTED. This directory does no ' +
        'referential integrity on purpose — deleting a user leaves their DN ' +
        'in every group that listed them — so refusing a dangling member ' +
        'here would make that state impossible to reproduce. /admin/groups ' +
        'reports them.'
      ],
      reachableNegatives: [
        { what: 'A request with no credential',
          answer: '401, with a WWW-Authenticate header per offered scheme — ' +
                  'RFC 7644 section 2 makes that header a SHALL. The ' +
                  'discovery endpoints are exempt unless scim.authDiscovery ' +
                  'is on.' },
        { what: 'An access token with the wrong scope for the operation',
          answer: '403 with WWW-Authenticate: Bearer ' +
                  'error="insufficient_scope", ' +
                  'scope="' + scimAuth.scopeWrite() + '". Reads need "' +
                  scimAuth.scopeRead() + '" and writes need "' +
                  scimAuth.scopeWrite() + '"; neither implies the other.' },
        { what: 'An access token this service did not issue, or one that was ' +
                'revoked',
          answer: '401. These endpoints verify the signature, unlike the ' +
                  'OID4VCI credential endpoints, which accept a foreign ' +
                  'token: a scope on a token nobody verified is a permission ' +
                  'its holder wrote for themselves.' },
        { what: 'Basic with the password "' + scimAuth.REFUSED_PASSWORD + '"',
          answer:
            '401 — the same reserved value every other family here refuses.' },
        { what: 'Digest with a wrong password, a stale nonce, or a repeated nc',
          answer: '401 three ways: the password really is checked here, a ' +
                  'stale nonce carries stale=true (which a conforming client ' +
                  'retries silently), and a replayed nonce count does NOT — ' +
                  'it was a valid credential and has been seen before, which ' +
                  'is a different sentence.' },
        { what: 'A HOBA signature that does not verify, or a reused ' +
                '(kid, challenge, nonce)',
          answer: '401. The signature is really verified — RSA-SHA256 over ' +
                  'RFC 7486 section 5\'s length-prefixed blob — and the ' +
                  'challenge may be REUSED until its max-age, so what is ' +
                  'refused as a replay is the triple and not the challenge.' },
        ...
        { what: 'GET /Me with no credential, or POST /Me',
          answer: '501 — the alias is unavailable rather than the resource ' +
                  'missing. A credential naming somebody with no entry gets ' +
                  '404.' },
        { what: 'A userName of "' + REFUSED_USERNAME + '"',
          answer: '400 invalidValue — the same reserved value the password ' +
                  'grant, WS-Trust, the WS-Federation sign-in screen and ' +
                  'every LDAP bind here refuse.' },
        ...
        { what: 'A userName or displayName carrying an RFC 4514 special ' +
                'character (a comma, a quote, a plus, a hash, a semicolon, ' +
                'an equals, an angle bracket or a backslash)',
          answer: '400 invalidValue — the entry\'s DN is built from the ' +
                  'name, so a name carrying one would produce an entry the ' +
                  'other doors cannot name. It is the SAME refusal the ' +
                  'console and the management API give, from the same rule; ' +
                  'an ldapadd can still create such an entry with the ' +
                  'escaping written out.' },
        { what: 'A userName that is DN-shaped or DID-shaped',
          answer: '400 invalidValue — an entry named by a distinguished name ' +
                  'gets here by presenting a client certificate and one ' +
                  'named by a decentralized identifier by presenting that, ' +
                  'so there is nothing to create one from by hand. Also the ' +
                  'console\'s and the management API\'s answer, from the ' +
                  'same function.' },
        { what: 'An id that names nothing', answer: '404.' },
        { what: 'A filter this server cannot evaluate',
          answer: '400 invalidFilter — refused rather than answered with an ' +
                  'empty list, because "no results" and "I could not read ' +
                  'your filter" are different answers.' },
        { what: 'Any method on /Me', answer: '501.' }
      ],
      mapping: Object.assign({
        what: 'Which LDAP attribute each SCIM member is. The whole table, ' +
              'with the schema document that defines each attribute, is on ' +
              '/admin/scim.'
      // ONE PROJECTION, IN scim_map.ts, SINCE 2026-09-06. This endpoint and
      ...
      // itself differently. That module's describeMapping() argues it at
      // length.
  ...
  registerRoutes(app: any): void {
    const { log, xmlEscape, baseUrlOf, audit, directory, scimAuth, errorCodes,
            SCIMMY } = this.deps;
    const { coded } = this;
    log.debug("Entering Scim.registerRoutes().");
    // -------------------------------------------------------------------------
    // THE ROUTES.
    //
    // Registered against the shared app one by one, in the order RFC 7644
    // section 3.2 tabulates them. Two ordering notes:
    //
    //   * `.search` is registered BEFORE `/:id` for each resource type. It does
    //     not matter today, because `.search` is a POST and there is no POST on
    //     `/:id` — but the day somebody adds one, `/Users/.search` would start
    //     being routed as an id of `.search`, and the failure would be a 404
    //     for a request that looks perfectly correct.
    //   * every one of these is visible to `GET /admin/sts-metadata`, which is
    //     the reason they are here rather than behind a mounted Router. See the
    //     header.
    // -------------------------------------------------------------------------

    // --- discovery (section 4) -----------------------------------------------

    app.get(BASE + '/ServiceProviderConfig', this.handle(
      { operation: 'discovery', resourceType: 'ServiceProviderConfig',
        need: 'none' },
      async (req, res) => {
        // Re-applied so that a runtime change to scim.maxResults, either bulk
        // limit, or which authentication schemes are offered is in the document
        // as well as in the enforcement. See applyCapabilities().
        this.applyCapabilities(baseUrlOf(req));
        const document =
          await new SCIMMY.Resources.ServiceProviderConfig().read();
        const body = JSON.parse(JSON.stringify(document));

        // ---------------------------------------------------------------------
        // THE THREE SCHEMES RFC 7643 HAS NO NAME FOR, AND THE `primary` FLAG.
        //
        // RFC 7644 section 2 names six ways to authenticate and RFC 7643
        // section 5 gives `authenticationSchemes.type` five canonical values,
        // and the two lists do not cover each other: there is no canonical
        // value for a client certificate, a cookie or HOBA. scimmy enforces the
        // canonical five (its ServiceProviderConfig definition carries them as
        // canonicalValues and its coercion throws on anything else), which is a
        // correct reading of that document — so the four it can validate go
        // through SCIMMY.Config and the other three are appended HERE, to the
        // serialised document, carrying an honest type of their own. Both
        // halves come from ONE table in scim_auth.ts, so the document cannot
        // advertise a scheme that is turned off nor omit one that is on.
        //
        // A ServiceProviderConfig listing four of the seven ways in would be
        // the most misleading document this service publishes, and it is the
        // first thing a SCIM client reads. `primary` is added here for the same
        // reason: RFC 7643's example in section 8.5 carries it and its schema
        // definition does not define it, so scimmy rejects it as an unknown
        // sub-attribute.
        // ---------------------------------------------------------------------
        ...
        body.authenticationSchemes.forEach((scheme) => {
        ...
        this.sendScim(req, res,
                      { operation: 'discovery',
                        resourceType: 'ServiceProviderConfig' },
                      200, body);
      }));

    app.get(BASE + '/ResourceTypes', this.handle(
      { operation: 'discovery', resourceType: 'ResourceType', need: 'none' },
      async (req, res) => {
        const list = await new SCIMMY.Resources.ResourceType(
            this.queryParams(req)).read();
        this.sendScim(req, res,
                      { operation: 'discovery', resourceType: 'ResourceType' },
                      200, list);
      }));

    app.get(BASE + '/ResourceTypes/:id', this.handle(
      { operation: 'discovery', resourceType: 'ResourceType', need: 'none' },
      async (req, res) => {
        const one = await new SCIMMY.Resources.ResourceType(
            String(req.params.id)).read();
        this.sendScim(req, res,
                      { operation: 'discovery', resourceType: 'ResourceType' },
                      200, one);
      }));

    app.get(BASE + '/Schemas', this.handle(
      { operation: 'discovery', resourceType: 'Schema', need: 'none' },
      async (req, res) => {
        const list =
          await new SCIMMY.Resources.Schema(this.queryParams(req)).read();
        this.sendScim(req, res,
                      { operation: 'discovery', resourceType: 'Schema' }, 200,
                      list);
      }));

    app.get(BASE + '/Schemas/:id', this.handle(
      { operation: 'discovery', resourceType: 'Schema', need: 'none' },
      async (req, res) => {
        const one =
          await new SCIMMY.Resources.Schema(String(req.params.id)).read();
        this.sendScim(req, res,
                      { operation: 'discovery', resourceType: 'Schema' }, 200,
                      one);
      }));

    app.get(BASE + '/Me', this.handle(
      { operation: 'read', resourceType: 'Self', need: 'read' },
      async (req, res) => {
        const one = await new SCIMMY.Resources.User(this.meSubject(req),
                                                    this.queryParams(req))
          .read({ req: req });
        this.sendScim(req, res, { operation: 'read', resourceType: 'Self' },
                      200, one);
      }));

    app.put(BASE + '/Me', this.handle(
      { operation: 'replace', resourceType: 'Self', need: 'write' },
      async (req, res) => {
        const updated = await new SCIMMY.Resources.User(this.meSubject(req),
                                                        this.queryParams(req))
          .write(this.scimBody(req), { req: req });
        this.sendScim(req, res, { operation: 'replace', resourceType: 'Self' },
                      200, updated);
      }));

    app.patch(BASE + '/Me', this.handle(
      { operation: 'modify', resourceType: 'Self', need: 'write' },
      async (req, res) => {
        const patched = await new SCIMMY.Resources.User(this.meSubject(req),
                                                        this.queryParams(req))
          .patch(this.scimBody(req), { req: req });
        if (patched === undefined) {
          this.sendScim(req, res, { operation: 'modify', resourceType: 'Self' },
                        204, undefined);
          return;
        }
        this.sendScim(req, res, { operation: 'modify', resourceType: 'Self' },
    ...
    app.delete(BASE + '/Me', this.handle(
      { operation: 'delete', resourceType: 'Self', need: 'write' },
      async (req, res) => {
        // It deletes the caller's own entry, and nothing here stops it — bar
        // the default realm's bootstrap administrator, which the shared delete
        // handler refuses at every door. That is the same permissiveness as the
        // rest of this surface rather than an oversight — a provisioning
        // client's deprovisioning path is exactly what this mock exists to let
        // somebody run, and refusing self-deletion would be this service
        // inventing a rule the specification does not have.
        await new SCIMMY.Resources.User(this.meSubject(req))
          .dispose({ req: req });
        this.sendScim(req, res, { operation: 'delete', resourceType: 'Self' },
                      204, undefined);
      }));

    app.post(BASE + '/Me', this.handle(
      ...
      (req) => {
        ...
        // request already exists by the time the request is being handled.
        // Refused with the reason rather than aliased onto a create, which
        // would put the caller's own name on somebody else's body.
        this.meSubject(req);
        throw coded('STS-SCIM-0017', new SCIMMY.Types.Error(501, null,
          'POST /Me would create the subject this request authenticated as, ' +
          'and that subject already exists — it is what the credential ' +
          'named. Create somebody with ' +
          'POST ' + BASE + '/Users. The other four methods on /Me do work.'));
      }));

    // HOBA registration — see the note above HOBA_REGISTER_PATH, at the top.

    app.get(HOBA_REGISTER_PATH, (req, res) => {
      log.debug("Entering GET " + HOBA_REGISTER_PATH + ".");
      const auth = scimAuth.describe(req);
      /** @type {any} */
      const hoba = auth.schemes.filter((row) => {
        return row.id === 'hoba';
      })[0] || {};
      res.status(200)
         ...
         .set('Cache-Control', 'no-store')
         .send(JSON.stringify({
        what: 'Client public key registration for HOBA (RFC 7486 section 7), ' +
              'which is one of the authentication schemes the SCIM endpoints ' +
              'here accept.',
        enabled: !!hoba.enabled,
        method: 'POST',
        contentType: 'application/x-www-form-urlencoded',
        parameters: {
          pub: 'REQUIRED. A PEM SubjectPublicKeyInfo block — an RSA public ' +
               'key. RFC 7486 registers algorithm 0 (RSA-SHA256) and 1 ' +
               '(RSA-SHA1); this service accepts 0.',
          username: 'REQUIRED unless the request carries a browser session ' +
                    'cookie. Who the key is for. This parameter is this ' +
                    'service\'s own: RFC 7486 registers a key inside an ' +
                    'already-authenticated context and there is rarely one ' +
                    'here.',
          kid: 'Optional. The key id you will send in the credential. ' +
               'Defaults to a hash of the key itself, so that two keys ' +
               'cannot claim one id.'
        },
        answers: 'On success, 201 with the header Hobareg: regok (RFC 7486 ' +
                 'section 7) and a JSON body naming the kid, the username ' +
                 'and the directory entry the key went on.',
        thenAuthenticateWith:
          'Authorization: HOBA result="kid.challenge.nonce.sig"',
        theChallengeIsOn: 'any 401 from ' + BASE +
                          ', in a WWW-Authenticate header',
        nothingIsCheckedAboutTheRegistration:
          'Anybody may register any key for any name, and the name is ' +
          'created in the directory if it is new. The SIGNATURE is then ' +
          'really verified, which is the half that makes this worth having — ' +
          'a signature check that passed anything would not be the scheme.',
        storedAs: 'hobaPublicKey on the person\'s entry under ' +
                  directory.usersDn() + ', ' +
    ...
    app.post(HOBA_REGISTER_PATH, (req, res) => {
      ...
      // is an entry being updated, and a reader filtering /admin/audit for
      // "what happened to this person" wants it beside the rest. The action is
      // `user.update` rather than one of its own for the reason auditScim()
      // gives: one act, one vocabulary, whichever door it came through.
      audit.audit({
        action: 'user.update', actor: result.body.username, protocol: 'SCIM',
        channel: 'http',
    ...
    [{ type: 'User', endpoint: '/Users', Resource: SCIMMY.Resources.User },
     { type: 'Group', endpoint: '/Groups',
       Resource: SCIMMY.Resources.Group }].forEach((row) => {
      const path = BASE + row.endpoint;
      // `need` is the whole of the access control policy at this layer: reading
      // needs the read scope and everything else needs the write scope, and the
      // two do not imply one another. A POST to `.search` is a READ that
      // happens to use POST because a filter can be longer than a URL (RFC 7644
      // section 3.4.3) — deciding this on the HTTP method rather than on the
      // operation would have made the one endpoint whose method lies about it
      // need the write scope.
      app.get(path,
              this.handle({ operation: 'list', resourceType: row.type,
                            need: 'read' },
                          this.listHandler(row.type, row.Resource)));
      app.post(path,
               this.handle({ operation: 'create', resourceType: row.type,
                             need: 'write' },
                           this.createHandler(row.type, row.Resource)));
      // Before /:id — see the note above.
      app.post(path + '/.search',
               this.handle({ operation: 'search', resourceType: row.type,
                             need: 'read' },
                           this.searchHandler(row.type, row.Resource)));
      app.get(path + '/:id',
              this.handle({ operation: 'read', resourceType: row.type,
                            need: 'read' },
                          this.readHandler(row.type, row.Resource)));
      app.put(path + '/:id',
              this.handle({ operation: 'replace', resourceType: row.type,
                            need: 'write' },
                          this.replaceHandler(row.type, row.Resource)));
      app.patch(path + '/:id',
                this.handle({ operation: 'modify', resourceType: row.type,
                              need: 'write' },
                            this.modifyHandler(row.type, row.Resource)));
      app.delete(path + '/:id',
                 this.handle({ operation: 'delete', resourceType: row.type,
                               need: 'write' },
                             this.deleteHandler(row.type, row.Resource)));
    });

    // --- the root search (section 3.4.3) -------------------------------------

    app.post(BASE + '/.search', this.handle(
      ...
      async (req, res) => {
        ...
        // ACROSS BOTH RESOURCE TYPES, which is what the root .search means and
        // is the one thing it does that the per-type one cannot. scimmy's
        // SearchRequest#apply() does the fan-out and the merge.
        //
        // ITS CONSTRUCTOR WANTS A DIFFERENT SHAPE FROM THE RESOURCE CONSTRUCTOR
        // and that asymmetry cost an afternoon. It validates `schemas` itself
        // and REFUSES a request object without one — so the members cannot
        // simply be handed over the way the per-type handlers hand them to a
        // Resource — and it wants `attributes`/`excludedAttributes` as ARRAYS,
        // which is the SearchRequest spelling, where a Resource wants the
        // comma-separated query-string one. So the two are converted back here
        // rather than reusing searchParams()'s output unchanged.
        const request = Object.assign({}, this.searchParams(body),
                                      { schemas: [SEARCH_REQUEST_URN] });
        ['attributes', 'excludedAttributes'].forEach((name) => {
        ...
        this.sendScim(req, res, { operation: 'search', resourceType: 'User' },
                      200, list);
      }));

    // --- bulk (section 3.7) --------------------------------------------------

    app.post(BASE + '/Bulk', this.handle(
      // A BulkRequest carries creates, replaces, patches and deletes and never
      // a read (RFC 7644 section 3.7 defines no GET operation inside one), so
      // it is a write whatever is in it — and a bulk of nothing but deletes
      // must not be reachable with a read-only credential because the envelope
      // looked harmless.
      { operation: 'bulk', resourceType: 'Bulk', need: 'write' },
      async (req, res) => {
        const body = this.scimBody(req);
        // The payload limit is checked here rather than left to the express
        // body parser, because the parser's limit is a service-wide 5 MB and
        // this one is advertised in the ServiceProviderConfig — a client reads
        // a published limit as a promise, and a request refused at a different
        // size than the document says is the drift this arrangement exists to
        // prevent.
        const size = Buffer.byteLength(
          typeof req.body === 'string' ? req.body : '', 'utf8');
        if (size > this.bulkMaxPayloadSize()) {
          throw coded('STS-SCIM-0019', new SCIMMY.Types.Error(413, null,
            'This BulkRequest is ' + size +
            ' bytes and the advertised maximum is ' +
            this.bulkMaxPayloadSize() + ' (scim.bulkMaxPayloadSize, ' +
                                        'published as ' +
        ...
        // happened inside it. The counters see it as one `bulk` plus whatever
        // the operations inside recorded on their own way through — which is
        // stated on /admin/scim, because a reader adding the column up will
    ...
    app.get('/scim', (req, res) => {
      ...
      const endpoints = info.endpoints.map((row) => {
        return '<tr><td><code>' + xmlEscape(row.method) +
          '</code></td><td><code>' +
          xmlEscape(row.path) + '</code></td><td>' + xmlEscape(row.what) +
          '</td></tr>';
      }).join('');
      const negatives = info.reachableNegatives.map((row) => {
        return '<tr><td>' + xmlEscape(row.what) + '</td><td>' +
          xmlEscape(row.answer) +
          '</td></tr>';
      }).join('');
      const auth = info.authentication;
      const schemeRows = auth.schemes.map((row) => {
        return '<tr><td>' + xmlEscape(row.name) +
          (row.primary ? ' <em>(primary)</em>' : '') + '</td>' +
          '<td><code>' + xmlEscape(row.type) + '</code>' +
          (row.canonical ? '' : ' <em>(no canonical value in RFC 7643)</em>') +
          '</td><td>' + (row.enabled ? 'offered' : 'off') +
          ' <span class="sub">(<code>' + xmlEscape(row.setting) +
          '</code>)</span></td><td>' + (row.scoped ? 'scopes' : 'everything') +
          '</td><td>' + xmlEscape(row.spec) + '</td></tr><tr><td ' +
          'colspan="5" class="sub">' + xmlEscape(row.description) +
          '</td></tr>';
      }).join('');
      const policyRows = auth.policy.map((text) => {
        return '<li>' + xmlEscape(text) + '</li>';
      }).join('');
      const mapping = info.mapping.user.map((row) => {
        return '<tr><td><code>' + xmlEscape(row.scim) +
          '</code></td><td><code>' +
      ...
      const inner = '<h1>SCIM 2.0 — provisioning into the directory</h1>' +
        '<p class="sub">RFC 7642, 7643 and 7644, at <code>' +
        xmlEscape(info.baseUrl) +
        '</code>. ' + (info.enabled ? '' : '<strong>Turned off</strong> ' +
          '(<code>scim.enabled</code>) — every endpoint answers 501. ') +
        'This page is not a SCIM endpoint; a real server publishes none of ' +
        'it.</p><div class="warn"><strong>These endpoints require a ' +
        'credential' +
        ...
        info.store.groupCount + ' group(s), by placement or by ' +
                                'objectClass</li>' +
        '<li>The whole directory holds ' + info.store.entryCount +
        ' of a maximum ' +
        info.store.maxEntries + ' entries</li></ul>' +
        '<h2>The <code>id</code> is the DN</h2>' +
        '<p>' + xmlEscape(info.identifiers.why) + '</p>' +
        '<p>For example: <code>' + xmlEscape(info.identifiers.example) +
        '</code></p><h2>Authentication</h2><p>RFC 7644 section 2 defines no ' +
        'credential of its own — it delegates to TLS and to RFC 7235 and ' +
        'NAMES six schemes. All six are here. Its one <em>SHALL</em> is that ' +
        'a provider indicate its schemes in <code>WWW-Authenticate</code>, ' +
        'which every 401 from these endpoints does; its one <em>MUST</em> is ' +
        'that a provider be able to map an authenticated client to an access ' +
        'control policy, which is the list under the table. The realm is ' +
        '<code>' + xmlEscape(auth.realm) + '</code>. Discovery ' +
        '(<code>/ServiceProviderConfig</code>, <code>/ResourceTypes</code>, ' +
        '<code>/Schemas</code>) is ' +
        (auth.discoveryOpen ? 'OPEN — a client ' +
          'has to be able to read which schemes exist before it can use one' :
        ...
        '</code> and <code>' + xmlEscape(auth.scopes.write) + '</code>, ' +
        'published in <code>scopes_supported</code> in both discovery ' +
        'documents. Digest ' +
        'offers ' + xmlEscape(auth.digestAlgorithms.join(', ')) +
        '. HOBA keys are registered at ' +
        '<code>' + xmlEscape(auth.hobaRegistration) + '</code>.</p>' +
        ...
        endpoints +
        '</table><h2>What ' +
        'it deliberately does not do</h2><ul>' +
        info.doesNotDo.map((text) => {
          return '<li>' + xmlEscape(text) + '</li>';
        }).join('') + '</ul>' +
        '<h2>Things you can make fail</h2>' +
        '<table><tr><th>Do this</th><th>Get this</th></tr>' + negatives +
        '</table><h2>The User ' +
        'mapping</h2><table><tr><th>SCIM</th><th>LDAP</th><th>How</th></tr>' +
        mapping + '</table><p ' +
        'class="sub"><a href="/scim?format=json">This page as JSON</a> ' +
        '&middot; <a href="' + xmlEscape(BASE) +
        '/ServiceProviderConfig">ServiceProviderConfig</a> ' +
        '&middot; <a href="/admin/scim">the console page</a> &middot; <a ' +
        'href="/admin/ldap/service">the directory this writes into</a></p>';
      res.status(200).type('html').set('Cache-Control', 'no-store')
         .send(this.pageShell('SCIM 2.0', inner));
      log.debug("Leaving GET /scim. HTML.");
    });
    log.debug("Leaving Scim.registerRoutes().");
  }
  ...
  fillConsoleSlot(): void {
    ...
    if (typeof adminConsole.setScimReader === 'function') {
      adminConsole.setScimReader(this.description.bind(this));
    } else {
      log.warn('scim: the admin console offers no setScimReader(), so ' +
               '/admin/scim will report the counters and not the surface. ' +
               'SCIM itself is unaffected.');
    }
    log.debug("Leaving Scim.fillConsoleSlot().");
  }
  ...
  announce(): void {
    ...
    log.info('scim: SCIM 2.0 is registered at ' + BASE + ' and provisions ' +
