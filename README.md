...
* **`product`** runs the same protocol implementations with the permissiveness taken
  ...
  * no deliberate defect, broken algorithm (SHA-1 signatures, RSA PKCS#1 v1.5
    key transport) or unsigned SAML assertion is used, whatever a setting
    says;
  * several containers can serve one PostgreSQL store.

`GET /admin/mode` (Server configuration → Mode) and `GET /admin-api/mode` say,
for the running realm, what each mode does and which development-only settings
...
| `federation/` | **federation relationships** — the register, the attribute mapping, the sign-in endpoints, a partner's sign-out in both directions, and the only outbound request this service makes |
...
requires the others and listens, and **`sts_metadata.ts`**, which reads the router
...
| **8092** | tcp | `spiffe.workloadPort` / `STS_SPIFFE_WORKLOAD_PORT` | The SPIFFE **Workload API** over gRPC. `0` turns it off and leaves the Unix socket alone. **Not bound in product mode** unless `spiffe.workloadTcpSourceAuthenticated` declares the network authenticates source addresses, and then only on a named `spiffe.grpcHost` (#166) |
...
| *(off)* | tcp | `spiffe.brokerPort` / `STS_SPIFFE_BROKER_PORT` | The **SPIFFE Broker API** over gRPC (#170), mutual TLS: a broker named in `spiffe.brokers` asks for the SVIDs of a workload it references by process id or Kubernetes pod, which this service attests. 0 by default, so nothing is bound until a realm sets it |
...
2026-09-22).** `oauthAllowedScope` is RFC 7591 section 2's `scope` — a
registration writes it and returns it — and is edited like any other list
attribute, on the console and through `/admin-api/applications/add` and
`remove`. This service's own protected scopes (`admin:read`, `admin:write`, the
SCIM and Shared Signals scopes, the debugger permission) are issued only to a
client that lists them, in both modes, and `/admin-api`, SCIM and Shared Signals
ask again on every call; in product every other scope is held to the list, or,
where there is none, to the default set of OpenID Connect's six and the realm's
OpenID4VCI scopes. `oauthScope` beside it is only what the client has asked for.
...
| `oid4vp.federationAuthorityHints` | `OID4VP_FEDERATION_AUTHORITY_HINTS` | *(empty)* | yes | The `authority_hints` of the realm's OpenID Federation Entity Configuration. |
...
* **The authentication policy's TOTP row off** (it was `totp.enabled` until
  #64; Directory → Policies) **does not disable an existing enrolment**, and that is
...
`display_values_supported` is absent, and so is `check_session_iframe` unless
`oauth2.sessionManagement` is on (the encryption
...
implementation detail. **In product mode (2026-09-21) both tokens must verify**, and
this paragraph describes development. And `actor_token` becomes the RFC 8693 `act`
claim — the delegation record saying *this* client is acting for *that* subject, which
is the only part of the response a downstream resource server can reason about — and
it **nests**: a subject token that already carried `act` keeps its actors beneath the
new one (section 4.1).

**Who may act for whom is decided since #108 (2026-09-23).** The client, and the actor
it names, must be allowed to reach every `audience` and `resource` — by
`appAllowedToDelegateTo` on the client's entry or `appAllowedToActOnBehalfOf` on the
target's — an exchange with no `actor_token` (impersonation) needs
`appTrustedToImpersonate`, the client's `appDelegationSubjectGroup` narrows who it may
act for, and a person carrying `stsNotDelegated` or on the console roster is never
delegated; then the issuance policy may Deny action-id `delegate`. Product refuses —
`invalid_request`, or `invalid_target` for a target — and refuses a `scope` wider than
the verified subject token's; development issues and `/admin/delegation` says what would
have been refused. **`may_act` (section 4.4) is read in every mode**: a subject token
naming somebody other than the actor is refused, and every access token about a person
who has named a delegate (`stsMayAct`, on `/portal/delegate` or through `POST
/admin-api/users/set-may-act`) carries it.
...
`authenticatorData ‖ SHA-256(clientDataJSON)` for ES256/384/512, RS256/384/512,
PS256/384/512, EdDSA and ML-DSA keys. **Since #105 the attestation statement is verified
too**, under `webauthn.attestationPolicy` — in product by default, in development only
when asked: all eight WebAuthn Level 3 section 8 formats, the chain against configured
anchors and the FIDO Metadata Service's roots, revocation, and MDS status reports. What
each key's statement proved is on its row on `/portal/keys`, `/admin/users` and
`GET /admin-api/users`; a key nothing verified is shown as *claimed*, because a client
that believed this service's word on an unverified statement would have learned
something false about a real device.
...
`saml.allowSha1Signatures`, and never in product mode, #181) — is checked in every mode against the service
provider's
...
| `GET /federation/metadata/{id}` | this service's own SAML metadata for that partner — an `SPSSODescriptor`, which is the half of this service that is a service provider, with its `SingleLogoutService` on the Redirect and POST bindings |
| `GET\|POST /federation/slo/{id}` | where a partner's sign-out arrives in a browser (#167): a SAML 2.0 `LogoutRequest` or `LogoutResponse`, a WS-Federation `wsignoutcleanup1.0` (ended only once the person confirms it), and the return from the partner's `end_session_endpoint`. Ends only the session the partner names |
| `POST /federation/backchannel-logout/{id}` | the OpenID Connect `backchannel_logout_uri` to register at the partner: its Logout Token, held to Back-Channel Logout 1.0 section 2.6 |
| `GET /federation/frontchannel-logout/{id}` | the OpenID Connect `frontchannel_logout_uri` to register at the partner, `iss` and `sid` required, framed by the partner's origin alone |
...
It does not check the partner's
certificate against a CA or its validity dates — `fedSigningCertificate` is a
**pinned key**, which is the stronger of the two for this purpose. It holds no
refresh token belonging to anybody else's service, and it does not poll a
partner about a person once the session exists.

**A PARTNER MAY ENCRYPT, AND PRODUCT REQUIRES IT (#168, 2026-09-23)** — an
`<EncryptedAssertion>` was refused as "no assertion" until then, so the only
way to federate was for the partner to send the person's identifier and
attributes in clear through their browser. Every SAML 2.0, WS-Federation and
OpenID Connect relationship now holds an encryption key of its own under the
realm's Intermediate, published in its metadata (`KeyDescriptor
use="encryption"`) or at `/federation/jwks/{id}` (`use: enc`), rotated from the
console or `POST /admin-api/federation/rotate-key` with a grace period
(`federation.encryptionKeyGraceS`). An `<EncryptedAssertion>`, `<EncryptedID>`
and `<EncryptedAttribute>`, a WS-Federation token and a JWE ID Token or Logout
Token are decrypted under exactly the algorithms the relationship publishes —
RSA-OAEP with SHA-256 and AES-256-GCM, or ECDH-ES and A256GCM, by default —
and AES-CBC, `rsa-1_5` and `RSA1_5` are refused in every mode. In product a
plaintext assertion is refused unless the relationship sets
`fedAllowUnencrypted`, which is documented with a warning. See
`docs/federation.md`.

**A PARTNER'S SIGN-OUT ENDS THE SESSION IT STARTED (#167, 2026-09-23)** — it
was refused at the ACS and dropped until then, so a session here outlived the
partner's by hours. `/federation/slo/{id}` takes a SAML 2.0 `<LogoutRequest>`
(signed and verified against the relationship's certificate, from `fedPeer`,
addressed here, fresh, and accepted once ever) and answers a signed
`<LogoutResponse>`, and takes WS-Federation's unsigned `wsignoutcleanup1.0` only
once the person confirms it on a page with a real button;
`/federation/backchannel-logout/{id}` holds an OpenID Connect Logout Token to
Back-Channel Logout 1.0 section 2.6, its `jti` once ever; and
`/federation/frontchannel-logout/{id}` needs `iss` and `sid` and may be framed by
the partner's origin alone. Each ends only the federated session it names, through
`/logout`'s one model, so this service's own relying parties are told in turn. The
partner's SAML `SessionNotOnOrAfter` is the session's latest end, and a sign-out
here offers the partner its own — a signed `LogoutRequest`, RP-Initiated Logout
with `id_token_hint`, or `wsignout1.0`. SAML 1.1 and OAuth 2.0 define no
sign-out.

`federation/CLAUDE.md` argues every one of those, and carries the list of
negatives a test has to cover — this is the surface where that costs most, the
only one here whose bugs are security bugs rather than fidelity bugs, and a
happy path proves close to nothing. `tests/vendored/sts_federation_realms.js`,
`sts_federation_subject_policy.js` and `sts_federation_signout.js` drive most of
those negatives over HTTP; `federation/CLAUDE.md` says which are still open.
...
**It was `/sts-metadata` until 2026-08-24 and it is a page of the admin console now.** Three things changed with it and all three are the reason: it is drawn by `admin.js`'s `page()`, so it has the console's sidebar — it used to be a cul-de-sac with no way back to anything — its breadcrumb and its gate banner; it is **behind the console gate** like everything else under `/admin` — unconditionally, since `global.mode` replaced `admin.authRequired` on 2026-09-06 — so a browser with no session is sent to the sign-in screen and a `?format=json` caller is refused `401 login_required` rather than redirected; and `sts_metadata.ts` now builds only the *body* of a page, because `page()` emits the console's one stylesheet and a second one inside `<body>` would be markup no validator accepts. The require goes one way only — `sts_metadata.ts` requires `admin.js` for the shell, never the reverse — because this module must stay LAST in `server.js` and a require from the console would drag every console route behind it.
...
**The endpoint list is read from the running Express router, not written down.** That is the whole design: a hand-kept list of endpoints in a file beside the endpoints goes stale the first time somebody adds a route, and the failure is silent in the worst direction — the page still looks complete. `app._router.stack` is walked **per request** (not at require time, where the answer would depend on module load order) and the table in `sts_metadata.ts` only supplies the *name* and the *description* for a path the router reports. Both kinds of drift are then reported on the page itself and fail this repository's own `tests/vendored/sts_metadata.js`:

* a route **registered and undescribed** is listed as UNDOCUMENTED — it still appears, with its methods, because the page's first duty is to be a true list of what is callable. Adding an endpoint to this service therefore costs one entry in `sts_metadata.ts`, which is the point.
...
**The authentication policy is on the same page** (#64), and it is a separate policy: which mechanisms a realm accepts as a FIRST factor (a password, a passkey, a TLS client certificate, a Kerberos ticket, a wallet, a federation partner, an emailed code, an emailed sign-in link) and as a SECOND (a password after another factor, a security key, an authenticator app, a recovery code, a wallet, an emailed code or link), and whether a second factor is required of everybody (`requireSecondFactor: always`) or of those who hold one (`if-held`). It is `cn=default,ou=authnPolicies`; **a realm with none of its own follows the default realm's**, and the built-in defaults where neither exists. It replaced `authn.mfaRequired`, `totp.enabled` and `backupCodes.enabled`. `/admin/policies` holds every kind of policy — `GET /admin-api/policies` answers each as a member, and `POST /admin-api/policies/{save,reset}-{password,authn}-policy` writes them.

**The emailed code and the emailed sign-in link are OFF by default**, because NIST SP 800-63B-4 section 3.1.3.1 says email SHALL NOT be used for out-of-band authentication; the page says so beside them. Turned on, they are offered only where the realm can send mail, and only to a person whose address is VERIFIED. As a second factor a person opts in on `/portal/mfa`; as a first factor the sign-in screen offers "Email me a sign-in code" and "Email me a sign-in link" beside the password, and answers an unknown account with the same page as a real one. A code is six digits, a link a single-use URL that works only in the browser that asked for it; each is kept as a scrypt hash, valid for at most ten minutes, spent once, and a person's emailed factor is turned off after `emailFailureLimit` consecutive failures. An emailed factor is `amr ["otp"]` and never meets a risk step-up.

**Which addresses are verified** (#64): an address an administrator sets (the console, `/admin-api/users/create` or `set-mail`), SCIM provisions, an administrator writes over LDAP, or a federation partner sends (unless it says `email_verified: false`) is verified as written; an address a PERSON provides is verified by following a link — on `/portal/email` the new address stays pending until they do, and the old one is told when it changes.
...
* **Require MFA** / **Stop requiring MFA** marks the person (`stsMfaRequired`), so the sign-in screen makes them enrol an authenticator app or a security key at `/authn/mfa-setup` before it starts a session. The authentication policy's `requireSecondFactor: always` (Directory → Policies, #64) does the same for everybody in the realm. In product mode either one also refuses the person's own password at the five password-only doors (an LDAP bind, WS-Trust, SCIM, SSF and EST Basic), where an app password is used instead.
* **App passwords** lists the person's app passwords — name, id, the doors each is scoped to, when made and last used, never the password — with a **Revoke** per row, and **Make an app password** generates one scoped to the doors ticked and shows it ONCE. `POST /admin-api/users/create-app-password` and `revoke-app-password`, and `GET /admin-api/users/app-passwords` (paged), are the same acts.

Every one of them sends Shared Signals to the streams that asked: a CAEP `credential-change` for each credential created, updated or removed (password, `app`, `fido2-platform` or `fido2-roaming`, and every other door since #145: see `docs/caep-events.md`), a RISC `account-credential-change-required` for a reset and a reset link, a RISC `recovery-activated` for a reset link, a RISC `credential-compromise` when the reset is marked as caused by one, and a RISC `recovery-information-changed` when recovery codes go. The older per-item Clear buttons and Set password send them too, and so does completing a reset link. `caep.autoEmitTypes` and `risc.autoEmitTypes` name the new types by default.
...
**IN PRODUCT MODE AN UNGRANTED PERMISSION IS REFUSED** (`invalid_scope`), whatever the setting says. **In development it refuses nothing by default.** With `oauth2.delegatedPermissionsEnforced` off — which it is unless somebody turned it on — an ungranted permission is honoured exactly as a granted one is, logged as ungranted, and marked on the console. With it on the same request is refused `invalid_scope` at the authorization endpoint, where the client can still be told, and at the token endpoint for the grants that never reach it. A grant already issued is not re-judged, so turning it on refuses the next request rather than invalidating what is outstanding. Both answers exercise a client and neither is right for every test, which is why the register is fully readable before anybody enforces anything.
...
Nothing here checks a password — that row of *what this service does not do* is unchanged, and consent is a question asked of somebody who has already been let in under any name they typed.

#### Withdrawn means withdrawn (#172)

**Taking a consent back ends what was issued under it.** Every access and refresh token the application holds for that person carrying the scope is revoked on the register every node reads, a refresh token with its whole grant (RFC 7009 section 2.1), and the instant is written down — `oauthConsentWithdrawn` on the person's entry, `oauthGlobalConsentWithdrawn` on the application's for a global consent. A refresh token carries when its grant was made, and **the refresh grant re-checks consent at every refresh**, in both modes, against the directory: a refresh token granted before a withdrawal is `invalid_grant` even after the person consents again, and withdrawing one scope revokes the whole refresh token, because the grant's scope was fixed when it was made (RFC 6749 section 1.5; OpenID Connect Core section 11 is why an `offline_access` grant matters most). With `oauth2.refreshRequiresConsent` on — the default — a refresh token from the authorization endpoint that no recorded consent covered is refused too, which is a grant obtained while consent was off. A global consent is taken away only by `revoke-global-consent`: the generic application edit refuses to remove one, because that door would do half a withdrawal. On this service's own console, portal or debugger it is not refused either — their sessions standing on it end at their next token renewal, and the person signs in again and is asked.

**`/admin/consent` is the register**, both halves under headings that say which is which, with five controls: consent a scope for everybody, stop consenting it, take back one person's answer, withdraw everything one person agreed to for one application, and forget everything one person agreed to. `GET /admin-api/consent` and `POST /admin-api/consent/{action}` are the same five without a browser, and **`/portal/consents`** is where a person withdraws their own.
...
RFC 3961/3962/8009 string-to-key runs for every enctype in `krb5.enctypes` (never `23` in product) with the default
...
* **A Workload API call with no `workload.spiffe.io: true` metadata header.** The
  ...
  without it (section 3). It is a hardening measure against server-side request
  forgery rather than an authentication — anybody can send a header, but a
  request an attacker merely causes a workload to make rarely carries it. It is enforced here
  ...
  else. `spiffe.requireSecurityHeader` turns it off — in development mode only
  (#181); product always refuses.
...
default) and `tls.ciphers` (BCP 195's suites by default, TLS 1.3 first), and
binds `global.host`.
