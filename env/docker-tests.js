// Configuration for the containerized test stack
// (docker-compose-run-tests.yml).
//
// Identical to env/local.js apart from this comment. The level was kept at
// debug until 2026-09-12, because the STS log is the record of what the mock
// issued when a test fails; it is INFO now, like every appconfig file here,
// because every function logs its entry and exit at debug and a debug stack
// writes all of it. STS_LOG_LEVEL=debug is the run that asks for the record
// back. See env/local.js's header and `config.js` for what each setting means.
var config = {
  ...
  oauth2: {
    ...
    // RFC 6749 section 4.1.2: a code used twice is refused in every mode
    // since #187, and this is the opt-in that restores the old development
    ...
    // job asserts the refusal (rcbj/id-proto-debugger#306). The OpenID
    // conformance jobs switch it off in their own realms, so they test the
    // default. env/test.js carries the same line, for the same job:
    // ./run-tests.sh gives the service that file unless a debug level asks
    // for this one.
