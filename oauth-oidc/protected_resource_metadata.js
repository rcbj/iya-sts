...
// understands, how it takes a bearer token. `/admin/applications/new` can now be
// handed one — pasted, uploaded, or fetched from a URL — and turn it into an
...
//     (`mode.acceptsNonconformingResourceMetadata()`). A document fetched from a
//     well-known URL MUST carry the resource identifier that URL was built from,
//     and a client MUST NOT use one that does not — it is how a document hosted
//     on one resource is kept from describing another. A pasted or uploaded
//     document has no URL to compare against and is not checked; that is said
//     rather than skipped. The same predicate decides a `resource` that is not
//     https (section 2's "a URL that uses the https scheme").
...
const USER_AGENT =
    require('../common/version').userAgent('protected-resource-metadata');
...
function memberRowFor(name) {
  log.debug("Entering memberRowFor().");
  const text = String(name || '');
  if (MEMBER_BY_NAME[text]) {
    log.debug("Leaving memberRowFor().");
...
function refusal(code, errors, extra) {
...
// ---------------------------------------------------------------------------
function absoluteUrlProblem(name, value) {
  log.debug("Entering absoluteUrlProblem().");
  let parsed = null;
  try {
    parsed = new URL(String(value));
  } catch (e) {
    log.debug("Caught in absoluteUrlProblem(): " + ((e && e.message) || e));
    log.debug("Leaving absoluteUrlProblem().");
...
function memberProblem(name, row, value) {
  log.debug("Entering memberProblem(). name=" + name);
  ...
  if (row.type === 'bool') {
    log.debug("Leaving memberProblem().");
  ...
  if (row.type === 'string' || row.type === 'uri' || row.type === 'jwt') {
    ...
    if (row.type === 'uri') {
      log.debug("Leaving memberProblem().");
      return absoluteUrlProblem(name, value);
    }
  ...
  if (row.type === 'uri-list') {
    const bad = value.map(function (one) {
      return absoluteUrlProblem(name, one);
...
// ---------------------------------------------------------------------------
function decodeSignedMetadata(jwt) {
  log.debug("Entering decodeSignedMetadata().");
  ...
  try {
    ...
    log.debug("Caught in decodeSignedMetadata(): " + ((e && e.message) || e));
    out.why = 'its header or claims are not base64url JSON (' + e.message +
              ')';
  }
  log.debug("Leaving decodeSignedMetadata().");
  return out;
}
...
// refusal. Every member RFC 9728 defines is type-checked; a member it does not
// define is KEPT and listed as an extension, because section 2 says a recipient
// ignores what it does not understand and an import that dropped it would store
// a different document from the one the operator loaded.
// ---------------------------------------------------------------------------
function parseDocument(text) {
  log.debug("Entering parseDocument().");
  const raw = String(text == null ? '' : text);
  if (!raw.trim()) {
    log.debug("Leaving parseDocument(). Empty.");
    return refusal('STS-REG-0076', 'The protected resource metadata ' +
                                   'document is empty.');
  }
  const cap = maxBytes();
  if (Buffer.byteLength(raw, 'utf8') > cap) {
    log.debug("Leaving parseDocument(). Too large.");
    return refusal('STS-REG-0076', 'The document is larger than ' + cap +
  ...
  try {
    ...
    log.debug("Caught in parseDocument(): " + ((e && e.message) || e));
    log.debug("Leaving parseDocument(). Not JSON.");
    return refusal('STS-REG-0076', 'The document is not JSON: ' + e.message +
                   '. RFC 9728 section 3.2 makes it a JSON object.');
  }
  if (!document || typeof document !== 'object' || Array.isArray(document)) {
    log.debug("Leaving parseDocument(). Not an object.");
    return refusal('STS-REG-0076', 'The document is JSON but not a JSON ' +
                   'object, and RFC 9728 section 3.2 makes it one.');
  }
  ...
  if (!walked.ok) {
    log.debug("Leaving parseDocument(). The walk refused it.");
    return refusal('STS-REG-0076', 'The document was refused: ' +
  ...
  Object.keys(document).forEach(function (name) {
    ...
    const problem = memberProblem(name, row, document[name]);
    if (problem) {
  ...
  if (problems.length) {
    log.debug("Leaving parseDocument(). " + problems.length + " problem(s).");
    return refusal('STS-REG-0077', problems, { members: members });
  }

  const warnings = [];
  const scheme = new URL(document.resource).protocol;
  const signedMetadata = typeof document.signed_metadata === 'string'
    ? decodeSignedMetadata(document.signed_metadata) : null;
  ...
  log.debug("Leaving parseDocument(). ok, " + members.length + " member(s).");
...
// removed" first — so `https://api.example.com/` and `https://api.example.com`
// share one well-known URL, and `https://api.example.com/v1` has
// `https://api.example.com/.well-known/oauth-protected-resource/v1`.
// ---------------------------------------------------------------------------
function wellKnownUrlFor(resource) {
  log.debug("Entering wellKnownUrlFor().");
  let parsed = null;
  try {
    parsed = new URL(String(resource || ''));
  } catch (e) {
    log.debug("Caught in wellKnownUrlFor(): " + ((e && e.message) || e));
    log.debug("Leaving wellKnownUrlFor().");
    return '';
  }
...
function resourcesForWellKnownUrl(url) {
  log.debug("Entering resourcesForWellKnownUrl().");
  let parsed = null;
  try {
    parsed = new URL(String(url || ''));
  } catch (e) {
    log.debug("Caught in resourcesForWellKnownUrl(): " +
              ((e && e.message) || e));
    log.debug("Leaving resourcesForWellKnownUrl().");
    return null;
  }
  const path = parsed.pathname;
  if (path !== WELL_KNOWN && path.indexOf(WELL_KNOWN + '/') !== 0) {
    log.debug("Leaving resourcesForWellKnownUrl(). Not a well-known URL.");
    return null;
  }
...
function resourceCheckFor(document, url) {
  log.debug("Entering resourceCheckFor().");
  const resource = String((document && document.resource) || '');
  const expectedWellKnown = wellKnownUrlFor(resource);
  if (!url) {
    log.debug("Leaving resourceCheckFor(). No URL.");
  ...
  const candidates = resourcesForWellKnownUrl(url);
  if (!candidates) {
    log.debug("Leaving resourceCheckFor(). Not well-known.");
    return { checked: false, matches: null, expected: [],
             wellKnownUrl: expectedWellKnown,
  ...
  log.debug("Leaving resourceCheckFor(). matches=" + matches);
  return { checked: true, matches: matches, expected: candidates,
           wellKnownUrl: expectedWellKnown,
           why: matches
             ? 'The document\'s `resource` is the identifier the well-known ' +
               'URL it was fetched from was built from.'
             : 'The document was fetched from ' + url + ', which is built ' +
               'from ' + candidates.join(' or ') + ', and it says its ' +
               '`resource` is ' + resource + '. RFC 9728 section 3.3 says a ' +
               'client MUST NOT use a document that fails this.' };
}
...
// does.
function internalAddressProblem(address) {
  log.debug("Entering internalAddressProblem(). address=" + address);
  ...
  if (!family) {
    log.debug("Leaving internalAddressProblem(). Not an address.");
    return '"' + address + '" is not an IP address';
  }
  const internal = INTERNAL.check(text, family === 4 ? 'ipv4' : 'ipv6');
  log.debug("Leaving internalAddressProblem(). internal=" + internal);
  return internal
...
// by the connection itself and nothing is pinned; in product every address the
// name resolves to is checked and the first is what the request connects to.
// ---------------------------------------------------------------------------
function vetHost(hostname) {
  log.debug("Entering vetHost(). hostname=" + hostname);
  const host = String(hostname || '').replace(/^\[|\]$/g, '');
  if (mode.dialsInternalAddresses()) {
    log.debug("Leaving vetHost(). Development: not pinned.");
  ...
  const judge = function (addresses) {
    ...
    const bad = addresses.map(function (one) {
      return internalAddressProblem(one.address);
    }).filter(function (one) { return !!one; });
    if (bad.length || !addresses.length) {
      log.debug("Leaving judge(). Refused.");
      return refusal('STS-REG-0080', '"' + host + '" resolves to ' +
  ...
  if (literal) {
    log.debug("Leaving vetHost(). A literal address.");
    return Promise.resolve(judge([{ address: host, family: literal }]));
  }
  log.debug("Leaving vetHost(). Resolving.");
  return new Promise(function (resolve) {
    dns.lookup(host, { all: true }, function (error, addresses) {
      if (error) {
        log.debug("Caught in vetHost(): " + ((error && error.message) ||
                                             error));
        resolve(refusal('STS-REG-0081', '"' + host + '" could not be ' +
...
function fetchDocument(url) {
  ...
  if (!fedHttp.outboundAllowed()) {
    log.debug("Leaving fetchDocument(). federation.outbound is off.");
    return Promise.resolve(refusal('STS-REG-0078', 'federation.outbound is ' +
      'off, so this service makes no outbound request at all and the ' +
      'document cannot be fetched. Paste or upload it instead.'));
  }
  const problem = fedHttp.urlProblem(asked);
  if (problem) {
    log.debug("Leaving fetchDocument(). The URL was refused.");
    return Promise.resolve(refusal('STS-REG-0079', 'The URL cannot be ' +
                                   'fetched: ' + problem + '.'));
  }
  const parsed = new URL(asked);
  log.debug("Leaving fetchDocument(). Vetting the host.");
  return vetHost(parsed.hostname).then(function (vetted) {
    if (!vetted.ok) {
      return vetted;
    }
    return new Promise(function (resolve) {
      ...
      const request = agent.get(asked, options, function (res) {
        if (res.statusCode >= 300 && res.statusCode < 400) {
          res.resume();
          done(refusal('STS-REG-0082', 'It answered ' + res.statusCode +
        ...
        if (res.statusCode !== 200) {
          res.resume();
          done(refusal('STS-REG-0083', 'It answered ' + res.statusCode +
        ...
        res.on('data', function (chunk) {
          ...
          if (size > cap) {
            request.destroy();
            done(refusal('STS-REG-0084', 'The document is larger than ' +
      ...
      request.setTimeout(Number(config.value('federation.outboundTimeoutMs')),
                         function () {
        request.destroy();
        done(refusal('STS-REG-0085', 'It did not answer within ' +
      ...
      request.on('error', function (e) {
        ...
        done(refusal('STS-REG-0086', 'The request failed: ' + e.message +
                     '.'));
      });
    });
  });
...
function authorizationServersOf(req) {
  log.debug("Entering authorizationServersOf().");
  ...
  log.debug("Leaving authorizationServersOf(). " + rows.length + " row(s).");
  return rows;
}
...
function compareAuthorizationServers(document, known) {
  log.debug("Entering compareAuthorizationServers().");
  ...
  const rows = listed.map(function (issuer) {
    const match = realm.filter(function (one) {
      return issuerKey(one.issuer) === issuerKey(issuer);
    })[0];
  ...
  log.debug("Leaving compareAuthorizationServers(). " + matched + " of " +
            rows.length + " matched.");
  return {
    listed: rows.length,
    matched: matched,
...
// `oauth2.registeredClientIdPrefix` and `oauth2.registeredClientIdBytes`, read
// the way `oauth2.js`'s registration endpoint reads them, so an application
// imported here and one that registered itself cannot be told apart by their
// identifiers. One already in the registry is drawn again.
// ---------------------------------------------------------------------------
function generateClientId() {
  log.debug("Entering generateClientId().");
  ...
  log.debug("Leaving generateClientId().");
  return id;
}
...
// ---------------------------------------------------------------------------
function permissionFor(scope, resource) {
  log.debug("Entering permissionFor().");
  ...
  log.debug("Leaving permissionFor().");
...
function planFor(document) {
  ...
  const permissions = scopes.map(function (scope) {
    const one = permissionFor(scope, resource);
  ...
  // RFC 9396: the types the resource says it understands become the types this
  // application DECLARES, which is what lets a detail of one reach it. The
  // built-in openid_credential, a name that cannot be one, and a type another
  // application already declares are left out and said so.
  ...
  (Array.isArray(document && document.authorization_details_types_supported)
    ...
    .forEach(function (type) {
      ...
      if (definition.problem) {
        warnings.push('The authorization_details type ' + JSON.stringify(type) +
  ...
  log.debug("Leaving planFor(). " + permissions.length + " permission(s).");
  return {
...
//
//   input    { document, url, file: { name, text } }  — exactly one of the three
...
// ---------------------------------------------------------------------------
function documentTextOf(value) {
  log.debug("Entering documentTextOf().");
  if (value && typeof value === 'object') {
    // A JSON caller may send the document as an object rather than as text.
    log.debug("Leaving documentTextOf(). An object.");
    return JSON.stringify(value);
  }
  log.debug("Leaving documentTextOf().");
  return String(value == null ? '' : value);
}

function refusedLoad(result, context, source) {
  log.debug("Entering refusedLoad().");
...
function load(input, context) {
  ...
  const given = input || {};
  const text = documentTextOf(given.document);
  ...
  if (!sources.length) {
    log.debug("Leaving load(). Nothing given.");
    return Promise.resolve(refusedLoad(refusal('STS-REG-0074',
  ...
  if (sources.length > 1) {
    log.debug("Leaving load(). More than one source.");
    return Promise.resolve(refusedLoad(refusal('STS-REG-0075',
  ...
  const obtained = source === 'url'
    ? fetchDocument(url)
    : Promise.resolve({ ok: true, text: source === 'upload' ? file.text : text,
                        url: '' });
  log.debug("Leaving load(). Source " + source + ".");
  return obtained.then(function (got) {
    if (!got.ok) {
      return refusedLoad(got, context, source);
    }
    if (source === 'url') {
    ...
    const view = analyse(got.text, { source: source, url: got.url || '',
                                     filename: file ? String(file.name || '')
                                                    : '',
                                     contentType: got.contentType || '' },
                         context);
    return view.ok ? view : refusedLoad(view, context, source);
  });
}

// THE HALF OF LOAD THAT NEEDS NO NETWORK — also what redraws the page after a
// refused create, from the document the form carried back, without fetching it
// again.
function analyse(text, origin, context) {
  log.debug("Entering analyse().");
  const from = origin || {};
  const parsed = parseDocument(text);
  if (!parsed.ok) {
    log.debug("Leaving analyse(). Unparsed.");
    return parsed;
  }
  const document = parsed.document;
  const lenient = mode.acceptsNonconformingResourceMetadata();
  const resourceCheck = resourceCheckFor(document, from.url);
  const warnings = parsed.warnings.slice(0);
  if (resourceCheck.checked && !resourceCheck.matches) {
    if (!lenient) {
      log.debug("Leaving analyse(). Section 3.3 refused.");
      return refusal('STS-REG-0087', resourceCheck.why + ' This service is ' +
        'running as a product (global.mode=product), so the document is ' +
  ...
  if (!parsed.httpsResource) {
    const why = '`resource` is ' + document.resource + '; RFC 9728 section ' +
                ...
                'scheme.';
    if (!lenient) {
      log.debug("Leaving analyse(). Not https, refused.");
      return refusal('STS-REG-0088', why + ' This service is running as a ' +
                     'product (global.mode=product), so the document is ' +
                     'refused.');
    }
  ...
  const servers = compareAuthorizationServers(document,
      (context && context.authorizationServers) || []);
  const plan = planFor(document);
  log.debug("Leaving analyse(). ok.");
  return {
    ok: true,
...
// field per member it read in `metadata.<member>`. A member's box being present
// is what makes it an edit; an emptied box removes the member. A member RFC 9728
// does not define, and `signed_metadata`, have no box and are kept exactly as
// they were. Returns the compact JSON to store, or '' where the form carried no
// document.
// ---------------------------------------------------------------------------
function documentFromForm(body) {
  log.debug("Entering documentFromForm().");
  const form = body || {};
  let document = null;
  try {
    document = JSON.parse(String(form.metadata || ''));
  } catch (e) {
    log.debug("Caught in documentFromForm(): " + ((e && e.message) || e));
    log.debug("Leaving documentFromForm(). No document.");
    return '';
  }
  if (!document || typeof document !== 'object' || Array.isArray(document)) {
    log.debug("Leaving documentFromForm(). Not an object.");
    return '';
  }
  Object.keys(document).forEach(function (name) {
    const row = memberRowFor(name);
  ...
  log.debug("Leaving documentFromForm().");
...
module.exports = {
  ...
  MEMBERS: MEMBERS,
  memberRowFor: memberRowFor,
  parseDocument: parseDocument,
  wellKnownUrlFor: wellKnownUrlFor,
  resourcesForWellKnownUrl: resourcesForWellKnownUrl,
  resourceCheckFor: resourceCheckFor,
  internalAddressProblem: internalAddressProblem,
  fetchDocument: fetchDocument,
  authorizationServersOf: authorizationServersOf,
  compareAuthorizationServers: compareAuthorizationServers,
  generateClientId: generateClientId,
  permissionFor: permissionFor,
  planFor: planFor,
  load: load,
  analyse: analyse,
  documentFromForm: documentFromForm
};
