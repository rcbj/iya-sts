...
//     reader, none of which requires it back, so `par.ts` and `oauth2.ts` can
...
// `par.ts`, which holds them anyway, and only while they live.
// ---------------------------------------------------------------------------

// ---------------------------------------------------------------------------
// TYPESCRIPT, AS A CLASS (#50, 2026-09-16) — `common/realm_chooser.ts`'s
// shape: `OAuth2Monitor` takes the logger, the error-code table, the counter
...
// One client's counters: a number per event counter, plus these.
...
interface OAuth2MonitorDeps {
  log: typeof helpers.log;
  errorCodes: typeof errorCodes;
  counters: CounterStore;
  replication: {
    remoteRows(handle: string, realmId: string | undefined,
               key: string): any[];
...
const EVENTS: Record<string, [string, string, string]> = {
  ...
  'stepup.challenged': ['stepUpChallenged',
...
class OAuth2Monitor {
  ...
  emptyRow(): CounterRow {
    ...
    const row: CounterRow = { lastAt: null, lastEvent: null, errors: {} };
    Object.keys(EVENTS).forEach(function (event) {
      row[EVENTS[event][0]] = 0;
    });
    log.debug("Leaving OAuth2Monitor.emptyRow().");
    return row;
  }
  ...
  record(clientId: unknown, event: string, detail?: EventDetail): void {
    ...
    try {
      if (!EVENTS[event]) {
        log.warn(errorCodes.tag('STS-OAUTH-0427') + 'oauth2: the event "' +
                 event + '" is not in oauth2_monitor.ts\'s vocabulary and ' +
                 'was not counted. Add it to EVENTS.');
        log.debug("Leaving OAuth2Monitor.record(). Not in the vocabulary.");
        return;
      }
      ...
      log.debug("Caught in OAuth2Monitor.record(): " +
    ...
    log.debug("Leaving OAuth2Monitor.record().");
  }
  ...
  private merge(id: string): CounterRow {
    ...
    log.debug("Entering OAuth2Monitor.merge().");
    const mine = counters.has(id) ? counters.get(id) : this.emptyRow();
    const theirs = replication.remoteRows('oauth2_monitor.counters',
                                          undefined, id);
    if (!theirs.length) {
      log.debug("Leaving OAuth2Monitor.merge(). This process alone.");
      return mine;
    }
    ...
    log.debug("Leaving OAuth2Monitor.merge().");
    return out;
  }
  ...
  private identifiers(): string[] {
    const { log, counters, replication } = this.deps;
    log.debug("Entering OAuth2Monitor.identifiers().");
    const out: string[] = [];
    ...
    log.debug("Leaving OAuth2Monitor.identifiers().");
    return out.sort();
  }
  ...
  snapshot(): any {
    const { log } = this.deps;
    const self = this;
    log.debug("Entering OAuth2Monitor.snapshot().");
    const rows: Record<string, CounterRow> = {};
    const totals = self.emptyRow();
    self.identifiers().forEach(function (id) {
      const row = self.merge(id);
      rows[id] = row;
      Object.keys(EVENTS).forEach(function (event) {
        const field = EVENTS[event][0];
    ...
    log.debug("Leaving OAuth2Monitor.snapshot(). " +
              Object.keys(rows).length + " row(s).");
    return {
      startedAt: startedAt,
  ...
  reset(): void {
    ...
    counters.clear();
    log.debug("Leaving OAuth2Monitor.reset().");
  }
}
