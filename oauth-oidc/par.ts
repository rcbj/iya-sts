...
// endpoint is `oauth2.ts`'s, because what a refusal looks like is protocol
...
// object is `request_object.ts`'s `verifyObject()`. What is here:
//
...
//   * `resolve()` — what `request_object.ts` calls for a request_uri in the
...
//      this as its first section — see `oauth2_monitor.ts`.
...
// `client_assertion`), which `oauth2.ts` strips before anything is kept, and
...
// modules and `oauth2_monitor.ts`, none of which requires it back. It is
// required LAZILY by `request_object.ts` and plainly by `oauth2.ts`.
// ===========================================================================

// ---------------------------------------------------------------------------
// TYPESCRIPT, AS A CLASS (#50, 2026-09-16) — `common/realm_chooser.ts`'s
// shape. `PushedRequests` takes the logger, `config`, `error_codes` and the
...
class PushedRequests {
  ...
  private refusal(errorCode: string, error: string,
    ...
    log.debug("Entering PushedRequests.refusal(). " + errorCode);
    log.debug("Leaving PushedRequests.refusal().");
  ...
  private capacity(): number {
    const { log, config } = this.deps;
    log.debug("Entering PushedRequests.capacity().");
    const raw = Number(config.value('oauth2.parMaxRequests'));
    log.debug("Leaving PushedRequests.capacity().");
  ...
  isPushedRequestUri(value: unknown): boolean {
    ...
    log.debug("Leaving PushedRequests.isPushedRequestUri().");
  ...
  private describe(record: Json, now?: number): Json {
    const { log } = this.deps;
    log.debug("Entering PushedRequests.describe().");
    const at = typeof now === 'number' ? now : Date.now();
    const state = record.spentAt ? 'spent'
      : (record.expiresAt <= at ? 'expired' : 'live');
    log.debug("Leaving PushedRequests.describe(). " + state);
    return {
      ...
      spent_at: record.spentAt ? new Date(record.spentAt).toISOString() :
        null,
  ...
  sweep(now?: number): number {
    ...
    log.debug("Leaving PushedRequests.sweep(). " + gone.length + " swept.");
    return gone.length;
  }

  // -------------------------------------------------------------------------
  // push(entry) — keep a validated pushed request, and answer its
  // request_uri.
  //
  ...
  push(entry: Json): Json {
    ...
    if (!clientId) {
      // Unreachable from the endpoint, which refuses a push with no
      // client_id (section 2.1) long before this — and a reference bound to
      // nobody is not one this file can make.
      log.debug("Leaving PushedRequests.push(). No client to bind it to.");
      return this.refusal('STS-OAUTH-0405', 'invalid_request',
        'a pushed authorization request is bound to the client that pushed ' +
        'it (RFC 9126 section 2.2), and this one names no client.');
    }
    const now = Date.now();
    if (pushedRequests.size >= this.capacity()) {
      this.sweep(now);
    }
    if (pushedRequests.size >= this.capacity()) {
      ...
      log.debug("Leaving PushedRequests.push(). The store is full.");
      return this.refusal('STS-OAUTH-0408', 'temporarily_unavailable',
        'this authorization server is holding as many pushed authorization ' +
        'requests as it is configured to (oauth2.parMaxRequests), all of ' +
        'them still live. Retry shortly; a live request_uri is never ' +
    ...
    log.info('par: client "' + clientId + '" pushed an authorization ' +
             'request (' + options.source + ', ' +
             (options.clientAuthenticated
               ? 'authenticated by ' + options.method :
               'not authenticated') +
             '), valid ' + seconds + 's.');
    log.debug("Leaving PushedRequests.push().");
    return { ok: true, requestUri: requestUri, expiresIn: seconds };
  }

  // -------------------------------------------------------------------------
  // resolve(requestUri, clientId, opts) — what `request_object.ts` calls for
  // a request_uri in this namespace at the authorization endpoint.
  //
  ...
  // error is `invalid_request_uri` (RFC 9101 section 7, which section 4 of
  // RFC 9126 builds on). Every refusal here is answered by `oauth2.ts` as a
  // 400 ON THIS SERVER, never redirected: the redirect_uri is inside the
  // request this could not find.
  //
  ...
  resolve(requestUri: unknown, clientId: unknown, opts?: Json): Json {
    const { log, monitor } = this.deps;
    const self = this;
    log.debug("Entering PushedRequests.resolve().");
    const options = opts || {};
    ...
    const refused = function (code, description) {
      ...
      return self.refusal(code, 'invalid_request_uri', description);
    };
    if (!this.isPushedRequestUri(uri)) {
      log.debug("Leaving PushedRequests.resolve(). Not in this namespace.");
      return refused('STS-OAUTH-0410', 'the request_uri "' + uri + '" is ' +
        'not one this authorization server issues: a pushed request_uri ' +
        'begins "' + REQUEST_URI_PREFIX + '" (RFC 9126 section 2.2).');
    }
    const now = Date.now();
    const record = pushedRequests.get(uri);
    if (!record) {
      log.debug("Leaving PushedRequests.resolve(). Unknown.");
      return refused('STS-OAUTH-0410', 'the request_uri is not one this ' +
        'authorization server issued, or it expired and was swept. A ' +
        'request_uri is valid for oauth2.parRequestUriLifetimeS seconds ' +
        'from the push (RFC 9126 section 2.2); push the request again.');
    }
    if (record.expiresAt <= now) {
      log.debug("Leaving PushedRequests.resolve(). Expired.");
      return refused('STS-OAUTH-0411', 'the request_uri expired ' +
        new Date(record.expiresAt).toISOString() + ' — "an expired ' +
        'request_uri MUST be rejected as invalid" (RFC 9126 section 4). ' +
        'Push the request again.');
    }
    if (String(record.clientId) !== client) {
      // Before "spent", so a client that is not the owner learns nothing
      // about the state of somebody else's reference.
      log.debug("Leaving PushedRequests.resolve(). Another client's.");
      return refused('STS-OAUTH-0413', 'the request_uri was pushed by a ' +
        'different client than client_id "' + client + '" — a request_uri ' +
        'is bound to the client that pushed it (RFC 9126 section 2.2).');
    }
    if (String(record.authorizationServer) !== server) {
      log.debug("Leaving PushedRequests.resolve(). Another authorization " +
                "server's.");
    ...
    if (record.spentAt) {
      log.debug("Leaving PushedRequests.resolve(). Already spent.");
      return refused('STS-OAUTH-0412', 'the request_uri was already used: ' +
        'an authorization response was issued on it at ' +
        new Date(record.spentAt).toISOString() + '. A client "MUST only ' +
        'use a request_uri value once" (RFC 9126 section 4); push the ' +
        'request again.');
    }
    record.reads = (record.reads || 0) + 1;
    pushedRequests.set(uri, record);
    monitor.record(client, 'par.resolved');
    log.debug("Leaving PushedRequests.resolve(). Read " + record.reads +
              " time(s).");
    return {
      ok: true,
  ...
  spend(requestUri: unknown): boolean {
    ...
    if (!record || record.spentAt) {
      log.debug("Leaving PushedRequests.spend(). Nothing to spend.");
      return false;
    }
    record.spentAt = Date.now();
    pushedRequests.set(uri, record);
    monitor.record(record.clientId, 'par.spent');
    log.debug("Leaving PushedRequests.spend().");
    return true;
  }
  ...
  list(opts?: Json): Json {
    const { log } = this.deps;
    const self = this;
    log.debug("Entering PushedRequests.list().");
    const options = opts || {};
    const now = Date.now();
    this.sweep(now);
    ...
    pushedRequests.forEach(function (record) {
      const shown = self.describe(record, now);
    ...
    const limit = isFinite(limitRaw) && limitRaw > 0 ?
      Math.min(limitRaw, 500) : 50;
    log.debug("Leaving PushedRequests.list(). " + rows.length + " row(s).");
    return {
      total: rows.length,
      offset: offset,
      limit: limit,
      capacity: this.capacity(),
      lifetime_s: this.lifetimeS(),
  ...
  get(requestUri: unknown): Json | null {
    ...
    log.debug("Leaving PushedRequests.get(). " +
  ...
  remove(requestUri: unknown): boolean {
    ...
    const record = pushedRequests.get(uri);
    if (!record) {
      log.debug("Leaving PushedRequests.remove(). Nothing to remove.");
      return false;
    }
    pushedRequests.delete(uri);
    monitor.record(record.clientId, 'par.deleted');
    log.debug("Leaving PushedRequests.remove().");
    return true;
  }
}
