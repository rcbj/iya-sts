...
// shape: `AuthorizationServers` takes the settings and the logger through its
// constructor. The catalogue (`MEMBERS`, `GROUPS`) and the profile store stay
...
interface AuthorizationServersDeps {
...
// Required for the order it was always required in; nothing here reads it.
void mode;
...
const MEMBERS: MemberRow[] = [
  ...
  // authorization server a request selected — see `request_object.ts`. A
...
const MEMBER_BY_NAME: Record<string, MemberRow> = {};
MEMBERS.forEach(function (row) {
  MEMBER_BY_NAME[row.name] = row;
});

const GROUPS: string[] = MEMBERS.reduce(function (out, row) {
  if (out.indexOf(row.group) < 0) {
    out.push(row.group);
  }
  return out;
}, []);
...
// default and the export below is a getter over the setting. See `ensure()`.
// ---------------------------------------------------------------------------
...
class AuthorizationServers {
  ...
  documentOf(member: string): string {
    ...
    const row = MEMBERS.filter(function (one) {
      return one.name === member;
    })[0];
    log.debug("Leaving AuthorizationServers.documentOf().");
    return row && row.document ? row.document : 'oauth';
  }

  private idProblem(id: unknown): string | null {
    const { log } = this.deps;
    log.debug("Entering AuthorizationServers.idProblem().");
    const text = String(id || '').trim();
    if (!text) {
      log.debug("Leaving AuthorizationServers.idProblem().");
    ...
    if (!ID_SHAPE.test(text)) {
      log.debug("Leaving AuthorizationServers.idProblem().");
      return '"' + text + '" cannot be a profile id. It has to be a single ' +
             'URL path segment: 1 to 64 characters of letters, digits, dot, ' +
             'dash, underscore or tilde, starting with a letter or a digit. ' +
             'A profile whose id had to be escaped to appear in a URL would ' +
             'be one nobody could find again.';
    }
    log.debug("Leaving AuthorizationServers.idProblem().");
    return null;
  }

  private blank(id: unknown): Json {
    const { log } = this.deps;
    log.debug("Entering AuthorizationServers.blank().");
    log.debug("Leaving AuthorizationServers.blank().");
    return { id: String(id), label: '', description: '',
             overrides: {}, removed: [], at: 0,
             // When this authorization server was first ASKED FOR, and how
             // many times. An auto-created one has been asked for once by
             // definition; the count is what /admin/sts-metadata lists the
             // accessed ones by.
  ...
  maxProfiles(): number {
    ...
    log.debug("Leaving AuthorizationServers.maxProfiles().");
    return isFinite(count) && count > 0 ? Math.floor(count) : MAX_PROFILES;
  }

  // -------------------------------------------------------------------------
  // EVERY NAME IS AN AUTHORIZATION SERVER, whether or not anybody configured
  // one.
  //
  // A path nobody has configured used to publish the ordinary document and
  // behave as the default one; now it is CREATED, with the defaults, on first
  // sight. The difference is that it can then be configured, appear on a page
  // and be told apart from its neighbours — an authorization server somebody
  // is using and cannot see is worse than one that exists with nothing special
  // about it.
  //
  ...
  //
  // Bounded, because the id comes off a URL path and any caller can invent
  // one. Past the cap a name is still SERVED — with the defaults, which is what
  ...
  // -------------------------------------------------------------------------
    ...
    log.debug("Entering AuthorizationServers.ensure(). id=" + key);
    if (!key || this.idProblem(key)) {
      log.debug("Leaving AuthorizationServers.ensure(). Not a usable id.");
      log.debug("Leaving AuthorizationServers.ensure().");
      return null;
    }
    let profile = profiles.get(key);
    if (!profile) {
      if (profiles.size >= this.maxProfiles()) {
        // Warned rather than thrown: the request that named it is answered
        // with the defaults, which is what an unconfigured name has always
        // produced.
        log.warn('authorization_servers: not recording "' + key + '"; ' +
                 ...
                 'defaults.');
        log.debug("Leaving AuthorizationServers.ensure(). The registry is " +
                  "full.");
        log.debug("Leaving AuthorizationServers.ensure().");
        return null;
      }
      profile = this.blank(key);
      profile.autoCreated = !!opts.autoCreated;
      profiles.set(key, profile);
      log.info('authorization_servers: "' + key + '" ' +
               (opts.autoCreated ? 'was asked for and did not exist, so it ' +
                                   'now does'
    ...
    log.debug("Leaving AuthorizationServers.ensure(). seen=" + profile.seen);
    log.debug("Leaving AuthorizationServers.ensure().");
    return profile;
  }

  // The stored profile itself. NOT the exported `get`, which is `view()`.
  private get(id: unknown): Json {
    const { log } = this.deps;
    log.debug("Entering AuthorizationServers.get().");
    log.debug("Leaving AuthorizationServers.get().");
  ...
  has(id: unknown): boolean {
    ...
    log.debug("Leaving AuthorizationServers.has().");
  ...
  count(): number {
    ...
    log.debug("Leaving AuthorizationServers.count().");
  ...
  //
  // Three operations and the order matters: REMOVALS happen after overrides,
  // so a member can be both set and removed and the removal wins — which is
  // what a reader of the form expects, and the alternative (a removal a later
  // override silently resurrects) is a control that does nothing.
  //
  // `null` is not special here. A member overridden to null is PUBLISHED as
  // null, because JSON has a null and a client that mishandles one is a client
  ...
  apply(metadata: Json, id?: string): Json {
    const { log } = this.deps;
    const self = this;
    log.debug("Entering AuthorizationServers.apply(). profile=" +
              (id || DEFAULT_ID));
    const profile = self.get(id || DEFAULT_ID);
    if (!profile) {
      // Not an error: an unconfigured path component publishes the document
      // this service always published, with the issuer built from the path.
      // That is what every existing caller sees and it must stay true.
      log.debug("Leaving AuthorizationServers.apply(). No profile by that " +
                "name; the document is unchanged.");
      return metadata;
    }
    Object.keys(profile.overrides).forEach(function (member) {
      if (self.documentOf(member) === 'oauth') {
    ...
    log.debug("Leaving AuthorizationServers.apply(). " +
              Object.keys(profile.overrides).length + " override(s), " +
  ...
  // endpoints AND by the document, which is the whole of how the two are kept
  // in step.
  //
  // `defaults` is the document this service builds for itself; the profile's
  // overrides and removals are applied on top; the result is what gets
  // published AND what the endpoints enforce. There is no second table of
  // "what tenant1 does" that could disagree with what tenant1 advertises,
  ...
  // enforceable member means the check does not run. That is the honest
  // reading: a client cannot learn from an absent
  // `code_challenge_methods_supported` that PKCE is unavailable, so a server
  ...
  capabilitiesOf(id: unknown, defaults?: Json, document?: string): Json {
    const { log } = this.deps;
    const self = this;
    log.debug("Entering AuthorizationServers.capabilitiesOf().");
    const wanted = document || 'oauth';
    const merged = Object.assign({}, defaults || {});
    const profile = self.get(id);
    if (!profile) {
      log.debug("Leaving AuthorizationServers.capabilitiesOf().");
      return merged;
    }
    Object.keys(profile.overrides).forEach(function (member) {
      if (self.documentOf(member) === wanted) {
  ...
  capabilityList(id: unknown, defaults: Json, member: string): string[] {
    ...
    const merged = this.capabilitiesOf(id, defaults, this.documentOf(member));
    const value = merged[member];
    if (value === undefined) {
      log.debug("Leaving AuthorizationServers.capabilityList().");
      return null;
    }
    log.debug("Leaving AuthorizationServers.capabilityList().");
  ...
  // misleading document is useful and one that did it quietly is a trap. It
  // is computed by comparing the profile's overrides against the document this
  // service would have published — so it needs no list of its own and cannot
  // go stale as the real document grows members.
  //
  ...
  driftOf(id: unknown, truth: Json): Json[] {
    const { log } = this.deps;
    const self = this;
    log.debug("Entering AuthorizationServers.driftOf(). profile=" + id);
    const profile = self.get(id);
    if (!profile) {
      log.debug("Leaving AuthorizationServers.driftOf(). No such profile.");
      return [];
    }
    const rows = [];
    Object.keys(profile.overrides).forEach(function (member) {
      ...
      // AN ENFORCEABLE MEMBER IS NOT DRIFT. It used to be — the document said
      // one thing and every endpoint did another — and now the endpoints read
      // this value, so a difference from the DEFAULT document is this
      // authorization server being configured rather than lying. What is left
      // as drift is the rest: a member this service cannot honour however it
      // is set.
      ...
      rows.push({
        ...
        what: actual === undefined
          ? 'Not a member this service builds, so nothing here backs it — ' +
            'which is allowed, and is how a document a client did not ' +
            'expect gets published.'
          : 'This service cannot make this true: it does not read this ' +
            'member, and would otherwise ' +
    ...
    profile.removed.forEach(function (member) {
      ...
      rows.push({
        ...
        what: spec && spec.enforces
          ? 'Removed, so this server says nothing about it — and the check ' +
            'it drives (' +
    ...
    log.debug("Leaving AuthorizationServers.driftOf(). " + rows.length +
              " member(s) this service cannot honour.");
    return rows;
  }
  ...
  create(detail: Json): Json {
    ...
    log.debug("Entering AuthorizationServers.create(). id=" + id);
    const problem = this.idProblem(id);
    if (problem) {
      log.debug("Leaving AuthorizationServers.create(). " + problem);
      log.debug("Leaving AuthorizationServers.create().");
      return { ok: false, errors: [problem] };
    }
    if (this.has(id)) {
      log.debug("Leaving AuthorizationServers.create(). It exists already.");
      log.debug("Leaving AuthorizationServers.create().");
      return { ok: false, errors: ['There is already an authorization ' +
                                   'server profile called "' +
                                   id + '". Change it rather than creating ' +
                                   'it again.'] };
    }
    ...
    log.debug("Leaving AuthorizationServers.create(). Created.");
    log.debug("Leaving AuthorizationServers.create().");
  ...
  setMember(id: unknown, member: unknown, rawValue: unknown): Json {
    ...
    if (!profile) {
      log.debug("Leaving AuthorizationServers.setMember().");
      return { ok: false, errors: ['There is no authorization server ' +
                                   'profile called "' + id +
                                   '".'] };
    }
    ...
    if (!name) {
      log.debug("Leaving AuthorizationServers.setMember().");
      return { ok: false, errors: ['Which member? Any RFC 8414 member name ' +
                                   'is accepted, and so is one this service ' +
                                   'has never heard of — publishing ' +
                                   'something a client did not expect is ' +
                                   'half the point.'] };
    }
    // JSON FIRST, then the raw string. A form field carrying `["S256"]` means
    // a list and one carrying `S256` means a string, and guessing from the
    // member's catalogue `kind` instead would be wrong for every member not in
    // the catalogue — which is the case this file exists to allow.
    let value = rawValue;
    if (typeof rawValue === 'string') {
      ...
      try {
        ...
        log.debug("Caught in AuthorizationServers.setMember(): " +
                  ((e && e.message) || e));
    ...
    // one that had been removed would leave it removed and the form would
    // appear to have done nothing.
    profile.removed = profile.removed.filter(function (one) {
      return one !== name;
    });
    profile.at = Date.now();
    ...
    log.info('authorization_servers: profile "' + id + '" now publishes ' +
             name + '=' +
             JSON.stringify(value) + '.');
    log.debug("Leaving AuthorizationServers.setMember(). Set.");
    return { ok: true, profile: this.view(id),
             message: name + ' is now ' + JSON.stringify(value) + ' in the "' +
                      id +
                      '" document.' + (MEMBER_BY_NAME[name] ? '' :
                      ' That is not a member this service recognises, which ' +
  ...
  removeMember(id: unknown, member: unknown): Json {
    ...
    const profile = this.get(id);
    if (!profile) {
      log.debug("Leaving AuthorizationServers.removeMember().");
      return { ok: false, errors: ['There is no authorization server ' +
                                   'profile called "' + id +
                                   '".'] };
    }
    ...
    // `admin_stats.js`'s CLAIM_SETS, and the same one-line answer.
    profiles.set(String(id || ''), profile);
    log.info('authorization_servers: profile "' + id + '" no longer ' +
             'publishes ' + name + '.');
    log.debug("Leaving AuthorizationServers.removeMember(). Removed.");
    return { ok: true, profile: this.view(id),
             message: name + ' is gone from the "' + id + '" document. A ' +
                      'client reading it cannot tell that this server ' +
                      'supports it — which is not the same as learning that ' +
                      'it does not.' };
  }
  ...
  resetMember(id: unknown, member: unknown): Json {
    ...
    const profile = this.get(id);
    if (!profile) {
      log.debug("Leaving AuthorizationServers.resetMember().");
      return { ok: false, errors: ['There is no authorization server ' +
                                   'profile called "' + id +
                                   '".'] };
    }
    const name = String(member || '').trim();
    const had = Object.prototype.hasOwnProperty.call(profile.overrides,
                                                     name) ||
    ...
    profiles.set(String(id || ''), profile);
    log.debug("Leaving AuthorizationServers.resetMember(). " +
              (had ? "Reset." : "It was not overridden."));
    return { ok: true, profile: this.view(id),
             message: had
  ...
  remove(id: unknown): Json {
    ...
    if (!this.has(id)) {
      log.debug("Leaving AuthorizationServers.remove().");
      return { ok: false, errors: ['There is no authorization server ' +
                                   'profile called "' + id +
                                   '".'] };
    }
    ...
    log.info('authorization_servers: profile "' + id + '" deleted. ' +
        profiles.size + ' ' +
        'left.');
    log.debug("Leaving AuthorizationServers.remove(). Deleted.");
    return { ok: true,
             message: '"' + id + '" is gone. The two discovery URLs that ' +
                      ...
                      'because an unconfigured path component has always ' +
  ...
  view(id: unknown): Json {
    ...
    const profile = this.get(id);
    if (!profile) {
      log.debug("Leaving AuthorizationServers.view().");
      return null;
    }
    log.debug("Leaving AuthorizationServers.view().");
    return {
      ...
      memberCount: Object.keys(profile.overrides).length +
        profile.removed.length,
      // The two URLs this profile is served at, spelt out rather than left to
      // be assembled: the whole reason both shapes exist is that people get
      // them wrong, and a page that made somebody derive them would be
      // repeating the problem it documents.
      ...
      urls: {
        ...
        // advertises, so a client that read that document is already using
        // them.
  ...
  list(): Json[] {
    const { log } = this.deps;
    const self = this;
    log.debug("Entering AuthorizationServers.list().");
    const rows = [];
    profiles.forEach(function (profile) {
      rows.push(self.view(profile.id));
    });
    rows.sort(function (a, b) {
      return a.id.localeCompare(b.id);
    });
    log.debug("Leaving AuthorizationServers.list().");
    return rows;
  }
}
