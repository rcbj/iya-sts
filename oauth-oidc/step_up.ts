...
//
// **THIS FILE DECIDES; `oauth2.ts` AND `dpop.ts` ANSWER.** A library (rule 3):
// it requires `common/` modules and `oauth2_monitor.ts`, none of which requires
// it back, so the authorization endpoint (`oauth2.ts`) and the resource-server
// check every protected endpoint shares (`dpop.ts`) can both reach it. What an
...
// A JSON-shaped value: a query, a session, claims, a refusal.
type Json = any;

// What a request or a resource requires.
...
// What `parseAcrValues()` answers.
interface ParsedAcrValues {
  values: string[];
  invalid: string[];
}

interface StepUpDeps {
...
// The round-trip marker's name. `oauth2.ts` declares it in the authorization
...
class StepUp {
  ...
  parseAcrValues(text: unknown): ParsedAcrValues {
    ...
    const out: ParsedAcrValues = { values: [], invalid: [] };
    ...
    log.debug("Leaving StepUp.parseAcrValues(). " + out.values.length +
              " value(s).");
    return out;
  }
  ...
  parseMaxAge(value: unknown): number | null {
    ...
    if (value === undefined || value === null ||
        String(value).trim() === '') {
      log.debug("Leaving StepUp.parseMaxAge(). Absent.");
      return null;
    }
    const text = String(value).trim();
    if (!/^\d{1,9}$/.test(text) || Number(text) > MAX_AGE_LIMIT) {
      log.debug("Leaving StepUp.parseMaxAge(). Not a whole number of " +
                "seconds.");
      return null;
    }
    log.debug("Leaving StepUp.parseMaxAge().");
    return Number(text);
  }
  ...
  ownResourceRequirement(): Requirement {
    ...
    const acr = this.parseAcrValues(config.value('oauth2.stepUpAcrValues'));
    if (acr.invalid.length) {
      // A string setting has no check of its own in `config.js`, so a value
      // that could not sit in a challenge header is reported here, where it
      // is dropped, rather than enforced as a requirement no token could
      // meet.
    ...
    log.debug("Leaving StepUp.ownResourceRequirement().");
    return {
      acrValues: acr.values,
      maxAge: maxAge,
      present: acr.values.length > 0 || maxAge !== null
  ...
  private levelOf(acr: unknown): number {
    const { log } = this.deps;
    log.debug("Entering StepUp.levelOf().");
    log.debug("Leaving StepUp.levelOf().");
    return LEVELS.indexOf(String(acr === undefined || acr === null ? '' :
                                 acr));
  }

  private amrOf(facts: Json): string[] {
    const { log } = this.deps;
    log.debug("Entering StepUp.amrOf().");
    const amr = facts && facts.amr;
    log.debug("Leaving StepUp.amrOf().");
  ...
  meets(requested: string, facts: Json): boolean {
    ...
    if (had && had === requested) {
      log.debug("Leaving StepUp.meets(). The same value.");
      return true;
    }
    ...
    if (KEY_ALIASES.indexOf(requested) >= 0) {
      ...
      log.debug("Leaving StepUp.meets(). A key alias; met=" + met);
      return met;
    }
    const wanted = this.levelOf(requested);
    if (wanted < 0) {
      log.debug("Leaving StepUp.meets(). A value this sign-in does not " +
                "produce.");
      return false;
    }
  ...
  satisfiedAcr(acrValues: string[] | null | undefined,
    ...
    const { log } = this.deps;
    log.debug("Entering StepUp.satisfiedAcr().");
    const wanted = acrValues || [];
    if (!wanted.length) {
      log.debug("Leaving StepUp.satisfiedAcr(). Nothing was requested.");
      return (facts && facts.acr) || null;
    }
    for (let i = 0; i < wanted.length; i++) {
      if (this.meets(wanted[i], facts)) {
        log.debug("Leaving StepUp.satisfiedAcr(). " + wanted[i] + ".");
        return wanted[i];
      }
    }
    log.debug("Leaving StepUp.satisfiedAcr(). None met.");
    return null;
  }
  ...
  demandsSecondFactor(acrValues: string[] | null | undefined): boolean {
    const { log } = this.deps;
    const self = this;
    log.debug("Entering StepUp.demandsSecondFactor().");
    const producible = (acrValues || []).filter(function (one) {
      return self.levelOf(one) >= 0 || KEY_ALIASES.indexOf(one) >= 0;
    });
    const demands = producible.length > 0 &&
      producible.every(function (one) {
        return KEY_ALIASES.indexOf(one) >= 0 ||
          self.levelOf(one) >= self.levelOf('mfa');
      });
    log.debug("Leaving StepUp.demandsSecondFactor(). " + demands);
    return demands;
  }
  ...
  private nowSec(): number {
    ...
    log.debug("Leaving StepUp.nowSec().");
    return Math.floor(Date.now() / 1000);
  }

  // -------------------------------------------------------------------------
  // THE AUTHORIZATION ENDPOINT'S QUESTION: does the session in hand answer
  // this request, and if not, may the person be sent to sign in again?
  //
  ...
  assessSession(requirement: Requirement | null | undefined, session: Json,
    ...
    log.debug("Leaving StepUp.assessSession(). Not met (" + reason +
              "), honoured=" + !!opts.honoured);
  ...
  unmetRefusal(requirement: Requirement | null | undefined, assessed: Json,
    ...
    log.debug("Entering StepUp.unmetRefusal().");
    const need = requirement || { acrValues: [], maxAge: null };
    const said = assessed || {};
    let out = null;
    if (promptNone) {
      out = errorCodes.mark({
        ...
        description: (said.reason === 'max_age'
          ? 'The session authenticated ' + said.elapsed + ' seconds ago ' +
            'and max_age is ' + need.maxAge
      ...
      out = errorCodes.mark({
        ...
        description: 'The person was asked to sign in again because ' +
          'max_age is ' + need.maxAge + ', and the session that came back ' +
  ...
  // its code under the Symbol, which `challengeHeader()` turns into section
  // 3's header.
  //
  // **A TOKEN WITH NO `auth_time` DOES NOT MEET A `max_age`**, and one with
  // no `acr` does not meet `acr_values`: RFC 9068 section 2.2.1 has them
  // absent where no authentication is behind the grant, and "not known" is
  // not "recent enough". A client_credentials token therefore cannot reach a
  // resource that requires either, which is the resource's statement to make.
  //
  // **NO CLOCK SKEW IS ALLOWED ON THE AGE.** `oauth2.clockSkewS` is an
  // allowance between two clocks on `exp` and `nbf`; `auth_time` was written
  ...
  tokenRefusal(requirement: Requirement | null | undefined, claims: Json,
    ...
    const { log, errorCodes } = this.deps;
    log.debug("Entering StepUp.tokenRefusal().");
    const need = requirement || { acrValues: [], maxAge: null };
    if (!need.present) {
      log.debug("Leaving StepUp.tokenRefusal(). No requirement.");
      return null;
    }
    const c = claims || {};
    const now = (options && options.now) || this.nowSec();
    const authTime = Number(c.auth_time) || 0;
    if (need.maxAge !== null &&
        (!authTime || now - authTime > need.maxAge)) {
      log.debug("Leaving StepUp.tokenRefusal(). Too old, or no auth_time.");
      return errorCodes.mark({
        error: 'insufficient_user_authentication',
        reason: 'max_age',
        description: authTime
          ? 'More recent authentication is required: this resource ' +
            'accepts an authentication at most ' + need.maxAge +
            ' seconds old, and this token\'s was ' + (now - authTime) +
            ' seconds ago.'
          : 'More recent authentication is required, and this token ' +
            'carries no auth_time — no authentication event is behind it.'
      }, 'STS-OAUTH-0504');
    }
    if (need.acrValues.length &&
        this.satisfiedAcr(need.acrValues,
                          { acr: c.acr, amr: c.amr }) === null) {
      log.debug("Leaving StepUp.tokenRefusal(). The acr does not meet it.");
      return errorCodes.mark({
        error: 'insufficient_user_authentication',
        reason: 'acr',
    ...
    log.debug("Leaving StepUp.tokenRefusal(). Met.");
    return null;
  }

  // Section 3's challenge. The two auth-params are sent together whenever the
  // resource requires both — section 3 allows it, and a client that stepped
  ...
  challengeHeader(scheme: string, requirement: Requirement | null | undefined,
    ...
    const { log } = this.deps;
    log.debug("Entering StepUp.challengeHeader().");
    const need = requirement || { acrValues: [], maxAge: null };
    ...
    log.debug("Leaving StepUp.challengeHeader().");
  ...
  record(clientId: string, event: string, detail?: Json): void {
    ...
    log.debug("Leaving StepUp.record().");
  }
}
