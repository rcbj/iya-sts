...
// Possession. This module is the checker; oauth2.ts binds the tokens it issues
...
// shape. `Dpop` takes every module it reads through its constructor
...
// HERE rather than at the protected endpoints because presentedAccessToken()
// below is the single check they share.
...
// modules and `oauth2_monitor.ts`, neither of which requires this file.
...
// A JSON-shaped value: a proof's header and claims, a refusal, a result.
type Json = any;
...
interface DpopDeps {
  ...
  // The one decision about whether a forwarded header is believable, shared
  // with baseUrlOf() so that two functions in this service cannot answer it
  // two ways.
  forwardedFrom: typeof helpers.forwardedFrom;
  trustProxy: typeof helpers.trustProxy;
...
// list is deliberately explicit rather than "whatever the JWT library
// accepts": an allow-list is the only thing that stops an `alg` the client
// chose from selecting a verification path the server did not intend.
...
// secp256k1 could register it, sign an ID Token request with it, and then
// find its DPoP proof refused with no explanation that named the real reason.
//
...
// TRUST REALM. `realms.map()` is a Map that holds a separate one for each
...
// ARRIVAL.** `proofClaims()` is a middleware `oauth2.ts` registers above every
...
// already sits (so every check before it keeps its own refusal), and KEEPS
// the reservation when it accepts. A reservation that was not kept — the
// proof was refused for something else, or never verified at all — is given
// back when the response finishes, which is `seenJtis`' rule exactly: only an
// ACCEPTED proof is remembered.
//
...
// carries a new nonce is a writing response the barrier holds until its
// commit lands, and the retry that carries it back — to any node — passes the
// barrier's catch-up first. So a nonce issued on A is current on B by the time
// B can be asked.
// ---------------------------------------------------------------------------
const PROOF_CLAIM = Symbol('sts.dpopProofClaim');

// Server-supplied nonces (sections 8 and 9). OFF by default: the mechanism is
// a second round trip on the first request of every session, so a deployment
// opts in. `nonceModeOn()` is read per request rather than captured at
// require time, so a test can turn it on and off without restarting the
// service. SHARED AND PERSISTED, NOT A MODULE VARIABLE (2026-09-08). It was
...
class Dpop {
  ...
  private iatSkewSeconds(): number {
    ...
    log.debug("Entering Dpop.iatSkewSeconds().");
    const seconds = Number(config.value('oauth2.dpopIatSkewS'));
    log.debug("Leaving Dpop.iatSkewSeconds().");
    return isFinite(seconds) && seconds > 0 ? Math.floor(seconds) :
           IAT_SKEW_SECONDS;
  }

  // The `jti` of a compact JWS, read without verifying it, or '' where there
  // is none to read. Anything malformed is left to `verifyProof()` to refuse.
  private unverifiedJtiOf(rawHeader: unknown): string {
    const { log, jsonFromB64u } = this.deps;
    log.debug("Entering Dpop.unverifiedJtiOf().");
    const raw = String(rawHeader || '').trim();
    const parts = raw.split('.');
    if (parts.length !== 3 || raw.indexOf(',') >= 0) {
      log.debug("Leaving Dpop.unverifiedJtiOf(). Not a compact JWS.");
      return '';
    }
    ...
    try {
      ...
      log.debug("Caught in Dpop.unverifiedJtiOf(): " +
                ((e && e.message) || e));
      payload = null;
    }
    const jti = payload && typeof payload.jti === 'string' ? payload.jti :
      '';
    log.debug("Leaving Dpop.unverifiedJtiOf().");
    return jti;
  }
  ...
  proofClaims(): (req: Req, res: Res, next: () => void) => void {
    ...
    return function dpopProofClaim(req, res, next) {
      ...
      clusterClaims.claim({
        ...
        ttlMs: self.iatSkewSeconds() * 2 * 1000
      }).then(function (answer: Json) {
  ...
  private nonceTtlSeconds(): number {
    const { log, config } = this.deps;
    log.debug("Entering Dpop.nonceTtlSeconds().");
    const seconds = Number(config.value('oauth2.dpopNonceTtlS'));
    log.debug("Leaving Dpop.nonceTtlSeconds().");
    return isFinite(seconds) && seconds > 0 ? Math.floor(seconds) :
           NONCE_TTL_SECONDS;
  }

  // WRITES THE SETTING, IN THE REALM IT IS CALLED IN. Returns what is now in
  // force, or throws with the setting's own refusal — which cannot happen for
  // a boolean on a runtime row, and is surfaced rather than swallowed because
  // a switch that silently did not switch is the invisible failure the
  // endpoint's own comment warns about.
  //
  // **TURNING IT BACK TO WHAT IT WOULD BE ANYWAY CLEARS THE OVERRIDE rather
  // than writing one**, which is tests/CLAUDE.md's "restore with reset" rule
  // applied inside the control: a test that turns nonces on and then off
  // would otherwise leave the row reading `source: override` with the
  ...
  setNonceMode(on: unknown): boolean {
    ...
    if (config.value('oauth2.dpopNonceRequired') !== wanted) {
      const written: Json = config.setOverride('oauth2.dpopNonceRequired',
                                               wanted);
      if (written && written.ok === false) {
        log.debug('Leaving Dpop.setNonceMode(). The setting refused the ' +
                  'value.');
        throw new Error((written.errors || []).join(' '));
      }
    }
    log.debug('Leaving Dpop.setNonceMode(). DPoP nonces are ' +
              (this.nonceModeOn() ? 'REQUIRED' : 'not required'));
  ...
  nonceModeOn(): boolean {
    ...
    log.debug("Leaving Dpop.nonceModeOn().");
  ...
  private pruneNonces(): void {
    ...
    log.debug("Entering Dpop.pruneNonces().");
    const cutoff = nowSec() - this.nonceTtlSeconds();
  ...
  // `key_ops`/`ext` hashes to the same value as the same key without them.
  // That is not tidiness: the wallet sends its key in every proof header, and
  // if a stray member changed the digest the token would stop matching its
  // own key.
  // -------------------------------------------------------------------------
  // -------------------------------------------------------------------------
  // THE RFC 7638 MEMBER TABLE AND THE CANONICAL JSON MOVED TO
  // `common/crypto.js` ON 2026-08-27, and the paragraph above is why they had
  // to move rather than be tidied: there were THREE copies of this computation
  // in the service, and the other two derived a `kid` with JSON.stringify over
  // an object literal whose keys happened to be in lexicographic order. All
  // three agreed. RFC 7638 exists so that two implementations agree, and three
  // that agree by coincidence are two bugs waiting for somebody to add a
  // member out of order.
  //
  ...
  // JWKS and a short one is readable in a log. This one is compared BYTE FOR
  ...
  thumbprint(jwk: Json): string {
    ...
    log.debug('Leaving Dpop.thumbprint(). jkt=' + jkt);
    return jkt;
  }
  ...
  // -------------------------------------------------------------------------
  // `htu` — the request's own target URI, without query or fragment,
  // normalized the way section 4.3 asks (RFC 3986 syntax- and scheme-based
  // normalization).
  //
  // Behind a proxy this has to be the URI the CLIENT used, not the one the
  // socket saw: with the api or a CORS proxy in front of this service,
  // `req.protocol` and `req.get('host')` describe the last hop and every proof
  // would be refused for naming the real endpoint.
  //
  // **THE FORWARDED HEADERS ARE HONOURED ONLY WHERE A PROXY IS TRUSTED**, and
  // that is a change — this function used to believe them unconditionally,
  // while `baseUrlOf()` in helpers.js ignored them, so two functions in one
  // service disagreed about whether a forwarded header was believable. They
  // share `forwardedFrom()` now and one setting decides.
  //
  // It is the htu check that makes the setting matter rather than the
  // metadata. `htu` binds a proof to the endpoint it was made for, which is
  // what stops a proof captured at one endpoint being replayed at another —
  // and if a CLIENT can set the expected value with a header, it can name the
  // endpoint it stole the proof from and the binding stops meaning anything.
  // So with `global.trustProxy` off, what the socket saw is what a proof must
  ...
  htuOf(req: Req): string {
    ...
    const from: Json = forwardedFrom(req);
    ...
    log.debug('Leaving Dpop.htuOf(). htu=' + htu);
    return htu;
  }
  ...
  normalizeHtu(value: unknown): string {
    ...
    try {
      ...
      log.debug("Caught in Dpop.normalizeHtu(): " + ((e && e.message) || e));
      // Unparseable, so it cannot match anything. Returned as-is so the
      // caller's comparison fails and says what arrived, rather than
      // throwing.
      log.debug('Leaving Dpop.normalizeHtu(). Not a URL.');
      return String(value || '');
    }
    ...
    const out = scheme + '//' + parsed.hostname.toLowerCase() +
                (port ? ':' + port : '') +
                parsed.pathname;
    log.debug('Leaving Dpop.normalizeHtu(). out=' + out);
    return out;
  }
  ...
  // description, needNonce }. It never sends a response: the caller decides
  // the status code and the header shape, because an authorization server
  // says `use_dpop_nonce` in a 400 JSON body while a resource server says it
  ...
  verifyProof(rawHeader: unknown, opts?: Json): Json {
    ...
    log.debug('Entering Dpop.verifyProof(). htm=' + (opts && opts.htm) +
              ', htu=' + (opts && opts.htu));
    const options = opts || {};
    // `code` names the condition for the caller that sends the response; it
    // is never serialised into one.
    const fail = function (code, description, extra?) {
      log.debug("Entering fail().");
      log.debug('Leaving Dpop.verifyProof(). REFUSED: ' + description);
    ...
    // Check 1: not more than one DPoP header field. Express joins repeated
    // header fields with ", " — and a compact JWS contains no comma — so a
    // comma here means two headers arrived, and accepting either of them
    // would let an attacker append their own proof to a captured request.
    ...
    if (raw.indexOf(',') >= 0) {
      log.debug("Leaving Dpop.verifyProof().");
      return fail('STS-OAUTH-0094', 'More than one DPoP header field was ' +
                                    'sent; RFC 9449 permits exactly one.');
    }
    ...
    if (parts.length !== 3) {
      log.debug("Leaving Dpop.verifyProof().");
      return fail('STS-OAUTH-0095', 'The DPoP proof is not a compact JWS ' +
                                    'with three parts.');
    }
    let header: Json;
    let claims: Json;
    try {
      ...
      log.debug("Caught in Dpop.verifyProof(): " + ((e && e.message) || e));
      log.debug("Leaving Dpop.verifyProof().");
    ...
    if (!header || typeof header !== 'object' || !claims ||
      ...
      log.debug("Leaving Dpop.verifyProof().");
      return fail('STS-OAUTH-0097', 'The DPoP proof header or payload is ' +
                                    'not a JSON object.');
    }

    // Check 4: typ. Before the signature, because it costs nothing and it is
    // the check that stops a JWT signed for another purpose being accepted
    // here.
    ...
    // table is common/crypto.js's, so this list and what the verifier below
    ...
    // Check 7: the jwk header must carry a public key and no private key.
    // Checked before the signature, because importing a key object that
    // carries private material would let a client hand over a whole key pair
    // and be believed.
    ...
    if (privateMembers.length) {
      log.debug("Leaving Dpop.verifyProof().");
      return fail('STS-OAUTH-0101', 'The DPoP proof header carries private ' +
                                    'key material (' +
    ...
    const absent = required.filter(function (c) {
      return claims[c] === undefined || claims[c] === null ||
        claims[c] === '';
    });
    if (absent.length) {
      log.debug("Leaving Dpop.verifyProof().");
    ...
    // passed EXPLICITLY — RFC 8725 section 3.1, and doubly so here, because
    // the key being verified against is the one the token itself supplied: a
    // verifier that also took the algorithm from the token would be letting
    // the proof choose both halves.
    try {
      ...
      // signature, a wrong length, a key that will not load, or a signature
      // that simply does not verify. A malformed one makes node throw rather
      // than return false, and this is what keeps that from becoming a 500.
      log.debug("Caught in Dpop.verifyProof(): " + ((e && e.message) || e));
      log.debug('the DPoP proof did not verify: ' + e.message);
      log.debug("Leaving Dpop.verifyProof().");
      return fail('STS-OAUTH-0104', 'The DPoP proof signature does not ' +
                  'verify with the key in its own header: ' + e.message);
    }

    // Check 8: htm matches this request's method.
    if (String(claims.htm).toUpperCase() !==
        String(options.htm || '').toUpperCase()) {
      log.debug("Leaving Dpop.verifyProof().");
    ...
    const presented = this.normalizeHtu(claims.htu);
    const expected = this.normalizeHtu(options.htu);
    if (presented !== expected) {
      log.debug("Leaving Dpop.verifyProof().");
      // The commonest cause of this on a deployment that works everywhere
      // else is a reverse proxy: the client made the request to the proxy's
      // URL, the socket here saw the last hop's, and the two differ in
      // scheme, host or both. So the setting is named rather than left to be
      // discovered — a proof refused for a reason nobody can see is an
      // afternoon, and this refusal would otherwise read as the client's bug.
      return fail('STS-OAUTH-0106',
                  ...
                  (trustProxy()
                    ...
                    : ' If something is terminating TLS in front of this ' +
                      ...
                      'X-Forwarded-Proto and X-Forwarded-Host are ignored ' +
                      'and this server describes the LAST HOP rather than ' +
                      'the URL the client used. Turn it on where a proxy ' +
                      'really is in front — and leave it off where one is ' +
                      'not, because those are headers any client can set, ' +
                      'and a client that chooses its own htu has unbound ' +
                      'its own proof.'));
    }

    // Check 11: iat within an acceptable window.
    const age = nowSec() - Number(claims.iat);
    const window = this.iatSkewSeconds();
    ...
    // matters: a missing nonce is not a refusal but a REQUEST, answered with a
    // fresh nonce for the client to retry with, so it is reported separately
    // from a wrong one.
    if (this.nonceModeOn()) {
      ...
      if (!this.nonceIsCurrent(claims.nonce)) {
        log.debug("Leaving Dpop.verifyProof().");
        return fail('STS-OAUTH-0109', 'The DPoP proof nonce is not one ' +
                                      'this server issued, or it has ' +
                                      'expired.',
                    { needNonce: true });
      }
    }
    ...
    // request arrived. After the local check, so a replay this process can
    // see keeps its own code.
    ...
    if (reserved && !reserved.answer.ok) {
      ...
      return reserved.answer.reason === 'used'
        ...
        : fail('STS-OAUTH-0520',
               'Whether this DPoP proof (jti ' + claims.jti + ') has ' +
               'already been used could not be recorded, so it is ' +
               'refused. Send a fresh proof.');
    }

    // Check 12, first half: ath, when an access token came with the proof.
    const jkt = this.thumbprint(jwk);
    if (options.accessToken) {
      if (claims.ath === undefined) {
        log.debug("Leaving Dpop.verifyProof().");
        return fail('STS-OAUTH-0111', 'The DPoP proof must carry ath when ' +
                    'it accompanies an access token; without it a proof ' +
                    'captured with one token could be presented with ' +
                    'another.');
      }
      if (claims.ath !== this.athOf(options.accessToken)) {
        log.debug("Leaving Dpop.verifyProof().");
        return fail('STS-OAUTH-0112', 'The DPoP proof ath does not match ' +
                                      'the access token presented with it.');
      }
    }

    // Check 12, second half: the token's own binding. This is the comparison
    // that makes the token sender-constrained — without it the client simply
    // presents whichever key it likes and is believed.
    if (options.expectedJkt && options.expectedJkt !== jkt) {
      log.debug("Leaving Dpop.verifyProof().");
      return fail('STS-OAUTH-0113', 'The access token is bound to a ' +
                  'different key than the one that signed this DPoP proof ' +
    ...
    log.debug('Leaving Dpop.verifyProof(). Accepted. jkt=' + jkt);
    return { ok: true, jkt: jkt, jwk: jwk, claims: claims, header: header };
  }

  private pruneJtis(): void {
    const { log, nowSec } = this.deps;
    log.debug("Entering Dpop.pruneJtis().");
    const cutoff = nowSec() - (this.iatSkewSeconds() * 2);
    seenJtis.forEach(function (seen, jti) {
      if (seen < cutoff) {
        seenJtis.delete(jti);
      }
    });
    log.debug("Leaving Dpop.pruneJtis().");
  }
  ...
  state(): Json {
    ...
    log.debug("Leaving Dpop.state().");
    return {
      signing_alg_values_supported: SIGNING_ALGS,
      nonces_required: this.nonceModeOn(),
      iat_skew_seconds: this.iatSkewSeconds(),
      nonce_ttl_seconds: this.nonceTtlSeconds(),
  ...
  forgetProofs(): void {
    ...
    log.debug('Entering Dpop.forgetProofs(). ' + seenJtis.size +
              ' remembered proof(s) discarded.');
    seenJtis.clear();
    log.debug('Leaving Dpop.forgetProofs().');
  }
  ...
  // 9449 section 7).
  //
  // Three of this service's protected endpoints — Credential, Deferred
  // Credential and Notification — used to carry their own copy of a
  // Bearer-only check. They share this one, because a per-endpoint copy is
  // how one of three ends up not demanding the proof, and the endpoint that
  // forgot is the one an attacker would use.
  //
  // **It lives here rather than in vc_issuer.js, where it was written,
  // because there were four.** /oauth2/userinfo was the fourth, and it is in
  // oauth2.ts — a module vc_issuer.js cannot be required from without either
  // building a cycle or moving OID4VCI ahead of OAuth2 in the route order.
  // Copying the check into the OAuth2 module instead is precisely the mistake
  // the paragraph above records having already been made once. dpop.ts
  // registers no routes and joins no cycle, so it is the one place every
  // caller can reach — seven surfaces since, listed at the #34 check below.
  //
  // It answers the request itself on failure and returns null, so a caller
  // reads:
  //
  ...
  //   if (!presented) return;
  //
  // What it will and will not vouch for. OID4VCI lets the authorization
  // server be somebody else (this suite points the metadata at Keycloak), so
  // a token this issuer cannot verify is still accepted — that is stated at
  // the top of vc_issuer.js and has not changed. The consequence for DPoP is
  // worth being explicit about: `cnf.jkt` is read from a token whose
  // signature may be unverifiable here, so for a foreign token the binding is
  // checked between the proof and a claim anyone could have written. When the
  // token IS one of ours the signature is checked first and the binding is
  // real. A production resource server has no such excuse and must verify
  // the token before trusting its cnf.
  //
  // `verified` in the returned object is how a caller that CANNOT live with
  // that tells the difference: the userinfo endpoint refuses a token it did
  // not issue, because a profile is a statement about somebody this server
  // authenticated, and there is nothing it can honestly say about the subject
  // of a signature it cannot check.
  //
  // **RFC 9470 IS THE LAST QUESTION, ASKED OF A TOKEN THAT PASSED EVERY
  // OTHER** (2026-09-13). Section 3's challenge says "the token is fine and
  // the authentication behind it is not", which would be a lie about a token
  // that is not fine — so it is asked after the binding and the proof, and a
  // client is never sent to step up for a token it would then be refused for
  // anyway. The requirement is `oauth2.stepUpAcrValues` /
  // `oauth2.stepUpMaxAgeS` unless the caller names one.
  //
  ...
  presentedAccessToken(req: Req, res: Res, where?: string,
    ...
    const { log, errorCodes, vciError, stsCrypto, STS, jsonFromB64u,
            jwtAccessToken, baseUrlOf, mtls, senderConstraints,
            bcp } = this.deps;
    log.debug("Entering Dpop.presentedAccessToken(). where=" + where);
    const opts = options || {};
    // RFC 9700 section 4.3.2 — an access token MUST NOT travel in a URI query
    // parameter. RFC 6750 section 2.3 defines a form that does, and this
    // service has never read it: the token comes from the Authorization
    // header and nowhere else, so one in the query has always been simply
    // ignored.
    //
    // IGNORED IS NOT THE SAME AS REFUSED, and the difference is what this
    // adds. A client that sends `?access_token=...` gets a 401 saying a token
    // is required — which is true, unhelpful, and sends somebody looking at
    // their credential rather than at where they put it. In RFC 9700 mode the
    // query is looked at ONLY to say so, and the refusal names the reason: a
    // URL goes into browser history, into the address bar, into server logs
    // and into the Referer of anything the page then fetches, and a token in
    // one is a token in all of those.
    //
    ...
    if (inQuery && bcp.enabled()) {
      ...
      log.warn('RFC 9700 section 4.3.2: a request to ' +
               (where || 'a protected endpoint') +
               ' carried an access token in the QUERY STRING. Refused. That ' +
               'URL is now in this client\'s browser history and in ' +
               'whatever logged the request.');
      log.debug("Leaving Dpop.presentedAccessToken(). A token was in the " +
                "query string.");
      ...
      vciError(res, 400, 'invalid_request',
        ...
        'query parameter. RFC 6750 section 2.3 defines that form and its ' +
        'own specification does not recommend it, because a URL ends up in ' +
        'browser history, in the address bar, in server logs and in the ' +
        'Referer header of anything the page goes on to fetch — so the ' +
        'token is in all of those too. This endpoint reads the ' +
        'Authorization header only, and treat the token you just sent as ' +
        'disclosed.');
    ...
    if (!match) {
      ...
      res.set('WWW-Authenticate', 'DPoP algs="' + SIGNING_ALGS.join(' ') +
              '", Bearer');
      log.debug("Leaving Dpop.presentedAccessToken(). No access token.");
      errorCodes.mark(res, 'STS-OAUTH-0116');
      vciError(res, 401, 'invalid_token',
        'An access token is required, presented as "Bearer <token>" or, ' +
        'when it is DPoP-bound, as "DPoP <token>" with a DPoP proof.');
      log.debug("Leaving Dpop.presentedAccessToken().");
      return null;
    }
    ...
    try {
      ...
      log.debug("Caught in Dpop.presentedAccessToken(): " +
                ((e && e.message) || e));
      log.debug("This access token is not one of ours, so its claims are " +
                "read unverified: " +
                e.message);
      try {
        claims = jsonFromB64u(String(accessToken).split('.')[1]) || {};
      } catch (e2) {
        // Not a JWT at all. Opaque tokens are legal, and this issuer accepts
        // them as it always has — there is simply no binding to find in one.
        log.debug("Caught in Dpop.presentedAccessToken(): " +
                  ((e2 && e2.message) || e2));
        log.debug("...and it is not a JWT either, so there is no cnf to " +
                  "read: " + e2.message);
        claims = {};
      }
    }
    const boundTo = this.jktOf(claims);

    // A resource answering for a registered application accepts only what it
    // can verify: an unverified token's `aud`, `acr` and `auth_time` are
    // strings its holder could have written, and those are exactly what it
    // decides on.
    if (!verified && opts.requireVerified) {
      res.set('WWW-Authenticate', 'Bearer error="invalid_token"');
      log.debug("Leaving Dpop.presentedAccessToken(). Not a token this " +
                "service can verify.");
      errorCodes.mark(res, 'STS-OAUTH-0507');
      vciError(res, 401, 'invalid_token',
        'This resource accepts only access tokens issued by this ' +
        'service\'s authorization servers, and this one did not verify ' +
        'against the realm\'s signing key.');
      log.debug("Leaving Dpop.presentedAccessToken().");
      return null;
    }

    // RFC 9068 SECTION 4 — THE TYPE, THE ISSUER AND THE AUDIENCE
    // (2026-09-13), for a token this service VERIFIED, and before anything
    // else is read off it: a token that is not an access token, or not one of
    // this service's authorization servers', or not addressed here, has no
    // binding worth checking. `jwt_access_token.ts` is the one reading of all
    // three, shared with the authorization server that mints the token.
    //
    ...
    // presented at 127.0.0.1 would pass. RFC 9068 does not allow that
    // reading, and it admitted `https://api.partner.example/resource` as
    // well: somebody else's resource server, narrowed to on purpose with RFC
    // 8707. The audience is compared whole now, against the address this
    // request arrived on.
    //
    ...
    if (verified) {
      ...
      if (profileProblem) {
        res.set('WWW-Authenticate', 'Bearer error="invalid_token"');
        log.debug("Leaving Dpop.presentedAccessToken(). RFC 9068 section 4 " +
                  "refused it.");
    ...
    // RFC 8705 section 3.1 — the OTHER sender constraint, checked here for
    // the same reason the DPoP one is: this function is the single check the
    ...
    const certificateProblem: Json = mtls.checkBinding(claims, req, verified);
    if (certificateProblem) {
      res.set('WWW-Authenticate', 'Bearer error="invalid_token"');
      log.debug("Leaving Dpop.presentedAccessToken(). The certificate " +
    ...
    // A bound token presented as Bearer is a protocol error even though the
    // bytes are the same. Accepting it would throw the binding away silently,
    // which is the single most likely way to implement DPoP and gain nothing.
    if (boundTo && scheme !== 'dpop') {
      res.set('WWW-Authenticate', 'DPoP error="invalid_token", ' +
                                  'error_description="the token is ' +
                                  'DPoP-bound and must be presented with ' +
                                  'the DPoP scheme"');
      log.debug("Leaving Dpop.presentedAccessToken(). A bound token was " +
                "presented as Bearer.");
      errorCodes.mark(res, 'STS-OAUTH-0117');
      vciError(res, 401, 'invalid_token',
        'This access token is DPoP-bound (it carries cnf.jkt), so it must ' +
        'be presented as "Authorization: DPoP <token>" with a DPoP proof — ' +
        'not as a Bearer token.');
      log.debug("Leaving Dpop.presentedAccessToken().");
      return null;
    }

    // #34 (2026-09-15): THE TWO SETTINGS THAT REQUIRE A CONSTRAINT RATHER
    // THAN CHECKING THE ONE A TOKEN HAPPENS TO CARRY.
    //
    // Everything above this point refuses a token whose OWN binding does not
    // hold and passes a token that carries none — which is the right default
    // and exactly what an operator turning `oauth2.accessTokenRequireDpop` or
    // `oauth2.accessTokenRequireMtls` on is asking to change.
    //
    // HERE, so that the seven surfaces sharing this function share the
    // answer: UserInfo, the step-up resource, the three OpenID4VCI endpoints,
    // SCIM and the Shared Signals endpoints. `/admin-api` and the debugger's
    // listener ask `senderConstraints` themselves, because each verifies its
    // own token.
    //
    // A FOREIGN TOKEN IS HELD TO IT TOO. The confirmation a token carries can
    ...
    const required: Json = senderConstraints.accessTokenRefusal({
      ...
      // the same code that decides it for a token that carries its own
      // binding.
    ...
    if (required) {
      ...
      log.debug("Leaving Dpop.presentedAccessToken(). A required sender " +
                "constraint was not met (" + required.setting + ").");
    ...
    if (!boundTo && req.headers['dpop'] === undefined) {
      if (this.stepUpChallenged(res, claims, 'Bearer', opts, where)) {
        log.debug("Leaving Dpop.presentedAccessToken(). Step-up is " +
                  "required.");
        return null;
      }
    ...
    if (!checked.ok) {
      ...
      // authorization server uses. Getting this shape wrong leaves a
      // conforming wallet unable to proceed, so the two are deliberately not
      // shared.
      if (checked.needNonce) {
        res.set('DPoP-Nonce', this.issueNonce());
        res.set('WWW-Authenticate', 'DPoP error="use_dpop_nonce", ' +
                                    'error_description="Resource server ' +
                                    'requires nonce in DPoP proof"');
        log.debug("Leaving Dpop.presentedAccessToken(). Asking the wallet " +
                  "for a DPoP nonce.");
        errorCodes.mark(res, checked.errorCode || 'STS-OAUTH-0108');
        vciError(res, 401, 'use_dpop_nonce', 'Resource server requires ' +
                                             'nonce in DPoP proof');
        log.debug("Leaving Dpop.presentedAccessToken().");
        return null;
      }
      res.set('WWW-Authenticate', 'DPoP error="invalid_dpop_proof"');
      log.debug("Leaving Dpop.presentedAccessToken(). The DPoP proof was " +
                "refused.");
    ...
    if (this.stepUpChallenged(res, claims, 'DPoP', opts, where)) {
  ...
  // when the token's authentication does not meet the requirement. The scheme
  // is the one the token was presented under, so a DPoP-bound token is told
  // to come back as DPoP. Counted against the token's client_id.
  private stepUpChallenged(res: Res, claims: Json, scheme: string,
                           opts: Json, where?: string): boolean {
    const { log, stepUp, errorCodes, vciError } = this.deps;
    log.debug("Entering Dpop.stepUpChallenged().");
    const requirement = opts.stepUp || stepUp.ownResourceRequirement();
    const refusal = stepUp.tokenRefusal(requirement, claims);
    if (!refusal) {
      log.debug("Leaving Dpop.stepUpChallenged(). Met, or nothing " +
                "required.");
      return false;
    }
    ...
    stepUp.record((claims && claims.client_id) || '', 'stepup.challenged',
                  { error: refusal.error });
    log.info('RFC 9470: ' + (where || 'a protected endpoint') +
             ' challenged a token for client "' +
    ...
    log.debug("Leaving Dpop.stepUpChallenged(). Challenged.");
    return true;
  }
  ...
  provideCapability(): void {
    ...
    log.debug("Leaving Dpop.provideCapability().");
  }
}
