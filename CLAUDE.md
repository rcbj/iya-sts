...
| 18j | `risk/risk_store`, `risk_datasets`, `risk_failures`, then `admin-ui/risk_admin` | 18a's placement and 18a's reason (#62): libraries, then the page, after the scheduler (whose two risk jobs `risk_datasets` registers when wired), before `mgmt-api/admin_api`, which requires the page. **Nothing requires the three libraries earlier**: `persistence.js` and `credentials.ts` reach them lazily. | `risk/CLAUDE.md` |
