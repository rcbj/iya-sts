...
| 3bi | What the rest of the OpenID conformance suite found (#187): every other plan that applies (OpenID Connect, logout, Identity Assurance, the FAPI variants, SSF, OpenID Federation, OpenID4VCI and OpenID4VP), each finding fixed in every mode, and the warnings and plans that stay with their reasons | `oauth-oidc/CLAUDE.md` |
